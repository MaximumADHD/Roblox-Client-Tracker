...
do
	...
	local function AttemptInvokeFunction(functionName, ...)
		local func =
			FindInCollectionByKeyAndType(communicationsConnections.ChatWindow, functionName, "BindableFunction")
		if func then
	...
	local function DoConnectGetCore(connectionName)
		StarterGui:RegisterGetCore(connectionName, function(data)
			local func =
				FindInCollectionByKeyAndType(communicationsConnections.GetCore, connectionName, "BindableFunction")
			local rVal = nil
			if func then
				rVal = func:Invoke(data)
			end
			return rVal
		end)
	end
	...
	local function RegisterCoreGuiConnections(containerTable)
		if type(containerTable) == "table" then
			...
			if type(chatWindowCollection) == "table" then
				...
				communicationsConnections.ChatWindow.ToggleVisibility =
					FindInCollectionByKeyAndType(chatWindowCollection, "ToggleVisibility", "BindableEvent")
				communicationsConnections.ChatWindow.SetVisible =
					FindInCollectionByKeyAndType(chatWindowCollection, "SetVisible", "BindableEvent")
				communicationsConnections.ChatWindow.FocusChatBar =
					FindInCollectionByKeyAndType(chatWindowCollection, "FocusChatBar", "BindableEvent")
				communicationsConnections.ChatWindow.EnterWhisperState =
					FindInCollectionByKeyAndType(chatWindowCollection, "EnterWhisperState", "BindableEvent")
				communicationsConnections.ChatWindow.TopbarEnabledChanged =
					FindInCollectionByKeyAndType(chatWindowCollection, "TopbarEnabledChanged", "BindableEvent")
				communicationsConnections.ChatWindow.IsFocused =
					FindInCollectionByKeyAndType(chatWindowCollection, "IsFocused", "BindableFunction")
				communicationsConnections.ChatWindow.SpecialKeyPressed =
					FindInCollectionByKeyAndType(chatWindowCollection, "SpecialKeyPressed", "BindableEvent")

				local function DoConnect(index)
					communicationsConnections.ChatWindow[index] =
						FindInCollectionByKeyAndType(chatWindowCollection, index, "BindableEvent")
					if communicationsConnections.ChatWindow[index] then
						local con = communicationsConnections.ChatWindow[index].Event:connect(function(...)
							moduleApiTable[index]:fire(...)
						end)
				...
				communicationsConnections.ChatWindow[index] =
					FindInCollectionByKeyAndType(chatWindowCollection, index, "BindableEvent")
				if communicationsConnections.ChatWindow[index] then
					local con = communicationsConnections.ChatWindow[index].Event:connect(function(message)
			...
			if type(setCoreCollection) == "table" and type(getCoreCollection) == "table" then
				...
				communicationsConnections.GetCore.ChatWindowPosition =
					FindInCollectionByKeyAndType(getCoreCollection, "ChatWindowPosition", "BindableFunction")
				communicationsConnections.GetCore.ChatWindowSize =
					FindInCollectionByKeyAndType(getCoreCollection, "ChatWindowSize", "BindableFunction")
				communicationsConnections.GetCore.ChatBarDisabled =
					FindInCollectionByKeyAndType(getCoreCollection, "ChatBarDisabled", "BindableFunction")

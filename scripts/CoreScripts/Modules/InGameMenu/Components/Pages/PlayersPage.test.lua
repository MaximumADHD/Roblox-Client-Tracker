...
local function getMountableTreeAndStore(props)
	...
	local playersPage = Roact.createElement(PlayersPage, Cryo.Dictionary.join({ pageTitle = "People" }, props or {}))
...
describe("PlayersPage rendering", function()
	it("mounts and unmounts with rendered children", function()
		...
		jestExpect(#Players.LocalPlayer.PlayerGui:GetChildren()).toBeGreaterThan(0)
	...
	it("Renders single player (and basic PlayersPage components)", function()
		...
		local renderedPlayersPage = Players.LocalPlayer.PlayerGui:FindFirstChild("PlayersPage")

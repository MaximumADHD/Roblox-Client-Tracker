...
local function UnfollowPlayer(player)
	return function(store)
		coroutine.wrap(function()
			...
			if not success then
				warn("unfollowPlayer() failed because", result)
				return
			end
			...
			if not success then
				warn("unfollowPlayer() JSONDecode failed because", result)
				return
			end

			if result["success"] then
				store:dispatch(SetPlayerIsFollowing(player, false))
				...
				if NewFollower then
					NewFollower:FireServer(player, false)
				end
			end
		end)()
	end

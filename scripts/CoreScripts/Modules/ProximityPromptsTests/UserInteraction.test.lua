...
local RoactAct = require(CorePackages.Packages.Roact).act

local testMenuKey = "ProximityPromptTestsMenuKey"

-- Skip suite when ProximityPrompts UI is unavailable (e.g. fastFlags.allOn on Windows CI
-- where PlayerViewService/asset delivery can fail and the CoreScript never creates the UI).
local Players = game:GetService("Players")
local localPlayer = Players.LocalPlayer
local localPlayerGui = localPlayer and localPlayer:WaitForChild("PlayerGui", 8)
local proximityPromptsGui = localPlayerGui and localPlayerGui:WaitForChild("ProximityPrompts", 8)
if not game:GetEngineFeature("ProximityPrompts")
	or not localPlayer
	or not localPlayerGui
	or not proximityPromptsGui
then
...
local function getNewPromptUICreatedInNextFrame()
	...
	end, 1.0, "Couldn't find correctly sized prompt within time limit.")
...
describe("Triggered behavior", function()
	...
	it("should fire Triggered signal when clicked if .ClickablePrompt=true", function()
		...
		Rhodium.VirtualInput.Mouse.sendMouseButtonEvent(
			math.floor(promptScreenPos.x),
			math.floor(promptScreenPos.y),
			0,
			true
		) -- mouse down
		PlayerHelper.WaitNFrames(1)
	...
	it(
		"should not fire Triggered signal when clicked or keypressed when in-game menu is open (CLIPLAYEREX-4155)",
		function()
			prompt.RequiresLineOfSight = false
			prompt.Enabled = true
			prompt.ClickablePrompt = true
	...
	it(
		"should not fire Triggered signal on gamepad button pressed while gamepad menu is open (CLIPLAYEREX-4155)",
		function()
			prompt.RequiresLineOfSight = false
			prompt.ClickablePrompt = true
			prompt.HoldDuration = 0
	...
	it.skip("Should only fire triggered signal after .HoldDuration time has elapsed for click", function()
		prompt.KeyboardKeyCode = Enum.KeyCode.E
		prompt.GamepadKeyCode = Enum.KeyCode.ButtonX
		prompt.ClickablePrompt = true
	...
	it.skip("Should only fire triggered signal after .HoldDuration time has elapsed for tap", function()

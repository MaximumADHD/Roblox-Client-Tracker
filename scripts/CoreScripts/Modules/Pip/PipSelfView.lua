...
function startRenderStepped(player)
	...
	renderSteppedConnection = RunService.RenderStepped:Connect(function(step)
		...
		if Players.LocalPlayer then
			...
			local currentStreamTrackWeight = 1
			if headRef then
			...
			local trackerWeight = 1
			local trackerData = nil
			local EnableCamOrientation = ReplicatedStorage:FindFirstChild("EnableCamOrientation")
			if EnableCamOrientation and EnableCamOrientation.Value and UserInputService.AccelerometerEnabled then

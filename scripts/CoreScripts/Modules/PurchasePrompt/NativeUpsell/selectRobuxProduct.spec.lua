return function()
	...
	describe("premium products", function()
		it("should select the appropriate premium product when user IS NOT premium", function()
			...
			selectRobuxProduct(Enum.Platform.Windows, 80, false)
				:andThen(function(product)
					expect(product).to.be.ok()
					expect(product.robuxValue).to.equal(400)
					expect(product.productId).to.equal("")
				end)

			selectRobuxProduct(Enum.Platform.OSX, 80, false)
				:andThen(function(product)
					expect(product).to.be.ok()
					expect(product.robuxValue).to.equal(400)
					expect(product.productId).to.equal("")
		...
		it("should select the appropriate premium product when user IS premium", function()
			...
			selectRobuxProduct(Enum.Platform.Windows, 80, true)
				:andThen(function(product)
					expect(product).to.be.ok()
					expect(product.robuxValue).to.equal(440)
					expect(product.productId).to.equal("")
				end)

			selectRobuxProduct(Enum.Platform.OSX, 80, true)
				:andThen(function(product)
					expect(product).to.be.ok()
					expect(product.robuxValue).to.equal(440)
					expect(product.productId).to.equal("")

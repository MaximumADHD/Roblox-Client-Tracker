...
function createListInventoryItemsUrl(userId : number, maxPageSize : number?, pageToken : string?, filter : string?) : string
    ...
    local url = string.format("%s%s/cloud/v2/users/%d/inventory-items", apisUrl, getUrlPrefix(), userId)
...
function listInventoryItems(args : any)
    ...
    return OpenCloudService:HttpRequestAsync({[URL] = createListInventoryItemsUrl(args[USER].UserId, args[MAX_PAGE_SIZE], args[PAGE_TOKEN], args[FILTER])})
end

...
local function getInferredPolePosition(ik : IKControl) : Vector3
	...
	local reach : Vector3 = ik.EndEffector.Position - ik.ChainRoot.Position
	local chainLength : number = 1.884--ik:GetChainLength()
	local reachNormLength : number = reach.Magnitude / chainLength
	...
	local midpoint : Vector3 = (ik.EndEffector.Position + ik.ChainRoot.Position)/2
...
local function initAutopole(ik : IKControl)
	if ik.Pole == nil and isLimb(ik) then
		...
		local character = ik:FindFirstAncestorOfClass("Model")
		pole.Parent = character:FindFirstChild("HumanoidRootPart", true)

...
function CharacterPhysicsControllerLoader.CreateGroundControllerController(character : Instance, name : string)
...
function CharacterPhysicsControllerLoader.CreateAirControllerController(character : Instance, name : string)
	local controllerManager = character:FindFirstChild("ControllerManager")
...
function CharacterPhysicsControllerLoader.CreateSwimControllerController(character : Instance, name : string)
	local controllerManager = character:FindFirstChild("ControllerManager")
	if controllerManager == nil then
		return nil
	end
...
function CharacterPhysicsControllerLoader.CreateClimbControllerController(character : Instance, name : string)
	local controllerManager = character:FindFirstChild("ControllerManager")
	if controllerManager == nil then
		return nil
	end
...
function CharacterPhysicsControllerLoader.AddDefaultControllers(character : Instance)
	...
	if runningController == nil then
		CharacterPhysicsControllerLoader.CreateGroundControllerController(character, "RunningController")	
	end

	local gettingUpController = controllerManager:FindFirstChild("GettingUpController")
	if gettingUpController == nil then
		CharacterPhysicsControllerLoader.CreateGroundControllerController(character, "GettingUpController")
	end

	local airController = controllerManager:FindFirstChild("AirController")
	if airController == nil then
		airController = CharacterPhysicsControllerLoader.CreateAirControllerController(character, "AirController")
	end

	local swimController = controllerManager:FindFirstChild("SwimController")
	if swimController == nil then
		swimController = CharacterPhysicsControllerLoader.CreateSwimControllerController(character, "SwimController")
	end

	local climbController = controllerManager:FindFirstChild("ClimbController")
	if climbController == nil then
		climbController = CharacterPhysicsControllerLoader.CreateClimbControllerController(character, "ClimbController")
	end
end

function CharacterPhysicsControllerLoader.CreateAndAddGroundSensor(character : Instance, humanoidRootPart : Part)
	local controllerManager = character:FindFirstChild("ControllerManager")
...
function CharacterPhysicsControllerLoader.CreateAndAddClimbSensor(character : Instance, humanoidRootPart : Part)
	local controllerManager = character:FindFirstChild("ControllerManager")
	if controllerManager == nil or humanoidRootPart == nil then
		return nil
	end
...
function CharacterPhysicsControllerLoader.AddSensorsUnderRootPart(character : Model)
	...
	if groundSensor == nil then
		groundSensor = CharacterPhysicsControllerLoader.CreateAndAddGroundSensor(character, humanoidRootPart)
	end
	local climbSensor = humanoidRootPart:FindFirstChild("ClimbSensor")
	if climbSensor == nil then 
		climbSensor = CharacterPhysicsControllerLoader.CreateAndAddClimbSensor(character, humanoidRootPart)	
	end
end

...
local sensorAndControllerLoader = require(script.Parent:WaitForChild("CharacterPhysicsControllerLoader"))
...
function ServerMonitor:onServerHumanoidRemoteEventServer(player : Player, eventType : string, data : any)
	...
	if eventType == "ChangeStateSM" then
		...
		if changeStateData.newState == "Dead" then
			if self.trackedOwnershipList[changeStateData.character] == player and 
			   humanoid and humanoid.BreakJointsOnDeath then
			...
			if transferRecord.oldOwner == contextXfer.oldOwner then
				...
				if  transferRecord.newOwner == nil then
					...
					if humanoid ~= nil and humanoid.Parent ~= nil then
						local localHumanoidStateMachineEventInstance  : Instance = game.ServerScriptService:WaitForChild("CharacterStateMachineMonitor"):WaitForChild("LocalHumanoidStateMachineEvent")
						local localHumanoidStateMachineEvent : BindableEvent  = nil
						...
						local record : commonTypes.RunningStateMachineRecord? = commonFuncs.ContinueStateMachine(
							contextXfer.character, humanoid, self.ccsmDefinition , self.localEvent, contextXfer.currentState, contextXfer.context)
					...
					if oldOwner == nil then
						...
						if oldOwner:IsA("Player") then
							...
							elseif currentOwnerInstance:IsA("Player") then
								if debug then
									print("Owner changed from", oldOwner.Name, "to ", currentOwnerInstance.Name)
								end
								self:requestOwnershipTransfer(characterModel, currentOwnerInstance, oldOwner)
							end
						else
...
local function onCharacterAdded(character : Model)
	...
	humanoid:GetPropertyChangedSignal("HipHeight"):Connect(function(dt)
		if sensorAndControllerLoader ~= nil then
			local runningController = sensorAndControllerLoader.GetControllerByName(character, "RunningController")
			if runningController ~= nil then
				runningController.GroundOffset = humanoid.HipHeight 
			end 
			local gettingUpController = sensorAndControllerLoader.GetControllerByName(character, "GettingUpController")
...
function ServerMonitor.new(event : BindableEvent, ccsmDef : any)
	...
	playersService.PlayerAdded:Connect(function(player) self:onPlayerAdded(player) end)

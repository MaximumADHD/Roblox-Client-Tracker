...
function Controller:Update(data)
    ...
    local castDir = if inputDir.Magnitude > 0 then inputDir else hrpCF.LookVector
    ...
    if isCrouching then
        ...
        if not (#upHit > 0 or self.input.crouchInput) then
            ...
            if headForwardHit then
                local wallHitEstimate = headForwardHit.Position
    ...
    -- Scan for collision from upper body.
    -- Casting in a cone both mimics crouching trajectory and suppresses false positives when turning away from walls.
    local frontUpperHit = Sensor:RaycastScan({
        origin = headBottom,
        dir = castDir * forwardCastDistance,
        radius = upperRadius * 0.8,
        rayCount = 24,
    ...
    local frontLowerHit = Sensor:RaycastScan({
        origin = Vector3.new(headBottom.X, bottomPosition.Y + crouchHeight / 2, headBottom.Z),

...
local ModuleManager = require(CharacterController.ModuleManager)
...
ModuleManager.forEachModule(function (module)
    local controller = module.getController()
...
game:GetService("RunService").Stepped:Connect(function(gameTime, dt)
    ...
    local input: Vector3 = humanoid.MoveDirection
    ...
    -- This is the actual physical velocity
...
game:GetService("RunService").PreSimulation:Connect(function(deltaTimeSim)
    if not ENABLE_ROOT_MOTION then
        return
    end

    local cm: ControllerManager = character.ControllerManager
    ...
    if animator.RootMotionWeight > 0.0 then
        ...
        if rootFrame then
            ...
            cm.MovingDirection = worldVelocity
            ac.MoveSpeedFactor = worldVelocity.Magnitude / cm.BaseMoveSpeed 
        end
    else
        ac.MoveSpeedFactor = 0
        cm.FacingDirection = hrp.CFrame.LookVector
    end
end)

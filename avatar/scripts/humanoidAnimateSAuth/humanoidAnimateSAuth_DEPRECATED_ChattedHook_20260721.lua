...
function module.setupAnimation(character)
	...
	-- Guard on the player: NPCs have no Player (player is nil), so indexing .Chatted would error.
	-- Skipping the hook lets the script run for NPCs out of the box.
	if not FFlagUserAnimateRemoveEmoteChatHook and player then

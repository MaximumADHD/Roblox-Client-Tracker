...
-- Delay before playing the mood animation after setup. This is needed to ensure it comes in
-- after all the prediction statuses on the Animator are set up correctly.
local MOOD_START_DELAY_TIME = 1.0
...
function module.setupAnimation(character)
	...
	-- Mood animation ID, set during mood animation setup below. Used by stop functions to preserve the mood track.
	local moodAnimationId = nil
	...
	local function stopToolAnimations(toolAnimIds, transitionTime, destroyTracks, animTracks)
		...
		for _, track in ipairs(animTracks or animator:GetPlayingAnimationTracks()) do
			...
			if moodAnimationId and track.Animation.AnimationId == moodAnimationId then
				continue
			end
	...
	-- Mood animation setup: find the first Animation under script.mood, preload it, and play it after a delay.
	local moodsEnabledValue = animateParent:FindFirstChild("MoodsEnabled")
	local isMoodAnimationEnabled = moodsEnabledValue and moodsEnabledValue.Value == true
	if isMoodAnimationEnabled then
		local moodFolder = animateParent:FindFirstChild("mood")
		if moodFolder then
			for _, child in ipairs(moodFolder:GetChildren()) do
				if child:IsA("Animation") then
					moodAnimationId = child.AnimationId
					-- Preload the mood animation like all other animations
					if preloadedAnims[moodAnimationId] == nil then
						animator:LoadAnimation(child)
						preloadedAnims[moodAnimationId] = true
					end
					task.delay(MOOD_START_DELAY_TIME, function()
						local moodTrack = animator:LoadAnimation(child)
						moodTrack.Looped = true
						moodTrack.Priority = Enum.AnimationPriority.Idle
						moodTrack:Play()
					end)
					break
				end

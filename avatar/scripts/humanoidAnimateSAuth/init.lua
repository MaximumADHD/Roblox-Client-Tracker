...
local danceOptionsR6 = {"dance1", "dance2", "dance3"}
...
local function configureAnimationSet(name, fileList, skipCheckForOverrides)
	...
	if not skipCheckForOverrides then
...
for name, fileList in pairs(DEFAULT_ANIMS) do
	local skipCheckForChildCustomAnimations = isR6
	configureAnimationSet(name, fileList, skipCheckForChildCustomAnimations)
end	
...
local function transitionToQueuedAnimation(animState)
	local queuedAnimIsEmote = (string.sub(animState.queuedPose, 1, 6) == "EMOTE_")
	local newAnimName = ""
	if queuedAnimIsEmote then
		newAnimName = string.sub(animState.queuedPose, 7)
		if isR6 and newAnimName == "dance" then
			-- For R6, dance randomly maps to dance1, dance2, or dance3
			local rng = getFrameRng()
			local roll = rng:NextInteger(1, #danceOptionsR6)
			newAnimName = danceOptionsR6[roll]
		end
	else

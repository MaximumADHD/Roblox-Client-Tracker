...
function Locomotion:CalculateMovement(context)
	...
	if humanoid.AutoRotate then
		if movementInput then
			context.controllerManager.FacingDirection = context.controllerManager.MovingDirection.Unit
		else
...
function Locomotion:AccountForMaxSlope(context)
	if not context.controllerManager or not context.controllerManager.ActiveController or context.controllerManager.ActiveController.ClassName ~= "GroundController" or not context.controllerManager.GroundSensor or not context.controllerManager.GroundSensor.SensedPart then
		return
	end
...
function Locomotion:UpdateFloorDistance(context)
	...
	local groundOffset = context.controllerManager.RunningController.GroundOffset

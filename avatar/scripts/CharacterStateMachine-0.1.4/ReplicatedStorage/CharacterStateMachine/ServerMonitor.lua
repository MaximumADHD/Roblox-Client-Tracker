...
function ServerMonitor:onServerHumanoidRemoteEventServer(player : Player, eventType : string, data : any)
	...
	if eventType == "ChangeStateSM" then
		...
		if changeStateData.newState == "Dead" then
			if self.humanoidOwnershipList[player] and self.humanoidOwnershipList[player]["humanoidInstance"] and self.humanoidOwnershipList[player]["humanoidInstance"].BreakJointsOnDeath then
				player.Character:BreakJoints()
			end
			self.trackedOwnershipList[player.Character] = nil
		end
	elseif  eventType == "ChangeCollisionSettings" then	
		...
		if settingsData ~= nil then
			for k,v in pairs(settingsData.parts) do
				local part : Part? = player.Character:FindFirstChild(k)
				if part ~= nil then
					part.CanCollide = v
				end
			end
	...
	elseif eventType == "ContextDataHumanoidSM" then
		...
		if (contextXfer.ccsmDef ~= self.ccsmDefinition) then
			warn("Error !! CCSM def from client doesnot match")
		end
...
local function onCharacterAdded(character : Model)
	...
	humanoid:GetPropertyChangedSignal("HipHeight"):Connect(function(dt)
		if sensorAndControllerLoader ~= nil then
			local runningController = sensorAndControllerLoader.GetControllerByName(humanoid, "RunningController")
			if runningController ~= nil then
				runningController.GroundOffset = humanoid.HipHeight 
			end 
			local gettingUpController = sensorAndControllerLoader.GetControllerByName(humanoid, "GettingUpController")

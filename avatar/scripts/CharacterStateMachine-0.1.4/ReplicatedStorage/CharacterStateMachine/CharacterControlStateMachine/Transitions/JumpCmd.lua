...
local sensorAndControllerLoader = require(script.Parent.Parent.Parent:WaitForChild("CharacterPhysicsControllerLoader"))
...
function JumpCmd:Test(stateMachine)
	...
	local groundSensor : ControllerPartSensor = stateMachine.context.humanoid.Parent.HumanoidRootPart.GroundSensor
	local tilt = groundSensor.HitNormal:Dot(Vector3.new(0,1,0))
	if ( tilt > bodyParts.SteepSlopeAngle(humanoid) or currentState == "Running") and humanoid.Jump and sensorAndControllerLoader.CalculateJumpPower(humanoid) > 0 then
		return true
	end
	...
	if stateMachine.context.noFloorTimer > noFloorJumpThreshold and ( currentState == "PlatformStanding" or  currentState == "Running") then
		return false
	end

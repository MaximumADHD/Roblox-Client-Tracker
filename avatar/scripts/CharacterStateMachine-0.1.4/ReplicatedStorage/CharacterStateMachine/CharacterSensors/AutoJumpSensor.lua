...
function AutoJumpSensor:OnStepped(ccsm:any, humanoid : Humanoid, dt : number)
	...
	local torsoRayResult:RaycastResult = game.Workspace:Raycast(torsoRayPos, rayVector, raycastParams)
	...
	if not game.PhysicsService:CollisionGroupsAreCollidable(torsoRayResult.Instance.CollisionGroup, humanoidRootPart.CollisionGroup) then
		return
	end

	local jumpRayResult:RaycastResult = game.Workspace:Raycast(jumpRayPos, rayVector, raycastParams)

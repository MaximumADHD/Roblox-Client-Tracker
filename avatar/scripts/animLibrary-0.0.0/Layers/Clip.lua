...
function ClipLayer:getDuration()
    ...
    if self.props.speed == 0 then
        return self.state.__evaluator.Duration
    end

    return math.abs(self.state.__evaluator.Duration / self.props.speed)
end

function ClipLayer:update(deltaTime: number)
    self:__updateLoadedAnimation()
    ...
    if 0 > self.state.timePosition or self.state.timePosition > evaluator.Duration then
        if self.state.clipIsLooping then
            self.state.timePosition %= evaluator.Duration
        else
            self.state.timePosition = math.clamp(self.state.timePosition, 0, evaluator.Duration)
        end
        self.hasLooped = true
    else
        self.hasLooped = false
...
function ClipLayer:updateToPhase(phase: number)
    self:__updateLoadedAnimation()
    ...
    -- Switch phase around if clip speed is negative.
    phase = if self.props.speed > 0 then phase else 1 - phase
    local prevTimePosition = self.state.timePosition
    self.state.timePosition = phase
        * self.state.__evaluator.Duration
        % self.state.__evaluator.Duration
...
function ClipLayer:evaluate(mask: AnimationMask)
    ...
    assert(self.state.__evaluator ~= nil)
    return self.state.__evaluator:Evaluate(mask, self.state.timePosition)
end

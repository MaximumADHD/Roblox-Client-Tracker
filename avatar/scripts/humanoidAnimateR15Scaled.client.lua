	...
	if (allowCustomAnimations and config ~= nil) then
		...
		for _, childPart in pairs(config:GetChildren()) do
			if (childPart:IsA("Animation")) then
				...
				local existingIdx = findExistingAnimationInSet(animTable[name], childPart)
				if existingIdx == 0 then
					animTable[name].count = animTable[name].count + 1
					idx = animTable[name].count
					animTable[name][idx] = {}
					animTable[name][idx].anim = childPart
					animTable[name][idx].loaded = false					
				elseif animTable[name][existingIdx].weight ~= newWeight then									
					animTable[name].totalWeight = animTable[name].totalWeight - animTable[name][existingIdx].weight
					idx = existingIdx
				end
	...
	if PreloadAnimsUserFlag then
		for i, animType in pairs(animTable) do
			for idx = 1, animType.count, 1 do
				if not animType[idx].loaded then
					Humanoid:LoadAnimation(animType[idx].anim)
					animType[idx].loaded = true
				end				
			end
		end
	end

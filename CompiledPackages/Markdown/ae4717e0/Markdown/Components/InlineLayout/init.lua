...
local HardBreakElement = require(Root.Components.InlineLayout.HardBreakElement)
local HoverContext = require(Root.Components.InlineLayout.InlineLayoutElements.HoverContext)
local View = Foundation.View

local FFlagDebugMarkdown = require(Root.SharedFlags).get("FFlagDebugMarkdown")
...
local function layoutElements(elements: { InlineElement }, width: number): { ReactNode }
	...
	if breakPoint == MAX_ITERATIONS and FFlagDebugMarkdown then
		warn(
	...
	return map(rows, function(row, index)
		return React.createElement(View, {
			tag = "auto-xy row align-x-center",
			key = tostring(index),
			LayoutOrder = index,
		}, row)
	end)

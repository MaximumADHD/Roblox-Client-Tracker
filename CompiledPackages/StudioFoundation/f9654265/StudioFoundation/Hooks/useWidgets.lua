...
local safeGetFastFlag = require(Main.Util.safeGetFastFlag)
local FFlagEnableInstanceGetStyledAPI = safeGetFastFlag("EnableInstanceGetStyledAPI")
...
function widgets:flush()
	...
	for _, entry in list do
		...
		entry.Visible = if FFlagEnableInstanceGetStyledAPI then entry.Widget:GetStyled("Visible") else entry.Widget
		...
		self.signals[uriString] = {
			...
			VisibleChanged = (if FFlagEnableInstanceGetStyledAPI
				then entry.Widget:GetStyledPropertyChangedSignal("Visible")
				else entry.Widget:GetPropertyChangedSignal("Visible")):Connect(function()
				self:register(entry.Uri, entry.Widget)
			end),

...
local itIfDeviceSimFixes: any = if FFlagStudioFoundationDialogDeviceSimFixes then it else it.skip
local itIfNoDeviceSimFixes: any = if FFlagStudioFoundationDialogDeviceSimFixes then it.skip else it
...
describe("DialogView full bleed content", function()
	local function createFullBleedElement(isContentFullBleed: boolean?)
		return createTestElement({
			type = "Default",
			title = "Title",
			isContentFullBleed = isContentFullBleed,
			primaryAction = {
				uri = StudioUri.fromWidget("Dialog", "Primary"),
				text = "Primary",
	...
	local function isPadded(rendered): boolean
		local dialog = rendered.getByTestId("--dialog") :: GuiObject
		local children = rendered.getByTestId("--dialog-children") :: GuiObject
		return dialog:HasTag("padding-x-large") or children:HasTag("padding-right-small")
	end

	itIfDeviceSimFixes("should let full bleed content run to the edge of the dialog", function()
		local rendered = render(createFullBleedElement(false))
		expect(isPadded(rendered)).toBe(true)

		rendered.rerender(createFullBleedElement(true))
		expect(isPadded(rendered)).toBe(false)
	end)

	itIfDeviceSimFixes("should separate and pad the actions of a full bleed dialog", function()
		local rendered = render(createFullBleedElement(true))

		local dialog = rendered.getByTestId("--dialog")
		local divider = rendered.getByTestId("--foundation-divider")
		local buttonGroup = rendered.getByTestId("--dialog-button-group")

		-- The divider and the padded button group share a container directly under the dialog
		local paddingContainer = buttonGroup.Parent :: GuiObject
		local actionsContainer = paddingContainer.Parent :: GuiObject
		expect(divider.Parent).toBe(actionsContainer)
		expect(actionsContainer.Parent).toBe(dialog)
	end)

	itIfDeviceSimFixes("should keep the actions undecorated when content isn't full bleed", function()
		local rendered = render(createFullBleedElement(false))

		expect(rendered.queryByTestId("--foundation-divider")).toBeNil()
		expect(rendered.getByTestId("--dialog-button-group").Parent).toBe(rendered.getByTestId("--dialog"))
	end)

	itIfNoDeviceSimFixes("should ignore isContentFullBleed when the flag is off", function()
		local rendered = render(createFullBleedElement(true))

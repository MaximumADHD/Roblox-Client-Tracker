...
describe("Dialog", function()
	...
	itIfThemeContext("should pass the presentation overrides to DialogPanel", function()
		...
		render(createTestElement({
			...
			toolDialog = true,
			escapeAction = {
				uri = StudioUri.fromWidget("Test", "Escape"),
				onClose = onClose,
			},
		}))

		local props = getDialogPanelProps()
		expect(props.modal).toBe(false)
		expect(props.toolDialog).toBe(true)
		expect(props.onClose).toBe(onClose)
	end)

	itIfThemeContext("should leave the presentation overrides undefined when they aren't provided", function()
		render(createTestElement({
			uri = dialogUri,
			type = "Default",
		}))

		-- DialogPanel, not Dialog, is responsible for defaulting modal to true
		local props = getDialogPanelProps()
		expect(props.modal).toBeNil()
		expect(props.toolDialog).toBeNil()
		expect(props.onClose).toBeNil()
	end)

	itIfThemeContext("should only pass the device simulator overrides to DialogView when the flag is on", function()
		render(createTestElement({
			uri = dialogUri,
			type = "Default",
			isContentFullBleed = true,
			tertiaryActionVariant = "Utility",
		}))

		local props = getDialogViewProps()
		if FFlagStudioFoundationDialogDeviceSimFixes then
			expect(props.isContentFullBleed).toBe(true)
			expect(props.tertiaryActionVariant).toBe("Utility")
		else
			expect(props.isContentFullBleed).toBeNil()
			expect(props.tertiaryActionVariant).toBeNil()
		end
	end)
end)

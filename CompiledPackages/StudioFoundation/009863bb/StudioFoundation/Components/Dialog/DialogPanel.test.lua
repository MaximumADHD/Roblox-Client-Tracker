...
local FFlagDevFrameworkUsesFoundationDialog = require(Main.SharedFlags.getFFlagDevFrameworkUsesFoundationDialog)()
...
-- MockPlugin discards the window options it is given, so spy on the call to inspect them
local function createMockPluginWithSpy()
	local mockPlugin = MockPlugin.new()
	local spy = jest.fn(function(...)
		return MockPlugin.CreateQWidgetPluginGui(...)
	end)
	mockPlugin.CreateQWidgetPluginGui = spy
	return mockPlugin, spy
end

local function getWindowOptions(spy): any
	expect(spy).toHaveBeenCalled()
	local calls = spy.mock.calls
	-- First two arguments are self and the dialog id
...
describe("DialogPanel window options", function()
	itIfThemeContext("should create a modal dialog without a title bar by default", function()
		local mockPlugin, spy = createMockPluginWithSpy()
		local getSize, _ = Signals.createSignal(Vector2.new(400, 300))
		...
		local options = getWindowOptions(spy)
		expect(options.Modal).toBe(true)
		expect(options.ToolDialog).toBeNil()
		-- Without an onClose there is no way to close the dialog from the title bar
		expect(options.DisableTitleBar).toBe(true)
	end)

	itIfThemeContext("should show the title bar when the dialog can be closed", function()
		local mockPlugin, spy = createMockPluginWithSpy()
		local getSize, _ = Signals.createSignal(Vector2.new(400, 300))
		...
		expect(getWindowOptions(spy).DisableTitleBar).toBe(false)
	end)

	itIfThemeContext("should honor modal and toolDialog", function()
		local mockPlugin, spy = createMockPluginWithSpy()
		local getSize, _ = Signals.createSignal(Vector2.new(400, 300))
		...
		local options = getWindowOptions(spy)
		if FFlagDevFrameworkUsesFoundationDialog then
			expect(options.Modal).toBe(false)
			expect(options.ToolDialog).toBe(true)
		else
			expect(options.Modal).toBe(true)
			expect(options.ToolDialog).toBeNil()
		end

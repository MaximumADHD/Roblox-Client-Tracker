...
local spyFoundationButton
jest.mock(Packages.Foundation, function()
	local actual = table.clone(jest.requireActual(Packages.Foundation))
	actual.Button, spyFoundationButton = createRenderSpy(actual.Button)
	return actual
end)
...
describe("DialogButtonGroup button variants", function()
	local noop = function() end

	local function makeAction(name: string): Types.DialogAction
		return { uri = StudioUri.fromWidget("Test", name), text = name, onActivated = noop }
	end

	local function renderButtons(tertiaryActionVariant: ("Standard" | "Utility")?)
		render(createTestElement({
			primaryAction = makeAction("Primary"),
			secondaryAction = makeAction("Secondary"),
			tertiaryAction = makeAction("Tertiary"),
			tertiaryActionVariant = tertiaryActionVariant,
		}))
	end

	local function getVariant(text: string): string?
		for _, call in spyFoundationButton.mock.calls do
			local props = call[1]
			if props.text == text then
	...
	beforeEach(function()
		mockButtonWidths = nil
		spyFoundationButton.mockClear()
	end)

	it("should render the primary and secondary buttons with fixed variants", function()
		renderButtons()

		expect(getVariant("Primary")).toBe("Emphasis")
		expect(getVariant("Secondary")).toBe("Standard")
	end)

	it("should default the tertiary button to the standard variant", function()
		renderButtons()

		expect(getVariant("Tertiary")).toBe("Standard")
	end)

	itIfDeviceSimFixes("should render the tertiary button with the requested variant", function()
		renderButtons("Utility")

		expect(getVariant("Tertiary")).toBe("Utility")
	end)

	itIfNoDeviceSimFixes("should ignore tertiaryActionVariant when the flag is off", function()
		renderButtons("Utility")

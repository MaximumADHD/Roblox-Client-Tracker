...
local RoundingBehaviour = Framework.Util.RoundingBehaviour
local localeInfos: any = {}
...
export type LocalizationContext = typeof(Localization)
...
local function Provider(props: Props)
	-- we setup localization to be updateable dynamically, but this doesn't actually happen in practice right now so this code is currently untested and should never get triggered
	-- but maybe that will change in the future.

	-- Localization is a more long living object than a value. Localization is updated only on props updates, which are rare.
	local localization, setLocalization = React.useState(props.localization)

	-- this is not very useful, you need to set up the effect to depend on all the objects inside the localization object for this to acutally do anything.
	React.useEffect(function()
		setLocalization(props.localization)
	end, {})

	-- Value is the value consumed by the components, it should change both when Localization is recreated and when it's updated internally,
	-- e.g., locale has changed.
	local value, setValue = React.useState(localization)

	-- Subscribe to Localization updates
	React.useEffect(function()
		-- Update the value instantly when the new Localization is created (assuming the signal doesn't fire on the first connection)
		setValue(localization:getConsumerItem())

		local connection = localization:getSignal():Connect(function(_)
			-- Update the value if something has changed inside localization with a new copy and trigger rerender of all the components using it
			setValue(localization:getConsumerItem())
		end)
...
local function MockProvider(props: {
	children: React.ReactNode,
})
	return React.createElement(Context.Provider, {
		value = Localization.mock(props),
	}, props.children)
end

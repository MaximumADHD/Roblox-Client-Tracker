PROTO_0:
        0 GETUPVAL                         R3 0
        1 GETTABLE                         R2 R3 R0
        2 JUMPIF                           R2 ; [+1]
        3 RETURN                           R0 0
        4 GETUPVAL                         R2 0
        5 LOADNIL                          R3
        6 SETTABLE                         R3 R2 R0
        7 JUMPIF                           R1 ; [+3]
        8 GETUPVAL                         R2 1
        9 LOADB                            R3 1
       10 SETTABLE                         R3 R2 R0
       11 GETUPVAL                         R2 2
       12 SUBK                             R2 R2 K0 [1]
       13 SETUPVAL                         R2 2
       14 RETURN                           R0 0

PROTO_1:
        0 GETUPVAL                         R0 0
        1 JUMPIFNOT                        R0 ; [+6]
        2 GETUPVAL                         R0 0
        3 NAMECALL                         R0 R0 K0 ["Disconnect"]
        5 CALL                             R0 1 0
        6 LOADNIL                          R0
        7 SETUPVAL                         R0 0
        8 RETURN                           R0 0

PROTO_2:
        0 GETIMPORT                        R1 K1 [pcall]
        2 GETUPVAL                         R2 0
        3 GETTABLEKS                       R2 R2 K2 ["JSONDecode"]
        5 GETUPVAL                         R3 0
        6 MOVE                             R4 R0
        7 CALL                             R1 3 2
        8 JUMPIFNOT                        R1 ; [+11]
        9 FASTCALL1                        TYPE R2 ; [+3]
       10 MOVE                             R4 R2
       11 GETIMPORT                        R3 K4 [type]
       13 CALL                             R3 1 1
       14 JUMPIFNOTEQKS                    R3 K5 ["table"] ; [+5]
       16 GETTABLEKS                       R3 R2 K6 ["IsComplete"]
       18 JUMPIFEQKB                       R3 TRUE ; [+2]
       20 RETURN                           R0 0
       21 GETTABLEKS                       R3 R2 K7 ["AssetId"]
       23 FASTCALL1                        TYPEOF R3 ; [+3]
       24 MOVE                             R5 R3
       25 GETIMPORT                        R4 K9 [typeof]
       27 CALL                             R4 1 1
       28 JUMPIFEQKS                       R4 K10 ["number"] ; [+2]
       30 RETURN                           R0 0
       31 GETUPVAL                         R5 1
       32 GETTABLE                         R4 R5 R3
       33 JUMPIF                           R4 ; [+1]
       34 RETURN                           R0 0
       35 GETUPVAL                         R6 1
       36 GETTABLE                         R5 R6 R3
       37 JUMPIF                           R5 ; [+1]
       38 JUMP                             ; [+6]
       39 GETUPVAL                         R5 1
       40 LOADNIL                          R6
       41 SETTABLE                         R6 R5 R3
       42 GETUPVAL                         R5 2
       43 SUBK                             R5 R5 K11 [1]
       44 SETUPVAL                         R5 2
       45 GETUPVAL                         R5 2
       46 JUMPIFNOTEQKN                    R5 K12 [0] ; [+4]
       48 GETUPVAL                         R5 3
       49 CALL                             R5 0 0
       50 RETURN                           R0 0
       51 RETURN                           R0 0

PROTO_3:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["ShareAccessToAssetsAsync"]
        4 CALL                             R0 2 -1
        5 RETURN                           R0 -1

PROTO_4:
        0 MOVE                             R3 R2
        1 GETUPVAL                         R4 0
        2 CALL                             R3 1 0
        3 GETUPVAL                         R3 2
        4 GETTABLEKS                       R3 R3 K0 ["DependencyGrantProgressReceived"]
        6 NEWCLOSURE                       R5 P0
        7 CAPTURE                          UPVAL U3
        8 CAPTURE                          UPVAL U4
        9 CAPTURE                          UPVAL U5
       10 CAPTURE                          VAL R0
       11 NAMECALL                         R3 R3 K1 ["Connect"]
       13 CALL                             R3 2 1
       14 SETUPVAL                         R3 1
       15 GETIMPORT                        R3 K3 [pcall]
       17 NEWCLOSURE                       R4 P1
       18 CAPTURE                          UPVAL U2
       19 CAPTURE                          UPVAL U6
       20 CALL                             R3 1 2
       21 JUMPIF                           R3 ; [+11]
       22 GETUPVAL                         R5 1
       23 JUMPIFNOT                        R5 ; [+6]
       24 GETUPVAL                         R5 1
       25 NAMECALL                         R5 R5 K4 ["Disconnect"]
       27 CALL                             R5 1 0
       28 LOADNIL                          R5
       29 SETUPVAL                         R5 1
       30 MOVE                             R5 R0
       31 CALL                             R5 0 0
       32 RETURN                           R0 0
       33 LENGTH                           R5 R4
       34 JUMPIFEQKN                       R5 K5 [0] ; [+32]
       36 GETIMPORT                        R5 K7 [warn]
       38 LOADK                            R6 K8 ["Failed to grant universe access to assetIds: %*"]
       39 GETIMPORT                        R8 K11 [table.concat]
       41 MOVE                             R9 R4
       42 LOADK                            R10 K12 [", "]
       43 CALL                             R8 2 1
       44 NAMECALL                         R6 R6 K13 ["format"]
       46 CALL                             R6 2 1
       47 CALL                             R5 1 0
       48 MOVE                             R5 R4
       49 LOADNIL                          R6
       50 LOADNIL                          R7
       51 FORGPREP                         R5
       52 GETUPVAL                         R11 4
       53 GETTABLE                         R10 R11 R9
       54 JUMPIF                           R10 ; [+1]
       55 JUMP                             ; [+9]
       56 GETUPVAL                         R10 4
       57 LOADNIL                          R11
       58 SETTABLE                         R11 R10 R9
       59 GETUPVAL                         R10 7
       60 LOADB                            R11 1
       61 SETTABLE                         R11 R10 R9
       62 GETUPVAL                         R10 5
       63 SUBK                             R10 R10 K14 [1]
       64 SETUPVAL                         R10 5
       65 FORGLOOP                         R5 2 ; [-14]
       67 GETUPVAL                         R5 6
       68 LOADNIL                          R6
       69 LOADNIL                          R7
       70 FORGPREP                         R5
       71 GETUPVAL                         R11 8
       72 GETTABLE                         R10 R11 R8
       73 JUMPIFNOT                        R10 ; [+14]
       74 GETIMPORT                        R11 K18 [Enum.AssetType.Model]
       76 JUMPIFEQ                         R10 R11 ; [+11]
       78 GETUPVAL                         R12 4
       79 GETTABLE                         R11 R12 R9
       80 JUMPIF                           R11 ; [+1]
       81 JUMP                             ; [+6]
       82 GETUPVAL                         R11 4
       83 LOADNIL                          R12
       84 SETTABLE                         R12 R11 R9
       85 GETUPVAL                         R11 5
       86 SUBK                             R11 R11 K14 [1]
       87 SETUPVAL                         R11 5
       88 FORGLOOP                         R5 2 ; [-18]
       90 GETUPVAL                         R5 5
       91 JUMPIFNOTEQKN                    R5 K5 [0] ; [+11]
       93 GETUPVAL                         R5 1
       94 JUMPIFNOT                        R5 ; [+6]
       95 GETUPVAL                         R5 1
       96 NAMECALL                         R5 R5 K4 ["Disconnect"]
       98 CALL                             R5 1 0
       99 LOADNIL                          R5
      100 SETUPVAL                         R5 1
      101 MOVE                             R5 R0
      102 CALL                             R5 0 0
      103 RETURN                           R0 0

PROTO_5:
        0 GETUPVAL                         R1 0
        1 GETTABLEKS                       R1 R1 K0 ["Error"]
        3 JUMPIFNOT                        R1 ; [+30]
        4 GETUPVAL                         R1 0
        5 GETTABLEKS                       R1 R1 K0 ["Error"]
        7 GETTABLEKS                       R1 R1 K1 ["isKind"]
        9 JUMPIFNOT                        R1 ; [+24]
       10 GETUPVAL                         R1 0
       11 GETTABLEKS                       R1 R1 K0 ["Error"]
       13 GETTABLEKS                       R1 R1 K1 ["isKind"]
       15 MOVE                             R2 R0
       16 GETUPVAL                         R3 0
       17 GETTABLEKS                       R3 R3 K0 ["Error"]
       19 GETTABLEKS                       R3 R3 K2 ["Kind"]
       21 GETTABLEKS                       R3 R3 K3 ["TimedOut"]
       23 CALL                             R1 2 1
       24 JUMPIFNOT                        R1 ; [+9]
       25 GETIMPORT                        R1 K5 [warn]
       27 LOADK                            R2 K6 ["Dependency grants timed out after %* seconds; continuing with insertion."]
       28 GETUPVAL                         R4 1
       29 NAMECALL                         R2 R2 K7 ["format"]
       31 CALL                             R2 2 1
       32 CALL                             R1 1 0
       33 RETURN                           R0 0
       34 RETURN                           R0 0

PROTO_6:
        0 GETUPVAL                         R0 0
        1 JUMPIFNOT                        R0 ; [+6]
        2 GETUPVAL                         R0 0
        3 NAMECALL                         R0 R0 K0 ["Disconnect"]
        5 CALL                             R0 1 0
        6 LOADNIL                          R0
        7 SETUPVAL                         R0 0
        8 RETURN                           R0 0

PROTO_7:
        0 GETUPVAL                         R3 0
        1 GETTABLEKS                       R3 R3 K0 ["collectSet"]
        3 MOVE                             R4 R0
        4 CALL                             R3 1 1
        5 GETUPVAL                         R4 0
        6 GETTABLEKS                       R4 R4 K1 ["count"]
        8 MOVE                             R5 R3
        9 CALL                             R4 1 1
       10 GETUPVAL                         R5 1
       11 CALL                             R5 0 1
       12 NEWTABLE                         R6 0 0
       14 NEWCLOSURE                       R7 P0
       15 CAPTURE                          VAL R3
       16 CAPTURE                          VAL R6
       17 CAPTURE                          REF R4
       18 LOADNIL                          R8
       19 NEWCLOSURE                       R9 P1
       20 CAPTURE                          REF R8
       21 GETUPVAL                         R10 2
       22 GETTABLEKS                       R10 R10 K2 ["new"]
       24 NEWCLOSURE                       R11 P2
       25 CAPTURE                          VAL R9
       26 CAPTURE                          REF R8
       27 CAPTURE                          VAL R2
       28 CAPTURE                          UPVAL U3
       29 CAPTURE                          VAL R3
       30 CAPTURE                          REF R4
       31 CAPTURE                          VAL R0
       32 CAPTURE                          VAL R6
       33 CAPTURE                          VAL R1
       34 CALL                             R10 1 1
       35 MOVE                             R13 R5
       36 NAMECALL                         R11 R10 K3 ["timeout"]
       38 CALL                             R11 2 1
       39 NEWCLOSURE                       R13 P3
       40 CAPTURE                          UPVAL U2
       41 CAPTURE                          VAL R5
       42 NAMECALL                         R11 R11 K4 ["catch"]
       44 CALL                             R11 2 1
       45 NEWCLOSURE                       R13 P4
       46 CAPTURE                          REF R8
       47 NAMECALL                         R11 R11 K5 ["finally"]
       49 CALL                             R11 2 1
       50 NAMECALL                         R11 R11 K6 ["await"]
       52 CALL                             R11 1 0
       53 CLOSEUPVALS                      R4
       54 RETURN                           R6 1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [game]
        3 LOADK                            R2 K2 ["HttpService"]
        4 NAMECALL                         R0 R0 K3 ["GetService"]
        6 CALL                             R0 2 1
        7 GETIMPORT                        R1 K5 [script]
        9 GETTABLEKS                       R1 R1 K6 ["Parent"]
       11 GETIMPORT                        R2 K8 [require]
       13 GETTABLEKS                       R3 R1 K6 ["Parent"]
       15 GETTABLEKS                       R3 R3 K9 ["Dash"]
       17 CALL                             R2 1 1
       18 GETIMPORT                        R3 K8 [require]
       20 GETTABLEKS                       R4 R1 K6 ["Parent"]
       22 GETTABLEKS                       R4 R4 K10 ["Promise"]
       24 CALL                             R3 1 1
       25 GETIMPORT                        R4 K8 [require]
       27 GETTABLEKS                       R5 R1 K11 ["Flags"]
       29 GETTABLEKS                       R5 R5 K12 ["getFIntAmrAssetDependencyGrantEventTimeout"]
       31 CALL                             R4 1 1
       32 DUPCLOSURE                       R5 K13 [PROTO_7]
       33 CAPTURE                          VAL R2
       34 CAPTURE                          VAL R4
       35 CAPTURE                          VAL R3
       36 CAPTURE                          VAL R0
       37 RETURN                           R5 1

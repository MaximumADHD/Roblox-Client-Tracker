...
local function MaterialGridItem(props: Props, ref)
	...
	local labelAreaElement = React.useMemo(function()
		...
		return React.createElement("Frame", {
			...
			Scrim = React.createElement("Frame", {
				BackgroundColor3 = LABEL_SCRIM_SURFACE_0,
				...
				Shade = React.createElement("UIGradient", {
					Color = ColorSequence.new(LABEL_SCRIM_SURFACE_0, LABEL_SCRIM_SURFACE_0),
					Rotation = 90,
	...
	end, { shouldShowLabel, style, props.Text } :: { unknown })
	...
	local onMouseLeave = React.useCallback(function()
		if not itemsDisabled and getFFlagMaterialPickerUIChanges() then
	...
	if props.Loading then
		...
		return React.createElement(Pane, {
			...
			Shimmer = if getFFlagDevFrameworkShimmerImprovements()
				then React.createElement(Shimmer, {
					AnchorPoint = Vector2.new(0.5, 0.5),
					CornerRadius = style.GridItemCornerRadius,
					Position = UDim2.fromScale(0.5, 0.5),
					Size = UDim2.new(1, -shimmerSizeOffset, 1, -shimmerSizeOffset),
				})
				else React.createElement(Shimmer),
		})
	else
		local tooltipAnchorPoint
		local tooltipPosition
		if getFFlagMaterialPickerInstantTooltip() and props.ShouldCenterTooltip then -- Position the tooltip on the bottom half of the item
		...
		if getFFlagMaterialPickerUIChanges() then
			...
			if getFFlagMaterialPickerInstantTooltip() then
				...
				if isHovered and not itemsDisabled then
					backgroundColor = style.GridItemBackgroundColorHovered
				end
			end
		else
			backgroundColor = if (isHovered and not itemsDisabled)
				then style.GridItemBackgroundColorHovered
				else style.GridItemBackgroundColor
		end
		...
		local swatchChildren = {
			MaterialPreview = if props.CustomPreview
				then React.createElement(props.CustomPreview, {
					...
					MeshPartBackgroundColor = if getFFlagMaterialPickerUIChanges()
						then style.GridItemBackgroundColor
						else nil,
					Position = UDim2.fromScale(0.5, 0.5),
					Static = true,
					Style = style.MaterialPreviewStyle,
					Transparent = true,
					ref = if getFFlagMaterialPickerUIChanges() then ref else nil,
				}),
		...
		return React.createElement("TextButton", {
			...
			[React.Event.MouseMoved] = if getFFlagMaterialPickerInstantTooltip() and not itemsDisabled
				then props.OnMouseMoved
				else nil,

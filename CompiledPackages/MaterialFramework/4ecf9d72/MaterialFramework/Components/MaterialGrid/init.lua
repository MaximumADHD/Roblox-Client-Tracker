...
local function MaterialGrid(props: Props)
	...
	local onClickForItem = React.useCallback(function(item: Item)
		...
		-- NOTE: MaterialGridTerrainEditorItemUx=on + MaterialPickerUIChanges=off is a broken combination;
		-- per-item viewportFrameRef is no longer accessible here. Snapshot capture is only used in the
		-- UIChanges=on path (takeDummySnapshot above), so this is safe to leave broken.
	...
	local getItemProps = function(item: Item, index: number, position: UDim2, size: UDim2)
		...
		return {
			...
			InitialDistance = props.InitialDistance,
			GridItemSize = if getFFlagMaterialPickerInstantTooltip() then gridItemSize else nil,
			HoverEffectsEnabled = if getFFlagMaterialPickerInstantTooltip()
				then (hoverEffectsEnabled and not itemsDisabled)
				else nil,
			Item = item,

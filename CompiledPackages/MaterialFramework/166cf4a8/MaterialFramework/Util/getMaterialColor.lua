--[[
	getMaterialColor - a representative flat Color3 for a base material, used as a
	fallback swatch when a live viewport preview isn't wanted. Ported from
	MaterialFramework (pure data, no DeveloperFramework); the original's unused
	`use2022Materials` argument is dropped, so callers upgrading to MF2 update their
	call sites.
]]
--selene: allow(roblox_internal_custom_color)
local enumToColor: { [Enum.Material]: Color3 } = {
...
local FALLBACK_COLOR = BrickColor.new("Medium stone gray").Color

return function(material: Enum.Material): Color3
	return enumToColor[material] or FALLBACK_COLOR
end

...
local FFlagDevFrameworkShimmerImprovements = Framework.SharedFlags.getFFlagDevFrameworkShimmerImprovements()
...
local function MaterialGridItem(props: Props, ref)
	...
	if props.Loading then
		...
		return React.createElement(Pane, {
			...
			Shimmer = if FFlagDevFrameworkShimmerImprovements
		...
		if getFFlagMaterialPickerUIChanges() then
			...
			backgroundColor = if isHovered then style.GridItemBackgroundColorHovered else style.GridItemBackgroundColor
		end
		...
		local children = {
			...
			Tooltip = if not shouldShowLabel
				then if getFFlagMaterialPickerInstantTooltip()
					...
					else React.createElement(Tooltip, {
						Text = props.Text,
					})
				else nil,
		}
		...
		return React.createElement("TextButton", {
			...
			[React.Event.MouseMoved] = if getFFlagMaterialPickerInstantTooltip() then props.OnMouseMoved else nil,

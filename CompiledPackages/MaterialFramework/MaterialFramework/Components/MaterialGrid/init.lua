...
local getFFlagMaterialPickerImprovedScroll = require(main.Flags.getFFlagMaterialPickerImprovedScroll)
...
local function convertToItems(items: { ItemOrMaterial }): { Item }
	return Dash.map(items, function(item: ItemOrMaterial)
...
function MaterialGrid(props: Props)
	...
	local items = convertToItems(props.Items)
	...
	local function createOnScrollUpdate(viewType)
		...
		return function(canvasPosition: Vector2, canvasSize: UDim2)
			if getFFlagMaterialPickerImprovedScroll() then
	...
	elseif props.SelectedItem then
		local convertedSelectedItem = convertToItems({ props.SelectedItem })[1]
	...
	local function getItemProps(item: Item, index: number, position: UDim2, size: UDim2)
		...
		if not item.Loading then
			if getFFlagMaterialPickerUIChanges() then
				selected = getSerializedMaterialIdentifier(material) == selectedItemId
			else
				selected = item.Id == selectedItemId
			end

			if getFFlagMaterialPickerUIChanges() then
				applied = getSerializedMaterialIdentifier(material) == appliedItemId
			end

			if material then
				text = getMaterialName(material, localization)
			end
		end

		local viewportFrameRef = React.createRef()

		return {
			Applied = if getFFlagMaterialPickerUIChanges() then applied else nil,
			CanvasPosition = if getFFlagMaterialPickerInstantTooltip() then tooltipCanvasPosition else nil,
			ShouldCenterTooltip = if getFFlagMaterialPickerInstantTooltip() then props.ShouldCenterTooltip else nil,
			ContainerXBounds = if getFFlagMaterialPickerInstantTooltip() then props.ContainerXBounds else nil,
			ContainerYBounds = if getFFlagMaterialPickerInstantTooltip() then props.ContainerYBounds else nil,
			CustomPreview = if props.CustomPreviews then props.CustomPreviews[material] else nil,
			InitialDistance = props.InitialDistance,
			GridItemSize = if getFFlagMaterialPickerInstantTooltip() then gridItemSize else nil,
			HoverEffectsEnabled = if getFFlagMaterialPickerInstantTooltip() then hoverEffectsEnabled else nil,
			InstantTooltipEnabled = if getFFlagMaterialPickerInstantTooltip() then instantTooltipEnabled else nil,
			LayoutOrder = index,
			...
			OnClick = function()
				...
				if not getFFlagMaterialPickerUIChanges() then
					if props.OnSnapshotTaken then
						local success, rbxTempId = pcall(function()
							return viewportFrameRef:getValue():CaptureSnapshotAsync()
						end)
			...
			OnMouseMoved = if getFFlagMaterialPickerInstantTooltip() then onMouseMoved else nil,
			OnTooltipShown = if getFFlagMaterialPickerInstantTooltip() then onTooltipShown else nil,
			Position = position,
			Selected = selected,
			...
			Style = style,
			Text = text,
			TooltipDelay = if getFFlagMaterialPickerInstantTooltip() then tooltipDelay else nil,
			ref = if getFFlagMaterialPickerUIChanges() then nil else viewportFrameRef,
		}
	end
	...
	local function renderGrid()
		return React.createElement(Grid, {
			...
			InitialCanvasPosition = if getFFlagMaterialPickerImprovedScroll() then gridCanvasPosition.current else nil,
			...
			Cells = items,
			GetCellProps = getItemProps,
			OnScrollUpdate = if getFFlagMaterialPickerInstantTooltip() or getFFlagMaterialPickerImprovedScroll()
				then createOnScrollUpdate(ViewType.Grid)
				else nil,
	...
	local function renderList()
		return React.createElement(List, {
			AutomaticSize = if getFFlagMaterialGridExtraProps() then props.AutomaticSize else nil,
			InitialCanvasPosition = if getFFlagMaterialPickerImprovedScroll() then listCanvasPosition.current else nil,
			GetRowProps = getItemProps,
			OnScrollUpdate = if getFFlagMaterialPickerImprovedScroll()
				then createOnScrollUpdate(ViewType.List)
				else nil,

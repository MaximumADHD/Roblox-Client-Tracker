...
PROTO_7:
  ...
  LOADK R8 K19 ["target is not a GuiButton or TextBox, so it will not sink inputs unless `Active` is true.
  ...
  CALL R5 -1 0
  JUMPIF R1 [+7]
  FASTCALL2K TABLE_INSERT R2 K24 [+5]
  MOVE R4 R2
  LOADK R5 K24 ["target is not Active"]
  GETIMPORT R3 K18 [table.insert]
  CALL R3 2 0
  GETTABLEKS R3 R0 K25 ["Visible"]
  JUMPIF R3 [+7]
  FASTCALL2K TABLE_INSERT R2 K26 [+5]
  MOVE R4 R2
  LOADK R5 K26 ["target is not Visible"]
  GETIMPORT R3 K18 [table.insert]
  CALL R3 2 0
  GETTABLEKS R4 R0 K27 ["AbsoluteSize"]
  GETTABLEKS R3 R4 K28 ["X"]
  LOADN R4 0
  JUMPIFNOTLE R3 R4 [+13]
  MOVE R4 R2
  GETIMPORT R5 K8 [string.format]
  LOADK R6 K29 ["target has 0 width; element bounds: %s"]
  GETUPVAL R7 0
  MOVE R8 R0
  CALL R7 1 -1
  CALL R5 -1 -1
  ...
  CALL R3 -1 0
  GETTABLEKS R4 R0 K27 ["AbsoluteSize"]
  GETTABLEKS R3 R4 K30 ["Y"]
  LOADN R4 0
  JUMPIFNOTLE R3 R4 [+13]
  MOVE R4 R2
  GETIMPORT R5 K8 [string.format]
  LOADK R6 K31 ["target has 0 height; element bounds: %s"]
  GETUPVAL R7 0
  MOVE R8 R0
  CALL R7 1 -1
  CALL R5 -1 -1
  ...
  GETIMPORT R3 K18 [table.insert]
  CALL R3 -1 0
  LOADK R5 K32 ["LayerCollector"]
  NAMECALL R3 R0 K33 ["FindFirstAncestorWhichIsA"]
  CALL R3 2 1
  JUMPIF R3 [+7]
  FASTCALL2K TABLE_INSERT R2 K34 [+5]
  MOVE R5 R2
  LOADK R6 K34 ["target is not a descendant of a LayerCollector, like a ScreenGui or a SurfaceGui"]
  GETIMPORT R4 K18 [table.insert]
  CALL R4 2 0
  LENGTH R5 R2
  JUMPIFEQKN R5 K35 [0] [+2]
  LOADB R4 0 +1
  LOADB R4 1
  MOVE R5 R2
  RETURN R4 2

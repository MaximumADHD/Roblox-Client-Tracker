...
local FFlagViewportToolingFrameworkTextLabelColor = game:DefineFastFlag("ViewportToolingFrameworkTextLabelColor", false)
...
local TOOLBAR_DISTANCE_FROM_EDGE = if getFFlagViewportToolingFrameworkToolbarSpacing() then 16 + CORNER_PADDING else 16
...
local function createStyle(): { StyleRule? }
	return {
		...
		rule("TextLabel", {
			BackgroundTransparency = 1,
			Font = Enum.Font.SourceSans,
			TextColor3 = if FFlagViewportToolingFrameworkTextLabelColor then "$TextPrimary" else nil,
			TextSize = if getFFlagViewportToolingFrameworkSplitButtons() then 16 else nil,
			TextXAlignment = Enum.TextXAlignment.Left,
			TextYAlignment = Enum.TextYAlignment.Top,
		}),

		-- Components
		if getFFlagViewportToolingFrameworkSplitButtons()
			then nil
			else rule(".VPF-AvoidFOUC", {
		...
		rule(".VPF-Toolbar", {
			...
			rule(".VPF-Toolbar--Horizontal", {}, {
				...
				if getFFlagViewportToolingFrameworkToolbarSpacing()
					then rule(".VPF-Toolbar--Left", {
			...
			rule(".VPF-Toolbar--Vertical", {}, {
				...
				rule(".VPF-Toolbar--Top", {
					...
					Position = UDim2.new(
						...
						0,
						if getFFlagViewportToolingFrameworkToolbarSpacing()
							then TOOLBAR_DISTANCE_FROM_EDGE + TOOLBAR_TOTAL_SIZE + TOOLBARS_GAP
							else TOOLBAR_DISTANCE_FROM_EDGE + TOOLBAR_DOMINANT_SIZE
					),
				}),
				...
				rule(".VPF-Toolbar--Bottom", {
					...
					Position = UDim2.new(
						...
						1,
						if getFFlagViewportToolingFrameworkToolbarSpacing()
							then -TOOLBAR_TOTAL_SIZE
							else -TOOLBAR_DOMINANT_SIZE
					),
				}),
			}),
		}),

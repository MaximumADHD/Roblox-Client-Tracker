...
local DEPRECATED_AvoidFOUC = require(ViewportToolingFramework.Components.DEPRECATED_AvoidFOUC)
...
local function Tooltip(props: Props)
	...
	if getFFlagViewportToolingFrameworkButtonDisabled() then
		descriptionSize = React.useMemo(function(): Vector2
		...
		descriptionSize = if props.Description
	...
	return React.createElement("Frame", {
		...
		VisibleTooltip = hovered.enabled and ReactRoblox.createPortal(
			React.createElement("Folder", {}, {
				AvoidFOUC = if getFFlagViewportToolingFrameworkSplitButtons()
					...
					else React.createElement(DEPRECATED_AvoidFOUC, {}, {
						VisibleTooltip = React.createElement("Frame", {
							BackgroundTransparency = 1,
							Position = UDim2.fromOffset(position.X, position.Y),
							Size = UDim2.fromOffset(size.X, size.Y),
								...
								Description = props.Description
									and React.createElement("TextLabel", {
										...
										Size = UDim2.fromOffset(
											descriptionSize.X + PADDING,
											descriptionSize.Y + PADDING
										),
										Text = props.Description,
										TextSize = DESCRIPTION_TEXT_SIZE,
										[React.Tag] = "VPF-Tooltip__Description",

PROTO_0:
        0 JUMPIFNOTEQKNIL                  R0 ; [+2]
        2 LOADB                            R3 0 +1
        3 LOADB                            R3 1
        4 FASTCALL2K                       ASSERT R3 K0 ; [+4]
        6 LOADK                            R4 K0 ["Luau"]
        7 GETIMPORT                        R2 K2 [assert]
        9 CALL                             R2 2 0
       10 GETTABLEKS                       R2 R0 K3 ["interactor"]
       12 GETUPVAL                         R3 0
       13 MOVE                             R4 R1
       14 CALL                             R3 1 -1
       15 RETURN                           R2 -1

PROTO_1:
        0 GETUPVAL                         R3 0
        1 GETUPVAL                         R4 1
        2 MOVE                             R5 R1
        3 CALL                             R4 1 1
        4 GETTABLE                         R2 R3 R4
        5 GETTABLEKS                       R2 R2 K0 ["read"]
        7 MOVE                             R3 R0
        8 CALL                             R2 1 -1
        9 RETURN                           R2 -1

PROTO_2:
        0 GETUPVAL                         R4 0
        1 GETUPVAL                         R5 1
        2 MOVE                             R6 R1
        3 CALL                             R5 1 1
        4 GETTABLE                         R3 R4 R5
        5 GETTABLEKS                       R3 R3 K0 ["write"]
        7 MOVE                             R4 R0
        8 MOVE                             R5 R2
        9 CALL                             R3 2 0
       10 RETURN                           R0 0

PROTO_3:
        0 GETTABLEKS                       R4 R1 K0 ["metaType"]
        2 JUMPIFEQKS                       R4 K1 ["Virtual"] ; [+2]
        4 LOADB                            R3 0 +1
        5 LOADB                            R3 1
        6 FASTCALL2K                       ASSERT R3 K2 ; [+4]
        8 LOADK                            R4 K2 ["Was not a virtual property"]
        9 GETIMPORT                        R2 K4 [assert]
       11 CALL                             R2 2 0
       12 GETTABLEKS                       R4 R1 K5 ["className"]
       14 NAMECALL                         R2 R0 K6 ["IsA"]
       16 CALL                             R2 2 -1
       17 RETURN                           R2 -1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [script]
        3 LOADK                            R2 K2 ["Properties"]
        4 NAMECALL                         R0 R0 K3 ["FindFirstAncestor"]
        6 CALL                             R0 2 1
        7 GETTABLEKS                       R1 R0 K4 ["Parent"]
        9 GETIMPORT                        R2 K6 [require]
       11 GETTABLEKS                       R3 R1 K7 ["Cryo"]
       13 CALL                             R2 1 1
       14 GETIMPORT                        R3 K6 [require]
       16 GETIMPORT                        R4 K1 [script]
       18 GETTABLEKS                       R4 R4 K4 ["Parent"]
       20 GETTABLEKS                       R4 R4 K8 ["InteractorTypes"]
       22 CALL                             R3 1 1
       23 GETIMPORT                        R4 K6 [require]
       25 GETTABLEKS                       R5 R0 K9 ["PropertyTypes"]
       27 CALL                             R4 1 1
       28 GETIMPORT                        R5 K6 [require]
       30 GETTABLEKS                       R6 R0 K10 ["PropertyInteraction"]
       32 GETTABLEKS                       R6 R6 K11 ["SpecialProperties"]
       34 CALL                             R5 1 1
       35 GETIMPORT                        R6 K6 [require]
       37 GETIMPORT                        R7 K1 [script]
       39 GETTABLEKS                       R7 R7 K12 ["VirtualPropertyDataTypes"]
       41 CALL                             R6 1 1
       42 GETIMPORT                        R7 K6 [require]
       44 GETTABLEKS                       R8 R0 K10 ["PropertyInteraction"]
       46 GETTABLEKS                       R8 R8 K13 ["propertyIdToString"]
       48 CALL                             R7 1 1
       49 GETTABLEKS                       R8 R2 K14 ["Dictionary"]
       51 GETTABLEKS                       R8 R8 K15 ["map"]
       53 GETTABLEKS                       R9 R5 K16 ["virtualPropertyDefinitions"]
       55 DUPCLOSURE                       R10 K17 [PROTO_0]
       56 CAPTURE                          VAL R7
       57 CALL                             R8 2 1
       58 DUPCLOSURE                       R9 K18 [PROTO_1]
       59 CAPTURE                          VAL R8
       60 CAPTURE                          VAL R7
       61 DUPCLOSURE                       R10 K19 [PROTO_2]
       62 CAPTURE                          VAL R8
       63 CAPTURE                          VAL R7
       64 DUPCLOSURE                       R11 K20 [PROTO_3]
       65 DUPTABLE                         R12 K24 [{"read", "write", "has"}]
       66 SETTABLEKS                       R9 R12 K21 ["read"]
       68 SETTABLEKS                       R10 R12 K22 ["write"]
       70 SETTABLEKS                       R11 R12 K23 ["has"]
       72 RETURN                           R12 1

...
export type NumberInputTextStateProps = {
	-- Input number value
	value: number?,
...
local function NumberInput(numberInputProps: NumberInputTextStateProps, ref: React.Ref<GuiObject>?)
	...
	local onFocusLost = React.useCallback(
		function(inputObject: InputObject?)
			...
			props.onChanged(round(v, props.precision), OnChangeCallbackReason.FocusLost)
	...
	local onIncrement = React.useCallback(function()
		...
		props.onChanged(upValue, OnChangeCallbackReason.Activate)
	...
	local onDecrement = React.useCallback(function()
		...
		props.onChanged(downValue, OnChangeCallbackReason.Activate)
	...
	local onDrag = React.useCallback(function(_rbx, position: Vector2)
		if dragStartTable.current then
			...
			props.onChanged(newValue, OnChangeCallbackReason.Drag)
		end
	end, { props.onChanged })

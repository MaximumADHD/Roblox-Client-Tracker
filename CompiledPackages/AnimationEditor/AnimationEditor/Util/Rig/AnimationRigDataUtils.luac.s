PROTO_0:
  JUMPIF R0 [+2]
  LOADB R1 0
  RETURN R1 1
  LOADK R3 K0 ["Workspace"]
  NAMECALL R1 R0 K1 ["IsA"]
  CALL R1 2 1
  JUMPIFNOT R1 [+2]
  LOADB R1 0
  RETURN R1 1
  LOADK R3 K2 ["Model"]
  NAMECALL R1 R0 K1 ["IsA"]
  CALL R1 2 1
  JUMPIF R1 [+5]
  LOADK R3 K3 ["Humanoid"]
  NAMECALL R1 R0 K1 ["IsA"]
  CALL R1 2 1
  JUMPIFNOT R1 [+2]
  LOADB R1 1
  RETURN R1 1
  LOADB R1 0
  RETURN R1 1

PROTO_1:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["findAnimatorParentFromAnimSaves"]
  MOVE R2 R0
  CALL R1 1 1
  JUMPIFNOT R1 [+1]
  RETURN R1 1
  LOADK R4 K1 ["Humanoid"]
  NAMECALL R2 R0 K2 ["FindFirstAncestorWhichIsA"]
  CALL R2 2 1
  JUMPIFNOT R2 [+1]
  RETURN R2 1
  LOADK R5 K3 ["Model"]
  NAMECALL R3 R0 K2 ["FindFirstAncestorWhichIsA"]
  CALL R3 2 1
  JUMPIFNOT R3 [+13]
  LOADK R6 K1 ["Humanoid"]
  NAMECALL R4 R3 K4 ["FindFirstChildWhichIsA"]
  CALL R4 2 1
  MOVE R2 R4
  JUMPIFNOT R2 [+1]
  RETURN R2 1
  LOADK R6 K3 ["Model"]
  NAMECALL R4 R3 K2 ["FindFirstAncestorWhichIsA"]
  CALL R4 2 1
  MOVE R3 R4
  JUMPBACK [-14]
  LOADNIL R4
  RETURN R4 1

PROTO_2:
  LOADK R3 K0 ["ObjectValue"]
  NAMECALL R1 R0 K1 ["FindFirstAncestorWhichIsA"]
  CALL R1 2 1
  JUMPIF R1 [+2]
  LOADNIL R2
  RETURN R2 1
  MOVE R2 R1
  JUMPIFNOT R2 [+2]
  GETTABLEKS R2 R1 K2 ["Value"]
  JUMPIFNOT R2 [+5]
  LOADK R5 K3 ["Model"]
  NAMECALL R3 R2 K4 ["IsA"]
  CALL R3 2 1
  JUMPIF R3 [+2]
  LOADNIL R3
  RETURN R3 1
  LOADK R5 K5 ["Humanoid"]
  NAMECALL R3 R2 K6 ["FindFirstChildWhichIsA"]
  CALL R3 2 1
  JUMPIF R3 [+2]
  LOADNIL R4
  RETURN R4 1
  RETURN R3 1

PROTO_3:
  LOADK R3 K0 ["Humanoid"]
  NAMECALL R1 R0 K1 ["IsA"]
  CALL R1 2 1
  JUMPIFNOT R1 [+1]
  RETURN R0 1
  LOADK R3 K0 ["Humanoid"]
  NAMECALL R1 R0 K2 ["FindFirstChildWhichIsA"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_4:
  GETUPVAL R0 1
  GETUPVAL R2 2
  NAMECALL R0 R0 K0 ["LoadFromHumanoid"]
  CALL R0 2 1
  SETUPVAL R0 0
  RETURN R0 0

PROTO_5:
  JUMPIF R0 [+2]
  LOADB R2 0
  RETURN R2 1
  JUMPIF R1 [+2]
  LOADB R2 0
  RETURN R2 1
  LOADNIL R2
  GETIMPORT R3 K1 [pcall]
  NEWCLOSURE R4 P0
  CAPTURE REF R2
  CAPTURE VAL R0
  CAPTURE VAL R1
  CALL R3 1 2
  JUMPIFNOT R3 [+1]
  JUMPIF R2 [+8]
  JUMPIFNOT R4 [+4]
  GETIMPORT R5 K3 [warn]
  MOVE R6 R4
  CALL R5 1 0
  LOADB R5 0
  CLOSEUPVALS R2
  RETURN R5 1
  LOADB R5 1
  CLOSEUPVALS R2
  RETURN R5 1

PROTO_6:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["getOrCreateAnimSavesObjectValue"]
  MOVE R2 R0
  CALL R1 1 1
  JUMPIF R1 [+2]
  LOADNIL R2
  RETURN R2 1
  GETTABLEKS R2 R1 K1 ["Value"]
  JUMPIFNOT R2 [+17]
  GETIMPORT R5 K3 [game]
  NAMECALL R3 R2 K4 ["IsDescendantOf"]
  CALL R3 2 1
  JUMPIFNOT R3 [+11]
  LOADK R6 K5 ["ObjectValue"]
  NAMECALL R4 R2 K6 ["IsA"]
  CALL R4 2 1
  FASTCALL2K ASSERT R4 K7 [+4]
  LOADK R5 K7 ["Existing AnimSaves link is not an ObjectValue"]
  GETIMPORT R3 K9 [assert]
  CALL R3 2 0
  RETURN R2 1
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K10 ["getOrCreateServerStorageContainer"]
  CALL R3 0 1
  GETIMPORT R4 K13 [Instance.new]
  LOADK R5 K5 ["ObjectValue"]
  CALL R4 1 1
  GETTABLEKS R5 R0 K14 ["Name"]
  SETTABLEKS R5 R4 K14 ["Name"]
  SETTABLEKS R0 R4 K1 ["Value"]
  SETTABLEKS R3 R4 K15 ["Parent"]
  SETTABLEKS R4 R1 K1 ["Value"]
  RETURN R4 1

PROTO_7:
  GETUPVAL R0 0
  LOADK R2 K0 ["RBX_ANIMSAVES"]
  NAMECALL R0 R0 K1 ["FindFirstChild"]
  CALL R0 2 1
  JUMPIFNOT R0 [+1]
  RETURN R0 1
  GETIMPORT R1 K4 [Instance.new]
  LOADK R2 K5 ["Model"]
  CALL R1 1 1
  LOADK R2 K0 ["RBX_ANIMSAVES"]
  SETTABLEKS R2 R1 K6 ["Name"]
  GETUPVAL R2 0
  SETTABLEKS R2 R1 K7 ["Parent"]
  RETURN R1 1

PROTO_8:
  LOADK R3 K0 ["AnimSaves"]
  NAMECALL R1 R0 K1 ["FindFirstChild"]
  CALL R1 2 1
  JUMPIFNOT R1 [+6]
  LOADK R4 K2 ["ObjectValue"]
  NAMECALL R2 R1 K3 ["IsA"]
  CALL R2 2 1
  JUMPIFNOT R2 [+1]
  RETURN R1 1
  GETIMPORT R2 K6 [Instance.new]
  LOADK R3 K2 ["ObjectValue"]
  CALL R2 1 1
  LOADK R3 K0 ["AnimSaves"]
  SETTABLEKS R3 R2 K7 ["Name"]
  SETTABLEKS R0 R2 K8 ["Parent"]
  RETURN R2 1

PROTO_9:
  LOADK R4 K0 ["AnimationRigData"]
  NAMECALL R2 R0 K1 ["FindFirstChildWhichIsA"]
  CALL R2 2 1
  JUMPIFNOT R2 [+1]
  RETURN R2 1
  GETTABLEKS R3 R1 K2 ["Parent"]
  JUMPIFNOT R3 [+5]
  GETTABLEKS R5 R3 K3 ["Name"]
  LOADK R6 K4 ["_"]
  CONCAT R4 R5 R6
  JUMP [+1]
  LOADK R4 K5 [""]
  GETIMPORT R5 K8 [Instance.new]
  LOADK R6 K0 ["AnimationRigData"]
  CALL R5 1 1
  MOVE R7 R4
  LOADK R8 K0 ["AnimationRigData"]
  CONCAT R6 R7 R8
  SETTABLEKS R6 R5 K3 ["Name"]
  SETTABLEKS R0 R5 K2 ["Parent"]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K9 ["loadAnimationRig"]
  MOVE R7 R5
  MOVE R8 R1
  CALL R6 2 1
  JUMPIF R6 [+5]
  NAMECALL R7 R5 K10 ["Destroy"]
  CALL R7 1 0
  LOADNIL R7
  RETURN R7 1
  RETURN R5 1

PROTO_10:
  LOADK R3 K0 ["AnimationRigData"]
  NAMECALL R1 R0 K1 ["FindFirstChildWhichIsA"]
  CALL R1 2 1
  JUMPIFNOT R1 [+1]
  RETURN R1 1
  GETTABLEKS R2 R0 K2 ["Parent"]
  JUMPIFNOT R2 [+5]
  GETTABLEKS R4 R2 K3 ["Name"]
  LOADK R5 K4 ["_"]
  CONCAT R3 R4 R5
  JUMP [+1]
  LOADK R3 K5 [""]
  GETIMPORT R4 K8 [Instance.new]
  LOADK R5 K0 ["AnimationRigData"]
  CALL R4 1 1
  MOVE R6 R3
  LOADK R7 K0 ["AnimationRigData"]
  CONCAT R5 R6 R7
  SETTABLEKS R5 R4 K3 ["Name"]
  SETTABLEKS R0 R4 K2 ["Parent"]
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K9 ["loadAnimationRig"]
  MOVE R6 R4
  MOVE R7 R0
  CALL R5 2 1
  JUMPIF R5 [+5]
  NAMECALL R6 R4 K10 ["Destroy"]
  CALL R6 1 0
  LOADNIL R6
  RETURN R6 1
  RETURN R4 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [game]
  LOADK R2 K2 ["ServerStorage"]
  NAMECALL R0 R0 K3 ["GetService"]
  CALL R0 2 1
  NEWTABLE R1 16 0
  DUPCLOSURE R2 K4 [PROTO_0]
  SETTABLEKS R2 R1 K5 ["canAnimate"]
  DUPCLOSURE R2 K6 [PROTO_1]
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K7 ["findAnimatorParent"]
  DUPCLOSURE R2 K8 [PROTO_2]
  SETTABLEKS R2 R1 K9 ["findAnimatorParentFromAnimSaves"]
  DUPCLOSURE R2 K10 [PROTO_3]
  SETTABLEKS R2 R1 K11 ["findHumanoidInParent"]
  DUPCLOSURE R2 K12 [PROTO_5]
  SETTABLEKS R2 R1 K13 ["loadAnimationRig"]
  DUPCLOSURE R2 K14 [PROTO_6]
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K15 ["getOrCreateAnimSavesFolder"]
  DUPCLOSURE R2 K16 [PROTO_7]
  CAPTURE VAL R0
  SETTABLEKS R2 R1 K17 ["getOrCreateServerStorageContainer"]
  DUPCLOSURE R2 K18 [PROTO_8]
  SETTABLEKS R2 R1 K19 ["getOrCreateAnimSavesObjectValue"]
  DUPCLOSURE R2 K20 [PROTO_9]
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K21 ["ensureAnimationRigDataInAnimationClip"]
  DUPCLOSURE R2 K22 [PROTO_10]
  CAPTURE VAL R1
  SETTABLEKS R2 R1 K23 ["ensureAnimationRigDataInHumanoid"]
  RETURN R1 1

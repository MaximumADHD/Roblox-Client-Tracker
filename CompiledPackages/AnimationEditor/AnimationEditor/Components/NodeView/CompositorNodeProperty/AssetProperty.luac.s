PROTO_0:
  FASTCALL1 TYPEOF R0 [+3]
  MOVE R2 R0
  GETIMPORT R1 K1 [typeof]
  CALL R1 1 1
  JUMPIFNOTEQKS R1 K2 ["string"] [+27]
  GETIMPORT R1 K4 [string.match]
  MOVE R2 R0
  LOADK R3 K5 ["rbxassetid://(%d+)"]
  CALL R1 2 1
  JUMPIFNOT R1 [+6]
  FASTCALL1 TONUMBER R1 [+3]
  MOVE R3 R1
  GETIMPORT R2 K7 [tonumber]
  CALL R2 1 1
  RETURN R2 1
  GETIMPORT R2 K4 [string.match]
  MOVE R3 R0
  LOADK R4 K8 ["(%d+)"]
  CALL R2 2 1
  JUMPIFNOT R2 [+6]
  FASTCALL1 TONUMBER R2 [+3]
  MOVE R4 R2
  GETIMPORT R3 K7 [tonumber]
  CALL R3 1 1
  RETURN R3 1
  LOADNIL R3
  RETURN R3 1
  FASTCALL1 TYPEOF R0 [+3]
  MOVE R2 R0
  GETIMPORT R1 K1 [typeof]
  CALL R1 1 1
  JUMPIFNOTEQKS R1 K9 ["number"] [+2]
  RETURN R0 1
  LOADNIL R1
  RETURN R1 1

PROTO_1:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["registerAssetIdAsync"]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K1 ["Type"]
  GETUPVAL R2 2
  CALL R0 2 0
  RETURN R0 0

PROTO_2:
  JUMPIFNOTEQKS R0 K0 ["createNewClip"] [+6]
  GETIMPORT R1 K2 [print]
  LOADK R2 K3 ["TODO: Create new animation clip"]
  CALL R1 1 0
  RETURN R0 0
  JUMPIFNOTEQKS R0 K4 ["browseClips"] [+27]
  GETUPVAL R1 0
  GETUPVAL R2 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K5 ["Type"]
  CALL R1 2 1
  JUMPIFNOT R1 [+30]
  GETIMPORT R2 K8 [task.spawn]
  NEWCLOSURE R3 P0
  CAPTURE UPVAL U3
  CAPTURE UPVAL U2
  CAPTURE VAL R1
  CALL R2 1 0
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K9 ["OnChanged"]
  LOADK R4 K10 ["rbxassetid://"]
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R6 R1
  GETIMPORT R5 K12 [tostring]
  CALL R5 1 1
  CONCAT R3 R4 R5
  CALL R2 1 0
  RETURN R0 0
  GETUPVAL R2 2
  GETTABLEKS R1 R2 K9 ["OnChanged"]
  LOADK R3 K10 ["rbxassetid://"]
  FASTCALL1 TOSTRING R0 [+3]
  MOVE R5 R0
  GETIMPORT R4 K12 [tostring]
  CALL R4 1 1
  CONCAT R2 R3 R4
  CALL R1 1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["registerAssetIdAsync"]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K1 ["Type"]
  GETUPVAL R2 2
  CALL R0 2 0
  RETURN R0 0

PROTO_4:
  NEWTABLE R0 0 0
  LOADNIL R1
  GETUPVAL R2 0
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K0 ["Value"]
  CALL R2 1 1
  GETUPVAL R7 2
  GETTABLEKS R6 R7 K1 ["registeredAssetIds"]
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K2 ["Type"]
  GETTABLE R3 R6 R7
  JUMPIF R3 [+2]
  NEWTABLE R3 0 0
  LOADNIL R4
  LOADNIL R5
  FORGPREP R3
  DUPTABLE R8 K5 [{"id", "text"}]
  SETTABLEKS R6 R8 K3 ["id"]
  GETTABLEKS R9 R7 K6 ["name"]
  SETTABLEKS R9 R8 K4 ["text"]
  GETUPVAL R9 0
  MOVE R10 R6
  CALL R9 1 1
  JUMPIFNOTEQ R9 R2 [+2]
  MOVE R1 R6
  FASTCALL2 TABLE_INSERT R0 R8 [+5]
  MOVE R10 R0
  MOVE R11 R8
  GETIMPORT R9 K9 [table.insert]
  CALL R9 2 0
  FORGLOOP R3 2 [-21]
  JUMPIFNOT R2 [+50]
  LOADB R3 0
  GETIMPORT R4 K11 [ipairs]
  MOVE R5 R0
  CALL R4 1 3
  FORGPREP_INEXT R4
  GETTABLEKS R9 R8 K3 ["id"]
  JUMPIFNOTEQ R9 R2 [+3]
  LOADB R3 1
  JUMP [+2]
  FORGLOOP R4 2 [inext] [-7]
  JUMPIF R3 [+35]
  GETIMPORT R4 K14 [task.spawn]
  NEWCLOSURE R5 P0
  CAPTURE UPVAL U2
  CAPTURE UPVAL U1
  CAPTURE VAL R2
  CALL R4 1 0
  LOADK R5 K15 ["pendingRegistration_"]
  FASTCALL1 TOSTRING R2 [+3]
  MOVE R7 R2
  GETIMPORT R6 K17 [tostring]
  CALL R6 1 1
  CONCAT R4 R5 R6
  DUPTABLE R7 K5 [{"id", "text"}]
  SETTABLEKS R4 R7 K3 ["id"]
  LOADK R9 K18 ["Unknown ("]
  FASTCALL1 TOSTRING R2 [+3]
  MOVE R13 R2
  GETIMPORT R12 K17 [tostring]
  CALL R12 1 1
  MOVE R10 R12
  LOADK R11 K19 [")"]
  CONCAT R8 R9 R11
  SETTABLEKS R8 R7 K4 ["text"]
  FASTCALL2 TABLE_INSERT R0 R7 [+4]
  MOVE R6 R0
  GETIMPORT R5 K9 [table.insert]
  CALL R5 2 0
  MOVE R1 R4
  NEWTABLE R3 0 2
  DUPTABLE R4 K21 [{"items"}]
  SETTABLEKS R0 R4 K20 ["items"]
  DUPTABLE R5 K21 [{"items"}]
  NEWTABLE R6 0 2
  DUPTABLE R7 K23 [{"id", "text", "isDisabled"}]
  LOADK R8 K24 ["createNewClip"]
  SETTABLEKS R8 R7 K3 ["id"]
  LOADK R8 K25 ["Create new"]
  SETTABLEKS R8 R7 K4 ["text"]
  LOADB R8 1
  SETTABLEKS R8 R7 K22 ["isDisabled"]
  DUPTABLE R8 K23 [{"id", "text", "isDisabled"}]
  LOADK R9 K26 ["browseClips"]
  SETTABLEKS R9 R8 K3 ["id"]
  LOADK R9 K27 ["Browse clips"]
  SETTABLEKS R9 R8 K4 ["text"]
  LOADB R9 0
  SETTABLEKS R9 R8 K22 ["isDisabled"]
  SETLIST R6 R7 2 [1]
  SETTABLEKS R6 R5 K20 ["items"]
  SETLIST R3 R4 2 [1]
  MOVE R4 R3
  MOVE R5 R1
  RETURN R4 2

PROTO_5:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["useContext"]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["Context"]
  CALL R1 1 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K2 ["ContextServices"]
  GETTABLEKS R2 R3 K3 ["Plugin"]
  NAMECALL R2 R2 K4 ["use"]
  CALL R2 1 1
  NAMECALL R2 R2 K5 ["get"]
  CALL R2 1 1
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K6 ["useCallback"]
  NEWCLOSURE R4 P0
  CAPTURE UPVAL U3
  CAPTURE VAL R2
  CAPTURE VAL R0
  CAPTURE VAL R1
  NEWTABLE R5 0 3
  MOVE R6 R2
  GETTABLEKS R7 R1 K7 ["registerAssetIdAsync"]
  GETTABLEKS R8 R0 K8 ["OnChanged"]
  SETLIST R5 R6 3 [1]
  CALL R3 2 1
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K9 ["useMemo"]
  NEWCLOSURE R5 P1
  CAPTURE UPVAL U4
  CAPTURE VAL R0
  CAPTURE VAL R1
  NEWTABLE R6 0 5
  GETTABLEKS R7 R0 K8 ["OnChanged"]
  GETTABLEKS R8 R0 K10 ["Value"]
  GETTABLEKS R9 R0 K11 ["Type"]
  GETTABLEKS R11 R1 K12 ["registeredAssetIds"]
  GETTABLEKS R12 R0 K11 ["Type"]
  GETTABLE R10 R11 R12
  GETTABLEKS R11 R1 K7 ["registerAssetIdAsync"]
  SETLIST R6 R7 5 [1]
  CALL R4 2 2
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K13 ["createElement"]
  GETUPVAL R8 5
  GETTABLEKS R7 R8 K14 ["View"]
  DUPTABLE R8 K17 [{"tag", "LayoutOrder"}]
  LOADK R9 K18 ["row auto-y gap-small size-full-700 align-y-center flex-x-between"]
  SETTABLEKS R9 R8 K15 ["tag"]
  GETTABLEKS R9 R0 K16 ["LayoutOrder"]
  SETTABLEKS R9 R8 K16 ["LayoutOrder"]
  DUPTABLE R9 K21 [{"CompositorNodeInputLabel", "Input"}]
  GETUPVAL R11 0
  GETTABLEKS R10 R11 K13 ["createElement"]
  GETUPVAL R12 5
  GETTABLEKS R11 R12 K22 ["Text"]
  DUPTABLE R12 K23 [{"tag", "Text", "LayoutOrder"}]
  LOADK R13 K24 ["text-body-small text-align-x-left auto-xy fill text-truncate-split"]
  SETTABLEKS R13 R12 K15 ["tag"]
  GETTABLEKS R13 R0 K25 ["Label"]
  SETTABLEKS R13 R12 K22 ["Text"]
  LOADN R13 1
  SETTABLEKS R13 R12 K16 ["LayoutOrder"]
  CALL R10 2 1
  SETTABLEKS R10 R9 K19 ["CompositorNodeInputLabel"]
  GETUPVAL R11 0
  GETTABLEKS R10 R11 K13 ["createElement"]
  GETUPVAL R13 5
  GETTABLEKS R12 R13 K26 ["Dropdown"]
  GETTABLEKS R11 R12 K27 ["Root"]
  DUPTABLE R12 K35 [{"size", "width", "label", "items", "value", "isDisabled", "onItemChanged", "LayoutOrder"}]
  GETUPVAL R16 5
  GETTABLEKS R15 R16 K36 ["Enums"]
  GETTABLEKS R14 R15 K37 ["InputSize"]
  GETTABLEKS R13 R14 K38 ["XSmall"]
  SETTABLEKS R13 R12 K28 ["size"]
  GETIMPORT R13 K41 [UDim.new]
  LOADN R14 0
  LOADN R15 90
  CALL R13 2 1
  SETTABLEKS R13 R12 K29 ["width"]
  LOADK R13 K42 [""]
  SETTABLEKS R13 R12 K30 ["label"]
  SETTABLEKS R4 R12 K31 ["items"]
  SETTABLEKS R5 R12 K32 ["value"]
  GETTABLEKS R13 R0 K43 ["IsParameterized"]
  SETTABLEKS R13 R12 K33 ["isDisabled"]
  SETTABLEKS R3 R12 K34 ["onItemChanged"]
  LOADN R13 2
  SETTABLEKS R13 R12 K16 ["LayoutOrder"]
  CALL R10 2 1
  SETTABLEKS R10 R9 K20 ["Input"]
  CALL R6 3 -1
  RETURN R6 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AnimationEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R4 R0 K6 ["Contexts"]
  GETTABLEKS R3 R4 K7 ["NodeContexts"]
  GETTABLEKS R2 R3 K8 ["AssetNodeContext"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R6 R0 K9 ["Components"]
  GETTABLEKS R5 R6 K10 ["NodeView"]
  GETTABLEKS R4 R5 K11 ["NodeActions"]
  GETTABLEKS R3 R4 K12 ["BrowseAssets"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R5 R0 K13 ["Parent"]
  GETTABLEKS R4 R5 K14 ["Foundation"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R6 R0 K13 ["Parent"]
  GETTABLEKS R5 R6 K15 ["Framework"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R7 R0 K13 ["Parent"]
  GETTABLEKS R6 R7 K16 ["React"]
  CALL R5 1 1
  DUPCLOSURE R6 K17 [PROTO_0]
  DUPCLOSURE R7 K18 [PROTO_5]
  CAPTURE VAL R5
  CAPTURE VAL R1
  CAPTURE VAL R4
  CAPTURE VAL R2
  CAPTURE VAL R6
  CAPTURE VAL R3
  RETURN R7 1

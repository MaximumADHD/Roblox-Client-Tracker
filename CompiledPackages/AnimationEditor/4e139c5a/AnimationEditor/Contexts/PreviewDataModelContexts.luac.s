PROTO_0:
        0 GETUPVAL                         R1 0
        1 GETTABLEKS                       R1 R1 K0 ["createElement"]
        3 GETUPVAL                         R2 1
        4 GETTABLEKS                       R2 R2 K1 ["ContextStack"]
        6 DUPTABLE                         R3 K3 [{"providers"}]
        7 NEWTABLE                         R4 0 6
        9 GETUPVAL                         R5 0
       10 GETTABLEKS                       R5 R5 K0 ["createElement"]
       12 GETUPVAL                         R6 2
       13 GETTABLEKS                       R6 R6 K4 ["EditableDataModelProvider"]
       15 CALL                             R5 1 1
       16 GETUPVAL                         R6 0
       17 GETTABLEKS                       R6 R6 K0 ["createElement"]
       19 GETUPVAL                         R7 3
       20 GETTABLEKS                       R7 R7 K5 ["PreviewDataModelProvider"]
       22 CALL                             R6 1 1
       23 GETUPVAL                         R7 0
       24 GETTABLEKS                       R7 R7 K0 ["createElement"]
       26 GETUPVAL                         R8 4
       27 GETTABLEKS                       R8 R8 K5 ["PreviewDataModelProvider"]
       29 CALL                             R7 1 1
       30 GETUPVAL                         R8 0
       31 GETTABLEKS                       R8 R8 K0 ["createElement"]
       33 GETUPVAL                         R9 5
       34 GETTABLEKS                       R9 R9 K4 ["EditableDataModelProvider"]
       36 CALL                             R8 1 1
       37 GETUPVAL                         R9 0
       38 GETTABLEKS                       R9 R9 K0 ["createElement"]
       40 GETUPVAL                         R10 6
       41 GETTABLEKS                       R10 R10 K5 ["PreviewDataModelProvider"]
       43 CALL                             R9 1 1
       44 GETUPVAL                         R10 0
       45 GETTABLEKS                       R10 R10 K0 ["createElement"]
       47 GETUPVAL                         R11 7
       48 GETTABLEKS                       R11 R11 K4 ["EditableDataModelProvider"]
       50 CALL                             R10 1 -1
       51 SETLIST                          R4 R5 -1 [1]
       53 SETTABLEKS                       R4 R3 K2 ["providers"]
       55 GETTABLEKS                       R4 R0 K6 ["children"]
       57 CALL                             R1 3 -1
       58 RETURN                           R1 -1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [script]
        3 LOADK                            R2 K2 ["AnimationEditor"]
        4 NAMECALL                         R0 R0 K3 ["FindFirstAncestor"]
        6 CALL                             R0 2 1
        7 GETIMPORT                        R1 K5 [require]
        9 GETTABLEKS                       R2 R0 K6 ["Contexts"]
       11 GETTABLEKS                       R2 R2 K7 ["GraphDebugDataContext"]
       13 CALL                             R1 1 1
       14 GETIMPORT                        R2 K5 [require]
       16 GETTABLEKS                       R3 R0 K6 ["Contexts"]
       18 GETTABLEKS                       R3 R3 K8 ["GraphDropdownContext"]
       20 CALL                             R2 1 1
       21 GETIMPORT                        R3 K5 [require]
       23 GETTABLEKS                       R4 R0 K6 ["Contexts"]
       25 GETTABLEKS                       R4 R4 K9 ["GraphPlayedContext"]
       27 CALL                             R3 1 1
       28 GETIMPORT                        R4 K5 [require]
       30 GETTABLEKS                       R5 R0 K6 ["Contexts"]
       32 GETTABLEKS                       R5 R5 K10 ["NativeGraphContext"]
       34 CALL                             R4 1 1
       35 GETIMPORT                        R5 K5 [require]
       37 GETTABLEKS                       R6 R0 K11 ["Parent"]
       39 GETTABLEKS                       R6 R6 K12 ["React"]
       41 CALL                             R5 1 1
       42 GETIMPORT                        R6 K5 [require]
       44 GETTABLEKS                       R7 R0 K11 ["Parent"]
       46 GETTABLEKS                       R7 R7 K13 ["ReactUtils"]
       48 CALL                             R6 1 1
       49 GETIMPORT                        R7 K5 [require]
       51 GETTABLEKS                       R8 R0 K6 ["Contexts"]
       53 GETTABLEKS                       R8 R8 K14 ["RigListContext"]
       55 CALL                             R7 1 1
       56 GETIMPORT                        R8 K5 [require]
       58 GETTABLEKS                       R9 R0 K6 ["Contexts"]
       60 GETTABLEKS                       R9 R9 K15 ["SelectedGraphContext"]
       62 CALL                             R8 1 1
       63 DUPCLOSURE                       R9 K16 [PROTO_0]
       64 CAPTURE                          VAL R5
       65 CAPTURE                          VAL R6
       66 CAPTURE                          VAL R8
       67 CAPTURE                          VAL R1
       68 CAPTURE                          VAL R3
       69 CAPTURE                          VAL R7
       70 CAPTURE                          VAL R4
       71 CAPTURE                          VAL R2
       72 RETURN                           R9 1

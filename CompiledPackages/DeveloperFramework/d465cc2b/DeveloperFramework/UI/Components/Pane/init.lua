...
function Pane:render()
	...
	if supportsStyleSheets then
		componentProps[React.Tag] = joinTags(
			...
			tags,
			deprecatedStyle,
			if FFlagDeveloperFrameworkMigrateStylingV2 and componentProps.Size == DEFAULT_SIZE
				then "X-DefaultSize"
				else nil,
			if FFlagDeveloperFrameworkMigrateStylingV2
					and componentProps.BackgroundTransparency == DEFAUlT_TRANSPARENCY
				then "X-DefaultTransparency"
				else nil
		)
	end

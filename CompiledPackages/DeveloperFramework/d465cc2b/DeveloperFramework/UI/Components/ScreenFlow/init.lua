...
local FFlagInitialOnScreenChanged = game:DefineFastFlag("InitialOnScreenChanged", false)
...
function ScreenFlow:init()
	...
	if FFlagInitialOnScreenChanged then
		self.OnScreenChanged = if useCustomBehavior then self.props.OnScreenChanged else self.OnScreenChangedDefault
	end

	self.GetNextIndexDefault = function(currentIndex)
		local props = self.props
		local screens = props.Screens
		assert(#screens > 0, "No screens")
	...
	if FFlagInitialOnScreenChanged then
		self.OnScreenChanged(self.state.currentScreenIndex)
	end
end
...
function ScreenFlow:render()
	...
	assert(currentScreen, "No currentScreen")

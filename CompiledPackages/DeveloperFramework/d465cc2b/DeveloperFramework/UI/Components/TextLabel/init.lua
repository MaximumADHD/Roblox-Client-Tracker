...
function TextLabel:render()
	...
	if supportsStyleSheets then
		textLabelProps[React.Tag] = joinTags(
			...
			tags,
			deprecatedStyle,
			if FFlagDeveloperFrameworkMigrateStylingV2 and size == DEFAULT_SIZE then "X-DefaultSize" else nil,
			if FFlagDeveloperFrameworkMigrateStylingV2 and backgroundTransparency == DEFAUlT_TRANSPARENCY

...
local function Spritesheet(image, props)
	...
	assert(
		spriteSizeType == "number" or spriteSizeType == "Vector2",
		"SpriteSize must be number or Vector2. Got type '" .. spriteSizeType .. "'"
	)
	assert(spriteCountType == "number", "NumSprites must be number. Got type'" .. spriteCountType .. "'")
	assert(
		sheetWidthType == "number" or sheetWidthType == "nil",
	...
	assert(
		spriteSize.X > 0 and spriteSize.Y > 0,
		"SpriteSize does not support <= 0 values. Got '" .. tostring(spriteSize) .. "'"
	)
	assert(numSprites > 0, "NumSprites must be > 0. Got '" .. numSprites)
	assert(sheetWidth > 0, "SpritesheetWidth does not support <= 0 values. Got '" .. sheetWidth .. "'")

...
return function(networkingImpl, baseUrl)
	...
	return function(placeId, body)
		assert(type(placeId) == "number", string.format("placeId must be number, received %s", type(placeId)))
		assert(type(body) == "table", string.format("body must be of type table, received %s", type(body)))

...
local FFlagDevFrameworkPluginWidgetCheckPluginGui = game:DefineFastFlag("DevFrameworkPluginWidgetCheckPluginGui", false)
local FFlagDevFrameworkPortalDesignInPlugins =
	require(Framework.SharedFlags.getFFlagDevFrameworkPortalDesignInPlugins)()
...
local function createPluginWidget(componentName, createWidgetFunc)
	...
	function PluginWidget:updateWidget(prevProps)
		...
		if widget then
			...
			if
				(not FFlagDevFrameworkPluginWidgetCheckPluginGui or widget:IsA("PluginGui"))
				and StudioWidgetsService ~= nil
				and props.Size ~= prevProps.Size
			then
	...
	function PluginWidget:render()
		...
		return ContextServices.provide({
			...
			Child = Roact.createElement(
				...
				if FFlagDevFrameworkPortalDesignInPlugins and self.props.PluginDesign
					then {

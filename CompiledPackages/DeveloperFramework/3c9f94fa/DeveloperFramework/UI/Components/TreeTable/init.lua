...
local FFlagLuaStartPageTableContextMenu = require(Framework.SharedFlags.getFFlagLuaStartPageTableContextMenu)()
...
local FFlagDevFrameworkFixTreeTableRightClick =
	require(Framework.SharedFlags.getFFlagDevFrameworkFixTreeTableRightClick)()
...
function TreeTable:getCellProps()
	...
	return {
		...
		OnCellRightClick = if FFlagDevFrameworkFixTreeTableRightClick then self.onRightClickRow else nil,
...
function TreeTable:render()
	...
	local tableProps = {
		...
		[Roact.Ref] = if FFlagLuaStartPageTableContextMenu then props.ForwardRef else nil,
	}
	if supportsStyleSheets then
		tableProps[React.Tag] = joinTags(
			...
			props[React.Tag],
			props.Variant,
			if FFlagLuaStartPageTableContextMenu and not props.DisableHover then "enable-hover" else nil
		)
	end
...
if FFlagLuaStartPageTableContextMenu then

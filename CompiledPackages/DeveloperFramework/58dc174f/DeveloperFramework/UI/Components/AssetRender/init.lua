...
local getFFlagAssetPreviewHide3DPreview = require(Framework.SharedFlags.getFFlagAssetPreviewHide3DPreview)
local getFFlagDevFrameworkDisableAnimation = require(Framework.SharedFlags.getFFlagDevFrameworkDisableAnimation)
...
function AssetRender:render()
	...
	local isFontFamily = props.AssetData.Asset.TypeId == Enum.AssetType.FontFamily.Value
	local canHaveMeshDependencies = nil
	local isDecal = nil
	if getFFlagAssetPreviewHide3DPreview() then
		canHaveMeshDependencies = props.AssetData.Asset.TypeId == Enum.AssetType.Model.Value
			or props.AssetData.Asset.TypeId == Enum.AssetType.MeshPart.Value
		isDecal = props.AssetData.Asset.TypeId == Enum.AssetType.Decal.Value
	end
	...
	if props.UseImageCarousel then
		renderView = Roact.createElement(ImageCarousel, {
			...
			DisableAnimations = if getFFlagDevFrameworkDisableAnimation() then true else nil,
		})
	elseif
		isModelWithoutInstance
		or isPlugin
		or (getFFlagAssetPreviewHide3DPreview() and props.Hide3DPreviewForAssetsWithMeshes and canHaveMeshDependencies)
	then
	...
	elseif renderType == AssetRenderType.Image then
		renderView = Roact.createElement(AssetRenderImage, {
			Image = if getFFlagAssetPreviewHide3DPreview() and isDecal

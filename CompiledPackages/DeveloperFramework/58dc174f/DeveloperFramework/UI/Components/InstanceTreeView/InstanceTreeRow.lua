...
local FFlagDevFrameworkFixTreeViewHover = require(Framework.SharedFlags.getFFlagDevFrameworkFixTreeViewHover)()
...
function InstanceTreeRow:render()
	...
	-- TODO with removal of FFlagDevFrameworkFixTreeViewHover, move this inline into the TextButton component
	local updatedBackgroundTransparency = nil
	if FFlagDevFrameworkFixTreeViewHover then
		updatedBackgroundTransparency = if self.state.isHovering or isSelected then 0 else 1
	end
	...
	return Roact.createElement(Pane, {
		...
		Label = Roact.createElement("TextButton", {
			...
			BackgroundTransparency = if FFlagDevFrameworkFixTreeViewHover
				then updatedBackgroundTransparency
				else self.state.isHovering or isSelected and 0 or 1,
			BorderSizePixel = 0,

...
function ResponsiveGrid:init()
	...
	self.ref = props.ForwardRef or Roact.createRef()
	...
	self.initializeCutOffs = function()
		...
		table.sort(keys, function(keyLhs, keyRhs)
			return cutOffs[keyLhs].MinWidth < cutOffs[keyRhs].MinWidth
		end)
		...
		self:setState(function()
			...
			for k in pairs(self.sortedCutOffList) do
	...
	self.updateCurrentCutOff = function()
		self:setState(function(prevState)
			...
			local width = ref.AbsoluteSize.X
			local usePrevCutOff = prevCutOffWidth
				and prevCutOffWidth.MinWidth <= width
				and currentCutOff.MinWidth > width
...
return withForwardRef(ResponsiveGrid)

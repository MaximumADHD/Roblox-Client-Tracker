--[[
	...
	Optional Props:
		...
		callback OnRightClick: An optional callback to determine behavior when right-clicking an item
]]
		...
		OnCheck = function(item)
			self.props.OnCheck(
				buildChange(
					...
					self.props.ToggleAncestors,
					self.props.ToggleDescendants
				)
			)
		end,
	}
...
function CheckboxTreeView:didUpdate(previousProps)
	self.ancestry = buildAncestry(self.props.RootItems, self.props.GetChildren)
...
function CheckboxTreeView:render()
	assert(
		(self.props.Expansion and self.props.OnExpansionChange)
			or (not self.props.Expansion and not self.props.OnExpansionChange),
	...
	local rowProps = join(self.rowProps, {
		...
		OnSecondaryPress = self.props.OnRightClick,
	})

...
return function()
	...
	return {
		...
		PausedLoading = Roact.createElement(
			StatelessAudioPlayer,
			Immutable.JoinDictionaries(baseProps, {
				IsLoaded = false,
			})
		),
		Playing = Roact.createElement(
			StatelessAudioPlayer,
			Immutable.JoinDictionaries(baseProps, {
				IsPlaying = true,
			})
		),
		PlayingProgress = Roact.createElement(
			StatelessAudioPlayer,
			Immutable.JoinDictionaries(baseProps, {
				IsPlaying = true,
				CurrentTime = 59,
		...
		PlayingLoading = Roact.createElement(
			StatelessAudioPlayer,
			Immutable.JoinDictionaries(baseProps, {
				IsPlaying = true,
				IsLoaded = false,

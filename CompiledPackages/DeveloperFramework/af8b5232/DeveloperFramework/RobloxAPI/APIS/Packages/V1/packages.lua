--[[
	...
	Example
	https://apis.roblox.com/packages-api/v1/universes/1552514118/packages

	Documented at : https://apis.roblox.com/docs/
...
return function(networkingImpl, baseUrl)
	...
	return function(universeId, cursor, limit, sortOrder)
		...
		if cursor then
			assert(type(cursor) == "string", string.format("cursor must be number, received %s", type(cursor)))
		end
		limit = limit or 10
		assert(type(limit) == "number", string.format("limit must be number, received %s", type(limit)))
		sortOrder = sortOrder or "Asc"
		assert(type(sortOrder) == "string", string.format("sortOrder must be string, received %s", type(sortOrder)))

		local url =
			baseUrl.composeUrl(baseUrl.APIS_URL, string.format("packages-api/v1/universes/%d/packages", universeId), {
				limit = limit,
				sortOrder = sortOrder,
				cursor = cursor,
			})

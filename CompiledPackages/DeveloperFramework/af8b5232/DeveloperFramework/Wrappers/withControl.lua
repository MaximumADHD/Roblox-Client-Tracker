--[[
	...
	depending on whether the component is hovered, selected, pressed or disabled.
...
function Controllable:init()
	...
	self.onInputBegan = function(instance, input)
		if isInputMainPress(input) then
			self:setState({
				isPressing = true,
			})
			if self.props.ComponentProps.OnPress then
				self.props.ComponentProps.OnPress(self.props.ComponentProps, instance, input)
			end
			if self.props.ComponentProps.OnDoubleClick and self.DoubleClickDetector:isDoubleClick() then
				self.props.ComponentProps.OnDoubleClick(self.props.ComponentProps, instance, input)
			end
		elseif input.UserInputType == Enum.UserInputType.MouseButton2 then
			if self.props.ComponentProps.OnSecondaryPress then
				self.props.ComponentProps.OnSecondaryPress(self.props.ComponentProps, instance, input)
			end
		end
	end
	self.onInputEnded = function(instance, input)
		if isInputMainPress(input) then
			self:setState({
				isPressing = false,
			})
			if self.props.ComponentProps.OnPressEnd then
				self.props.ComponentProps.OnPressEnd(self.props.ComponentProps, instance, input)
			end
		end
	end
end

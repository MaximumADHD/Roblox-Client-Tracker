...
local RESTRICTED_useDialog = require(script.Parent)

local ALL_INTENTS: { RESTRICTED_useDialog.Intent } = {
	"Confirmation",
	"Destructive",
	"Error",
	"Information",
...
local function IntentStory(props: { Intent: RESTRICTED_useDialog.Intent })
	local detailsAction, withDetails =
		useDialogDetailsAction("Here are the details:\n- This is a detail\n- You rock!\n- Have a nice day!")

	local show = RESTRICTED_useDialog({
		Intent = props.Intent,
...
for _, intent: RESTRICTED_useDialog.Intent in ALL_INTENTS do

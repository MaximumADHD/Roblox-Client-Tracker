...
function DragBox:render()
	...
	local maxKnobBackgroundStyle = style.KnobBackgroundStyle or {}
	...
	local shouldUseHighlightKnobStyle = foregroundStyle ~= nil and (percentX ~= 0.5 or percentY ~= 0.5)
	...
	return Roact.createElement("Frame", {
		...
		Knob = Roact.createElement(Container, {
			...
			ZIndex = 3,
			--show knob colored when component has non defaault value:
			ImageColor3 = if shouldUseHighlightKnobStyle then foregroundStyle.Color else maxKnobBackgroundStyle.Color,
		}),
		Tooltip = hasTooltip and Roact.createElement(Tooltip, {
			...
			--needed when sliders close together like in Face Animation Editor
			ZIndex = 5,
		}),
		...
		ClickHandler = not isDisabled and Roact.createElement("ImageButton", {
			...
			Size = UDim2.new(
				1,
				self.state.pressed and horizontalDragBuffer or knobSize.X,
				1,
				self.state.pressed and verticalDragBuffer or 0
			),
			ZIndex = 4,

...
local getFFlagDevFrameworkLoadingBarAddOvertime =
	require(Framework.SharedFlags.getFFlagDevFrameworkLoadingBarAddOvertime)
...
function TimeProgressBar:didMount()
	...
	self.updateConnection = RunService.RenderStepped:Connect(function()
		...
		if currentTime > props.TotalTime then
			if getFFlagDevFrameworkLoadingBarAddOvertime() and self.props.CanGoOverTime then
				self.overTime = true
			else
...
function TimeProgressBar:render()
	...
	return Roact.createElement(Pane, {
		...
		LoadingBar = Roact.createElement(LoadingBar, {
			LayoutOrder = 2,
			Progress = progress,
			OverTime = if getFFlagDevFrameworkLoadingBarAddOvertime() and props.CanGoOverTime
				then self.overTime
				else nil,

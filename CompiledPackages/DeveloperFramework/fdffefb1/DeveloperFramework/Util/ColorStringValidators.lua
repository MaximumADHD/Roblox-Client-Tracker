return {
	RGBFromRGBString = function(text)
		return text:match("^%s*(%d+)%s*,%s*(%d+)%s*,%s*(%d+)%s*%s*$")
	end,

	RGBFromHexString = function(text)
		return text:match("^%s*%#?(%x%x)(%x%x)(%x%x)%s*$")
	end,

	HSVFromHSVString = function(text)
		return text:match("^%s*(%d+)%s*,%s*(%d+)%s*,%s*(%d+)%s*%s*$")
	end,

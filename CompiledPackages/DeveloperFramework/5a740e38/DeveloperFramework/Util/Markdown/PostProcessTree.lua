...
function module.buildParagraph(
	...
	while i <= #nodes do
		...
		if PLAIN_TEXT[node.name] then
			local textCount, textNode = module.mergeTextNodes(nodes, i, includeWhitespace)
			-- Only add text nodes that actually have text
			if textNode.text and #textNode.text > 0 then
				insert(children, textNode)
			end
...
function module.mergeTextNodes(nodes: { Node }, offset: number, includeWhitespace: boolean?): (number, Node)
	local text = ""
	local count = 0
	for i = offset, #nodes do
		local node = nodes[i]

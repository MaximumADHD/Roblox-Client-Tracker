...
local FIntScriptStringLimit = game:DefineFastInt("ScriptStringLimit", 200_000)
...
function EmbedFramework.install(root: Instance)
	...
	EmbedFramework.stubScript(framework.UI.ContextServices.Localization)
...
return require(Packages._Index["RoactCompat"]["RoactCompat"])]]
	...
	forEach(react17Packages, function(libraryName: string)
		...
		print("Install React 17 package", libraryName)
		local embeddedLibrary = library:Clone()
		embeddedLibrary.Parent = index
	end)
	EmbedFramework.installPeerDependency(root, pluginPackages, "React")
	EmbedFramework.installPeerDependency(root, pluginPackages, "ReactIs")
	EmbedFramework.installPeerDependency(root, pluginPackages, "ReactRoblox")
	EmbedFramework.installPeerDependency(root, pluginPackages, "Roact")
	EmbedFramework.installPeerDependency(root, pluginPackages, "enumerate")
	EmbedFramework.installPeerDependency(root, pluginPackages, "Dash")
	...
	EmbedFramework.installPeerDependency(root, pluginPackages, "TestLoader")
	EmbedFramework.installPeerDependency(root, pluginPackages, "MaterialFramework")
	EmbedFramework.installPeerDependency(root, pluginPackages, "ViewportToolingFramework")
end
...
function EmbedFramework.installPeerDependency(root: Instance, pluginPackages: Folder, packageName: string)
	...
	local librarySource = pluginPackages:FindFirstChild(packageName)
	if not librarySource then
		warn("Skipping peer library", packageName)
		return
	end
...
local function tryCall<T>(fn: () -> T, default: T)
...
-- There are cases where we need to rewrite dependency specific flags and this helper funciton is usefull
EmbedFramework.rewriteFlag = rewriteFlag

function EmbedFramework.rewriteFlagsForScript(script: ModuleScript)
	if #script.Source > FIntScriptStringLimit then
		warn("Skipping rewriteFlagsForScript for", script.Name, "because it's too large")
		return
	end

	-- These API calls are roblox-security only, so cannot be used in a running game
	script.Source = script.Source
		:gsub([[game:DefineFastInt%(%s*"([^"]+)"[^)]*%)]], rewriteInt)
		...
		:gsub(
			...
			[[({StudioLocaleId = "en-us", GetPropertyChangedSignal = function() return {Connect = function() return {Disconnect = function() end} end} end, HasInternalPermission = function() return true end})]]
		)
		:gsub([[game:GetService%("StudioSdkService"%)]], [[({GetSdk = function() return nil end})]])
end

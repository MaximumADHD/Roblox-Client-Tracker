...
local FFlagDevFrameworkBreadcrumbAddDelimiterToImageLookup =
	require(Framework.SharedFlags.getFFlagDevFrameworkBreadcrumbAddDelimiterToImageLookup)()
	...
	elseif props.Delimiters then
		...
		while #suffix > 0 do
			...
			if props.GetImageProps then
				local componentImage = props.GetImageProps(
					componentText,
					if FFlagDevFrameworkBreadcrumbAddDelimiterToImageLookup then lastFoundDelimiter else nil
				)
			...
			if delimiterStart then
				...
				if FFlagDevFrameworkBreadcrumbAddDelimiterToImageLookup then
...
Breadcrumb.GetSelectorIcon = function(text: string)
	...
	if className then
		...
		if FFlagDevFrameworkSettingsCliFix then
			theme = getStudioThemeNameOrDefault()
		else
			ok, theme = pcall(function()

...
local FFlagDevFrameworkTreeTableCellShowActions =
	require(Framework.SharedFlags.getFFlagDevFrameworkTreeTableCellShowActions)()
local FFlagDevFrameworkCompactTreeTable = require(Framework.SharedFlags.getFFlagDevFrameworkCompactTreeTable)()
local FFlagDevFrameworkTreeTableCellHoverActionsDeficit =
	game:DefineFastFlag("DevFrameworkTreeTableCellHoverActionsDeficit", false)
...
function TreeTableCell:init()
	...
	self.getIndent = function(): number
		...
		if FFlagDevFrameworkCompactTreeTable then
			...
			return if supportsStyleSheets then INDENT else style.Indent
		end
	end
	...
	self.getCellPadding = function(): number
		...
		if FFlagDevFrameworkCompactTreeTable then
			...
			return if supportsStyleSheets then CELL_PADDING else style.CellSpacing
		end
	end
	...
	self.fetchCellContentsWidthIfOverflow = function()
		if self.state.textOverflows then
			...
			local padding
			if FFlagDevFrameworkCompactTreeTable then
				padding = self.getCellPadding() + self.getIndent()
			else
				padding = (if supportsStyleSheets then CELL_PADDING + INDENT else style.CellPadding.Left + style.Indent)
			end
...
function TreeTableCell:render()
	...
	local isCompact
	if FFlagDevFrameworkCompactTreeTable then
		isCompact = props.Variant == "compact"
	end
	...
	if typeof(value) == "table" and value.Actions then
		...
		if FFlagDevFrameworkTreeTableCellShowActions then
			...
			Left = Roact.createElement(
				...
				if supportsStyleSheets
					then {
						...
						Size = if FFlagDevFrameworkTreeTableCellHoverActionsDeficit
							then UDim2.new(1, -leftDeficit - indent, 1, 0)
							else UDim2.new(1, if showActions then -leftDeficit - indent else -indent, 1, 0),
						Position = UDim2.new(0, indent, 0, 0),
						[React.Tag] = if FFlagDevFrameworkCompactTreeTable
							then joinTags("Left X-Middle", if isCompact then "X-RowS" else "X-RowM")
							else "Left X-RowM X-Middle",
					}
					else {
						...
						Size = if FFlagDevFrameworkTreeTableCellHoverActionsDeficit
							then UDim2.new(1, -leftDeficit, 1, 0)
							else UDim2.new(1, if showActions then -leftDeficit else 0, 1, 0),
					},
				{
				...
				then Roact.createElement(
					...
					if supportsStyleSheets
						then {
							...
							[React.Tag] = if FFlagDevFrameworkCompactTreeTable
								then joinTags("X-Fit X-Middle", if isCompact then "X-RowS" else "X-RowM")
								else "X-RowM X-Fit X-Middle",
						}
						else {
							AnchorPoint = Vector2.new(1, 0.5),
							AutomaticSize = Enum.AutomaticSize.XY,

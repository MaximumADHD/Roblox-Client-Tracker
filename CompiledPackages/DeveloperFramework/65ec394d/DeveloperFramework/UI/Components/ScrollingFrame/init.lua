...
local FFlagDeveloperFrameworkMigrateStylingV2 = SharedFlags.getFFlagDeveloperFrameworkMigrateStylingV2()
...
function ScrollingFrame:init()
	...
	self.getScrollingFrameProps = function(props, style)
		...
		local size = if FFlagDeveloperFrameworkMigrateStylingV2
			then if scrollingFrameSize ~= nil then scrollingFrameSize else UDim2.fromScale(scaleX, scaleY)
			else nil
		...
		return Cryo.Dictionary.join(
			if supportsStyleSheets
				then {
					[React.Tag] = if FFlagDeveloperFrameworkMigrateStylingV2 and size == DEFAULT_SIZE
						then "X-DefaultSize"
						else nil,
				}
				else style,
			...
			self.propFilters.parentContainerProps,
			{
				...
				Size = if FFlagDeveloperFrameworkMigrateStylingV2
					then size
					else if scrollingFrameSize ~= nil then scrollingFrameSize else UDim2.fromScale(scaleX, scaleY),

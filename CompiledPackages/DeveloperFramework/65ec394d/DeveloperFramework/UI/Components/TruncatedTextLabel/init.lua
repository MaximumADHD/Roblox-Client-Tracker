...
function TruncatedTextLabel:init()
	...
	self.getTextWidth = function(text)
		...
		return TextService:GetTextSize(
			text,
			if FFlagDeveloperFrameworkMigrateStylingV2 then textRef:GetStyled("TextSize") else textRef.TextSize,
			if FFlagDeveloperFrameworkMigrateStylingV2 then textRef:GetStyled("Font") else textRef.Font,
	...
	self.updateText = function()
		...
		if text ~= self.lastText or maxWidth ~= self.lastTextWidth then
			textRef.Text = ellipsizeMiddle(text, maxWidth, self.getTextWidth, suffixLength)
			self.lastText = text

...
local React = require(DevFrameworkRoot.Util.React)
...
return function(contextItemsList: { any }?, children: { [string]: React.Node }?)
	if contextItemsList then
		assert(type(contextItemsList) == "table", "Expected contextItemsList to be a table.")
		assert(
			type((next(contextItemsList))) == "number" or type((next(contextItemsList))) == "nil",
	...
	-- Store
	local storeObject = Rodux.Store.new(function() end)
	...
	-- Design
	local design = ContextServices.Design.mock(Instance.new("StyleSheet"))
	table.insert(contextItems, design)

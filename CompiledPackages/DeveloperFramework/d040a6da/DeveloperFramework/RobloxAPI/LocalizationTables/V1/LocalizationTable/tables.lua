...
return function(networkingImpl, baseUrl)
	...
	return function(tableId, gameId, body)
		assert(type(tableId) == "string", string.format("tableId must be string, received %s", type(tableId)))
		assert(type(gameId) == "number", string.format("gameId must be number, received %s", type(gameId)))
		assert(type(body) == "string", string.format("patch must be string, received %s", type(body)))

--[[
	...
	Optional Props:
		...
		integer ZIndex: The render index of this component.
...
local LoadingIndicator = require(UI.Components.LoadingIndicator)
...
local function getAbsoluteCellSize(cellSize: UDim2, cellPadding: UDim2, absoluteSize: Vector2): Vector2
	local scalePixels = Vector2.new(
		math.floor((cellSize.X.Scale + cellPadding.X.Scale) * absoluteSize.X),
		math.floor((cellSize.Y.Scale + cellPadding.Y.Scale) * absoluteSize.Y)
	)
	return Vector2.new(
		scalePixels.X + cellSize.X.Offset + cellPadding.X.Offset,
		scalePixels.Y + cellSize.Y.Offset + cellPadding.Y.Offset
	)
end
...
function InfiniteScrollingGrid:init()
	...
	self.onUpdateView = function(canvasPosition: Vector2, absoluteSizeChanged: boolean?)
		...
		if state.canvasPosition and not absoluteSizeChanged then
		...
		if minItemIndex ~= state.minItemIndex or maxItemIndex ~= state.maxItemIndex then
			...
			minItemIndex = (cellsPerRow * minRowIndex) + 1
			maxItemIndex = (cellsPerRow * maxRowIndex) + absoluteMax % cellsPerRow
			...
			if
				minItemIndex ~= state.minItemIndex
				or maxItemIndex ~= state.maxItemIndex
				or targetMinItemIndex ~= targetMinItemIndex
				or targetMaxItemIndex ~= targetMaxItemIndex
			then
				self:setState({
	...
	self.onBoundsUpdated = function()
		...
		local absoluteTargetMinItemIndex = math.min(
			targetMinItemIndex,
			absoluteMax - absoluteMax % state.cellsPerRow - (targetMaxItemIndex - targetMinItemIndex)
		)
...
function InfiniteScrollingGrid:didUpdate(prevProps: any)
	...
	local absoluteCellPadding = getAbsoluteCellPadding(cellPadding, absoluteSize)
	local cellsPerRow = getCellsPerRow(absoluteSize, absoluteCellSize, absoluteCellPadding)
	local cellSizeChanged = prevProps.CellSize.X.Offset ~= cellSize.X.Offset
		or prevProps.CellSize.Y.Offset ~= cellSize.Y.Offset

	if prevProps.Items ~= self.props.Items or cellSizeChanged then
		self:setState({
	...
	if cellsPerRow ~= 0 then
		local scrollFrameRefValue = self.scrollingFrameRef:getValue()
		if
			not state.absoluteSize
			or state.absoluteSize.X ~= absoluteSize.X
			or state.absoluteSize.Y ~= absoluteSize.Y
			or cellSizeChanged
			or (scrollFrameRefValue and scrollFrameRefValue.CanvasPosition ~= state.canvasPosition)
		then
			local canvasPosition = if scrollFrameRefValue
				then scrollFrameRefValue.CanvasPosition
				else state.canvasPosition
			self.onUpdateView(canvasPosition or Vector2.new(0, 0), true)
		end
	end
		...
		}, props.WrapperProps),
		{
			scrollingFrame = Roact.createElement(ScrollingFrame, {
				...
				Loader = Roact.createElement(Pane, {
					...
					Size = UDim2.fromOffset(0, padding or 0),
				}, {
					Indicator = indicator,
				}),
			}),

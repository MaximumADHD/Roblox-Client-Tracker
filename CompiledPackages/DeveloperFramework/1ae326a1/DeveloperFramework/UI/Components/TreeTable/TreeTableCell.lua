...
function TreeTableCell:init()
	...
	if FFlagDevFrameworkFixTreeTableRightClick then
	...
	self.getPropertyCell = function(props)
		...
		return Roact.createElement(PropertyCell, {
			...
			OnRightClick = if FFlagDevFrameworkFixTreeTableRightClick then props.OnRightClick else nil,
			Renderers = renderers,
			Schema = props.Schema,
					...
					},
				{
							...
							Icon = Roact.createElement(
								...
								if supportsStyleSheets
									then {
										...
										[Roact.Event.InputBegan] = if FFlagDevFrameworkFixTreeTableRightClick
											then self.onCellInputBegan
											else nil,
									}
									else {
					...
					Text = if hasSchema
						then self.getPropertyCell({
							...
							OnRightClick = if FFlagDevFrameworkFixTreeTableRightClick
								then self.onCellRightClick
								else nil,

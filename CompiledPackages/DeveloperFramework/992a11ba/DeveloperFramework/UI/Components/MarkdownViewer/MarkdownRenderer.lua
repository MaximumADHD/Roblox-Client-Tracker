...
local FFlagDevFrameworkMarkdownNewCodeblockComponent =
	require(Framework.SharedFlags.getFFlagDevFrameworkMarkdownNewCodeblockComponent)()
local FFlagDevFrameworkMarkdownControlCodeFont =
	require(Framework.SharedFlags.getFFlagDevFrameworkMarkdownControlCodeFont)()
local FFlagDevFrameworkMarkdownInlineCodeSelectable =
	require(Framework.SharedFlags.getFFlagDevFrameworkMarkdownInlineCodeSelectable)()
...
local function getCodeTextStyles(context: Context): TextStyles
	assert(FFlagDevFrameworkMarkdownControlCodeFont, "Code text styles are only available when the flag is enabled")
...
local function getBaseTextElementTextStyle(props: TextStyles?): BaseTextElement.BaseTextElementTextStyle?
	if props then
		if FFlagDevFrameworkMarkdownControlCodeFont then
			return {
				Bold = props.bold,
				Italic = props.italic,
				Strike = props.strike,
...
local function renderCodeBlock_DEPRECATED(node: Types.Node, context: Context): React.ReactElement
	...
	local backgroundTransparency = backgroundStyles.backgroundTransparency or nil

	return React.createElement("Frame", {
		key = tostring(node.index),
		LayoutOrder = node.index,
		[React.Tag] = "X-FitY X-Transparent CodeBlock",
	}, {
...
local function renderStyledText(node: Types.Node, context: Context): { React.ReactElement }
	...
	elseif FFlagDevFrameworkMarkdownCommonMarkCompliance and style == "INLINE_CODE" then
		styleProps = if FFlagDevFrameworkMarkdownControlCodeFont then context.codeStyles else { font = Enum.Font.Code }

		if FFlagDevFrameworkMarkdownInlineCodeSelectable then
...
local function renderCodeSpan(node: Types.Node, context: Context): React.ReactElement
	local textStyles = if FFlagDevFrameworkMarkdownControlCodeFont
		then context.codeStyles
		else { font = Enum.Font.Code }

	if FFlagDevFrameworkMarkdownInlineCodeSelectable then
		textStyles = Dash.joinDeep(textStyles, { selectable = true })
	end

	return renderText(node, mergeContexts(context, { textStyles = textStyles }))
end
...
MarkdownRenderer.defaultRenderers = {
	...
	["CODE_BLOCK"] = if FFlagDevFrameworkMarkdownNewCodeblockComponent
		then renderCodeBlock
		else renderCodeBlock_DEPRECATED,
	["STYLED_TEXT"] = renderStyledText,
	["TEXT"] = renderText,
	["INLINE_CODE"] = if FFlagDevFrameworkMarkdownCommonMarkCompliance then nil else renderCodeSpan,
	["LINK"] = renderLink,
	["LIST"] = renderList,
	["LIST_ITEM"] = renderListItem,
	["DIVIDER"] = renderThematicBreak,
	["DIVIDER_2"] = if FFlagDevFrameworkMarkdownCommonMarkCompliance then nil else renderThematicBreak,
...
function MarkdownRenderer.render(node: Types.Node, props: Props): React.ReactElement?
	...
	local intitialContext: Context
	if FFlagDevFrameworkMarkdownControlCodeFont then
		intitialContext = Dash.joinDeep({
			textStyles = {

...
Check the definition of the component %q."]
  ...
  LOADK R16 K31 ["%s: type specification of %s `%s` is invalid; the type checker function must return `nil` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument)."]
  ORK R17 R4 K32 ["React class"]
  MOVE R18 R3
  MOVE R19 R9
  FASTCALL1 TYPEOF R12 [+3]
  MOVE R21 R12
  ...
  JUMPIFNOT R13 [+71]
  GETUPVAL R15 4
  GETTABLEKS R16 R12 K33 ["message"]
  ...
  GETIMPORT R15 K30 [string.format]
  LOADK R16 K34 ["Failed %s type: %s"]
  MOVE R17 R3
  GETTABLEKS R19 R12 K33 ["message"]

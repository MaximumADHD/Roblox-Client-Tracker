PROTO_0:
        0 DUPTABLE                         R1 K4 [{[1] = True, ["LocalScript"] = True, ["ModuleScript"] = True}]
        1 NEWTABLE                         R2 0 0
        3 GETIMPORT                        R3 K7 [string.gmatch]
        5 MOVE                             R4 R0
        6 LOADK                            R5 K8 ["[^%s,]+"]
        7 CALL                             R3 2 3
        8 FORGPREP                         R3
        9 MOVE                             R9 R2
       10 NAMECALL                         R10 R6 K9 ["lower"]
       12 CALL                             R10 1 -1
       13 FASTCALL                         TABLE_INSERT ; [+2]
       14 GETIMPORT                        R8 K12 [table.insert]
       16 CALL                             R8 -1 0
       17 FORGLOOP                         R3 1 ; [-9]
       19 NEWTABLE                         R3 0 0
       21 NEWTABLE                         R4 0 0
       23 GETIMPORT                        R5 K14 [game]
       25 NAMECALL                         R5 R5 K15 ["GetDescendants"]
       27 CALL                             R5 1 3
       28 FORGPREP                         R5
       29 GETTABLEKS                       R11 R9 K16 ["ClassName"]
       31 GETTABLE                         R10 R1 R11
       32 JUMPIFNOT                        R10 ; [+57]
       33 LENGTH                           R10 R4
       34 LOADN                            R11 10
       35 JUMPIFNOTLT                      R10 R11 ; [+54]
       37 NAMECALL                         R10 R9 K17 ["GetFullName"]
       39 CALL                             R10 1 1
       40 GETUPVAL                         R11 0
       41 GETTABLEKS                       R11 R11 K18 ["isPathExcluded"]
       43 MOVE                             R12 R10
       44 CALL                             R11 1 1
       45 JUMPIF                           R11 ; [+44]
       46 GETTABLEKS                       R11 R9 K19 ["Name"]
       48 NAMECALL                         R11 R11 K9 ["lower"]
       50 CALL                             R11 1 1
       51 MOVE                             R12 R2
       52 LOADNIL                          R13
       53 LOADNIL                          R14
       54 FORGPREP                         R12
       55 MOVE                             R19 R16
       56 LOADN                            R20 1
       57 LOADB                            R21 1
       58 NAMECALL                         R17 R11 K20 ["find"]
       60 CALL                             R17 4 1
       61 JUMPIFNOT                        R17 ; [+26]
       62 MOVE                             R18 R10
       63 LOADK                            R19 K21 ["|"]
       64 GETTABLEKS                       R20 R9 K16 ["ClassName"]
       66 CONCAT                           R17 R18 R20
       67 GETTABLE                         R18 R3 R17
       68 JUMPIF                           R18 ; [+21]
       69 LOADK                            R18 K22 ["Name: %* | Type: %* | Path: %*"]
       70 GETTABLEKS                       R20 R9 K19 ["Name"]
       72 GETTABLEKS                       R21 R9 K16 ["ClassName"]
       74 MOVE                             R22 R10
       75 NAMECALL                         R18 R18 K23 ["format"]
       77 CALL                             R18 4 1
       78 FASTCALL2                        TABLE_INSERT R4 R18 ; [+5]
       80 MOVE                             R20 R4
       81 MOVE                             R21 R18
       82 GETIMPORT                        R19 K12 [table.insert]
       84 CALL                             R19 2 0
       85 LOADB                            R19 1
       86 SETTABLE                         R19 R3 R17
       87 JUMP                             ; [+2]
       88 FORGLOOP                         R12 2 ; [-34]
       90 FORGLOOP                         R5 2 ; [-62]
       92 LENGTH                           R5 R4
       93 JUMPIFNOTEQKN                    R5 K24 [0] ; [+3]
       95 DUPTABLE                         R5 K27 [{["text"] = "No matches found for the given query."}]
       96 RETURN                           R5 1
       97 LENGTH                           R5 R4
       98 LENGTH                           R6 R4
       99 LOADN                            R7 10
      100 JUMPIFNOTLE                      R7 R6 ; [+8]
      102 GETUPVAL                         R8 1
      103 FASTCALL2                        TABLE_INSERT R4 R8 ; [+4]
      105 MOVE                             R7 R4
      106 GETIMPORT                        R6 K12 [table.insert]
      108 CALL                             R6 2 0
      109 DUPTABLE                         R6 K29 [{"text", "count"}]
      110 GETIMPORT                        R7 K31 [table.concat]
      112 MOVE                             R8 R4
      113 LOADK                            R9 K32 ["\n"]
      114 CALL                             R7 2 1
      115 SETTABLEKS                       R7 R6 K25 ["text"]
      117 SETTABLEKS                       R5 R6 K28 ["count"]
      119 RETURN                           R6 1

PROTO_1:
        0 GETUPVAL                         R2 0
        1 GETTABLEKS                       R3 R1 K0 ["keywords"]
        3 CALL                             R2 1 1
        4 RETURN                           R2 1

PROTO_2:
        0 GETUPVAL                         R1 0
        1 LOADNIL                          R2
        2 MOVE                             R3 R0
        3 CALL                             R1 2 1
        4 GETUPVAL                         R2 1
        5 CALL                             R2 0 1
        6 GETTABLEKS                       R4 R1 K0 ["text"]
        8 NAMECALL                         R2 R2 K1 ["addText"]
       10 CALL                             R2 2 1
       11 DUPTABLE                         R4 K4 [{"count", "keywords"}]
       12 GETTABLEKS                       R6 R1 K2 ["count"]
       14 ORK                              R5 R6 K5 [0]
       15 SETTABLEKS                       R5 R4 K2 ["count"]
       17 GETTABLEKS                       R5 R0 K3 ["keywords"]
       19 SETTABLEKS                       R5 R4 K3 ["keywords"]
       21 NAMECALL                         R2 R2 K6 ["setStructuredContent"]
       23 CALL                             R2 2 1
       24 NAMECALL                         R2 R2 K7 ["build"]
       26 CALL                             R2 1 -1
       27 RETURN                           R2 -1

PROTO_3:
        0 GETTABLEKS                       R1 R0 K0 ["networking"]
        2 LOADK                            R4 K1 ["FileSearchTool_searchScript"]
        3 DUPCLOSURE                       R5 K2 [PROTO_1]
        4 CAPTURE                          UPVAL U0
        5 NAMECALL                         R2 R1 K3 ["OnHostInvokeAsync"]
        7 CALL                             R2 3 1
        8 NEWCLOSURE                       R3 P1
        9 CAPTURE                          VAL R2
       10 CAPTURE                          UPVAL U1
       11 GETUPVAL                         R4 2
       12 GETTABLEKS                       R4 R4 K4 ["define"]
       14 CALL                             R4 0 1
       15 GETUPVAL                         R6 3
       16 GETTABLEKS                       R6 R6 K5 ["FileSearch"]
       18 NAMECALL                         R4 R4 K6 ["setName"]
       20 CALL                             R4 2 1
       21 LOADK                            R6 K7 ["Fast script search based on fuzzy matching against script names.\nUse if you know part of the script name but don't know where it's located exactly.\nResponse will be capped to 10 results.\nMake your query more specific if need to filter results further.\nNote: Pattern matching is not supported such as asterisk (*) or question mark (?) wildcards.\n"]
       22 NAMECALL                         R4 R4 K8 ["setDescription"]
       24 CALL                             R4 2 1
       25 LOADK                            R6 K9 ["keywords"]
       26 DUPTABLE                         R7 K14 [{["type"] = "string", ["description"] = "The comma-separated keywords string to search for in the game's scripts. Each keyword is case-insensitive."}]
       27 NAMECALL                         R4 R4 K15 ["addArgument"]
       29 CALL                             R4 3 1
       30 MOVE                             R6 R3
       31 NAMECALL                         R4 R4 K16 ["setHandler"]
       33 CALL                             R4 2 1
       34 DUPTABLE                         R6 K25 [{["title"] = "File Search", ["readOnlyHint"] = True, ["destructiveHint"] = False, ["idempotentHint"] = True, ["openWorldHint"] = False}]
       35 NAMECALL                         R4 R4 K26 ["setAnnotations"]
       37 CALL                             R4 2 1
       38 NAMECALL                         R4 R4 K27 ["build"]
       40 CALL                             R4 1 1
       41 DUPTABLE                         R5 K29 [{"definition"}]
       42 SETTABLEKS                       R4 R5 K28 ["definition"]
       44 RETURN                           R5 1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [script]
        3 LOADK                            R2 K2 ["AssistantUI"]
        4 NAMECALL                         R0 R0 K3 ["FindFirstAncestor"]
        6 CALL                             R0 2 1
        7 GETIMPORT                        R1 K5 [require]
        9 GETTABLEKS                       R2 R0 K6 ["Parent"]
       11 GETTABLEKS                       R2 R2 K7 ["ModelContextProtocol"]
       13 CALL                             R1 1 1
       14 GETIMPORT                        R2 K5 [require]
       16 GETTABLEKS                       R3 R0 K8 ["Tools"]
       18 GETTABLEKS                       R3 R3 K9 ["ToolTypes"]
       20 CALL                             R2 1 1
       21 GETIMPORT                        R3 K5 [require]
       23 GETTABLEKS                       R4 R0 K10 ["Util"]
       25 GETTABLEKS                       R4 R4 K11 ["ToolUtils"]
       27 CALL                             R3 1 1
       28 GETTABLEKS                       R4 R1 K10 ["Util"]
       30 GETTABLEKS                       R4 R4 K12 ["ToolBuilder"]
       32 GETTABLEKS                       R5 R1 K10 ["Util"]
       34 GETTABLEKS                       R5 R5 K13 ["ToolResult"]
       36 GETTABLEKS                       R6 R2 K14 ["ToolNames"]
       38 LOADK                            R7 K15 ["... Search stopped after reaching the limit of %* matches."]
       39 LOADN                            R9 10
       40 NAMECALL                         R7 R7 K16 ["format"]
       42 CALL                             R7 2 1
       43 DUPCLOSURE                       R8 K17 [PROTO_0]
       44 CAPTURE                          VAL R3
       45 CAPTURE                          VAL R7
       46 DUPCLOSURE                       R9 K18 [PROTO_3]
       47 CAPTURE                          VAL R8
       48 CAPTURE                          VAL R5
       49 CAPTURE                          VAL R4
       50 CAPTURE                          VAL R6
       51 RETURN                           R9 1

...
PROTO_6:
       ...
       15 CALL                             R2 2 1
       16 LOADK                            R4 K4 ["Wait for a primitive generation job to finish. Returns the final status and details when the job reaches a terminal state (Completed, Failed, or Cancelled).\n\nDO NOT call this tool automatically after primitive_gen. The primitive_gen tool already handles generation in the background and updates the UI.\n\nOnly call this tool when:\n- The user EXPLICITLY asks to wait for the generation to finish (e.g. \"wait for it to complete\", \"let me know when it's done\")\n- You need to confirm the generation result before performing a follow-up action that depends on it (e.g. the user says \"create a car with primitives and then change its color to red\")"]
       17 NAMECALL                         R2 R2 K5 ["setDescription"]
       19 CALL                             R2 2 1
       20 LOADK                            R4 K6 ["generationId"]
       21 DUPTABLE                         R5 K9 [{"type", "description"}]
       22 LOADK                            R6 K10 ["string"]
       23 SETTABLEKS                       R6 R5 K7 ["type"]
       25 LOADK                            R6 K11 ["The generation ID returned by the primitive_gen tool."]
       26 SETTABLEKS                       R6 R5 K8 ["description"]
       28 NAMECALL                         R2 R2 K12 ["addArgument"]
       30 CALL                             R2 3 1
       31 LOADK                            R4 K13 ["timeout"]
       32 DUPTABLE                         R5 K9 [{"type", "description"}]
       33 LOADK                            R6 K14 ["number"]
       34 SETTABLEKS                       R6 R5 K7 ["type"]
       36 LOADK                            R6 K15 ["Maximum time in seconds to wait for the job to finish. Defaults to 600 (10 minutes)."]
       37 SETTABLEKS                       R6 R5 K8 ["description"]
       39 NAMECALL                         R2 R2 K16 ["addOptionalArgument"]
       41 CALL                             R2 3 1
       42 MOVE                             R4 R1
       43 NAMECALL                         R2 R2 K17 ["setHandler"]
       45 CALL                             R2 2 1
       46 NAMECALL                         R2 R2 K18 ["build"]
       48 CALL                             R2 1 1
       49 DUPTABLE                         R3 K21 [{"definition", "displayNameFunction"}]
       50 SETTABLEKS                       R2 R3 K19 ["definition"]
       52 DUPCLOSURE                       R4 K22 [PROTO_5]
       53 CAPTURE                          UPVAL U7
       54 SETTABLEKS                       R4 R3 K20 ["displayNameFunction"]
       56 RETURN                           R3 1

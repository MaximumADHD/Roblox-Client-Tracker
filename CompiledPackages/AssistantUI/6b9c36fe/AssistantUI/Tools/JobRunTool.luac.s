...
PROTO_2:
        ...
        9 LOADK                            R3 K5 [""]
       10 LOADK                            R5 K6 ["Use this when you want to run a tool (%*) without blocking on its completion — for example when kicking off several generations, or when the user does not need to wait for the result before you continue."]
       11 GETIMPORT                        R7 K3 [table.concat]
       13 GETUPVAL                         R8 1
       14 LOADK                            R9 K7 [", "]
       15 CALL                             R7 2 1
       16 NAMECALL                         R5 R5 K8 ["format"]
       18 CALL                             R5 2 1
       19 MOVE                             R4 R5
       20 LOADK                            R5 K5 [""]
       21 LOADK                            R7 K9 ["Pass the target tool's name as 'toolName' and its arguments as 'arguments'. The tool runs asynchronously and updates its own UI. Poll %* with the returned jobId when you need the final result."]
       22 GETUPVAL                         R9 2
       23 GETTABLEKS                       R9 R9 K10 ["JobWait"]
       25 NAMECALL                         R7 R7 K8 ["format"]
       27 CALL                             R7 2 1
       28 MOVE                             R6 R7
       29 LOADK                            R7 K5 [""]
       30 LOADK                            R8 K11 ["If you DO need the result before proceeding (e.g. a follow-up action depends on it), call the target tool directly instead of wrapping it here."]
       31 SETLIST                          R1 R2 7 [1]
       33 LOADK                            R2 K12 ["\n"]
       34 CALL                             R0 2 -1
       35 RETURN                           R0 -1
       36 GETIMPORT                        R0 K3 [table.concat]
       38 NEWTABLE                         R1 0 9
       40 LOADK                            R2 K4 ["Run a tool asynchronously and return immediately with a jobId."]
       41 LOADK                            R3 K5 [""]
       42 LOADK                            R5 K13 ["Use this when you want to run a tool (%*) without blocking on its completion — for example when the user does not need to wait for the result before you continue."]
       43 GETIMPORT                        R7 K3 [table.concat]
       45 GETUPVAL                         R8 1
       46 LOADK                            R9 K7 [", "]
       47 CALL                             R7 2 1
       48 NAMECALL                         R5 R5 K8 ["format"]
       50 CALL                             R5 2 1
       51 MOVE                             R4 R5
       52 LOADK                            R5 K5 [""]
       53 LOADK                            R7 K14 ["Call this at most ONCE per turn. If several jobs are needed, start one now and start the next on a following turn; never emit more than one %* call in the same response."]
       54 GETUPVAL                         R9 2
       55 GETTABLEKS                       R9 R9 K15 ["JobRun"]
       57 NAMECALL                         R7 R7 K8 ["format"]
       59 CALL                             R7 2 1
       60 MOVE                             R6 R7
       61 LOADK                            R7 K5 [""]
       62 LOADK                            R9 K9 ["Pass the target tool's name as 'toolName' and its arguments as 'arguments'. The tool runs asynchronously and updates its own UI. Poll %* with the returned jobId when you need the final result."]
       63 GETUPVAL                         R11 2
       64 GETTABLEKS                       R11 R11 K10 ["JobWait"]
       66 NAMECALL                         R9 R9 K8 ["format"]
       68 CALL                             R9 2 1
       69 MOVE                             R8 R9
       70 LOADK                            R9 K5 [""]
       71 LOADK                            R10 K11 ["If you DO need the result before proceeding (e.g. a follow-up action depends on it), call the target tool directly instead of wrapping it here."]
       72 SETLIST                          R1 R2 9 [1]
       74 LOADK                            R2 K12 ["\n"]
       75 CALL                             R0 2 -1
       76 RETURN                           R0 -1

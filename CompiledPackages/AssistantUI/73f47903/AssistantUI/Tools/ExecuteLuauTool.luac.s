PROTO_0:
        0 GETUPVAL                         R2 0
        1 GETTABLEKS                       R2 R2 K0 ["stopCode"]
        3 GETTABLEKS                       R3 R1 K1 ["id"]
        5 CALL                             R2 1 0
        6 RETURN                           R0 0

PROTO_1:
        0 RETURN                           R0 0

PROTO_2:
        0 RETURN                           R0 0

PROTO_3:
        0 GETUPVAL                         R0 0
        1 JUMPIFEQKNIL                     R0 ; [+2]
        3 RETURN                           R0 0
        4 GETUPVAL                         R0 1
        5 GETTABLEKS                       R0 R0 K0 ["contentId"]
        7 JUMPIFNOT                        R0 ; [+14]
        8 LOADB                            R0 1
        9 SETUPVAL                         R0 2
       10 GETUPVAL                         R0 3
       11 GETUPVAL                         R2 4
       12 GETTABLEKS                       R2 R2 K1 ["codeRunStatus"]
       14 GETUPVAL                         R3 5
       15 DUPTABLE                         R4 K5 [{["id"], ["status"] = True}]
       16 GETUPVAL                         R5 6
       17 SETTABLEKS                       R5 R4 K2 ["id"]
       19 NAMECALL                         R0 R0 K6 ["FireGuest"]
       21 CALL                             R0 4 0
       22 RETURN                           R0 0

PROTO_4:
        0 GETUPVAL                         R0 0
        1 GETTABLEKS                       R0 R0 K0 ["runCode"]
        3 GETUPVAL                         R1 1
        4 CALL                             R0 1 -1
        5 RETURN                           R0 -1

PROTO_5:
        0 GETUPVAL                         R2 0
        1 LOADB                            R4 0
        2 NAMECALL                         R2 R2 K0 ["GenerateGUID"]
        4 CALL                             R2 2 1
        5 GETUPVAL                         R3 1
        6 GETTABLEKS                       R3 R3 K1 ["startRecording"]
        8 MOVE                             R4 R2
        9 CALL                             R3 1 0
       10 GETTABLEKS                       R3 R1 K2 ["id"]
       12 JUMPIFNOT                        R3 ; [+16]
       13 GETUPVAL                         R4 2
       14 GETTABLEKS                       R4 R4 K3 ["isRunning"]
       16 MOVE                             R5 R3
       17 CALL                             R4 1 1
       18 JUMPIFNOT                        R4 ; [+10]
       19 GETIMPORT                        R4 K5 [error]
       21 LOADK                            R6 K6 ["Code is already running with id: "]
       22 FASTCALL1                        TOSTRING R3 ; [+3]
       23 MOVE                             R8 R3
       24 GETIMPORT                        R7 K8 [tostring]
       26 CALL                             R7 1 1
       27 CONCAT                           R5 R6 R7
       28 CALL                             R4 1 0
       29 GETTABLEKS                       R4 R1 K9 ["code"]
       31 JUMPIF                           R3 ; [+25]
       32 JUMPIFNOT                        R4 ; [+24]
       33 GETUPVAL                         R5 2
       34 GETTABLEKS                       R5 R5 K10 ["loadCode"]
       36 MOVE                             R6 R4
       37 CALL                             R5 1 1
       38 MOVE                             R3 R5
       39 GETTABLEKS                       R6 R1 K11 ["contentId"]
       41 JUMPIFNOT                        R6 ; [+15]
       42 GETUPVAL                         R6 3
       43 GETUPVAL                         R8 4
       44 GETTABLEKS                       R8 R8 K12 ["bindCodeId"]
       46 MOVE                             R9 R0
       47 DUPTABLE                         R10 K14 [{"contentId", "newCodeId"}]
       48 GETTABLEKS                       R11 R1 K11 ["contentId"]
       50 SETTABLEKS                       R11 R10 K11 ["contentId"]
       52 SETTABLEKS                       R3 R10 K13 ["newCodeId"]
       54 NAMECALL                         R6 R6 K15 ["FireGuest"]
       56 CALL                             R6 4 0
       57 FASTCALL2K                       ASSERT R3 K16 ; [+5]
       59 MOVE                             R6 R3
       60 LOADK                            R7 K16 ["No loadedId provided or created"]
       61 GETIMPORT                        R5 K18 [assert]
       63 CALL                             R5 2 0
       64 LOADNIL                          R5
       65 LOADNIL                          R6
       66 LOADB                            R7 0
       67 GETIMPORT                        R8 K21 [task.delay]
       69 LOADK                            R9 K22 [0.1]
       70 NEWCLOSURE                       R10 P0
       71 CAPTURE                          REF R5
       72 CAPTURE                          VAL R1
       73 CAPTURE                          REF R7
       74 CAPTURE                          UPVAL U3
       75 CAPTURE                          UPVAL U4
       76 CAPTURE                          VAL R0
       77 CAPTURE                          REF R3
       78 CALL                             R8 2 0
       79 GETIMPORT                        R8 K24 [pcall]
       81 NEWCLOSURE                       R9 P1
       82 CAPTURE                          UPVAL U2
       83 CAPTURE                          REF R3
       84 CALL                             R8 1 2
       85 MOVE                             R5 R8
       86 MOVE                             R6 R9
       87 JUMPIFNOT                        R7 ; [+11]
       88 GETUPVAL                         R8 3
       89 GETUPVAL                         R10 4
       90 GETTABLEKS                       R10 R10 K25 ["codeRunStatus"]
       92 MOVE                             R11 R0
       93 DUPTABLE                         R12 K28 [{["id"], ["status"] = False}]
       94 SETTABLEKS                       R3 R12 K2 ["id"]
       96 NAMECALL                         R8 R8 K15 ["FireGuest"]
       98 CALL                             R8 4 0
       99 GETUPVAL                         R8 1
      100 GETTABLEKS                       R8 R8 K29 ["endRecording"]
      102 MOVE                             R9 R2
      103 CALL                             R8 1 0
      104 DUPTABLE                         R8 K33 [{"success", "result", "loadedId"}]
      105 SETTABLEKS                       R5 R8 K30 ["success"]
      107 GETUPVAL                         R9 5
      108 GETTABLEKS                       R9 R9 K34 ["serializeToString"]
      110 MOVE                             R10 R6
      111 GETUPVAL                         R11 6
      112 CALL                             R11 0 1
      113 GETUPVAL                         R12 7
      114 CALL                             R12 0 -1
      115 CALL                             R9 -1 1
      116 SETTABLEKS                       R9 R8 K31 ["result"]
      118 SETTABLEKS                       R3 R8 K32 ["loadedId"]
      120 CLOSEUPVALS                      R3
      121 RETURN                           R8 1

PROTO_6:
        0 GETUPVAL                         R3 0
        1 CALL                             R3 0 1
        2 JUMPIFNOT                        R3 ; [+7]
        3 GETUPVAL                         R3 1
        4 CALL                             R3 0 1
        5 JUMPIFNOT                        R3 ; [+4]
        6 JUMPIFNOT                        R1 ; [+3]
        7 GETTABLEKS                       R2 R1 K0 ["targetDataModel"]
        9 JUMPIF                           R2 ; [+1]
       10 LOADNIL                          R2
       11 GETUPVAL                         R3 2
       12 MOVE                             R4 R2
       13 MOVE                             R5 R0
       14 CALL                             R3 2 1
       15 GETTABLEKS                       R4 R3 K1 ["success"]
       17 JUMPIFNOT                        R4 ; [+11]
       18 GETUPVAL                         R4 3
       19 CALL                             R4 0 1
       20 GETTABLEKS                       R6 R3 K2 ["result"]
       22 NAMECALL                         R4 R4 K3 ["addText"]
       24 CALL                             R4 2 1
       25 NAMECALL                         R4 R4 K4 ["build"]
       27 CALL                             R4 1 -1
       28 RETURN                           R4 -1
       29 GETIMPORT                        R4 K6 [error]
       31 GETTABLEKS                       R5 R3 K2 ["result"]
       33 CALL                             R4 1 0
       34 RETURN                           R0 0

PROTO_7:
        0 GETUPVAL                         R0 0
        1 LOADK                            R2 K0 ["SlashCommandDescriptions"]
        2 LOADK                            R3 K1 ["ExecuteLuau"]
        3 NAMECALL                         R0 R0 K2 ["getText"]
        5 CALL                             R0 3 -1
        6 RETURN                           R0 -1

PROTO_8:
        0 DUPTABLE                         R1 K2 [{"name", "arguments"}]
        1 GETUPVAL                         R2 0
        2 GETTABLEKS                       R2 R2 K3 ["ExecuteLuau"]
        4 SETTABLEKS                       R2 R1 K0 ["name"]
        6 DUPTABLE                         R2 K5 [{"code"}]
        7 SETTABLEKS                       R0 R2 K4 ["code"]
        9 SETTABLEKS                       R2 R1 K1 ["arguments"]
       11 RETURN                           R1 1

PROTO_9:
        0 DUPTABLE                         R0 K5 [{[1], ["code"] = "", ["expanded"] = False}]
        1 GETUPVAL                         R1 0
        2 GETTABLEKS                       R1 R1 K6 ["Type"]
        4 SETTABLEKS                       R1 R0 K0 ["type"]
        6 RETURN                           R0 1

PROTO_10:
        0 GETUPVAL                         R1 0
        1 SETTABLEKS                       R1 R0 K0 ["code"]
        3 RETURN                           R0 0

PROTO_11:
        0 GETUPVAL                         R1 0
        1 GETTABLEKS                       R1 R1 K0 ["parseJSONForCode"]
        3 MOVE                             R2 R0
        4 CALL                             R1 1 1
        5 JUMPIF                           R1 ; [+2]
        6 LOADNIL                          R2
        7 RETURN                           R2 1
        8 NEWCLOSURE                       R2 P0
        9 CAPTURE                          VAL R1
       10 RETURN                           R2 1

PROTO_12:
        0 GETUPVAL                         R1 0
        1 SETTABLEKS                       R1 R0 K0 ["code"]
        3 RETURN                           R0 0

PROTO_13:
        0 GETTABLEKS                       R1 R0 K0 ["input"]
        2 JUMPIFNOT                        R1 ; [+4]
        3 GETTABLEKS                       R1 R0 K0 ["input"]
        5 GETTABLEKS                       R1 R1 K1 ["code"]
        7 JUMPIF                           R1 ; [+2]
        8 LOADNIL                          R2
        9 RETURN                           R2 1
       10 NEWCLOSURE                       R2 P0
       11 CAPTURE                          VAL R1
       12 RETURN                           R2 1

PROTO_14:
        0 JUMPIFNOT                        R1 ; [+20]
        1 GETTABLEKS                       R2 R1 K0 ["code"]
        3 JUMPIFNOT                        R2 ; [+17]
        4 GETTABLEKS                       R3 R1 K0 ["code"]
        6 FASTCALL1                        TYPEOF R3 ; [+2]
        7 GETIMPORT                        R2 K2 [typeof]
        9 CALL                             R2 1 1
       10 JUMPIFNOTEQKS                    R2 K3 ["string"] ; [+10]
       12 GETUPVAL                         R2 0
       13 GETTABLEKS                       R3 R1 K0 ["code"]
       15 CALL                             R2 1 1
       16 JUMPIFNOT                        R2 ; [+4]
       17 DUPTABLE                         R3 K7 [{["shouldConfirm"] = True, ["warningMessage"]}]
       18 SETTABLEKS                       R2 R3 K6 ["warningMessage"]
       20 RETURN                           R3 1
       21 LOADNIL                          R2
       22 RETURN                           R2 1

PROTO_15:
        0 GETUPVAL                         R0 0
        1 LOADK                            R2 K0 ["Tools"]
        2 LOADK                            R3 K1 ["ExecuteLuau"]
        3 NAMECALL                         R0 R0 K2 ["getText"]
        5 CALL                             R0 3 -1
        6 RETURN                           R0 -1

PROTO_16:
        0 GETTABLEKS                       R1 R0 K0 ["networking"]
        2 GETTABLEKS                       R2 R0 K1 ["environment"]
        4 GETUPVAL                         R5 0
        5 GETTABLEKS                       R5 R5 K2 ["stopCode"]
        7 DUPCLOSURE                       R6 K3 [PROTO_0]
        8 CAPTURE                          UPVAL U1
        9 NAMECALL                         R3 R1 K4 ["OnHostEvent"]
       11 CALL                             R3 3 0
       12 GETUPVAL                         R5 0
       13 GETTABLEKS                       R5 R5 K5 ["bindCodeId"]
       15 DUPCLOSURE                       R6 K6 [PROTO_1]
       16 NAMECALL                         R3 R1 K7 ["OnGuestEvent"]
       18 CALL                             R3 3 0
       19 GETUPVAL                         R5 0
       20 GETTABLEKS                       R5 R5 K8 ["codeRunStatus"]
       22 DUPCLOSURE                       R6 K9 [PROTO_2]
       23 NAMECALL                         R3 R1 K7 ["OnGuestEvent"]
       25 CALL                             R3 3 0
       26 GETUPVAL                         R5 0
       27 GETTABLEKS                       R5 R5 K10 ["loadAndRunCodeAsync"]
       29 NEWCLOSURE                       R6 P3
       30 CAPTURE                          UPVAL U2
       31 CAPTURE                          VAL R2
       32 CAPTURE                          UPVAL U1
       33 CAPTURE                          VAL R1
       34 CAPTURE                          UPVAL U0
       35 CAPTURE                          UPVAL U3
       36 CAPTURE                          UPVAL U4
       37 CAPTURE                          UPVAL U5
       38 NAMECALL                         R3 R1 K11 ["OnHostInvokeAsync"]
       40 CALL                             R3 3 1
       41 NEWCLOSURE                       R4 P4
       42 CAPTURE                          UPVAL U6
       43 CAPTURE                          UPVAL U7
       44 CAPTURE                          VAL R3
       45 CAPTURE                          UPVAL U8
       46 GETUPVAL                         R5 9
       47 GETTABLEKS                       R5 R5 K12 ["define"]
       49 CALL                             R5 0 1
       50 GETUPVAL                         R7 10
       51 GETTABLEKS                       R7 R7 K13 ["ExecuteLuau"]
       53 NAMECALL                         R5 R5 K14 ["setName"]
       55 CALL                             R5 2 1
       56 GETUPVAL                         R8 11
       57 CALL                             R8 0 1
       58 JUMPIFNOT                        R8 ; [+2]
       59 GETUPVAL                         R7 12
       60 JUMP                             ; [+1]
       61 LOADK                            R7 K15 ["Executes Luau code in Roblox Studio. Returns the result of the executed code or an error message if the code fails to execute."]
       62 NAMECALL                         R5 R5 K16 ["setDescription"]
       64 CALL                             R5 2 1
       65 LOADK                            R7 K17 ["code"]
       66 DUPTABLE                         R8 K21 [{["type"] = "string", ["description"]}]
       67 GETUPVAL                         R10 11
       68 CALL                             R10 0 1
       69 JUMPIFNOT                        R10 ; [+2]
       70 LOADK                            R9 K22 ["The Luau code to execute. Must NOT be used to edit script source -- use multi_edit for that."]
       71 JUMP                             ; [+1]
       72 LOADK                            R9 K23 ["The Luau code to execute."]
       73 SETTABLEKS                       R9 R8 K20 ["description"]
       75 NAMECALL                         R5 R5 K24 ["addArgument"]
       77 CALL                             R5 3 1
       78 MOVE                             R7 R4
       79 NAMECALL                         R5 R5 K25 ["setHandler"]
       81 CALL                             R5 2 1
       82 DUPTABLE                         R7 K34 [{["title"] = "Execute Luau", ["readOnlyHint"] = False, ["destructiveHint"] = True, ["idempotentHint"] = False, ["openWorldHint"] = True}]
       83 NAMECALL                         R5 R5 K35 ["setAnnotations"]
       85 CALL                             R5 2 1
       86 NAMECALL                         R5 R5 K36 ["build"]
       88 CALL                             R5 1 1
       89 DUPTABLE                         R6 K41 [{["command"] = "run", ["getDescription"], ["mapToToolCall"]}]
       90 DUPCLOSURE                       R7 K42 [PROTO_7]
       91 CAPTURE                          UPVAL U13
       92 SETTABLEKS                       R7 R6 K39 ["getDescription"]
       94 DUPCLOSURE                       R7 K43 [PROTO_8]
       95 CAPTURE                          UPVAL U10
       96 SETTABLEKS                       R7 R6 K40 ["mapToToolCall"]
       98 DUPTABLE                         R7 K47 [{"transformInitialContent", "getTransformDeltaFn", "getTransformPreExecuteFn"}]
       99 DUPCLOSURE                       R8 K48 [PROTO_9]
      100 CAPTURE                          UPVAL U14
      101 SETTABLEKS                       R8 R7 K44 ["transformInitialContent"]
      103 DUPCLOSURE                       R8 K49 [PROTO_11]
      104 CAPTURE                          UPVAL U15
      105 SETTABLEKS                       R8 R7 K45 ["getTransformDeltaFn"]
      107 DUPCLOSURE                       R8 K50 [PROTO_13]
      108 SETTABLEKS                       R8 R7 K46 ["getTransformPreExecuteFn"]
      110 DUPTABLE                         R8 K58 [{"availableDataModelTypes", "definition", "slashCommands", "contentWidgets", "streamTransform", "getPreExecuteWarning", "displayNameFunction"}]
      111 GETUPVAL                         R10 16
      112 CALL                             R10 0 1
      113 JUMPIFNOT                        R10 ; [+23]
      114 GETUPVAL                         R10 7
      115 CALL                             R10 0 1
      116 JUMPIFNOT                        R10 ; [+20]
      117 NEWTABLE                         R9 0 3
      119 GETUPVAL                         R10 17
      120 GETTABLEKS                       R10 R10 K59 ["Types"]
      122 GETTABLEKS                       R10 R10 K60 ["Edit"]
      124 GETUPVAL                         R11 17
      125 GETTABLEKS                       R11 R11 K59 ["Types"]
      127 GETTABLEKS                       R11 R11 K61 ["Client"]
      129 GETUPVAL                         R12 17
      130 GETTABLEKS                       R12 R12 K59 ["Types"]
      132 GETTABLEKS                       R12 R12 K62 ["Server"]
      134 SETLIST                          R9 R10 3 [1]
      136 JUMPIF                           R9 ; [+1]
      137 LOADNIL                          R9
      138 SETTABLEKS                       R9 R8 K51 ["availableDataModelTypes"]
      140 SETTABLEKS                       R5 R8 K52 ["definition"]
      142 NEWTABLE                         R9 0 1
      144 MOVE                             R10 R6
      145 SETLIST                          R9 R10 1 [1]
      147 SETTABLEKS                       R9 R8 K53 ["slashCommands"]
      149 GETUPVAL                         R10 18
      150 CALL                             R10 0 1
      151 JUMPIFNOT                        R10 ; [+2]
      152 LOADNIL                          R9
      153 JUMP                             ; [+5]
      154 NEWTABLE                         R9 0 1
      156 GETUPVAL                         R10 14
      157 SETLIST                          R9 R10 1 [1]
      159 SETTABLEKS                       R9 R8 K54 ["contentWidgets"]
      161 SETTABLEKS                       R7 R8 K55 ["streamTransform"]
      163 DUPCLOSURE                       R9 K63 [PROTO_14]
      164 CAPTURE                          UPVAL U19
      165 SETTABLEKS                       R9 R8 K56 ["getPreExecuteWarning"]
      167 GETUPVAL                         R10 18
      168 CALL                             R10 0 1
      169 JUMPIFNOT                        R10 ; [+2]
      170 LOADNIL                          R9
      171 JUMP                             ; [+2]
      172 DUPCLOSURE                       R9 K64 [PROTO_15]
      173 CAPTURE                          UPVAL U13
      174 SETTABLEKS                       R9 R8 K57 ["displayNameFunction"]
      176 RETURN                           R8 1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [script]
        3 LOADK                            R2 K2 ["AssistantUI"]
        4 NAMECALL                         R0 R0 K3 ["FindFirstAncestor"]
        6 CALL                             R0 2 1
        7 GETIMPORT                        R1 K5 [game]
        9 LOADK                            R3 K6 ["HttpService"]
       10 NAMECALL                         R1 R1 K7 ["GetService"]
       12 CALL                             R1 2 1
       13 GETIMPORT                        R2 K9 [require]
       15 GETTABLEKS                       R3 R0 K10 ["Util"]
       17 GETTABLEKS                       R3 R3 K11 ["CheckCodeSafety"]
       19 CALL                             R2 1 1
       20 GETIMPORT                        R3 K9 [require]
       22 GETTABLEKS                       R4 R0 K10 ["Util"]
       24 GETTABLEKS                       R4 R4 K12 ["CommandExecution"]
       26 CALL                             R3 1 1
       27 GETIMPORT                        R4 K9 [require]
       29 GETTABLEKS                       R5 R0 K13 ["Parent"]
       31 GETTABLEKS                       R5 R5 K14 ["ModelContextProtocol"]
       33 CALL                             R4 1 1
       34 GETIMPORT                        R5 K9 [require]
       36 GETTABLEKS                       R6 R0 K10 ["Util"]
       38 GETTABLEKS                       R6 R6 K15 ["OutputParser"]
       40 CALL                             R5 1 1
       41 GETIMPORT                        R6 K9 [require]
       43 GETTABLEKS                       R7 R0 K16 ["Components"]
       45 GETTABLEKS                       R7 R7 K17 ["ContentWidgets"]
       47 GETTABLEKS                       R7 R7 K18 ["RunCodeContentWidget"]
       49 CALL                             R6 1 1
       50 GETIMPORT                        R7 K9 [require]
       52 GETTABLEKS                       R8 R0 K19 ["Tools"]
       54 GETTABLEKS                       R8 R8 K20 ["ToolTypes"]
       56 CALL                             R7 1 1
       57 GETIMPORT                        R8 K9 [require]
       59 GETTABLEKS                       R9 R0 K10 ["Util"]
       61 GETTABLEKS                       R9 R9 K21 ["ToolUtils"]
       63 CALL                             R8 1 1
       64 GETIMPORT                        R9 K9 [require]
       66 GETTABLEKS                       R10 R0 K22 ["Resources"]
       68 GETTABLEKS                       R10 R10 K23 ["Localization"]
       70 GETTABLEKS                       R10 R10 K24 ["Translator"]
       72 CALL                             R9 1 1
       73 GETIMPORT                        R10 K9 [require]
       75 GETTABLEKS                       R11 R0 K25 ["Types"]
       77 CALL                             R10 1 1
       78 GETIMPORT                        R11 K9 [require]
       80 GETTABLEKS                       R12 R0 K10 ["Util"]
       82 GETTABLEKS                       R12 R12 K26 ["DataModelType"]
       84 CALL                             R11 1 1
       85 GETIMPORT                        R12 K9 [require]
       87 GETTABLEKS                       R13 R0 K27 ["Flags"]
       89 GETTABLEKS                       R13 R13 K28 ["FFlagAssistantBackgroundDataModelToolCall"]
       91 CALL                             R12 1 1
       92 GETIMPORT                        R13 K9 [require]
       94 GETTABLEKS                       R14 R0 K27 ["Flags"]
       96 GETTABLEKS                       R14 R14 K29 ["FFlagAssistantExecuteLuaBackground"]
       98 CALL                             R13 1 1
       99 GETIMPORT                        R14 K9 [require]
      101 GETTABLEKS                       R15 R0 K27 ["Flags"]
      103 GETTABLEKS                       R15 R15 K30 ["FFlagAssistantHintMultiEditOverExecLuau"]
      105 CALL                             R14 1 1
      106 GETIMPORT                        R15 K9 [require]
      108 GETTABLEKS                       R16 R0 K27 ["Flags"]
      110 GETTABLEKS                       R16 R16 K31 ["FFlagAssistantSplitToolsAndWidgets"]
      112 CALL                             R15 1 1
      113 GETIMPORT                        R16 K9 [require]
      115 GETTABLEKS                       R17 R0 K27 ["Flags"]
      117 GETTABLEKS                       R17 R17 K32 ["FIntExecuteLuauMaxJsonLength"]
      119 CALL                             R16 1 1
      120 GETIMPORT                        R17 K9 [require]
      122 GETTABLEKS                       R18 R0 K27 ["Flags"]
      124 GETTABLEKS                       R18 R18 K33 ["FIntExecuteLuauMaxStringLength"]
      126 CALL                             R17 1 1
      127 GETIMPORT                        R18 K9 [require]
      129 GETTABLEKS                       R19 R0 K27 ["Flags"]
      131 GETTABLEKS                       R19 R19 K34 ["getIsAssistantBackgroundDataModelToolCall"]
      133 CALL                             R18 1 1
      134 GETTABLEKS                       R19 R4 K10 ["Util"]
      136 GETTABLEKS                       R19 R19 K35 ["ToolBuilder"]
      138 GETTABLEKS                       R20 R4 K10 ["Util"]
      140 GETTABLEKS                       R20 R20 K36 ["ToolResult"]
      142 GETTABLEKS                       R21 R7 K37 ["ToolNames"]
      144 GETTABLEKS                       R22 R21 K38 ["replaceTokens"]
      146 LOADK                            R23 K39 ["Executes a Luau snippet in Roblox Studio (like the command bar / a REPL) and returns the value of the last expression, or an error message if execution fails.\n\nUse this for one-off runtime queries or data model mutations that DO NOT involve editing script source code -- for example: counting or inspecting instances, reading or setting properties, moving / inserting / deleting instances, or running ad-hoc computations against the place.\n\nDO NOT use this tool to edit, create, rename, or refactor the source of any Script, LocalScript, or ModuleScript. For ANY change to script source code -- including creating a new script, inserting or replacing code, renaming identifiers, or applying multiple edits across a file -- use the {ToolNames.MultiEdit} tool instead. {ToolNames.MultiEdit} preserves script identity, supports atomic multi-edit diffs reviewable by the user, and is the only correct tool for source-code authoring. Setting `script.Source = ...` from this tool is NOT a valid substitute and must not be done."]
      147 CALL                             R22 1 1
      148 GETTABLEKS                       R23 R6 K40 ["NETWORK_KEYS"]
      150 DUPCLOSURE                       R24 K41 [PROTO_16]
      151 CAPTURE                          VAL R23
      152 CAPTURE                          VAL R3
      153 CAPTURE                          VAL R1
      154 CAPTURE                          VAL R8
      155 CAPTURE                          VAL R17
      156 CAPTURE                          VAL R16
      157 CAPTURE                          VAL R18
      158 CAPTURE                          VAL R13
      159 CAPTURE                          VAL R20
      160 CAPTURE                          VAL R19
      161 CAPTURE                          VAL R21
      162 CAPTURE                          VAL R14
      163 CAPTURE                          VAL R22
      164 CAPTURE                          VAL R9
      165 CAPTURE                          VAL R6
      166 CAPTURE                          VAL R5
      167 CAPTURE                          VAL R12
      168 CAPTURE                          VAL R11
      169 CAPTURE                          VAL R15
      170 CAPTURE                          VAL R2
      171 RETURN                           R24 1

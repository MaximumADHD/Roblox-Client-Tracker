return function()
	...
	local function ansi256(code)
		return string.format("%c[%d;5;%dm", 27, 38, code)
	end

	local function bgAnsi256(code)
		return string.format("%c[%d;5;%dm", 27, 48, code)
	end
	...
	it("styles string", function()
		expect(chalk.underline("foo")).to.equal(string.format("%sfoo%s", ansi16(4), ansi16(24)))
		expect(chalk.red("foo")).to.equal(string.format("%sfoo%s", ansi16(31), ansi16(39)))
		expect(chalk.bgRed("foo")).to.equal(string.format("%sfoo%s", ansi16(41), ansi16(49)))
	end)
	...
	it("supports nesting styles of different types", function()
		expect(chalk.red("r" .. chalk.bold("b") .. "r")).to.equal(
			string.format("%sr%sb%sr%s", ansi16(31), ansi16(1), ansi16(22), ansi16(39))
		)
	end)
	...
	it("supports composing multiple styles", function()
		...
		expect(redBold("foo")).to.equal(
			string.format("%s%sfoo%s%s", ansi16(31), ansi16(1), ansi16(22), ansi16(39))
		)
	end)
	...
	describe(".rgb()", function()
		...
		it("composes with modifiers", function()
			...
			expect(redBold("foo")).to.equal(
				string.format("%s%sfoo%s%s", ansi256(196), ansi16(1), ansi16(22), ansi16(39))
			)
		end)

		it("clamps to 16", function()
			expect(chalk.rgb(0, 0, 0)("foo")).to.equal(
				string.format("%sfoo%s", ansi256(16), ansi16(39))
			)
		end)

		it("clamps to 231", function()
			expect(chalk.rgb(255, 255, 255)("foo")).to.equal(
				string.format("%sfoo%s", ansi256(231), ansi16(39))
			)
		end)

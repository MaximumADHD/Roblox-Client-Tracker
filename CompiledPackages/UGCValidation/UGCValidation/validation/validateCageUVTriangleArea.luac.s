PROTO_0:
  ...
  NAMECALL R0 R0 K1 ["ValidateEditableMeshCageUVTriangleArea"]
  CALL R0 2 -1
  RETURN R0 -1
...
PROTO_1:
  ...
  LOADK R8 K8 ["Failed to load UVs for '%s'. Make sure the model has a valid UV map and try again."]
  ...
  GETTABLEKS R7 R8 K12 ["validateCageUVTriangleArea_FailedToLoadMesh"]
  LOADNIL R8
  MOVE R9 R1
  CALL R6 3 0
  LOADB R6 0
  ...
  LOADK R9 K8 ["Failed to load UVs for '%s'. Make sure the model has a valid UV map and try again."]
  ...
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K11 ["ErrorType"]
  GETTABLEKS R7 R8 K13 ["validateCageUVTriangleArea_ZeroAreaTriangle"]
  LOADNIL R8
  MOVE R9 R1
  CALL R6 3 0
  LOADB R6 0
  NEWTABLE R7 0 1
  GETIMPORT R8 K7 [string.format]
  LOADK R9 K14 ["Detected zero-area triangle in UV map of '%s'. You need to edit the UV map to fix this issue."]
  GETTABLEKS R10 R0 K9 ["fullName"]
  CALL R8 2 -1
  SETLIST R7 R8 -1 [1]
  RETURN R6 2

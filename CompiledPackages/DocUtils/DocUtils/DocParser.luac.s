...
PROTO_2:
  ...
  CALL R6 2 1
  JUMPIFNOT R6 [+16]
  LOADK R6 K10 [".validateProps%s+=%s+"]
  LOADK R7 K9 ["t%.s?t?r?i?c?t?[iI]nterface"]
  LOADK R8 K11 ["%(%{
(.-)
%}%)"]
  CONCAT R4 R6 R8
  GETTABLEKS R6 R2 K12 ["Name"]
  LOADK R7 K13 ["%.defaultProps"]
  LOADK R8 K14 ["%s+=%s+%{
(.-)
%}"]
  CONCAT R5 R6 R8
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K15 ["Typecheckers"]
  GETTABLEKS R6 R7 K16 ["T"]
  SETTABLEKS R6 R1 K2 ["Typechecking"]
  LOADK R7 K17 ["local function "]
  GETTABLEKS R8 R2 K12 ["Name"]
  LOADK R9 K18 ["%(%w-[pP]rops: (%w+)"]
  CONCAT R6 R7 R9
  GETIMPORT R7 K20 [string.match]
  MOVE R8 R3
  MOVE R9 R6
  CALL R7 2 1
  JUMPIFNOT R7 [+26]
...
"]
  CONCAT R4 R8 R9
  LOADK R8 K23 ["defaultProps:?%s*%w*"]
  LOADK R9 K14 ["%s+=%s+%{
(.-)
%}"]
  CONCAT R5 R8 R9
  GETUPVAL R10 1
  GETTABLEKS R9 R10 K15 ["Typecheckers"]
  GETTABLEKS R8 R9 K24 ["Luau"]
  SETTABLEKS R8 R1 K2 ["Typechecking"]
  JUMP [+10]
  JUMPIFNOTEQKNIL R4 [+9]
  GETIMPORT R8 K26 [print]
  LOADK R10 K27 ["📝 Unable to find props definition in "]
  GETTABLEKS R11 R2 K12 ["Name"]
  CONCAT R9 R10 R11
  CALL R8 1 0
  RETURN R1 1
  GETIMPORT R8 K20 [string.match]
  MOVE R9 R3
  MOVE R10 R4
  CALL R8 2 2
  GETIMPORT R10 K20 [string.match]
  MOVE R11 R3
  MOVE R12 R5
  CALL R10 2 1
  ...
  LOADK R13 K28 ["💔 Couldn't find props for "]
  GETTABLEKS R14 R2 K12 ["Name"]
  CONCAT R12 R13 R14
  CALL R11 1 0
  JUMP [+10]
  GETUPVAL R12 2
  GETTABLEKS R11 R12 K29 ["parseProps"]
  ...
  JUMPIFNOT R9 [+7]
  GETIMPORT R11 K31 [string.split]
  MOVE R12 R9
  LOADK R13 K32 [" & "]
  CALL R11 2 1
  SETTABLEKS R11 R1 K1 ["Inherits"]
  NEWTABLE R11 0 0
  GETIMPORT R12 K34 [string.gmatch]
  MOVE R13 R3
  LOADK R14 K35 ["R[oe]act%.createElement%(%s*([%w%.]+)%s*,"]
  CALL R12 2 3
  FORGPREP R12
  LOADB R17 1
  ...
  FORGLOOP R12 1 [-3]
  GETUPVAL R14 3
  GETTABLEKS R13 R14 K36 ["Dictionary"]
  GETTABLEKS R12 R13 K37 ["keys"]
  MOVE R13 R11
  CALL R12 1 1
  SETTABLEKS R12 R1 K38 ["Components"]
  RETURN R1 1
...
MAIN:
  ...
  LOADK R10 K15 ["🔌 DocParser is being used in a mode that cannot access script source."]
  LOADK R11 K16 [" Features relying on access to plugin source may not function as expected."]
  CONCAT R9 R10 R11
  CALL R8 1 0
  ...
  CAPTURE VAL R2
  CAPTURE VAL R4
  SETTABLEKS R8 R5 K20 ["parse"]
  DUPCLOSURE R8 K21 [PROTO_3]
  SETTABLEKS R8 R5 K22 ["_type"]
  RETURN R5 1

...
Check the definition of the component %q."]
  ...
  LOADK R17 K32 ["%s: type specification of %s"]
  LOADK R18 K33 [" `%s` is invalid; the type checker "]
  LOADK R19 K34 ["function must return `nil` or an `Error` but returned a %s. "]
  LOADK R20 K35 ["You may have forgotten to pass an argument to the type checker "]
  LOADK R21 K36 ["creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and "]
  LOADK R22 K37 ["shape all require an argument)."]
  CONCAT R16 R17 R22
  ORK R17 R4 K38 ["React class"]
  MOVE R18 R3
  MOVE R19 R9
  FASTCALL1 TYPEOF R12 [+3]
  MOVE R21 R12
  ...
  JUMPIFNOT R13 [+71]
  GETUPVAL R15 5
  GETTABLEKS R16 R12 K39 ["message"]
  ...
  GETIMPORT R15 K16 [string.format]
  LOADK R16 K40 ["Failed %s type: %s"]
  MOVE R17 R3
  GETTABLEKS R19 R12 K39 ["message"]

...
function CredentialsProtocol:registerCredentials(credType: string, data: string, timeoutSeconds: number): Promise<string?>
    -- If a previous in-flight request was already sent to the native layer, then reject.
    if not self.registerCredentialsRequestInFlight then
        local promise = Promise.race({
            Promise.new(function(resolve, _)
                self.subscriber:subscribe(self.REGISTER_CREDENTIALS_RESPONSE_DESCRIPTOR[credType], function(params: Table)

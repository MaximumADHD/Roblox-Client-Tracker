...
local FFlagLuaAppFixMessageBusUnsubscribe =
	require(CorePackages.Workspace.Packages.SharedFlags).FFlagLuaAppFixMessageBusUnsubscribe
...
function MessageBus.publishProtocolMethodRequest(
	desc: ProtocolMethodDescriptor,
	params: Table,
	customTelemetryData: Table
): ()
	assert(desc.validateParams(params))
	MessageBusService:PublishProtocolMethodRequest(desc.protocolName, desc.methodName, params, customTelemetryData)
end

function MessageBus.publishProtocolMethodResponse(
	desc: ProtocolMethodDescriptor,
	params: Table,
	responseCode: number,
	customTelemetryData: Table
	...
	if LuaAppFixMessageBusServiceCase then
		...
		(MessageBusService :: any):publishProtocolMethodResponse(
			desc.protocolName,
			desc.methodName,
			params,
			responseCode,
...
function Subscriber:subscribeProtocolMethodRequest(
	desc: ProtocolMethodDescriptor,
	callback: (Table) -> (),
	sticky: boolean?,
	once: boolean?
...
function Subscriber:subscribeProtocolMethodResponse(
	desc: ProtocolMethodDescriptor,
	callback: (Table) -> (),
	sticky: boolean?,
	once: boolean?

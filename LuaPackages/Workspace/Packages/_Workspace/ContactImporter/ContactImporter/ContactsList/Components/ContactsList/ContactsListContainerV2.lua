...
function ContactsListContainerV2:init()
	if getFFlagContactImporterChunkingCalls() then
		...
		-- TODO SACQ-124: make this local function in next PR
		...
		self.fireContactImportingTimeEvent = function(args: { isSuccessfulUpload: true, numMatchedContacts: number })
			local props: InternalProps = self.props
		...
		self.uploadDeviceContacts = function(formattedContacts)
			...
			if contactsCount == 0 or props.variant == IXPVariants.INVITES_ONLY then
				-- don't upload if contacts are empty
				return Promise.resolve({})
			elseif #formattedContacts.contacts < getFIntContactImporterUploadContactsMin() then
				-- return error if attempting to upload less than minimum
				return Promise.reject()
			else
				return props.uploadContacts(formattedContacts :: any):andThen(self.getMatchedContacts)
			end
		end
		...
		self.getMatchedContacts = function()
			...
			return props.findContacts():andThen(props.getContactEntities)
		end
		...
		self.getContacts = function()
			self.getContactsFromDevice()
				:andThen(self.formatContactsFromDevice)
				:andThen(self.uploadDeviceContacts)
				:andThen(self.fireHasLoaded)
				:catch(self.failedToUpload)
		end

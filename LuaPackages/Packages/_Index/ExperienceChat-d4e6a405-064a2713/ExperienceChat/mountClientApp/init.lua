...
local getFFlagTextChatServiceBubbleAdded = require(ExperienceChat.Flags.getFFlagTextChatServiceBubbleAdded)
...
return function(config: Config): { roactInstance: any }
	...
	local function runOnBubbleDisplayed(partOrModel: Instance, message: string)
		local player = nil
	...
	if getEngineFeatureFlagEnableDisplayBubble() and getFFlagTextChatServiceBubbleAdded() then
		TextChatService.BubbleDisplayed:Connect(function(partOrModel, message)
			runOnBubbleDisplayed(partOrModel, message)
		end)
	end

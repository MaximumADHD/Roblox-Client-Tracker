...
local function watchPlayersService(config, store)
	...
	local function onPlayerAdded(player: Player)
		...
		local humanoidDiedConn: RBXScriptConnection?
		local function onCharacterLoaded(character: Model)
			-- Make sure that character is parented
			...
			-- Make sure that humanoid is parented
			...
			-- Make sure that the root part is parented
			local rootPart = character.PrimaryPart
			while character:IsDescendantOf(game) and not rootPart do
				waitForFirst(
					character.ChildAdded,
					character:GetPropertyChangedSignal("PrimaryPart"),
			...
			if rootPart and character:IsDescendantOf(game) and player.Character == character then
				...
				-- Clean up any previous humanoid.Died connections

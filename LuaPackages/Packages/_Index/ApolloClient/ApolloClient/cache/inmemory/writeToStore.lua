...
function StoreWriter:processSelectionSet(ref_: ProcessSelectionSetOptions): StoreObject | Reference
	...
	-- policies.identify.
	dataId = Boolean.toJSBoolean(dataId) and dataId or id
	...
	-- ROOT_QUERY.
	local typename: string | nil = (function()
	...
	for _, selection in selections do
		...
		if isField(selection) then
			...
			context.clientOnly = wasClientOnly
				or (
					Boolean.toJSBoolean(selection.directives)
			...
			if value ~= nil then
				...
				if selection.selectionSet ~= nil then
					...
					if not Boolean.toJSBoolean(childTypename) and isReference(incomingValue) then
			...
			if
				...
				and Boolean.toJSBoolean(policies:fragmentMatches(fragment, typename, result, context.variables))
			then
...
function StoreWriter:applyMerges<T>(
	...
	if Boolean.toJSBoolean(mergeTree.map.size) and not isReference(incoming) then
		...
		if e ~= nil and not Boolean.toJSBoolean(getStorageArgs) then
		...
		mergeTree.map:forEach(function(childTree, storeFieldName)
			...
			if aVal ~= iVal then
				changedFields = Boolean.toJSBoolean(changedFields) and changedFields or Map.new(nil);
	...
	if Boolean.toJSBoolean(mergeTree.info) then
...
function mergeTreeIsEmpty(tree: MergeTree | nil): boolean
	return not Boolean.toJSBoolean(tree)
		or not (Boolean.toJSBoolean((tree :: MergeTree).info) or Boolean.toJSBoolean((tree :: MergeTree).map.size))
end

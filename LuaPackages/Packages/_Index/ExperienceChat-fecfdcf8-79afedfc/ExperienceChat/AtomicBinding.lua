...
function AtomicBinding:bindRoot(root)
	...
	assert(rootInstToManifest[root] == nil, "rootInstToManifest is nil")
	...
	local function processNode(node)
		local instance = assert(node.instance, "instance does not exist")
		...
		if not isLeaf then
			...
			local function processDeleteChild(childInstance)
				...
				assert(childNode.instance == nil, "unbindNodeDescend failed") -- If this triggers, unbindNodeDescend failed
...
function AtomicBinding:unbindRoot(root)
	...
	if rootNode then
		local resolvedManifest = assert(rootInstToManifest[root], "rootInstToManifest is nil")
...
function AtomicBinding:destroy()
	...
	for rootInst, rootNode in pairs(self._rootInstToRootNode) do
		local resolvedManifest = assert(rootInstToManifest[rootInst], "rootInstToManifest is nil")
		unbindNodeDescend(rootNode, resolvedManifest)
	end

...
local getNameFromWhisper = require(ExperienceChat.getNameFromWhisper)
local handleWhisperCommandErrors = require(ExperienceChat.Commands.handleWhisperCommandErrors)
local getAutocompletePlayersFromString = require(ExperienceChat.Commands.getAutocompletePlayersFromString)
...
function ChatInputBar:init()
	...
	self.onTextChanged = function(rbx)
		...
		local displayNameOrUsername = getNameFromWhisper(newText)
		if displayNameOrUsername then
			local filteredPlayers = Dictionary.join(self.props.players, {
				byUserId = Dictionary.removeKey(self.props.players.byUserId, tostring(self.props.localPlayer.UserId)),
			})
			local matchingPlayers = getAutocompletePlayersFromString(filteredPlayers, displayNameOrUsername)
			if #matchingPlayers == 0 then
				Logger:debug("ChatInputBar Autocomplete: No matches!")
			elseif #matchingPlayers == 1 then
				local otherPlayer = matchingPlayers[1]
				Logger:debug("ChatInputBar activateWhisperMode()")

				local didCreateWhisperChannel = self.props.activateWhisperMode(
					self.props.localPlayer.UserId,
					otherPlayer.userId
				)

				if didCreateWhisperChannel then
					rbx.Text = ""
					self:setState({
						inputText = "",
					})
				...
				Logger:debug("ChatInputBar Autocomplete: Too many matches!")
			end
		end
	end
	...
	self.onSendActivated = function()
		...
		if self.props.onSendChat and string.find(text, "%S") ~= nil then
			local displayNameOrUsername = getNameFromWhisper(text)
			if displayNameOrUsername and context.translator then
				handleWhisperCommandErrors(
					displayNameOrUsername,
					self.props.players,
					self.props.localPlayer.UserId,
					context.translator,
					self.props.defaultSystemTextChannel
				)
			else
	...
	self.onBackspacePressedConnection = self.props.userInputService.InputEnded:connect(function(inputObj, _)
		...
		then
			...
			self.props.deactivateWhisperMode()
		end
	end)

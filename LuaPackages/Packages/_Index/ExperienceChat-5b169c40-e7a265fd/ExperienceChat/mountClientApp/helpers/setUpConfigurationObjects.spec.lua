...
return function()
	...
	if chatWindowConfiguration and chatInputBarConfiguration and bubbleChatConfiguration then
		...
		it("SHOULD change state when configuration property changes", function(c)
			...
			expect(newState.ChatVisibility.isChatWindowEnabled.value).toEqual(chatInputBarConfiguration.Enabled) -- doesnt work
			expect(newState.ChatVisibility.isChatInputBarEnabled.value).toEqual(chatWindowConfiguration.Enabled) -- doesnt work
		end)
	end
end

...
local ControlsBubble = require(BubbleChat.ControlsBubble)
...
function BubbleChatBillboard:init()
	...
	local iconStyle = self.props.userId ~= tostring(Players.LocalPlayer.UserId) and "SpeakerDark" or "MicDark"
...
function BubbleChatBillboard:getRenderControlsBubble()
...
function BubbleChatBillboard:render()
	...
	if GetFFlagBubbleChatWithControlButtons() and self:getRenderControlsBubble() then
		children.ControlsBubble = Roact.createElement(ControlsBubble, {
		...
		if
			showVoiceIndicator
			and (
				not self.props.isBubbleChatEnabled
				or not self.state.filteredMessages
				or #self.state.filteredMessages == 0
			)
		then

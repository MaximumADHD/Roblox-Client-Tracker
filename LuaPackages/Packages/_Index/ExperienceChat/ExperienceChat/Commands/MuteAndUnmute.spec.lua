...
return function()
	...
	it("SHOULD return error when string does not match any players", function(c)
		...
		expect(RBXUnmuteCommand.clientRun(c.store, c.textSource, "/unmute someRandomName")).toHaveProperty(
			"metadata",
			"Roblox.Unmute.Error.PlayerNotFound"
		)
	end)
	...
	it("SHOULD return error when string matches local user", function(c)
		expect(RBXMuteCommand.clientRun(c.store, c.textSource, "/mute Myself")).toHaveProperty(
			"metadata",
			"Roblox.Mute.Error.CannotMuteSelf"
		)
		expect(RBXUnmuteCommand.clientRun(c.store, c.textSource, "/unmute Myself")).toHaveProperty(
			"metadata",
	...
	it("SHOULD return error when string matches multiple users", function(c)
		expect(RBXMuteCommand.clientRun(c.store, c.textSource, "/mute DisplayName")).toHaveProperty(
			"metadata",
			"Roblox.Mute.Error.MultipleMatches"
		)
		expect(RBXUnmuteCommand.clientRun(c.store, c.textSource, "/unmute DisplayName")).toHaveProperty(
			"metadata",
	...
	it("SHOULD return success when string matches a single user via DisplayName", function(c)
		expect(RBXMuteCommand.clientRun(c.store, c.textSource, "/mute Player4+DN")).toHaveProperty(
			"metadata",
			"Roblox.Mute.Info.Success"
		)
		expect(RBXUnmuteCommand.clientRun(c.store, c.textSource, "/unmute Player4+DN")).toHaveProperty(
			"metadata",
	...
	it("SHOULD return success when string matches a single user via Username", function(c)
		expect(RBXMuteCommand.clientRun(c.store, c.textSource, "/mute @Player1")).toHaveProperty(
			"metadata",
			"Roblox.Mute.Info.Success"
		)
		expect(RBXUnmuteCommand.clientRun(c.store, c.textSource, "/unmute @Player1")).toHaveProperty(
			"metadata",
			"Roblox.Unmute.Info.Success"
		)
	end)

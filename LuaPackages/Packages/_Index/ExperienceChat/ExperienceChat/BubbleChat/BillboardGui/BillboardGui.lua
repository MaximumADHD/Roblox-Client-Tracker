...
local IXPServiceWrapper = require(ExperienceChat.IXPServiceWrapper)
local FFlagEasierUnmuting = game:DefineFastFlag("EasierUnmutingTCS4", false)
...
local FStringEasierUnmutingIXPLayerName = game:DefineFastString("EasierUnmutingIXPLayerNameTCS", "Voice.UserAgency")
local FStringEasierUnmutingIXPLayerValue =
	game:DefineFastString("EasierUnmutingIXPLayerValueTCS", "VoiceUserAgencyEnabled")
...
local FIntExperienceChatHeartbeatIncrement = game:DefineFastInt("ExperienceChatHeartbeatIncrement", 5)
local getFFlagFixBubbleChatTailVisible = require(ExperienceChat.Flags.getFFlagFixBubbleChatTailVisible)
...
function BubbleChatBillboard:isShowingDueToEasierUnmuting()
	return FFlagEasierUnmuting
		...
		and not (FFlagEasierUnmutingHideIfMuted and self.props.isMuted)
end
...
function BubbleChatBillboard:init()
	...
	-- wrap click callback to provide context for analytics
	self.wrappedOnClickVoiceIndicator = function(targetUserId, voiceState)
		local context = if self:isShowingDueToEasierUnmuting() then "easierunmuting" else "bubblechat"

		return self.props.onClickedVoiceIndicator(targetUserId, voiceState, context)
	end
	...
	self.renderInsert = function()
		return Roact.createElement(VoiceIndicator, {
			...
			onClicked = self.wrappedOnClickVoiceIndicator,
			iconStyle = iconStyle,
		})
	end
...
function BubbleChatBillboard:shouldRenderPlayerButtons()
	if getFFlagFixBubbleChatTailVisible() then
		if not self.props.voiceEnabled then
			-- We should only show player buttons when the user has voice.
			return false
		end
	end
...
function BubbleChatBillboard:render()
	...
	-- Chrome hides the local player's bubbles.
	if GlobalFlags.ChromeEnabled and isLocalPlayer then
	...
	local clientSettings = self.props.clientSettings
	if GlobalFlags.AvatarChatEnabled or GlobalFlags.ChromeEnabled then
		if self:shouldRenderPlayerButtons() then
			local PlayerButtonSettings = {
				...
				onClickedVoiceIndicator = self.wrappedOnClickVoiceIndicator,
				...
				setCameraPermissionStateFromControl = self.props.setCameraPermissionStateFromControl,
			...
			children.PlayerButtons = Roact.createElement(PlayerButtons, PlayerButtonSettings)
	...
	local showCaret = if getFFlagFixBubbleChatTailVisible() then self:shouldRenderCaret() else nil
...
function BubbleChatBillboard:didUpdate(_lastProps, lastState)
	...
	local isLocalPlayer = self.props.userId == tostring(if Players.LocalPlayer then Players.LocalPlayer.UserId else 0)

	if
		FFlagEasierUnmuting
		and isLocalPlayer
		and not self.state.haveLoggedUserExposure
		and (
			self.props.voiceState == "Inactive"
			or self.props.voiceState == "Talking"
			or self.props.voiceState == "Muted"
		)
	then
...
function BubbleChatBillboard:didMount()
	...
	self.heartbeatConn = RunService.Heartbeat:Connect(function()
		if getFFlagEnableExperienceChatOptimizations() then
			if self.heartbeatCount < FIntExperienceChatHeartbeatIncrement then
		...
		if workspace.CurrentCamera and adorneeInstance then
			...
			if FFlagEasierUnmuting and self.state.isInEasierUnmutingTreatment then
				...
				if localAdorneeInstance then
					if localAdorneeInstance:IsA("Attachment") then
						localPosition = localAdorneeInstance.WorldPosition
					else
						localPosition = localAdorneeInstance.Position
					end
				end
			...
			if
				...
				or (
					FFlagEasierUnmuting
					and self.state.isInEasierUnmutingTreatment
					and isInsideEasierUnmutingDistance ~= self.state.isInsideEasierUnmutingDistance
				)
			then
				self:setState({
...
local function mapStateToProps(state, props)
	...
	if props.isTextChatServiceOn then
		...
		chatSettings = getSettingsForMessage(state.LegacyBubbleChatSettings, lastMessage)
	end

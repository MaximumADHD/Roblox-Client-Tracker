...
function ChatBubble:render()
	...
	local backgroundImageSettings
	local backgroundGradientSettings
	local bubbleCorner
	local bubbleUIPadding
	local fontFace
	if self.props.isTextChatServiceOn then
		backgroundImageSettings = chatSettings.ImageLabel
		backgroundGradientSettings = chatSettings.UIGradient
		bubbleCorner = chatSettings.UICorner
		bubbleUIPadding = chatSettings.UIPadding
		fontFace = chatSettings.FontFace
	else
		if GlobalFlags.ConsolidateBubbleChat then
			-- Grab legacy settings for backgroundImage, backgroundGradient, corner radius, padding from chatSettings
			backgroundImageSettings = chatSettings.BackgroundImage

			backgroundGradientSettings = chatSettings.BackgroundGradient

			if chatSettings.CornerEnabled then
				bubbleCorner = { CornerRadius = chatSettings.CornerRadius }
			elseif chatSettings.CornerEnabled == false then
				bubbleCorner = { CornerRadius = UDim.new(0, 0) }
			end

			bubbleUIPadding = (
				chatSettings.Padding
				and {
	...
	if getFFlagFixBubbleChatTailVisible() then
		isCaretVisible = self.props.isMostRecent and chatSettings.TailVisible and self.props.showCaret
	else
	...
	return Roact.createElement("Frame", {
		...
		ChatBubbleFrame = Roact.createElement(
			...
			}),
			{
				...
				Padding = (bubbleUIPadding and Roact.createElement("UIPadding", bubbleUIPadding))
				...
				Text = Roact.createElement("TextLabel", {
					...
					FontFace = if fontFace then fontFace else nil,
					Font = if not fontFace then chatSettings.Font else nil,

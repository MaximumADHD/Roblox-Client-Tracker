...
ChatInputBar.defaultProps = {
	...
	getChannelFromName = function(name)
		return TextChatService:FindFirstChild(name, true)
	end,
}
...
function ChatInputBar:init()
	...
	self.onTextChanged = function(rbx)
		...
		if newText == "/t " or newText == "/team " or newText == "% " then
			local team = self.props.localPlayer.Team
			Logger:debug("Team switch triggered. Team: {}", team and team.Name or "nil")
			if team then
				local channelName = "RBXTeam" .. tostring(team.TeamColor.Name)
				local channel = self.props.getChannelFromName(channelName)
				if channel then
					Logger:trace("Setting TargetTextChannel to: {}", channel.Name)
					self.props.setTargetTextChannel(channel)

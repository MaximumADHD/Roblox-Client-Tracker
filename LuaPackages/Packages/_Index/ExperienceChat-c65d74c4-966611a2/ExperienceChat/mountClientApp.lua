...
local TargetTextChannelChanged = require(Actions.TargetTextChannelChanged)
	...
	local function onPlayerAdded(player: Player)
		...
		if player == Players.LocalPlayer then
			...
			if player.Character then
				Logger:debug("Local character loaded: {}", player.Character.Name)
				store:dispatch(LocalCharacterLoaded(player.Character))
			end
...
return function(config: Config): { roactInstance: any }
	...
	if chatInputBarConfiguration then
		...
		store:dispatch(TargetTextChannelChanged(config.defaultTargetTextChannel))
		chatInputBarConfiguration:GetPropertyChangedSignal("TargetTextChannel"):Connect(function()
			store:dispatch(TargetTextChannelChanged(chatInputBarConfiguration.TargetTextChannel))
		end)
		store:dispatch(TargetTextChannelChanged(chatInputBarConfiguration.TargetTextChannel))
	end

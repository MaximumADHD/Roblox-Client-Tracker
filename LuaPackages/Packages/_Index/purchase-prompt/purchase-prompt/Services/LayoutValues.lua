...
function LayoutValues.new(isTenFoot, isNewRobuxIconEnabled)
	local self = {}
	setmetatable(self, {
	...
	self.layout = self:generate(isTenFoot, isNewRobuxIconEnabled)

	return self
end

function LayoutValues:update(isTenFoot, isNewRobuxIconEnabled)
	self.layout = self:generate(isTenFoot, isNewRobuxIconEnabled)
	self.signal:fire(self.layout)
end

function LayoutValues:generate(isTenFoot, isNewRobuxIconEnabled)
	local EnableRobuxIcon = isNewRobuxIconEnabled or game:GetFastFlag("EnableRobuxHexIconV2")
	...
	local RobuxIconWidth = EnableRobuxIcon and 16 * scaleFactor or 20 * scaleFactor

...
function AppContainer:init()
	local isTextChatServiceOn: boolean
	if GlobalFlags.ConsolidateBubbleChat then
		isTextChatServiceOn = self.props.textChatService.ChatVersion == Enum.ChatVersion.TextChatService
	else
		-- If ConsolidateBubbleChat flag is false, then new in-experience chat should have never mounted if ChatVersion is legacy.
		-- Thus, if ConsolidateBubbleChat is false, ChatVersion MUST be TextChatService.
		isTextChatServiceOn = true
	end
	self.isTextChatServiceOn = isTextChatServiceOn
end
...
function AppContainer:render()
	...
	local bubbleChatElement = Roact.createElement(BubbleChat, {
		...
		isTextChatServiceOn = self.isTextChatServiceOn,

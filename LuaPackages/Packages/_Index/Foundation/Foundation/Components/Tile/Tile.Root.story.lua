...
local function renderButton(button, index: number)
...
local function PlaygroundStory(props: {
	...
	local tileOffset = ((props.controls.numTitleLines or 0) + props.controls.numSubtitleLines + footerTextLines)
		* textSizeOffset
...
local function FunTileStory(props: {
	controls: {
		itemId: number,
	},
})
	local tokens = useTokens()
	local itemBG =
		`component_assets/itemBG_{if tokens.Config.ColorMode.Name == ColorMode.Dark then "dark" else "light"}`
...
local function ExperienceTileStory(props: {
	controls: {
		placeId: number,
	},
})
	...
	end, { isHoveringWide, tokens.Size.Size_200 } :: { unknown })
...
local function ItemTileStory(props: {
	controls: {
		itemId: number,
	},
})
...
local function PlayerTileStory()
	local tokens = useTokens()
	local avatarBG =
		`component_assets/avatarBG_{if tokens.Config.ColorMode.Name == ColorMode.Dark then "dark" else "light"}`
	...
	end, { mouseEntered, backgroundColor, hoverColor } :: { unknown })
...
local function MarketplaceTileStory(props: {
	controls: {
		placeId: number,
	},
})

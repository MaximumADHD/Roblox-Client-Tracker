...
local SingleMonthCalendar = require(script.Parent.SingleMonthCalendar)
...
local TimeDropdown = require(script.Parent.Parent.Time.TimeDropdown)
...
local function Calendar(props: Props): React.ReactNode
	if Flags.FoundationDateTimePickerBetaUpdate then
		return React.createElement(SingleMonthCalendar, {
			...
			selectedDates = props.defaultDates,
			showTimeList = props.showTimeDropdown,
			testId = props.testId,
	...
	local defaultDates = (if not props.defaultDates then { DateTime.now() } else props.defaultDates) :: { DateTime }
	...
	local currViewDate, setCurrViewDate = React.useState({
		month = (defaultDates[1]:ToLocalTime() :: DateTimeUtilities.DateTimeLocalTime).Month,
		year = (defaultDates[1]:ToLocalTime() :: DateTimeUtilities.DateTimeLocalTime).Year,
	})

	local localSelectedDateTimes = {}
	for _, dateTime in selectedDateTimes do
		table.insert(localSelectedDateTimes, dateTime:ToLocalTime() :: DateTimeUtilities.DateTimeLocalTime)
	end
	...
	local handleDateActivated = React.useCallback(
		function(day: number, month: number, year: number)
			local dateTime = DateTime.fromLocalTime(
				...
				month,
				day,
				(selectedDateTimes[1]:ToLocalTime() :: DateTimeUtilities.DateTimeLocalTime).Hour,
				(selectedDateTimes[1]:ToLocalTime() :: DateTimeUtilities.DateTimeLocalTime).Minute
			)

...
local function ChatInputBar(props)
	...
	local onSendActivated = React.useCallback(function()
		...
		local text = if EngineFeatureTextChatServiceEscapeSendAsync then inputText else sanitizeForRichText(inputText)
	...
	React.useEffect(function()
		local function handleAction(
			...
			if inputObject.KeyCode == props.focusKeyCode then
	...
	local onTextChanged = React.useCallback(function(rbx)
		...
		if utf8Length and utf8Length > props.messageCharacterLimit then
	...
	return React.createElement("Frame", {
		...
		BackgroundTransparency = setTransparency(chatWindowSettings.BackgroundTransparency, props.transparencyValue),
		BorderSizePixel = 0,
		...
		Background = React.createElement("Frame", {
			...
			BackgroundTransparency = setTransparency(
				chatInputBarSettings.BackgroundTransparency,

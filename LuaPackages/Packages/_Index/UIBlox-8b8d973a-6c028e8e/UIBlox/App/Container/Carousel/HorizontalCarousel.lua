...
function HorizontalCarousel:render()
	...
	return Roact.createElement("Frame", {
		...
		InfiniteScrollerCarousel = Roact.createElement(Scroller, {
			BackgroundTransparency = 1,
			...
			itemList = itemList,
			loadingBuffer = self.state.numberOfItemsShown + 1,
			mountingBuffer = self.state.numberOfItemsShown * 6 * itemSize.X + itemSize.X,
			loadNext = loadNext,

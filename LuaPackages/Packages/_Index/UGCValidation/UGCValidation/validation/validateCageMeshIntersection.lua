...
local function validateCageMeshIntersection(
	...
	if game:GetFastFlag("UGCReturnAllValidations") then
		...
		if not checkIrrelevantCageModified then
			return false,
				{
					"Some outer cage geometry that is far away from the clothing mesh has been modified. This will lead to issues when other clothing is layered on top.",
				}
		end
		...
		if not checkOuterCageFarExtendedFromMesh then
			return false,
				{
					"There are outer cage vertex(s) that are too far away from the clothing mesh. This will lead to issues when other clothing is layered on top.",
				}
		end
		...
		if not checkAverageOuterCageToMeshVertDistances then
			return false,
				{
					"The average distance between the clothing and outer cage is too large. This will lead to bloating issues when other clothing is layered on top.",
				}
		end

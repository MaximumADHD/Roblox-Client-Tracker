...
local function ChatInputBar(props)
	...
	local onSendActivated = React.useCallback(function()
		...
		if textBoxRef.current then
			local text = textBoxRef.current.Text
			...
			Logger:debug("onSendActivated could not send message because textBoxRef.current was nil")
		end
	end, { inputText })
	...
	local chatInputBarBackgroundTransparency =
		useInterpolatedClientSettings(chatInputBarSettings).BackgroundTransparency
	local chatWindowBackgroundTransparency = useInterpolatedClientSettings(chatWindowSettings).BackgroundTransparency
	local chatWindowBackgroundColor = useInterpolatedClientSettings(chatWindowSettings).BackgroundColor3

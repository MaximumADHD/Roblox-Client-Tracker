...
return function()
	...
	describe(
		"WHEN ChatVersion is LegacyChatService and we use LegacyBubbleCreated and LegacyBubbleTextUpdated",
		function()
			beforeAll(function(c)
				c.prevFlag = GlobalFlags.ConsolidateBubbleChat
			...
			afterAll(function(c)
				GlobalFlags.ConsolidateBubbleChat = c.prevFlag
			end)

			it("SHOULD add the entry by its messageId", function(c)
				assert(c.state1.byMessageId.id1, "Did not add message 'id1' to map")
				expect(c.state1.byMessageId.id1).toHaveProperty("status", Enum.TextChatMessageStatus.Success)
				expect(c.state1.byMessageId.id1.text).toEqual("Hello world")
			end)

			it("SHOULD add entries for multiple messages by messageId", function(c)
				local state2 = reducer(
					c.state1,
					LegacyBubbleCreated({
						id = "id2",
				...
				assert(state2.byMessageId.id2, "Did not add message 'id2' to map")
				expect(state2.byMessageId.id2).toHaveProperty("status", Enum.TextChatMessageStatus.Success)
				expect(state2.byMessageId.id2.text).toEqual("Goodbye world")
				expect(Dictionary.count(state2.byMessageId)).toBe(2)

				local state3 = reducer(
					state2,
					LegacyBubbleCreated({
						id = "id3",
						userId = 1,
				...
				assert(state3.byMessageId.id3, "Did not add message 'id3' to map")
				expect(state3.byMessageId.id3).toHaveProperty("status", Enum.TextChatMessageStatus.Success)
				expect(state3.byMessageId.id3.text).toEqual("Hello world")
				expect(Dictionary.count(state3.byMessageId)).toBe(3)
			end)

			it("SHOULD replace existing entry if messageId already exists in byMessageId", function(c)
				local state2 = reducer(
					c.state1,
					LegacyBubbleCreated({
						id = "id1",
				...
				assert(state2.byMessageId.id1, "Did not add message 'id1' to map")
				expect(state2.byMessageId.id1).toHaveProperty("status", Enum.TextChatMessageStatus.Success)
				expect(state2.byMessageId.id1.text).toEqual("Goodbye world")
				expect(Dictionary.count(state2.byMessageId)).toBe(1)
			end)

			it("SHOULD update state correctly if there is a partOrModel and no messageUserId for a message", function(c)
				local state1 = reducer(
					c.initialState,
					LegacyBubbleCreated({
						id = "id1",
				...
				assert(state1.byMessageId.id1, "Did not add message 'id1' to map")
				expect(Dictionary.count(state1.mockUserIdToPartOrModel)).toBe(1)
				expect(Dictionary.count(state1.partOrModelToMockUserId)).toBe(1)
				expect(state1.byMessageId.id1.text).toEqual("Hello world")
			end)

			it("SHOULD update state correctly if the timestamp is missing for the message", function(c)
				local state1 = reducer(
					c.initialState,
					LegacyBubbleCreated({
						id = "id1",
				...
				assert(state1.byMessageId.id1, "Did not add message 'id1' to map")
				expect(state1.byMessageId.id1.text).toEqual("Hello world")
				expect(state1.byMessageId.id1).toHaveProperty("status", Enum.TextChatMessageStatus.Success)
				expect(state1.byMessageId.id1.timestamp).never.toBeNil()
			end)

			it("SHOULD not update text by its messageId if the messageId doesn't exist in byMessageId", function(c)
				local messageId = "id2"
				local newText = "Goodbye world"
				local state2 = reducer(c.state1, LegacyBubbleTextUpdated(messageId, newText))

				expect(Dictionary.count(state2.byMessageId)).toBe(1)
				expect(state2.byMessageId.id2).toBeNil()
			end)

			it("SHOULD update text if messageId exists in byMessageId", function(c)
				expect(c.state1.byMessageId.id1.text).toEqual("Hello world")

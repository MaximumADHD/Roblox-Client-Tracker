...
local getFFlagChatTranslationUIEnabledBubbleChat =
	require(ExperienceChat.Flags.getFFlagChatTranslationUIEnabledBubbleChat)
...
function BubbleChatList.getDerivedStateFromProps(nextProps, lastState)
	...
	for _, message in ipairs(nextProps.messages) do
		...
		table.insert(bubbles, {
			...
			fadingOut = false,
			text = message.text,
			translation = if getFFlagChatTranslationUIEnabledBubbleChat() then message.translation else nil,
			useTranslation = if getFFlagChatTranslationUIEnabledBubbleChat() then message.useTranslation else nil,
	...
	for index, bubble in ipairs(self.state.bubbles) do
		...
		if getFixBubblesOutofOrderWhenZoomedIn() then
			children["Bubble" .. bubble.messageId] = Roact.createElement(ChatBubble, {
				messageId = bubble.messageId,
				text = bubble.text or "",
				translation = if getFFlagChatTranslationUIEnabledBubbleChat() then bubble.translation else nil,
				useTranslation = if getFFlagChatTranslationUIEnabledBubbleChat() then bubble.useTranslation else nil,

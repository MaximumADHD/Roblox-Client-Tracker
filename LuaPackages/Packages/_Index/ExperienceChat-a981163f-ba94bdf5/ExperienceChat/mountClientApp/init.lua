...
type Config = {
	...
	isCameraOnlyUser: (() -> boolean)?,
	validateLegacyBubbleChatSettings: any?,
	isVoiceServiceInitialized: (() -> any)?,
}
...
return function(config: Config): { roactInstance: any }
	...
	-- Add support for mounting legacy bubble chat within TextChatService
	if GlobalFlags.ConsolidateBubbleChat then
		local onBubbleChatEnabledChanged = function()
			local enabled = Chat.BubbleChatEnabled

			store:dispatch(LegacyBubbleChatEnabledChanged("Enabled", enabled))
		end

		Chat:GetPropertyChangedSignal("BubbleChatEnabled"):Connect(onBubbleChatEnabledChanged)
		onBubbleChatEnabledChanged()

		Chat.BubbleChatSettingsChanged:Connect(function(chatSettings)
			if config.validateLegacyBubbleChatSettings then
				local ok, message = config.validateLegacyBubbleChatSettings(chatSettings)
				assert(
					ok,
					"Bad settings object passed to Chat:SetBubbleChatSettings:\n inside consolidated BubbleChat "
						.. (message or "")
				)
				store:dispatch(LegacyBubbleChatSettingsChanged(chatSettings))
			end
		end)
	end
	...
	local root = Roact.createElement(AppStyleProvider, {
		...
		app = Roact.createElement(App, {
			...
			isCameraOnlyUser = config.isCameraOnlyUser,
			isVoiceServiceInitialized = config.isVoiceServiceInitialized,
		}),
	})

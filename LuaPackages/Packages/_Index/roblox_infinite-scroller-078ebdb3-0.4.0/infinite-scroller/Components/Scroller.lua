...
function Scroller:didMount()
	self.motor:onStep(function(value)
		...
		if self:getCurrent() then
			...
			self.indexChanged.currentPos = value
		end
	end)
...
local function singleUseSpring(initalPosition, finalPosition, inputOption, lock)
	debugPrint("singleUseSpring", initalPosition, " to ", finalPosition, "lock", lock)
	local otterSpring = Otter.spring(finalPosition, inputOption)
	local stepOverride = function(self, state, dt)
		if self.__lock ~= lock then
			state.value = initalPosition
			self.__lock = lock
		end
		local stepState = otterSpring.step(self, state, dt)
		if Round.nearest(stepState.value) == Round.nearest(finalPosition) then
			stepState.value = Round.nearest(finalPosition)
			stepState.complete = true
		end
		return stepState
	end
	otterSpring.step = stepOverride
	return otterSpring
end
...
function Scroller:scrollToAnchor()
	...
	self.motor:setGoal(singleUseSpring(self.indexChanged.currentPos, self.indexChanged.newPos,
		self.props.animateOptions, self.springLock))
end

...
return function(command: CommandTypes.Command, textSource: TextSource, rawText: string)
	return Promise.try(function()
		assert(context.store, "Store should exist")
		if matchesAlias(command.alias, rawText) then
			local systemMessageResponse = command.clientRun(context.store, textSource, rawText)

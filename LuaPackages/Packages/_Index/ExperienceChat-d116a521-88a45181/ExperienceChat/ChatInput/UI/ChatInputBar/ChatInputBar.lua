...
function ChatInputBar:init()
	...
	self.onTextChanged = function(rbx)
		...
		if self.props.textSource then
			local promises = {
				autoCastCommand(RBXTeamCommand, self.props.textSource, newText),
				autoCastCommand(RBXWhisperCommand, self.props.textSource, newText),
			}
	...
	self.onBackspacePressedConnection = self.props.userInputService.InputEnded:connect(function(inputObj, _)
		...
		then
			...
			self.props.chatInputBarResetTargetChannel()
		end
	end)

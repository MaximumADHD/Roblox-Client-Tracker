...
local Batchinator = require(srcWorkspace.Utils["react-native"].Libraries.Interaction.Batchinator)
...
local ScrollView = require(srcWorkspace.Utils["react-native"].Libraries.Components.ScrollView.ScrollView)
local StyleSheet = require(srcWorkspace.Utils.stylesheet)

local View = require(srcWorkspace.Utils.view)
...
local infoLog = require(srcWorkspace.Utils["react-native"].Libraries.Utilities.infoLog)
local invariant = require(srcWorkspace.Utils.invariant)
...
local ScrollViewModule = require(srcWorkspace.Utils["react-native"].Libraries.Components.ScrollView.ScrollView)
...
function VirtualizedList:init(props: Props)
	...
	self._invertedCanvasLength = self.props.invertedCanvasLength or 1000
	...
	self._updateCellsToRender = function()
		...
		self:setState(function(state)
			...
			if Boolean.toJSBoolean(newState) and self._nestedChildLists.size > 0 then
				...
				while ii <= newLast do
					...
					for _, childKey in ipairs(childListKeys) do
						...
						if
							Boolean.toJSBoolean(childList)
							and Boolean.toJSBoolean(childList.ref)
							and childList.ref:hasMore()
						then
	...
	self._getNestedChildState = function(key: string): ChildListState?
		...
		return if Boolean.toJSBoolean(existingChildData) then existingChildData.state else existingChildData
	end
	...
	self._registerAsNestedChild = function(
		...
		if Boolean.toJSBoolean(existingChildData) and existingChildData.ref ~= nil then
			console.error(
		...
		elseif self.props.inverted and width > 0 and height > 0 and not self._hasDoneInitialScroll then
			...
			if self.props.contentOffset == nil then
				...
				if self.props.horizontal then
					-- ROBLOX TODO: This doesn't seem to cause a scroll event
	...
	self._onScroll = function(rbx): ()
		self._nestedChildLists:forEach(function(childList)
			if Boolean.toJSBoolean(childList.ref) then
		...
		if
			...
			and not Boolean.toJSBoolean(self._hasWarned.perf)
		then
			infoLog(
		...
		if not Boolean.toJSBoolean(self.props) then
			return
		end
		self:_maybeCallOnEndReached()
		if velocity ~= 0 then
...
function VirtualizedList:scrollToEnd(params: {
	...
	local offset = math.max(0, frame.offset + frame.length + self._footerLength - self._scrollMetrics.visibleLength)
...
VirtualizedList.getDerivedStateFromProps = function(newProps: Props, prevState: State): State
	...
	local newFirstItem = (
			if newProps.data and newProps.getItemCount(newProps.data) > 0
				then newProps.getItem(newProps.data, 1)
				else nil
		) :: any?
	...
	then
		scrollPositionDelta = if compare(prevFirstItemTracked, newFirstItem)
			then 0
			else findItem(newProps.data, prevFirstItemTracked) - 1
	end

	if scrollPositionDelta > 0 then
		for i = 1, scrollPositionDelta do
			newProps.getItem(newProps.data, i).shouldResizeOnMount = true
		end
	end
	...
	return {
		first = math.max(1, math.min(prevState.first, getItemCount(data) - maxToRenderPerBatch)), -- ROBLOX deviation: index starts at 1
		last = math.max(1, math.min(prevState.last, getItemCount(data))), -- ROBLOX deviation: index starts at 1,
...
function VirtualizedList:render(): React_Node
	...
	if itemCount > 0 then
		...
		if (not Boolean.toJSBoolean(self._hasWarned.keys)) and Boolean.toJSBoolean(_usedIndexForKey) then
			console.warn(
	...
	then
		if self:getScrollRef() ~= nil and self:getScrollRef()._nativeRef.current ~= nil then
			...
			elseif
				listEndOffset ~= (self._invertedCanvasLength - self._footerLength)
				and self.state.scrollPositionDelta == 0
			then
	...
	-- ROBLOX TODO: Handle case where getItemLayout is defined and we add more items to the list
	-- ROBLOX DEVIATION END

	local scrollProps = Object.assign({}, self.props, {
	...
	if _G.__DEV__ then
		ret = React.createElement(ScrollView.Context.Consumer, nil, function(scrollContext)
			if
				...
				and not Boolean.toJSBoolean(self._hasWarned.nesting)
...
function VirtualizedList:_onCellLayout(rbx, cellKey: string, index: number): ()
	...
	if not curr or math.abs(next_.offset - curr.offset) > 1 or next_.length ~= curr.length or index ~= curr.index then
		...
		if not self.props.getItemLayout and item.shouldResizeOnMount then
...
function VirtualizedList:_triggerRemeasureForChildListsInCell(cellKey: string): ()
	...
	if Boolean.toJSBoolean(childListKeys) then
		childListKeys.forEach(function(childKey)
			local childList = self._nestedChildLists:get(childKey)
			if Boolean.toJSBoolean(childList) and Boolean.toJSBoolean(childList.ref) then
...
function VirtualizedList:_scheduleCellsToRenderUpdate(): ()
	...
	if first > 1 then --[[ ROBLOX deviation: index starts at 1 ]]
		...
		hiPri = (Boolean.toJSBoolean(hiPri) and hiPri)
			or distTop < 0
			or (velocity < -2 and distTop < scrollingThreshold)
	end
	...
	if last < itemCount then --[[ ROBLOX deviation: index starts at 1 ]]
		...
		hiPri = (Boolean.toJSBoolean(hiPri) and hiPri) or distBottom < 0 or velocity > 2
	end
...
function CellRenderer:render()
	...
	local result = if not Boolean.toJSBoolean(CellRendererComponent)
      ...
      To see the error, delete this comment and run Flow. ]]
			...
			Object.assign({}, self.props, { style = cellStyle, onLayout = onLayout }),
			if itemSeparator
				then React.createElement("UIListLayout", {
					SortOrder = Enum.SortOrder.LayoutOrder,

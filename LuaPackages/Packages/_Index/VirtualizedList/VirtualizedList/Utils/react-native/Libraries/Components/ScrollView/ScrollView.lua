...
type ScrollView = ScrollViewImperativeMethods & {
	...
	_onStickyHeaderLayout: (self: ScrollView, index: any, event: any, key: any) -> any,
...
function ScrollView:init(props: Props)
	...
	self.scrollResponderScrollNativeHandleToKeyboard = function<T>(
		...
		self._preventNegativeScrollOffset = not not Boolean.toJSBoolean(preventNegativeScrollOffset)
	...
	self._handleScrollEndDrag = function(rbx, input)
		...
		if
			not Boolean.toJSBoolean(self:_isAnimating())
			and (not Boolean.toJSBoolean(velocity) or velocity.x == 0 and velocity.y == 0)
		then
			FrameRateLogger:endScroll()
		end
	...
	self._handleResponderRelease = function(e: PressEvent)
		...
		if
			...
			and not Boolean.toJSBoolean(self._observedScrollSinceBecomingResponder)
			and not Boolean.toJSBoolean(self._becameResponderWhileAnimating)
		then
	...
	self._handleResponderTerminationRequest = function()
		return not Boolean.toJSBoolean(self._observedScrollSinceBecomingResponder)
	end
	...
	self._handleStartShouldSetResponder = function(e: PressEvent)
		...
		if
			...
			and Boolean.toJSBoolean(self:_keyboardIsDismissible())
	...
	self._handleStartShouldSetResponderCapture = function(e: PressEvent)
		...
		if Boolean.toJSBoolean(self:_isAnimating()) then
			return true
		end
		...
		if
			Boolean.toJSBoolean(keyboardNeverPersistTaps)
			and Boolean.toJSBoolean(self:_keyboardIsDismissible())
			and e.target ~= nil
	...
	self._keyboardIsDismissible = function()
		...
		return if Boolean.toJSBoolean(hasFocusedTextInput) then softKeyboardMayBeOpen else hasFocusedTextInput
	end
	...
	self._handleTouchEnd = function(rbx, input: InputObject)
		...
		if Boolean.toJSBoolean(self.props.onTouchEnd) then
	...
	self._handleTouchCancel = function(e: PressEvent)
		...
		if Boolean.toJSBoolean(self.props.onTouchCancel) then
	...
	self._handleTouchStart = function(rbx, input: InputObject)
		...
		if Boolean.toJSBoolean(self.props.onTouchStart) then
	...
	self._handleTouchMove = function(rbx, input: InputObject)
		if Boolean.toJSBoolean(self.props.onTouchMove) then
			self.props.onTouchMove(rbx, input)
		end
	end

	self._scrollAnimatedValue = AnimatedImplementation.Value.new(
		if Boolean.toJSBoolean(self.props.contentOffset) and Boolean.toJSBoolean(self.props.contentOffset.y)
			then self.props.contentOffset.y
			else 0
	)
	self._scrollAnimatedValue:setOffset(
		if Boolean.toJSBoolean(self.props.contentInset) and Boolean.toJSBoolean(self.props.contentInset.top)
		...
		console.warn(
			("'keyboardShouldPersistTaps={%s}' is deprecated. "):format(
				...
				.. ("Use 'keyboardShouldPersistTaps=\"%s\"' instead"):format(
					Boolean.toJSBoolean(self.props.keyboardShouldPersistTaps) and "always" or "never"
				)
		)
	end
...
function ScrollView:componentDidUpdate(prevProps: Props)
	local prevContentInsetTop = if Boolean.toJSBoolean(prevProps.contentInset) then prevProps.contentInset.top else 0
	local newContentInsetTop = if Boolean.toJSBoolean(self.props.contentInset) then self.props.contentInset.top else 0
...
function ScrollView:componentWillUnmount()
	...
	if Boolean.toJSBoolean(self._scrollAnimatedValueAttachment) then
...
function ScrollView._getKeyForIndex(index, childArray)
	...
	return if Boolean.toJSBoolean(child) then child.key else child
end
...
function ScrollView:_updateAnimatedNodeAttachment()
	...
	if Boolean.toJSBoolean(self.props.stickyHeaderIndices) and #self.props.stickyHeaderIndices > 0 then
...
function ScrollView:_onStickyHeaderLayout(index, event, key)
	...
	if not Boolean.toJSBoolean(stickyHeaderIndices) then
		return
	end
	...
	if previousHeaderIndex ~= nil then
		...
		if Boolean.toJSBoolean(previousHeader) and Boolean.toJSBoolean(previousHeader.setNextHeaderY) then
...
function ScrollView:render()
	...
	if _G.__DEV__ and self.props.style ~= nil then
		...
		local childLayoutProps = Array.filter({ "alignItems", "justifyContent" }, function(prop)
			return Boolean.toJSBoolean(style) and style[tostring(prop)] ~= nil
		end)
		invariant(
			#childLayoutProps == 0,
	...
	if stickyHeaderIndices ~= nil and #stickyHeaderIndices > 0 then
		...
		children = Array.map(childArray, function(child, index)
			...
			if indexOfIndex > -1 then
				...
				local StickyHeaderComponent = Boolean.toJSBoolean(self.props.StickyHeaderComponent)
						and self.props.StickyHeaderComponent
					or ScrollViewStickyHeader
	...
	local props = Object.assign({}, self.props, {
	...
	if Boolean.toJSBoolean(refreshControl) then
		if Platform.OS == "ios" or Platform.OS == "roblox" then
			...
			return React.createElement(
				...
				Object.assign({}, props, { ref = self._setNativeRef }),
				refreshControl,
				contentContainer
			)
...
local function Wrapper(props, ref)
	return React.createElement(ScrollView, Object.assign({}, props, { scrollViewRef = ref }))
end

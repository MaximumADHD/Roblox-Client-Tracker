...
function ScrollViewStickyHeader:init(props)
	...
	self._onLayout = function(_self, event: any)
		...
		if Boolean.toJSBoolean(child.props.onLayout) then
...
function ScrollViewStickyHeader:updateTranslateListener(
	...
	if not Boolean.toJSBoolean(self._animatedValueListener) then
		...
		self._animatedValueListener = function(ref)
			...
			if value == 0 and not Boolean.toJSBoolean(self._haveReceivedInitialZeroTranslateY) then
				self._haveReceivedInitialZeroTranslateY = true
				return
			end
			if Boolean.toJSBoolean(self._timer) then
...
function ScrollViewStickyHeader:render(): React_Node
	-- Fabric Detection
	local isFabric = Boolean.toJSBoolean(self._ref)
		and Boolean.toJSBoolean(self._ref["_internalInstanceHandle"])
		and Boolean.toJSBoolean(self._ref["_internalInstanceHandle"].stateNode)
	...
	if Boolean.toJSBoolean(self._shouldRecreateTranslateY) then
		...
		if Boolean.toJSBoolean(measured) then
			if Boolean.toJSBoolean(inverted) then
		...
		self:updateTranslateListener(
			...
			isFabric,
			if Boolean.toJSBoolean(self.props.hiddenOnScroll)

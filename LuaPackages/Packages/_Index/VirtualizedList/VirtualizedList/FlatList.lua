...
local reactNativeDir = srcWorkspace.Utils["react-native"]
local deepDiffer = require(reactNativeDir.Libraries.Utilities.differ.deepDiffer)
local React = require(Packages.React)
type React_Element_Config<ItemT> = React.ElementConfig<ItemT>
type React_Node = React.React_Node

local VirtualizedList = require(srcWorkspace.VirtualizedList)
...
local scrollViewWorkspace = srcWorkspace.Utils["react-native"].Libraries.Components.ScrollView
...
local ViewabilityHelperModule = require(srcWorkspace.ViewabilityHelper)
...
local VirtualizedListModule = require(srcWorkspace.VirtualizedList)
...
function FlatList:_checkProps<ItemT>(props: Props<ItemT>)
	...
	invariant(
		not (Boolean.toJSBoolean(onViewableItemsChanged) and Boolean.toJSBoolean(viewabilityConfigCallbackPairs)),

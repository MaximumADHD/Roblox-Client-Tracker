...
local EnableEmojiAutocomplete = require(ExperienceChat.Flags.FFlagEnableEmojiAutocomplete)
...
local function ChatInputBar(props)
	...
	local onAutocompleteSelected = React.useCallback(function(rbx)
		...
		if textBox then
			...
			local newText = string.gsub(textBeforeCursor, patternToReplace, emojiUnicode)
	...
	React.useEffect(function()
		if EnableEmojiAutocomplete() then
			local textBox = textBoxRef.current
			if textBox then
	...
	local onFocusLost = React.useCallback(function(_, enterPressed: boolean, inputObj: InputObject)
		...
		if isMountedRef.current then
			if EnableEmojiAutocomplete() and autocompleteDropdown and props.autocompleteDropdownReducer.activated then
	...
	-- function to handle emoji autocomplete
	local function handleEmojiAutocomplete(rbx, newText, lastChar)
		-- Check if reducer is either activated or has results
		...
		if props.autocompleteDropdownReducer.activated or #props.autocompleteDropdownReducer.results > 0 then
			...
			elseif lastChar == ":" then
				...
				if index ~= emojiIndex then
					...
					if props.autocompleteDropdownReducer.results[1] == autocompleteInput then
	...
	local onTextChanged = React.useCallback(function(rbx)
		...
		if EnableEmojiAutocomplete() and rbx.CursorPosition < #newText + 1 then
		...
		if EnableEmojiAutocomplete() then
			handleEmojiAutocomplete(rbx, newText, lastChar)
		end
	...
	return React.createElement("Frame", {
		...
		AutocompleteDropdown = EnableEmojiAutocomplete() and React.createElement(AutocompleteDropdown, {

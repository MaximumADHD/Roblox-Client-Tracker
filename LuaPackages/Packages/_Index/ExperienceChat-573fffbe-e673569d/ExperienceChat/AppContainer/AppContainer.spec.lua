...
return function()
	...
	describe("WHEN mounted and ChatVersion differs", function()
		...
		it("SHOULD render Chat Window, Chat Input Bar, and Bubble Chat for a TCS experience", function(c)
			local mountResult = c.mount({}, c)
			...
			expect(c.findFirstInstance(mountResult.instance, { Name = "bubbleChat" })).never.toBeNil()
			mountResult.unmount()
		end)

		it("SHOULD only render Bubble Chat for a legacy chat experience", function(c)
			local createProps = function(_)
				return {
					textChatService = {
			...
			local mountResult = c.mount({}, c, createProps)

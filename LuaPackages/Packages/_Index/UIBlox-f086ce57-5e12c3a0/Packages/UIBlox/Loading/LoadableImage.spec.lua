return function()
	...
	local LoadableImage = require(ComponentRoot.LoadableImage)
	...
	describe("LoadableImage", function()
		...
		it("should not set loading image if image is already in cache", function()
			...
			local element = mockStyleComponent({
				TestProvider = Roact.createElement(ImageSetProvider, {
					imageSetData = {},
				}, {
					Image = Roact.createElement(LoadableImage, {
						Image = testImage,
			...
			expect(container.LoadableImageSample.TestProvider.Image).never.to.equal(defaultLoadImage)
			Roact.unmount(instance)
		end)

...
return function(options)
	...
	return Rodux.createReducer({}, {
		...
		[NetworkingUsers.SetDescription.Succeeded.name] = function(state, action)
			local response = action.responseBody
			local userId = tostring(action.additionalData.LocalUserId)

			local user = {
				[userId] = {
					isDeleted = state[userId].isBanned,
					description = response.description,
					hasVerifiedBadge = state[userId].hasVerifiedBadge,
				},
			}
			return Dash.join(state, user)
		end,

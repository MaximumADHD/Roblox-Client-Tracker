...
function module.postProcessTree(node: Node, options: ParserOptions): { Node }
	...
	while i <= #nodes do
		...
		elseif PLAIN_TEXT[child.name] then
			local textCount, textNode = module.mergeTextNodes(nodes, i, nil, options)
...
function module.buildParagraph(
	nodes: { Node },
	offset: number,
	...
	while i <= #nodes do
		...
		if PLAIN_TEXT[node.name] then
			local textCount, textNode = module.mergeTextNodes(nodes, i, includeWhitespace, options)
			-- Only add text nodes that actually have text
			if textNode.text and #textNode.text > 0 then
				insert(children, textNode)
			end
			...
			count += textCount
			i += textCount
		elseif node.name == "STYLED_TEXT" then
			-- Process the children of the styled text node as if it were a paragraph,
			-- then extract the processed children
			local _, paragraphNode = module.buildParagraph(node.children, 1, options, true)
			if node.attributes and node.attributes.style == "INLINE_CODE" then
				node.children = module.applyCodeSpanRules(paragraphNode.children)
			else
				node.children = paragraphNode.children
			end
...
function module.outputRichText(node: Node, options: ParserOptions): string
	...
	if tag then
		...
		local count, output = module.mergeTextNodes(node.children, 1, nil, options)
...
function module.mergeTextNodes(
	nodes: { Node },
	offset: number,
	includeWhitespace: boolean?,
	options: ParserOptions
...
function module.applyCodeSpanRules(nodes: { Node }): { Node }
	local startNode = nil
	local endNode = nil
	local containsNonSpace = false
	...
	local startText = (startNode.text or ""):gsub("\n", " ")
	local endText = (endNode.text or ""):gsub("\n", " ")

	-- If the content string both begins and ends with a space character,
	-- but does not consist entirely of space characters,
	-- a single space character is removed from the front and back.
	if startText:sub(1, 1) == " " and endText:sub(-1, -1) == " " and containsNonSpace then
		if startNode == endNode then
			startText = startText:sub(2, -2)
			endText = startText
		else
			startText = startText:sub(2)
			endText = endText:sub(1, -2)
		end
	end

	startNode.text = startText
	endNode.text = endText

...
return function()
	...
	it("SHOULD return alphabetically sorted list of strings containing inputted prefix", function(c)
		local res = c.autocompleteText:getResults("sm", sortEmojiResultsCallback)
		for i = 1, List.count(res) - 1 do
			local isSortedAndMatchesPrefix = res[i] < res[i + 1] and res[i]:find("sm", 1, true) == 1

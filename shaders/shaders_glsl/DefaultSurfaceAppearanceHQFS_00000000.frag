...
void main()
{
    ...
    float f42 = 1.0 - f20;
    vec3 f43 = ((((((vec3(f42) - (f39 * (f18 * f42))) * CB0[15].xyz) * f32) + (CB0[17].xyz * (f42 * clamp(-f30, 0.0, 1.0)))) + (min((f25.xyz * (f25.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f26.x)), vec3(CB0[21].w)) * 1.0)) * f16.xyz) + (((f39 * (((f40 + (f40 * f40)) / max(((f41 * f41) * ((f35 * 3.0) + 0.5)) * ((f34 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f32)) * CB0[15].xyz) * 1.0);
    vec4 f44 = vec4(0.0);
    f44.x = f43.x;
    vec4 f45 = f44;
    f45.y = f43.y;

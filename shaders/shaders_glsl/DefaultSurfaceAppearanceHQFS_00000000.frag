...
void main()
{
    ...
    float f2 = f1.w;
    vec4 f3 = mix(vec4(f1.xyz, VARYING2.w * f2), vec4(mix(VARYING2.xyz, f1.xyz, vec3(f2)), VARYING2.w), vec4(CB3[0].x));
    vec2 f4 = texture2D(NormalMapTexture, VARYING0).wy * 2.0;
    vec2 f5 = f4 - vec2(1.0);
    ...
    vec4 f8 = texture2D(SpecularMapTexture, VARYING0);
    ...
    float f28 = f21.z;
    vec3 f29 = -CB0[16].xyz;
    float f30 = dot(f12, f29) * ((1.0 - ((step(f27.x, f28) * clamp(CB0[29].z + (CB0[29].w * abs(f28 - 0.5)), 0.0, 1.0)) * f27.y)) * f26.y);
    vec3 f31 = normalize((VARYING4.xyz / vec3(f0)) - CB0[16].xyz);
    float f32 = clamp(f30, 0.0, 1.0);
    float f33 = f19 * f19;
    float f34 = max(0.001000000047497451305389404296875, dot(f12, f31));
    float f35 = dot(f29, f31);
    float f36 = 1.0 - f35;
    float f37 = f36 * f36;
    float f38 = (f37 * f37) * f36;
    vec3 f39 = vec3(f38) + (mix(vec3(0.039999999105930328369140625), f17.xyz, vec3(f20)) * (1.0 - f38));
    float f40 = f33 * f33;
    float f41 = (((f34 * f40) - f34) * f34) + 1.0;
    float f42 = 1.0 - f20;
    vec3 f43 = ((((((vec3(f42) - (f39 * (f18 * f42))) * CB0[15].xyz) * f32) + (CB0[17].xyz * (f42 * clamp(-f30, 0.0, 1.0)))) + (min((f25.xyz * (f25.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f26.x)), vec3(CB0[21].w)) * 1.0)) * f17.xyz) + (((f39 * (((f40 + (f40 * f40)) / max(((f41 * f41) * ((f35 * 3.0) + 0.5)) * ((f34 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f32)) * CB0[15].xyz) * 1.0);
    vec4 f44 = vec4(0.0);
    f44.x = f43.x;
    vec4 f45 = f44;
    f45.y = f43.y;
    ...
    vec4 f48 = f46;
    f48.w = f47;
    vec3 f49 = mix(CB0[19].xyz, f48.xyz, vec3(clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f50 = f48;
    f50.x = f49.x;
    vec4 f51 = f50;
    f51.y = f49.y;

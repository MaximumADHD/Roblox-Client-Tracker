...
void main()
{
    ...
    for (;;)
    {
        if (f35 < f18)
        {
            ...
            vec2 f44 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(texture3DLod(DetailTex3DTexture, vec4((f24 * (0.5 * CB5[4].y)) + (f41 * f43.x), f39).xyz, f39).x, texture3DLod(DetailTex3DTexture, vec4((f24 * (2.0 * CB5[4].y)) + (f41 * f43.y), f39).xyz, f39).x) * 2.0) - vec2(1.0));
            float f45 = texture3DLod(CloudsDistanceFieldTexture, vec4(((f24 + (f41 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f39).xyz, f39).x + (f44.x + f44.y);
            float f46 = (f42 * 50.0) * (CB5[2].x - f45);
            bool f47 = f45 < CB5[2].x;
            if (f47)
            {
                vec4 f48 = texture2DLod(BeerShadowMapTexture, vec4(f24.xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f49 = min(f48.z * 0.5, (f48.y + (0.3499999940395355224609375 * texture2DLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)) * max(0.0, (1.0 - ((float(f35) / f19) + (f45 / f19))) - f48.x));
                float f50 = f37 + f46;
                vec3 f51 = f3 * f3;
    ...
    vec3 f75 = mix(CB0[19].xyz, f73.xyz, vec3(f74));
    vec4 f76 = f73;
    f76.x = f75.x;
    vec4 f77 = f76;
    f77.y = f75.y;

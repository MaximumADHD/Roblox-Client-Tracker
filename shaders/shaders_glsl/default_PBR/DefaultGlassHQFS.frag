...
void main()
{
    ...
    vec4 f32 = texture2D(PrecomputedBRDFTexture, vec2(f12, f29));
    vec3 f33 = textureCube(EnvironmentMapTexture, reflect(-VARYING4.xyz, f15)).xyz;
    vec3 f34 = vec3(f22);
    vec3 f35 = mix(f21, (f33 * f33) * CB0[15].x, f34) * mix(vec3(1.0), f25, vec3(0.5));
    vec3 f36 = -CB0[11].xyz;
    ...
    float f61 = f55.y;
    float f62 = f55.z;
    vec3 f63 = (mix(textureCubeLod(PrefilteredEnvIndoorTexture, f31, f30).xyz, textureCubeLod(PrefilteredEnvTexture, f31, f30).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f15.y * 1.58823525905609130859375, 0.0, 1.0))), f34) * f51) * f28;
    float f64 = 1.0 - f29;

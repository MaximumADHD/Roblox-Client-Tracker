...
void main()
{
    ...
    vec4 f33 = texture2D(PrecomputedBRDFTexture, vec2(f12, f30));
    vec3 f34 = textureCube(EnvironmentMapTexture, reflect(-VARYING4.xyz, f16)).xyz;
    vec3 f35 = vec3(f23);
    vec3 f36 = mix(f22, (f34 * f34) * CB0[15].x, f35) * mix(vec3(1.0), f26, vec3(0.5));
    vec3 f37 = -CB0[11].xyz;
    ...
    float f62 = f56.y;
    float f63 = f56.z;
    vec3 f64 = (mix(textureCubeLod(PrefilteredEnvIndoorTexture, f32, f31).xyz, textureCubeLod(PrefilteredEnvTexture, f32, f31).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f16.y * 1.58823525905609130859375, 0.0, 1.0))), f35) * f52) * f29;
    float f65 = 1.0 - f30;
    ...
    vec4 f78 = texture2D(GBufferColorTexture, f72 + (f76 * clamp(min(texture2D(GBufferDepthTexture, f74 + f77).x * 500.0, texture2D(GBufferDepthTexture, f74 - f77).x * 500.0) - f71, 0.0, 1.0)));
    vec3 f79 = f78.xyz;

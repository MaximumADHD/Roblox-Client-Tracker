...
void main()
{
    vec4 f0 = mix(texture2D(DiffuseMapTexture, VARYING0 * CB2[2].y), texture2D(DiffuseMapTexture, VARYING0), vec4(clamp((clamp(1.0 - (VARYING4.w * 0.0033333334140479564666748046875), 0.0, 1.0) * CB2[4].x) - (0.60000002384185791015625 * CB2[4].x), 0.0, 1.0)));
    ...
    vec3 f9 = (f1 * f1).xyz;
    vec3 f10 = (((VARYING5.xyz * f8) + min((f5.xyz * (f5.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f6.x)), vec3(CB0[16].w))) * f9) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f9, vec3(VARYING6.w))) * (VARYING5.w * f8));

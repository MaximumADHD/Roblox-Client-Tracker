...
void main()
{
    ...
    float f23 = f15.y;
    vec3 f24 = -CB0[11].xyz;
    vec3 f25 = normalize(f24 + normalize(VARYING7.xyz));
    ...
    vec3 f37 = (((((((((vec3(1.0) - (f34 * (CB0[26].w * f13))) * 1.0) * CB0[10].xyz) * f27) + ((CB0[12].xyz * 1.0) * clamp(-f26, 0.0, 1.0))) * f22) + min((f19.xyz * (f19.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f20.x)), vec3(CB0[16].w))) + vec3((f15.z * 2.0) * f13)) * (f12 * f12).xyz) + ((((((f34 * ((f35 + (f35 * f35)) / (((f36 * f36) * ((f30 * 3.0) + 0.5)) * ((f29 * 0.75) + 0.25)))) * CB0[10].xyz) * f27) * f22) * f13) * VARYING0.w);

...
void main()
{
    ...
    gl_FragData[0] = vec4(sqrt(clamp(mix(vec4((((((((CB1[12] * f5.x) + (CB1[13] * f7.x)) + (CB1[14] * f5.y)) + (CB1[15] * f7.y)) + (CB1[16] * f5.z)) + (CB1[17] * f7.z)).xyz * CB0[14].xyz) + (CB0[10].xyz * (clamp(pow(vec3(0.5 + (dot(f2, -CB0[11].xyz) * 0.5)), CB1[11].xyz) * (1.0 - f3), vec3(0.0), vec3(1.0)) * CB1[11].w)), f8).xyz, (f1 * f1).xyz, vec3(clamp(f8, 0.0, 1.0))).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) * f0.w, 1.0 - f8);
}

...
void main()
{
    ...
    float f66 = f59.z;
    vec3 f67 = -CB0[16].xyz;
    float f68 = (dot(f56, f67) * CB0[14].w) * ((1.0 - ((step(f65.x, f66) * clamp(CB0[29].z + (CB0[29].w * abs(f66 - 0.5)), 0.0, 1.0)) * f65.y)) * f64.y);
    vec3 f69 = normalize(normalize(VARYING8) - CB0[16].xyz);
    ...
    float f80 = 1.0 - f58;
    vec3 f81 = (((((((vec3(f80) - (f77 * ((CB0[31].w * f52) * f80))) * CB0[15].xyz) * f70) + (CB0[17].xyz * (f80 * clamp(-f68, 0.0, 1.0)))) + (min((f63.xyz * (f63.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f64.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f14.z * 2.0) * f52)) * f51) + ((((f77 * (((f78 + (f78 * f78)) / (((f79 * f79) * ((f73 * 3.0) + 0.5)) * ((f72 * 0.75) + 0.25))) * f70)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * (f52 * VARYING0.w));
    vec4 f82 = vec4(0.0);
    f82.x = f81.x;
    vec4 f83 = f82;
    f83.y = f81.y;
    ...
    vec4 f85 = f84;
    f85.w = 1.0;
    vec3 f86 = mix(CB0[19].xyz, f85.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f87 = f85;
    f87.x = f86.x;
    vec4 f88 = f87;
    f88.y = f86.y;

...
void main()
{
    ...
    vec3 f21 = f16 + ((f2 * f10) * (0.1500000059604644775390625 + (0.949999988079071044921875 * texture2DLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)));
    f21.y = 0.0;
    bool f22;
    vec3 f23;
    vec4 f24;
    ...
    for (;;)
    {
        if (f34 < f17)
        {
            ...
            vec2 f39 = (texture2DLod(AdvectionTexTexture, vec4((f23.xz * 0.125) + vec2(0.0, CB5[0].w * (-17.5)), 0.0, f38).xy, f38).xy * 2.0) - vec2(1.0);
            ...
            vec2 f43 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(texture3DLod(DetailTex3DTexture, vec4((f23 * (0.5 * CB5[4].y)) + (f40 * f42.x), f38).xyz, f38).x, texture3DLod(DetailTex3DTexture, vec4((f23 * (2.0 * CB5[4].y)) + (f40 * f42.y), f38).xyz, f38).x) * 2.0) - vec2(1.0));
            float f44 = texture3DLod(CloudsDistanceFieldTexture, vec4(((f23 + (f40 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f38).xyz, f38).x + (f43.x + f43.y);
            float f45 = (f41 * 50.0) * (CB5[2].x - f44);
            bool f46 = f44 < CB5[2].x;
            if (f46)
            {
                vec4 f47 = texture2DLod(BeerShadowMapTexture, vec4(f23.xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f48 = min(f47.z * 0.5, (f47.y + (0.3499999940395355224609375 * texture2DLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)) * max(0.0, (1.0 - ((float(f34) / f18) + (f44 / f18))) - f47.x));
                float f49 = f36 + f45;
                vec3 f50 = f2 * f2;
    ...
    vec3 f68 = mix(CB0[14].xyz, (f32.xyz + (((((((CB0[10].xyz * clamp(1.0 - ((0.07999999821186065673828125 * f28) / f18), 0.0, 1.0)) * 2.0) * clamp(-f11, 0.0, 1.0)) * f15) * f66) * exp2(0.10999999940395355224609375 - (0.0350000001490116119384765625 * f28))) * CB5[3].xyz)).xyz, vec3(f67));
    vec4 f69 = vec4(f68.x, f68.y, f68.z, f32.w);
    f69.w = f66;
    vec4 f70 = f69;
    f70.w = f66 * max(f67, CB0[13].y);
    gl_FragData[0] = f70;
}

...
void main()
{
    ...
    float f28 = clamp(dot(f12, f26), 0.0, 1.0);
    float f29 = f24 * f24;
    float f30 = max(0.001000000047497451305389404296875, dot(f12, f27));
    ...
    float f38 = 1.0 - f25;
    vec3 f39 = ((((((vec3(f38) - (f35 * (f23 * f38))) * CB0[10].xyz) * f28) * f20) + min((f17.xyz * (f17.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f18.x)), vec3(CB0[16].w))) * f22) + (((f35 * (((f36 + (f36 * f36)) / (((f37 * f37) * ((f31 * 3.0) + 0.5)) * ((f30 * 0.75) + 0.25))) * f28)) * CB0[10].xyz) * f20);
    float f40 = f6.w;
    vec4 f41 = vec4(f39.x, f39.y, f39.z, vec4(0.0).w);
    f41.w = f40;
    vec3 f42 = sqrt(clamp(mix(CB0[14].xyz, f41.xyz, vec3(clamp(exp2((CB0[13].z * f21) + CB0[13].x) - CB0[13].w, 0.0, 1.0))).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))));
    vec4 f43 = vec4(f42.x, f42.y, f42.z, f41.w);
    f43.w = f40;
    gl_FragData[0] = f43;
}

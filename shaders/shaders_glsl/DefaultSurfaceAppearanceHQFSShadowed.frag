...
void main()
{
    ...
    float f29 = clamp(dot(f13, f27), 0.0, 1.0);
    float f30 = f25 * f25;
    float f31 = max(0.001000000047497451305389404296875, dot(f13, f28));
    ...
    float f39 = 1.0 - f26;
    vec3 f40 = ((((((vec3(f39) - (f36 * (f24 * f39))) * CB0[10].xyz) * f29) * f21) + min((f18.xyz * (f18.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f19.x)), vec3(CB0[16].w))) * f23) + (((f36 * (((f37 + (f37 * f37)) / (((f38 * f38) * ((f32 * 3.0) + 0.5)) * ((f31 * 0.75) + 0.25))) * f29)) * CB0[10].xyz) * f21);
    float f41 = f7.w;
    vec4 f42 = vec4(f40.x, f40.y, f40.z, vec4(0.0).w);
    f42.w = f41;
    vec2 f43 = min(VARYING0.wz, VARYING1.wz);
    float f44 = min(f43.x, f43.y) / f0;
    vec3 f45 = sqrt(clamp(mix(CB0[14].xyz, (f42.xyz * clamp((clamp((f0 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f44)) + f44, 0.0, 1.0)).xyz, vec3(clamp(exp2((CB0[13].z * f22) + CB0[13].x) - CB0[13].w, 0.0, 1.0))).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))));

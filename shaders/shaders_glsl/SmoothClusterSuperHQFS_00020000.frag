...
void main()
{
    ...
    float f80 = 1.0 - f58;
    vec3 f81 = (((((((vec3(f80) - (f77 * ((CB0[31].w * f52) * f80))) * CB0[15].xyz) * f70) + (CB0[17].xyz * (f80 * clamp(-f68, 0.0, 1.0)))) + (min((f63.xyz * (f63.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f64.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f14.z * 2.0) * f52)) * f51) + ((((f77 * (((f78 + (f78 * f78)) / max(((f79 * f79) * ((f73 * 3.0) + 0.5)) * ((f72 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f70)) * CB0[15].xyz) * 1.0) * (f52 * VARYING0.w));
    vec4 f82 = vec4(0.0);
    f82.x = f81.x;
    vec4 f83 = f82;
    f83.y = f81.y;

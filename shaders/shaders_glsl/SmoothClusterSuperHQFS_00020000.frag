...
void main()
{
    ...
    float f84 = 1.0 - f62;
    vec3 f85 = (((((((vec3(f84) - (f81 * ((CB0[31].w * f60) * f84))) * CB0[15].xyz) * f74) + (CB0[17].xyz * (f84 * clamp(-f72, 0.0, 1.0)))) + (min((f67.xyz * (f67.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f68.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f12.z * 2.0) * f60)) * f59) + ((((f81 * (((f82 + (f82 * f82)) / max(((f83 * f83) * ((f77 * 3.0) + 0.5)) * ((f76 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f74)) * CB0[15].xyz) * 1.0) * (f60 * VARYING0.w));
    vec4 f86 = vec4(0.0);
    f86.x = f85.x;
    vec4 f87 = f86;
    f87.y = f85.y;

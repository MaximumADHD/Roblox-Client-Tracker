...
void main()
{
    ...
    vec2 f5 = mix(texture2D(NormalMapTexture, f2 * CB2[1].z), texture2D(NormalMapTexture, f2), vec4(clamp((f3 * CB2[3].y) - (CB2[2].y * CB2[3].y), 0.0, 1.0))).wy * 2.0;
    vec2 f6 = f5 - vec2(1.0);
    ...
    vec2 f12 = f11.xy * f3;
    float f13 = f12.x;
    vec4 f14 = mix(texture2D(SpecularMapTexture, f2 * CB2[1].w), texture2D(SpecularMapTexture, f2), vec4(clamp((f3 * CB2[3].z) - (CB2[2].z * CB2[3].z), 0.0, 1.0)));
    ...
    vec3 f38 = normalize((VARYING4.xyz / vec3(f0)) + f36);
    ...
    float f49 = 1.0 - f27;
    vec3 f50 = ((((((vec3(f49) - (f46 * (f25 * f49))) * CB0[15].xyz) * f39) + (CB0[17].xyz * (f49 * clamp(-f37, 0.0, 1.0)))) + (min((f32.xyz * (f32.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f33.x)), vec3(CB0[21].w)) * 1.0)) * f24.xyz) + (((f46 * (((f47 + (f47 * f47)) / max(((f48 * f48) * ((f42 * 3.0) + 0.5)) * ((f41 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f39)) * CB0[15].xyz) * 1.0);
    vec4 f51 = vec4(0.0);
    f51.x = f50.x;
    vec4 f52 = f51;
    f52.y = f50.y;

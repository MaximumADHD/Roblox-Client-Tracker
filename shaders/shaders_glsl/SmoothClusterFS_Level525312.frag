...
void main()
{
    ...
    float f4 = clamp(dot(step(CB0[19].xyz, abs(VARYING4 - CB0[18].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    vec3 f10 = (min((f7.xyz * (f7.w * 120.0)) + (CB0[8].xyz + (CB0[9].xyz * f8.x)), vec3(CB0[16].w)) + (VARYING6 * ((1.0 - ((step(f9.x, VARYING5.z) * clamp(CB0[24].z + (CB0[24].w * abs(VARYING5.z - 0.5)), 0.0, 1.0)) * f9.y)) * f8.y))) * (f3 * f3);
    vec4 f11 = vec4(0.0);
    f11.x = f10.x;
    vec4 f12 = f11;
    f12.y = f10.y;
    ...
    vec4 f14 = f13;
    f14.w = 1.0;
    vec3 f15 = mix(CB0[14].xyz, f14.xyz, vec3(clamp(exp2((CB0[13].z * VARYING5.w) + CB0[13].x) - CB0[13].w, 0.0, 1.0)));
    vec4 f16 = f14;
    f16.x = f15.x;
    vec4 f17 = f16;
    f17.y = f15.y;

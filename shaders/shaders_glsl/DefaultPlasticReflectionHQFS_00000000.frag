...
void main()
{
    ...
    vec2 f3 = texture2D(NormalMapTexture, f2).wy * 2.0;
    vec2 f4 = f3 - vec2(1.0);
    ...
    vec3 f9 = f8;
    f9.y = f7.y;
    vec2 f10 = f9.xy * (clamp((vec2(0.0033333334140479564666748046875, CB0[28].y) * (-VARYING4.w)) + vec2(1.0), vec2(0.0), vec2(1.0)).y * CB2[3].w);
    vec4 f11 = vec4(VARYING2.xyz * (texture2D(DiffuseMapTexture, f2).x * 2.0), VARYING2.w);
    ...
    vec3 f22 = textureCube(EnvironmentMapTexture, reflect(-f1, f15)).xyz;
    ...
    float f30 = f23.z;
    vec3 f31 = -CB0[16].xyz;
    float f32 = dot(f15, f31) * ((1.0 - ((step(f29.x, f30) * clamp(CB0[29].z + (CB0[29].w * abs(f30 - 0.5)), 0.0, 1.0)) * f29.y)) * f28.y);
    vec3 f33 = normalize(f1 - CB0[16].xyz);
    ...
    vec3 f44 = ((((((vec3(1.0) - (f41 * (CB0[31].w * clamp(1.0 - (VARYING4.w * CB0[28].y), 0.0, 1.0)))) * CB0[15].xyz) * f34) + (CB0[17].xyz * clamp(-f32, 0.0, 1.0))) + (min((f27.xyz * (f27.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f28.x)), vec3(CB0[21].w)) * 1.0)) * mix(f20.xyz, (f22 * f22) * CB0[20].x, vec3(VARYING7.w))) + (((f41 * (((f42 + (f42 * f42)) / max(((f43 * f43) * ((f37 * 3.0) + 0.5)) * ((f36 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f34)) * CB0[15].xyz) * 1.0);
    vec4 f45 = vec4(0.0);
    f45.x = f44.x;
    vec4 f46 = f45;
    f46.y = f44.y;

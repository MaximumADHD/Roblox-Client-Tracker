...
void main()
{
    ...
    vec3 f44 = ((((((vec3(1.0) - (f41 * (CB0[31].w * clamp(1.0 - (VARYING4.w * CB0[28].y), 0.0, 1.0)))) * CB0[15].xyz) * f34) + (CB0[17].xyz * clamp(-f32, 0.0, 1.0))) + (min((f27.xyz * (f27.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f28.x)), vec3(CB0[21].w)) * 1.0)) * mix(f18.xyz, (f22 * f22) * CB0[20].x, vec3(VARYING7.w))) + (((f41 * (((f42 + (f42 * f42)) / max(((f43 * f43) * ((f37 * 3.0) + 0.5)) * ((f36 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f34)) * CB0[15].xyz) * 1.0);
    vec4 f45 = vec4(0.0);
    f45.x = f44.x;
    vec4 f46 = f45;
    f46.y = f44.y;

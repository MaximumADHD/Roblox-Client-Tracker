...
void main()
{
    ...
    float f53 = 1.0 - f31;
    vec3 f54 = ((((((vec3(f53) - (f50 * (f29 * f53))) * CB0[15].xyz) * f43) + (CB0[17].xyz * (f53 * clamp(-f41, 0.0, 1.0)))) + (min((f36.xyz * (f36.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f37.x)), vec3(CB0[21].w)) * 1.0)) * f27.xyz) + (((f50 * (((f51 + (f51 * f51)) / max(((f52 * f52) * ((f46 * 3.0) + 0.5)) * ((f45 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f43)) * CB0[15].xyz) * 1.0);
    vec4 f55 = vec4(0.0);
    f55.x = f54.x;
    vec4 f56 = f55;
    f56.y = f54.y;

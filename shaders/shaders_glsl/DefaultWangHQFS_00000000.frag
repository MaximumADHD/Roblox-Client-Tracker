...
void main()
{
    ...
    float f40 = f33.z;
    vec3 f41 = -CB0[16].xyz;
    float f42 = dot(f24, f41) * ((1.0 - ((step(f39.x, f40) * clamp(CB0[29].z + (CB0[29].w * abs(f40 - 0.5)), 0.0, 1.0)) * f39.y)) * f38.y);
    vec3 f43 = normalize((VARYING4.xyz / vec3(f0)) - CB0[16].xyz);
    ...
    vec3 f51 = vec3(f50) + (mix(vec3(0.039999999105930328369140625), f29.xyz, vec3(f32)) * (1.0 - f50));
    float f52 = f45 * f45;
    float f53 = (((f46 * f52) - f46) * f46) + 1.0;
    float f54 = 1.0 - f32;
    vec3 f55 = ((((((vec3(f54) - (f51 * (f30 * f54))) * CB0[15].xyz) * f44) + (CB0[17].xyz * (f54 * clamp(-f42, 0.0, 1.0)))) + min((f37.xyz * (f37.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f38.x)), vec3(CB0[21].w))) * f29.xyz) + (((f51 * (((f52 + (f52 * f52)) / max(((f53 * f53) * ((f47 * 3.0) + 0.5)) * ((f46 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f44)) * CB0[15].xyz) * 1.0);
    vec4 f56 = vec4(0.0);
    f56.x = f55.x;
    vec4 f57 = f56;
    f57.y = f55.y;
    ...
    vec4 f59 = f58;
    f59.w = VARYING2.w;
    vec3 f60 = mix(CB0[19].xyz, f59.xyz, vec3(clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f61 = f59;
    f61.x = f60.x;
    vec4 f62 = f61;
    f62.y = f60.y;

...
void main()
{
    vec3 f0 = (texture2D(DiffuseMapTexture, VARYING0.xy).xyz * VARYING2.xyz).xyz;
    ...
    vec3 f9 = textureCube(EnvironmentMapTexture, reflect(-VARYING4.xyz, VARYING5.xyz)).xyz;
    vec3 f10 = ((min(((f4.xyz * (f4.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f6), vec3(CB0[16].w)) + (VARYING6.xyz * f8)) * ((mix(vec3(1.0), (f9 * f9) * CB0[15].x, vec3(f6)) * VARYING7.w) + ((f0 * f0).xyz * (VARYING2.w - VARYING7.w))).xyz) + (CB0[10].xyz * (VARYING6.w * f8));
    vec4 f11 = vec4(f10.x, f10.y, f10.z, vec4(0.0).w);
    f11.w = VARYING2.w;
    vec3 f12 = sqrt(clamp(f11.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    gl_FragData[0] = mix(vec4(CB0[14].xyz, 1.0), vec4(f12.x, f12.y, f12.z, f11.w), vec4(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
}

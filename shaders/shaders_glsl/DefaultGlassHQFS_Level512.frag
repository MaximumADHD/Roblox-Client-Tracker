...
void main()
{
    ...
    vec3 f29 = -CB0[11].xyz;
    float f30 = dot(f14, f29) * ((1.0 - ((step(f23.x, f24) * clamp(CB0[24].z + (CB0[24].w * abs(f24 - 0.5)), 0.0, 1.0)) * f23.y)) * f20.y);
    vec3 f31 = normalize(f29 + f27);
    ...
    vec3 f42 = mix(f21, (f28 * f28) * CB0[15].x, vec3(f22)) * mix(vec3(1.0), f25, vec3(0.5));
    float f43 = 1.0 - max(9.9999997473787516355514526367188e-05, dot(f14, f27));
    vec4 f44 = mix(vec4(mix((((((vec3(1.0) - (f39 * (CB0[26].w * f1))) * CB0[10].xyz) * f32) + (CB0[12].xyz * clamp(-f30, 0.0, 1.0))) + min(f21 + (CB0[8].xyz + (CB0[9].xyz * f22)), vec3(CB0[16].w))) * (f25 * f13), f42, vec3(VARYING7.w)) * f13, f13), vec4(f42, 1.0), vec4(((f43 * f43) * 0.800000011920928955078125) * clamp(f12, 0.0, 1.0))) + vec4((f39 * (((f40 + (f40 * f40)) / (((f41 * f41) * ((f35 * 3.0) + 0.5)) * ((f34 * 0.75) + 0.25))) * f32)) * CB0[10].xyz, 0.0);

...
void main()
{
    ...
    vec4 f35 = mix(CB7[int(floor(texture2DLod(Texture0Texture, VARYING0, 0.0).x * 255.5)) * 1 + 0], vec4(CB7[(int(floor((f32 ? f14 : (f29 ? f12 : (f25 ? f10 : (f21 ? f8 : 0.0)))) * 255.5)) + 256) * 1 + 0].xyz, (texture2DLod(Texture1Texture, f34, 0.0).x < texture2DLod(Texture2Texture, f34, 0.0).x) ? CB1[1].y : 1.0), vec4(clamp(255.0 * max(abs(f10 - f8), abs(f14 - f12)), 0.0, 1.0)));

...
void main()
{
    ...
    vec3 f33 = -VARYING5;
    vec3 f34 = f33 * f33;
    bvec3 f35 = lessThan(f33, vec3(0.0));
    vec3 f36 = vec3(f35.x ? f34.x : vec3(0.0).x, f35.y ? f34.y : vec3(0.0).y, f35.z ? f34.z : vec3(0.0).z);
    vec3 f37 = f34 - f36;
    vec3 f38 = (((((CB0[34].xyz * f37.x) + (CB0[36].xyz * f37.y)) + (CB0[38].xyz * f37.z)) + (CB0[35].xyz * f36.x)) + (CB0[37].xyz * f36.y)) + (CB0[39].xyz * f36.z);
    bvec3 f39 = bvec3(!(CB0[18].w == 0.0));
    vec3 f40 = mix(vec3(f39.x ? CB0[19].xyz.x : f38.x, f39.y ? CB0[19].xyz.y : f38.y, f39.z ? CB0[19].xyz.z : f38.z), f32.xyz, vec3(clamp(exp2((CB0[18].z * length(VARYING5)) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f41 = f32;
    f41.x = f40.x;
    vec4 f42 = f41;
    f42.y = f40.y;

...
void main()
{
    ...
    vec3 f59 = mix(vec3(f58.x ? CB0[19].xyz.x : f57.x, f58.y ? CB0[19].xyz.y : f57.y, f58.z ? CB0[19].xyz.z : f57.z), f51.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f60 = f51;
    f60.x = f59.x;
    vec4 f61 = f60;
    f61.y = f59.y;
    vec4 f62 = f61;
    f62.z = f59.z;
    vec3 f63 = sqrt(clamp(f62.xyz * CB0[20].y, vec3(0.0), vec3(1.0)));
    vec4 f64 = f62;
    f64.x = f63.x;
    vec4 f65 = f64;
    f65.y = f63.y;

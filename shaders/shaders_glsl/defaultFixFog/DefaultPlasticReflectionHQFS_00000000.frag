...
void main()
{
    ...
    vec3 f22 = -f20;
    vec3 f23 = textureCube(EnvironmentMapTexture, reflect(f22, f13)).xyz;
    ...
    float f31 = f24.z;
    vec3 f32 = -CB0[16].xyz;
    float f33 = dot(f13, f32) * ((1.0 - ((step(f30.x, f31) * clamp(CB0[29].z + (CB0[29].w * abs(f31 - 0.5)), 0.0, 1.0)) * f30.y)) * f29.y);
    vec3 f34 = normalize(f20 + f32);
    float f35 = clamp(f33, 0.0, 1.0);
    float f36 = f21 * f21;
    float f37 = max(0.001000000047497451305389404296875, dot(f13, f34));
    ...
    vec3 f45 = ((((((vec3(1.0) - (f42 * (CB0[31].w * clamp(1.0 - (VARYING4.w * CB0[28].y), 0.0, 1.0)))) * CB0[15].xyz) * f35) + (CB0[17].xyz * clamp(-f33, 0.0, 1.0))) + (min((f28.xyz * (f28.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f29.x)), vec3(CB0[21].w)) * 1.0)) * mix(f18.xyz, (f23 * f23) * CB0[20].x, vec3(VARYING7.w))) + (((f42 * (((f43 + (f43 * f43)) / (((f44 * f44) * ((f38 * 3.0) + 0.5)) * ((f37 * 0.75) + 0.25))) * f35)) * CB0[15].xyz) * 1.0);
    vec4 f46 = vec4(0.0);
    f46.x = f45.x;
    vec4 f47 = f46;
    f47.y = f45.y;
    ...
    f49.w = VARYING2.w;
    vec3 f50 = f22 * f22;
    bvec3 f51 = lessThan(f22, vec3(0.0));
    ...
    vec3 f56 = mix(vec3(f55.x ? CB0[19].xyz.x : f54.x, f55.y ? CB0[19].xyz.y : f54.y, f55.z ? CB0[19].xyz.z : f54.z), f49.xyz, vec3(clamp(exp2((CB0[18].z * f19) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f57 = f49;
    f57.x = f56.x;
    vec4 f58 = f57;
    f58.y = f56.y;

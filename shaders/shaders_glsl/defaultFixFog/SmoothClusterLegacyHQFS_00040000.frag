...
void main()
{
    ...
    vec4 f44 = f43;
    f44.w = 1.0;
    vec3 f45 = normalize(-VARYING8);
    vec3 f46 = f45 * f45;
    bvec3 f47 = lessThan(f45, vec3(0.0));
    vec3 f48 = vec3(f47.x ? f46.x : vec3(0.0).x, f47.y ? f46.y : vec3(0.0).y, f47.z ? f46.z : vec3(0.0).z);
    vec3 f49 = f46 - f48;
    vec3 f50 = (((((CB0[34].xyz * f49.x) + (CB0[36].xyz * f49.y)) + (CB0[38].xyz * f49.z)) + (CB0[35].xyz * f48.x)) + (CB0[37].xyz * f48.y)) + (CB0[39].xyz * f48.z);
    bvec3 f51 = bvec3(!(CB0[18].w == 0.0));
    vec3 f52 = mix(vec3(f51.x ? CB0[19].xyz.x : f50.x, f51.y ? CB0[19].xyz.y : f50.y, f51.z ? CB0[19].xyz.z : f50.z), f44.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f53 = f44;
    f53.x = f52.x;
    vec4 f54 = f53;
    f54.y = f52.y;

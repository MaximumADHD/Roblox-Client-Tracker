...
void main()
{
    ...
    vec3 f66 = normalize(normalize(VARYING8) + f64);
    ...
    float f77 = 1.0 - f55;
    vec3 f78 = (((((((vec3(f77) - (f74 * ((CB0[31].w * f52) * f77))) * CB0[15].xyz) * f67) + (CB0[17].xyz * (f77 * clamp(-f65, 0.0, 1.0)))) + (min((f60.xyz * (f60.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f61.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f14.z * 2.0) * f52)) * f51) + ((((f74 * (((f75 + (f75 * f75)) / (((f76 * f76) * ((f70 * 3.0) + 0.5)) * ((f69 * 0.75) + 0.25))) * f67)) * CB0[15].xyz) * 1.0) * (f52 * VARYING0.w));
    vec4 f79 = vec4(0.0);
    f79.x = f78.x;
    vec4 f80 = f79;
    f80.y = f78.y;
    ...
    vec4 f82 = f81;
    f82.w = 1.0;
    vec3 f83 = normalize(-VARYING8);
    vec3 f84 = f83 * f83;
    bvec3 f85 = lessThan(f83, vec3(0.0));
    vec3 f86 = vec3(f85.x ? f84.x : vec3(0.0).x, f85.y ? f84.y : vec3(0.0).y, f85.z ? f84.z : vec3(0.0).z);
    vec3 f87 = f84 - f86;
    vec3 f88 = (((((CB0[34].xyz * f87.x) + (CB0[36].xyz * f87.y)) + (CB0[38].xyz * f87.z)) + (CB0[35].xyz * f86.x)) + (CB0[37].xyz * f86.y)) + (CB0[39].xyz * f86.z);
    bvec3 f89 = bvec3(!(CB0[18].w == 0.0));
    vec3 f90 = mix(vec3(f89.x ? CB0[19].xyz.x : f88.x, f89.y ? CB0[19].xyz.y : f88.y, f89.z ? CB0[19].xyz.z : f88.z), f82.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f91 = f82;
    f91.x = f90.x;
    vec4 f92 = f91;
    f92.y = f90.y;

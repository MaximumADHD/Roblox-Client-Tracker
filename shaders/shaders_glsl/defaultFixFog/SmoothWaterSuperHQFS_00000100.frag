...
void main()
{
    ...
    f94.w = 1.0;
    vec3 f95 = f17 * f17;
    bvec3 f96 = lessThan(f17, vec3(0.0));
    vec3 f97 = vec3(f96.x ? f95.x : vec3(0.0).x, f96.y ? f95.y : vec3(0.0).y, f96.z ? f95.z : vec3(0.0).z);
    vec3 f98 = f95 - f97;
    vec3 f99 = (((((CB0[34].xyz * f98.x) + (CB0[36].xyz * f98.y)) + (CB0[38].xyz * f98.z)) + (CB0[35].xyz * f97.x)) + (CB0[37].xyz * f97.y)) + (CB0[39].xyz * f97.z);
    bvec3 f100 = bvec3(!(CB0[18].w == 0.0));
    vec3 f101 = mix(vec3(f100.x ? CB0[19].xyz.x : f99.x, f100.y ? CB0[19].xyz.y : f99.y, f100.z ? CB0[19].xyz.z : f99.z), f94.xyz, vec3(VARYING6.w));
    vec4 f102 = f94;
    f102.x = f101.x;
    vec4 f103 = f102;
    f103.y = f101.y;

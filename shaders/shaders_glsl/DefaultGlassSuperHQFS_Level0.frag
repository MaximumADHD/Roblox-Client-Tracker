...
void main()
{
    ...
    vec3 f30 = -CB0[11].xyz;
    float f31 = dot(f15, f30) * ((1.0 - ((step(f24.x, f25) * clamp(CB0[24].z + (CB0[24].w * abs(f25 - 0.5)), 0.0, 1.0)) * f24.y)) * f21.y);
    vec3 f32 = normalize(f30 + f28);
    ...
    vec3 f43 = mix(f22, (f29 * f29) * CB0[15].x, vec3(f23)) * mix(vec3(1.0), f26, vec3(0.5));

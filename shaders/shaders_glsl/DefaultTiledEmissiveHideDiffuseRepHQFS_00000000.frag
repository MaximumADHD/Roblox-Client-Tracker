...
void main()
{
    ...
    float f49 = 1.0 - f27;
    vec3 f50 = (((((((vec3(f49) - (f46 * (f25 * f49))) * CB0[15].xyz) * f39) + (CB0[17].xyz * (f49 * clamp(-f37, 0.0, 1.0)))) + (min((f32.xyz * (f32.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f33.x)), vec3(CB0[21].w)) * 1.0)) + vec3(((1.0 - f13.w) * 2.0) * f2)) * f23.xyz) + (((f46 * (((f47 + (f47 * f47)) / max(((f48 * f48) * ((f42 * 3.0) + 0.5)) * ((f41 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f39)) * CB0[15].xyz) * 1.0);
    vec4 f51 = vec4(0.0);
    f51.x = f50.x;
    vec4 f52 = f51;
    f52.y = f50.y;

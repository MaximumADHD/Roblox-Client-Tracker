...
void main()
{
    ...
    float f17 = clamp(1.0 - (VARYING5.w * CB0[23].y), 0.0, 1.0);
    ...
    float f28 = f19.y;
    vec3 f29 = -CB0[11].xyz;
    float f30 = dot(f18, f29) * ((1.0 - ((step(f26.x, f27) * clamp(CB0[24].z + (CB0[24].w * abs(f27 - 0.5)), 0.0, 1.0)) * f26.y)) * f25.y);
    ...
    vec3 f42 = (((((((vec3(1.0) - (f39 * (CB0[26].w * f17))) * CB0[10].xyz) * f32) + (CB0[12].xyz * clamp(-f30, 0.0, 1.0))) + min((f24.xyz * (f24.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f25.x)), vec3(CB0[16].w))) + vec3((f19.z * 2.0) * f17)) * (f16 * f16).xyz) + ((((f39 * (((f40 + (f40 * f40)) / (((f41 * f41) * ((f35 * 3.0) + 0.5)) * ((f34 * 0.75) + 0.25))) * f32)) * CB0[10].xyz) * f17) * VARYING0.w);

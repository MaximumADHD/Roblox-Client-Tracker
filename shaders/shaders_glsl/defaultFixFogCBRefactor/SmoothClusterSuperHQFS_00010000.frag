...
void main()
{
    ...
    f31.z = f30;
    vec4 f32 = f21;
    f32.x = CB5[f14 * 3 + 2].x * f11.y;
    vec4 f33 = f32;
    f33.y = (CB5[f14 * 3 + 2].y * f18) - (CB5[f14 * 3 + 2].z * f20);
    vec4 f34 = f33;
    f34.z = (CB5[f14 * 3 + 2].z * f18) + (CB5[f14 * 3 + 2].y * f20);
    vec4 f35 = f26;
    f35.x = CB5[f15 * 3 + 2].x * f12.y;
    vec4 f36 = f35;
    f36.y = (CB5[f15 * 3 + 2].y * f23) - (CB5[f15 * 3 + 2].z * f25);
    vec4 f37 = f36;
    f37.z = (CB5[f15 * 3 + 2].z * f23) + (CB5[f15 * 3 + 2].y * f25);
    vec4 f38 = f31;
    f38.x = CB5[f16 * 3 + 2].x * f13.y;
    vec4 f39 = f38;
    f39.y = (CB5[f16 * 3 + 2].y * f28) - (CB5[f16 * 3 + 2].z * f30);
    vec4 f40 = f39;
    f40.z = (CB5[f16 * 3 + 2].z * f28) + (CB5[f16 * 3 + 2].y * f30);
    ...
    vec3 f68 = (((((((vec3(1.0) - (f65 * (CB0[31].w * f45))) * CB0[15].xyz) * f58) + (CB0[17].xyz * clamp(-f56, 0.0, 1.0))) + (min((f51.xyz * (f51.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f52.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f45)) * (f44 * f44)) + ((((f65 * (((f66 + (f66 * f66)) / (((f67 * f67) * ((f61 * 3.0) + 0.5)) * ((f60 * 0.75) + 0.25))) * f58)) * CB0[15].xyz) * 1.0) * (f45 * VARYING0.w));
    vec4 f69 = vec4(0.0);
    f69.x = f68.x;
    vec4 f70 = f69;
    f70.y = f68.y;
    ...
    vec4 f72 = f71;
    f72.w = 1.0;
    vec3 f73 = normalize(-VARYING8);
    vec3 f74 = f73 * f73;
    bvec3 f75 = lessThan(f73, vec3(0.0));
    vec3 f76 = vec3(f75.x ? f74.x : vec3(0.0).x, f75.y ? f74.y : vec3(0.0).y, f75.z ? f74.z : vec3(0.0).z);
    vec3 f77 = f74 - f76;
    vec3 f78 = (((((CB0[34].xyz * f77.x) + (CB0[36].xyz * f77.y)) + (CB0[38].xyz * f77.z)) + (CB0[35].xyz * f76.x)) + (CB0[37].xyz * f76.y)) + (CB0[39].xyz * f76.z);
    bvec3 f79 = bvec3(!(CB0[18].w == 0.0));
    vec3 f80 = mix(vec3(f79.x ? CB0[19].xyz.x : f78.x, f79.y ? CB0[19].xyz.y : f78.y, f79.z ? CB0[19].xyz.z : f78.z), f72.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f81 = f72;
    f81.x = f80.x;
    vec4 f82 = f81;
    f82.y = f80.y;

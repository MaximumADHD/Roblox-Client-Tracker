...
void main()
{
    ...
    vec3 f39 = (((((((vec3(1.0) - (f36 * (CB0[31].w * f16))) * CB0[15].xyz) * f29) + (CB0[17].xyz * clamp(-f27, 0.0, 1.0))) + (min((f22.xyz * (f22.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f23.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f16)) * (f15 * f15)) + ((((f36 * (((f37 + (f37 * f37)) / (((f38 * f38) * ((f32 * 3.0) + 0.5)) * ((f31 * 0.75) + 0.25))) * f29)) * CB0[15].xyz) * 1.0) * (f16 * VARYING0.w));
    vec4 f40 = vec4(0.0);
    f40.x = f39.x;
    vec4 f41 = f40;
    f41.y = f39.y;
    ...
    vec3 f45 = f44 * f44;
    bvec3 f46 = lessThan(f44, vec3(0.0));
    ...
    vec3 f51 = mix(vec3(f50.x ? CB0[19].xyz.x : f49.x, f50.y ? CB0[19].xyz.y : f49.y, f50.z ? CB0[19].xyz.z : f49.z), f43.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f52 = f43;
    f52.x = f51.x;
    vec4 f53 = f52;
    f53.y = f51.y;

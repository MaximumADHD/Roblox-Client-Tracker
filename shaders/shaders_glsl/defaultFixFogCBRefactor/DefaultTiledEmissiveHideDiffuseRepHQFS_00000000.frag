...
void main()
{
    ...
    float f50 = 1.0 - f28;
    vec3 f51 = (((((((vec3(f50) - (f47 * (f26 * f50))) * CB0[15].xyz) * f40) + (CB0[17].xyz * (f50 * clamp(-f38, 0.0, 1.0)))) + (min((f33.xyz * (f33.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f34.x)), vec3(CB0[21].w)) * 1.0)) + vec3(((1.0 - f13.w) * 2.0) * f2)) * f23.xyz) + (((f47 * (((f48 + (f48 * f48)) / (((f49 * f49) * ((f43 * 3.0) + 0.5)) * ((f42 * 0.75) + 0.25))) * f40)) * CB0[15].xyz) * 1.0);
    vec4 f52 = vec4(0.0);
    f52.x = f51.x;
    vec4 f53 = f52;
    f53.y = f51.y;

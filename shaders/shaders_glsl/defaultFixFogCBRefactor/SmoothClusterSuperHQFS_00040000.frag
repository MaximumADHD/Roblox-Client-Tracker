...
void main()
{
    ...
    vec3 f14 = (((mix(vec3(1.0), CB5[int(VARYING9.x + 0.5) * 3 + 2].xyz, vec3(f11.w)) * f11.xyz) * VARYING0.x) + ((mix(vec3(1.0), CB5[int(VARYING9.y + 0.5) * 3 + 2].xyz, vec3(f12.w)) * f12.xyz) * VARYING0.y)) + ((mix(vec3(1.0), CB5[int(VARYING9.z + 0.5) * 3 + 2].xyz, vec3(f13.w)) * f13.xyz) * VARYING0.z);
    vec3 f15 = f14 * f14;
    ...
    float f40 = 1.0 - f18;
    vec3 f41 = (((((((vec3(f40) - (f37 * ((CB0[31].w * f16) * f40))) * CB0[15].xyz) * f30) + (CB0[17].xyz * (f40 * clamp(-f28, 0.0, 1.0)))) + (min((f23.xyz * (f23.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f24.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f16)) * f15) + ((((f37 * (((f38 + (f38 * f38)) / (((f39 * f39) * ((f33 * 3.0) + 0.5)) * ((f32 * 0.75) + 0.25))) * f30)) * CB0[15].xyz) * 1.0) * (f16 * VARYING0.w));
    vec4 f42 = vec4(0.0);
    f42.x = f41.x;
    vec4 f43 = f42;
    f43.y = f41.y;
    ...
    vec3 f47 = f46 * f46;
    bvec3 f48 = lessThan(f46, vec3(0.0));
    vec3 f49 = vec3(f48.x ? f47.x : vec3(0.0).x, f48.y ? f47.y : vec3(0.0).y, f48.z ? f47.z : vec3(0.0).z);
    vec3 f50 = f47 - f49;
    vec3 f51 = (((((CB0[34].xyz * f50.x) + (CB0[36].xyz * f50.y)) + (CB0[38].xyz * f50.z)) + (CB0[35].xyz * f49.x)) + (CB0[37].xyz * f49.y)) + (CB0[39].xyz * f49.z);
    bvec3 f52 = bvec3(!(CB0[18].w == 0.0));
    vec3 f53 = mix(vec3(f52.x ? CB0[19].xyz.x : f51.x, f52.y ? CB0[19].xyz.y : f51.y, f52.z ? CB0[19].xyz.z : f51.z), f45.xyz, vec3(clamp(exp2((CB0[18].z * VARYING5.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f54 = f45;
    f54.x = f53.x;
    vec4 f55 = f54;
    f55.y = f53.y;

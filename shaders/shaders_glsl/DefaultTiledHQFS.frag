...
void main()
{
    ...
    vec3 f12 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f4.w + CB2[3].w, 0.0, 1.0))) * f4.xyz) * (1.0 + (f9 * CB2[1].x))) * (texture2D(StudsMapTexture, f0).x * 2.0), VARYING2.w).xyz;
    ...
    vec4 f29 = texture2D(PrecomputedBRDFTexture, vec2(f25, max(9.9999997473787516355514526367188e-05, dot(f11, f22))));
    float f30 = f10.x * f24;
    ...
    float f58 = f52.y;
    float f59 = f52.z;
    vec3 f60 = (mix(textureCubeLod(PrefilteredEnvIndoorTexture, f28, f27).xyz * f18, textureCubeLod(PrefilteredEnvTexture, f28, f27).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f26.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f19)) * f48) * f24;
    ...
    gl_FragData[0] = vec4(f65.x, f65.y, f65.z, f62.w);
}

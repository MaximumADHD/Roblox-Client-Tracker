...
void main()
{
    ...
    float f46 = 1.0 - f24;
    vec3 f47 = ((((((vec3(f46) - (f43 * (f22 * f46))) * CB0[15].xyz) * f36) + (CB0[17].xyz * (f46 * clamp(-f34, 0.0, 1.0)))) + min((f29.xyz * (f29.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f30.x)), vec3(CB0[21].w))) * f21.xyz) + (((f43 * (((f44 + (f44 * f44)) / max(((f45 * f45) * ((f39 * 3.0) + 0.5)) * ((f38 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f36)) * CB0[15].xyz) * 1.0);
    vec4 f48 = vec4(0.0);
    f48.x = f47.x;
    vec4 f49 = f48;
    f49.y = f47.y;

...
void main()
{
    ...
    float f34 = 0.08900000154972076416015625 + (mix(texture2D(SpecularMapTexture, f1 * CB2[1].w), texture2D(SpecularMapTexture, f1), vec4(clamp((f0 * CB2[3].z) - (CB2[2].z * CB2[3].z), 0.0, 1.0))).y * 0.9110000133514404296875);
    vec3 f35 = textureCube(EnvironmentMapTexture, reflect(-VARYING4.xyz, f20)).xyz;
    vec3 f36 = -CB0[11].xyz;
    float f37 = (dot(f20, f36) * CB0[9].w) * ((1.0 - ((step(f29.x, f30) * clamp(CB0[24].z + (CB0[24].w * abs(f30 - 0.5)), 0.0, 1.0)) * f29.y)) * f26.y);
    vec3 f38 = normalize(f36 + f33);
    ...
    vec3 f49 = mix(f27, (f35 * f35) * CB0[15].x, vec3(f28)) * mix(vec3(1.0), f31, vec3(0.5));
    float f50 = 1.0 - max(9.9999997473787516355514526367188e-05, dot(f20, f33));
    ...
    vec4 f55 = vec4(CB0[7].xyz - ((VARYING4.xyz * (1.0 + ((3.0 * f52) / max(dot(VARYING4.xyz, f20), 0.00999999977648258209228515625)))) + (f20 * (3.0 * (1.0 - f52)))), 1.0) * f53;
    float f56 = f54.w;
    ...
    vec4 f80 = mix(mix(f79, vec4(mix((((((vec3(1.0) - (f46 * (CB0[26].w * f0))) * CB0[10].xyz) * f39) + (CB0[12].xyz * clamp(-f37, 0.0, 1.0))) + min(f27 + (CB0[8].xyz + (CB0[9].xyz * f28)), vec3(CB0[16].w))) * (f31 * f15), f49, vec3(VARYING7.w)), 1.0), vec4(f15)), vec4(f49, 1.0), vec4(((f50 * f50) * 0.800000011920928955078125) * f16)) + vec4(((f46 * (((f47 + (f47 * f47)) / (((f48 * f48) * ((f42 * 3.0) + 0.5)) * ((f41 * 0.75) + 0.25))) * f39)) * CB0[10].xyz) * (CB0[9].w * CB0[9].w), 0.0);
    float f81 = clamp(exp2((CB0[13].z * f32) + CB0[13].x) - CB0[13].w, 0.0, 1.0);

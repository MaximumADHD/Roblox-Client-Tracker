...
void main()
{
    ...
    vec2 f8 = f6.wy;
    vec2 f9 = f6.zy;
    vec4 f10 = (((((((((texture2DLod(cloudColor, VARYING0, 0.0) * 4.0) + texture2DLod(cloudColor, VARYING0 - f7, 0.0)) + (texture2DLod(cloudColor, VARYING0 - f8, 0.0) * 2.0)) + texture2DLod(cloudColor, VARYING0 - f9, 0.0)) + (texture2DLod(cloudColor, VARYING0 + f6.zw, 0.0) * 2.0)) + (texture2DLod(cloudColor, VARYING0 + f6.xw, 0.0) * 2.0)) + texture2DLod(cloudColor, VARYING0 + f9, 0.0)) + (texture2DLod(cloudColor, VARYING0 + f8, 0.0) * 2.0)) + texture2DLod(cloudColor, VARYING0 + f7, 0.0)) * vec4(0.0625);
    ...
    vec4 f18 = texture2DLod(distLastFrame, f17, 0.0);
    float f19 = f18.y * 7.0;
    ...
    vec4 f23 = texture2DLod(oldCloudColor, f17, 0.0);
    vec2 f24;
    vec4 f25;

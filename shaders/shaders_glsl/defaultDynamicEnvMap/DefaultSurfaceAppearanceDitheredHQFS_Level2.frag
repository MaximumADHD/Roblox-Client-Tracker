...
void main()
{
    ...
    vec4 f6 = vec4(mix(vec4(f2.xyz * VARYING2.xyz, f4).xyz, f3.xyz, vec3(f4)), 0.0);
    vec4 f7 = vec4(f3.xyz, 0.0);
    ...
    vec3 f18 = vec4(vec4(f8.x ? f6.x : f7.x, f8.y ? f6.y : f7.y, f8.z ? f6.z : f7.z, f8.w ? f6.w : f7.w).xyz, VARYING2.w).xyz;
    ...
    f46.w = VARYING2.w;
    vec3 f47 = sqrt(clamp(mix(CB0[14].xyz, f46.xyz, vec3(clamp(exp2((CB0[13].z * f0) + CB0[13].x) - CB0[13].w, 0.0, 1.0))).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * f5));

...
void main()
{
    ...
    for (;;)
    {
        if (f34 < f17)
        {
            ...
            vec2 f44 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(texture3DLod(DetailTex3DTexture, vec4((f41 * (0.5 * CB5[4].y)) + (f40 * f43.x), f38).xyz, f38).x, texture3DLod(DetailTex3DTexture, vec4((f41 * (2.0 * CB5[4].y)) + (f40 * f43.y), f38).xyz, f38).x) * 2.0) - vec2(1.0));
            float f45 = texture3DLod(CloudsDistanceFieldTexture, vec4(((f23 + (f40 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f38).xyz, f38).x + (f44.x + f44.y);
            float f46 = (f42 * 50.0) * (CB5[2].x - f45);
            bool f47 = f45 < CB5[2].x;
            if (f47)
            {
                vec4 f48 = texture2DLod(BeerShadowMapTexture, vec4(f23.xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f49 = min(f48.z * 0.5, (f48.y + (0.3499999940395355224609375 * texture2DLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)) * max(0.0, (1.0 - ((float(f34) / f18) + (f45 / f18))) - f48.x));
                float f50 = f36 + f46;
                vec3 f51 = f2 * f2;
    ...
    vec3 f69 = mix(CB0[14].xyz, (f32.xyz + (((((((CB0[10].xyz * clamp(1.0 - ((0.07999999821186065673828125 * f28) / f18), 0.0, 1.0)) * 2.0) * clamp(-f11, 0.0, 1.0)) * f15) * f67) * exp2(0.10999999940395355224609375 - (0.0350000001490116119384765625 * f28))) * CB5[3].xyz)).xyz, vec3(f68));
    vec4 f70 = vec4(f69.x, f69.y, f69.z, f32.w);
    f70.w = f67;
    vec4 f71 = f70;
    f71.w = f67 * max(f68, CB0[13].y);
    gl_FragData[0] = f71;
}

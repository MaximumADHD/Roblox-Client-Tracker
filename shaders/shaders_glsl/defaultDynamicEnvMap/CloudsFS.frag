...
void main()
{
    ...
    float f16 = float(f15);
    vec3 f17 = f14 * 2.5;
    vec3 f18 = f14 + ((f4 * f11) * (0.1500000059604644775390625 + (0.009499999694526195526123046875 * (texture3DLod(NoiseTexTexture, vec4(fract(f17), 0.0).xyz, 0.0).x + (0.25 * texture3DLod(NoiseTexTexture, vec4(fract((f17 * mat3(vec3(0.0, 0.800000011920928955078125, 0.60000002384185791015625), vec3(-0.800000011920928955078125, 0.36000001430511474609375, -0.4799999892711639404296875), vec3(-0.60000002384185791015625, -0.4799999892711639404296875, 0.63999998569488525390625))) * 2.019999980926513671875), 0.0).xyz, 0.0).x)))));
    f18.y = 0.0;
    vec3 f19;
    vec4 f20;
    f20 = vec4(0.0);
    ...
    for (;;)
    {
        if (f29 < f15)
        {
            ...
            vec2 f34 = (texture2DLod(AdvectionTexTexture, vec4((f19.xz * 0.125) + vec2(0.0, CB5[0].w * (-17.5)), 0.0, f33).xy, f33).xy * 2.0) - vec2(1.0);
            ...
            vec2 f40 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(texture3DLod(DetailTex3DTexture, vec4((f36 * (0.5 * CB5[4].y)) + (f35 * f39.x), f33).xyz, f33).x, texture3DLod(DetailTex3DTexture, vec4((f36 * (2.0 * CB5[4].y)) + (f35 * f39.y), f33).xyz, f33).x) * 2.0) - vec2(1.0));
            float f41 = texture3DLod(CloudsDistanceFieldTexture, vec4(((f19 + (f35 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f33).xyz, f33).x + (f40.x + f40.y);
            bool f42 = f41 < CB5[2].x;
            if (f42)
            {
                float f43 = (f38 * 50.0) * (CB5[2].x - f41);
                vec3 f44 = -CB0[11].xyz;
                vec4 f45 = texture2DLod(BeerShadowMapTexture, vec4((f19 - (f44 * (f19.y * (1.0 / max(f44.y, 9.9999997473787516355514526367188e-05))))).xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f46 = f45.x;
    ...
    vec3 f64 = mix(CB0[14].xyz, (f27.xyz * (1.6480000019073486328125 - ((0.00200000009499490261077880859375 * f16) * f16))).xyz, vec3(f63));
    float f65 = 1.0 - f25;
    vec4 f66 = vec4(f64.x, f64.y, f64.z, f27.w);
    f66.w = f65;
    vec4 f67 = f66;
    f67.w = f65 * max(f63, CB0[13].y);
    gl_FragData[0] = f67;
}

...
void main()
{
    ...
    vec3 f37 = ((((((vec3(1.0) - (f34 * (CB0[31].w * clamp(1.0 - (VARYING3.w * CB0[28].y), 0.0, 1.0)))) * CB0[15].xyz) * f27) + (CB0[17].xyz * clamp(-f25, 0.0, 1.0))) + (min((f20.xyz * (f20.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f21.x)), vec3(CB0[21].w)) * 1.0)) * f13.xyz) + (((f34 * (((f35 + (f35 * f35)) / (((f36 * f36) * ((f30 * 3.0) + 0.5)) * ((f29 * 0.75) + 0.25))) * f27)) * CB0[15].xyz) * 1.0);
    vec4 f38 = vec4(0.0);
    f38.x = f37.x;
    vec4 f39 = f38;
    f39.y = f37.y;

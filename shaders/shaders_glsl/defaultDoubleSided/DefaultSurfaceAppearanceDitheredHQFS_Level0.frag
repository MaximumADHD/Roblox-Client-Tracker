...
void main()
{
    ...
    if (smoothstep(0.0, 1.0, f3) < (f4 + ((fract(CB0[47].z) - f4) * clamp((0.001000000047497451305389404296875 * floor(CB0[47].z)) * VARYING4.w, 0.0, 1.0))))
    {
        discard;
    }
    ...
    f47.w = f46;
    vec3 f48 = sqrt(clamp(mix(CB0[14].xyz, f47.xyz, vec3(clamp(exp2((CB0[13].z * f0) + CB0[13].x) - CB0[13].w, 0.0, 1.0))).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * f4));

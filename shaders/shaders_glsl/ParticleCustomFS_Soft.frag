...
void main()
{
    vec4 f0 = mix(texture2D(texTexture, VARYING0.xy), texture2D(texTexture, VARYING4), vec4(VARYING5));
    ...
    vec4 f17 = f16;
    f17.z = f14.z;
    float f18 = (VARYING1.w * f0.w) * (clamp(VARYING3.w * abs((texture2D(depthTexTexture, gl_FragCoord.xy * CB0[45].xy).x * 500.0) - gl_FragCoord.w), 0.0, 1.0) * clamp(exp2((CB0[13].z * VARYING0.z) + CB0[13].x) - CB0[13].w, 0.0, 1.0));
    vec4 f19 = f17;
    f19.w = f18;

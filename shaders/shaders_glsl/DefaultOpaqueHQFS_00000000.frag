...
void main()
{
    ...
    float f31 = 1.0 - f9;
    vec3 f32 = ((((((vec3(f31) - (f28 * (f7 * f31))) * CB0[15].xyz) * f21) + (CB0[17].xyz * (f31 * clamp(-f19, 0.0, 1.0)))) + (min((f14.xyz * (f14.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f15.x)), vec3(CB0[21].w)) * 1.0)) * f5.xyz) + (((f28 * (((f29 + (f29 * f29)) / (((f30 * f30) * ((f24 * 3.0) + 0.5)) * ((f23 * 0.75) + 0.25))) * f21)) * CB0[15].xyz) * 1.0);
    vec4 f33 = vec4(0.0);
    f33.x = f32.x;
    vec4 f34 = f33;
    f34.y = f32.y;

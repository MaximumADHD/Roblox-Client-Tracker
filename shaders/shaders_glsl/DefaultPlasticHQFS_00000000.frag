...
void main()
{
    ...
    vec2 f2 = texture2D(NormalMapTexture, f1).wy * 2.0;
    vec2 f3 = f2 - vec2(1.0);
    float f4 = sqrt(clamp(1.0 + dot(vec2(1.0) - f2, f3), 0.0, 1.0));
    vec3 f5 = vec3(f3, f4);
    vec2 f6 = f5.xy + (vec3((texture2D(NormalDetailMapTexture, VARYING0 * CB2[0].x).wy * 2.0) - vec2(1.0), 0.0).xy * (clamp(VARYING2.w - 0.5, 0.0, 1.0) * clamp(1.0 - (2.0 * VARYING7.w), 0.0, 1.0)));
    vec3 f7 = f5;
    f7.x = f6.x;
    vec3 f8 = f7;
    f8.y = f6.y;
    vec2 f9 = f8.xy * (clamp((vec2(0.0033333334140479564666748046875, CB0[28].y) * (-VARYING4.w)) + vec2(1.0), vec2(0.0), vec2(1.0)).y * CB2[3].w);
    vec4 f10 = vec4(VARYING2.xyz * (texture2D(DiffuseMapTexture, f1).x * 2.0), VARYING2.w);
    ...
    float f28 = f21.z;
    vec3 f29 = -CB0[16].xyz;
    float f30 = dot(f14, f29) * ((1.0 - ((step(f27.x, f28) * clamp(CB0[29].z + (CB0[29].w * abs(f28 - 0.5)), 0.0, 1.0)) * f27.y)) * f26.y);
    vec3 f31 = normalize((VARYING4.xyz / vec3(f0)) - CB0[16].xyz);
    float f32 = clamp(f30, 0.0, 1.0);
    float f33 = f20 * f20;
    float f34 = max(0.001000000047497451305389404296875, dot(f14, f31));
    ...
    vec3 f42 = ((((((vec3(1.0) - (f39 * (CB0[31].w * clamp(1.0 - (VARYING4.w * CB0[28].y), 0.0, 1.0)))) * CB0[15].xyz) * f32) + (CB0[17].xyz * clamp(-f30, 0.0, 1.0))) + (min((f25.xyz * (f25.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f26.x)), vec3(CB0[21].w)) * 1.0)) * f19.xyz) + (((f39 * (((f40 + (f40 * f40)) / max(((f41 * f41) * ((f35 * 3.0) + 0.5)) * ((f34 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f32)) * CB0[15].xyz) * 1.0);
    vec4 f43 = vec4(0.0);
    f43.x = f42.x;
    vec4 f44 = f43;
    f44.y = f42.y;
    ...
    vec4 f46 = f45;
    f46.w = VARYING2.w;
    vec3 f47 = mix(CB0[19].xyz, f46.xyz, vec3(clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0)));
    vec4 f48 = f46;
    f48.x = f47.x;
    vec4 f49 = f48;
    f49.y = f47.y;

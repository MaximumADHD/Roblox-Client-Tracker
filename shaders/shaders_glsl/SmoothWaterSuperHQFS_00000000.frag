...
void main()
{
    ...
    vec3 f33 = reflect(-f17, f16);
    float f34 = VARYING8.w * 0.20000000298023223876953125;
    vec4 f35 = vec4(f33, 0.0) * mat4(CB0[0], CB0[1], CB0[2], CB0[3]);
    ...
    vec3 f77 = mix(mix((f32 * f32) * CB0[20].x, ((min(f5 + (CB0[32].xyz + (CB0[33].xyz * f6)), vec3(CB0[21].w)) + (((((((CB0[40].xyz * f22) + (CB0[42].xyz * f23)) + (CB0[44].xyz * f24)) + (CB0[41].xyz * f25)) + (CB0[43].xyz * f26)) + (CB0[45].xyz * f27)) + (((((((CB0[34].xyz * f22) + (CB0[36].xyz * f23)) + (CB0[38].xyz * f24)) + (CB0[35].xyz * f25)) + (CB0[37].xyz * f26)) + (CB0[39].xyz * f27)) * f6))) + (CB0[15].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((mix(texture2D(GBufferDepthTexture, f28).x * 500.0, f30, f31) - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(((f64 * f64) * CB0[20].x) * f6, (f65 * f65) * CB0[20].x, vec3((((float(abs(f63.x - 0.5) < 0.550000011920928955078125) * float(abs(f63.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f62) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture2D(GBufferDepthTexture, f63).x * 500.0) - f62) < 10.0)) * float(f35.w > 0.0))) + (f5 * 0.100000001490116119384765625), vec3((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f16, f17))), 0.0, 1.0) + 0.300000011920928955078125) * (VARYING0.w * CB3[2].z))) + (((((vec3(f74) + (vec3(0.0199999995529651641845703125) * (1.0 - f74))) * (((f75 + (f75 * f75)) / max(((f76 * f76) * ((f71 * 3.0) + 0.5)) * ((f70 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * clamp(dot(f15, f67) * f7, 0.0, 1.0))) * CB0[15].xyz) * 1.0) * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f78 = vec4(0.0);
    f78.x = f77.x;
    vec4 f79 = f78;
    f79.y = f77.y;

...
void main()
{
    ...
    vec3 f73 = -CB0[16].xyz;
    vec3 f74 = normalize(f17 - CB0[16].xyz);
    float f75 = f72 * f72;
    ...
    vec3 f83 = mix(mix(f38.xyz, ((min(f5 + (CB0[32].xyz + ((CB0[33].xyz * 1.0) * f6)), vec3(CB0[21].w)) + (((((((CB0[40].xyz * f22) + (CB0[42].xyz * f23)) + (CB0[44].xyz * f24)) + (CB0[41].xyz * f25)) + (CB0[43].xyz * f26)) + (CB0[45].xyz * f27)) + (((((((CB0[34].xyz * f22) + (CB0[36].xyz * f23)) + (CB0[38].xyz * f24)) + (CB0[35].xyz * f25)) + (CB0[37].xyz * f26)) + (CB0[39].xyz * f27)) * f6))) + (CB0[15].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((f33.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(((f70 * f70) * CB0[20].x) * f6, (f71 * f71) * CB0[20].x, vec3((((float(abs(f69.x - 0.5) < 0.550000011920928955078125) * float(abs(f69.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f68) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture2D(GBufferDepthTexture, f69).x * 500.0) - f68) < 10.0)) * float(f41.w > 0.0))) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f16, f17))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (((((vec3(f80) + (vec3(0.0199999995529651641845703125) * (1.0 - f80))) * (min((f81 + (f81 * f81)) / (((f82 * f82) * ((f77 * 3.0) + 0.5)) * ((f76 * 0.75) + 0.25)), 65504.0) * clamp(dot(f15, f73) * f7, 0.0, 1.0))) * CB0[15].xyz) * 1.0) * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f84 = vec4(0.0);
    f84.x = f83.x;
    vec4 f85 = f84;
    f85.y = f83.y;
    ...
    vec4 f87 = f86;
    f87.w = 1.0;
    vec3 f88 = mix(CB0[19].xyz, f87.xyz, vec3(VARYING6.w));
    vec4 f89 = f87;
    f89.x = f88.x;
    vec4 f90 = f89;
    f90.y = f88.y;

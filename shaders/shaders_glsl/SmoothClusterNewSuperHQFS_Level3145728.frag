...
void main()
{
    ...
    vec3 f28 = -CB0[11].xyz;
    float f29 = (dot(f18, f28) * CB0[9].w) * ((1.0 - ((step(f25.x, f26) * clamp(CB0[24].z + (CB0[24].w * abs(f26 - 0.5)), 0.0, 1.0)) * f25.y)) * f24.y);
    vec3 f30 = normalize(normalize(VARYING8) - CB0[11].xyz);
    ...
    vec3 f41 = (((((((vec3(1.0) - (f38 * (CB0[26].w * f14))) * CB0[10].xyz) * f31) + (CB0[12].xyz * clamp(-f29, 0.0, 1.0))) + (min((f23.xyz * (f23.w * 120.0)) + (CB0[8].xyz + (CB0[9].xyz * f24.x)), vec3(CB0[16].w)) * 1.0)) + vec3((f8.z * 2.0) * f14)) * (f13 * f13)) + (((((f38 * (((f39 + (f39 * f39)) / (((f40 * f40) * ((f34 * 3.0) + 0.5)) * ((f33 * 0.75) + 0.25))) * f31)) * CB0[10].xyz) * (CB0[9].w * CB0[9].w)) * f14) * VARYING0.w);
    vec4 f42 = vec4(0.0);
    f42.x = f41.x;
    vec4 f43 = f42;
    f43.y = f41.y;

...
void main()
{
    ...
    float f22 = 0.08900000154972076416015625 + (mix(texture2D(SpecularMapTexture, f1 * CB2[1].w), texture2D(SpecularMapTexture, f1), vec4(clamp((f0 * CB2[3].z) - (CB2[2].z * CB2[3].z), 0.0, 1.0))).y * 0.9110000133514404296875);
    vec3 f23 = textureCube(EnvironmentMapTexture, reflect(-VARYING4.xyz, f18)).xyz;
    ...
    float f33 = f24.z;
    vec3 f34 = mix(f30, (f23 * f23) * CB0[20].x, vec3(f31)) * mix(vec3(1.0), f19, vec3(0.5));
    vec3 f35 = -CB0[16].xyz;
    float f36 = dot(f18, f35) * ((1.0 - ((step(f32.x, f33) * clamp(CB0[29].z + (CB0[29].w * abs(f33 - 0.5)), 0.0, 1.0)) * f32.y)) * f29.y);
    vec3 f37 = normalize(f21 - CB0[16].xyz);
    ...
    float f48 = 1.0 - max(9.9999997473787516355514526367188e-05, dot(f18, f21));
    vec4 f49 = mix(vec4(mix((((((vec3(1.0) - (f45 * (CB0[31].w * f0))) * CB0[15].xyz) * f38) + (CB0[17].xyz * clamp(-f36, 0.0, 1.0))) + min(f30 + (CB0[13].xyz + (CB0[14].xyz * f31)), vec3(CB0[21].w))) * (f19 * f14), f34, vec3(VARYING7.w)) * f14, f14), vec4(f34, 1.0), vec4(((f48 * f48) * 0.800000011920928955078125) * clamp(f13, 0.0, 1.0))) + vec4(((f45 * (min((f46 + (f46 * f46)) / (((f47 * f47) * ((f41 * 3.0) + 0.5)) * ((f40 * 0.75) + 0.25)), 65504.0) * f38)) * CB0[15].xyz) * 1.0, 0.0);
    float f50 = clamp(exp2((CB0[18].z * f20) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f51 = mix(CB0[19].xyz, f49.xyz, vec3(f50));
    vec4 f52 = f49;
    f52.x = f51.x;
    vec4 f53 = f52;
    f53.y = f51.y;

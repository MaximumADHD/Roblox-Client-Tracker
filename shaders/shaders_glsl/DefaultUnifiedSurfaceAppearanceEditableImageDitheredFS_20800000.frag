...
void main()
{
    ...
    vec3 f6 = mix(vec3(0.0, 0.0, 1.0), (texture2D(NormalMapTexture, VARYING0).xyz * 2.0) - vec3(1.0), vec3(clamp((vec2(0.0033333334140479564666748046875, CB0[28].y) * (-VARYING4.w)) + vec2(1.0), vec2(0.0), vec2(1.0)).y));
    float f7 = gl_FrontFacing ? 1.0 : (-1.0);
    vec3 f8 = VARYING6.xyz * f7;
    vec3 f9 = VARYING5.xyz * f7;
    vec3 f10 = normalize(((f8 * f6.x) + ((cross(f9, f8) * VARYING6.w) * f6.y)) + (f9 * f6.z));
    vec3 f11 = f5.xyz;
    vec3 f12 = f11 * f11;
    vec4 f13 = f5;
    f13.x = f12.x;
    ...
    float f18 = 0.08900000154972076416015625 + (texture2D(EditableImageRoughnessMapTexture, VARYING0).x * 0.9110000133514404296875);
    float f19 = texture2D(SpecularMapTexture, VARYING0).x * f17;

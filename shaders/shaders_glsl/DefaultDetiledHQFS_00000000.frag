...
void main()
{
    ...
    float f49 = f42.z;
    vec3 f50 = -CB0[16].xyz;
    float f51 = dot(f33, f50) * ((1.0 - ((step(f48.x, f49) * clamp(CB0[29].z + (CB0[29].w * abs(f49 - 0.5)), 0.0, 1.0)) * f48.y)) * f47.y);
    ...
    float f63 = 1.0 - f41;
    vec3 f64 = ((((((vec3(f63) - (f60 * (f39 * f63))) * CB0[15].xyz) * f53) + (CB0[17].xyz * (f63 * clamp(-f51, 0.0, 1.0)))) + min((f46.xyz * (f46.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f47.x)), vec3(CB0[21].w))) * f38.xyz) + (((f60 * (min((f61 + (f61 * f61)) / (((f62 * f62) * ((f56 * 3.0) + 0.5)) * ((f55 * 0.75) + 0.25)), 65504.0) * f53)) * CB0[15].xyz) * 1.0);
    vec4 f65 = vec4(0.0);
    f65.x = f64.x;
    vec4 f66 = f65;
    f66.y = f64.y;

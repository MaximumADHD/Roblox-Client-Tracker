...
void main()
{
    ...
    vec3 f50 = normalize((VARYING4.xyz / vec3(f0)) + f48);
    ...
    float f61 = 1.0 - f39;
    vec3 f62 = ((((((vec3(f61) - (f58 * (f37 * f61))) * CB0[15].xyz) * f51) + (CB0[17].xyz * (f61 * clamp(-f49, 0.0, 1.0)))) + (min((f44.xyz * (f44.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f45.x)), vec3(CB0[21].w)) * 1.0)) * f36.xyz) + (((f58 * (((f59 + (f59 * f59)) / max(((f60 * f60) * ((f54 * 3.0) + 0.5)) * ((f53 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f51)) * CB0[15].xyz) * 1.0);
    vec4 f63 = vec4(0.0);
    f63.x = f62.x;
    vec4 f64 = f63;
    f64.y = f62.y;

...
void main()
{
    ...
    float f62 = 1.0 - f40;
    vec3 f63 = ((((((vec3(f62) - (f59 * (f38 * f62))) * CB0[15].xyz) * f52) + (CB0[17].xyz * (f62 * clamp(-f50, 0.0, 1.0)))) + min((f45.xyz * (f45.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f46.x)), vec3(CB0[21].w))) * f37.xyz) + (((f59 * (((f60 + (f60 * f60)) / max(((f61 * f61) * ((f55 * 3.0) + 0.5)) * ((f54 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f52)) * CB0[15].xyz) * 1.0);
    vec4 f64 = vec4(0.0);
    f64.x = f63.x;
    vec4 f65 = f64;
    f65.y = f63.y;

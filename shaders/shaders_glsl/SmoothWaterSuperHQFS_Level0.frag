...
void main()
{
    ...
    vec3 f82 = mix(mix(f38.xyz, ((min(f5 + (CB0[27].xyz + ((CB0[28].xyz * (2.0 - CB0[9].w)) * f6)), vec3(CB0[16].w)) + (((((((CB0[35].xyz * f22) + (CB0[37].xyz * f23)) + (CB0[39].xyz * f24)) + (CB0[36].xyz * f25)) + (CB0[38].xyz * f26)) + (CB0[40].xyz * f27)) + (((((((CB0[29].xyz * f22) + (CB0[31].xyz * f23)) + (CB0[33].xyz * f24)) + (CB0[30].xyz * f25)) + (CB0[32].xyz * f26)) + (CB0[34].xyz * f27)) * f6))) + (CB0[10].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((f33.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(((f70 * f70) * CB0[15].x) * f6, (f71 * f71) * CB0[15].x, vec3((((float(abs(f69.x - 0.5) < 0.550000011920928955078125) * float(abs(f69.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f68) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture2D(GBufferDepthTexture, f69).x * 500.0) - f68) < 10.0)) * float(f41.w > 0.0))) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f16, f17))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (((((vec3(f79) + (vec3(0.0199999995529651641845703125) * (1.0 - f79))) * (((f80 + (f80 * f80)) / (((f81 * f81) * ((f76 * 3.0) + 0.5)) * ((f75 * 0.75) + 0.25))) * clamp((dot(f15, f72) * CB0[9].w) * f7, 0.0, 1.0))) * CB0[10].xyz) * (CB0[9].w * CB0[9].w)) * clamp(1.0 - (VARYING7.w * CB0[23].y), 0.0, 1.0));
    vec4 f83 = vec4(0.0);
    f83.x = f82.x;
    vec4 f84 = f83;
    f84.y = f82.y;
    ...
    vec4 f86 = f85;
    f86.w = 1.0;
    vec3 f87 = mix(CB0[14].xyz, f86.xyz, vec3(VARYING6.w));
    vec4 f88 = f86;
    f88.x = f87.x;
    vec4 f89 = f88;
    f89.y = f87.y;

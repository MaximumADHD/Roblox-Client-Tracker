...
void main()
{
    ...
    vec3 f73 = -CB0[11].xyz;
    vec3 f74 = normalize(f73 + f18);
    float f75 = f13 * f13;
    float f76 = max(0.001000000047497451305389404296875, dot(f16, f74));
    ...
    vec3 f83 = mix(mix(f39.xyz, ((min(f5 + (CB0[27].xyz + ((CB0[28].xyz * (2.0 - CB0[9].w)) * f6)), vec3(CB0[16].w)) + (((((((CB0[35].xyz * f23) + (CB0[37].xyz * f24)) + (CB0[39].xyz * f25)) + (CB0[36].xyz * f26)) + (CB0[38].xyz * f27)) + (CB0[40].xyz * f28)) + (((((((CB0[29].xyz * f23) + (CB0[31].xyz * f24)) + (CB0[33].xyz * f25)) + (CB0[30].xyz * f26)) + (CB0[32].xyz * f27)) + (CB0[34].xyz * f28)) * f6))) + (CB0[10].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((f34.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(((f71 * f71) * CB0[15].x) * f6, (f72 * f72) * CB0[15].x, vec3((((float(abs(f70.x - 0.5) < 0.550000011920928955078125) * float(abs(f70.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f69) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture2D(GBufferDepthTexture, f70).x * 500.0) - f69) < 10.0)) * float(f42.w > 0.0))) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f17, f18))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (((((vec3(f80) + (vec3(0.0199999995529651641845703125) * (1.0 - f80))) * (((f81 + (f81 * f81)) / (((f82 * f82) * ((f77 * 3.0) + 0.5)) * ((f76 * 0.75) + 0.25))) * clamp((dot(f16, f73) * CB0[9].w) * f7, 0.0, 1.0))) * CB0[10].xyz) * (CB0[9].w * CB0[9].w)) * clamp(1.0 - (VARYING7.w * CB0[23].y), 0.0, 1.0));
    vec4 f84 = vec4(0.0);
    f84.x = f83.x;
    vec4 f85 = f84;
    f85.y = f83.y;

...
void main()
{
    ...
    vec3 f18 = f13 + ((f1 * f9) * (0.1500000059604644775390625 + (0.949999988079071044921875 * texture2DLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)));
    f18.y = 0.0;
    bool f19;
    float f20;
    vec4 f21;
    ...
    for (; f28 < f14; f22 = f24, f21 = f27, f29 = f23, f20 = f26, f19 = f25, f28++)
    {
        ...
        vec2 f31 = (texture2DLod(AdvectionTexTexture, vec4((f22.xz * 0.125) + vec2(0.0, CB5[0].w * (-17.5)), 0.0, f30).xy, f30).xy * 2.0) - vec2(1.0);
        ...
        vec2 f37 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(texture3DLod(DetailTex3DTexture, vec4((f33 * (0.5 * CB5[4].y)) + (f32 * f36.x), f30).xyz, f30).x, texture3DLod(DetailTex3DTexture, vec4((f33 * (2.0 * CB5[4].y)) + (f32 * f36.y), f30).xyz, f30).x) * 2.0) - vec2(1.0));
        float f38 = texture3DLod(CloudsDistanceFieldTexture, vec4(((f22 + (f32 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f30).xyz, f30).x + (f37.x + f37.y);
        float f39 = (f35 * 50.0) * (CB5[2].x - f38);
        bool f40 = f38 < CB5[2].x;
        if (f40)
        {
            vec3 f41 = -CB0[11].xyz;
            vec4 f42 = texture2DLod(BeerShadowMapTexture, vec4((f22 - (f41 * (f22.y * (1.0 / max(f41.y, 9.9999997473787516355514526367188e-05))))).xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
            float f43 = f42.x;
    ...
    vec3 f59 = mix(CB0[14].xyz, (f21.xyz * (1.6480000019073486328125 - ((0.00200000009499490261077880859375 * f15) * f15))).xyz, vec3(f58));
    float f60 = 1.0 - f20;
    vec4 f61 = vec4(f59.x, f59.y, f59.z, f21.w);
    f61.w = f60;
    vec4 f62 = f61;
    f62.w = f60 * max(f58, CB0[13].y);
    gl_FragData[0] = f62;
}

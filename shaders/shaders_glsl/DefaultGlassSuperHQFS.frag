...
void main()
{
    ...
    vec4 f45 = texture2D(GBufferColorTexture, f39 + (f43 * clamp(min(texture2D(GBufferDepthTexture, f41 + f44).x * 500.0, texture2D(GBufferDepthTexture, f41 - f44).x * 500.0) - f38, 0.0, 1.0)));
    vec3 f46 = f45.xyz;
    ...
    gl_FragData[0] = mix(vec4(CB0[14].xyz, 1.0), vec4(f52.x, f52.y, f52.z, f50.w), vec4(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
}

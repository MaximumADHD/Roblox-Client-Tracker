...
void main()
{
    ...
    vec3 f1 = (texture2D(DiffuseMapTexture, VARYING0) * VARYING2).xyz;
    ...
    vec3 f15 = -CB0[11].xyz;
    float f16 = dot(f12, f15) * ((1.0 - ((step(f8.x, f9) * clamp(CB0[24].z + (CB0[24].w * abs(f9 - 0.5)), 0.0, 1.0)) * f8.y)) * f7.y);
    ...
    float f28 = 1.0 - f14;
    vec3 f29 = ((((((vec3(f28) - (f25 * (f11 * f28))) * CB0[10].xyz) * f18) + (CB0[12].xyz * (f28 * clamp(-f16, 0.0, 1.0)))) + min((f6.xyz * (f6.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f7.x)), vec3(CB0[16].w))) * f10) + ((f25 * (((f26 + (f26 * f26)) / (((f27 * f27) * ((f21 * 3.0) + 0.5)) * ((f20 * 0.75) + 0.25))) * f18)) * CB0[10].xyz);

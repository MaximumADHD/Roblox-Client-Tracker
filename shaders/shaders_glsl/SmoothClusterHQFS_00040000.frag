...
void main()
{
    ...
    float f37 = 1.0 - f15;
    vec3 f38 = (((((((vec3(f37) - (f34 * ((CB0[31].w * f13) * f37))) * CB0[15].xyz) * f27) + (CB0[17].xyz * (f37 * clamp(-f25, 0.0, 1.0)))) + (min((f20.xyz * (f20.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f21.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f13)) * f12) + ((((f34 * (((f35 + (f35 * f35)) / max(((f36 * f36) * ((f30 * 3.0) + 0.5)) * ((f29 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f27)) * CB0[15].xyz) * 1.0) * (f13 * VARYING0.w));
    vec4 f39 = vec4(0.0);
    f39.x = f38.x;
    vec4 f40 = f39;
    f40.y = f38.y;

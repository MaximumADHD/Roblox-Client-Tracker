...
void main()
{
    ...
    vec3 f12 = sqrt(clamp(mix(CB0[14].xyz, mix(f11, (min((f8.xyz * (f8.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f9.x)), vec3(CB0[16].w)) + (VARYING2 * ((1.0 - ((step(f10.x, VARYING4.z) * clamp(CB0[24].z + (CB0[24].w * abs(VARYING4.z - 0.5)), 0.0, 1.0)) * f10.y)) * f9.y))) * f11, vec3(CB2[0].z)).xyz, vec3(clamp(exp2((CB0[13].z * length(VARYING5)) + CB0[13].x) - CB0[13].w, 0.0, 1.0))).xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    gl_FragData[0] = vec4(f12.x, f12.y, f12.z, f4.w);
}

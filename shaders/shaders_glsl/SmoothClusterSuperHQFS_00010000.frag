...
void main()
{
    ...
    vec3 f68 = (((((((vec3(1.0) - (f65 * (CB0[31].w * f45))) * CB0[15].xyz) * f58) + (CB0[17].xyz * clamp(-f56, 0.0, 1.0))) + (min((f51.xyz * (f51.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f52.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f45)) * (f44 * f44)) + ((((f65 * (((f66 + (f66 * f66)) / max(((f67 * f67) * ((f61 * 3.0) + 0.5)) * ((f60 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f58)) * CB0[15].xyz) * 1.0) * (f45 * VARYING0.w));
    vec4 f69 = vec4(0.0);
    f69.x = f68.x;
    vec4 f70 = f69;
    f70.y = f68.y;

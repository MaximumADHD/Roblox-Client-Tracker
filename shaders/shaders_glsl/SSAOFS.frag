...
void main()
{
    vec4 _357 = texture2DLod(depthBufferTexture, vec4(VARYING0, 0.0, 0.0).xy, 0.0);
    float _358 = _357.x;
    vec4 _372 = (texture2D(randMapTexture, fract((VARYING0 * CB1[0].xy) * vec2(0.25))) * 2.0) - vec4(1.0);
    float _374 = _372.y;
    ...
    for (int _583 = 0; _583 < 8; )
    {
        ...
        vec2 _576 = vec2(0.0);
        _576.x = texture2DLod(depthBufferTexture, vec4(VARYING0 + _443, 0.0, _415).xy, _415).x;
        vec2 _578 = _576;
        _578.y = texture2DLod(depthBufferTexture, vec4(VARYING0 - _443, 0.0, _415).xy, _415).x;
    ...
    gl_FragData[0] = vec4((_358 > 0.9900000095367431640625) ? 1.0 : pow(clamp((_584 / _585) * 2.0, 0.0, 1.0), 1.0), _358, 0.0, 1.0);
}

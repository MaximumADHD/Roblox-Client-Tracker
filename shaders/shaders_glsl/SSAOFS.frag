...
void main()
{
    ...
    float f1 = f0.x;
    vec4 f2 = (texture2D(randMapTexture, fract((VARYING0 * CB1[0].xy) * vec2(0.25))) * 2.0) - vec4(1.0);
    float f3 = f2.y;
    ...
    for (int f9 = 0; f9 < 8; )
    {
        ...
        vec2 f16 = f8;
        f16.x = texture2DLod(depthBufferTexture, vec4(VARYING0 + f15, 0.0, f5).xy, f5).x;
        vec2 f17 = f16;
        f17.y = texture2DLod(depthBufferTexture, vec4(VARYING0 - f15, 0.0, f5).xy, f5).x;
    ...
    gl_FragData[0] = vec4((f1 > 0.9900000095367431640625) ? 1.0 : pow(clamp((f6 / f7) * 2.0, 0.0, 1.0), 1.0), f1, 0.0, 1.0);
}

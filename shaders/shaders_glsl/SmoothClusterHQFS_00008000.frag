...
void main()
{
    ...
    vec3 f36 = (((((((vec3(1.0) - (f33 * (CB0[31].w * f13))) * CB0[15].xyz) * f26) + (CB0[17].xyz * clamp(-f24, 0.0, 1.0))) + (min((f19.xyz * (f19.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f20.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f13)) * (f12 * f12)) + ((((f33 * (((f34 + (f34 * f34)) / max(((f35 * f35) * ((f29 * 3.0) + 0.5)) * ((f28 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f26)) * CB0[15].xyz) * 1.0) * (f13 * VARYING0.w));
    vec4 f37 = vec4(0.0);
    f37.x = f36.x;
    vec4 f38 = f37;
    f38.y = f36.y;

...
void main()
{
    ...
    vec3 f37 = normalize((VARYING4.xyz / vec3(f0)) + f35);
    ...
    float f48 = 1.0 - f26;
    vec3 f49 = ((((((vec3(f48) - (f45 * (f24 * f48))) * CB0[15].xyz) * f38) + (CB0[17].xyz * (f48 * clamp(-f36, 0.0, 1.0)))) + (min((f31.xyz * (f31.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f32.x)), vec3(CB0[21].w)) * 1.0)) * f23.xyz) + (((f45 * (((f46 + (f46 * f46)) / max(((f47 * f47) * ((f41 * 3.0) + 0.5)) * ((f40 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f38)) * CB0[15].xyz) * 1.0);
    vec4 f50 = vec4(0.0);
    f50.x = f49.x;
    vec4 f51 = f50;
    f51.y = f49.y;

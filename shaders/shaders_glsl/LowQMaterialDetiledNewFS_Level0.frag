...
void main()
{
    ...
    vec4 f13 = f12;
    f13.z = f10.z;
    vec3 f14 = VARYING6.xyz - (CB0[11].xyz * VARYING3.w);
    ...
    float f21 = f14.z;
    float f22 = (1.0 - ((step(f20.x, f21) * clamp(CB0[24].z + (CB0[24].w * abs(f21 - 0.5)), 0.0, 1.0)) * f20.y)) * f19.y;
    vec3 f23 = (((VARYING5.xyz * f22) + min((f18.xyz * (f18.w * 120.0)) + (CB0[8].xyz + (CB0[9].xyz * f19.x)), vec3(CB0[16].w))) * f13.xyz) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f13.xyz, vec3(VARYING6.w * CB0[26].w))) * (VARYING5.w * f22));
    vec4 f24 = vec4(0.0);
    f24.x = f23.x;
    vec4 f25 = f24;
    f25.y = f23.y;
    ...
    vec4 f27 = f26;
    f27.w = VARYING2.w;
    vec3 f28 = mix(CB0[14].xyz, f27.xyz, vec3(clamp(exp2((CB0[13].z * length(VARYING4.xyz)) + CB0[13].x) - CB0[13].w, 0.0, 1.0)));
    vec4 f29 = f27;
    f29.x = f28.x;
    vec4 f30 = f29;
    f30.y = f28.y;
    vec4 f31 = f30;
    f31.z = f28.z;
    vec3 f32 = sqrt(clamp(f31.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    vec4 f33 = f31;
    f33.x = f32.x;
    vec4 f34 = f33;
    f34.y = f32.y;

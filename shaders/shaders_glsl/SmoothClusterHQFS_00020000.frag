...
void main()
{
    ...
    float f75 = 1.0 - f53;
    vec3 f76 = (((((((vec3(f75) - (f72 * ((CB0[31].w * f51) * f75))) * CB0[15].xyz) * f65) + (CB0[17].xyz * (f75 * clamp(-f63, 0.0, 1.0)))) + (min((f58.xyz * (f58.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f59.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f12.z * 2.0) * f51)) * f50) + ((((f72 * (((f73 + (f73 * f73)) / max(((f74 * f74) * ((f68 * 3.0) + 0.5)) * ((f67 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f65)) * CB0[15].xyz) * 1.0) * (f51 * VARYING0.w));
    vec4 f77 = vec4(0.0);
    f77.x = f76.x;
    vec4 f78 = f77;
    f78.y = f76.y;

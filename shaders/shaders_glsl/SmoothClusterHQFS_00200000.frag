...
void main()
{
    ...
    vec3 f38 = (((((((vec3(1.0) - (f35 * (CB0[31].w * f14))) * CB0[15].xyz) * f28) + (CB0[17].xyz * clamp(-f26, 0.0, 1.0))) + (min((f21.xyz * (f21.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f22.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f14)) * (f13 * f13)) + ((((f35 * (((f36 + (f36 * f36)) / max(((f37 * f37) * ((f31 * 3.0) + 0.5)) * ((f30 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f28)) * CB0[15].xyz) * 1.0) * (f14 * VARYING0.w));
    vec4 f39 = vec4(0.0);
    f39.x = f38.x;
    vec4 f40 = f39;
    f40.y = f38.y;

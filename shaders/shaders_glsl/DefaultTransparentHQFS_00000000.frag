...
void main()
{
    ...
    float f32 = 1.0 - f10;
    vec3 f33 = ((((((vec3(f32) - (f29 * (f8 * f32))) * CB0[15].xyz) * f22) + (CB0[17].xyz * (f32 * clamp(-f20, 0.0, 1.0)))) + (min((f15.xyz * (f15.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f16.x)), vec3(CB0[21].w)) * 1.0)) * f7.xyz) + (((f29 * (((f30 + (f30 * f30)) / max(((f31 * f31) * ((f25 * 3.0) + 0.5)) * ((f24 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f22)) * CB0[15].xyz) * 1.0);
    vec4 f34 = vec4(0.0);
    f34.x = f33.x;
    vec4 f35 = f34;
    f35.y = f33.y;

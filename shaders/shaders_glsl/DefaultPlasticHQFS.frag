...
void main()
{
    ...
    vec3 f24 = textureCubeLod(PrefilteredEnvTexture, f23, f22).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f21.y * 1.58823525905609130859375, 0.0, 1.0)));
    vec4 f25 = texture2D(PrecomputedBRDFTexture, vec2(f20, max(9.9999997473787516355514526367188e-05, dot(f7, f18))));
    ...
    float f52 = f46.y;
    float f53 = f46.z;
    vec3 f54 = (mix(textureCubeLod(PrefilteredEnvIndoorTexture, f23, f22).xyz * f14, f24, vec3(f15)) * f42) * f19;
    ...
    gl_FragData[0] = vec4(f59.x, f59.y, f59.z, f56.w);
}

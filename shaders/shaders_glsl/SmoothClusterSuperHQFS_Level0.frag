...
void main()
{
    ...
    vec3 f19 = vec4(normalize(((mix(vec3(VARYING6.z, 0.0, -VARYING6.x), vec3(-VARYING6.y, VARYING6.x, 0.0), f18) * f17.x) + (mix(vec3(0.0, 1.0, 0.0), vec3(0.0, -VARYING6.z, VARYING6.y), f18) * f17.y)) + (VARYING6 * sqrt(clamp(1.0 + dot(vec2(1.0) - f16, f17), 0.0, 1.0)))), 0.0).xyz;
    ...
    vec3 f32 = normalize(normalize(VARYING8) - CB0[11].xyz);
    ...
    vec3 f40 = vec3(f39) + (mix(vec3(0.039999999105930328369140625), f14, vec3(f29)) * (1.0 - f39));
    float f41 = f34 * f34;
    float f42 = (((f35 * f41) - f35) * f35) + 1.0;
    float f43 = 1.0 - f29;
    vec3 f44 = (((((((vec3(f43) - (f40 * ((CB0[26].w * f15) * f43))) * CB0[10].xyz) * f33) + (CB0[12].xyz * (f43 * clamp(-f31, 0.0, 1.0)))) + (min((f24.xyz * (f24.w * 120.0)) + (CB0[8].xyz + (CB0[9].xyz * f25.x)), vec3(CB0[16].w)) * 1.0)) + vec3((f8.z * 2.0) * f15)) * f14) + (((((f40 * (((f41 + (f41 * f41)) / (((f42 * f42) * ((f36 * 3.0) + 0.5)) * ((f35 * 0.75) + 0.25))) * f33)) * CB0[10].xyz) * (CB0[9].w * CB0[9].w)) * f15) * VARYING0.w);
    vec4 f45 = vec4(0.0);
    f45.x = f44.x;
    vec4 f46 = f45;
    f46.y = f44.y;

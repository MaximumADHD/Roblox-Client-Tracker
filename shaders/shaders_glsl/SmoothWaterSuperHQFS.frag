...
void main()
{
    ...
    vec4 f19 = texture2D(GBufferColorTexture, f17);
    f19.w = texture2D(GBufferDepthTexture, f17).x * 500.0;
    float f20 = texture2D(GBufferDepthTexture, f18).x * 500.0;
    vec4 f21 = texture2D(GBufferColorTexture, f18);
    f21.w = f20;
    ...
    vec3 f59 = mix(mix(((f23 * f23) * CB0[15].x).xyz, (min(f5 + (CB0[8].xyz + (CB0[9].xyz * f6)), vec3(CB0[16].w)) + (CB0[10].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((f22.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(((f57 * f57) * CB0[15].x) * vec3(f6), (f58 * f58) * CB0[15].x, vec3((((float(abs(f56.x - 0.5) < 0.550000011920928955078125) * float(abs(f56.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f55) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture2D(GBufferDepthTexture, f52.xy / vec2(f53)).x * 500.0) - f53) < 10.0)) * float(f27 > 0.0))) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f15, f16))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (CB0[10].xyz * ((((2.0 * clamp(1.0 - (VARYING7.w * CB0[23].y), 0.0, 1.0)) * CB3[2].z) * f7) * pow(clamp(dot(f14, normalize((-CB0[11].xyz) + f16)), 0.0, 1.0), 900.0)));

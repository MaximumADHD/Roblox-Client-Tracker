...
void main()
{
    ...
    float f40 = 1.0 - f18;
    vec3 f41 = (((((((vec3(f40) - (f37 * ((CB0[31].w * f16) * f40))) * CB0[15].xyz) * f30) + (CB0[17].xyz * (f40 * clamp(-f28, 0.0, 1.0)))) + (min((f23.xyz * (f23.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f24.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f6.z * 2.0) * f16)) * f15) + ((((f37 * (((f38 + (f38 * f38)) / max(((f39 * f39) * ((f33 * 3.0) + 0.5)) * ((f32 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f30)) * CB0[15].xyz) * 1.0) * (f16 * VARYING0.w));
    vec4 f42 = vec4(0.0);
    f42.x = f41.x;
    vec4 f43 = f42;
    f43.y = f41.y;

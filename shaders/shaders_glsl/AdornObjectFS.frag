...
void main()
{
    ...
    vec4 _407 = mix(texture3D(LightMapTexture, _396), vec4(0.0), _406);
    vec3 _460 = vec3(CB0[15].x);
    vec3 _342 = (min(((_407.xyz * (_407.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * mix(texture3D(LightGridSkylightTexture, _396), vec4(1.0), _406).x), vec3(CB0[17].w)) + VARYING2) * mix(VARYING1.xyz, VARYING1.xyz * VARYING1.xyz, _460);
    ...
    gl_FragData[0] = vec4(_500.x, _500.y, _500.z, _546.w);
}

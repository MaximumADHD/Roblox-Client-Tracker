...
void main()
{
    ...
    vec4 f9 = texture(ShadowMapTexture, VARYING2.xy);
    vec3 f10 = (min((f7.xyz * (f7.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f8.x)), vec3(CB0[21].w)) + (VARYING4 * ((1.0 - ((step(f9.x, VARYING2.z) * clamp(CB0[29].z + (CB0[29].w * abs(VARYING2.z - 0.5)), 0.0, 1.0)) * f9.y)) * f8.y))) * f3;
    vec4 f11 = vec4(0.0);
    f11.x = f10.x;
    vec4 f12 = f11;
    f12.y = f10.y;
    ...
    vec4 f14 = f13;
    f14.w = 1.0;
    float f15 = clamp(exp2((CB0[18].z * VARYING2.w) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f16 = textureLod(PrefilteredEnvTexture, vec4(-VARYING5, 0.0).xyz, max(CB0[18].y, f15) * 5.0).xyz;

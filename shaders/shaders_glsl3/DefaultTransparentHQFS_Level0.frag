...
void main()
{
    ...
    vec3 f18 = reflect(-f1, f16);
    float f19 = f17 * 5.0;
    vec3 f20 = vec4(f18, f19).xyz;
    vec4 f21 = texture(PrecomputedBRDFTexture, vec2(f17, max(9.9999997473787516355514526367188e-05, dot(f16, f1))));
    float f22 = VARYING6.w * f15;
    vec3 f23 = mix(vec3(0.039999999105930328369140625), f14, vec3(f22));
    vec3 f24 = -CB0[11].xyz;
    float f25 = dot(f16, f24) * ((1.0 - ((step(f12.x, f13) * clamp(CB0[24].z + (CB0[24].w * abs(f13 - 0.5)), 0.0, 1.0)) * f12.y)) * f9.y);
    ...
    float f52 = f46.y;
    float f53 = f46.z;
    vec3 f54 = (mix(textureLod(PrefilteredEnvIndoorTexture, f20, f19).xyz * f10, textureLod(PrefilteredEnvTexture, f20, f19).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f18.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f11)) * f42) * f15;
    vec3 f55 = ((((((((f39 - (f34 * f38)) * CB0[10].xyz) * f27) + (CB0[12].xyz * (f37 * clamp(-f25, 0.0, 1.0)))) + (f43 * (((((((CB0[35].xyz * f48) + (CB0[37].xyz * f49)) + (CB0[39].xyz * f50)) + (CB0[36].xyz * f51)) + (CB0[38].xyz * f52)) + (CB0[40].xyz * f53)) + (((((((CB0[29].xyz * f48) + (CB0[31].xyz * f49)) + (CB0[33].xyz * f50)) + (CB0[30].xyz * f51)) + (CB0[32].xyz * f52)) + (CB0[34].xyz * f53)) * f11)))) + ((CB0[27].xyz + (CB0[28].xyz * f11)) * 1.0)) * f14) + (((f34 * (((f35 + (f35 * f35)) / (((f36 * f36) * ((f30 * 3.0) + 0.5)) * ((f29 * 0.75) + 0.25))) * f27)) * CB0[10].xyz) + f54)) + ((f10 * mix(f14, f54 * (1.0 / (max(max(f54.x, f54.y), f54.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f43) * (f15 * (1.0 - f11)))) * 1.0);
    float f56 = f2.w;

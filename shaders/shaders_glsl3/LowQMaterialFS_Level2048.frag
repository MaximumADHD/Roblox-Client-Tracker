...
void main()
{
    ...
    vec3 f19 = (((VARYING5.xyz * f18) + min((f14.xyz * (f14.w * 120.0)) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f15.x)), vec3(CB0[21].w))) * f9.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f9.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f18));
    vec4 f20 = vec4(0.0);
    f20.x = f19.x;
    vec4 f21 = f20;
    f21.y = f19.y;
    ...
    vec4 f23 = f22;
    f23.w = VARYING2.w;
    float f24 = clamp(exp2((CB0[18].z * length(VARYING4.xyz)) + CB0[18].x) - CB0[18].w, 0.0, 1.0);

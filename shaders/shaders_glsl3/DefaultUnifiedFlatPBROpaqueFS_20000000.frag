...
void main()
{
    ...
    vec4 f3 = texture(PbrDecalRoughnessMapTexture, VARYING0);
    vec4 f4 = vec4((texture(DiffuseMapTexture, VARYING0) * VARYING2).xyz, 1.0);
    float f5 = gl_FrontFacing ? 1.0 : (-1.0);
    vec3 f6 = VARYING6.xyz * f5;
    vec3 f7 = VARYING5.xyz * f5;
    vec3 f8 = normalize(((f6 * f1.x) + (cross(f7, f6) * f1.y)) + (f7 * f1.z));
    vec3 f9 = f4.xyz;
    vec3 f10 = f9 * f9;
    vec4 f11 = f4;
    f11.x = f10.x;

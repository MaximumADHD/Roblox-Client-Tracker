...
void main()
{
    ...
    float f43 = f33.z;
    vec3 f44 = -CB0[16].xyz;
    float f45 = dot(f19, f44) * ((1.0 - ((step(f42.x, f43) * clamp(CB0[29].z + (CB0[29].w * abs(f43 - 0.5)), 0.0, 1.0)) * f42.y)) * f40.y);
    vec3 f46 = normalize(f26 + f44);
    float f47 = clamp(f45, 0.0, 1.0);
    float f48 = f28 * f28;
    float f49 = max(0.001000000047497451305389404296875, dot(f19, f46));
    ...
    vec4 f64 = texture(PrecomputedBRDFTexture, vec2(f28, max(9.9999997473787516355514526367188e-05, dot(f19, f26))));
    float f65 = f64.x;
    float f66 = f64.y;
    vec3 f67 = ((f32 * f65) + vec3(f66)) / vec3(f65 + f66);
    vec3 f68 = f19 * f19;
    bvec3 f69 = lessThan(f19, vec3(0.0));

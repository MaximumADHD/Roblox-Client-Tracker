...
void main()
{
    ...
    vec3 f36 = -CB0[11].xyz;
    float f37 = dot(f14, f36) * ((1.0 - ((step(f23.x, f24) * clamp(CB0[24].z + (CB0[24].w * abs(f24 - 0.5)), 0.0, 1.0)) * f23.y)) * f20.y);
    vec3 f38 = normalize(f36 + f27);
    ...
    vec3 f49 = vec3(f22);
    vec3 f50 = mix(f21, f34, f49) * mix(vec3(1.0), f25, vec3(0.5));
    float f51 = f35.x;
    float f52 = f35.y;
    ...
    float f63 = f57.y;
    float f64 = f57.z;
    vec3 f65 = ((((((CB0[35].xyz * f59) + (CB0[37].xyz * f60)) + (CB0[39].xyz * f61)) + (CB0[36].xyz * f62)) + (CB0[38].xyz * f63)) + (CB0[40].xyz * f64)) + (((((((CB0[29].xyz * f59) + (CB0[31].xyz * f60)) + (CB0[33].xyz * f61)) + (CB0[30].xyz * f62)) + (CB0[32].xyz * f63)) + (CB0[34].xyz * f64)) * f22);
    vec3 f66 = (mix(textureLod(PrefilteredEnvIndoorTexture, f33, f32).xyz, f34, f49) * f53) * f29;
    float f67 = 1.0 - f30;
    vec4 f68 = mix(vec4(mix((((((((vec3(1.0) - (f46 * f29)) * CB0[10].xyz) * f39) + (CB0[12].xyz * clamp(-f37, 0.0, 1.0))) + ((((vec3(1.0) - f54) * f65) * CB0[25].w) * f13)) + (CB0[27].xyz + (CB0[28].xyz * f22))) * f28) + ((f21 * mix(f28, f66 * (1.0 / (max(max(f65.x, f65.y), f65.z) + 0.00999999977648258209228515625)), f54 * (f29 * (1.0 - f22)))) * f13), f50, vec3(VARYING7.w)) * f13, f13), vec4(f50, 1.0), vec4(((f67 * f67) * 0.800000011920928955078125) * clamp(f12, 0.0, 1.0))) + vec4(((f46 * (((f47 + (f47 * f47)) / (((f48 * f48) * ((f42 * 3.0) + 0.5)) * ((f41 * 0.75) + 0.25))) * f39)) * CB0[10].xyz) + ((f66 * f22) * f13), 0.0);
    float f69 = clamp(exp2((CB0[13].z * f26) + CB0[13].x) - CB0[13].w, 0.0, 1.0);
    ...
    f73.w = 1.0 - ((1.0 - f68.w) * f69);
    vec3 f74 = sqrt(clamp(f73.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    _entryPointOutput = vec4(f74.x, f74.y, f74.z, f73.w);
}

...
void main()
{
    ...
    float f17 = f16.y;
    vec3 f18 = ((((CB3[1].xyz * (1.0 - texture(SpecularMapTexture, VARYING0).w)) + (VARYING5.xyz * f17)) + min((f15.xyz * (f15.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f16.x)), vec3(CB0[21].w))) * f10.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f10.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f17));
    vec4 f19 = vec4(0.0);
    f19.x = f18.x;
    vec4 f20 = f19;
    f20.y = f18.y;

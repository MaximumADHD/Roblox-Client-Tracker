...
void main()
{
    vec4 _302 = texture(texTexture, VARYING0.xy);
    vec4 _307 = VARYING1 * CB1[0];
    vec3 _319 = _302.xyz * _307.xyz;
    vec4 _470 = vec4(_319.x, _319.y, _319.z, vec4(0.0).w);
    _470.w = (_302.w * _307.w) * clamp(VARYING3.w * abs((texture(depthTexTexture, VARYING3.xy).x * 500.0) - VARYING3.z), 0.0, 1.0);
    vec3 _388 = vec3(CB0[15].x);
    vec4 _398 = texture(LightingAtlasTexture, VARYING2);
    ...
    _entryPointOutput = vec4(_441.x, _441.y, _441.z, _470.w);
}

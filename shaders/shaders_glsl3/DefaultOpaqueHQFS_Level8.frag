...
void main()
{
    ...
    float f12 = 0.08900000154972076416015625 + (VARYING5.w * 0.9110000133514404296875);
    vec3 f13 = reflect(-f1, f3);
    float f14 = VARYING6.w * f11;
    vec3 f15 = mix(vec3(0.039999999105930328369140625), f10.xyz, vec3(f14));
    vec3 f16 = VARYING6.xyz - (CB0[16].xyz * VARYING3.w);
    ...
    float f74 = f68.y;
    float f75 = f68.z;
    vec3 f76 = ((((((f22.xyz * (f22.w * 120.0)) * 1.0) + (((f57 - (f52 * f56)) * CB0[15].xyz) * f45)) + ((f57 - (f65 * f56)) * (((((((CB0[40].xyz * f70) + (CB0[42].xyz * f71)) + (CB0[44].xyz * f72)) + (CB0[41].xyz * f73)) + (CB0[43].xyz * f74)) + (CB0[45].xyz * f75)) + (((((((CB0[34].xyz * f70) + (CB0[36].xyz * f71)) + (CB0[38].xyz * f72)) + (CB0[35].xyz * f73)) + (CB0[37].xyz * f74)) + (CB0[39].xyz * f75)) * f24)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f24))) * f10.xyz) + ((((f52 * (((f53 + (f53 * f53)) / (((f54 * f54) * ((f48 * 3.0) + 0.5)) * ((f47 * 0.75) + 0.25))) * f45)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) + ((mix(f61, textureLod(PrefilteredEnvTexture, f59, f58).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f13.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f24)) * f65) * f11));
    vec4 f77 = vec4(0.0);
    f77.x = f76.x;
    vec4 f78 = f77;
    f78.y = f76.y;

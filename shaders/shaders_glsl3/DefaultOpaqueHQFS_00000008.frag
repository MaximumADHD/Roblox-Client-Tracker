...
void main()
{
    ...
    float f46 = clamp((f5 * CB0[14].w) * (((f5 * CB0[52].x) > 0.0) ? mix(min((f35 <= f39) ? 1.0 : clamp(((f40 / (f40 + (f41 * f41))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f36 <= f42) ? 1.0 : clamp(((f43 / (f43 + (f44 * f44))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f26, clamp((length(f17 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f26), 0.0, 1.0);
    float f47 = f12 * f12;
    float f48 = max(0.001000000047497451305389404296875, dot(f3, f45));
    float f49 = dot(f4, f45);
    float f50 = 1.0 - f49;
    float f51 = f50 * f50;
    ...
    vec4 f63 = texture(PrecomputedBRDFTexture, vec2(f12, max(9.9999997473787516355514526367188e-05, dot(f3, f1))));
    float f64 = f63.x;
    float f65 = f63.y;
    vec3 f66 = ((f16 * f64) + vec3(f65)) / vec3(f64 + f65);
    vec3 f67 = f3 * f3;
    bvec3 f68 = lessThan(f3, vec3(0.0));
    ...
    vec3 f83 = textureLod(PrefilteredEnvTexture, vec4(f13, 0.0).xyz, max(CB0[18].y, f82) * 5.0).xyz;

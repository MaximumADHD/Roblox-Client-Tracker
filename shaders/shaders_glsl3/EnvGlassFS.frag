...
void main()
{
    ...
    vec3 f1 = VARYING2.xyz * VARYING2.xyz;
    vec4 f2 = VARYING2;
    f2.x = f1.x;
    vec4 f3 = f2;
    f3.y = f1.y;
    ...
    vec3 f14 = (((VARYING5.xyz * f13) + min((f9.xyz * (f9.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f10.x)), vec3(CB0[21].w))) * f4.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f4.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f13));
    vec4 f15 = vec4(0.0);
    f15.x = f14.x;
    vec4 f16 = f15;
    f16.y = f14.y;
    ...
    vec4 f18 = f17;
    f18.w = VARYING2.w;
    float f19 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f20 = textureLod(PrefilteredEnvTexture, vec4(-(VARYING4.xyz / vec3(f0)), 0.0).xyz, max(CB0[18].y, f19) * 5.0).xyz;

...
void main()
{
    ...
    float f11 = f4.z;
    float f12 = (1.0 - ((step(f10.x, f11) * clamp(CB0[24].z + (CB0[24].w * abs(f11 - 0.5)), 0.0, 1.0)) * f10.y)) * f9.y;
    vec3 f13 = (((VARYING5.xyz * f12) + min((f8.xyz * (f8.w * 120.0)) + (CB0[27].xyz + ((CB0[28].xyz * (2.0 - CB0[9].w)) * f9.x)), vec3(CB0[16].w))) * f3.xyz) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f3.xyz, vec3(VARYING6.w * CB0[26].w))) * (VARYING5.w * f12));
    vec4 f14 = vec4(0.0);
    f14.x = f13.x;
    vec4 f15 = f14;
    f15.y = f13.y;
    ...
    vec3 f21 = mix(vec3(f20.x ? CB0[14].xyz.x : f19.x, f20.y ? CB0[14].xyz.y : f19.y, f20.z ? CB0[14].xyz.z : f19.z), f17.xyz, vec3(f18));
    vec4 f22 = f17;
    f22.x = f21.x;
    vec4 f23 = f22;
    f23.y = f21.y;

...
void main()
{
    ...
    vec4 f4 = texture(CloudsTexture, f3);
    vec4 f5 = vec4(1.0, 1.0, -1.0, 0.0) * CB2[0].xyxy;
    vec2 f6 = f5.xy;
    vec2 f7 = f5.wy;
    vec2 f8 = f5.zy;
    vec4 f9 = (((((((((f4 * 4.0) + textureLod(CloudsTexture, f3 - f6, 0.0)) + (textureLod(CloudsTexture, f3 - f7, 0.0) * 2.0)) + textureLod(CloudsTexture, f3 - f8, 0.0)) + (textureLod(CloudsTexture, f3 + f5.zw, 0.0) * 2.0)) + (textureLod(CloudsTexture, f3 + f5.xw, 0.0) * 2.0)) + textureLod(CloudsTexture, f3 + f8, 0.0)) + (textureLod(CloudsTexture, f3 + f7, 0.0) * 2.0)) + textureLod(CloudsTexture, f3 + f6, 0.0)) * vec4(0.0625);
    vec4 f10 = f4;
    f10.x = f9.x;
    vec4 f11 = f10;
    f11.y = f9.y;

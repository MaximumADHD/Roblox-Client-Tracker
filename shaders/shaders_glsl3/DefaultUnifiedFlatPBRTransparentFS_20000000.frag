...
void main()
{
    ...
    vec3 f3 = mix(vec3(0.0, 0.0, 1.0), (f2.xyz * 2.0) - vec3(1.0), vec3(clamp((vec2(0.0033333334140479564666748046875, CB0[28].y) * (-VARYING4.w)) + vec2(1.0), vec2(0.0), vec2(1.0)).y));
    vec4 f4 = texture(SpecularMapTexture, VARYING0);
    vec4 f5 = texture(PbrDecalRoughnessMapTexture, VARYING0);
    float f6 = gl_FrontFacing ? 1.0 : (-1.0);
    vec3 f7 = VARYING6.xyz * f6;
    vec3 f8 = VARYING5.xyz * f6;
    vec3 f9 = normalize(((f7 * f3.x) + (cross(f8, f7) * f3.y)) + (f8 * f3.z));
    vec3 f10 = f1.xyz;
    vec3 f11 = f10 * f10;
    vec4 f12 = f1;
    f12.x = f11.x;
    ...
    float f18 = 0.08900000154972076416015625 + (f5.x * 0.9110000133514404296875);
    vec3 f19 = -f16;
    ...
    vec4 f75 = f73;
    f75.w = f74;
    float f76 = clamp(exp2((CB0[18].z * f15) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f77 = textureLod(PrefilteredEnvTexture, vec4(f19, 0.0).xyz, max(CB0[18].y, f76) * 5.0).xyz;

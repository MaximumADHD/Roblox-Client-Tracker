...
void main()
{
    ...
    float f30 = texture(GBufferDepthTexture, f29).x * 500.0;
    float f31 = clamp(f30 - VARYING8.w, 0.0, 1.0);
    vec3 f32 = mix(texture(GBufferColorTexture, f28).xyz, texture(GBufferColorTexture, f29).xyz, vec3(f31));
    vec3 f33 = -f17;
    vec3 f34 = reflect(f33, f16);
    float f35 = VARYING8.w * 0.20000000298023223876953125;
    vec4 f36 = vec4(f34, 0.0);
    ...
    vec3 f68 = -CB0[16].xyz;
    vec3 f69 = normalize(f17 - CB0[16].xyz);
    float f70 = f67 * f67;
    ...
    vec3 f78 = mix(mix((f32 * f32) * CB0[20].x, ((min(f5 + (CB0[32].xyz + (CB0[33].xyz * f6)), vec3(CB0[21].w)) + (((((((CB0[40].xyz * f22) + (CB0[42].xyz * f23)) + (CB0[44].xyz * f24)) + (CB0[41].xyz * f25)) + (CB0[43].xyz * f26)) + (CB0[45].xyz * f27)) + (((((((CB0[34].xyz * f22) + (CB0[36].xyz * f23)) + (CB0[38].xyz * f24)) + (CB0[35].xyz * f25)) + (CB0[37].xyz * f26)) + (CB0[39].xyz * f27)) * f6))) + (CB0[15].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((mix(texture(GBufferDepthTexture, f28).x * 500.0, f30, f31) - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix((textureLod(PrefilteredEnvTexture, f36.xyz, 0.0).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f34.y * 1.58823525905609130859375, 0.0, 1.0)))) * f6, (f66 * f66) * CB0[20].x, vec3((((float(abs(f65.x - 0.5) < 0.550000011920928955078125) * float(abs(f65.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f64) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture(GBufferDepthTexture, f65).x * 500.0) - f64) < 10.0)) * float(f37.w > 0.0))) + (f5 * 0.100000001490116119384765625), vec3((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f16, f17))), 0.0, 1.0) + 0.300000011920928955078125) * (VARYING0.w * CB3[2].z))) + (((((vec3(f75) + (vec3(0.0199999995529651641845703125) * (1.0 - f75))) * (((f76 + (f76 * f76)) / max(((f77 * f77) * ((f72 * 3.0) + 0.5)) * ((f71 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * clamp(dot(f15, f68) * f7, 0.0, 1.0))) * CB0[15].xyz) * 1.0) * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f79 = vec4(0.0);
    f79.x = f78.x;
    vec4 f80 = f79;
    f80.y = f78.y;
    ...
    vec4 f82 = f81;
    f82.w = 1.0;
    vec3 f83 = textureLod(PrefilteredEnvTexture, vec4(f33, 0.0).xyz, max(CB0[18].y, VARYING6.w) * 5.0).xyz;

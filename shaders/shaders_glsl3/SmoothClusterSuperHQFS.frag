...
void main()
{
    ...
    vec3 f11 = vec3(dot(VARYING7, VARYING0.xyz));
    ...
    vec3 f25 = (f6 * f6).xyz;
    float f26 = f13.y;
    float f27 = CB0[26].w * f7;
    vec3 f28 = reflect(-f24, f12);
    vec3 f29 = -CB0[11].xyz;
    float f30 = dot(f12, f29) * ((1.0 - ((step(f22.x, f23) * clamp(CB0[24].z + (CB0[24].w * abs(f23 - 0.5)), 0.0, 1.0)) * f22.y)) * f19.y);
    ...
    vec4 f44 = texture(PrecomputedBRDFTexture, vec2(f26, max(9.9999997473787516355514526367188e-05, dot(f12, f24))));
    float f45 = f44.x;
    float f46 = f44.y;

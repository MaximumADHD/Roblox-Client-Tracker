...
void main()
{
    ...
    float f64 = f58.y;
    float f65 = f58.z;
    vec3 f66 = (mix(textureLod(PrefilteredEnvIndoorTexture, f34, f33).xyz * f22, f35, f50) * f54) * f30;
    float f67 = 1.0 - f31;
    ...
    vec4 f85 = mix(mix(f84, vec4(mix((((((((vec3(1.0) - (f47 * f30)) * CB0[10].xyz) * f40) + (CB0[12].xyz * clamp(-f38, 0.0, 1.0))) + (((vec3(1.0) - f55) * (((((((CB0[35].xyz * f60) + (CB0[37].xyz * f61)) + (CB0[39].xyz * f62)) + (CB0[36].xyz * f63)) + (CB0[38].xyz * f64)) + (CB0[40].xyz * f65)) + (((((((CB0[29].xyz * f60) + (CB0[31].xyz * f61)) + (CB0[33].xyz * f62)) + (CB0[30].xyz * f63)) + (CB0[32].xyz * f64)) + (CB0[34].xyz * f65)) * f23))) * f13)) + (CB0[27].xyz + (CB0[28].xyz * f23))) * f29) + ((f22 * mix(f29, f66 * (1.0 / (max(max(f66.x, f66.y), f66.z) + 0.00999999977648258209228515625)), f55 * (f30 * (1.0 - f23)))) * f13), f51, vec3(VARYING7.w)), 1.0), vec4(f13)), vec4(f51, 1.0), vec4(((f67 * f67) * 0.800000011920928955078125) * f14)) + vec4(((f47 * (((f48 + (f48 * f48)) / (((f49 * f49) * ((f43 * 3.0) + 0.5)) * ((f42 * 0.75) + 0.25))) * f40)) * CB0[10].xyz) + (f66 * f13), 0.0);

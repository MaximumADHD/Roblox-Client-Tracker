...
void main()
{
    ...
    float f36 = f26.z;
    vec3 f37 = -CB0[16].xyz;
    float f38 = dot(f16, f37) * ((1.0 - ((step(f35.x, f36) * clamp(CB0[29].z + (CB0[29].w * abs(f36 - 0.5)), 0.0, 1.0)) * f35.y)) * f33.y);
    ...
    vec4 f54 = texture(PrecomputedBRDFTexture, vec2(f23, max(9.9999997473787516355514526367188e-05, dot(f16, f1))));
    float f55 = f54.x;
    float f56 = f54.y;

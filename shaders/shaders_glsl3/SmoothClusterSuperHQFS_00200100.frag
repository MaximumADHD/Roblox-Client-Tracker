...
void main()
{
    ...
    float f63 = clamp(1.0 - (VARYING5.w * CB0[28].y), 0.0, 1.0);
    ...
    vec3 f72 = normalize(VARYING9);
    float f73 = 0.08900000154972076416015625 + (f28.y * 0.9110000133514404296875);
    float f74 = CB0[31].w * f63;
    vec3 f75 = reflect(-f72, f71);
    ...
    float f84 = f83.x;
    vec4 f85 = texture(ShadowMapTexture, f76.xy);
    float f86 = f76.z;
    vec3 f87 = -CB0[16].xyz;
    float f88 = dot(f71, f87) * ((1.0 - ((step(f85.x, f86) * clamp(CB0[29].z + (CB0[29].w * abs(f86 - 0.5)), 0.0, 1.0)) * f85.y)) * f83.y);
    vec3 f89 = normalize(f72 - CB0[16].xyz);
    ...
    vec4 f104 = texture(PrecomputedBRDFTexture, vec2(f73, max(9.9999997473787516355514526367188e-05, dot(f71, f72))));
    float f105 = f104.x;
    float f106 = f104.y;
    ...
    float f116 = f110.y;
    float f117 = f110.z;
    vec3 f118 = (((((((f82.xyz * (f82.w * 120.0)) * 1.0) + ((((vec3(1.0) - (f97 * f74)) * CB0[15].xyz) * f90) + (CB0[17].xyz * clamp(-f88, 0.0, 1.0)))) + ((vec3(1.0) - (f107 * f74)) * (((((((CB0[40].xyz * f112) + (CB0[42].xyz * f113)) + (CB0[44].xyz * f114)) + (CB0[41].xyz * f115)) + (CB0[43].xyz * f116)) + (CB0[45].xyz * f117)) + (((((((CB0[34].xyz * f112) + (CB0[36].xyz * f113)) + (CB0[38].xyz * f114)) + (CB0[35].xyz * f115)) + (CB0[37].xyz * f116)) + (CB0[39].xyz * f117)) * f84)))) + ((CB0[32].xyz + (CB0[33].xyz * f84)) * 1.0)) + vec3((f28.z * 2.0) * f63)) * (f62 * f62)) + (((((f97 * (min((f98 + (f98 * f98)) / (((f99 * f99) * ((f93 * 3.0) + 0.5)) * ((f92 * 0.75) + 0.25)), 65504.0) * f90)) * CB0[15].xyz) * 1.0) * (f63 * VARYING0.w)) + ((mix(f103, textureLod(PrefilteredEnvTexture, f101, f100).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f75.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f84)) * f107) * f74));
    vec4 f119 = vec4(0.0);
    f119.x = f118.x;
    vec4 f120 = f119;
    f120.y = f118.y;
    ...
    vec3 f124 = textureLod(PrefilteredEnvTexture, vec4(-VARYING9, 0.0).xyz, max(CB0[18].y, f123) * 5.0).xyz;

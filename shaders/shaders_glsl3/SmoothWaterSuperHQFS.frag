...
void main()
{
    ...
    vec4 f18 = texture(GBufferColorTexture, f16);
    f18.w = texture(GBufferDepthTexture, f16).x * 500.0;
    float f19 = texture(GBufferDepthTexture, f17).x * 500.0;
    vec4 f20 = texture(GBufferColorTexture, f17);
    f20.w = f19;
    ...
    float f31 = f30.w;
    float f32 = f29 + clamp((texture(GBufferDepthTexture, f30.xy / vec2(f31)).x * 500.0) - f31, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f33 = VARYING8 + (f28 * f32);
    float f34 = f33.w;
    float f35 = f32 + clamp((texture(GBufferDepthTexture, f33.xy / vec2(f34)).x * 500.0) - f34, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f36 = VARYING8 + (f28 * f35);
    float f37 = f36.w;
    float f38 = f35 + clamp((texture(GBufferDepthTexture, f36.xy / vec2(f37)).x * 500.0) - f37, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f39 = VARYING8 + (f28 * f38);
    float f40 = f39.w;
    float f41 = f38 + clamp((texture(GBufferDepthTexture, f39.xy / vec2(f40)).x * 500.0) - f40, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f42 = VARYING8 + (f28 * f41);
    float f43 = f42.w;
    float f44 = f41 + clamp((texture(GBufferDepthTexture, f42.xy / vec2(f43)).x * 500.0) - f43, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f45 = VARYING8 + (f28 * f44);
    float f46 = f45.w;
    float f47 = f44 + clamp((texture(GBufferDepthTexture, f45.xy / vec2(f46)).x * 500.0) - f46, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f48 = VARYING8 + (f28 * f47);
    float f49 = f48.w;
    float f50 = f47 + clamp((texture(GBufferDepthTexture, f48.xy / vec2(f49)).x * 500.0) - f49, VARYING8.w * (-0.20000000298023223876953125), f24);
    vec4 f51 = VARYING8 + (f28 * f50);
    float f52 = f51.w;
    vec4 f53 = VARYING8 + (f28 * f50);
    float f54 = f53.w;
    vec2 f55 = f53.xy / vec2(f54);
    vec3 f56 = texture(EnvMapTexture, f23).xyz;
    vec3 f57 = texture(GBufferColorTexture, f55).xyz;
    vec3 f58 = mix(mix(((f22 * f22) * CB0[15].x).xyz, CB3[1].xyz * (min((f5 + CB0[8].xyz) + (CB0[9].xyz * f6), vec3(CB0[16].w)) + (CB0[10].xyz * f7)), vec3(clamp(clamp(((f21.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(((f56 * f56) * CB0[15].x) * f6, (f57 * f57) * CB0[15].x, vec3((((float(abs(f55.x - 0.5) < 0.550000011920928955078125) * float(abs(f55.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f54) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture(GBufferDepthTexture, f51.xy / vec2(f52)).x * 500.0) - f52) < 10.0)) * float(f26 > 0.0))) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f14, f15))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (CB0[10].xyz * ((((2.0 * clamp(1.0 - (VARYING7.w * CB0[23].y), 0.0, 1.0)) * CB3[2].z) * f7) * pow(clamp(dot(f13, normalize((-CB0[11].xyz) + f15)), 0.0, 1.0), 900.0)));

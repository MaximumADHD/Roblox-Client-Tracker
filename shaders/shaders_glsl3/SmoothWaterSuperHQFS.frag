...
void main()
{
    ...
    float f44 = f43.w;
    float f45 = f42 + clamp((texture(GBufferDepthTexture, f43.xy / vec2(f44)).x * 500.0) - f44, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f46 = VARYING8 + (f41 * f45);
    float f47 = f46.w;
    float f48 = f45 + clamp((texture(GBufferDepthTexture, f46.xy / vec2(f47)).x * 500.0) - f47, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f49 = VARYING8 + (f41 * f48);
    float f50 = f49.w;
    float f51 = f48 + clamp((texture(GBufferDepthTexture, f49.xy / vec2(f50)).x * 500.0) - f50, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f52 = VARYING8 + (f41 * f51);
    float f53 = f52.w;
    float f54 = f51 + clamp((texture(GBufferDepthTexture, f52.xy / vec2(f53)).x * 500.0) - f53, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f55 = VARYING8 + (f41 * f54);
    float f56 = f55.w;
    float f57 = f54 + clamp((texture(GBufferDepthTexture, f55.xy / vec2(f56)).x * 500.0) - f56, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f58 = VARYING8 + (f41 * f57);
    float f59 = f58.w;
    float f60 = f57 + clamp((texture(GBufferDepthTexture, f58.xy / vec2(f59)).x * 500.0) - f59, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f61 = VARYING8 + (f41 * f60);
    float f62 = f61.w;
    float f63 = f60 + clamp((texture(GBufferDepthTexture, f61.xy / vec2(f62)).x * 500.0) - f62, VARYING8.w * (-0.20000000298023223876953125), f37);
    vec4 f64 = VARYING8 + (f41 * f63);
    float f65 = f64.w;
    vec4 f66 = VARYING8 + (f41 * f63);
    float f67 = f66.w;
    vec2 f68 = f66.xy / vec2(f67);
    vec3 f69 = texture(EnvMapTexture, f36).xyz;
    vec3 f70 = texture(GBufferColorTexture, f68).xyz;
    vec3 f71 = -CB0[11].xyz;

...
void main()
{
    ...
    _entryPointOutput = vec4(clamp(vec3(1.0) + (min((vec3(uvec3((textureLod(Texture6Texture, vec4(fract((f0 * CB1[17].xy) + CB1[16].xy), 0.0, 0.0).xy, 0.0).xyz * 255.0) + vec3(0.5)) ^ uvec3(f4 & 255u, (f4 >> 8u) & 255u, (f4 >> 16u) & 255u)) * 0.007843137718737125396728515625) - vec3(1.0), vec3(0.0)) * CB1[16].z), vec3(0.0), vec3(1.0)), 1.0);
}

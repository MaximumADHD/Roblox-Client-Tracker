...
void main()
{
    ...
    vec3 f31 = min(f7, max(f6, f4 / vec3(f5)));
    vec2 f32;
    do
    {
        uint f33 = uint(CB1[17].w);
        uint f34 = uint(CB1[0].x) - 1u;
        uint f35 = uint(CB1[0].y) - 1u;
        if (f33 == 0u)
        {
            f32 = gl_FragCoord.xy;
            break;
        ...
        if (f33 == 1u)
        {
            f32 = vec2(gl_FragCoord.y, float(f34) - gl_FragCoord.x);
            break;
        }
        if (f33 == 2u)
        {
            f32 = vec2(float(f34) - gl_FragCoord.x, float(f35) - gl_FragCoord.y);
            break;
        }
        f32 = vec2(float(f35) - gl_FragCoord.y, gl_FragCoord.x);
        break;
    } while(false);

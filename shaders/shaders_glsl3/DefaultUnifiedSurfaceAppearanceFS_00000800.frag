...
void main()
{
    ...
    float f14 = f13.y;
    vec3 f15 = (((VARYING5.xyz * f14) + min((f12.xyz * (f12.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f13.x)), vec3(CB0[21].w))) * f7.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f7.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f14));
    vec4 f16 = vec4(0.0);
    f16.x = f15.x;
    vec4 f17 = f16;
    f17.y = f15.y;
    ...
    vec4 f19 = f18;
    f19.w = f2.w;
    float f20 = clamp(exp2((CB0[18].z * f8) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f21 = textureLod(PrefilteredEnvTexture, vec4(-(VARYING4.xyz / vec3(f8)), 0.0).xyz, max(CB0[18].y, f20) * 5.0).xyz;

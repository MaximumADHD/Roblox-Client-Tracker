...
void main()
{
    ...
    vec3 f3 = normalize(VARYING5.xyz);
    vec3 f4 = -CB0[11].xyz;
    float f5 = dot(f3, f4);
    vec3 f6 = f2.xyz;
    ...
    float f43 = clamp(f5 * ((f5 > 0.0) ? mix(f14, mix(min((f23 <= f27) ? 1.0 : clamp(((f28 / (f28 + (f29 * f29))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f24 <= f30) ? 1.0 : clamp(((f31 / (f31 + (f32 * f32))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f14, clamp((length(f7 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)), CB0[47].x) : 0.0), 0.0, 1.0);
    float f44 = f35 * f35;
    ...
    vec3 f72 = (((((((f55 - (f50 * f54)) * CB0[10].xyz) * f43) + ((f59 * f70) * CB0[25].w)) + ((CB0[27].xyz + (CB0[28].xyz * f13)) * 1.0)) * f33) + (((f50 * (((f51 + (f51 * f51)) / (((f52 * f52) * ((f46 * 3.0) + 0.5)) * ((f45 * 0.75) + 0.25))) * f43)) * CB0[10].xyz) + (f71 * f13))) + (((f11.xyz * (f11.w * 120.0)).xyz * mix(f33, f71 * (1.0 / (max(max(f70.x, f70.y), f70.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f59) * (f34 * (1.0 - f13)))) * 1.0);
    float f73 = f2.w;

...
void main()
{
    ...
    float f21 = 0.08900000154972076416015625 + (f10.y * 0.9110000133514404296875);
    vec3 f22 = -f1;
    vec3 f23 = reflect(f22, f14);
    float f24 = f10.x * f20;
    vec3 f25 = mix(vec3(0.039999999105930328369140625), f19.xyz, vec3(f24));
    ...
    vec4 f57 = texture(PrecomputedBRDFTexture, vec2(f21, max(9.9999997473787516355514526367188e-05, dot(f14, f1))));
    float f58 = f57.x;
    float f59 = f57.y;
    vec3 f60 = ((f25 * f58) + vec3(f59)) / vec3(f58 + f59);
    vec3 f61 = f14 * f14;
    bvec3 f62 = lessThan(f14, vec3(0.0));
    ...
    vec4 f76 = f74;
    f76.w = f75;
    float f77 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);

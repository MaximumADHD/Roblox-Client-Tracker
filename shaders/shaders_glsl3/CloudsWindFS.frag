...
void main()
{
    ...
    for (;;)
    {
        if (f34 < f17)
        {
            ...
            if (f46)
            {
                ...
                f66.z = f63.z;
                float f67 = f37 * f62;
                if (f67 <= 9.9999997473787516355514526367188e-06)
                {
                    f32 = f66;
                    f29 = 0.0;
                    f28 = f49;
    ...
    float f68 = 1.0 - f29;
    vec3 f69 = f32.xyz + (((((((CB0[10].xyz * clamp(1.0 - ((0.07999999821186065673828125 * f28) / f18), 0.0, 1.0)) * 2.0) * clamp(-f11, 0.0, 1.0)) * f15) * f68) * exp2(0.10999999940395355224609375 - (0.0350000001490116119384765625 * f28))) * CB5[3].xyz);
    vec4 f70 = f32;
    f70.x = f69.x;
    vec4 f71 = f70;
    f71.y = f69.y;
    ...
    float f73 = pow(max(exp2((CB0[13].z * 3.5714285373687744140625) * (f9 * f9)), 9.9999997473787516355514526367188e-05), 0.125);
    vec3 f74 = textureLod(PrefilteredEnvTexture, vec4(f2.x, f3, f2.z, 0.0).xyz, max(CB0[13].y, f73) * 5.0).xyz;
    ...
    f80.w = f68;
    vec4 f81 = f80;
    f81.w = f68 * max(f73, CB0[13].y);
    _entryPointOutput_color = f81;
}

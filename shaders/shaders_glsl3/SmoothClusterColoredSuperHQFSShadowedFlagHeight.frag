...
void main()
{
    ...
    float f100 = clamp(f66 * ((f66 > 0.0) ? mix(f75, mix(min((f84 <= f88) ? 1.0 : clamp(((f89 / (f89 + (f90 * f90))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f85 <= f91) ? 1.0 : clamp(((f92 / (f92 + (f93 * f93))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f75, clamp((length(f67 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)), CB0[47].x) : 0.0), 0.0, 1.0);
    float f101 = f96 * f96;

...
void main()
{
    ...
    vec3 f20 = (min((f17.xyz * (f17.w * 120.0)) + (CB0[27].xyz + ((CB0[28].xyz * (2.0 - CB0[9].w)) * f18.x)), vec3(CB0[16].w)) + (VARYING6 * ((1.0 - ((step(f19.x, VARYING5.z) * clamp(CB0[24].z + (CB0[24].w * abs(VARYING5.z - 0.5)), 0.0, 1.0)) * f19.y)) * f18.y))) * (f13.xyz * f13.xyz);
    vec4 f21 = vec4(0.0);
    f21.x = f20.x;
    vec4 f22 = f21;
    f22.y = f20.y;
    ...
    vec4 f24 = f23;
    f24.w = 1.0;
    float f25 = clamp(exp2((CB0[13].z * VARYING5.w) + CB0[13].x) - CB0[13].w, 0.0, 1.0);
    vec3 f26 = textureLod(PrefilteredEnvTexture, vec4(-VARYING7, 0.0).xyz, max(CB0[13].y, f25) * 5.0).xyz;

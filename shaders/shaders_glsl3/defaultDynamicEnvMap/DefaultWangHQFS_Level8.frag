...
void main()
{
    ...
    vec3 f29 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f14.w + CB2[2].w, 0.0, 1.0))) * f14.xyz) * (1.0 + (f20 * CB2[0].z))) * (f21.x * 2.0), VARYING2.w).xyz;
    ...
    float f70 = clamp(f28 * ((f28 > 0.0) ? mix(f39, mix(min((f48 <= f52) ? 1.0 : clamp(((f53 / (f53 + (f54 * f54))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f49 <= f55) ? 1.0 : clamp(((f56 / (f56 + (f57 * f57))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f39, clamp((length(f30 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)), CB0[47].x) : 0.0), 0.0, 1.0);
    float f71 = f60 * f60;

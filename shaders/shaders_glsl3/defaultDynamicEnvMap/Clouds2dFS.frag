...
void main()
{
    ...
    if (f22)
    {
        ...
        float f29 = f28.x;
        float f30 = clamp(f29 + ((((15.0 * (f29 - f19)) * CB5[4].x) + f20) * textureLod(DetailTexTexture, vec4(f26 * f18, 0.0, f27).xy, f27).x), 0.0, 1.0);
    ...
    f49.w = f48;
    vec4 f50 = f49;
    f50.w = f48 * max(f44, CB0[13].y);
    _entryPointOutput_color = f50;
}

...
void main()
{
    ...
    vec3 f21 = f15 + ((f4 * f11) * (0.1500000059604644775390625 + (0.009499999694526195526123046875 * (textureLod(NoiseTexTexture, vec4(fract(f19), 0.0).xyz, 0.0).x + (0.25 * textureLod(NoiseTexTexture, vec4(fract((f19 * mat3(vec3(0.0, 0.800000011920928955078125, 0.60000002384185791015625), vec3(-0.800000011920928955078125, 0.36000001430511474609375, -0.4799999892711639404296875), vec3(-0.60000002384185791015625, -0.4799999892711639404296875, 0.63999998569488525390625))) * 2.019999980926513671875), 0.0).xyz, 0.0).x)))));
    f21.y = 0.0;
    vec3 f22;
    vec4 f23;
    f23 = vec4(0.0);
    ...
    for (;;)
    {
        if (f32 < f16)
        {
            ...
            vec2 f37 = (textureLod(AdvectionTexTexture, vec4((f22.xz * 0.125) + vec2(0.0, CB5[0].w * (-17.5)), 0.0, f36).xy, f36).xy * 2.0) - vec2(1.0);
            ...
            vec2 f43 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(textureLod(DetailTex3DTexture, vec4((f39 * (0.5 * CB5[4].y)) + (f38 * f42.x), f36).xyz, f36).x, textureLod(DetailTex3DTexture, vec4((f39 * (2.0 * CB5[4].y)) + (f38 * f42.y), f36).xyz, f36).x) * 2.0) - vec2(1.0));
            float f44 = textureLod(CloudsDistanceFieldTexture, vec4(((f22 + (f38 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f36).xyz, f36).x + (f43.x + f43.y);
            bool f45 = f44 < CB5[2].x;
            if (f45)
            {
                float f46 = (f41 * 50.0) * (CB5[2].x - f44);
                vec3 f47 = -CB0[11].xyz;
                vec4 f48 = textureLod(BeerShadowMapTexture, vec4((f22 - (f47 * (f22.y * (1.0 / max(f47.y, 9.9999997473787516355514526367188e-05))))).xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f49 = f48.x;
    ...
    vec3 f67 = textureLod(PrefilteredEnvTexture, vec4(f4, 0.0).xyz, max(CB0[13].y, f66) * 5.0).xyz;
    bvec3 f68 = bvec3(!(CB0[13].w == 0.0));
    vec3 f69 = mix(vec3(f68.x ? CB0[14].xyz.x : f67.x, f68.y ? CB0[14].xyz.y : f67.y, f68.z ? CB0[14].xyz.z : f67.z), (f30.xyz * (1.6480000019073486328125 - ((0.00200000009499490261077880859375 * f17) * f17))).xyz, vec3(f66));
    float f70 = 1.0 - f28;
    vec4 f71 = vec4(f69.x, f69.y, f69.z, f30.w);
    f71.w = f70;
    vec4 f72 = f71;
    f72.w = f70 * max(f66, CB0[13].y);

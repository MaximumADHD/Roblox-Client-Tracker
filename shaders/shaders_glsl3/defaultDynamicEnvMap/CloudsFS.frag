...
void main()
{
    ...
    for (;;)
    {
        if (f34 < f14)
        {
            vec4 f37 = texture(CloudsDistanceFieldTexture, (f23 * vec3(0.03125, 1.0, 0.03125)).xzy);
    ...
    float f70 = exp2((CB0[13].z * 3.5714285373687744140625) * (f22 * f22));
    vec3 f71 = textureLod(PrefilteredEnvTexture, vec4(f1.x, f2, f1.z, 0.0).xyz, max(CB0[13].y, f70) * 5.0).xyz;
    bvec3 f72 = bvec3(CB0[13].w != 0.0);
    vec3 f73 = mix(vec3(f72.x ? CB0[14].xyz.x : f71.x, f72.y ? CB0[14].xyz.y : f71.y, f72.z ? CB0[14].xyz.z : f71.z), f32.xyz, vec3(f70));
    vec4 f74 = vec4(f73.x, f73.y, f73.z, f32.w);
    f74.w = 1.0 - f30;
    _entryPointOutput_color = f74;
}

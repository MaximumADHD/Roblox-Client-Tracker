...
void main()
{
    ...
    vec3 f15 = (f5 * f5).xyz;
    vec3 f16 = (((VARYING5.xyz * f14) + min((f10.xyz * (f10.w * 120.0)).xyz + (CB0[27].xyz + (CB0[28].xyz * f11.x)), vec3(CB0[16].w))) * f15) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f15, vec3(VARYING6.w * CB0[26].w))) * (VARYING5.w * f14));
    ...
    vec3 f21 = max(vec3(0.0), mix(vec3(f20.x ? CB0[14].xyz.x : f19.x, f20.y ? CB0[14].xyz.y : f19.y, f20.z ? CB0[14].xyz.z : f19.z), f17.xyz, vec3(f18)).xyz);
    _entryPointOutput = vec4(f21.x, f21.y, f21.z, f17.w);
}

...
void main()
{
    ...
    vec4 f33 = texture(PrecomputedBRDFTexture, vec2(f25, max(9.9999997473787516355514526367188e-05, dot(f16, f1))));
    ...
    float f62 = f54 - f60;
    float f63 = (f18 * CB0[14].w) * (((f18 * CB0[52].x) > 0.0) ? mix(min((f53 <= f57) ? 1.0 : clamp(((f58 / (f58 + (f59 * f59))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f54 <= f60) ? 1.0 : clamp(((f61 / (f61 + (f62 * f62))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f44, clamp((length(f34 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f44);
    vec3 f64;
    vec3 f65;
    if (f63 > 0.0)
    {
    ...
    vec3 f80 = f16 * f16;
    bvec3 f81 = lessThan(f16, vec3(0.0));

...
void main()
{
    ...
    float f10 = f8.x;
    vec4 f11 = texture(ShadowMapTexture, f3.xy);
    float f12 = f3.z;
    vec3 f13 = (f2 * f2).xyz;
    ...
    vec3 f23 = -CB0[11].xyz;
    float f24 = dot(f15, f23) * ((1.0 - ((step(f11.x, f12) * clamp(CB0[24].z + (CB0[24].w * abs(f12 - 0.5)), 0.0, 1.0)) * f11.y)) * f8.y);
    ...
    float f51 = f45.y;
    float f52 = f45.z;
    vec3 f53 = (mix(textureLod(PrefilteredEnvIndoorTexture, f19, f18).xyz * f9, textureLod(PrefilteredEnvTexture, f19, f18).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f17.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f10)) * f41) * f14;
    vec3 f54 = ((((((((f38 - (f33 * f37)) * CB0[10].xyz) * f26) + (CB0[12].xyz * (f36 * clamp(-f24, 0.0, 1.0)))) + (f42 * (((((((CB0[35].xyz * f47) + (CB0[37].xyz * f48)) + (CB0[39].xyz * f49)) + (CB0[36].xyz * f50)) + (CB0[38].xyz * f51)) + (CB0[40].xyz * f52)) + (((((((CB0[29].xyz * f47) + (CB0[31].xyz * f48)) + (CB0[33].xyz * f49)) + (CB0[30].xyz * f50)) + (CB0[32].xyz * f51)) + (CB0[34].xyz * f52)) * f10)))) + (CB0[27].xyz + (CB0[28].xyz * f10))) * f13) + (((f33 * (((f34 + (f34 * f34)) / (((f35 * f35) * ((f29 * 3.0) + 0.5)) * ((f28 * 0.75) + 0.25))) * f26)) * CB0[10].xyz) + f53)) + (f9 * mix(f13, f53 * (1.0 / (max(max(f53.x, f53.y), f53.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f42) * (f14 * (1.0 - f10))));

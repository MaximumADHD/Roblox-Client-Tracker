...
void main()
{
    ...
    if (f52 > 0.0)
    {
        vec3 f55 = normalize(f19 - CB0[16].xyz);
        ...
        f53 = ((f63 * (((f64 + (f64 * f64)) / max(((f65 * f65) * ((f59 * 3.0) + 0.5)) * ((f58 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f56)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f54 = f31;

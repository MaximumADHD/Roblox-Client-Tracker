...
void main()
{
    ...
    _entryPointOutput = mix(vec4(0.0), vec4(CB6[5].xyz, (textureLod(Texture1Texture, f22, 0.0).x < textureLod(Texture2Texture, f22, 0.0).x) ? CB1[1].y : 1.0), vec4(clamp(255.0 * max(abs(f10 - f8), abs(f14 - f12)), 0.0, 1.0)));
}

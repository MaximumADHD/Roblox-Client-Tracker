...
void main()
{
    ...
    vec3 f20 = (((VARYING5.xyz * f19) + min((f15.xyz * (f15.w * 120.0)) + (CB0[27].xyz + ((CB0[28].xyz * (2.0 - CB0[9].w)) * f16.x)), vec3(CB0[16].w))) * f10.xyz) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f10.xyz, vec3(VARYING6.w * CB0[26].w))) * (VARYING5.w * f19));
    vec4 f21 = vec4(0.0);
    f21.x = f20.x;
    vec4 f22 = f21;
    f22.y = f20.y;
    ...
    vec4 f24 = f23;
    f24.w = f5.w;
    float f25 = clamp(exp2((CB0[13].z * length(VARYING4.xyz)) + CB0[13].x) - CB0[13].w, 0.0, 1.0);
    vec3 f26 = textureLod(PrefilteredEnvTexture, vec4(-VARYING4.xyz, 0.0).xyz, max(CB0[13].y, f25) * 5.0).xyz;
    bvec3 f27 = bvec3(!(CB0[13].w == 0.0));
    vec3 f28 = mix(vec3(f27.x ? CB0[14].xyz.x : f26.x, f27.y ? CB0[14].xyz.y : f26.y, f27.z ? CB0[14].xyz.z : f26.z), f24.xyz, vec3(f25));
    vec4 f29 = f24;
    f29.x = f28.x;
    vec4 f30 = f29;
    f30.y = f28.y;

...
void main()
{
    ...
    vec3 f18 = (((VARYING5.xyz * f17) + min((f13.xyz * (f13.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f14.x)), vec3(CB0[21].w))) * f8.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f8.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f17));
    vec4 f19 = vec4(0.0);
    f19.x = f18.x;
    vec4 f20 = f19;
    f20.y = f18.y;
    ...
    vec4 f22 = f21;
    f22.w = f3.w;
    float f23 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f24 = textureLod(PrefilteredEnvTexture, vec4(-(VARYING4.xyz / vec3(f0)), 0.0).xyz, max(CB0[18].y, f23) * 5.0).xyz;

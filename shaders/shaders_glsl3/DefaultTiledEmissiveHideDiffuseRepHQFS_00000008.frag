...
void main()
{
    ...
    float f30 = 0.08900000154972076416015625 + (f16.y * 0.9110000133514404296875);
    vec3 f31 = -f1;
    vec3 f32 = reflect(f31, f21);
    float f33 = f16.x * f29;
    ...
    float f63 = f55 - f61;
    float f64 = (f23 * CB0[14].w) * (((f23 * CB0[52].x) > 0.0) ? mix(min((f54 <= f58) ? 1.0 : clamp(((f59 / (f59 + (f60 * f60))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f55 <= f61) ? 1.0 : clamp(((f62 / (f62 + (f63 * f63))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f45, clamp((length(f35 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f45);
    vec3 f65;
    vec3 f66;
    if (f64 > 0.0)
    {
        vec3 f67 = normalize(f1 - CB0[16].xyz);
        ...
        f65 = ((f75 * (((f76 + (f76 * f76)) / max(((f77 * f77) * ((f71 * 3.0) + 0.5)) * ((f70 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f68)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f66 = f43;
    ...
    vec4 f83 = texture(PrecomputedBRDFTexture, vec2(f30, max(9.9999997473787516355514526367188e-05, dot(f21, f1))));
    float f84 = f83.x;
    float f85 = f83.y;
    vec3 f86 = ((f34 * f84) + vec3(f85)) / vec3(f84 + f85);
    float f87 = 1.0 - f33;
    vec3 f88 = f21 * f21;
    bvec3 f89 = lessThan(f21, vec3(0.0));
    ...
    vec4 f102 = f101;
    f102.w = VARYING2.w;
    float f103 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);

...
void main()
{
    ...
    float f131 = f125.y;
    float f132 = f125.z;
    vec3 f133 = (((((((f82.xyz * (f82.w * 120.0)) * 1.0) + (((vec3(1.0) - (f112 * f74)) * CB0[15].xyz) * f105)) + ((vec3(1.0) - (f122 * f74)) * (((((((CB0[40].xyz * f127) + (CB0[42].xyz * f128)) + (CB0[44].xyz * f129)) + (CB0[41].xyz * f130)) + (CB0[43].xyz * f131)) + (CB0[45].xyz * f132)) + (((((((CB0[34].xyz * f127) + (CB0[36].xyz * f128)) + (CB0[38].xyz * f129)) + (CB0[35].xyz * f130)) + (CB0[37].xyz * f131)) + (CB0[39].xyz * f132)) * f84)))) + ((CB0[32].xyz + (CB0[33].xyz * (f84 * (2.0 - CB0[14].w)))) * 1.0)) + vec3((f25.z * 2.0) * f61)) * (f59 * f59)) + (((((f112 * (min((f113 + (f113 * f113)) / (((f114 * f114) * ((f108 * 3.0) + 0.5)) * ((f107 * 0.75) + 0.25)), 65504.0) * f105)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * (f61 * VARYING0.w)) + ((mix(f118, textureLod(PrefilteredEnvTexture, f116, f115).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f75.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f84)) * f122) * f74));
    vec4 f134 = vec4(0.0);
    f134.x = f133.x;
    vec4 f135 = f134;
    f135.y = f133.y;

...
void main()
{
    ...
    if (f79 > 0.0)
    {
        ...
        f80 = ((f90 * (((f91 + (f91 * f91)) / max(((f92 * f92) * ((f86 * 3.0) + 0.5)) * ((f85 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f83)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f81 = f58;
    ...
    float f109 = f103.y;
    float f110 = f103.z;
    vec3 f111 = ((((f81 + (((vec3(1.0) - (f100 * f48)) * (((((((CB0[40].xyz * f105) + (CB0[42].xyz * f106)) + (CB0[44].xyz * f107)) + (CB0[41].xyz * f108)) + (CB0[43].xyz * f109)) + (CB0[45].xyz * f110)) + (((((((CB0[34].xyz * f105) + (CB0[36].xyz * f106)) + (CB0[38].xyz * f107)) + (CB0[35].xyz * f108)) + (CB0[37].xyz * f109)) + (CB0[39].xyz * f110)) * f59))) * 1.0)) + ((CB0[32].xyz + (CB0[33].xyz * (f59 * (2.0 - CB0[14].w)))) * 1.0)) + vec3((f25.z * 2.0) * f35)) * (f33 * f33)) + ((f80 * (f35 * VARYING0.w)) + ((mix(f96, textureLod(PrefilteredEnvTexture, f94, f93).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f49.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f59)) * f100) * f48));
    vec4 f112 = vec4(0.0);
    f112.x = f111.x;
    vec4 f113 = f112;
    f113.y = f111.y;

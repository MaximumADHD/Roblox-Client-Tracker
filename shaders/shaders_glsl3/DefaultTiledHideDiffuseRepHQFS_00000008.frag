...
void main()
{
    ...
    float f62 = f54 - f60;
    float f63 = (f20 * CB0[14].w) * (((f20 * CB0[53].x) > 0.0) ? mix(min((f53 <= f57) ? 1.0 : clamp(((f58 / (f58 + (f59 * f59))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f54 <= f60) ? 1.0 : clamp(((f61 / (f61 + (f62 * f62))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f44, clamp((length(f34 - CB0[11].xyz) * CB0[52].y) - (CB0[52].x * CB0[52].y), 0.0, 1.0)) : f44);
    vec3 f64;
    vec3 f65;
    if (f63 > 0.0)
    {

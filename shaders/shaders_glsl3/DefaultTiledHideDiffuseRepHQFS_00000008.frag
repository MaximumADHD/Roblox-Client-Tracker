...
void main()
{
    ...
    vec4 f4 = mix(texture(DiffuseMapTexture, f3 * CB2[1].y), texture(DiffuseMapTexture, f3), vec4(clamp((f2.x * CB2[3].x) - (CB2[2].x * CB2[3].x), 0.0, 1.0)));
    ...
    float f62 = f54 - f60;
    float f63 = (f22 * CB0[14].w) * (((f22 * CB0[52].x) > 0.0) ? mix(min((f53 <= f57) ? 1.0 : clamp(((f58 / (f58 + (f59 * f59))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f54 <= f60) ? 1.0 : clamp(((f61 / (f61 + (f62 * f62))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f44, clamp((length(f34 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f44);
    vec3 f64;
    vec3 f65;
    if (f63 > 0.0)
    {
        vec3 f66 = normalize(f1 - CB0[16].xyz);
        ...
        f64 = ((f74 * (((f75 + (f75 * f75)) / max(((f76 * f76) * ((f70 * 3.0) + 0.5)) * ((f69 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f67)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f65 = f42;
    ...
    vec4 f82 = texture(PrecomputedBRDFTexture, vec2(f29, max(9.9999997473787516355514526367188e-05, dot(f20, f1))));
    float f83 = f82.x;
    float f84 = f82.y;
    vec3 f85 = ((f33 * f83) + vec3(f84)) / vec3(f83 + f84);
    float f86 = 1.0 - f32;
    vec3 f87 = f20 * f20;
    bvec3 f88 = lessThan(f20, vec3(0.0));

...
void main()
{
    ...
    vec3 f55 = ((((((((f38 - (f33 * f37)) * CB0[10].xyz) * f26) + (CB0[12].xyz * (f36 * clamp(-f24, 0.0, 1.0)))) + ((f42 * f53) * CB0[25].w)) + (CB0[27].xyz + (CB0[28].xyz * f10))) * f13) + (((f33 * (((f34 + (f34 * f34)) / (((f35 * f35) * ((f29 * 3.0) + 0.5)) * ((f28 * 0.75) + 0.25))) * f26)) * CB0[10].xyz) + f54)) + ((f8.xyz * (f8.w * 120.0)).xyz * mix(f13, f54 * (1.0 / (max(max(f53.x, f53.y), f53.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f42) * (f14 * (1.0 - f10))));
    float f56 = f2.w;

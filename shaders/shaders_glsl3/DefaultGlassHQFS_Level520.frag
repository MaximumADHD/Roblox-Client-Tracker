...
void main()
{
    ...
    vec3 f55 = normalize(f15 + f46);
    float f56 = clamp(f16 * ((f16 > 0.0) ? mix(f25, mix(min((f34 <= f38) ? 1.0 : clamp(((f39 / (f39 + (f40 * f40))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f35 <= f41) ? 1.0 : clamp(((f42 / (f42 + (f43 * f43))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f25, clamp((length(f17 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)), CB0[47].x) : 0.0), 0.0, 1.0);
    float f57 = f11 * f11;
    ...
    vec3 f66 = vec3(f24);
    vec3 f67 = mix(f23, f53, f66) * mix(vec3(1.0), f44, vec3(0.5));
    float f68 = f54.x;
    float f69 = f54.y;
    ...
    float f80 = f74.y;
    float f81 = f74.z;
    vec3 f82 = ((((((CB0[35].xyz * f76) + (CB0[37].xyz * f77)) + (CB0[39].xyz * f78)) + (CB0[36].xyz * f79)) + (CB0[38].xyz * f80)) + (CB0[40].xyz * f81)) + (((((((CB0[29].xyz * f76) + (CB0[31].xyz * f77)) + (CB0[33].xyz * f78)) + (CB0[30].xyz * f79)) + (CB0[32].xyz * f80)) + (CB0[34].xyz * f81)) * f24);
    vec3 f83 = (mix(textureLod(PrefilteredEnvIndoorTexture, f52, f51).xyz, f53, f66) * f70) * f48;
    float f84 = 1.0 - f49;
    vec4 f85 = mix(vec4(mix(((((((vec3(1.0) - (f63 * f48)) * CB0[10].xyz) * f56) + ((((vec3(1.0) - f71) * f82) * CB0[25].w) * f13)) + (CB0[27].xyz + (CB0[28].xyz * f24))) * f47) + ((f23 * mix(f47, f83 * (1.0 / (max(max(f82.x, f82.y), f82.z) + 0.00999999977648258209228515625)), f71 * (f48 * (1.0 - f24)))) * f13), f67, vec3(VARYING7.w)) * f13, f13), vec4(f67, 1.0), vec4(((f84 * f84) * 0.800000011920928955078125) * clamp(f12, 0.0, 1.0))) + vec4(((f63 * (((f64 + (f64 * f64)) / (((f65 * f65) * ((f59 * 3.0) + 0.5)) * ((f58 * 0.75) + 0.25))) * f56)) * CB0[10].xyz) + ((f83 * f24) * f13), 0.0);
    float f86 = clamp(exp2((CB0[13].z * f45) + CB0[13].x) - CB0[13].w, 0.0, 1.0);

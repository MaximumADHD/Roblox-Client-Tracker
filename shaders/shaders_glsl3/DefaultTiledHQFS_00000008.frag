...
void main()
{
    ...
    float f61 = f53 - f59;
    float f62 = (f21 * CB0[14].w) * (((f21 * CB0[52].x) > 0.0) ? mix(min((f52 <= f56) ? 1.0 : clamp(((f57 / (f57 + (f58 * f58))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f53 <= f59) ? 1.0 : clamp(((f60 / (f60 + (f61 * f61))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f43, clamp((length(f33 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f43);
    vec3 f63;
    vec3 f64;
    if (f62 > 0.0)
    {
        vec3 f65 = normalize(f1 + f20);
        float f66 = clamp(f62, 0.0, 1.0);
        float f67 = f28 * f28;
        float f68 = max(0.001000000047497451305389404296875, dot(f19, f65));
        float f69 = dot(f20, f65);
        float f70 = 1.0 - f69;
        float f71 = f70 * f70;
        ...
        f63 = ((f73 * (((f74 + (f74 * f74)) / max(((f75 * f75) * ((f69 * 3.0) + 0.5)) * ((f68 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f66)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f64 = f41;
    ...
    vec4 f81 = texture(PrecomputedBRDFTexture, vec2(f28, max(9.9999997473787516355514526367188e-05, dot(f19, f1))));
    float f82 = f81.x;
    float f83 = f81.y;
    vec3 f84 = ((f32 * f82) + vec3(f83)) / vec3(f82 + f83);
    float f85 = 1.0 - f31;
    vec3 f86 = f19 * f19;
    bvec3 f87 = lessThan(f19, vec3(0.0));

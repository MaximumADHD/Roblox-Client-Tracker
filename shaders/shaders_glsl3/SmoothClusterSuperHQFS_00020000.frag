...
void main()
{
    ...
    float f116 = f110.y;
    float f117 = f110.z;
    vec3 f118 = ((((((f79.xyz * (f79.w * 120.0)) + ((((f99 - (f94 * f98)) * CB0[15].xyz) * f87) + (CB0[17].xyz * (f97 * clamp(-f85, 0.0, 1.0))))) + (((f99 - (f107 * f98)) * (((((((CB0[40].xyz * f112) + (CB0[42].xyz * f113)) + (CB0[44].xyz * f114)) + (CB0[41].xyz * f115)) + (CB0[43].xyz * f116)) + (CB0[45].xyz * f117)) + (((((((CB0[34].xyz * f112) + (CB0[36].xyz * f113)) + (CB0[38].xyz * f114)) + (CB0[35].xyz * f115)) + (CB0[37].xyz * f116)) + (CB0[39].xyz * f117)) * f81))) * 1.0)) + ((CB0[32].xyz + (CB0[33].xyz * f81)) * 1.0)) + vec3((f26.z * 2.0) * f66)) * f65) + (((((f94 * (((f95 + (f95 * f95)) / max(((f96 * f96) * ((f90 * 3.0) + 0.5)) * ((f89 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f87)) * CB0[15].xyz) * 1.0) * (f66 * VARYING0.w)) + ((mix(f103, textureLod(PrefilteredEnvTexture, f101, f100).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f72.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f81)) * f107) * f71));
    vec4 f119 = vec4(0.0);
    f119.x = f118.x;
    vec4 f120 = f119;
    f120.y = f118.y;

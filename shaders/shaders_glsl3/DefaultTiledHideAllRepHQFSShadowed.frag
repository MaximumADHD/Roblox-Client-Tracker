...
void main()
{
    ...
    float f42 = length(VARYING4.xyz);
    vec3 f43 = VARYING4.xyz / vec3(f42);
    vec3 f44 = (f14 * f14).xyz;
    float f45 = CB0[26].w * f2;
    float f46 = max(f10.y, 0.04500000178813934326171875);
    vec3 f47 = reflect(-f43, f11);
    float f48 = f46 * 5.0;
    vec3 f49 = vec4(f47, f48).xyz;
    vec4 f50 = texture(PrecomputedBRDFTexture, vec2(f46, max(9.9999997473787516355514526367188e-05, dot(f11, f43))));
    float f51 = f10.x * f45;
    vec3 f52 = mix(vec3(0.039999999105930328369140625), f44, vec3(f51));
    vec3 f53 = normalize(f12 + f43);
    float f54 = clamp(f13, 0.0, 1.0);
    float f55 = f46 * f46;
    float f56 = max(0.001000000047497451305389404296875, dot(f11, f53));
    float f57 = dot(f12, f53);
    float f58 = 1.0 - f57;
    float f59 = f58 * f58;
    ...
    vec3 f71 = f11 * f11;
    bvec3 f72 = lessThan(f11, vec3(0.0));
    ...
    float f79 = f73.y;
    float f80 = f73.z;
    vec3 f81 = (mix(textureLod(PrefilteredEnvIndoorTexture, f49, f48).xyz * f20, textureLod(PrefilteredEnvTexture, f49, f48).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f47.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f21)) * f69) * f45;
    vec3 f82 = ((((((((f66 - (f61 * f65)) * CB0[10].xyz) * f54) * f41) + (f70 * (((((((CB0[35].xyz * f75) + (CB0[37].xyz * f76)) + (CB0[39].xyz * f77)) + (CB0[36].xyz * f78)) + (CB0[38].xyz * f79)) + (CB0[40].xyz * f80)) + (((((((CB0[29].xyz * f75) + (CB0[31].xyz * f76)) + (CB0[33].xyz * f77)) + (CB0[30].xyz * f78)) + (CB0[32].xyz * f79)) + (CB0[34].xyz * f80)) * f21)))) + (CB0[27].xyz + (CB0[28].xyz * f21))) * f44) + ((((f61 * (((f62 + (f62 * f62)) / (((f63 * f63) * ((f57 * 3.0) + 0.5)) * ((f56 * 0.75) + 0.25))) * f54)) * CB0[10].xyz) * f41) + f81)) + (f20 * mix(f44, f81 * (1.0 / (max(max(f81.x, f81.y), f81.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f70) * (f45 * (1.0 - f21))));
    vec4 f83 = vec4(f82.x, f82.y, f82.z, vec4(0.0).w);
    f83.w = VARYING2.w;
    vec2 f84 = min(VARYING0.wz, VARYING1.wz);
    float f85 = min(f84.x, f84.y) / f1;
    float f86 = clamp(exp2((CB0[13].z * f42) + CB0[13].x) - CB0[13].w, 0.0, 1.0);
    vec3 f87 = textureLod(PrefilteredEnvTexture, vec4(-VARYING4.xyz, 0.0).xyz, max(CB0[13].y, f86) * 5.0).xyz;
    bvec3 f88 = bvec3(CB0[13].w != 0.0);
    vec3 f89 = sqrt(clamp(mix(vec3(f88.x ? CB0[14].xyz.x : f87.x, f88.y ? CB0[14].xyz.y : f87.y, f88.z ? CB0[14].xyz.z : f87.z), (f83.xyz * clamp((clamp((f1 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f85)) + f85, 0.0, 1.0)).xyz, vec3(f86)).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))));
    vec4 f90 = vec4(f89.x, f89.y, f89.z, f83.w);
    f90.w = VARYING2.w;

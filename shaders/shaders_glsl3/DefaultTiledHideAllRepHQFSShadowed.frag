...
void main()
{
    ...
    vec3 f14 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f4.w + CB2[3].w, 0.0, 1.0))) * f4.xyz) * (1.0 + (f9 * CB2[1].x))) * (texture(StudsMapTexture, f0).x * 2.0), VARYING2.w).xyz;
    ...
    float f44 = max(CB0[26].w, CB2[0].w) * f2;
    ...
    float f64 = f49.x;
    float f65 = f49.y;
    vec3 f66 = ((f51 * f64) + vec3(f65)) / vec3(f64 + f65);
    vec3 f67 = (vec3(1.0) - (f66 * f44)) * f63;
    vec3 f68 = f11 * f11;
    bvec3 f69 = lessThan(f11, vec3(0.0));
    ...
    float f76 = f70.y;
    float f77 = f70.z;
    vec3 f78 = (mix(textureLod(PrefilteredEnvIndoorTexture, f48, f47).xyz * f20, textureLod(PrefilteredEnvTexture, f48, f47).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f46.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f21)) * f66) * f44;
    vec3 f79 = (((((((((vec3(1.0) - (f60 * f44)) * f63) * CB0[10].xyz) * f53) * f41) + (f67 * (((((((CB0[35].xyz * f72) + (CB0[37].xyz * f73)) + (CB0[39].xyz * f74)) + (CB0[36].xyz * f75)) + (CB0[38].xyz * f76)) + (CB0[40].xyz * f77)) + (((((((CB0[29].xyz * f72) + (CB0[31].xyz * f73)) + (CB0[33].xyz * f74)) + (CB0[30].xyz * f75)) + (CB0[32].xyz * f76)) + (CB0[34].xyz * f77)) * f21)))) + (CB0[27].xyz + (CB0[28].xyz * f21))) * f43) + (((((f60 * ((f61 + (f61 * f61)) / (((f62 * f62) * ((f56 * 3.0) + 0.5)) * ((f55 * 0.75) + 0.25)))) * CB0[10].xyz) * f53) * f41) + f78)) + (f20 * mix(f43, f78 * (1.0 / (max(max(f78.x, f78.y), f78.z) + 0.00999999977648258209228515625)), ((vec3(1.0) - f67) * f44) * (1.0 - f21)));
    vec4 f80 = vec4(f79.x, f79.y, f79.z, vec4(0.0).w);
    f80.w = VARYING2.w;
    vec2 f81 = min(VARYING0.wz, VARYING1.wz);
    float f82 = min(f81.x, f81.y) / f1;
    vec3 f83 = mix(CB0[14].xyz, (sqrt(clamp((f80.xyz * clamp((clamp((f1 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f82)) + f82, 0.0, 1.0)).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    _entryPointOutput = vec4(f83.x, f83.y, f83.z, f80.w);
}

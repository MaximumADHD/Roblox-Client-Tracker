...
void main()
{
    ...
    vec4 f30 = ((texture(AlbedoMapTexture, f0).yxzw * f6.x) + (texture(AlbedoMapTexture, f2).yxzw * f6.y)) + (texture(AlbedoMapTexture, f4).yxzw * f6.z);
    vec2 f31 = f30.yz - vec2(0.5);
    float f32 = f30.x;
    float f33 = f32 - f31.y;
    vec3 f34 = vec4(vec3(f33, f32, f33) + (vec3(f31.xyx) * vec3(1.0, 1.0, -1.0)), 0.0).xyz;
    _entryPointOutput_albedo = vec4(f34 * f34, 1.0);

...
void main()
{
    ...
    _entryPointOutput = vec4(sqrt(clamp(mix(f9 * ((vec3(max(f10 - CB1[9].y, 0.0)) / vec3(f10 + 0.001000000047497451305389404296875)) * CB1[9].x), clamp((f11 * f11) * 4.0, min(f3, f7), max(f3, f7)), vec3(CB1[1].z)) * 0.25, vec3(0.0), vec3(1.0))), 1.0);
}

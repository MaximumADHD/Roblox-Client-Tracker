...
void main()
{
    ...
    vec3 f30 = -CB0[16].xyz;
    vec3 f31 = normalize(f16 - CB0[16].xyz);
    float f32 = f29 * f29;
    ...
    vec3 f40 = mix(((min(f5 + (CB0[32].xyz + ((CB0[33].xyz * 1.0) * f6)), vec3(CB0[21].w)) + (((((((CB0[40].xyz * f21) + (CB0[42].xyz * f22)) + (CB0[44].xyz * f23)) + (CB0[41].xyz * f24)) + (CB0[43].xyz * f25)) + (CB0[45].xyz * f26)) + (((((((CB0[34].xyz * f21) + (CB0[36].xyz * f22)) + (CB0[38].xyz * f23)) + (CB0[35].xyz * f24)) + (CB0[37].xyz * f25)) + (CB0[39].xyz * f26)) * f6))) + (CB0[15].xyz * f7)) * CB3[1].xyz, ((textureLod(PrefilteredEnvTexture, vec4(f28, 0.0).xyz, 0.0).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f28.y * 1.58823525905609130859375, 0.0, 1.0)))) * f6) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f15, f16))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (((((vec3(f37) + (vec3(0.0199999995529651641845703125) * (1.0 - f37))) * (min((f38 + (f38 * f38)) / (((f39 * f39) * ((f34 * 3.0) + 0.5)) * ((f33 * 0.75) + 0.25)), 65504.0) * clamp(dot(f14, f30) * f7, 0.0, 1.0))) * CB0[15].xyz) * 1.0) * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f41 = vec4(0.0);
    f41.x = f40.x;
    vec4 f42 = f41;
    f42.y = f40.y;
    ...
    vec4 f44 = f43;
    f44.w = 1.0;
    vec3 f45 = textureLod(PrefilteredEnvTexture, vec4(f27, 0.0).xyz, max(CB0[18].y, VARYING6.w) * 5.0).xyz;

...
void main()
{
    ...
    float f19 = f18.y;
    vec3 f20 = ((((CB3[1].xyz * (1.0 - texture(SpecularMapTexture, VARYING0).w)) + (VARYING5.xyz * f19)) + min((f17.xyz * (f17.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f18.x)), vec3(CB0[21].w))) * f12.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f12.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f19));
    vec4 f21 = vec4(0.0);
    f21.x = f20.x;
    vec4 f22 = f21;
    f22.y = f20.y;

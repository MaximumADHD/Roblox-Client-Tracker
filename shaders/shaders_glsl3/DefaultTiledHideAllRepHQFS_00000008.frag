...
void main()
{
    ...
    float f33 = 0.08900000154972076416015625 + (f19.y * 0.9110000133514404296875);
    vec3 f34 = -f1;
    vec3 f35 = reflect(f34, f24);
    float f36 = f19.x * f32;
    vec3 f37 = mix(vec3(0.039999999105930328369140625), f31.xyz, vec3(f36));
    ...
    float f57 = exp(CB0[51].z * f56);
    float f58 = -exp((-CB0[51].w) * f56);
    vec2 f59 = (f55 * CB0[52].y) * vec2(f57, f58);
    vec2 f60 = f59 * f59;
    float f61 = f53.x;
    ...
    float f66 = f58 - f64;
    float f67 = (f26 * CB0[14].w) * (((f26 * CB0[52].x) > 0.0) ? mix(min((f57 <= f61) ? 1.0 : clamp(((f62 / (f62 + (f63 * f63))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f58 <= f64) ? 1.0 : clamp(((f65 / (f65 + (f66 * f66))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f48, clamp((length(f38 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f48);
    vec3 f68;
    vec3 f69;
    if (f67 > 0.0)
    {
        vec3 f70 = normalize(f1 + f25);
        float f71 = clamp(f67, 0.0, 1.0);
        float f72 = f33 * f33;
        float f73 = max(0.001000000047497451305389404296875, dot(f24, f70));
        float f74 = dot(f25, f70);
        float f75 = 1.0 - f74;
        float f76 = f75 * f75;
        ...
        f68 = ((f78 * (((f79 + (f79 * f79)) / max(((f80 * f80) * ((f74 * 3.0) + 0.5)) * ((f73 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f71)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f69 = f46;
    ...
    vec4 f86 = texture(PrecomputedBRDFTexture, vec2(f33, max(9.9999997473787516355514526367188e-05, dot(f24, f1))));
    float f87 = f86.x;
    float f88 = f86.y;
    vec3 f89 = ((f37 * f87) + vec3(f88)) / vec3(f87 + f88);
    float f90 = 1.0 - f36;
    vec3 f91 = f24 * f24;
    bvec3 f92 = lessThan(f24, vec3(0.0));

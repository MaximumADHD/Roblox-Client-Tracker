...
void main()
{
    ...
    vec3 f22 = f17 + ((f3 * f11) * (0.1500000059604644775390625 + (0.949999988079071044921875 * textureLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)));
    f22.y = 0.0;
    bool f23;
    vec3 f24;
    vec4 f25;
    ...
    for (;;)
    {
        if (f35 < f18)
        {
            ...
            vec2 f40 = (textureLod(AdvectionTexTexture, vec4((f24.xz * 0.125) + vec2(0.0, CB5[0].w * (-17.5)), 0.0, f39).xy, f39).xy * 2.0) - vec2(1.0);
            vec3 f41 = vec3(f40.x, 0.0, f40.y);
            float f42 = CB5[2].y * clamp(1.0 + (0.001000000047497451305389404296875 * (6000.0 - CB0[11].y)), 0.0, 1.0);
            ...
            vec2 f44 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(textureLod(DetailTex3DTexture, vec4((f24 * (0.5 * CB5[4].y)) + (f41 * f43.x), f39).xyz, f39).x, textureLod(DetailTex3DTexture, vec4((f24 * (2.0 * CB5[4].y)) + (f41 * f43.y), f39).xyz, f39).x) * 2.0) - vec2(1.0));
            float f45 = textureLod(CloudsDistanceFieldTexture, vec4(((f24 + (f41 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f39).xyz, f39).x + (f44.x + f44.y);
            float f46 = (f42 * 50.0) * (CB5[2].x - f45);
            bool f47 = f45 < CB5[2].x;
            if (f47)
            {
                vec4 f48 = textureLod(BeerShadowMapTexture, vec4(f24.xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f49 = min(f48.z * 0.5, (f48.y + (0.3499999940395355224609375 * textureLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)) * max(0.0, (1.0 - ((float(f35) / f19) + (f45 / f19))) - f48.x));
                float f50 = f37 + f46;
                vec3 f51 = f3 * f3;
                bvec3 f52 = lessThan(f3, vec3(0.0));
                ...
                float f59 = f53.y;
                float f60 = f53.z;
                float f61 = (1.44269502162933349609375 / f19) * f46;
                vec3 f62 = ((max(mix(vec3(0.1500000059604644775390625 + (0.300000011920928955078125 * f24.y)), mix(CB0[31].xyz, CB0[30].xyz, vec3(f24.y)), vec3(clamp(exp2(CB0[16].y), 0.0, 1.0))) * (((((((CB0[40].xyz * f55) + (CB0[42].xyz * f56)) + (CB0[44].xyz * f57)) + (CB0[41].xyz * f58)) + (CB0[43].xyz * f59)) + (CB0[45].xyz * f60)) + (((((((CB0[34].xyz * f55) + (CB0[36].xyz * f56)) + (CB0[38].xyz * f57)) + (CB0[35].xyz * f58)) + (CB0[37].xyz * f59)) + (CB0[39].xyz * f60)) * 1.0)), CB0[33].xyz + CB0[32].xyz) + ((CB0[15].xyz * ((4.0 * (((((0.2899999916553497314453125 * exp2(f49 * (-23.701419830322265625))) + ((0.1689999997615814208984375 * mix(0.07999999821186065673828125, 1.0, smoothstep(0.0, 1.5, abs(f12)))) * exp2(f49 * (-659.51776123046875)))) + ((((1.0 - f42) * 320.0) * clamp(f12 * f16, 0.0, 1.0)) * exp2(f49 * (-10.0)))) * (f15 * 0.0292499996721744537353515625)) + (0.092000000178813934326171875 * exp2(f48.w * (-2.3250000476837158203125))))) + (0.0425000004470348358154296875 + (((f46 / f19) * 0.13500000536441802978515625) * f24.y)))) * 2.099999904632568359375)) * CB5[3].xyz) * f61;
                float f63 = exp2(-f61);
                vec3 f64 = f25.xyz + (((f62 - (f62 * f63)) * f38) / vec3((f61 < 9.9999997473787516355514526367188e-05) ? 9.9999997473787516355514526367188e-05 : f61));
                vec4 f65 = f25;
                f65.x = f64.x;
                vec4 f66 = f65;
                f66.y = f64.y;
    ...
    float f69 = 1.0 - f30;
    vec3 f70 = f33.xyz + (((((((CB0[15].xyz * clamp(1.0 - ((0.07999999821186065673828125 * f29) / f19), 0.0, 1.0)) * 2.0) * clamp(-f12, 0.0, 1.0)) * f16) * f69) * exp2(0.10999999940395355224609375 - (0.0350000001490116119384765625 * f29))) * CB5[3].xyz);
    vec4 f71 = f33;
    f71.x = f70.x;
    vec4 f72 = f71;
    f72.y = f70.y;
    ...
    float f74 = pow(max(exp2((CB0[18].z * 3.5714285373687744140625) * (f10 * f10)), 9.9999997473787516355514526367188e-05), 0.125);
    vec3 f75 = textureLod(PrefilteredEnvTexture, vec4(f3.x, f4, f3.z, 0.0).xyz, max(CB0[18].y, f74) * 5.0).xyz;

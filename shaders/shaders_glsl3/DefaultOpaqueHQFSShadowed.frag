...
void main()
{
    ...
    vec3 f59 = f0 * f0;
    bvec3 f60 = lessThan(f0, vec3(0.0));
    ...
    float f67 = f61.y;
    float f68 = f61.z;
    vec3 f69 = (mix(textureLod(PrefilteredEnvIndoorTexture, f37, f36).xyz * f9, textureLod(PrefilteredEnvTexture, f37, f36).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f35.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f10)) * f57) * f33;
    vec3 f70 = ((((((((f54 - (f49 * f53)) * CB0[10].xyz) * f42) * f30) + (f58 * (((((((CB0[35].xyz * f63) + (CB0[37].xyz * f64)) + (CB0[39].xyz * f65)) + (CB0[36].xyz * f66)) + (CB0[38].xyz * f67)) + (CB0[40].xyz * f68)) + (((((((CB0[29].xyz * f63) + (CB0[31].xyz * f64)) + (CB0[33].xyz * f65)) + (CB0[30].xyz * f66)) + (CB0[32].xyz * f67)) + (CB0[34].xyz * f68)) * f10)))) + (CB0[27].xyz + (CB0[28].xyz * f10))) * f32) + ((((f49 * (((f50 + (f50 * f50)) / (((f51 * f51) * ((f45 * 3.0) + 0.5)) * ((f44 * 0.75) + 0.25))) * f42)) * CB0[10].xyz) * f30) + f69)) + (f9 * mix(f32, f69 * (1.0 / (max(max(f69.x, f69.y), f69.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f58) * (f33 * (1.0 - f10))));
    vec4 f71 = vec4(f70.x, f70.y, f70.z, vec4(0.0).w);
    f71.w = 1.0;
    vec3 f72 = mix(CB0[14].xyz, (sqrt(clamp(f71.xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    vec4 f73 = vec4(f72.x, f72.y, f72.z, f71.w);
    f73.w = 1.0;

...
void main()
{
    ...
    if (f78 > 0.0)
    {
        vec3 f81 = normalize(f43 - CB0[16].xyz);
        ...
        f79 = ((f89 * (((f90 + (f90 * f90)) / max(((f91 * f91) * ((f85 * 3.0) + 0.5)) * ((f84 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f82)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f80 = f57;

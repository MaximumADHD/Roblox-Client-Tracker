...
void main()
{
    ...
    float f47 = f41.y;
    float f48 = f41.z;
    vec3 f49 = (mix(textureLod(PrefilteredEnvIndoorTexture, f17, f16).xyz * f6, textureLod(PrefilteredEnvTexture, f17, f16).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f15.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f7)) * f37) * f12;
    ...
    _entryPointOutput = vec4(f52.x, f52.y, f52.z, f51.w);
}

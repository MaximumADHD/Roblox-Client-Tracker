...
void main()
{
    ...
    vec3 f60 = (((((((((vec3(1.0) - (f38 * f26)) * CB0[10].xyz) * f31) + (CB0[12].xyz * clamp(-f29, 0.0, 1.0))) + (((vec3(1.0) - f47) * f58) * CB0[25].w)) + (CB0[27].xyz + (CB0[28].xyz * f20))) + vec3((f2.z * 2.0) * f12)) * f24) + (((((f38 * (((f39 + (f39 * f39)) / (((f40 * f40) * ((f34 * 3.0) + 0.5)) * ((f33 * 0.75) + 0.25))) * f31)) * CB0[10].xyz) * f12) * VARYING0.w) + f59)) + ((f18.xyz * (f18.w * 120.0)).xyz * mix(f24, f59 * (1.0 / (max(max(f58.x, f58.y), f58.z) + 0.00999999977648258209228515625)), f47 * (f26 * (1.0 - f20))));

...
void main()
{
    ...
    float f56 = f48 - f54;
    vec3 f57 = normalize(f1 - CB0[16].xyz);
    float f58 = clamp((f19 * CB0[14].w) * (((f19 * CB0[52].x) > 0.0) ? mix(min((f47 <= f51) ? 1.0 : clamp(((f52 / (f52 + (f53 * f53))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f48 <= f54) ? 1.0 : clamp(((f55 / (f55 + (f56 * f56))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f38, clamp((length(f29 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f38), 0.0, 1.0);
    float f59 = f26 * f26;
    ...
    vec4 f72 = texture(PrecomputedBRDFTexture, vec2(f26, max(9.9999997473787516355514526367188e-05, dot(f17, f1))));
    float f73 = f72.x;
    float f74 = f72.y;
    ...
    float f84 = f78.y;
    float f85 = f78.z;
    vec3 f86 = ((((((f35.xyz * (f35.w * 120.0)) * 1.0) + (((vec3(1.0) - (f65 * f25)) * CB0[15].xyz) * f58)) + ((vec3(1.0) - (f75 * f25)) * (((((((CB0[40].xyz * f80) + (CB0[42].xyz * f81)) + (CB0[44].xyz * f82)) + (CB0[41].xyz * f83)) + (CB0[43].xyz * f84)) + (CB0[45].xyz * f85)) + (((((((CB0[34].xyz * f80) + (CB0[36].xyz * f81)) + (CB0[38].xyz * f82)) + (CB0[35].xyz * f83)) + (CB0[37].xyz * f84)) + (CB0[39].xyz * f85)) * f37)))) + ((CB0[32].xyz + (CB0[33].xyz * (f37 * (2.0 - CB0[14].w)))) * 1.0)) * f24.xyz) + ((((f65 * (((f66 + (f66 * f66)) / max(((f67 * f67) * ((f61 * 3.0) + 0.5)) * ((f60 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f58)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) + ((mix(f71, textureLod(PrefilteredEnvTexture, f69, f68).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f28.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f37)) * f75) * f25));
    vec4 f87 = vec4(0.0);
    f87.x = f86.x;
    vec4 f88 = f87;
    f88.y = f86.y;
    ...
    vec4 f90 = f89;
    f90.w = VARYING2.w;
    float f91 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f92 = textureLod(PrefilteredEnvTexture, vec4(f27, 0.0).xyz, max(CB0[18].y, f91) * 5.0).xyz;
    ...
    vec4 f97 = f96;
    f97.z = f94.z;
    vec3 f98 = sqrt(clamp(f97.xyz * CB0[20].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))));
    vec4 f99 = f97;
    f99.x = f98.x;
    vec4 f100 = f99;
    f100.y = f98.y;

...
void main()
{
    ...
    vec4 f11 = textureLod(distLastFrame, f10, 0.0);
    float f12 = f11.y * 7.0;
    ...
    vec4 f16 = textureLod(oldCloudColor, f10, 0.0);
    vec4 f17 = vec4(1.0, 1.0, -1.0, 0.0) * CB1[0].zwzw;
    vec2 f18 = f17.xy;
    vec2 f19 = f17.wy;
    vec2 f20 = f17.zy;
    vec4 f21 = (((((((((textureLod(cloudColor, VARYING0, 0.0) * 4.0) + textureLod(cloudColor, VARYING0 - f18, 0.0)) + (textureLod(cloudColor, VARYING0 - f19, 0.0) * 2.0)) + textureLod(cloudColor, VARYING0 - f20, 0.0)) + (textureLod(cloudColor, VARYING0 + f17.zw, 0.0) * 2.0)) + (textureLod(cloudColor, VARYING0 + f17.xw, 0.0) * 2.0)) + textureLod(cloudColor, VARYING0 + f20, 0.0)) + (textureLod(cloudColor, VARYING0 + f19, 0.0) * 2.0)) + textureLod(cloudColor, VARYING0 + f18, 0.0)) * vec4(0.0625);

...
void main()
{
    ...
    vec3 f66 = ((((((((f49 - (f44 * f48)) * CB0[10].xyz) * f37) + (CB0[12].xyz * (f47 * clamp(-f35, 0.0, 1.0)))) + ((f53 * f64) * CB0[25].w)) + ((CB0[27].xyz + (CB0[28].xyz * f22)) * 1.0)) * f25) + (((f44 * (((f45 + (f45 * f45)) / (((f46 * f46) * ((f40 * 3.0) + 0.5)) * ((f39 * 0.75) + 0.25))) * f37)) * CB0[10].xyz) + (f65 * f22))) + (((f20.xyz * (f20.w * 120.0)).xyz * mix(f25, f65 * (1.0 / (max(max(f64.x, f64.y), f64.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f53) * (f26 * (1.0 - f22)))) * 1.0);
    float f67 = f8.w;

...
void main()
{
    ...
    vec4 f13 = textureLod(CloudsDistanceFieldTexture, vec4(f11 * vec2(0.03125), 0.0, f12).xy, f12);
    float f14 = f13.x;
    ...
    float f18 = f14 + ((((15.0 * (f14 - f16)) * CB5[4].x) + f17) * textureLod(DetailTexTexture, vec4(f11 * f15, 0.0, f12).xy, f12).x);
    ...
    if (f19)
    {
        ...
        vec4 f25 = textureLod(CloudsDistanceFieldTexture, vec4(f23 * vec2(0.03125), 0.0, f24).xy, f24);
        float f26 = f25.x;
        float f27 = clamp(f26 + ((((15.0 * (f26 - f16)) * CB5[4].x) + f17) * textureLod(DetailTexTexture, vec4(f23 * f15, 0.0, f24).xy, f24).x), 0.0, 1.0);
    ...
    vec3 f42 = textureLod(PrefilteredEnvTexture, vec4(f1, 0.0).xyz, max(CB0[13].y, f41) * 5.0).xyz;
    bvec3 f43 = bvec3(!(CB0[13].w == 0.0));
    vec3 f44 = mix(vec3(f43.x ? CB0[14].xyz.x : f42.x, f43.y ? CB0[14].xyz.y : f42.y, f43.z ? CB0[14].xyz.z : f42.z), f21.xyz, vec3(f41));
    float f45 = 1.0 - f20;
    vec4 f46 = vec4(f44.x, f44.y, f44.z, f21.w);
    f46.w = f45;
    vec4 f47 = f46;
    f47.w = f45 * max(f41, CB0[13].y);
    _entryPointOutput_color = f47;
}

...
void main()
{
    ...
    if (f58 > 0.0)
    {
        vec3 f61 = normalize(f25 - CB0[16].xyz);
        ...
        f59 = ((f69 * (((f70 + (f70 * f70)) / max(((f71 * f71) * ((f65 * 3.0) + 0.5)) * ((f64 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f62)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f60 = f37;

...
void main()
{
    ...
    vec4 f1 = texture(texTexture, VARYING4);
    ...
    do
    {
        if (0.0 == CB1[3].x)
        {
            f3 = clamp(VARYING3.w * abs((texture(depthTexTexture, VARYING3.xy).x * 500.0) - VARYING3.z), 0.0, 1.0);
            break;
        }
        else
        {
            f3 = clamp(VARYING3.w * abs((texture(depthTexTexture, gl_FragCoord.xy * CB0[45].xy).x * 500.0) - gl_FragCoord.w), 0.0, 1.0);
            break;
        }
    } while(false);
    ...
    vec4 f11 = f10;
    f11.z = f8.z;
    vec4 f12 = texture(LightingAtlasTexture, VARYING2);

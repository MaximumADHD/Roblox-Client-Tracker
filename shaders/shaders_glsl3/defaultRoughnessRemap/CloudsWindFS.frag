...
void main()
{
    ...
    for (;;)
    {
        if (f34 < f17)
        {
            ...
            float f41 = CB5[2].y * clamp(1.0 + (0.001000000047497451305389404296875 * (6000.0 - CB0[7].y)), 0.0, 1.0);
            ...
            vec2 f43 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(textureLod(DetailTex3DTexture, vec4((f23 * (0.5 * CB5[4].y)) + (f40 * f42.x), f38).xyz, f38).x, textureLod(DetailTex3DTexture, vec4((f23 * (2.0 * CB5[4].y)) + (f40 * f42.y), f38).xyz, f38).x) * 2.0) - vec2(1.0));
            float f44 = textureLod(CloudsDistanceFieldTexture, vec4(((f23 + (f40 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f38).xyz, f38).x + (f43.x + f43.y);
            float f45 = (f41 * 50.0) * (CB5[2].x - f44);
            bool f46 = f44 < CB5[2].x;
            if (f46)
            {
                vec4 f47 = textureLod(BeerShadowMapTexture, vec4(f23.xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f48 = min(f47.z * 0.5, (f47.y + (0.3499999940395355224609375 * textureLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)) * max(0.0, (1.0 - ((float(f34) / f18) + (f44 / f18))) - f47.x));
                float f49 = f36 + f45;
                vec3 f50 = f2 * f2;
                bvec3 f51 = lessThan(f2, vec3(0.0));
                ...
                float f58 = f52.y;
                float f59 = f52.z;
                float f60 = (1.44269502162933349609375 / f18) * f45;
                vec3 f61 = ((max(mix(vec3(0.1500000059604644775390625 + (0.300000011920928955078125 * f23.y)), mix(CB0[26].xyz, CB0[25].xyz, vec3(f23.y)), vec3(clamp(exp2(CB0[11].y), 0.0, 1.0))) * (((((((CB0[35].xyz * f54) + (CB0[37].xyz * f55)) + (CB0[39].xyz * f56)) + (CB0[36].xyz * f57)) + (CB0[38].xyz * f58)) + (CB0[40].xyz * f59)) + (((((((CB0[29].xyz * f54) + (CB0[31].xyz * f55)) + (CB0[33].xyz * f56)) + (CB0[30].xyz * f57)) + (CB0[32].xyz * f58)) + (CB0[34].xyz * f59)) * 1.0)), CB0[28].xyz + CB0[27].xyz) + ((CB0[10].xyz * ((4.0 * (((((0.2899999916553497314453125 * exp2(f48 * (-23.701419830322265625))) + ((0.1689999997615814208984375 * mix(0.07999999821186065673828125, 1.0, smoothstep(0.0, 1.5, abs(f11)))) * exp2(f48 * (-659.51776123046875)))) + ((((1.0 - f41) * 320.0) * clamp(f11 * f15, 0.0, 1.0)) * exp2(f48 * (-10.0)))) * (f14 * 0.0292499996721744537353515625)) + (0.092000000178813934326171875 * exp2(f47.w * (-2.3250000476837158203125))))) + (0.0425000004470348358154296875 + (((f45 / f18) * 0.13500000536441802978515625) * f23.y)))) * 2.099999904632568359375)) * CB5[3].xyz) * f60;
                float f62 = exp2(-f60);
                vec3 f63 = f24.xyz + (((f61 - (f61 * f62)) * f37) / vec3((f60 < 9.9999997473787516355514526367188e-05) ? 9.9999997473787516355514526367188e-05 : f60));
                vec4 f64 = vec4(f63.x, f63.y, f63.z, f24.w);
                float f65 = f37 * f62;
    ...
    float f67 = pow(max(exp2((CB0[13].z * 3.5714285373687744140625) * (f9 * f9)), 9.9999997473787516355514526367188e-05), 0.125);
    vec3 f68 = textureLod(PrefilteredEnvTexture, vec4(f2.x, f3, f2.z, 0.0).xyz, max(CB0[13].y, f67) * 5.0).xyz;
    bvec3 f69 = bvec3(!(CB0[13].w == 0.0));
    vec3 f70 = mix(vec3(f69.x ? CB0[14].xyz.x : f68.x, f69.y ? CB0[14].xyz.y : f68.y, f69.z ? CB0[14].xyz.z : f68.z), (f32.xyz + (((((((CB0[10].xyz * clamp(1.0 - ((0.07999999821186065673828125 * f28) / f18), 0.0, 1.0)) * 2.0) * clamp(-f11, 0.0, 1.0)) * f15) * f66) * exp2(0.10999999940395355224609375 - (0.0350000001490116119384765625 * f28))) * CB5[3].xyz)).xyz, vec3(f67));
    vec4 f71 = vec4(f70.x, f70.y, f70.z, f32.w);
    f71.w = f66;
    vec4 f72 = f71;
    f72.w = f66 * max(f67, CB0[13].y);
    _entryPointOutput_color = f72;
}

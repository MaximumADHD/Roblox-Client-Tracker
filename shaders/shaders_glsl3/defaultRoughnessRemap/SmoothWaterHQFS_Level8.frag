...
void main()
{
    ...
    vec3 f51 = normalize(f37 - CB0[11].xyz);
    float f52 = f50 * f50;
    ...
    vec4 f64 = f63;
    f64.w = 1.0;
    vec3 f65 = textureLod(PrefilteredEnvTexture, vec4(f48, 0.0).xyz, max(CB0[13].y, VARYING6.w) * 5.0).xyz;
    bvec3 f66 = bvec3(!(CB0[13].w == 0.0));
    vec3 f67 = mix(vec3(f66.x ? CB0[14].xyz.x : f65.x, f66.y ? CB0[14].xyz.y : f65.y, f66.z ? CB0[14].xyz.z : f65.z), f64.xyz, vec3(VARYING6.w));
    vec4 f68 = f64;
    f68.x = f67.x;
    vec4 f69 = f68;
    f69.y = f67.y;
    vec4 f70 = f69;
    f70.z = f67.z;
    vec3 f71 = sqrt(clamp(f70.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    vec4 f72 = f70;
    f72.x = f71.x;
    vec4 f73 = f72;
    f73.y = f71.y;

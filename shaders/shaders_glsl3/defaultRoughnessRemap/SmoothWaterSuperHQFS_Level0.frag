...
void main()
{
    ...
    float f74 = 0.08900000154972076416015625 + (f13 * 0.9110000133514404296875);
    vec3 f75 = -CB0[11].xyz;
    vec3 f76 = normalize(f75 + f18);
    float f77 = f74 * f74;
    float f78 = max(0.001000000047497451305389404296875, dot(f16, f76));
    float f79 = dot(f75, f76);
    float f80 = 1.0 - f79;
    float f81 = f80 * f80;
    float f82 = (f81 * f81) * f80;
    float f83 = f77 * f77;
    float f84 = (((f78 * f83) - f78) * f78) + 1.0;
    vec3 f85 = mix(mix(f39.xyz, ((min(f5 + (CB0[27].xyz + ((CB0[28].xyz * (2.0 - CB0[9].w)) * f6)), vec3(CB0[16].w)) + (((((((CB0[35].xyz * f23) + (CB0[37].xyz * f24)) + (CB0[39].xyz * f25)) + (CB0[36].xyz * f26)) + (CB0[38].xyz * f27)) + (CB0[40].xyz * f28)) + (((((((CB0[29].xyz * f23) + (CB0[31].xyz * f24)) + (CB0[33].xyz * f25)) + (CB0[30].xyz * f26)) + (CB0[32].xyz * f27)) + (CB0[34].xyz * f28)) * f6))) + (CB0[10].xyz * f7)) * CB3[1].xyz, vec3(clamp(clamp(((f34.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix((textureLod(PrefilteredEnvTexture, f43.xyz, 0.0).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f41.y * 1.58823525905609130859375, 0.0, 1.0)))) * f6, (f73 * f73) * CB0[15].x, vec3((((float(abs(f72.x - 0.5) < 0.550000011920928955078125) * float(abs(f72.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f71) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture(GBufferDepthTexture, f72).x * 500.0) - f71) < 10.0)) * float(f44.w > 0.0))) + (f5 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f17, f18))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (((((vec3(f82) + (vec3(0.0199999995529651641845703125) * (1.0 - f82))) * (((f83 + (f83 * f83)) / (((f84 * f84) * ((f79 * 3.0) + 0.5)) * ((f78 * 0.75) + 0.25))) * clamp((dot(f16, f75) * CB0[9].w) * f7, 0.0, 1.0))) * CB0[10].xyz) * (CB0[9].w * CB0[9].w)) * clamp(1.0 - (VARYING7.w * CB0[23].y), 0.0, 1.0));
    vec4 f86 = vec4(0.0);
    f86.x = f85.x;
    vec4 f87 = f86;
    f87.y = f85.y;
    ...
    vec4 f89 = f88;
    f89.w = 1.0;
    vec3 f90 = textureLod(PrefilteredEnvTexture, vec4(f40, 0.0).xyz, max(CB0[13].y, VARYING6.w) * 5.0).xyz;
    bvec3 f91 = bvec3(!(CB0[13].w == 0.0));
    vec3 f92 = mix(vec3(f91.x ? CB0[14].xyz.x : f90.x, f91.y ? CB0[14].xyz.y : f90.y, f91.z ? CB0[14].xyz.z : f90.z), f89.xyz, vec3(VARYING6.w));
    vec4 f93 = f89;
    f93.x = f92.x;
    vec4 f94 = f93;
    f94.y = f92.y;

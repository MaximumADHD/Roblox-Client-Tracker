...
void main()
{
    ...
    for (;;)
    {
        if (f34 < f17)
        {
            ...
            if (f47)
            {
                ...
                vec4 f65 = vec4(f64.x, f64.y, f64.z, f24.w);
                float f66 = f37 * f63;
                if (f66 <= 9.9999997473787516355514526367188e-06)
                {
                    f32 = f65;
                    f29 = 0.0;
                    f28 = f50;
    ...
    float f68 = pow(max(exp2((CB0[13].z * 3.5714285373687744140625) * (f9 * f9)), 9.9999997473787516355514526367188e-05), 0.125);
    vec3 f69 = textureLod(PrefilteredEnvTexture, vec4(f2.x, f3, f2.z, 0.0).xyz, max(CB0[13].y, f68) * 5.0).xyz;
    bvec3 f70 = bvec3(!(CB0[13].w == 0.0));
    vec3 f71 = mix(vec3(f70.x ? CB0[14].xyz.x : f69.x, f70.y ? CB0[14].xyz.y : f69.y, f70.z ? CB0[14].xyz.z : f69.z), (f32.xyz + (((((((CB0[10].xyz * clamp(1.0 - ((0.07999999821186065673828125 * f28) / f18), 0.0, 1.0)) * 2.0) * clamp(-f11, 0.0, 1.0)) * f15) * f67) * exp2(0.10999999940395355224609375 - (0.0350000001490116119384765625 * f28))) * CB5[3].xyz)).xyz, vec3(f68));
    vec4 f72 = vec4(f71.x, f71.y, f71.z, f32.w);
    f72.w = f67;
    vec4 f73 = f72;
    f73.w = f67 * max(f68, CB0[13].y);
    _entryPointOutput_color = f73;
}

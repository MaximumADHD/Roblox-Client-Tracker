...
void main()
{
    ...
    vec3 f95 = normalize(f38 - CB0[16].xyz);
    float f96 = f94 * f94;
    ...
    vec3 f104 = mix(mix(f59.xyz, ((min(f7 + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f8)), vec3(CB0[21].w)) + (((((((CB0[40].xyz * f43) + (CB0[42].xyz * f44)) + (CB0[44].xyz * f45)) + (CB0[41].xyz * f46)) + (CB0[43].xyz * f47)) + (CB0[45].xyz * f48)) + (((((((CB0[34].xyz * f43) + (CB0[36].xyz * f44)) + (CB0[38].xyz * f45)) + (CB0[35].xyz * f46)) + (CB0[37].xyz * f47)) + (CB0[39].xyz * f48)) * f8))) + (CB0[15].xyz * f28)) * CB3[1].xyz, vec3(clamp(clamp(((f54.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix((textureLod(PrefilteredEnvTexture, f63.xyz, 0.0).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f61.y * 1.58823525905609130859375, 0.0, 1.0)))) * f8, (f93 * f93) * CB0[20].x, vec3((((float(abs(f92.x - 0.5) < 0.550000011920928955078125) * float(abs(f92.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f91) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture(GBufferDepthTexture, f92).x * 500.0) - f91) < 10.0)) * float(f64.w > 0.0))) + (f7 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f37, f38))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (((((vec3(f101) + (vec3(0.0199999995529651641845703125) * (1.0 - f101))) * (min((f102 + (f102 * f102)) / (((f103 * f103) * ((f98 * 3.0) + 0.5)) * ((f97 * 0.75) + 0.25)), 65504.0) * clamp((dot(f36, f1) * CB0[14].w) * f28, 0.0, 1.0))) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f105 = vec4(0.0);
    f105.x = f104.x;
    vec4 f106 = f105;
    f106.y = f104.y;
    ...
    vec4 f108 = f107;
    f108.w = 1.0;
    vec3 f109 = textureLod(PrefilteredEnvTexture, vec4(f60, 0.0).xyz, max(CB0[18].y, VARYING6.w) * 5.0).xyz;
    bvec3 f110 = bvec3(!(CB0[18].w == 0.0));
    vec3 f111 = mix(vec3(f110.x ? CB0[19].xyz.x : f109.x, f110.y ? CB0[19].xyz.y : f109.y, f110.z ? CB0[19].xyz.z : f109.z), f108.xyz, vec3(VARYING6.w));
    vec4 f112 = f108;
    f112.x = f111.x;
    vec4 f113 = f112;
    f113.y = f111.y;

...
void main()
{
    ...
    vec3 f47 = f44 * f44;
    bvec3 f48 = lessThan(f44, vec3(0.0));
    ...
    float f55 = f49.y;
    float f56 = f49.z;
    vec2 f57 = VARYING8.xy / vec2(VARYING8.w);
    vec2 f58 = f57 + (f43.xz * 0.0500000007450580596923828125);
    vec4 f59 = texture(GBufferColorTexture, f57);
    vec4 f60 = texture(GBufferDepthTexture, f57);
    vec4 f61 = texture(GBufferColorTexture, f58);
    vec4 f62 = texture(GBufferDepthTexture, f58);
    float f63 = f62.x * 500.0;
    float f64 = clamp(f63 - VARYING8.w, 0.0, 1.0);
    vec3 f65 = mix(f59.xyz, f61.xyz, vec3(f64));
    vec3 f66 = -f46;
    vec3 f67 = reflect(f66, f45);
    float f68 = VARYING8.w * 0.20000000298023223876953125;
    vec4 f69 = vec4(f67, 0.0);
    ...
    if (f110 > 0.0)
    {
        vec3 f112 = normalize(f46 - CB0[16].xyz);
        ...
        f111 = (((vec3(f118) + (vec3(0.0199999995529651641845703125) * (1.0 - f118))) * (((f119 + (f119 * f119)) / max(((f120 * f120) * ((f115 * 3.0) + 0.5)) * ((f114 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * clamp(f110, 0.0, 1.0))) * CB0[15].xyz) * (CB0[14].w * CB0[14].w);
    }
    else
    {
        f111 = vec3(0.0);
    }
    vec3 f121 = mix(mix((f65 * f65) * CB0[20].x, ((min(f9 + (CB0[32].xyz + (CB0[33].xyz * (f10 * (2.0 - CB0[14].w)))), vec3(CB0[21].w)) + (((((((CB0[40].xyz * f51) + (CB0[42].xyz * f52)) + (CB0[44].xyz * f53)) + (CB0[41].xyz * f54)) + (CB0[43].xyz * f55)) + (CB0[45].xyz * f56)) + (((((((CB0[34].xyz * f51) + (CB0[36].xyz * f52)) + (CB0[38].xyz * f53)) + (CB0[35].xyz * f54)) + (CB0[37].xyz * f55)) + (CB0[39].xyz * f56)) * f10))) + (CB0[15].xyz * f30)) * CB3[1].xyz, vec3(clamp(clamp(((mix(f60.x * 500.0, f63, f64) - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix((textureLod(PrefilteredEnvTexture, f69.xyz, 0.0).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f67.y * 1.58823525905609130859375, 0.0, 1.0)))) * f10, (f108 * f108) * CB0[20].x, vec3((((float(abs(f105.x - 0.5) < 0.550000011920928955078125) * float(abs(f105.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f104) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((f106.x * 500.0) - f104) < 10.0)) * float(f70.w > 0.0))) + (f9 * 0.100000001490116119384765625), vec3((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f45, f46))), 0.0, 1.0) + 0.300000011920928955078125) * (VARYING0.w * CB3[2].z))) + (f111 * clamp(1.0 - (VARYING7.w * CB0[28].y), 0.0, 1.0));
    vec4 f122 = vec4(0.0);
    f122.x = f121.x;
    vec4 f123 = f122;
    f123.y = f121.y;
    ...
    vec4 f125 = f124;
    f125.w = 1.0;
    vec3 f126 = textureLod(PrefilteredEnvTexture, vec4(f66, 0.0).xyz, max(CB0[18].y, VARYING6.w) * 5.0).xyz;

...
void main()
{
    ...
    float f1 = texture(NoiseMapTexture, VARYING0 * CB2[2].w).x * 8.0;
    float f2 = floor(f1);
    ...
    vec3 f6 = mix(vec3(1.0), VARYING2.xyz, vec3(f5.w)) * f5.xyz;
    vec4 f7 = vec4(0.0);
    f7.x = f6.x;
    vec4 f8 = f7;
    f8.y = f6.y;
    ...
    vec3 f24 = (((VARYING5.xyz * f23) + min((f19.xyz * (f19.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f20.x)), vec3(CB0[21].w))) * f14.xyz) + ((CB0[15].xyz * mix(vec3(0.100000001490116119384765625), f14.xyz, vec3(VARYING6.w * CB0[31].w))) * (VARYING5.w * f23));
    vec4 f25 = vec4(0.0);
    f25.x = f24.x;
    vec4 f26 = f25;
    f26.y = f24.y;
    ...
    vec4 f28 = f27;
    f28.w = VARYING2.w;
    float f29 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f30 = textureLod(PrefilteredEnvTexture, vec4(-(VARYING4.xyz / vec3(f0)), 0.0).xyz, max(CB0[18].y, f29) * 5.0).xyz;

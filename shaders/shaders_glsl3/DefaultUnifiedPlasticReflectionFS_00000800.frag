...
void main()
{
    ...
    float f16 = f15.y;
    vec3 f17 = (((VARYING6.xyz * f16) + min((f14.xyz * (f14.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f15.x)), vec3(CB0[21].w))) * mix(f6.xyz, textureLod(PrefilteredEnvTexture, vec4(f9, f10).xyz, f10).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f9.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(VARYING7.w))) + (CB0[15].xyz * ((VARYING6.w * f16) * 0.100000001490116119384765625));
    vec4 f18 = vec4(0.0);
    f18.x = f17.x;
    vec4 f19 = f18;
    f19.y = f17.y;

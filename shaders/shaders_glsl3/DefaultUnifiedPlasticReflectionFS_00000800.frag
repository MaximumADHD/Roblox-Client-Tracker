...
void main()
{
    ...
    float f18 = f17.y;
    vec3 f19 = (((VARYING6.xyz * f18) + min((f16.xyz * (f16.w * 120.0)) + (CB0[32].xyz + (CB0[33].xyz * f17.x)), vec3(CB0[21].w))) * mix(f6.xyz, textureLod(PrefilteredEnvTexture, vec4(vec3(f12.x ? f11.x : f9.x, f12.y ? f11.y : f9.y, f12.z ? f11.z : f9.z), f10).xyz, f10).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f9.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(VARYING7.w))) + (CB0[15].xyz * ((VARYING6.w * f18) * 0.100000001490116119384765625));
    vec4 f20 = vec4(0.0);
    f20.x = f19.x;
    vec4 f21 = f20;
    f21.y = f19.y;

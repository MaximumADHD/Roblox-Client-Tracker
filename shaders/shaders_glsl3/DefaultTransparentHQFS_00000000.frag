...
void main()
{
    ...
    vec3 f12 = -f1;
    vec3 f13 = reflect(f12, f4);
    float f14 = VARYING5.w * f10;
    vec3 f15 = mix(vec3(0.039999999105930328369140625), f9.xyz, vec3(f14));
    ...
    float f26 = f16.z;
    vec3 f27 = -CB0[16].xyz;
    float f28 = dot(f4, f27) * ((1.0 - ((step(f25.x, f26) * clamp(CB0[29].z + (CB0[29].w * abs(f26 - 0.5)), 0.0, 1.0)) * f25.y)) * f23.y);
    vec3 f29 = normalize(f1 - CB0[16].xyz);
    float f30 = clamp(f28, 0.0, 1.0);
    float f31 = f11 * f11;
    float f32 = max(0.001000000047497451305389404296875, dot(f4, f29));
    ...
    vec4 f47 = texture(PrecomputedBRDFTexture, vec2(f11, max(9.9999997473787516355514526367188e-05, dot(f4, f1))));
    float f48 = f47.x;
    float f49 = f47.y;
    ...
    float f59 = f53.y;
    float f60 = f53.z;
    vec3 f61 = ((((((f22.xyz * (f22.w * 120.0)) * 1.0) + ((((f42 - (f37 * f41)) * CB0[15].xyz) * f30) + (CB0[17].xyz * (f40 * clamp(-f28, 0.0, 1.0))))) + ((f42 - (f50 * f41)) * (((((((CB0[40].xyz * f55) + (CB0[42].xyz * f56)) + (CB0[44].xyz * f57)) + (CB0[41].xyz * f58)) + (CB0[43].xyz * f59)) + (CB0[45].xyz * f60)) + (((((((CB0[34].xyz * f55) + (CB0[36].xyz * f56)) + (CB0[38].xyz * f57)) + (CB0[35].xyz * f58)) + (CB0[37].xyz * f59)) + (CB0[39].xyz * f60)) * f24)))) + ((CB0[32].xyz + (CB0[33].xyz * f24)) * 1.0)) * f9.xyz) + ((((f37 * (((f38 + (f38 * f38)) / max(((f39 * f39) * ((f33 * 3.0) + 0.5)) * ((f32 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f30)) * CB0[15].xyz) * 1.0) + ((mix(f46, textureLod(PrefilteredEnvTexture, f44, f43).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f13.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f24)) * f50) * f10));
    vec4 f62 = vec4(0.0);
    f62.x = f61.x;
    vec4 f63 = f62;
    f63.y = f61.y;
    ...
    vec4 f66 = f64;
    f66.w = f65;
    float f67 = clamp(exp2((CB0[18].z * f0) + CB0[18].x) - CB0[18].w, 0.0, 1.0);

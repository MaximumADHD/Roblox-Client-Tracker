...
void main()
{
    ...
    float f15 = f14.y;
    vec3 f16 = VARYING7.xyz - CB0[25].xyz;
    vec3 f17 = VARYING7.xyz - CB0[26].xyz;
    vec3 f18 = VARYING7.xyz - CB0[27].xyz;
    vec4 f19 = vec4(VARYING7.xyz, 1.0) * mat4(CB8[((dot(f16, f16) < CB0[25].w) ? 0 : ((dot(f17, f17) < CB0[26].w) ? 1 : ((dot(f18, f18) < CB0[27].w) ? 2 : 3))) * 4 + 0], CB8[((dot(f16, f16) < CB0[25].w) ? 0 : ((dot(f17, f17) < CB0[26].w) ? 1 : ((dot(f18, f18) < CB0[27].w) ? 2 : 3))) * 4 + 1], CB8[((dot(f16, f16) < CB0[25].w) ? 0 : ((dot(f17, f17) < CB0[26].w) ? 1 : ((dot(f18, f18) < CB0[27].w) ? 2 : 3))) * 4 + 2], CB8[((dot(f16, f16) < CB0[25].w) ? 0 : ((dot(f17, f17) < CB0[26].w) ? 1 : ((dot(f18, f18) < CB0[27].w) ? 2 : 3))) * 4 + 3]);
    ...
    float f24 = exp(CB0[29].z * f23);
    float f25 = -exp((-CB0[29].w) * f23);
    vec2 f26 = (f22 * CB0[30].y) * vec2(f24, f25);
    vec2 f27 = f26 * f26;
    float f28 = f20.x;
    ...
    float f33 = f25 - f31;
    float f34 = (f8 > 0.0) ? mix(f15, mix(min((f24 <= f28) ? 1.0 : clamp(((f29 / (f29 + (f30 * f30))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f25 <= f31) ? 1.0 : clamp(((f32 / (f32 + (f33 * f33))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f15, clamp((length(VARYING7.xyz - CB0[7].xyz) * CB0[29].y) - (CB0[29].x * CB0[29].y), 0.0, 1.0)), CB0[30].x) : 0.0;
    vec3 f35 = texture(EnvironmentMapTexture, reflect(-VARYING4.xyz, f6)).xyz;
    vec3 f36 = ((min(((f13.xyz * (f13.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f14.x), vec3(CB0[16].w)) + (((CB0[10].xyz * clamp(f8, 0.0, 1.0)) + (CB0[12].xyz * max(-f8, 0.0))) * f34)) * mix((f9 * f9).xyz, (f35 * f35) * CB0[15].x, vec3(VARYING7.w)).xyz) + (CB0[10].xyz * (((step(0.0, f8) * CB2[0].y) * f34) * pow(clamp(dot(f6, normalize(f7 + normalize(VARYING4.xyz))), 0.0, 1.0), CB2[0].z)));

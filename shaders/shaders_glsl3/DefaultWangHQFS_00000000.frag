...
void main()
{
    ...
    float f34 = 0.08900000154972076416015625 + (f22.y * 0.9110000133514404296875);
    vec3 f35 = -f1;
    vec3 f36 = reflect(f35, f27);
    float f37 = f22.x * f33;
    vec3 f38 = mix(vec3(0.039999999105930328369140625), f32.xyz, vec3(f37));
    ...
    float f49 = f39.z;
    vec3 f50 = -CB0[16].xyz;
    float f51 = dot(f27, f50) * ((1.0 - ((step(f48.x, f49) * clamp(CB0[29].z + (CB0[29].w * abs(f49 - 0.5)), 0.0, 1.0)) * f48.y)) * f46.y);
    vec3 f52 = normalize(f1 + f50);
    float f53 = clamp(f51, 0.0, 1.0);
    float f54 = f34 * f34;
    float f55 = max(0.001000000047497451305389404296875, dot(f27, f52));
    ...
    vec4 f70 = texture(PrecomputedBRDFTexture, vec2(f34, max(9.9999997473787516355514526367188e-05, dot(f27, f1))));
    float f71 = f70.x;
    float f72 = f70.y;
    vec3 f73 = ((f38 * f71) + vec3(f72)) / vec3(f71 + f72);
    vec3 f74 = f27 * f27;
    bvec3 f75 = lessThan(f27, vec3(0.0));

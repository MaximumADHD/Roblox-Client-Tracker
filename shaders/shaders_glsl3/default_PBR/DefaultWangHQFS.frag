...
void main()
{
    ...
    vec3 f35 = textureLod(PrefilteredEnvTexture, f34, f33).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f20.y * 1.58823525905609130859375, 0.0, 1.0)));
    vec4 f36 = texture(PrecomputedBRDFTexture, vec2(f32, max(9.9999997473787516355514526367188e-05, dot(f20, f30))));
    ...
    float f65 = f59.y;
    float f66 = f59.z;
    vec3 f67 = (mix(textureLod(PrefilteredEnvIndoorTexture, f34, f33).xyz, f35, vec3(f27)) * f55) * f31;
    ...
    _entryPointOutput = vec4(f72.x, f72.y, f72.z, f69.w);
}

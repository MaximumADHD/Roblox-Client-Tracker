...
void main()
{
    ...
    float f64 = f56 - f62;
    float f65 = (f39 > 0.0) ? mix(f46, mix(min((f55 <= f59) ? 1.0 : clamp(((f60 / (f60 + (f61 * f61))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f56 <= f62) ? 1.0 : clamp(((f63 / (f63 + (f64 * f64))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f46, clamp((length(VARYING5.xyz - CB0[7].xyz) * CB0[45].y) - (CB0[45].x * CB0[45].y), 0.0, 1.0)), CB0[46].x) : 0.0;
    vec3 f66 = normalize(VARYING7.xyz);
    vec3 f67 = (f29 * f29).xyz;
    float f68 = f37.x;
    float f69 = f37.y;
    vec3 f70 = mix(vec3(0.039999999105930328369140625), f67, vec3(f68));
    float f71 = CB0[26].w * f30;
    float f72 = f69 * 5.0;
    vec3 f73 = vec4(reflect(-f66, f36), f72).xyz;
    vec4 f74 = texture(PrecomputedBRDFTexture, vec2(f69, max(9.9999997473787516355514526367188e-05, dot(f36, f66))));
    vec3 f75 = normalize(f38 + f66);
    float f76 = clamp(f39, 0.0, 1.0);
    float f77 = f69 * f69;
    float f78 = max(0.001000000047497451305389404296875, dot(f36, f75));
    float f79 = dot(f38, f75);
    float f80 = 1.0 - f79;
    float f81 = f80 * f80;
    float f82 = (f81 * f81) * f80;
    vec3 f83 = vec3(f82) + (f70 * (1.0 - f82));
    float f84 = f77 * f77;
    float f85 = (((f78 * f84) - f78) * f78) + 1.0;
    float f86 = 1.0 - (f68 * f71);
    float f87 = f74.x;
    float f88 = f74.y;
    vec3 f89 = ((f70 * f87) + vec3(f88)) / vec3(f87 + f88);
    vec3 f90 = (vec3(1.0) - (f89 * f71)) * f86;
    vec3 f91 = f36 * f36;
    bvec3 f92 = lessThan(f36, vec3(0.0));
    ...
    float f99 = f93.y;
    float f100 = f93.z;
    vec3 f101 = (mix(textureLod(PrefilteredEnvIndoorTexture, f73, f72).xyz, textureLod(PrefilteredEnvTexture, f73, f72).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f35.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f45)) * f89) * f71;
    vec3 f102 = ((((((((((vec3(1.0) - (f83 * f71)) * f86) * CB0[10].xyz) * f76) * f65) + (f90 * (((((((CB0[35].xyz * f95) + (CB0[37].xyz * f96)) + (CB0[39].xyz * f97)) + (CB0[36].xyz * f98)) + (CB0[38].xyz * f99)) + (CB0[40].xyz * f100)) + (((((((CB0[29].xyz * f95) + (CB0[31].xyz * f96)) + (CB0[33].xyz * f97)) + (CB0[30].xyz * f98)) + (CB0[32].xyz * f99)) + (CB0[34].xyz * f100)) * f45)))) + (CB0[27].xyz + (CB0[28].xyz * f45))) + vec3((f37.z * 2.0) * f30)) * f67) + ((((((f83 * ((f84 + (f84 * f84)) / (((f85 * f85) * ((f79 * 3.0) + 0.5)) * ((f78 * 0.75) + 0.25)))) * CB0[10].xyz) * f76) * f65) * f30) + f101)) + ((f43.xyz * (f43.w * 120.0)).xyz * mix(f67, f101 * (1.0 / (max(max(f101.x, f101.y), f101.z) + 0.00999999977648258209228515625)), ((vec3(1.0) - f90) * f71) * (1.0 - f45)));
    vec4 f103 = vec4(f102.x, f102.y, f102.z, vec4(0.0).w);
    f103.w = 1.0;
    vec3 f104 = mix(CB0[14].xyz, sqrt(clamp(f103.xyz * CB0[15].y, vec3(0.0), vec3(1.0))).xyz, vec3(clamp(VARYING5.w, 0.0, 1.0)));

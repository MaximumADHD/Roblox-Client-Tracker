...
void main()
{
    ...
    float f45 = f44.w;
    float f46 = f43 + clamp((texture(GBufferDepthTexture, f44.xy / vec2(f45)).x * 500.0) - f45, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f47 = VARYING8 + (f42 * f46);
    float f48 = f47.w;
    float f49 = f46 + clamp((texture(GBufferDepthTexture, f47.xy / vec2(f48)).x * 500.0) - f48, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f50 = VARYING8 + (f42 * f49);
    float f51 = f50.w;
    float f52 = f49 + clamp((texture(GBufferDepthTexture, f50.xy / vec2(f51)).x * 500.0) - f51, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f53 = VARYING8 + (f42 * f52);
    float f54 = f53.w;
    float f55 = f52 + clamp((texture(GBufferDepthTexture, f53.xy / vec2(f54)).x * 500.0) - f54, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f56 = VARYING8 + (f42 * f55);
    float f57 = f56.w;
    float f58 = f55 + clamp((texture(GBufferDepthTexture, f56.xy / vec2(f57)).x * 500.0) - f57, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f59 = VARYING8 + (f42 * f58);
    float f60 = f59.w;
    float f61 = f58 + clamp((texture(GBufferDepthTexture, f59.xy / vec2(f60)).x * 500.0) - f60, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f62 = VARYING8 + (f42 * f61);
    float f63 = f62.w;
    float f64 = f61 + clamp((texture(GBufferDepthTexture, f62.xy / vec2(f63)).x * 500.0) - f63, VARYING8.w * (-0.20000000298023223876953125), f38);
    vec4 f65 = VARYING8 + (f42 * f64);
    float f66 = f65.w;
    vec4 f67 = VARYING8 + (f42 * f64);
    float f68 = f67.w;
    vec2 f69 = f67.xy / vec2(f68);
    vec3 f70 = texture(EnvMapTexture, f37).xyz;
    vec3 f71 = texture(GBufferColorTexture, f69).xyz;
    vec3 f72 = -CB0[11].xyz;
    ...
    _entryPointOutput = vec4(f84.x, f84.y, f84.z, f83.w);
}

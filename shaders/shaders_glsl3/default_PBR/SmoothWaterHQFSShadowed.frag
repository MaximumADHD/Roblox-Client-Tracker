...
void main()
{
    ...
    float f48 = f42.y;
    float f49 = f42.z;
    vec3 f50 = texture(EnvMapTexture, reflect(-f39, f38)).xyz;
    vec3 f51 = normalize(f1 + f39);
    float f52 = f34 * f34;
    float f53 = max(0.001000000047497451305389404296875, dot(f37, f51));
    ...
    vec3 f60 = mix(((min(f7 + (CB0[27].xyz + (CB0[28].xyz * f8)), vec3(CB0[16].w)) + (((((((CB0[35].xyz * f44) + (CB0[37].xyz * f45)) + (CB0[39].xyz * f46)) + (CB0[36].xyz * f47)) + (CB0[38].xyz * f48)) + (CB0[40].xyz * f49)) + (((((((CB0[29].xyz * f44) + (CB0[31].xyz * f45)) + (CB0[33].xyz * f46)) + (CB0[30].xyz * f47)) + (CB0[32].xyz * f48)) + (CB0[34].xyz * f49)) * f8))) + (CB0[10].xyz * f28)) * CB3[1].xyz, (((f50 * f50) * CB0[15].x) * (vec3(f8) * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f36.y * 1.58823525905609130859375, 0.0, 1.0))))) + (f7 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f38, f39))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + ((((((vec3(f57) + (vec3(0.0199999995529651641845703125) * (1.0 - f57))) * ((f58 + (f58 * f58)) / (((f59 * f59) * ((f54 * 3.0) + 0.5)) * ((f53 * 0.75) + 0.25)))) * CB0[10].xyz) * clamp(dot(f37, f1), 0.0, 1.0)) * f28) * clamp(1.0 - (VARYING7.w * CB0[23].y), 0.0, 1.0));
    vec4 f61 = vec4(f60.x, f60.y, f60.z, vec4(0.0).w);
    f61.w = 1.0;
    vec3 f62 = mix(CB0[14].xyz, sqrt(clamp(f61.xyz * CB0[15].y, vec3(0.0), vec3(1.0))).xyz, vec3(clamp(VARYING6.w, 0.0, 1.0)));

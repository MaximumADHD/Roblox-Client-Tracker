...
void main()
{
    ...
    vec3 f22 = textureLod(PrefilteredEnvTexture, f21, f20).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f7.y * 1.58823525905609130859375, 0.0, 1.0)));
    vec4 f23 = texture(PrecomputedBRDFTexture, vec2(f19, max(9.9999997473787516355514526367188e-05, dot(f7, f17))));
    ...
    float f50 = f44.y;
    float f51 = f44.z;
    vec3 f52 = (mix(textureLod(PrefilteredEnvIndoorTexture, f21, f20).xyz, f22, vec3(f14)) * f40) * f18;
    ...
    _entryPointOutput = vec4(f57.x, f57.y, f57.z, f54.w);
}

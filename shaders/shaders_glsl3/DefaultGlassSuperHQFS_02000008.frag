...
void main()
{
    ...
    vec2 f118 = CB0[50].xy * 0.5;
    vec2 f119 = f118 / CB0[50].zw;
    vec2 f120 = (f114 + f117) + f119;
    vec2 f121 = (f114 - f117) + f119;
    vec4 f122 = texture(GBufferColorTexture, (f109 + (f116 * clamp(min(texture(GBufferDepthTexture, (vec2(clamp(f120.x, 0.0, 1.0), clamp(f120.y, 0.0, 1.0)) * CB0[50].zw) - f118).x * 500.0, texture(GBufferDepthTexture, (vec2(clamp(f121.x, 0.0, 1.0), clamp(f121.y, 0.0, 1.0)) * CB0[50].zw) - f118).x * 500.0) - f105, 0.0, 1.0))) * CB0[50].zw);
    vec3 f123 = f122.xyz;
    vec3 f124 = (f123 * f123) * CB0[20].x;
    vec4 f125 = f122;
    f125.x = f124.x;
    vec4 f126 = f125;
    f126.y = f124.y;
    vec4 f127 = f126;
    f127.z = f124.z;
    vec3 f128 = f127.xyz * mix(vec3(1.0), VARYING2.xyz, vec3(f19));
    vec4 f129 = vec4(0.0);
    f129.x = f128.x;
    vec4 f130 = f129;
    f130.y = f128.y;
    ...
    f131.z = f128.z;
    vec4 f132 = f131;
    f132.w = mix(1.0, f122.w, dot(f131.xyz, vec3(1.0)) / (dot(f127.xyz, vec3(1.0)) + 0.00999999977648258209228515625));
    vec4 f133 = mix(mix(f132, vec4(mix(((f74 + (((vec3(1.0) - (f88 * f30)) * (((((((CB0[40].xyz * f93) + (CB0[42].xyz * f94)) + (CB0[44].xyz * f95)) + (CB0[41].xyz * f96)) + (CB0[43].xyz * f97)) + (CB0[45].xyz * f98)) + (((((((CB0[34].xyz * f93) + (CB0[36].xyz * f94)) + (CB0[38].xyz * f95)) + (CB0[35].xyz * f96)) + (CB0[37].xyz * f97)) + (CB0[39].xyz * f98)) * f49))) * f18)) + (CB0[32].xyz + (CB0[33].xyz * (f49 * (2.0 - CB0[14].w))))) * (f26 * f18), f71, vec3(VARYING7.w)), 1.0), vec4(f18)), vec4(f71, 1.0), vec4(((f99 * f99) * 0.800000011920928955078125) * f19)) + vec4(f73 + (((mix(f38, f36, f70) * f88) * f30) * ((CB0[14].w * CB0[14].w) * f18)), 0.0);
    float f134 = clamp(exp2((CB0[18].z * f27) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f135 = textureLod(PrefilteredEnvTexture, vec4(f32, 0.0).xyz, max(CB0[18].y, f134) * 5.0).xyz;
    bvec3 f136 = bvec3(!(CB0[18].w == 0.0));
    vec3 f137 = mix(vec3(f136.x ? CB0[19].xyz.x : f135.x, f136.y ? CB0[19].xyz.y : f135.y, f136.z ? CB0[19].xyz.z : f135.z), f133.xyz, vec3(f134));
    vec4 f138 = f133;
    f138.x = f137.x;
    vec4 f139 = f138;
    f139.y = f137.y;
    ...
    vec4 f141 = f140;
    f141.w = f134 * f133.w;
    vec3 f142 = sqrt(clamp(f141.xyz * CB0[20].y, vec3(0.0), vec3(1.0)));
    vec4 f143 = f141;
    f143.x = f142.x;
    vec4 f144 = f143;
    f144.y = f142.y;

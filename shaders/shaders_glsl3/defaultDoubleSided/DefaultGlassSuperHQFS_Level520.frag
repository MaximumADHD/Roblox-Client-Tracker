...
void main()
{
    ...
    float f60 = clamp(f20 * ((f20 > 0.0) ? mix(f29, mix(min((f38 <= f42) ? 1.0 : clamp(((f43 / (f43 + (f44 * f44))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f39 <= f45) ? 1.0 : clamp(((f46 / (f46 + (f47 * f47))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f29, clamp((length(f21 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)), CB0[47].x) : 0.0), 0.0, 1.0);
    float f61 = f11 * f11;
    ...
    float f84 = f78.y;
    float f85 = f78.z;
    vec3 f86 = ((((((CB0[35].xyz * f80) + (CB0[37].xyz * f81)) + (CB0[39].xyz * f82)) + (CB0[36].xyz * f83)) + (CB0[38].xyz * f84)) + (CB0[40].xyz * f85)) + (((((((CB0[29].xyz * f80) + (CB0[31].xyz * f81)) + (CB0[33].xyz * f82)) + (CB0[30].xyz * f83)) + (CB0[32].xyz * f84)) + (CB0[34].xyz * f85)) * f28);
    vec3 f87 = (mix(textureLod(PrefilteredEnvIndoorTexture, f56, f55).xyz, f57, f70) * f74) * f52;
    float f88 = 1.0 - f53;
    float f89 = 1.0 - VARYING2.w;
    float f90 = mix(0.660000026226043701171875, 1.0, f89 * f89);
    mat4 f91 = mat4(CB0[0], CB0[1], CB0[2], CB0[3]);
    vec4 f92 = vec4(CB0[7].xyz - VARYING4.xyz, 1.0) * f91;
    vec4 f93 = vec4(CB0[7].xyz - ((VARYING4.xyz * (1.0 + ((3.0 * f90) / max(dot(VARYING4.xyz, f18), 0.00999999977648258209228515625)))) + (f18 * (3.0 * (1.0 - f90)))), 1.0) * f91;
    float f94 = f92.w;
    vec2 f95 = ((f92.xy * 0.5) + vec2(0.5 * f94)).xy / vec2(f94);
    float f96 = f93.w;
    vec2 f97 = ((f93.xy * 0.5) + vec2(0.5 * f96)).xy / vec2(f96);
    vec2 f98 = f97 - vec2(0.5);
    vec2 f99 = (f97 - f95) * clamp(vec2(1.0) - ((f98 * f98) * 4.0), vec2(0.0), vec2(1.0));
    vec2 f100 = normalize(f99) * CB0[23].x;
    vec4 f101 = texture(GBufferColorTexture, f95 + (f99 * clamp(min(texture(GBufferDepthTexture, f97 + f100).x * 500.0, texture(GBufferDepthTexture, f97 - f100).x * 500.0) - f94, 0.0, 1.0)));
    vec3 f102 = f101.xyz;
    vec3 f103 = ((f102 * f102) * CB0[15].x).xyz;
    vec3 f104 = f103 * mix(vec3(1.0), VARYING2.xyz, vec3(f14));
    vec4 f105 = vec4(f104.x, f104.y, f104.z, vec4(0.0).w);
    f105.w = mix(1.0, f101.w, dot(f104.xyz, vec3(1.0)) / (dot(f103, vec3(1.0)) + 0.00999999977648258209228515625));
    vec4 f106 = mix(mix(f105, vec4(mix(((((((vec3(1.0) - (f67 * f52)) * CB0[10].xyz) * f60) + ((((vec3(1.0) - f75) * f86) * CB0[25].w) * f13)) + (CB0[27].xyz + (CB0[28].xyz * f28))) * f51) + ((f27 * mix(f51, f87 * (1.0 / (max(max(f86.x, f86.y), f86.z) + 0.00999999977648258209228515625)), f75 * (f52 * (1.0 - f28)))) * f13), f71, vec3(VARYING7.w)), 1.0), vec4(f13)), vec4(f71, 1.0), vec4(((f88 * f88) * 0.800000011920928955078125) * f14)) + vec4(((f67 * (((f68 + (f68 * f68)) / (((f69 * f69) * ((f63 * 3.0) + 0.5)) * ((f62 * 0.75) + 0.25))) * f60)) * CB0[10].xyz) + ((f87 * f28) * f13), 0.0);
    float f107 = clamp(exp2((CB0[13].z * f49) + CB0[13].x) - CB0[13].w, 0.0, 1.0);
    vec3 f108 = textureLod(PrefilteredEnvTexture, vec4(-VARYING4.xyz, 0.0).xyz, max(CB0[13].y, f107) * 5.0).xyz;
    ...
    vec4 f111 = vec4(f110.x, f110.y, f110.z, f106.w);
    f111.w = f107 * f106.w;
    vec3 f112 = sqrt(clamp(f111.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    _entryPointOutput = vec4(f112.x, f112.y, f112.z, f111.w);
}

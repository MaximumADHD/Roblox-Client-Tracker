...
void main()
{
    ...
    float f67 = f61.y;
    float f68 = f61.z;
    vec3 f69 = (mix(textureLod(PrefilteredEnvIndoorTexture, f37, f36).xyz * f25, f38, f53) * f57) * f33;
    float f70 = 1.0 - f34;
    float f71 = 1.0 - VARYING2.w;
    float f72 = mix(0.660000026226043701171875, 1.0, f71 * f71);
    mat4 f73 = mat4(CB0[0], CB0[1], CB0[2], CB0[3]);
    vec4 f74 = vec4(CB0[7].xyz - VARYING4.xyz, 1.0) * f73;
    vec4 f75 = vec4(CB0[7].xyz - ((VARYING4.xyz * (1.0 + ((3.0 * f72) / max(dot(VARYING4.xyz, f18), 0.00999999977648258209228515625)))) + (f18 * (3.0 * (1.0 - f72)))), 1.0) * f73;
    float f76 = f74.w;
    vec2 f77 = ((f74.xy * 0.5) + vec2(0.5 * f76)).xy / vec2(f76);
    float f78 = f75.w;
    vec2 f79 = ((f75.xy * 0.5) + vec2(0.5 * f78)).xy / vec2(f78);
    vec2 f80 = f79 - vec2(0.5);
    vec2 f81 = (f79 - f77) * clamp(vec2(1.0) - ((f80 * f80) * 4.0), vec2(0.0), vec2(1.0));
    vec2 f82 = normalize(f81) * CB0[23].x;
    vec4 f83 = texture(GBufferColorTexture, f77 + (f81 * clamp(min(texture(GBufferDepthTexture, f79 + f82).x * 500.0, texture(GBufferDepthTexture, f79 - f82).x * 500.0) - f76, 0.0, 1.0)));
    vec3 f84 = f83.xyz;
    vec3 f85 = ((f84 * f84) * CB0[15].x).xyz;
    vec3 f86 = f85 * mix(vec3(1.0), VARYING2.xyz, vec3(f14));
    vec4 f87 = vec4(f86.x, f86.y, f86.z, vec4(0.0).w);
    f87.w = mix(1.0, f83.w, dot(f86.xyz, vec3(1.0)) / (dot(f85, vec3(1.0)) + 0.00999999977648258209228515625));
    vec4 f88 = mix(mix(f87, vec4(mix((((((((vec3(1.0) - (f50 * f33)) * CB0[10].xyz) * f43) + (CB0[12].xyz * clamp(-f41, 0.0, 1.0))) + (((vec3(1.0) - f58) * (((((((CB0[35].xyz * f63) + (CB0[37].xyz * f64)) + (CB0[39].xyz * f65)) + (CB0[36].xyz * f66)) + (CB0[38].xyz * f67)) + (CB0[40].xyz * f68)) + (((((((CB0[29].xyz * f63) + (CB0[31].xyz * f64)) + (CB0[33].xyz * f65)) + (CB0[30].xyz * f66)) + (CB0[32].xyz * f67)) + (CB0[34].xyz * f68)) * f26))) * f13)) + (CB0[27].xyz + (CB0[28].xyz * f26))) * f32) + ((f25 * mix(f32, f69 * (1.0 / (max(max(f69.x, f69.y), f69.z) + 0.00999999977648258209228515625)), f58 * (f33 * (1.0 - f26)))) * f13), f54, vec3(VARYING7.w)), 1.0), vec4(f13)), vec4(f54, 1.0), vec4(((f70 * f70) * 0.800000011920928955078125) * f14)) + vec4(((f50 * (((f51 + (f51 * f51)) / (((f52 * f52) * ((f46 * 3.0) + 0.5)) * ((f45 * 0.75) + 0.25))) * f43)) * CB0[10].xyz) + (f69 * f13), 0.0);
    float f89 = clamp(exp2((CB0[13].z * f30) + CB0[13].x) - CB0[13].w, 0.0, 1.0);

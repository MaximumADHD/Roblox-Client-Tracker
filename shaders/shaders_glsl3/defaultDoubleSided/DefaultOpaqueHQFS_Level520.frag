...
void main()
{
    ...
    float f42 = clamp(f4 * ((f4 > 0.0) ? mix(f13, mix(min((f22 <= f26) ? 1.0 : clamp(((f27 / (f27 + (f28 * f28))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f23 <= f29) ? 1.0 : clamp(((f30 / (f30 + (f31 * f31))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f13, clamp((length(f6 - CB0[7].xyz) * CB0[45].y) - (CB0[45].x * CB0[45].y), 0.0, 1.0)), CB0[46].x) : 0.0), 0.0, 1.0);
    float f43 = f34 * f34;
    ...
    vec3 f71 = (((((((f54 - (f49 * f53)) * CB0[10].xyz) * f42) + ((f58 * f69) * CB0[25].w)) + (CB0[27].xyz + (CB0[28].xyz * f12))) * f32) + (((f49 * (((f50 + (f50 * f50)) / (((f51 * f51) * ((f45 * 3.0) + 0.5)) * ((f44 * 0.75) + 0.25))) * f42)) * CB0[10].xyz) + f70)) + ((f10.xyz * (f10.w * 120.0)).xyz * mix(f32, f70 * (1.0 / (max(max(f69.x, f69.y), f69.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f58) * (f33 * (1.0 - f12))));

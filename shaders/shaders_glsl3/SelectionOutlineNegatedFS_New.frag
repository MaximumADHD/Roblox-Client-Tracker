...
void main()
{
    ...
    float f24 = (textureLod(Texture1Texture, min(f23, CB1[12].zw), 0.0).x < textureLod(Texture2Texture, min(f23, CB1[13].xy), 0.0).x) ? CB1[2].y : 1.0;
    vec4 f25 = f15;
    f25.w = f15.w * f24;
    vec4 f26 = mix(f25, vec4(CB6[8].xyz, f24), vec4(clamp(255.0 * max(abs(f10 - f8), abs(f14 - f12)), 0.0, 1.0)));
    vec3 f27 = f26.xyz * f26.w;
    vec4 f28 = f26;
    f28.x = f27.x;
    vec4 f29 = f28;
    f29.y = f27.y;

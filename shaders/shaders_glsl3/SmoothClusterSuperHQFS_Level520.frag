...
void main()
{
    ...
    float f33 = exp(CB0[46].z * f32);
    float f34 = -exp((-CB0[46].w) * f32);
    vec2 f35 = (f31 * CB0[47].y) * vec2(f33, f34);
    vec2 f36 = f35 * f35;
    float f37 = f29.x;
    ...
    float f42 = f34 - f40;
    vec3 f43 = normalize(f8);
    vec3 f44 = (f7 * f7).xyz;
    float f45 = f2.y;
    float f46 = CB0[26].w * f9;
    vec3 f47 = reflect(-f43, f14);
    vec3 f48 = normalize(f15 + f43);
    float f49 = clamp(f16 * ((f16 > 0.0) ? mix(f24, mix(min((f33 <= f37) ? 1.0 : clamp(((f38 / (f38 + (f39 * f39))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f34 <= f40) ? 1.0 : clamp(((f41 / (f41 + (f42 * f42))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f24, clamp((length(f17 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)), CB0[47].x) : 0.0), 0.0, 1.0);
    float f50 = f45 * f45;
    ...
    vec3 f78 = ((((((((vec3(1.0) - (f56 * f46)) * CB0[10].xyz) * f49) + (((vec3(1.0) - f65) * f76) * CB0[25].w)) + ((CB0[27].xyz + (CB0[28].xyz * f23)) * 1.0)) + vec3((f2.z * 2.0) * f9)) * f44) + (((((f56 * (((f57 + (f57 * f57)) / (((f58 * f58) * ((f52 * 3.0) + 0.5)) * ((f51 * 0.75) + 0.25))) * f49)) * CB0[10].xyz) * f9) * VARYING0.w) + f77)) + (((f21.xyz * (f21.w * 120.0)).xyz * mix(f44, f77 * (1.0 / (max(max(f76.x, f76.y), f76.z) + 0.00999999977648258209228515625)), f65 * (f46 * (1.0 - f23)))) * 1.0);

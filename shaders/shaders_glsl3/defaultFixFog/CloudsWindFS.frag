...
void main()
{
    ...
    for (;;)
    {
        if (f35 < f18)
        {
            ...
            vec2 f45 = vec2(CB5[4].x, 0.3499999940395355224609375 * CB5[4].x) * ((vec2(textureLod(DetailTex3DTexture, vec4((f42 * (0.5 * CB5[4].y)) + (f41 * f44.x), f39).xyz, f39).x, textureLod(DetailTex3DTexture, vec4((f42 * (2.0 * CB5[4].y)) + (f41 * f44.y), f39).xyz, f39).x) * 2.0) - vec2(1.0));
            float f46 = textureLod(CloudsDistanceFieldTexture, vec4(((f24 + (f41 * 0.75)) * vec3(0.03125, 1.0, 0.03125)).xzy, f39).xyz, f39).x + (f45.x + f45.y);
            float f47 = (f43 * 50.0) * (CB5[2].x - f46);
            bool f48 = f46 < CB5[2].x;
            if (f48)
            {
                vec4 f49 = textureLod(BeerShadowMapTexture, vec4(f24.xz * vec2(0.03125), 0.0, 0.0).xy, 0.0);
                float f50 = min(f49.z * 0.5, (f49.y + (0.3499999940395355224609375 * textureLod(BlueNoiseTexTexture, vec4(gl_FragCoord.xy, 0.0, 0.0).xy, 0.0).x)) * max(0.0, (1.0 - ((float(f35) / f19) + (f46 / f19))) - f49.x));
                float f51 = f37 + f47;
                vec3 f52 = f3 * f3;
    ...
    vec3 f76 = textureLod(PrefilteredEnvTexture, vec4(f3.x, f4, f3.z, 0.0).xyz, max(CB0[18].y, f75) * 5.0).xyz;
    ...
    f82.w = f70;
    vec4 f83 = f82;
    f83.w = f70 * max(f75, CB0[18].y);
    _entryPointOutput_color = f83;
}

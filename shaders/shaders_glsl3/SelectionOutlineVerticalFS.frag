...
void main()
{
    ...
    for (; f18 <= f0; f11 = f14, f10 = f13, f9 = f17, f19 = f12, f8 = f16, f7 = f15, f18++)
    {
        ...
        vec4 f22 = vec4(255.0) - (textureLod(Texture0Texture, f14, 0.0) * 255.0);
        vec4 f23 = f21 * f21;
        vec4 f24 = f22 * f22;
        vec3 f25 = vec3(f20);
    ...
    float f62 = f51.z;
    float f63 = f51.x;
    _entryPointOutput = vec4(mix(mix(mix(mix(CB7[f58 * 1 + 0].xyz * f60, CB7[(f58 + 256) * 1 + 0].xyz, vec3(f59)), CB6[6].xyz, vec3(f61)), CB6[7].xyz, vec3(f62)), CB6[5].xyz, vec3(f63)), max(max(max(max(CB7[f58 * 1 + 0].w * f60, f59), f61), f62), f63));
}

...
void main()
{
    ...
    float f89 = f83.y;
    float f90 = f83.z;
    vec3 f91 = ((((((f52.xyz * (f52.w * 120.0)) + ((((f72 - (f67 * f71)) * CB0[15].xyz) * f60) + (CB0[17].xyz * (f70 * clamp(-f58, 0.0, 1.0))))) + (((f72 - (f80 * f71)) * (((((((CB0[40].xyz * f85) + (CB0[42].xyz * f86)) + (CB0[44].xyz * f87)) + (CB0[41].xyz * f88)) + (CB0[43].xyz * f89)) + (CB0[45].xyz * f90)) + (((((((CB0[34].xyz * f85) + (CB0[36].xyz * f86)) + (CB0[38].xyz * f87)) + (CB0[35].xyz * f88)) + (CB0[37].xyz * f89)) + (CB0[39].xyz * f90)) * f54))) * 1.0)) + ((CB0[32].xyz + (CB0[33].xyz * f54)) * 1.0)) + vec3((f6.z * 2.0) * f39)) * f38) + (((((f67 * (((f68 + (f68 * f68)) / max(((f69 * f69) * ((f63 * 3.0) + 0.5)) * ((f62 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f60)) * CB0[15].xyz) * 1.0) * (f39 * VARYING0.w)) + ((mix(f76, textureLod(PrefilteredEnvTexture, f74, f73).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f45.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f54)) * f80) * f44));
    vec4 f92 = vec4(0.0);
    f92.x = f91.x;
    vec4 f93 = f92;
    f93.y = f91.y;

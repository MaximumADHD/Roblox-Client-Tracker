...
void main()
{
    ...
    vec3 f15 = (((VARYING5.xyz * f14) + min((f10.xyz * (f10.w * 120.0)) + (CB0[8].xyz + (CB0[9].xyz * f11.x)), vec3(CB0[16].w))) * f5.xyz) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f5.xyz, vec3(VARYING6.w * CB0[26].w))) * (VARYING5.w * f14));
    vec4 f16 = vec4(0.0);
    f16.x = f15.x;
    vec4 f17 = f16;
    f17.y = f15.y;
    ...
    vec3 f24 = mix(vec3(f23.x ? CB0[14].xyz.x : f22.x, f23.y ? CB0[14].xyz.y : f22.y, f23.z ? CB0[14].xyz.z : f22.z), f20.xyz, vec3(f21));
    vec4 f25 = f20;
    f25.x = f24.x;
    vec4 f26 = f25;
    f26.y = f24.y;
    vec4 f27 = f26;
    f27.z = f24.z;
    vec3 f28 = sqrt(clamp(f27.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    vec4 f29 = f27;
    f29.x = f28.x;
    vec4 f30 = f29;
    f30.y = f28.y;

...
void main()
{
    ...
    float f73 = f28.y;
    float f74 = f28.x;
    vec3 f75 = mix(vec3(0.039999999105930328369140625), f72, vec3(f74));
    float f76 = CB0[26].w * f35;
    vec3 f77 = reflect(-f71, f41);
    vec3 f78 = normalize(f42 + f71);
    float f79 = clamp(f43 * (((f43 * CB0[47].x) > 0.0) ? mix(min((f61 <= f65) ? 1.0 : clamp(((f66 / (f66 + (f67 * f67))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f62 <= f68) ? 1.0 : clamp(((f69 / (f69 + (f70 * f70))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f52, clamp((length(f44 - CB0[7].xyz) * CB0[46].y) - (CB0[46].x * CB0[46].y), 0.0, 1.0)) : f52), 0.0, 1.0);
    float f80 = f73 * f73;
    ...
    vec3 f91 = vec3(f89);
    float f92 = f73 * 5.0;
    vec3 f93 = vec4(f77, f92).xyz;
    vec4 f94 = texture(PrecomputedBRDFTexture, vec2(f73, max(9.9999997473787516355514526367188e-05, dot(f41, f71))));
    float f95 = f94.x;
    float f96 = f94.y;
    vec3 f97 = ((f75 * f95) + vec3(f96)) / vec3(f95 + f96);
    vec3 f98 = f91 - (f97 * f90);
    vec3 f99 = f41 * f41;
    bvec3 f100 = lessThan(f41, vec3(0.0));
    ...
    float f107 = f101.y;
    float f108 = f101.z;
    vec3 f109 = (mix(textureLod(PrefilteredEnvIndoorTexture, f93, f92).xyz * f50, textureLod(PrefilteredEnvTexture, f93, f92).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f77.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f51)) * f97) * f76;
    vec3 f110 = ((((((((f91 - (f86 * f90)) * CB0[10].xyz) * f79) + (f98 * (((((((CB0[35].xyz * f103) + (CB0[37].xyz * f104)) + (CB0[39].xyz * f105)) + (CB0[36].xyz * f106)) + (CB0[38].xyz * f107)) + (CB0[40].xyz * f108)) + (((((((CB0[29].xyz * f103) + (CB0[31].xyz * f104)) + (CB0[33].xyz * f105)) + (CB0[30].xyz * f106)) + (CB0[32].xyz * f107)) + (CB0[34].xyz * f108)) * f51)))) + (CB0[27].xyz + (CB0[28].xyz * f51))) + vec3((f28.z * 2.0) * f35)) * f72) + (((((f86 * (((f87 + (f87 * f87)) / (((f88 * f88) * ((f82 * 3.0) + 0.5)) * ((f81 * 0.75) + 0.25))) * f79)) * CB0[10].xyz) * f35) * VARYING0.w) + f109)) + ((f50 * mix(f72, f109 * (1.0 / (max(max(f109.x, f109.y), f109.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f98) * (f76 * (1.0 - f51)))) * 1.0);
    vec4 f111 = vec4(f110.x, f110.y, f110.z, vec4(0.0).w);
    f111.w = 1.0;
    float f112 = clamp(exp2((CB0[13].z * VARYING5.w) + CB0[13].x) - CB0[13].w, 0.0, 1.0);
    vec3 f113 = textureLod(PrefilteredEnvTexture, vec4(-f34, 0.0).xyz, max(CB0[13].y, f112) * 5.0).xyz;

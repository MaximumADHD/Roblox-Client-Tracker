...
void main()
{
    ...
    float f12 = f5.z;
    float f13 = (1.0 - ((step(f11.x, f12) * clamp(CB0[24].z + (CB0[24].w * abs(f12 - 0.5)), 0.0, 1.0)) * f11.y)) * f10.y;
    vec3 f14 = (((VARYING5.xyz * f13) + min((f9.xyz * (f9.w * 120.0)) + (CB0[8].xyz + (CB0[9].xyz * f10.x)), vec3(CB0[16].w))) * f4.xyz) + ((CB0[10].xyz * mix(vec3(0.100000001490116119384765625), f4.xyz, vec3(VARYING6.w * CB0[26].w))) * (VARYING5.w * f13));
    vec4 f15 = vec4(0.0);
    f15.x = f14.x;
    vec4 f16 = f15;
    f16.y = f14.y;
    ...
    vec3 f22 = mix(vec3(f21.x ? CB0[14].xyz.x : f20.x, f21.y ? CB0[14].xyz.y : f20.y, f21.z ? CB0[14].xyz.z : f20.z), f18.xyz, vec3(f19));
    vec4 f23 = f18;
    f23.x = f22.x;
    vec4 f24 = f23;
    f24.y = f22.y;
    vec4 f25 = f24;
    f25.z = f22.z;
    vec3 f26 = sqrt(clamp(f25.xyz * CB0[15].y, vec3(0.0), vec3(1.0)));
    vec4 f27 = f25;
    f27.x = f26.x;
    vec4 f28 = f27;
    f28.y = f26.y;

...
void main()
{
    ...
    float f62 = f56.y;
    float f63 = f56.z;
    vec3 f64 = ((((((f25.xyz * (f25.w * 120.0)) + ((((f45 - (f40 * f44)) * CB0[15].xyz) * f33) + (CB0[17].xyz * (f43 * clamp(-f31, 0.0, 1.0))))) + (((f45 - (f53 * f44)) * (((((((CB0[40].xyz * f58) + (CB0[42].xyz * f59)) + (CB0[44].xyz * f60)) + (CB0[41].xyz * f61)) + (CB0[43].xyz * f62)) + (CB0[45].xyz * f63)) + (((((((CB0[34].xyz * f58) + (CB0[36].xyz * f59)) + (CB0[38].xyz * f60)) + (CB0[35].xyz * f61)) + (CB0[37].xyz * f62)) + (CB0[39].xyz * f63)) * f27))) * 1.0)) + ((CB0[32].xyz + (CB0[33].xyz * f27)) * 1.0)) + vec3((f5.z * 2.0) * f11)) * f10) + (((((f40 * (((f41 + (f41 * f41)) / max(((f42 * f42) * ((f36 * 3.0) + 0.5)) * ((f35 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f33)) * CB0[15].xyz) * 1.0) * (f11 * VARYING0.w)) + ((mix(f49, textureLod(PrefilteredEnvTexture, f47, f46).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f18.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f27)) * f53) * f17));
    vec4 f65 = vec4(0.0);
    f65.x = f64.x;
    vec4 f66 = f65;
    f66.y = f64.y;

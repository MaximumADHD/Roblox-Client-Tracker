...
void main()
{
    ...
    vec4 f35 = texture(PrecomputedBRDFTexture, vec2(f25, f27));
    ...
    float f45 = f36.z;
    vec3 f46 = vec3(f43);
    vec3 f47 = mix(f42, f32, f46) * mix(vec3(1.0), f22, vec3(0.5));
    vec3 f48 = -CB0[16].xyz;
    float f49 = dot(f21, f48) * ((1.0 - ((step(f44.x, f45) * clamp(CB0[29].z + (CB0[29].w * abs(f45 - 0.5)), 0.0, 1.0)) * f44.y)) * f41.y);
    ...
    float f73 = f66.z;
    float f74 = 1.0 - f27;
    vec4 f75 = mix(vec4(mix(((((f42 * f17) + ((((vec3(1.0) - (f58 * f26)) * CB0[15].xyz) * f51) + (CB0[17].xyz * clamp(-f49, 0.0, 1.0)))) + (((vec3(1.0) - (f63 * f26)) * (((((((CB0[40].xyz * f68) + (CB0[42].xyz * f69)) + (CB0[44].xyz * f70)) + (CB0[41].xyz * f71)) + (CB0[43].xyz * f72)) + (CB0[45].xyz * f73)) + (((((((CB0[34].xyz * f68) + (CB0[36].xyz * f69)) + (CB0[38].xyz * f70)) + (CB0[35].xyz * f71)) + (CB0[37].xyz * f72)) + (CB0[39].xyz * f73)) * f43))) * f17)) + (CB0[32].xyz + (CB0[33].xyz * f43))) * (f22 * f17), f47, vec3(VARYING7.w)) * f17, f17), vec4(f47, 1.0), vec4(((f74 * f74) * 0.800000011920928955078125) * clamp(f16, 0.0, 1.0))) + vec4((((f58 * (((f59 + (f59 * f59)) / max(((f60 * f60) * ((f54 * 3.0) + 0.5)) * ((f53 * 0.75) + 0.25), 3.0999999580672010779380798339844e-05)) * f51)) * CB0[15].xyz) * 1.0) + (((mix(f34, f32, f46) * f63) * f26) * f17), 0.0);
    float f76 = clamp(exp2((CB0[18].z * f23) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f77 = textureLod(PrefilteredEnvTexture, vec4(f28, 0.0).xyz, max(CB0[18].y, f76) * 5.0).xyz;

...
void main()
{
    ...
    vec3 f2 = (texture(DiffuseMapTexture, VARYING0) * VARYING2).xyz;
    ...
    vec3 f16 = reflect(-f1, f14);
    float f17 = f15 * 5.0;
    vec3 f18 = vec4(f16, f17).xyz;
    vec4 f19 = texture(PrecomputedBRDFTexture, vec2(f15, max(9.9999997473787516355514526367188e-05, dot(f14, f1))));
    float f20 = VARYING6.w * f13;
    vec3 f21 = mix(vec3(0.039999999105930328369140625), f12, vec3(f20));
    vec3 f22 = -CB0[11].xyz;
    float f23 = dot(f14, f22) * ((1.0 - ((step(f10.x, f11) * clamp(CB0[24].z + (CB0[24].w * abs(f11 - 0.5)), 0.0, 1.0)) * f10.y)) * f8.y);
    vec3 f24 = normalize(f22 + f1);
    float f25 = clamp(f23, 0.0, 1.0);
    float f26 = f15 * f15;
    float f27 = max(0.001000000047497451305389404296875, dot(f14, f24));
    ...
    float f50 = f44.y;
    float f51 = f44.z;
    vec3 f52 = ((((((CB0[35].xyz * f46) + (CB0[37].xyz * f47)) + (CB0[39].xyz * f48)) + (CB0[36].xyz * f49)) + (CB0[38].xyz * f50)) + (CB0[40].xyz * f51)) + (((((((CB0[29].xyz * f46) + (CB0[31].xyz * f47)) + (CB0[33].xyz * f48)) + (CB0[30].xyz * f49)) + (CB0[32].xyz * f50)) + (CB0[34].xyz * f51)) * f9);
    vec3 f53 = (mix(textureLod(PrefilteredEnvIndoorTexture, f18, f17).xyz, textureLod(PrefilteredEnvTexture, f18, f17).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f16.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f9)) * f40) * f13;
    vec3 f54 = ((((((((f37 - (f32 * f36)) * CB0[10].xyz) * f25) + (CB0[12].xyz * (f35 * clamp(-f23, 0.0, 1.0)))) + ((f41 * f52) * CB0[25].w)) + ((CB0[27].xyz + (CB0[28].xyz * f9)) * 1.0)) * f12) + (((f32 * (((f33 + (f33 * f33)) / (((f34 * f34) * ((f28 * 3.0) + 0.5)) * ((f27 * 0.75) + 0.25))) * f25)) * CB0[10].xyz) + (f53 * f9))) + (((f7.xyz * (f7.w * 120.0)).xyz * mix(f12, f53 * (1.0 / (max(max(f52.x, f52.y), f52.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f41) * (f13 * (1.0 - f9)))) * 1.0);

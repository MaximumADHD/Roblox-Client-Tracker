...
void main()
{
    ...
    vec3 f24 = reflect(-f20, f8);
    vec3 f25 = -CB0[11].xyz;
    float f26 = dot(f8, f25) * ((1.0 - ((step(f18.x, f19) * clamp(CB0[24].z + (CB0[24].w * abs(f19 - 0.5)), 0.0, 1.0)) * f18.y)) * f15.y);
    vec3 f27 = normalize(f25 + f20);
    float f28 = clamp(f26, 0.0, 1.0);
    float f29 = f22 * f22;
    float f30 = max(0.001000000047497451305389404296875, dot(f8, f27));
    ...
    vec4 f40 = texture(PrecomputedBRDFTexture, vec2(f22, max(9.9999997473787516355514526367188e-05, dot(f8, f20))));
    float f41 = f40.x;
    float f42 = f40.y;

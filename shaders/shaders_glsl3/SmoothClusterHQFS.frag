...
void main()
{
    ...
    vec3 f19 = (((min(((f15.xyz * (f15.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f16.x), vec3(CB0[17].w)) + (((CB0[10].xyz * clamp(f11, 0.0, 1.0)) + (CB0[12].xyz * max(-f11, 0.0))) * f18)) + vec3((f9.z * 2.0) * f7)) * (f6 * f6).xyz) + (CB0[10].xyz * (((((step(0.0, f11) * f9.x) * f7) * CB3[0].z) * f18) * pow(clamp(dot(f8, normalize(f10 + normalize(VARYING7.xyz))), 0.0, 1.0), (clamp(f9.y, 0.0, 1.0) * 128.0) + 0.00999999977648258209228515625)));

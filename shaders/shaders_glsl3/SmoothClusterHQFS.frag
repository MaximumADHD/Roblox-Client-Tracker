...
void main()
{
    ...
    float f19 = (1.0 - ((step(f18.x, VARYING5.z) * clamp(CB0[25].z + (CB0[25].w * abs(VARYING5.z - 0.5)), 0.0, 1.0)) * f18.y)) * f17.y;
    vec3 f20 = (((min(((f16.xyz * (f16.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f17.x), vec3(CB0[17].w)) + (((CB0[10].xyz * clamp(f12, 0.0, 1.0)) + (CB0[12].xyz * max(-f12, 0.0))) * f19)) + vec3((f10.z * 2.0) * f8)) * mix(f6, f6 * f6, f7).xyz) + (CB0[10].xyz * (((((step(0.0, f12) * f10.x) * f8) * CB3[0].z) * f19) * pow(clamp(dot(f9, normalize(f11 + normalize(VARYING7.xyz))), 0.0, 1.0), (clamp(f10.y, 0.0, 1.0) * 128.0) + 0.00999999977648258209228515625)));
    vec4 f21 = vec4(f20.x, f20.y, f20.z, vec4(0.0).w);
    f21.w = 1.0;
    vec3 f22 = mix(CB0[14].xyz, mix(f21.xyz, sqrt(clamp(f21.xyz * CB0[15].z, vec3(0.0), vec3(1.0))), f7).xyz, vec3(clamp(VARYING4.w, 0.0, 1.0)));
    _entryPointOutput = vec4(f22.x, f22.y, f22.z, f21.w);
}

...
void main()
{
    ...
    vec2 f1 = (VARYING0 * CB2[0].x) * 4.0;
    vec2 f2 = f1 * 0.25;
    vec4 f3 = vec4(dFdx(f2), dFdy(f2));
    vec4 f4 = textureGrad(DiffuseMapTexture, (texture(WangTileMapTexture, f1 * vec2(0.0078125)).xy * 0.99609375) + (fract(f1) * 0.25), f3.xy, f3.zw);
    ...
    vec3 f29 = textureLod(PrefilteredEnvTexture, vec4(-(VARYING4.xyz / vec3(f0)), 0.0).xyz, max(CB0[18].y, f28) * 5.0).xyz;

...
void main()
{
    ...
    vec3 f20 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f10.w + CB2[3].w, 0.0, 1.0))) * f10.xyz) * (1.0 + (f15 * CB2[1].x))) * (texture(StudsMapTexture, f0).x * 2.0), VARYING2.w).xyz;
    ...
    float f50 = max(CB0[26].w, CB2[0].w) * f2;
    ...
    float f70 = f55.x;
    float f71 = f55.y;
    vec3 f72 = ((f57 * f70) + vec3(f71)) / vec3(f70 + f71);
    vec3 f73 = (vec3(1.0) - (f72 * f50)) * f69;
    vec3 f74 = f17 * f17;
    ...
    float f82 = f76.y;
    float f83 = f76.z;
    vec3 f84 = (mix(textureLod(PrefilteredEnvIndoorTexture, f54, f53).xyz * f26, textureLod(PrefilteredEnvTexture, f54, f53).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f52.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f27)) * f72) * f50;
    vec3 f85 = (((((((((vec3(1.0) - (f66 * f50)) * f69) * CB0[10].xyz) * f59) * f47) + (f73 * (((((((CB0[35].xyz * f78) + (CB0[37].xyz * f79)) + (CB0[39].xyz * f80)) + (CB0[36].xyz * f81)) + (CB0[38].xyz * f82)) + (CB0[40].xyz * f83)) + (((((((CB0[29].xyz * f78) + (CB0[31].xyz * f79)) + (CB0[33].xyz * f80)) + (CB0[30].xyz * f81)) + (CB0[32].xyz * f82)) + (CB0[34].xyz * f83)) * f27)))) + (CB0[27].xyz + (CB0[28].xyz * f27))) * f49) + (((((f66 * ((f67 + (f67 * f67)) / (((f68 * f68) * ((f62 * 3.0) + 0.5)) * ((f61 * 0.75) + 0.25)))) * CB0[10].xyz) * f59) * f47) + f84)) + (f26 * mix(f49, f84 * (1.0 / (max(max(f84.x, f84.y), f84.z) + 0.00999999977648258209228515625)), ((vec3(1.0) - f73) * f50) * (1.0 - f27)));
    vec4 f86 = vec4(f85.x, f85.y, f85.z, vec4(0.0).w);
    f86.w = VARYING2.w;
    vec2 f87 = min(VARYING0.wz, VARYING1.wz);
    float f88 = min(f87.x, f87.y) / f1;
    vec3 f89 = mix(CB0[14].xyz, (sqrt(clamp((f86.xyz * clamp((clamp((f1 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f88)) + f88, 0.0, 1.0)).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    _entryPointOutput = vec4(f89.x, f89.y, f89.z, f86.w);
}

...
void main()
{
    ...
    float f45 = (1.0 - ((step(f44.x, VARYING5.z) * clamp(CB0[25].z + (CB0[25].w * abs(VARYING5.z - 0.5)), 0.0, 1.0)) * f44.y)) * f43.y;
    vec3 f46 = (((min(((f42.xyz * (f42.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f43.x), vec3(CB0[17].w)) + (((CB0[10].xyz * clamp(f38, 0.0, 1.0)) + (CB0[12].xyz * max(-f38, 0.0))) * f45)) + vec3((f36.z * 2.0) * f30)) * (f29 * f29).xyz) + (CB0[10].xyz * (((((step(0.0, f38) * f36.x) * f30) * CB3[0].z) * f45) * pow(clamp(dot(f35, normalize(f37 + normalize(VARYING7.xyz))), 0.0, 1.0), (clamp(f36.y, 0.0, 1.0) * 128.0) + 0.00999999977648258209228515625)));

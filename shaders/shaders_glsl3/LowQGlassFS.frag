...
void main()
{
    ...
    vec3 f12 = mix(CB0[14].xyz, sqrt(clamp(((((VARYING6.xyz * f8) + min((f4.xyz * (f4.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f6)), vec3(CB0[16].w))) * ((mix(vec3(1.0), textureLod(PrefilteredEnvTexture, vec4(f9, f10).xyz, f10).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f9.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f6)) * VARYING7.w) + ((f0 * f0).xyz * (VARYING2.w - VARYING7.w)))) + (CB0[10].xyz * ((VARYING6.w * f8) * 0.100000001490116119384765625))).xyz * CB0[15].y, vec3(0.0), vec3(1.0))).xyz, vec3(f11));

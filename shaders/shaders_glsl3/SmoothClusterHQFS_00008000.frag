...
void main()
{
    ...
    float f61 = f55.y;
    float f62 = f55.z;
    vec3 f63 = ((((((f27.xyz * (f27.w * 120.0)) + ((((vec3(1.0) - (f42 * f19)) * CB0[15].xyz) * f35) + (CB0[17].xyz * clamp(-f33, 0.0, 1.0)))) + (((vec3(1.0) - (f52 * f19)) * (((((((CB0[40].xyz * f57) + (CB0[42].xyz * f58)) + (CB0[44].xyz * f59)) + (CB0[41].xyz * f60)) + (CB0[43].xyz * f61)) + (CB0[45].xyz * f62)) + (((((((CB0[34].xyz * f57) + (CB0[36].xyz * f58)) + (CB0[38].xyz * f59)) + (CB0[35].xyz * f60)) + (CB0[37].xyz * f61)) + (CB0[39].xyz * f62)) * f29))) * 1.0)) + ((CB0[32].xyz + (CB0[33].xyz * f29)) * 1.0)) + vec3((f6.z * 2.0) * f16)) * (f15 * f15)) + (((((f42 * (((f43 + (f43 * f43)) / max(((f44 * f44) * ((f38 * 3.0) + 0.5)) * ((f37 * 0.75) + 0.25), 7.0000001869630068540573120117188e-05)) * f35)) * CB0[15].xyz) * 1.0) * (f16 * VARYING0.w)) + ((mix(f48, textureLod(PrefilteredEnvTexture, f46, f45).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f20.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f29)) * f52) * f19));
    vec4 f64 = vec4(0.0);
    f64.x = f63.x;
    vec4 f65 = f64;
    f65.y = f63.y;

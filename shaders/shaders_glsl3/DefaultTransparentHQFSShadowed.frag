...
void main()
{
    ...
    vec3 f60 = f1 * f1;
    bvec3 f61 = lessThan(f1, vec3(0.0));
    ...
    vec3 f71 = ((((((((f55 - (f50 * f54)) * CB0[10].xyz) * f43) * f31) + (f59 * (((((((CB0[35].xyz * f64) + (CB0[37].xyz * f65)) + (CB0[39].xyz * f66)) + (CB0[36].xyz * f67)) + (CB0[38].xyz * f68)) + (CB0[40].xyz * f69)) + (((((((CB0[29].xyz * f64) + (CB0[31].xyz * f65)) + (CB0[33].xyz * f66)) + (CB0[30].xyz * f67)) + (CB0[32].xyz * f68)) + (CB0[34].xyz * f69)) * f11)))) + (CB0[27].xyz + (CB0[28].xyz * f11))) * f33) + ((((f50 * (((f51 + (f51 * f51)) / (((f52 * f52) * ((f46 * 3.0) + 0.5)) * ((f45 * 0.75) + 0.25))) * f43)) * CB0[10].xyz) * f31) + f70)) + (f10 * mix(f33, f70 * (1.0 / (max(max(f70.x, f70.y), f70.z) + 0.00999999977648258209228515625)), (vec3(1.0) - f59) * (f34 * (1.0 - f11))));
    float f72 = f0.w;
    vec4 f73 = vec4(f71.x, f71.y, f71.z, vec4(0.0).w);
    f73.w = f72;
    vec3 f74 = mix(CB0[14].xyz, (sqrt(clamp(f73.xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    vec4 f75 = vec4(f74.x, f74.y, f74.z, f73.w);
    f75.w = f72;

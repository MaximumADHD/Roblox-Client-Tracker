...
void main()
{
    ...
    float f21 = f20.y;
    vec3 f22 = VARYING7.xyz - CB0[25].xyz;
    vec3 f23 = VARYING7.xyz - CB0[26].xyz;
    vec3 f24 = VARYING7.xyz - CB0[27].xyz;
    vec4 f25 = vec4(VARYING7.xyz, 1.0) * mat4(CB8[((dot(f22, f22) < CB0[25].w) ? 0 : ((dot(f23, f23) < CB0[26].w) ? 1 : ((dot(f24, f24) < CB0[27].w) ? 2 : 3))) * 4 + 0], CB8[((dot(f22, f22) < CB0[25].w) ? 0 : ((dot(f23, f23) < CB0[26].w) ? 1 : ((dot(f24, f24) < CB0[27].w) ? 2 : 3))) * 4 + 1], CB8[((dot(f22, f22) < CB0[25].w) ? 0 : ((dot(f23, f23) < CB0[26].w) ? 1 : ((dot(f24, f24) < CB0[27].w) ? 2 : 3))) * 4 + 2], CB8[((dot(f22, f22) < CB0[25].w) ? 0 : ((dot(f23, f23) < CB0[26].w) ? 1 : ((dot(f24, f24) < CB0[27].w) ? 2 : 3))) * 4 + 3]);
    ...
    float f30 = exp(CB0[29].z * f29);
    float f31 = -exp((-CB0[29].w) * f29);
    vec2 f32 = (f28 * CB0[30].y) * vec2(f30, f31);
    vec2 f33 = f32 * f32;
    float f34 = f26.x;
    ...
    float f39 = f31 - f37;
    float f40 = (f14 > 0.0) ? mix(f21, mix(min((f30 <= f34) ? 1.0 : clamp(((f35 / (f35 + (f36 * f36))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f31 <= f37) ? 1.0 : clamp(((f38 / (f38 + (f39 * f39))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f21, clamp((length(VARYING7.xyz - CB0[7].xyz) * CB0[29].y) - (CB0[29].x * CB0[29].y), 0.0, 1.0)), CB0[30].x) : 0.0;
    vec3 f41 = texture(EnvironmentMapTexture, reflect(-VARYING4.xyz, f12)).xyz;
    vec3 f42 = ((min(((f19.xyz * (f19.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f20.x), vec3(CB0[16].w)) + (((CB0[10].xyz * clamp(f14, 0.0, 1.0)) + (CB0[12].xyz * max(-f14, 0.0))) * f40)) * mix((f15 * f15).xyz, (f41 * f41) * CB0[15].x, vec3((f10.y * f2) * CB2[0].w)).xyz) + (CB0[10].xyz * (((step(0.0, f14) * f11.x) * f40) * pow(clamp(dot(f12, normalize(f13 + normalize(VARYING4.xyz))), 0.0, 1.0), f11.y)));

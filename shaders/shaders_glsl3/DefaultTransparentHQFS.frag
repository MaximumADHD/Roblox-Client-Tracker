...
void main()
{
    ...
    float f48 = f42.y;
    float f49 = f42.z;
    vec3 f50 = (mix(textureLod(PrefilteredEnvIndoorTexture, f18, f17).xyz * f7, textureLod(PrefilteredEnvTexture, f18, f17).xyz * mix(CB0[26].xyz, CB0[25].xyz, vec3(clamp(f16.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f8)) * f38) * f13;
    ...
    _entryPointOutput = vec4(f53.x, f53.y, f53.z, f52.w);
}

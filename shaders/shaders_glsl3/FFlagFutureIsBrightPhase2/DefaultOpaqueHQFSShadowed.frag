...
void main()
{
    ...
    float f27 = f19 - f25;
    float f28 = (dot(f0, f1) > 0.0) ? mix(f9, mix(min((f18 <= f22) ? 1.0 : clamp(((f23 / (f23 + (f24 * f24))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f19 <= f25) ? 1.0 : clamp(((f26 / (f26 + (f27 * f27))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f9, clamp((length(VARYING7.xyz - CB0[7].xyz) * CB0[30].y) - (CB0[30].x * CB0[30].y), 0.0, 1.0)), CB0[31].x) : 0.0;
    vec3 f29 = ((min(((f7.xyz * (f7.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f8.x), vec3(CB0[17].w)) + (VARYING6.xyz * f28)) * mix(f2, f2 * f2, f3).xyz) + (CB0[10].xyz * ((VARYING6.w * f28) * pow(clamp(dot(f0, normalize(f1 + normalize(VARYING4.xyz))), 0.0, 1.0), VARYING5.w)));
    vec4 f30 = vec4(f29.x, f29.y, f29.z, vec4(0.0).w);
    f30.w = 1.0;
    vec3 f31 = mix(CB0[14].xyz, mix(f30.xyz, sqrt(clamp(f30.xyz * CB0[15].z, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125)))))), f3).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));

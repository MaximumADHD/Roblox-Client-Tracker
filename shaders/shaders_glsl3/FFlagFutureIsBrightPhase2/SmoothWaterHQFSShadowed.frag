...
void main()
{
    ...
    vec3 f38 = vec3(CB0[15].x);
    vec3 f39 = mix(CB3[1].xyz * (min((f7 + CB0[8].xyz) + (CB0[9].xyz * f8), vec3(CB0[17].w)) + (CB0[10].xyz * f28)), (mix(f37, (f37 * f37) * CB0[15].w, f38) * f8) + (f7 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f35, f36))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (CB0[10].xyz * ((((2.0 * clamp(1.0 - (VARYING7.w * CB0[24].y), 0.0, 1.0)) * CB3[2].z) * f28) * pow(clamp(dot(f34, normalize(f1 + f36)), 0.0, 1.0), 900.0)));
    vec4 f40 = vec4(f39.x, f39.y, f39.z, vec4(0.0).w);
    f40.w = 1.0;
    vec3 f41 = mix(CB0[14].xyz, mix(f40.xyz, sqrt(clamp(f40.xyz * CB0[15].z, vec3(0.0), vec3(1.0))), f38).xyz, vec3(clamp(VARYING5.w, 0.0, 1.0)));

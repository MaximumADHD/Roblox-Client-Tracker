...
void main()
{
    ...
    vec3 f14 = -CB0[11].xyz;
    float f15 = dot(VARYING6, f14);
    float f16 = clamp(dot(step(CB0[20].xyz, abs(VARYING4.xyz - CB0[19].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    float f21 = f20.y;
    vec3 f22 = VARYING5 - CB0[26].xyz;
    vec3 f23 = VARYING5 - CB0[27].xyz;
    vec3 f24 = VARYING5 - CB0[28].xyz;
    vec4 f25 = vec4(VARYING5, 1.0) * mat4(CB8[((dot(f22, f22) < CB0[26].w) ? 0 : ((dot(f23, f23) < CB0[27].w) ? 1 : ((dot(f24, f24) < CB0[28].w) ? 2 : 3))) * 4 + 0], CB8[((dot(f22, f22) < CB0[26].w) ? 0 : ((dot(f23, f23) < CB0[27].w) ? 1 : ((dot(f24, f24) < CB0[28].w) ? 2 : 3))) * 4 + 1], CB8[((dot(f22, f22) < CB0[26].w) ? 0 : ((dot(f23, f23) < CB0[27].w) ? 1 : ((dot(f24, f24) < CB0[28].w) ? 2 : 3))) * 4 + 2], CB8[((dot(f22, f22) < CB0[26].w) ? 0 : ((dot(f23, f23) < CB0[27].w) ? 1 : ((dot(f24, f24) < CB0[28].w) ? 2 : 3))) * 4 + 3]);
    ...
    float f39 = f31 - f37;
    float f40 = (f15 > 0.0) ? mix(f21, mix(min((f30 <= f34) ? 1.0 : clamp(((f35 / (f35 + (f36 * f36))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f31 <= f37) ? 1.0 : clamp(((f38 / (f38 + (f39 * f39))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f21, clamp((length(VARYING5 - CB0[7].xyz) * CB0[30].y) - (CB0[30].x * CB0[30].y), 0.0, 1.0)), CB0[31].x) : 0.0;
    vec3 f41 = (((min(((f19.xyz * (f19.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f20.x), vec3(CB0[17].w)) + (((CB0[10].xyz * clamp(f15, 0.0, 1.0)) + (CB0[12].xyz * max(-f15, 0.0))) * f40)) + vec3((f13.z * 2.0) * f12)) * mix(f10, f10 * f10, f11).xyz) + (CB0[10].xyz * (((((step(0.0, f15) * f13.x) * f12) * CB3[0].z) * f40) * pow(clamp(dot(VARYING6, normalize(f14 + normalize(VARYING7.xyz))), 0.0, 1.0), (clamp(f13.y, 0.0, 1.0) * 128.0) + 0.00999999977648258209228515625)));
    vec4 f42 = vec4(f41.x, f41.y, f41.z, vec4(0.0).w);
    f42.w = 1.0;
    vec3 f43 = mix(CB0[14].xyz, mix(f42.xyz, sqrt(clamp(f42.xyz * CB0[15].z, vec3(0.0), vec3(1.0))), f11).xyz, vec3(clamp(VARYING4.w, 0.0, 1.0)));

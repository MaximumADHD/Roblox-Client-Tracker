...
void main()
{
    ...
    vec3 f1 = -CB0[11].xyz;
    float f2 = clamp(dot(step(CB0[20].xyz, abs(VARYING5.xyz - CB0[19].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    float f27 = f19 - f25;
    float f28 = (dot(VARYING6, f1) > 0.0) ? mix(f9, mix(min((f18 <= f22) ? 1.0 : clamp(((f23 / (f23 + (f24 * f24))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f19 <= f25) ? 1.0 : clamp(((f26 / (f26 + (f27 * f27))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f9, clamp((length(f0 - CB0[7].xyz) * CB0[30].y) - (CB0[30].x * CB0[30].y), 0.0, 1.0)), CB0[31].x) : 0.0;
    ...
    vec3 f34 = normalize(((mix(vec3(VARYING6.z, 0.0, f30), vec3(VARYING6.y, f30, 0.0), f33) * f32.x) + (mix(vec3(0.0, -1.0, 0.0), vec3(0.0, -VARYING6.z, VARYING6.y), f33) * f32.y)) + (VARYING6 * sqrt(clamp(1.0 + dot(vec2(1.0) - f31, f32), 0.0, 1.0))));
    vec3 f35 = mix(VARYING6, f34, vec3(0.25));
    ...
    vec3 f43 = f42.xyz;
    vec3 f44 = vec3(CB0[15].x);
    vec3 f45 = reflect(-f36, f35);
    ...
    vec3 f80 = mix(mix(mix(f43, (f43 * f43) * CB0[15].y, f44).xyz, CB3[1].xyz * (min((f7 + CB0[8].xyz) + (CB0[9].xyz * f8), vec3(CB0[17].w)) + (CB0[10].xyz * f28)), vec3(clamp(clamp(((f42.w - VARYING8.w) * CB3[2].x) + CB3[2].y, 0.0, 1.0) + clamp((VARYING8.w * 0.0040000001899898052215576171875) - 1.0, 0.0, 1.0), 0.0, 1.0))), mix(mix(f78, (f78 * f78) * CB0[15].w, f44) * f8, mix(f79, (f79 * f79) * CB0[15].y, f44), vec3((((float(abs(f77.x - 0.5) < 0.550000011920928955078125) * float(abs(f77.y - 0.5) < 0.5)) * clamp(3.900000095367431640625 - (max(VARYING8.w, f76) * 0.008000000379979610443115234375), 0.0, 1.0)) * float(abs((texture(GBufferDepthTexture, f73.xy / vec2(f74)).x * 500.0) - f74) < 10.0)) * float(f48 > 0.0))) + (f7 * 0.100000001490116119384765625), vec3(((clamp(0.7799999713897705078125 - (2.5 * abs(dot(f35, f36))), 0.0, 1.0) + 0.300000011920928955078125) * VARYING0.w) * CB3[2].z)) + (CB0[10].xyz * ((((2.0 * clamp(1.0 - (VARYING7.w * CB0[24].y), 0.0, 1.0)) * CB3[2].z) * f28) * pow(clamp(dot(f34, normalize(f1 + f36)), 0.0, 1.0), 900.0)));
    vec4 f81 = vec4(f80.x, f80.y, f80.z, vec4(0.0).w);
    f81.w = 1.0;
    vec3 f82 = mix(CB0[14].xyz, mix(f81.xyz, sqrt(clamp(f81.xyz * CB0[15].z, vec3(0.0), vec3(1.0))), f44).xyz, vec3(clamp(VARYING5.w, 0.0, 1.0)));

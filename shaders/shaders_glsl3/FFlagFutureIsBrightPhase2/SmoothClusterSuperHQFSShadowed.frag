...
void main()
{
    ...
    vec3 f42 = (((min(((f20.xyz * (f20.w * 120.0)).xyz + CB0[8].xyz) + (CB0[9].xyz * f21.x), vec3(CB0[17].w)) + (((CB0[10].xyz * clamp(f16, 0.0, 1.0)) + (CB0[12].xyz * max(-f16, 0.0))) * f41)) + vec3((f14.z * 2.0) * f8)) * mix(f6, f6 * f6, f7).xyz) + (CB0[10].xyz * (((((step(0.0, f16) * f14.x) * f8) * CB3[0].z) * f41) * pow(clamp(dot(f13, normalize(f15 + normalize(VARYING7.xyz))), 0.0, 1.0), (clamp(f14.y, 0.0, 1.0) * 128.0) + 0.00999999977648258209228515625)));
    vec4 f43 = vec4(f42.x, f42.y, f42.z, vec4(0.0).w);
    f43.w = 1.0;
    vec3 f44 = mix(CB0[14].xyz, mix(f43.xyz, sqrt(clamp(f43.xyz * CB0[15].z, vec3(0.0), vec3(1.0))), f7).xyz, vec3(clamp(VARYING4.w, 0.0, 1.0)));

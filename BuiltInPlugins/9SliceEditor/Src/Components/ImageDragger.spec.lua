return function()
	...
	it("should have correct position for a vertical dragger", function ()
		withTestComponent(ImageDragger, {
			...
			local imageButton = container:FindFirstChildOfClass("ImageButton")
			expect(imageButton.Position.X.Scale).to.be.near(16/100)
		end)
	end)
	...
	it("should have correct position for a horizontal dragger", function ()
		withTestComponent(ImageDragger, {
			...
			local imageButton = container:FindFirstChildOfClass("ImageButton")
			expect(imageButton.Position.Y.Scale).to.be.near(20/100)
		end)
	end)
	...
	it("horizontal dragger position should be within parent bounds", function ()
		withTestComponent(ImageDragger, {
			...
			expect(imageButton.Position.Y.Scale).to.be.near(0)
	
			updatePropsFunc({
				sliceRect = {10, 10, 280, 10},
			})
			expect(imageButton.Position.Y.Scale).to.be.near(1)
	...
	it("vertical dragger position should be within parent bounds", function ()
		withTestComponent(ImageDragger, {
			...
			expect(imageButton.Position.X.Scale).to.be.near(0)
	
			updatePropsFunc({
				sliceRect = {500, 10, 280, 10},
			})
			expect(imageButton.Position.X.Scale).to.be.near(1)

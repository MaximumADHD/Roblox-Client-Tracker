...
function FreeformDragger:_updatePhysical()
	...
	local cameraPlaneDragTarget = DragHelper.getCameraPlaneDragTarget(
		self._draggerToolModel._draggerContext:getMouseRay(),
	...
	if cameraPlaneDragTarget then
		...
		local actualGlobalTransformUsed =
			self._partMover:moveToWithIk(cameraPlaneDragTarget, collisionsMode)

...
return function()
    ...
    describe("DraftOutdatedStateChanged", function()
        it("has a DraftOutdatedStateChanged signal", function()
            local mockService = MockDraftsService.new(TEST_CASES.DEFAULT)
            local connection = mockService.DraftOutdatedStateChanged:connect(function(script, isOutdated)
            end)
            connection.disconnect()
        end)
    end)

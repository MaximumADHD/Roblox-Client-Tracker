PROTO_0:
  LOADB R2 0
  GETTABLEKS R3 R1 K0 ["Name"]
  JUMPIFNOTEQKS R3 K1 ["AnimSaves"] [+10]
  LOADK R4 K2 ["Model"]
  NAMECALL R2 R1 K3 ["IsA"]
  CALL R2 2 1
  JUMPIF R2 [+4]
  LOADK R4 K4 ["ObjectValue"]
  NAMECALL R2 R1 K3 ["IsA"]
  CALL R2 2 1
  JUMPIF R2 [+20]
  GETIMPORT R3 K6 [ipairs]
  NAMECALL R4 R1 K7 ["GetChildren"]
  CALL R4 1 -1
  CALL R3 -1 3
  FORGPREP_INEXT R3
  FASTCALL2 TABLE_INSERT R0 R7 [+5]
  MOVE R9 R0
  MOVE R10 R7
  GETIMPORT R8 K10 [table.insert]
  CALL R8 2 0
  GETUPVAL R8 0
  MOVE R9 R0
  MOVE R10 R7
  CALL R8 2 0
  FORGLOOP R3 2 [inext] [-12]
  RETURN R0 1

PROTO_1:
  GETIMPORT R1 K1 [game]
  GETTABLEKS R0 R1 K2 ["Workspace"]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K3 ["TEMPORARY_CONSTRAINTS"]
  NAMECALL R0 R0 K4 ["FindFirstChild"]
  CALL R0 2 -1
  RETURN R0 -1

PROTO_2:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["getTemporaryConstraints"]
  CALL R1 0 1
  JUMPIFNOTEQKNIL R1 [+2]
  LOADB R0 0 +1
  LOADB R0 1
  RETURN R0 1

PROTO_3:
  NEWTABLE R1 0 0
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["getTemporaryConstraints"]
  CALL R3 0 1
  OR R2 R3 R0
  GETUPVAL R3 1
  NEWTABLE R4 0 0
  MOVE R5 R2
  CALL R3 2 1
  GETIMPORT R4 K2 [ipairs]
  MOVE R5 R3
  CALL R4 1 3
  FORGPREP_INEXT R4
  LOADK R11 K3 ["BallSocketConstraint"]
  NAMECALL R9 R8 K4 ["IsA"]
  CALL R9 2 1
  JUMPIF R9 [+5]
  LOADK R11 K5 ["HingeConstraint"]
  NAMECALL R9 R8 K4 ["IsA"]
  CALL R9 2 1
  JUMPIFNOT R9 [+7]
  FASTCALL2 TABLE_INSERT R1 R8 [+5]
  MOVE R10 R1
  MOVE R11 R8
  GETIMPORT R9 K8 [table.insert]
  CALL R9 2 0
  FORGLOOP R4 2 [inext] [-18]
  RETURN R1 1

PROTO_4:
  NEWTABLE R1 0 0
  GETIMPORT R2 K1 [ipairs]
  GETUPVAL R3 0
  CALL R2 1 3
  FORGPREP_INEXT R2
  GETTABLEKS R8 R6 K2 ["Part0"]
  GETTABLEKS R7 R8 K3 ["Name"]
  JUMPIFNOTEQ R7 R0 [+11]
  GETTABLEKS R10 R6 K4 ["Part1"]
  GETTABLEKS R9 R10 K3 ["Name"]
  FASTCALL2 TABLE_INSERT R1 R9 [+4]
  MOVE R8 R1
  GETIMPORT R7 K7 [table.insert]
  CALL R7 2 0
  FORGLOOP R2 2 [inext] [-17]
  GETIMPORT R2 K1 [ipairs]
  GETUPVAL R3 1
  CALL R2 1 3
  FORGPREP_INEXT R2
  GETUPVAL R8 2
  GETTABLEKS R7 R8 K8 ["getPart0"]
  MOVE R8 R6
  CALL R7 1 1
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K9 ["getPart1"]
  MOVE R9 R6
  CALL R8 1 1
  JUMPIFNOT R7 [+13]
  JUMPIFNOT R8 [+12]
  GETTABLEKS R9 R7 K3 ["Name"]
  JUMPIFNOTEQ R9 R0 [+9]
  GETTABLEKS R11 R8 K3 ["Name"]
  FASTCALL2 TABLE_INSERT R1 R11 [+4]
  MOVE R10 R1
  GETIMPORT R9 K7 [table.insert]
  CALL R9 2 0
  FORGLOOP R2 2 [inext] [-25]
  GETIMPORT R2 K1 [ipairs]
  GETUPVAL R3 3
  CALL R2 1 3
  FORGPREP_INEXT R2
  GETTABLEKS R8 R6 K10 ["Parent"]
  GETTABLEKS R7 R8 K3 ["Name"]
  JUMPIFNOTEQ R7 R0 [+9]
  GETTABLEKS R9 R6 K3 ["Name"]
  FASTCALL2 TABLE_INSERT R1 R9 [+4]
  MOVE R8 R1
  GETIMPORT R7 K7 [table.insert]
  CALL R7 2 0
  FORGLOOP R2 2 [inext] [-15]
  RETURN R1 1

PROTO_5:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["findRootPart"]
  MOVE R2 R0
  CALL R1 1 1
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K1 ["getMotors"]
  MOVE R3 R0
  CALL R2 1 1
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["getBones"]
  MOVE R4 R0
  CALL R3 1 1
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K3 ["getAnimationConstraints"]
  MOVE R5 R0
  CALL R4 1 1
  FASTCALL2K ASSERT R1 K4 [+5]
  MOVE R6 R1
  LOADK R7 K4 ["Rig is missing a root part."]
  GETIMPORT R5 K6 [assert]
  CALL R5 2 0
  GETTABLEKS R5 R1 K7 ["Name"]
  NEWTABLE R6 1 0
  GETUPVAL R7 1
  MOVE R8 R5
  NEWCLOSURE R9 P0
  CAPTURE VAL R2
  CAPTURE VAL R4
  CAPTURE UPVAL U0
  CAPTURE VAL R3
  CALL R7 2 1
  SETTABLE R7 R6 R5
  RETURN R6 1

PROTO_6:
  LOADK R3 K0 ["Humanoid"]
  NAMECALL R1 R0 K1 ["FindFirstChildOfClass"]
  CALL R1 2 1
  JUMPIFNOT R1 [+11]
  LOADK R4 K0 ["Humanoid"]
  NAMECALL R2 R1 K2 ["IsA"]
  CALL R2 2 1
  JUMPIFNOT R2 [+6]
  GETTABLEKS R2 R1 K3 ["RigType"]
  GETIMPORT R3 K7 [Enum.HumanoidRigType.R15]
  JUMPIFEQ R2 R3 [+3]
  LOADB R2 0
  RETURN R2 1
  GETTABLEKS R2 R1 K8 ["Parent"]
  JUMPIFNOT R2 [+5]
  LOADK R5 K9 ["Model"]
  NAMECALL R3 R2 K2 ["IsA"]
  CALL R3 2 1
  JUMPIF R3 [+2]
  LOADB R3 0
  RETURN R3 1
  LOADB R3 1
  RETURN R3 1

PROTO_7:
  LOADK R3 K0 ["Humanoid"]
  NAMECALL R1 R0 K1 ["FindFirstChildOfClass"]
  CALL R1 2 1
  JUMPIFNOT R1 [+11]
  LOADK R4 K0 ["Humanoid"]
  NAMECALL R2 R1 K2 ["IsA"]
  CALL R2 2 1
  JUMPIFNOT R2 [+6]
  GETTABLEKS R2 R1 K3 ["RigType"]
  GETIMPORT R3 K7 [Enum.HumanoidRigType.R6]
  JUMPIFEQ R2 R3 [+3]
  LOADB R2 0
  RETURN R2 1
  GETTABLEKS R2 R1 K8 ["Parent"]
  JUMPIFNOT R2 [+5]
  LOADK R5 K9 ["Model"]
  NAMECALL R3 R2 K2 ["IsA"]
  CALL R3 2 1
  JUMPIF R3 [+2]
  LOADB R3 0
  RETURN R3 1
  LOADB R3 1
  RETURN R3 1

PROTO_8:
  LOADNIL R1
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["isR6Humanoid"]
  MOVE R3 R0
  CALL R2 1 1
  JUMPIFNOT R2 [+9]
  LOADK R4 K1 ["HumanoidRootPart"]
  NAMECALL R2 R0 K2 ["FindFirstChild"]
  CALL R2 2 1
  JUMPIF R2 [+2]
  GETTABLEKS R2 R0 K3 ["PrimaryPart"]
  MOVE R1 R2
  JUMP [+8]
  GETTABLEKS R2 R0 K3 ["PrimaryPart"]
  JUMPIF R2 [+4]
  LOADK R4 K1 ["HumanoidRootPart"]
  NAMECALL R2 R0 K2 ["FindFirstChild"]
  CALL R2 2 1
  MOVE R1 R2
  JUMPIFNOT R1 [+1]
  RETURN R1 1
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K4 ["getRigInfo"]
  MOVE R3 R0
  CALL R2 1 1
  GETTABLEKS R3 R2 K5 ["PartNameToMotor"]
  GETTABLEKS R4 R2 K6 ["PartNameToAnimConstraint"]
  GETTABLEKS R5 R2 K7 ["BoneNameToBone"]
  LOADNIL R6
  GETIMPORT R7 K9 [next]
  MOVE R8 R3
  JUMPIF R8 [+2]
  NEWTABLE R8 0 0
  CALL R7 1 1
  JUMPIFNOT R7 [+13]
  JUMPIF R6 [+12]
  GETTABLE R8 R3 R7
  JUMPIFNOT R8 [+8]
  GETTABLEKS R9 R8 K10 ["Part0"]
  JUMPIFNOT R9 [+5]
  GETTABLEKS R9 R8 K10 ["Part0"]
  GETTABLEKS R7 R9 K11 ["Name"]
  JUMP [+1]
  MOVE R6 R7
  JUMPBACK [-13]
  GETIMPORT R8 K9 [next]
  MOVE R9 R5
  JUMPIF R9 [+2]
  NEWTABLE R9 0 0
  CALL R8 1 1
  JUMPIFNOT R8 [+10]
  JUMPIF R6 [+9]
  GETTABLE R9 R5 R8
  JUMPIFNOT R9 [+5]
  GETTABLEKS R10 R9 K12 ["Parent"]
  GETTABLEKS R8 R10 K11 ["Name"]
  JUMP [+1]
  MOVE R6 R8
  JUMPBACK [-10]
  JUMPIFNOT R4 [+18]
  GETIMPORT R9 K9 [next]
  MOVE R10 R4
  CALL R9 1 1
  JUMPIFNOT R9 [+13]
  JUMPIF R6 [+12]
  GETTABLE R10 R4 R9
  GETUPVAL R12 0
  GETTABLEKS R11 R12 K13 ["getPart0"]
  MOVE R12 R10
  CALL R11 1 1
  JUMPIFNOT R11 [+3]
  GETTABLEKS R9 R11 K11 ["Name"]
  JUMP [+1]
  MOVE R6 R9
  JUMPBACK [-13]
  GETUPVAL R9 1
  NEWTABLE R10 0 0
  MOVE R11 R0
  CALL R9 2 1
  GETIMPORT R10 K15 [ipairs]
  MOVE R11 R9
  CALL R10 1 3
  FORGPREP_INEXT R10
  GETTABLEKS R15 R14 K11 ["Name"]
  JUMPIFNOTEQ R15 R6 [+7]
  LOADK R17 K16 ["BasePart"]
  NAMECALL R15 R14 K17 ["IsA"]
  CALL R15 2 1
  JUMPIFNOT R15 [+1]
  RETURN R14 1
  FORGLOOP R10 2 [inext] [-11]
  LOADNIL R10
  RETURN R10 1

PROTO_9:
  LOADK R3 K0 ["Humanoid"]
  NAMECALL R1 R0 K1 ["FindFirstChildOfClass"]
  CALL R1 2 1
  JUMPIF R1 [+4]
  LOADK R3 K2 ["AnimationController"]
  NAMECALL R1 R0 K1 ["FindFirstChildOfClass"]
  CALL R1 2 1
  RETURN R1 1

PROTO_10:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["getAnimationController"]
  MOVE R2 R0
  CALL R1 1 1
  JUMPIF R1 [+2]
  LOADNIL R2
  RETURN R2 1
  LOADK R4 K1 ["Animator"]
  NAMECALL R2 R1 K2 ["FindFirstChildOfClass"]
  CALL R2 2 1
  JUMPIF R2 [+5]
  GETIMPORT R2 K5 [Instance.new]
  LOADK R3 K1 ["Animator"]
  MOVE R4 R1
  CALL R2 2 1
  RETURN R2 1

PROTO_11:
  MOVE R1 R0
  JUMPIFNOT R1 [+7]
  GETTABLEKS R1 R0 K0 ["Attachment0"]
  JUMPIFNOT R1 [+4]
  GETTABLEKS R2 R0 K0 ["Attachment0"]
  GETTABLEKS R1 R2 K1 ["Parent"]
  JUMPIFNOT R1 [+15]
  LOADK R5 K2 ["BasePart"]
  NAMECALL R3 R1 K3 ["IsA"]
  CALL R3 2 1
  JUMPIF R3 [+8]
  GETUPVAL R3 0
  CALL R3 0 1
  JUMPIFNOT R3 [+7]
  LOADK R5 K4 ["Attachment"]
  NAMECALL R3 R1 K3 ["IsA"]
  CALL R3 2 1
  JUMPIFNOT R3 [+2]
  MOVE R2 R1
  RETURN R2 1
  LOADNIL R2
  RETURN R2 1

PROTO_12:
  MOVE R1 R0
  JUMPIFNOT R1 [+7]
  GETTABLEKS R1 R0 K0 ["Attachment1"]
  JUMPIFNOT R1 [+4]
  GETTABLEKS R2 R0 K0 ["Attachment1"]
  GETTABLEKS R1 R2 K1 ["Parent"]
  JUMPIFNOT R1 [+15]
  LOADK R5 K2 ["BasePart"]
  NAMECALL R3 R1 K3 ["IsA"]
  CALL R3 2 1
  JUMPIF R3 [+8]
  GETUPVAL R3 0
  CALL R3 0 1
  JUMPIFNOT R3 [+7]
  LOADK R5 K4 ["Attachment"]
  NAMECALL R3 R1 K3 ["IsA"]
  CALL R3 2 1
  JUMPIFNOT R3 [+2]
  MOVE R2 R1
  RETURN R2 1
  LOADNIL R2
  RETURN R2 1

PROTO_13:
  NEWTABLE R1 0 0
  GETUPVAL R2 0
  NEWTABLE R3 0 0
  MOVE R4 R0
  CALL R2 2 1
  GETIMPORT R3 K1 [ipairs]
  MOVE R4 R2
  CALL R3 1 3
  FORGPREP_INEXT R3
  LOADK R10 K2 ["Motor6D"]
  NAMECALL R8 R7 K3 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+7]
  FASTCALL2 TABLE_INSERT R1 R7 [+5]
  MOVE R9 R1
  MOVE R10 R7
  GETIMPORT R8 K6 [table.insert]
  CALL R8 2 0
  FORGLOOP R3 2 [inext] [-13]
  RETURN R1 1

PROTO_14:
  NEWTABLE R1 0 0
  GETUPVAL R2 0
  NEWTABLE R3 0 0
  MOVE R4 R0
  CALL R2 2 1
  GETIMPORT R3 K1 [ipairs]
  MOVE R4 R2
  CALL R3 1 3
  FORGPREP_INEXT R3
  LOADK R10 K2 ["Bone"]
  NAMECALL R8 R7 K3 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+7]
  FASTCALL2 TABLE_INSERT R1 R7 [+5]
  MOVE R9 R1
  MOVE R10 R7
  GETIMPORT R8 K6 [table.insert]
  CALL R8 2 0
  FORGLOOP R3 2 [inext] [-13]
  RETURN R1 1

PROTO_15:
  NEWTABLE R1 0 0
  GETUPVAL R2 0
  NEWTABLE R3 0 0
  MOVE R4 R0
  CALL R2 2 1
  GETIMPORT R3 K1 [ipairs]
  MOVE R4 R2
  CALL R3 1 3
  FORGPREP_INEXT R3
  LOADK R10 K2 ["AnimationConstraint"]
  NAMECALL R8 R7 K3 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+7]
  FASTCALL2 TABLE_INSERT R1 R7 [+5]
  MOVE R9 R1
  MOVE R10 R7
  GETIMPORT R8 K6 [table.insert]
  CALL R8 2 0
  FORGLOOP R3 2 [inext] [-13]
  RETURN R1 1

PROTO_16:
  GETTABLEKS R3 R0 K0 ["Name"]
  GETTABLEKS R4 R1 K0 ["Name"]
  JUMPIFLT R3 R4 [+2]
  LOADB R2 0 +1
  LOADB R2 1
  RETURN R2 1

PROTO_17:
  NEWTABLE R2 0 0
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["findRootPart"]
  MOVE R4 R0
  CALL R3 1 1
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K1 ["getRigInfo"]
  MOVE R5 R0
  CALL R4 1 1
  GETTABLEKS R5 R4 K2 ["Parts"]
  GETTABLEKS R6 R4 K3 ["PartNameToMotor"]
  GETTABLEKS R7 R4 K4 ["PartNameToAnimConstraint"]
  GETTABLEKS R8 R4 K5 ["BoneNameToBone"]
  GETIMPORT R9 K7 [ipairs]
  MOVE R10 R5
  CALL R9 1 3
  FORGPREP_INEXT R9
  GETTABLEKS R15 R13 K8 ["Name"]
  GETTABLE R14 R8 R15
  JUMPIF R14 [+11]
  JUMPIFEQ R13 R3 [+48]
  GETTABLEKS R15 R13 K8 ["Name"]
  GETTABLE R14 R6 R15
  JUMPIF R14 [+5]
  JUMPIFNOT R7 [+42]
  GETTABLEKS R15 R13 K8 ["Name"]
  GETTABLE R14 R7 R15
  JUMPIFNOT R14 [+38]
  LOADB R14 0
  GETIMPORT R15 K7 [ipairs]
  MOVE R16 R1
  CALL R15 1 3
  FORGPREP_INEXT R15
  GETTABLEKS R20 R13 K8 ["Name"]
  GETTABLEKS R21 R19 K8 ["Name"]
  JUMPIFNOTEQ R20 R21 [+3]
  LOADB R14 1
  JUMP [+2]
  FORGLOOP R15 2 [inext] [-9]
  JUMPIF R14 [+21]
  DUPTABLE R17 K11 [{"Name", "Instance", "Type"}]
  GETTABLEKS R18 R13 K8 ["Name"]
  SETTABLEKS R18 R17 K8 ["Name"]
  LOADK R18 K12 ["Root"]
  SETTABLEKS R18 R17 K9 ["Instance"]
  GETUPVAL R20 1
  GETTABLEKS R19 R20 K13 ["TRACK_TYPES"]
  GETTABLEKS R18 R19 K14 ["CFrame"]
  SETTABLEKS R18 R17 K10 ["Type"]
  FASTCALL2 TABLE_INSERT R2 R17 [+4]
  MOVE R16 R2
  GETIMPORT R15 K17 [table.insert]
  CALL R15 2 0
  FORGLOOP R9 2 [inext] [-54]
  GETIMPORT R9 K19 [table.sort]
  MOVE R10 R2
  DUPCLOSURE R11 K20 [PROTO_16]
  CALL R9 2 0
  RETURN R2 1

PROTO_18:
  GETUPVAL R0 0
  NEWTABLE R1 0 0
  SETTABLEKS R1 R0 K0 ["Cache"]
  GETUPVAL R0 0
  NEWTABLE R1 0 0
  SETTABLEKS R1 R0 K1 ["FaceControlsCache"]
  RETURN R0 0

PROTO_19:
  NEWTABLE R1 0 0
  NEWTABLE R2 0 0
  NEWTABLE R3 0 0
  NEWTABLE R4 0 0
  NEWTABLE R5 0 0
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K0 ["Cache"]
  GETTABLEKS R6 R7 K1 ["CacheRig"]
  JUMPIFEQ R0 R6 [+135]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["getMotors"]
  MOVE R7 R0
  CALL R6 1 1
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K3 ["getConstraints"]
  MOVE R8 R0
  CALL R7 1 1
  GETUPVAL R9 0
  GETTABLEKS R8 R9 K4 ["getAnimationConstraints"]
  MOVE R9 R0
  CALL R8 1 1
  GETUPVAL R9 1
  NEWTABLE R10 0 0
  MOVE R11 R0
  CALL R9 2 1
  GETIMPORT R10 K6 [ipairs]
  MOVE R11 R9
  CALL R10 1 3
  FORGPREP_INEXT R10
  LOADK R17 K7 ["BasePart"]
  NAMECALL R15 R14 K8 ["IsA"]
  CALL R15 2 1
  JUMPIFNOT R15 [+66]
  GETIMPORT R15 K6 [ipairs]
  MOVE R16 R6
  CALL R15 1 3
  FORGPREP_INEXT R15
  GETTABLEKS R20 R19 K9 ["Part1"]
  JUMPIFNOTEQ R20 R14 [+12]
  GETTABLEKS R20 R14 K10 ["Name"]
  SETTABLE R19 R2 R20
  FASTCALL2 TABLE_INSERT R1 R14 [+5]
  MOVE R21 R1
  MOVE R22 R14
  GETIMPORT R20 K13 [table.insert]
  CALL R20 2 0
  JUMP [+2]
  FORGLOOP R15 2 [inext] [-16]
  GETIMPORT R15 K6 [ipairs]
  MOVE R16 R7
  CALL R15 1 3
  FORGPREP_INEXT R15
  GETUPVAL R21 0
  GETTABLEKS R20 R21 K14 ["getPart1"]
  MOVE R21 R19
  CALL R20 1 1
  JUMPIFNOTEQ R20 R14 [+5]
  GETTABLEKS R20 R14 K10 ["Name"]
  SETTABLE R19 R3 R20
  JUMP [+2]
  FORGLOOP R15 2 [inext] [-12]
  GETIMPORT R15 K6 [ipairs]
  MOVE R16 R8
  CALL R15 1 3
  FORGPREP_INEXT R15
  GETUPVAL R21 0
  GETTABLEKS R20 R21 K14 ["getPart1"]
  MOVE R21 R19
  CALL R20 1 1
  JUMPIFNOTEQ R20 R14 [+12]
  GETTABLEKS R20 R14 K10 ["Name"]
  SETTABLE R19 R4 R20
  FASTCALL2 TABLE_INSERT R1 R14 [+5]
  MOVE R21 R1
  MOVE R22 R14
  GETIMPORT R20 K13 [table.insert]
  CALL R20 2 0
  JUMP [+18]
  FORGLOOP R15 2 [inext] [-19]
  JUMP [+15]
  LOADK R17 K15 ["Bone"]
  NAMECALL R15 R14 K8 ["IsA"]
  CALL R15 2 1
  JUMPIFNOT R15 [+10]
  GETTABLEKS R15 R14 K10 ["Name"]
  SETTABLE R14 R5 R15
  FASTCALL2 TABLE_INSERT R1 R14 [+5]
  MOVE R16 R1
  MOVE R17 R14
  GETIMPORT R15 K13 [table.insert]
  CALL R15 2 0
  FORGLOOP R10 2 [inext] [-87]
  GETUPVAL R11 0
  GETTABLEKS R10 R11 K0 ["Cache"]
  SETTABLEKS R0 R10 K1 ["CacheRig"]
  GETUPVAL R11 0
  GETTABLEKS R10 R11 K0 ["Cache"]
  DUPTABLE R11 K21 [{"Parts", "PartNameToMotor", "PartNameToConstraint", "PartNameToAnimConstraint", "BoneNameToBone"}]
  SETTABLEKS R1 R11 K16 ["Parts"]
  SETTABLEKS R2 R11 K17 ["PartNameToMotor"]
  SETTABLEKS R3 R11 K18 ["PartNameToConstraint"]
  SETTABLEKS R4 R11 K19 ["PartNameToAnimConstraint"]
  SETTABLEKS R5 R11 K20 ["BoneNameToBone"]
  SETTABLEKS R11 R10 K22 ["CacheData"]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K0 ["Cache"]
  GETTABLEKS R6 R7 K22 ["CacheData"]
  RETURN R6 1

PROTO_20:
  NEWTABLE R1 0 0
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["FaceControlsCache"]
  GETTABLEKS R2 R3 K1 ["CacheRig"]
  JUMPIFEQ R0 R2 [+35]
  GETUPVAL R2 1
  NEWTABLE R3 0 0
  MOVE R4 R0
  CALL R2 2 1
  GETIMPORT R3 K3 [ipairs]
  MOVE R4 R2
  CALL R3 1 3
  FORGPREP_INEXT R3
  LOADK R10 K4 ["FaceControls"]
  NAMECALL R8 R7 K5 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+7]
  FASTCALL2 TABLE_INSERT R1 R7 [+5]
  MOVE R9 R1
  MOVE R10 R7
  GETIMPORT R8 K8 [table.insert]
  CALL R8 2 0
  FORGLOOP R3 2 [inext] [-13]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["FaceControlsCache"]
  SETTABLEKS R0 R3 K1 ["CacheRig"]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["FaceControlsCache"]
  SETTABLEKS R1 R3 K4 ["FaceControls"]
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K0 ["FaceControlsCache"]
  GETTABLEKS R3 R4 K4 ["FaceControls"]
  GETTABLEN R2 R3 1
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K0 ["FaceControlsCache"]
  GETTABLEKS R3 R4 K4 ["FaceControls"]
  RETURN R2 2

PROTO_21:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["getRigInfo"]
  MOVE R3 R0
  CALL R2 1 1
  GETTABLEKS R3 R2 K1 ["PartNameToMotor"]
  GETTABLEKS R4 R2 K2 ["PartNameToAnimConstraint"]
  JUMPIFNOT R3 [+5]
  GETTABLE R5 R3 R1
  JUMPIFNOT R5 [+3]
  GETTABLEKS R6 R5 K3 ["Part1"]
  RETURN R6 1
  JUMPIFNOT R4 [+8]
  GETTABLE R5 R4 R1
  JUMPIFNOT R5 [+6]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K4 ["getPart1"]
  MOVE R7 R5
  CALL R6 1 -1
  RETURN R6 -1
  LOADNIL R5
  RETURN R5 1

PROTO_22:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["getRigInfo"]
  MOVE R3 R0
  CALL R2 1 1
  GETTABLEKS R4 R2 K1 ["BoneNameToBone"]
  GETTABLE R3 R4 R1
  RETURN R3 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AnimationClipEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R4 R0 K6 ["Src"]
  GETTABLEKS R3 R4 K7 ["Util"]
  GETTABLEKS R2 R3 K8 ["buildHierarchy"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R5 R0 K6 ["Src"]
  GETTABLEKS R4 R5 K7 ["Util"]
  GETTABLEKS R3 R4 K9 ["Constants"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R5 R0 K10 ["LuaFlags"]
  GETTABLEKS R4 R5 K11 ["GetFFlagAnimConstraintParent"]
  CALL R3 1 1
  DUPTABLE R4 K14 [{"Cache", "FaceControlsCache"}]
  DUPTABLE R5 K17 [{"CacheRig", "CacheData"}]
  LOADNIL R6
  SETTABLEKS R6 R5 K15 ["CacheRig"]
  LOADNIL R6
  SETTABLEKS R6 R5 K16 ["CacheData"]
  SETTABLEKS R5 R4 K12 ["Cache"]
  DUPTABLE R5 K17 [{"CacheRig", "CacheData"}]
  LOADNIL R6
  SETTABLEKS R6 R5 K15 ["CacheRig"]
  LOADNIL R6
  SETTABLEKS R6 R5 K16 ["CacheData"]
  SETTABLEKS R5 R4 K13 ["FaceControlsCache"]
  DUPCLOSURE R5 K18 [PROTO_0]
  CAPTURE VAL R5
  DUPCLOSURE R6 K19 [PROTO_1]
  CAPTURE VAL R2
  SETTABLEKS R6 R4 K20 ["getTemporaryConstraints"]
  DUPCLOSURE R6 K21 [PROTO_2]
  CAPTURE VAL R4
  SETTABLEKS R6 R4 K22 ["usingTemporaryConstraints"]
  DUPCLOSURE R6 K23 [PROTO_3]
  CAPTURE VAL R4
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K24 ["getConstraints"]
  DUPCLOSURE R6 K25 [PROTO_5]
  CAPTURE VAL R4
  CAPTURE VAL R1
  SETTABLEKS R6 R4 K26 ["buildRigHierarchy"]
  DUPCLOSURE R6 K27 [PROTO_6]
  SETTABLEKS R6 R4 K28 ["isR15Humanoid"]
  DUPCLOSURE R6 K29 [PROTO_7]
  SETTABLEKS R6 R4 K30 ["isR6Humanoid"]
  DUPCLOSURE R6 K31 [PROTO_8]
  CAPTURE VAL R4
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K32 ["findRootPart"]
  DUPCLOSURE R6 K33 [PROTO_9]
  SETTABLEKS R6 R4 K34 ["getAnimationController"]
  DUPCLOSURE R6 K35 [PROTO_10]
  CAPTURE VAL R4
  SETTABLEKS R6 R4 K36 ["getAnimator"]
  DUPCLOSURE R6 K37 [PROTO_11]
  CAPTURE VAL R3
  SETTABLEKS R6 R4 K38 ["getPart0"]
  DUPCLOSURE R6 K39 [PROTO_12]
  CAPTURE VAL R3
  SETTABLEKS R6 R4 K40 ["getPart1"]
  DUPCLOSURE R6 K41 [PROTO_13]
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K42 ["getMotors"]
  DUPCLOSURE R6 K43 [PROTO_14]
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K44 ["getBones"]
  DUPCLOSURE R6 K45 [PROTO_15]
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K46 ["getAnimationConstraints"]
  DUPCLOSURE R6 K47 [PROTO_17]
  CAPTURE VAL R4
  CAPTURE VAL R2
  SETTABLEKS R6 R4 K48 ["getUnusedRigTracks"]
  DUPCLOSURE R6 K49 [PROTO_18]
  CAPTURE VAL R4
  SETTABLEKS R6 R4 K50 ["clearCache"]
  DUPCLOSURE R6 K51 [PROTO_19]
  CAPTURE VAL R4
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K52 ["getRigInfo"]
  DUPCLOSURE R6 K53 [PROTO_20]
  CAPTURE VAL R4
  CAPTURE VAL R5
  SETTABLEKS R6 R4 K54 ["getFaceControls"]
  DUPCLOSURE R6 K55 [PROTO_21]
  CAPTURE VAL R4
  SETTABLEKS R6 R4 K56 ["getPartByName"]
  DUPCLOSURE R6 K57 [PROTO_22]
  CAPTURE VAL R4
  SETTABLEKS R6 R4 K58 ["getBoneByName"]
  RETURN R4 1

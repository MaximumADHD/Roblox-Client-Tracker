...
function TransformHandlesImplementation:beginDrag(selection, initialSelectionInfo)
	local partsToMove, attachmentsToMove = initialSelectionInfo:getObjectsToTransform()
	...
	basisCFrame, offset, self._boundingBoxSize = initialSelectionInfo:getBoundingBox()
	...
	if not self:_shouldSolveConstraints() then
		...
		if self._tool == Enum.RibbonTool.Move then
			if self._effectorCFrame then
				...
				local effectorInRange = (rootPart.CFrame.p - updatedCFrame.p).Magnitude
					<= Constants.MIN_EFFECTOR_DISTANCE
				local translationStiffness = effectorInRange and Constants.MIN_TRANSLATION_STIFFNESS
					or Constants.MIN_TRANSLATION_STIFFNESS
				local rotationStiffness = effectorInRange and Constants.MIN_ROTATION_STIFFNESS
					or Constants.MIN_ROTATION_STIFFNESS
				Workspace:IKMoveTo(
					joint.Part1,
					updatedCFrame,
		...
		if self._tool == Enum.RibbonTool.Rotate then
			if self._effectorCFrame then
				...
				Workspace:IKMoveTo(
					joint.Part1,
					updatedCFrame,
					Constants.TRANSLATION_STIFFNESS,
					Constants.ROTATION_STIFFNESS,
					Enum.IKCollisionsMode.NoCollisions
				)
				local actualCFrame = joint.Part1.CFrame
				local actualGlobalTransform = actualCFrame * self._effectorCFrame:Inverse()

...
function CurveUtils.getSlopes(track, tick)
	if track.Keyframes and not isEmpty(track.Keyframes) then
		...
		if lowIndex and highIndex then
			...
			-- Trivial: Get the slopes at an existing tick
			if tick == lowTick then
				return lowKey.LeftSlope, lowKey.RightSlope
			elseif tick == highTick then
				return highKey.LeftSlope, highKey.RightSlope
			end
			...
			local slope = (g00 * lowTick.Value + g01 * highTick.Value) / deltaTick
				+ g10 * lowTick.RightSlope
				+ g11 * highTick.LeftSlope

return function()
	...
	describe("SetBoneVisualization", function()
		it("should set bone visualization", function()
			local store = createTestStore()
			store:dispatch(SetBoneVisualization(true))
			expect(store:getState().VisualizeBones).to.equal(true)
			store:dispatch(SetBoneVisualization(false))
			expect(store:getState().VisualizeBones).to.equal(false)
		end)
	end)

	describe("SetDefaultEulerAnglesOrde", function()
		it("should set the default Euler angles order", function()
			local store = createTestStore()
			store:dispatch(SetDefaultEulerAnglesOrder(Enum.RotationOrder.ZXY))
			expect(store:getState().DefaultEulerAnglesOrder).to.equal(Enum.RotationOrder.ZXY)
			store:dispatch(SetDefaultEulerAnglesOrder(Enum.RotationOrder.XYZ))
			expect(store:getState().DefaultEulerAnglesOrder).to.equal(Enum.RotationOrder.XYZ)
		end)
	end)

	describe("SetDefaultRotationType", function()
		it("should set the default rotation type", function()
			local store = createTestStore()
			store:dispatch(SetDefaultRotationType(Constants.TRACK_TYPES.EulerAngles))
			expect(store:getState().DefaultRotationType).to.equal(Constants.TRACK_TYPES.EulerAngles)
			store:dispatch(SetDefaultRotationType(Constants.TRACK_TYPES.Quaternion))
			expect(store:getState().DefaultRotationType).to.equal(Constants.TRACK_TYPES.Quaternion)
		end)
	end)

	describe("SetPlaybackSpeed", function()
		it("should set the playback speed", function()
			local store = createTestStore()
			store:dispatch(SetPlaybackSpeed(0.25))
			expect(store:getState().PlaybackSpeed).to.equal(0.25)
			store:dispatch(SetPlaybackSpeed(0.125))
			expect(store:getState().PlaybackSpeed).to.equal(0.125)
		end)
	end)

	describe("SetReadOnly", function()
		it("should enter and exit read only mode", function()
			local store = createTestStore()
			store:dispatch(SetReadOnly(true))
			expect(store:getState().ReadOnly).to.equal(true)
			store:dispatch(SetReadOnly(false))
			expect(store:getState().ReadOnly).to.equal(false)
		end)
	end)
end

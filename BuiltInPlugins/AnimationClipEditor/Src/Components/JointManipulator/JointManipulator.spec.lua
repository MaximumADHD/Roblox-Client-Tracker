return function()
	...
	it("should render line handles if using the Move tool", function()
		...
		expect(handlesContainer.LineHandles.HandleX).to.be.ok()
		expect(handlesContainer.LineHandles.HandleY).to.be.ok()
		expect(handlesContainer.LineHandles.HandleZ).to.be.ok()
		Roact.unmount(instance)
	end)
end
...
local deprecatedTextureMapSelector = Roact.PureComponent:extend("deprecatedTextureMapSelector")

function deprecatedTextureMapSelector:init()
	self.state = {
		importAsset = {},
		uploading = false,
	}
	...
	self.uploadTextureMap = function(file: File)
		...
		local _promise = assetHandler
			...
			:andThen(function(assetId)
				local pbrMaterial = props.PBRMaterial :: any
				props.GeneralServiceController:setTextureMap(pbrMaterial, props.MapType, assetId, file.Name)
				return assetId
			end)
			:andThen(function(assetId)
			...
			:andThen(function(assetId)
				local success = props.GeneralServiceController:insertAssetId(assetId, file.Name)
				if not success then
					self.errorMessage = ErrorTypes.FailedToInsertAssetManager
				end
			end)
			:catch(function(err)
...
function deprecatedTextureMapSelector:didMount()
	self._isMounted = true
end

function deprecatedTextureMapSelector:willUnmount()
	self._isMounted = false
end

function deprecatedTextureMapSelector:didUpdate(prevProps)
...
function deprecatedTextureMapSelector:render()
	local props: _Props = self.props
	...
	if pbrMaterial[props.MapType] == "" then
		if self.errorMessage and self.errorMessage ~= "" and self.errorMessage ~= ErrorTypes.FailedToInsertAssetManager then
			...
			elseif self.errorMessage == ErrorTypes.FailedToUploadTooLarge then
...
deprecatedTextureMapSelector = withContext({
	Analytics = Analytics,
	GeneralServiceController = GeneralServiceController,
	Localization = Localization,
	ImportAssetHandler = ImportAssetHandler,
})(deprecatedTextureMapSelector)

return deprecatedTextureMapSelector

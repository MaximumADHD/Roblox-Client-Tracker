return function()
    ...
    local MaterialOptions = require(script.Parent.MaterialOptions)

    local function createTestElement(props: MaterialOptions.Props?)
        props = props or {
            OpenPrompt = function(type: _Types.MaterialPromptType) end
        }

        return mockContext({
            MaterialOptions = Roact.createElement(MaterialOptions, props)
        })
    end

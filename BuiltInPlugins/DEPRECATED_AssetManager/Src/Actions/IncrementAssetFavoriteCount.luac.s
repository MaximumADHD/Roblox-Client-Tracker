PROTO_0:
  ...
  LOADK R5 K6 ["IncrementAssetFavoriteCount requires assetId to be a number, not %s"]
  FASTCALL1 TYPE R0 [+3]
  MOVE R7 R0
  ...
  LOADB R3 1
  GETIMPORT R4 K5 [string.format]
  LOADK R5 K9 ["IncrementAssetFavoriteCount requires incrementCount to be a number, not %s"]
  FASTCALL1 TYPE R1 [+3]
  MOVE R7 R1
  GETIMPORT R6 K1 [type]
  CALL R6 1 1
  ...
  GETIMPORT R2 K8 [assert]
  CALL R2 -1 0
  DUPTABLE R2 K12 [{"assetId", "incrementCount"}]
  SETTABLEKS R0 R2 K10 ["assetId"]
  SETTABLEKS R1 R2 K11 ["incrementCount"]
  RETURN R2 1

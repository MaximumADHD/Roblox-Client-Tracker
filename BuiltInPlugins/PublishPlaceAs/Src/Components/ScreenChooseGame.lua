...
local FFlagDevFrameworkDropdownMenuHeight = game:GetFastFlag("DevFrameworkDropdownMenuHeight")
...
local StyledDropDown = if not FFlagRemoveUILibraryStyledDropdown or not FFlagDevFrameworkDropdownShowsLabel
...
function ScreenChooseGame:init()
	...
	self.onItemActivated = if FFlagRemoveUILibraryStyledDropdown and FFlagDevFrameworkDropdownShowsLabel
		then function(item)
			if item.Key ~= SelectedItemKey then
				SelectedItemKey = item.Key
				SelectedItemType = item.Type
				SelectedItemText = item.Text
				self.props.DispatchLoadExistingGames(item.Type, item.Key)
			end
		end
		else nil
...
function ScreenChooseGame:render()
	...
	local components = {
		Roact.createElement("UIGridLayout", {
			...
			CellPadding = UDim2.new(
				0,
				theme.SCREEN_CHOOSE_GAME.CELL_PADDING_X,
				0,
	...
	return Roact.createElement("Frame", {
		...
		GroupDropdown = if not FFlagRemoveUILibraryStyledDropdown or not FFlagDevFrameworkDropdownShowsLabel
			then Roact.createElement(StyledDropDown, {
		...
		GroupDropdownMenu = if FFlagRemoveUILibraryStyledDropdown and FFlagDevFrameworkDropdownShowsLabel
			then Roact.createElement(SelectInput, {
				Items = map(dropdownItems, self.createContentForSelectInput),
				MenuHeight = if FFlagDevFrameworkDropdownMenuHeight then 600 else nil,
				OnItemActivated = self.onItemActivated,
				PlaceholderText = dropdownDisplayText,
				Position = UDim2.new(0, 30, 0, 70),
				SelectedIndex = SelectedItemKey,
				Size = UDim2.new(0, theme.selectInput.width.creator, 0, theme.selectInput.button.height),
...
function ScreenChooseGame:willUnmount()
	for key, _ in self.props.Games do
		self.props.Games[key] = nil
	end
end

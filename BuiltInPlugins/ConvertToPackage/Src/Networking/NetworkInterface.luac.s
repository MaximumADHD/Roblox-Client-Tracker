PROTO_0:
  DUPTABLE R0 K1 [{"_networkImp"}]
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K2 ["new"]
  CALL R1 0 1
  SETTABLEKS R1 R0 K0 ["_networkImp"]
  GETUPVAL R3 1
  FASTCALL2 SETMETATABLE R0 R3 [+4]
  MOVE R2 R0
  GETIMPORT R1 K4 [setmetatable]
  CALL R1 2 0
  RETURN R0 1

PROTO_1:
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K0 ["shouldDebugUrls"]
  CALL R4 0 1
  JUMPIFNOT R4 [+33]
  GETIMPORT R4 K2 [print]
  LOADK R5 K3 ["NetworkInterface:%s()"]
  MOVE R7 R0
  NAMECALL R5 R5 K4 ["format"]
  CALL R5 2 -1
  CALL R4 -1 0
  GETIMPORT R4 K2 [print]
  LOADK R5 K5 ["	%s %s"]
  NAMECALL R8 R1 K7 ["upper"]
  CALL R8 1 1
  ORK R7 R8 K6 ["method=nil"]
  ORK R8 R2 K8 ["url=nil"]
  NAMECALL R5 R5 K4 ["format"]
  CALL R5 3 -1
  CALL R4 -1 0
  JUMPIFNOT R3 [+12]
  GETIMPORT R4 K2 [print]
  LOADK R5 K9 ["	%s"]
  FASTCALL1 TOSTRING R3 [+3]
  MOVE R8 R3
  GETIMPORT R7 K11 [tostring]
  CALL R7 1 1
  NAMECALL R5 R5 K4 ["format"]
  CALL R5 2 -1
  CALL R4 -1 0
  RETURN R0 0

PROTO_2:
  GETTABLEKS R2 R0 K0 ["_networkImp"]
  MOVE R4 R1
  NAMECALL R2 R2 K1 ["jsonEncode"]
  CALL R2 2 -1
  RETURN R2 -1

PROTO_3:
  GETUPVAL R11 0
  GETTABLEKS R10 R11 K0 ["constructPostUploadAssetUrl"]
  MOVE R11 R1
  MOVE R12 R2
  MOVE R13 R3
  MOVE R14 R4
  MOVE R15 R5
  MOVE R16 R6
  MOVE R17 R7
  MOVE R18 R8
  CALL R10 8 1
  GETUPVAL R11 1
  LOADK R12 K1 ["uploadCatalogItem"]
  LOADK R13 K2 ["POST"]
  MOVE R14 R10
  MOVE R15 R9
  CALL R11 4 0
  GETTABLEKS R11 R0 K3 ["_networkImp"]
  MOVE R13 R10
  MOVE R14 R9
  NAMECALL R11 R11 K4 ["httpPost"]
  CALL R11 3 -1
  RETURN R11 -1

PROTO_4:
  GETTABLEKS R1 R0 K0 ["responseBody"]
  GETTABLEKS R3 R0 K0 ["responseBody"]
  GETTABLEKS R2 R3 K1 ["groups"]
  SETTABLEKS R2 R1 K2 ["data"]
  RETURN R0 1

PROTO_5:
  GETUPVAL R2 0
  JUMPIFNOT R2 [+21]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K0 ["constructGetGroupsForSurfaceUrl"]
  LOADK R3 K1 ["StudioConvertToPackage"]
  CALL R2 1 1
  GETUPVAL R3 2
  LOADK R4 K2 ["getMyGroups"]
  LOADK R5 K3 ["GET"]
  MOVE R6 R2
  CALL R3 3 0
  GETTABLEKS R3 R0 K4 ["_networkImp"]
  MOVE R5 R2
  NAMECALL R3 R3 K5 ["httpGetJson"]
  CALL R3 2 1
  DUPCLOSURE R5 K6 [PROTO_4]
  NAMECALL R3 R3 K7 ["andThen"]
  CALL R3 2 -1
  RETURN R3 -1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K8 ["constructGetMyGroupUrl"]
  CALL R2 0 1
  GETUPVAL R3 2
  LOADK R4 K2 ["getMyGroups"]
  LOADK R5 K3 ["GET"]
  MOVE R6 R2
  CALL R3 3 0
  GETTABLEKS R3 R0 K4 ["_networkImp"]
  MOVE R5 R2
  NAMECALL R3 R3 K5 ["httpGetJson"]
  CALL R3 2 -1
  RETURN R3 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R3 K1 [script]
  GETTABLEKS R2 R3 K2 ["Parent"]
  GETTABLEKS R1 R2 K2 ["Parent"]
  GETTABLEKS R0 R1 K2 ["Parent"]
  GETIMPORT R2 K4 [require]
  GETTABLEKS R4 R0 K5 ["DEPRECATED_modules"]
  GETTABLEKS R3 R4 K6 ["Http"]
  CALL R2 1 1
  GETTABLEKS R1 R2 K7 ["Networking"]
  GETIMPORT R2 K4 [require]
  GETTABLEKS R5 R0 K8 ["Src"]
  GETTABLEKS R4 R5 K9 ["Util"]
  GETTABLEKS R3 R4 K10 ["DebugFlags"]
  CALL R2 1 1
  GETIMPORT R3 K4 [require]
  GETTABLEKS R6 R0 K8 ["Src"]
  GETTABLEKS R5 R6 K9 ["Util"]
  GETTABLEKS R4 R5 K11 ["Urls"]
  CALL R3 1 1
  GETIMPORT R4 K13 [game]
  LOADK R6 K14 ["EnableReverseDelegationConvertToPackage"]
  LOADB R7 0
  NAMECALL R4 R4 K15 ["DefineFastFlag"]
  CALL R4 3 1
  NEWTABLE R5 8 0
  SETTABLEKS R5 R5 K16 ["__index"]
  DUPCLOSURE R6 K17 [PROTO_0]
  CAPTURE VAL R1
  CAPTURE VAL R5
  SETTABLEKS R6 R5 K18 ["new"]
  DUPCLOSURE R6 K19 [PROTO_1]
  CAPTURE VAL R2
  DUPCLOSURE R7 K20 [PROTO_2]
  SETTABLEKS R7 R5 K21 ["jsonEncode"]
  DUPCLOSURE R7 K22 [PROTO_3]
  CAPTURE VAL R3
  CAPTURE VAL R6
  SETTABLEKS R7 R5 K23 ["postUploadAsset"]
  DUPCLOSURE R7 K24 [PROTO_5]
  CAPTURE VAL R4
  CAPTURE VAL R3
  CAPTURE VAL R6
  SETTABLEKS R7 R5 K25 ["getMyGroups"]
  RETURN R5 1

PROTO_0:
  MOVE R4 R0
  NAMECALL R2 R1 K0 ["FindFirstChild"]
  CALL R2 2 1
  JUMPIFNOT R2 [+6]
  LOADK R5 K1 ["Folder"]
  NAMECALL R3 R2 K2 ["IsA"]
  CALL R3 2 1
  JUMPIFNOT R3 [+1]
  RETURN R2 1
  GETIMPORT R3 K5 [Instance.new]
  LOADK R4 K1 ["Folder"]
  CALL R3 1 1
  SETTABLEKS R0 R3 K6 ["Name"]
  SETTABLEKS R1 R3 K7 ["Parent"]
  RETURN R3 1

PROTO_1:
  DUPCLOSURE R1 K0 [PROTO_0]
  GETUPVAL R2 0
  JUMPIF R2 [+24]
  GETUPVAL R3 1
  LOADK R6 K1 ["R15MigratorProgress"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R2 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K1 ["R15MigratorProgress"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R2 R5
  SETUPVAL R2 0
  JUMPIFNOT R0 [+23]
  GETUPVAL R3 0
  MOVE R6 R0
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R2 R4
  RETURN R2 1
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  SETTABLEKS R0 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R2 R5
  RETURN R2 1
  GETUPVAL R2 0
  RETURN R2 1

PROTO_2:
  GETUPVAL R1 0
  JUMPIF R1 [+14]
  GETUPVAL R1 1
  LOADK R3 K0 ["R15MigratorProgress"]
  NAMECALL R1 R1 K1 ["FindFirstChild"]
  CALL R1 2 1
  JUMPIFNOT R1 [+5]
  LOADK R4 K2 ["Folder"]
  NAMECALL R2 R1 K3 ["IsA"]
  CALL R2 2 1
  JUMPIF R2 [+2]
  LOADNIL R2
  RETURN R2 1
  SETUPVAL R1 0
  JUMPIFNOT R0 [+6]
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K1 ["FindFirstChild"]
  CALL R1 2 1
  RETURN R1 1
  GETUPVAL R1 0
  RETURN R1 1

PROTO_3:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["clearPublishTags"]
  CALL R1 2 0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K1 ["clearTags"]
  MOVE R2 R0
  CALL R1 1 0
  RETURN R0 0

PROTO_4:
  DUPCLOSURE R4 K0 [PROTO_0]
  GETUPVAL R5 0
  JUMPIF R5 [+24]
  GETUPVAL R6 1
  LOADK R9 K1 ["R15MigratorProgress"]
  NAMECALL R7 R6 K2 ["FindFirstChild"]
  CALL R7 2 1
  JUMPIFNOT R7 [+7]
  LOADK R10 K3 ["Folder"]
  NAMECALL R8 R7 K4 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+2]
  MOVE R5 R7
  JUMP [+10]
  GETIMPORT R8 K7 [Instance.new]
  LOADK R9 K3 ["Folder"]
  CALL R8 1 1
  LOADK R9 K1 ["R15MigratorProgress"]
  SETTABLEKS R9 R8 K8 ["Name"]
  SETTABLEKS R6 R8 K9 ["Parent"]
  MOVE R5 R8
  SETUPVAL R5 0
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K9 ["Parent"]
  JUMPIFEQ R2 R0 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_5:
  DUPCLOSURE R2 K0 [PROTO_0]
  GETUPVAL R3 0
  JUMPIF R3 [+24]
  GETUPVAL R4 1
  LOADK R7 K1 ["R15MigratorProgress"]
  NAMECALL R5 R4 K2 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+7]
  LOADK R8 K3 ["Folder"]
  NAMECALL R6 R5 K4 ["IsA"]
  CALL R6 2 1
  JUMPIFNOT R6 [+2]
  MOVE R3 R5
  JUMP [+10]
  GETIMPORT R6 K7 [Instance.new]
  LOADK R7 K3 ["Folder"]
  CALL R6 1 1
  LOADK R7 K1 ["R15MigratorProgress"]
  SETTABLEKS R7 R6 K8 ["Name"]
  SETTABLEKS R4 R6 K9 ["Parent"]
  MOVE R3 R6
  SETUPVAL R3 0
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K10 ["IsDescendantOf"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_6:
  DUPCLOSURE R3 K0 [PROTO_0]
  GETUPVAL R4 0
  JUMPIF R4 [+24]
  GETUPVAL R5 1
  LOADK R8 K1 ["R15MigratorProgress"]
  NAMECALL R6 R5 K2 ["FindFirstChild"]
  CALL R6 2 1
  JUMPIFNOT R6 [+7]
  LOADK R9 K3 ["Folder"]
  NAMECALL R7 R6 K4 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+2]
  MOVE R4 R6
  JUMP [+10]
  GETIMPORT R7 K7 [Instance.new]
  LOADK R8 K3 ["Folder"]
  CALL R7 1 1
  LOADK R8 K1 ["R15MigratorProgress"]
  SETTABLEKS R8 R7 K8 ["Name"]
  SETTABLEKS R5 R7 K9 ["Parent"]
  MOVE R4 R7
  SETUPVAL R4 0
  GETUPVAL R2 0
  JUMPIFEQ R2 R0 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_7:
  DUPCLOSURE R4 K0 [PROTO_0]
  GETUPVAL R5 0
  JUMPIF R5 [+24]
  GETUPVAL R6 1
  LOADK R9 K1 ["R15MigratorProgress"]
  NAMECALL R7 R6 K2 ["FindFirstChild"]
  CALL R7 2 1
  JUMPIFNOT R7 [+7]
  LOADK R10 K3 ["Folder"]
  NAMECALL R8 R7 K4 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+2]
  MOVE R5 R7
  JUMP [+10]
  GETIMPORT R8 K7 [Instance.new]
  LOADK R9 K3 ["Folder"]
  CALL R8 1 1
  LOADK R9 K1 ["R15MigratorProgress"]
  SETTABLEKS R9 R8 K8 ["Name"]
  SETTABLEKS R6 R8 K9 ["Parent"]
  MOVE R5 R8
  SETUPVAL R5 0
  GETUPVAL R3 0
  NAMECALL R1 R0 K10 ["IsDescendantOf"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_8:
  GETUPVAL R0 0
  LOADK R2 K0 ["R15MigratorProgress"]
  NAMECALL R0 R0 K1 ["FindFirstChild"]
  CALL R0 2 1
  JUMPIFNOTEQKNIL R0 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_9:
  GETUPVAL R0 0
  LOADK R2 K0 ["R15MigratorProgress"]
  NAMECALL R0 R0 K1 ["FindFirstChild"]
  CALL R0 2 1
  JUMPIFEQKNIL R0 [+4]
  NAMECALL R1 R0 K2 ["Destroy"]
  CALL R1 1 0
  RETURN R0 0

PROTO_10:
  LOADK R4 K0 ["R15MigratorConversionId"]
  NAMECALL R2 R0 K1 ["GetAttribute"]
  CALL R2 2 1
  JUMPIFNOTEQKNIL R2 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_11:
  DUPCLOSURE R0 K0 [PROTO_10]
  RETURN R0 1

PROTO_12:
  LOADK R3 K0 ["R15MigratorConversionId"]
  LOADNIL R4
  NAMECALL R1 R0 K1 ["SetAttribute"]
  CALL R1 3 0
  RETURN R0 0

PROTO_13:
  LOADK R3 K0 ["R15MigratorConversionId"]
  NAMECALL R1 R0 K1 ["GetAttribute"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_14:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["getConversionId"]
  MOVE R3 R0
  CALL R2 1 1
  JUMPIFNOTEQKNIL R2 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_15:
  DUPCLOSURE R2 K0 [PROTO_0]
  GETUPVAL R3 0
  JUMPIF R3 [+24]
  GETUPVAL R4 1
  LOADK R7 K1 ["R15MigratorProgress"]
  NAMECALL R5 R4 K2 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+7]
  LOADK R8 K3 ["Folder"]
  NAMECALL R6 R5 K4 ["IsA"]
  CALL R6 2 1
  JUMPIFNOT R6 [+2]
  MOVE R3 R5
  JUMP [+10]
  GETIMPORT R6 K7 [Instance.new]
  LOADK R7 K3 ["Folder"]
  CALL R6 1 1
  LOADK R7 K1 ["R15MigratorProgress"]
  SETTABLEKS R7 R6 K8 ["Name"]
  SETTABLEKS R4 R6 K9 ["Parent"]
  MOVE R3 R6
  SETUPVAL R3 0
  GETUPVAL R3 0
  LOADK R6 K10 ["R6Characters"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R1 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K10 ["R6Characters"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R1 R5
  MOVE R2 R0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  LOADK R9 K11 ["R15MigratorConversionId"]
  NAMECALL R7 R6 K12 ["GetAttribute"]
  CALL R7 2 1
  JUMPIF R7 [+27]
  GETUPVAL R8 2
  NAMECALL R8 R8 K13 ["GenerateGUID"]
  CALL R8 1 1
  MOVE R7 R8
  LOADK R10 K11 ["R15MigratorConversionId"]
  MOVE R11 R7
  NAMECALL R8 R6 K14 ["SetAttribute"]
  CALL R8 3 0
  NAMECALL R8 R6 K15 ["Clone"]
  CALL R8 1 1
  SETTABLEKS R7 R8 K8 ["Name"]
  SETTABLEKS R1 R8 K9 ["Parent"]
  GETUPVAL R9 3
  MOVE R11 R8
  NAMECALL R9 R9 K16 ["clearPublishTags"]
  CALL R9 2 0
  GETUPVAL R10 4
  GETTABLEKS R9 R10 K17 ["clearTags"]
  MOVE R10 R8
  CALL R9 1 0
  FORGLOOP R2 2 [-33]
  RETURN R0 0

PROTO_16:
  DUPCLOSURE R1 K0 [PROTO_0]
  GETUPVAL R2 0
  JUMPIF R2 [+24]
  GETUPVAL R3 1
  LOADK R6 K1 ["R15MigratorProgress"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R2 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K1 ["R15MigratorProgress"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R2 R5
  SETUPVAL R2 0
  GETUPVAL R2 0
  LOADK R5 K10 ["R6Characters"]
  NAMECALL R3 R2 K2 ["FindFirstChild"]
  CALL R3 2 1
  JUMPIFNOT R3 [+7]
  LOADK R6 K3 ["Folder"]
  NAMECALL R4 R3 K4 ["IsA"]
  CALL R4 2 1
  JUMPIFNOT R4 [+2]
  MOVE R0 R3
  JUMP [+10]
  GETIMPORT R4 K7 [Instance.new]
  LOADK R5 K3 ["Folder"]
  CALL R4 1 1
  LOADK R5 K10 ["R6Characters"]
  SETTABLEKS R5 R4 K8 ["Name"]
  SETTABLEKS R2 R4 K9 ["Parent"]
  MOVE R0 R4
  NEWTABLE R1 0 0
  NAMECALL R2 R0 K11 ["GetChildren"]
  CALL R2 1 3
  FORGPREP R2
  GETTABLEKS R7 R6 K8 ["Name"]
  SETTABLE R6 R1 R7
  FORGLOOP R2 2 [-4]
  RETURN R1 1

PROTO_17:
  DUPCLOSURE R2 K0 [PROTO_0]
  GETUPVAL R3 0
  JUMPIF R3 [+24]
  GETUPVAL R4 1
  LOADK R7 K1 ["R15MigratorProgress"]
  NAMECALL R5 R4 K2 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+7]
  LOADK R8 K3 ["Folder"]
  NAMECALL R6 R5 K4 ["IsA"]
  CALL R6 2 1
  JUMPIFNOT R6 [+2]
  MOVE R3 R5
  JUMP [+10]
  GETIMPORT R6 K7 [Instance.new]
  LOADK R7 K3 ["Folder"]
  CALL R6 1 1
  LOADK R7 K1 ["R15MigratorProgress"]
  SETTABLEKS R7 R6 K8 ["Name"]
  SETTABLEKS R4 R6 K9 ["Parent"]
  MOVE R3 R6
  SETUPVAL R3 0
  GETUPVAL R1 0
  LOADK R3 K10 ["ScriptConversionRules"]
  GETUPVAL R4 2
  MOVE R6 R0
  NAMECALL R4 R4 K11 ["JSONEncode"]
  CALL R4 2 -1
  NAMECALL R1 R1 K12 ["SetAttribute"]
  CALL R1 -1 0
  RETURN R0 0

PROTO_18:
  GETUPVAL R1 0
  JUMPIF R1 [+14]
  GETUPVAL R1 1
  LOADK R3 K0 ["R15MigratorProgress"]
  NAMECALL R1 R1 K1 ["FindFirstChild"]
  CALL R1 2 1
  JUMPIFNOT R1 [+5]
  LOADK R4 K2 ["Folder"]
  NAMECALL R2 R1 K3 ["IsA"]
  CALL R2 2 1
  JUMPIF R2 [+2]
  LOADNIL R0
  JUMP [+2]
  SETUPVAL R1 0
  GETUPVAL R0 0
  JUMPIF R0 [+2]
  LOADNIL R0
  RETURN R0 1
  GETUPVAL R1 0
  JUMPIF R1 [+14]
  GETUPVAL R1 1
  LOADK R3 K0 ["R15MigratorProgress"]
  NAMECALL R1 R1 K1 ["FindFirstChild"]
  CALL R1 2 1
  JUMPIFNOT R1 [+5]
  LOADK R4 K2 ["Folder"]
  NAMECALL R2 R1 K3 ["IsA"]
  CALL R2 2 1
  JUMPIF R2 [+2]
  LOADNIL R0
  JUMP [+2]
  SETUPVAL R1 0
  GETUPVAL R0 0
  LOADK R2 K4 ["ScriptConversionRules"]
  NAMECALL R0 R0 K5 ["GetAttribute"]
  CALL R0 2 1
  JUMPIFNOT R0 [+6]
  GETUPVAL R1 2
  MOVE R3 R0
  NAMECALL R1 R1 K6 ["JSONDecode"]
  CALL R1 2 1
  RETURN R1 1
  LOADNIL R1
  RETURN R1 1

PROTO_19:
  LOADK R3 K0 ["KeyframeSequence"]
  NAMECALL R1 R0 K1 ["IsA"]
  CALL R1 2 1
  JUMPIFNOT R1 [+8]
  LOADK R4 K2 ["R15MigratorConversionId"]
  NAMECALL R2 R0 K3 ["GetAttribute"]
  CALL R2 2 1
  JUMPIFNOTEQKNIL R2 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_20:
  DUPCLOSURE R2 K0 [PROTO_0]
  GETUPVAL R3 0
  JUMPIF R3 [+24]
  GETUPVAL R4 1
  LOADK R7 K1 ["R15MigratorProgress"]
  NAMECALL R5 R4 K2 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+7]
  LOADK R8 K3 ["Folder"]
  NAMECALL R6 R5 K4 ["IsA"]
  CALL R6 2 1
  JUMPIFNOT R6 [+2]
  MOVE R3 R5
  JUMP [+10]
  GETIMPORT R6 K7 [Instance.new]
  LOADK R7 K3 ["Folder"]
  CALL R6 1 1
  LOADK R7 K1 ["R15MigratorProgress"]
  SETTABLEKS R7 R6 K8 ["Name"]
  SETTABLEKS R4 R6 K9 ["Parent"]
  MOVE R3 R6
  SETUPVAL R3 0
  GETUPVAL R3 0
  LOADK R6 K10 ["R6KeyframeSequences"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R1 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K10 ["R6KeyframeSequences"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R1 R5
  MOVE R2 R0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  LOADK R9 K11 ["KeyframeSequence"]
  NAMECALL R7 R6 K4 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+39]
  LOADK R9 K12 ["R15MigratorConversionId"]
  NAMECALL R7 R6 K13 ["GetAttribute"]
  CALL R7 2 1
  JUMPIF R7 [+34]
  GETUPVAL R8 2
  NAMECALL R8 R8 K14 ["GenerateGUID"]
  CALL R8 1 1
  MOVE R7 R8
  LOADK R10 K12 ["R15MigratorConversionId"]
  MOVE R11 R7
  NAMECALL R8 R6 K15 ["SetAttribute"]
  CALL R8 3 0
  NAMECALL R8 R6 K16 ["Clone"]
  CALL R8 1 1
  SETTABLEKS R7 R8 K8 ["Name"]
  GETUPVAL R12 3
  GETTABLEKS R11 R12 K17 ["OriginalId"]
  LOADNIL R12
  NAMECALL R9 R8 K15 ["SetAttribute"]
  CALL R9 3 0
  SETTABLEKS R1 R8 K9 ["Parent"]
  GETUPVAL R9 4
  MOVE R11 R8
  NAMECALL R9 R9 K18 ["clearPublishTags"]
  CALL R9 2 0
  GETUPVAL R10 5
  GETTABLEKS R9 R10 K19 ["clearTags"]
  MOVE R10 R8
  CALL R9 1 0
  FORGLOOP R2 2 [-45]
  RETURN R0 0

PROTO_21:
  LOADK R4 K0 ["R15MigratorConversionId"]
  NAMECALL R2 R0 K1 ["GetAttribute"]
  CALL R2 2 1
  JUMPIF R2 [+37]
  GETIMPORT R3 K3 [warn]
  LOADK R5 K4 ["Could not find matching R6 KeyframeSequence. Was the %* folder edited?"]
  DUPCLOSURE R8 K5 [PROTO_0]
  GETUPVAL R9 0
  JUMPIF R9 [+24]
  GETUPVAL R10 1
  LOADK R13 K6 ["R15MigratorProgress"]
  NAMECALL R11 R10 K7 ["FindFirstChild"]
  CALL R11 2 1
  JUMPIFNOT R11 [+7]
  LOADK R14 K8 ["Folder"]
  NAMECALL R12 R11 K9 ["IsA"]
  CALL R12 2 1
  JUMPIFNOT R12 [+2]
  MOVE R9 R11
  JUMP [+10]
  GETIMPORT R12 K12 [Instance.new]
  LOADK R13 K8 ["Folder"]
  CALL R12 1 1
  LOADK R13 K6 ["R15MigratorProgress"]
  SETTABLEKS R13 R12 K13 ["Name"]
  SETTABLEKS R10 R12 K14 ["Parent"]
  MOVE R9 R12
  SETUPVAL R9 0
  GETUPVAL R7 0
  NAMECALL R5 R5 K15 ["format"]
  CALL R5 2 1
  MOVE R4 R5
  CALL R3 1 0
  RETURN R0 0
  LOADNIL R3
  JUMPIFNOT R1 [+2]
  GETTABLE R3 R1 R2
  JUMP [+55]
  DUPCLOSURE R5 K5 [PROTO_0]
  GETUPVAL R6 0
  JUMPIF R6 [+24]
  GETUPVAL R7 1
  LOADK R10 K6 ["R15MigratorProgress"]
  NAMECALL R8 R7 K7 ["FindFirstChild"]
  CALL R8 2 1
  JUMPIFNOT R8 [+7]
  LOADK R11 K8 ["Folder"]
  NAMECALL R9 R8 K9 ["IsA"]
  CALL R9 2 1
  JUMPIFNOT R9 [+2]
  MOVE R6 R8
  JUMP [+10]
  GETIMPORT R9 K12 [Instance.new]
  LOADK R10 K8 ["Folder"]
  CALL R9 1 1
  LOADK R10 K6 ["R15MigratorProgress"]
  SETTABLEKS R10 R9 K13 ["Name"]
  SETTABLEKS R7 R9 K14 ["Parent"]
  MOVE R6 R9
  SETUPVAL R6 0
  GETUPVAL R6 0
  LOADK R9 K16 ["R6KeyframeSequences"]
  NAMECALL R7 R6 K7 ["FindFirstChild"]
  CALL R7 2 1
  JUMPIFNOT R7 [+7]
  LOADK R10 K8 ["Folder"]
  NAMECALL R8 R7 K9 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+2]
  MOVE R4 R7
  JUMP [+10]
  GETIMPORT R8 K12 [Instance.new]
  LOADK R9 K8 ["Folder"]
  CALL R8 1 1
  LOADK R9 K16 ["R6KeyframeSequences"]
  SETTABLEKS R9 R8 K13 ["Name"]
  SETTABLEKS R6 R8 K14 ["Parent"]
  MOVE R4 R8
  MOVE R7 R2
  NAMECALL R5 R4 K7 ["FindFirstChild"]
  CALL R5 2 1
  MOVE R3 R5
  JUMPIF R3 [+37]
  GETIMPORT R4 K3 [warn]
  LOADK R6 K17 ["Original R6 KeyframeSequence is unavailable. Was the %* folder edited?"]
  DUPCLOSURE R9 K5 [PROTO_0]
  GETUPVAL R10 0
  JUMPIF R10 [+24]
  GETUPVAL R11 1
  LOADK R14 K6 ["R15MigratorProgress"]
  NAMECALL R12 R11 K7 ["FindFirstChild"]
  CALL R12 2 1
  JUMPIFNOT R12 [+7]
  LOADK R15 K8 ["Folder"]
  NAMECALL R13 R12 K9 ["IsA"]
  CALL R13 2 1
  JUMPIFNOT R13 [+2]
  MOVE R10 R12
  JUMP [+10]
  GETIMPORT R13 K12 [Instance.new]
  LOADK R14 K8 ["Folder"]
  CALL R13 1 1
  LOADK R14 K6 ["R15MigratorProgress"]
  SETTABLEKS R14 R13 K13 ["Name"]
  SETTABLEKS R11 R13 K14 ["Parent"]
  MOVE R10 R13
  SETUPVAL R10 0
  GETUPVAL R8 0
  NAMECALL R6 R6 K15 ["format"]
  CALL R6 2 1
  MOVE R5 R6
  CALL R4 1 0
  RETURN R0 0
  RETURN R3 1

PROTO_22:
  DUPCLOSURE R2 K0 [PROTO_0]
  GETUPVAL R3 0
  JUMPIF R3 [+24]
  GETUPVAL R4 1
  LOADK R7 K1 ["R15MigratorProgress"]
  NAMECALL R5 R4 K2 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+7]
  LOADK R8 K3 ["Folder"]
  NAMECALL R6 R5 K4 ["IsA"]
  CALL R6 2 1
  JUMPIFNOT R6 [+2]
  MOVE R3 R5
  JUMP [+10]
  GETIMPORT R6 K7 [Instance.new]
  LOADK R7 K3 ["Folder"]
  CALL R6 1 1
  LOADK R7 K1 ["R15MigratorProgress"]
  SETTABLEKS R7 R6 K8 ["Name"]
  SETTABLEKS R4 R6 K9 ["Parent"]
  MOVE R3 R6
  SETUPVAL R3 0
  GETUPVAL R3 0
  LOADK R6 K10 ["R6KeyframeSequences"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R1 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K10 ["R6KeyframeSequences"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R1 R5
  NEWTABLE R2 0 0
  NAMECALL R3 R1 K11 ["GetChildren"]
  CALL R3 1 3
  FORGPREP R3
  GETTABLEKS R8 R7 K8 ["Name"]
  SETTABLE R7 R2 R8
  FORGLOOP R3 2 [-4]
  NEWTABLE R3 0 0
  MOVE R4 R0
  LOADNIL R5
  LOADNIL R6
  FORGPREP R4
  GETUPVAL R10 2
  GETTABLEKS R9 R10 K12 ["getR6KeyframeSequence"]
  MOVE R10 R8
  MOVE R11 R2
  CALL R9 2 1
  SETTABLE R9 R3 R8
  FORGLOOP R4 2 [-8]
  RETURN R3 1

PROTO_23:
  DUPCLOSURE R2 K0 [PROTO_0]
  GETUPVAL R3 0
  JUMPIF R3 [+24]
  GETUPVAL R4 1
  LOADK R7 K1 ["R15MigratorProgress"]
  NAMECALL R5 R4 K2 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+7]
  LOADK R8 K3 ["Folder"]
  NAMECALL R6 R5 K4 ["IsA"]
  CALL R6 2 1
  JUMPIFNOT R6 [+2]
  MOVE R3 R5
  JUMP [+10]
  GETIMPORT R6 K7 [Instance.new]
  LOADK R7 K3 ["Folder"]
  CALL R6 1 1
  LOADK R7 K1 ["R15MigratorProgress"]
  SETTABLEKS R7 R6 K8 ["Name"]
  SETTABLEKS R4 R6 K9 ["Parent"]
  MOVE R3 R6
  SETUPVAL R3 0
  GETUPVAL R3 0
  LOADK R6 K10 ["R15KeyframeSequences"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R1 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K10 ["R15KeyframeSequences"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R1 R5
  NEWTABLE R2 0 0
  NAMECALL R3 R1 K11 ["GetChildren"]
  CALL R3 1 3
  FORGPREP R3
  GETTABLEKS R9 R7 K8 ["Name"]
  FASTCALL1 TONUMBER R9 [+2]
  GETIMPORT R8 K13 [tonumber]
  CALL R8 1 1
  LOADB R9 1
  SETTABLE R9 R2 R8
  FORGLOOP R3 2 [-9]
  MOVE R3 R0
  LOADNIL R4
  LOADNIL R5
  FORGPREP R3
  GETTABLE R8 R2 R7
  JUMPIF R8 [+34]
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K14 ["getConvertedKeyframeSequence"]
  MOVE R9 R7
  CALL R8 1 1
  JUMPIFNOT R8 [+28]
  LOADB R9 1
  SETTABLE R9 R2 R7
  GETTABLEKS R9 R8 K9 ["Parent"]
  NAMECALL R9 R9 K15 ["Clone"]
  CALL R9 1 1
  SETTABLEKS R7 R9 K8 ["Name"]
  GETUPVAL R13 3
  GETTABLEKS R12 R13 K16 ["TempId"]
  LOADNIL R13
  NAMECALL R10 R9 K17 ["SetAttribute"]
  CALL R10 3 0
  SETTABLEKS R1 R9 K9 ["Parent"]
  GETUPVAL R10 4
  MOVE R12 R9
  NAMECALL R10 R10 K18 ["clearPublishTags"]
  CALL R10 2 0
  GETUPVAL R11 5
  GETTABLEKS R10 R11 K19 ["clearTags"]
  MOVE R11 R9
  CALL R10 1 0
  FORGLOOP R3 2 [-37]
  RETURN R0 0

PROTO_24:
  JUMPIFNOT R0 [+1]
  JUMPIF R1 [+1]
  RETURN R0 0
  FASTCALL1 TOSTRING R0 [+3]
  MOVE R3 R0
  GETIMPORT R2 K1 [tostring]
  CALL R2 1 1
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R4 R1
  GETIMPORT R3 K1 [tostring]
  CALL R3 1 1
  DUPCLOSURE R5 K2 [PROTO_0]
  GETUPVAL R6 0
  JUMPIF R6 [+24]
  GETUPVAL R7 1
  LOADK R10 K3 ["R15MigratorProgress"]
  NAMECALL R8 R7 K4 ["FindFirstChild"]
  CALL R8 2 1
  JUMPIFNOT R8 [+7]
  LOADK R11 K5 ["Folder"]
  NAMECALL R9 R8 K6 ["IsA"]
  CALL R9 2 1
  JUMPIFNOT R9 [+2]
  MOVE R6 R8
  JUMP [+10]
  GETIMPORT R9 K9 [Instance.new]
  LOADK R10 K5 ["Folder"]
  CALL R9 1 1
  LOADK R10 K3 ["R15MigratorProgress"]
  SETTABLEKS R10 R9 K10 ["Name"]
  SETTABLEKS R7 R9 K11 ["Parent"]
  MOVE R6 R9
  SETUPVAL R6 0
  GETUPVAL R6 0
  LOADK R9 K12 ["R15KeyframeSequences"]
  NAMECALL R7 R6 K4 ["FindFirstChild"]
  CALL R7 2 1
  JUMPIFNOT R7 [+7]
  LOADK R10 K5 ["Folder"]
  NAMECALL R8 R7 K6 ["IsA"]
  CALL R8 2 1
  JUMPIFNOT R8 [+2]
  MOVE R4 R7
  JUMP [+10]
  GETIMPORT R8 K9 [Instance.new]
  LOADK R9 K5 ["Folder"]
  CALL R8 1 1
  LOADK R9 K12 ["R15KeyframeSequences"]
  SETTABLEKS R9 R8 K10 ["Name"]
  SETTABLEKS R6 R8 K11 ["Parent"]
  MOVE R4 R8
  MOVE R7 R2
  NAMECALL R5 R4 K4 ["FindFirstChild"]
  CALL R5 2 1
  JUMPIFNOT R5 [+5]
  LOADK R8 K13 ["StringValue"]
  NAMECALL R6 R5 K6 ["IsA"]
  CALL R6 2 1
  JUMPIF R6 [+1]
  RETURN R0 0
  GETTABLEKS R7 R5 K14 ["Value"]
  LENGTH R6 R7
  LOADN R7 0
  JUMPIFNOTLT R7 R6 [+39]
  GETIMPORT R6 K16 [warn]
  LOADK R8 K17 ["%*.Value in folder %* should be empty. Was the %* folder edited?"]
  MOVE R10 R2
  LOADK R11 K12 ["R15KeyframeSequences"]
  DUPCLOSURE R13 K2 [PROTO_0]
  GETUPVAL R14 0
  JUMPIF R14 [+24]
  GETUPVAL R15 1
  LOADK R18 K3 ["R15MigratorProgress"]
  NAMECALL R16 R15 K4 ["FindFirstChild"]
  CALL R16 2 1
  JUMPIFNOT R16 [+7]
  LOADK R19 K5 ["Folder"]
  NAMECALL R17 R16 K6 ["IsA"]
  CALL R17 2 1
  JUMPIFNOT R17 [+2]
  MOVE R14 R16
  JUMP [+10]
  GETIMPORT R17 K9 [Instance.new]
  LOADK R18 K5 ["Folder"]
  CALL R17 1 1
  LOADK R18 K3 ["R15MigratorProgress"]
  SETTABLEKS R18 R17 K10 ["Name"]
  SETTABLEKS R15 R17 K11 ["Parent"]
  MOVE R14 R17
  SETUPVAL R14 0
  GETUPVAL R12 0
  NAMECALL R8 R8 K18 ["format"]
  CALL R8 4 1
  MOVE R7 R8
  CALL R6 1 0
  SETTABLEKS R3 R5 K14 ["Value"]
  RETURN R0 0

PROTO_25:
  NEWTABLE R1 0 0
  MOVE R2 R0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  LOADK R9 K0 ["Animation"]
  NAMECALL R7 R6 K1 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+27]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K2 ["getNormalizedAssetId"]
  MOVE R8 R6
  CALL R7 1 1
  MOVE R9 R7
  JUMPIFNOT R9 [+9]
  FASTCALL1 TYPEOF R7 [+3]
  MOVE R11 R7
  GETIMPORT R10 K4 [typeof]
  CALL R10 1 1
  JUMPIFEQKS R10 K5 ["number"] [+2]
  LOADB R9 0 +1
  LOADB R9 1
  FASTCALL1 ASSERT R9 [+2]
  GETIMPORT R8 K7 [assert]
  CALL R8 1 0
  FASTCALL2 TABLE_INSERT R1 R7 [+5]
  MOVE R9 R1
  MOVE R10 R7
  GETIMPORT R8 K10 [table.insert]
  CALL R8 2 0
  FORGLOOP R2 2 [-33]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K11 ["addConvertibleAssetIds"]
  MOVE R3 R1
  CALL R2 1 0
  RETURN R0 0

PROTO_26:
  DUPCLOSURE R1 K0 [PROTO_0]
  GETUPVAL R2 0
  JUMPIF R2 [+24]
  GETUPVAL R3 1
  LOADK R6 K1 ["R15MigratorProgress"]
  NAMECALL R4 R3 K2 ["FindFirstChild"]
  CALL R4 2 1
  JUMPIFNOT R4 [+7]
  LOADK R7 K3 ["Folder"]
  NAMECALL R5 R4 K4 ["IsA"]
  CALL R5 2 1
  JUMPIFNOT R5 [+2]
  MOVE R2 R4
  JUMP [+10]
  GETIMPORT R5 K7 [Instance.new]
  LOADK R6 K3 ["Folder"]
  CALL R5 1 1
  LOADK R6 K1 ["R15MigratorProgress"]
  SETTABLEKS R6 R5 K8 ["Name"]
  SETTABLEKS R3 R5 K9 ["Parent"]
  MOVE R2 R5
  SETUPVAL R2 0
  GETUPVAL R2 0
  LOADK R5 K10 ["R15KeyframeSequences"]
  NAMECALL R3 R2 K2 ["FindFirstChild"]
  CALL R3 2 1
  JUMPIFNOT R3 [+7]
  LOADK R6 K3 ["Folder"]
  NAMECALL R4 R3 K4 ["IsA"]
  CALL R4 2 1
  JUMPIFNOT R4 [+2]
  MOVE R0 R3
  JUMP [+10]
  GETIMPORT R4 K7 [Instance.new]
  LOADK R5 K3 ["Folder"]
  CALL R4 1 1
  LOADK R5 K10 ["R15KeyframeSequences"]
  SETTABLEKS R5 R4 K8 ["Name"]
  SETTABLEKS R2 R4 K9 ["Parent"]
  MOVE R0 R4
  NEWTABLE R1 0 0
  NAMECALL R2 R0 K11 ["GetChildren"]
  CALL R2 1 3
  FORGPREP R2
  LOADK R9 K12 ["StringValue"]
  NAMECALL R7 R6 K4 ["IsA"]
  CALL R7 2 1
  JUMPIFNOT R7 [+30]
  GETTABLEKS R8 R6 K8 ["Name"]
  FASTCALL1 TONUMBER R8 [+2]
  GETIMPORT R7 K14 [tonumber]
  CALL R7 1 1
  JUMPIFNOT R7 [+23]
  GETTABLEKS R9 R6 K15 ["Value"]
  FASTCALL1 TONUMBER R9 [+2]
  GETIMPORT R8 K14 [tonumber]
  CALL R8 1 1
  JUMPIFNOT R8 [+16]
  DUPTABLE R9 K19 [{"converted", "kfs", "name"}]
  SETTABLEKS R8 R9 K16 ["converted"]
  LOADK R12 K20 ["KeyframeSequence"]
  NAMECALL R10 R6 K21 ["FindFirstChildWhichIsA"]
  CALL R10 2 1
  SETTABLEKS R10 R9 K17 ["kfs"]
  LOADK R12 K18 ["name"]
  NAMECALL R10 R6 K22 ["GetAttribute"]
  CALL R10 2 1
  SETTABLEKS R10 R9 K18 ["name"]
  SETTABLE R9 R1 R7
  FORGLOOP R2 2 [-36]
  RETURN R1 1

MAIN:
  PREPVARARGS 0
  NEWTABLE R0 32 0
  GETIMPORT R1 K1 [game]
  LOADK R3 K2 ["ServerStorage"]
  NAMECALL R1 R1 K3 ["GetService"]
  CALL R1 2 1
  GETIMPORT R2 K1 [game]
  LOADK R4 K4 ["HttpService"]
  NAMECALL R2 R2 K3 ["GetService"]
  CALL R2 2 1
  GETIMPORT R3 K6 [script]
  LOADK R5 K7 ["R15Migrator"]
  NAMECALL R3 R3 K8 ["FindFirstAncestor"]
  CALL R3 2 1
  GETIMPORT R4 K10 [require]
  GETTABLEKS R6 R3 K11 ["Src"]
  GETTABLEKS R5 R6 K12 ["Types"]
  CALL R4 1 1
  GETIMPORT R6 K10 [require]
  GETTABLEKS R9 R3 K11 ["Src"]
  GETTABLEKS R8 R9 K13 ["Resources"]
  GETTABLEKS R7 R8 K14 ["Constants"]
  CALL R6 1 1
  GETTABLEKS R5 R6 K15 ["Attributes"]
  GETIMPORT R6 K10 [require]
  GETTABLEKS R9 R3 K11 ["Src"]
  GETTABLEKS R8 R9 K16 ["Util"]
  GETTABLEKS R7 R8 K17 ["PublishTagging"]
  CALL R6 1 1
  GETIMPORT R7 K10 [require]
  GETTABLEKS R10 R3 K11 ["Src"]
  GETTABLEKS R9 R10 K16 ["Util"]
  GETTABLEKS R8 R9 K18 ["QuickAccessTagging"]
  CALL R7 1 1
  GETTABLEKS R10 R3 K11 ["Src"]
  GETTABLEKS R9 R10 K16 ["Util"]
  GETTABLEKS R8 R9 K19 ["AnimationConversion"]
  GETIMPORT R9 K10 [require]
  GETTABLEKS R10 R8 K20 ["AnimationConversionManager"]
  CALL R9 1 1
  LOADNIL R10
  NEWCLOSURE R11 P0
  CAPTURE REF R10
  CAPTURE VAL R1
  NEWCLOSURE R12 P1
  CAPTURE REF R10
  CAPTURE VAL R1
  DUPCLOSURE R13 K21 [PROTO_3]
  CAPTURE VAL R6
  CAPTURE VAL R7
  NEWCLOSURE R14 P3
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K22 ["isRootSaveFolderChildOf"]
  NEWCLOSURE R14 P4
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K23 ["isRootSaveFolderDescendantOf"]
  NEWCLOSURE R14 P5
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K24 ["isRootSaveFolder"]
  NEWCLOSURE R14 P6
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K25 ["isDescendantOfSaveFolder"]
  DUPCLOSURE R14 K26 [PROTO_8]
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K27 ["doesSaveFolderExistInDM"]
  DUPCLOSURE R14 K28 [PROTO_9]
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K29 ["removeAllSavedData"]
  DUPCLOSURE R14 K30 [PROTO_11]
  SETTABLEKS R14 R0 K31 ["conversionIdFilter"]
  DUPCLOSURE R14 K32 [PROTO_12]
  SETTABLEKS R14 R0 K33 ["removeConversionId"]
  DUPCLOSURE R14 K34 [PROTO_13]
  SETTABLEKS R14 R0 K35 ["getConversionId"]
  DUPCLOSURE R14 K36 [PROTO_14]
  CAPTURE VAL R0
  SETTABLEKS R14 R0 K37 ["hasConvertedFrom"]
  NEWCLOSURE R14 P13
  CAPTURE REF R10
  CAPTURE VAL R1
  CAPTURE VAL R2
  CAPTURE VAL R6
  CAPTURE VAL R7
  SETTABLEKS R14 R0 K38 ["addConvertibleCharacters"]
  NEWCLOSURE R14 P14
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K39 ["getR6Characters"]
  NEWCLOSURE R14 P15
  CAPTURE REF R10
  CAPTURE VAL R1
  CAPTURE VAL R2
  SETTABLEKS R14 R0 K40 ["addScriptConversionRules"]
  NEWCLOSURE R14 P16
  CAPTURE REF R10
  CAPTURE VAL R1
  CAPTURE VAL R2
  SETTABLEKS R14 R0 K41 ["getScriptConversionRules"]
  DUPCLOSURE R14 K42 [PROTO_19]
  SETTABLEKS R14 R0 K43 ["isConvertibleKeyframeSequence"]
  NEWCLOSURE R14 P18
  CAPTURE REF R10
  CAPTURE VAL R1
  CAPTURE VAL R2
  CAPTURE VAL R5
  CAPTURE VAL R6
  CAPTURE VAL R7
  SETTABLEKS R14 R0 K44 ["addConvertibleKeyframeSequences"]
  NEWCLOSURE R14 P19
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K45 ["getR6KeyframeSequence"]
  NEWCLOSURE R14 P20
  CAPTURE REF R10
  CAPTURE VAL R1
  CAPTURE VAL R0
  SETTABLEKS R14 R0 K46 ["getOriginalR6KeyframeSequences"]
  NEWCLOSURE R14 P21
  CAPTURE REF R10
  CAPTURE VAL R1
  CAPTURE VAL R9
  CAPTURE VAL R5
  CAPTURE VAL R6
  CAPTURE VAL R7
  SETTABLEKS R14 R0 K47 ["addConvertibleAssetIds"]
  NEWCLOSURE R14 P22
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K48 ["setTargetAssetId"]
  DUPCLOSURE R14 K49 [PROTO_25]
  CAPTURE VAL R9
  CAPTURE VAL R0
  SETTABLEKS R14 R0 K50 ["addConvertibleAnimations"]
  NEWCLOSURE R14 P24
  CAPTURE REF R10
  CAPTURE VAL R1
  SETTABLEKS R14 R0 K51 ["getAllSavedAnimationAssetIds"]
  CLOSEUPVALS R10
  RETURN R0 1

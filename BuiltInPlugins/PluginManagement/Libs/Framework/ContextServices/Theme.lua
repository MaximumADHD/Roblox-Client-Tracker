	...
	Example usage:
		...
		local function makeTheme()
			...
			return Theme.new(function(theme, getColor)
				return {
					Framework = StudioFrameworkTheme.new(theme, getColor),
					Plugin = pluginTheme,
				}
			end)
		end
...
You need to define a 'Default' Style in the component style table.]]

local noFallbackError = [[
No '%s' Style was defined for component '%s', and there were no defaults to fall back to.
You need to define a Default table in the component StyleTable.]]
...
function Theme:get(namespace)
	if self.values[namespace] then
		...
		error("Invalid Theme namespace: %s", namespace)
	end
end

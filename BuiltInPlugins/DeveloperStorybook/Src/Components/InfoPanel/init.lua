...
function InfoPanel:render()
		...
		return React.createElement(ScrollingFrame, {
			...
			Content = React.createElement(View, {
				...
				Prompt = React.createElement(Text, {
					...
					tag = "Error auto-xy text-wrap text-align-x-left",
				}),
			}),
		})
	end
	...
	if not storyProps then
		return React.createElement(View, {
			tag = "col align-x-center align-y-center gap-medium size-full-full",
		}, {
	...
	if definition.story then
		children.Story = React.createElement(StoryHost, {
			...
			ColorMode = props.ColorMode or ThemeSwitcher.getThemeName(),
	...
	if definitelyStoryProps.storybook.fixedSize then
		main = React.createElement(View, {
			LayoutOrder = 1,
			tag = "col fill gap-medium size-full-0 auto-y",
			ref = self.storyRef,
		}, children)
	else
	...
	return React.createElement(View, {
		tag = "col flex-fill size-full-full clip",
	}, {
		Main = main,
		...
		Dialog = React.createElement(StyledDialog, {
			...
			Contents = React.createElement(Text, {
				...
				tag = "size-full-0 auto-y text-wrap text-align-x-left content-action-alert",
			}),
		}),
	})
end

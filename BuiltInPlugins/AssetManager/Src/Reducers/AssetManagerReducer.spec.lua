...
return function()
	it("should return a table with the correct members", function()
		...
		expect(state.recentAssets).to.be.ok()
	...
	describe("SetRecentAssets action", function()
		it("should validate its inputs", function()
			expect(function()
				SetRecentAssets(nil)
			end).to.throw()
		...
		it("should preserve immutability", function()
			local immutabilityPreserved = testImmutability(AssetManagerReducer, SetRecentAssets({asset = "mesh"}))
			expect(immutabilityPreserved).to.equal(true)
		end)

		it("should set recently imported assets table", function()
			local r = Rodux.Store.new(AssetManagerReducer)
			local state = r:getState()
			expect(#state.recentAssets).to.equal(0)

            state = AssetManagerReducer(state, SetRecentAssets({asset = "mesh"}))
            expect(state.recentAssets.asset).to.equal("mesh")
		end)
	end)

	describe("SetRecentViewToggled action", function()
		it("should validate its inputs", function()
			expect(function()
				SetRecentViewToggled(nil)
			end).to.throw()

			expect(function()
				SetRecentViewToggled({ key = "value", })
			end).to.throw()

			expect(function()
				SetRecentViewToggled(1)
			end).to.throw()
		end)

		it("should preserve immutability", function()
			local immutabilityPreserved = testImmutability(AssetManagerReducer, SetRecentViewToggled(true))
			expect(immutabilityPreserved).to.equal(true)
		end)

		it("should set recent view toggled", function()
			local r = Rodux.Store.new(AssetManagerReducer)
			local state = r:getState()
			expect(state.recentViewToggled).to.equal(false)

            state = AssetManagerReducer(state, SetRecentViewToggled(true))
            expect(state.recentViewToggled).to.equal(true)
		end)
	end)

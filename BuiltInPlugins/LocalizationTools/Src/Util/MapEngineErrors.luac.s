PROTO_0:
        ...
        6 CALL                             R2 2 0
        7 NEWTABLE                         R2 0 4
        9 DUPTABLE                         R3 K9 [{["pattern"] = "^Error reading (.-): Missing a Key or Source header column.$", ["key"] = "MissingKeyOrSource", ["numArguments"] = 1}]
       10 DUPTABLE                         R4 K11 [{["pattern"] = "^.+%s(.+)%s?: Missing a Key or Source header column.%s?.*$", ["key"] = "MissingKeyOrSource", ["numArguments"] = 1}]
       11 DUPTABLE                         R5 K14 [{["pattern"] = "^.+%s(.+)%s?: Error opening CSV .-: Invalid UTF%-8. File must be UTF%-8 encoded.%s?.*$", ["key"] = "NonUtf8", ["numArguments"] = 1}]
       12 DUPTABLE                         R6 K16 [{["pattern"] = "^Error opening CSV (.-): Invalid UTF%-8. File must be UTF%-8 encoded.$", ["key"] = "NonUtf8", ["numArguments"] = 1}]
       13 SETLIST                          R2 R3 4 [1]
       15 MOVE                             R3 R2
       16 LOADNIL                          R4
       17 LOADNIL                          R5
       18 FORGPREP                         R3
       19 GETGLOBAL                        R8 K17 ["matchPatternToMessage"]
       21 MOVE                             R9 R0
       22 MOVE                             R10 R7
       23 MOVE                             R11 R1
       24 CALL                             R8 3 2

PROTO_0:
  GETIMPORT R1 K2 [string.gsub]
  MOVE R2 R0
  LOADK R3 K3 ["%s*Animation%s*"]
  LOADK R4 K4 [""]
  CALL R1 3 1
  MOVE R0 R1
  GETIMPORT R1 K2 [string.gsub]
  MOVE R2 R0
  LOADK R3 K5 ["%s*Node%s*"]
  LOADK R4 K4 [""]
  CALL R1 3 1
  MOVE R0 R1
  GETIMPORT R1 K2 [string.gsub]
  MOVE R2 R0
  LOADK R3 K6 ["([a-z])([A-Z])"]
  LOADK R4 K7 ["%1 %2"]
  CALL R1 3 1
  MOVE R0 R1
  RETURN R0 1

PROTO_1:
  JUMPIFNOT R0 [+6]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["PARAMETER_NODE_CLASSNAME"]
  JUMPIFNOTEQ R1 R2 [+2]
  RETURN R0 1
  JUMPIFNOT R0 [+40]
  JUMPIFNOT R1 [+39]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["formatName"]
  MOVE R3 R1
  CALL R2 1 1
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K1 ["formatName"]
  MOVE R4 R0
  CALL R3 1 1
  JUMPIFNOTEQ R3 R2 [+2]
  RETURN R3 1
  GETIMPORT R4 K4 [string.gsub]
  MOVE R5 R3
  LOADK R7 K5 ["\b%*\b"]
  MOVE R9 R2
  NAMECALL R7 R7 K6 ["format"]
  CALL R7 2 1
  MOVE R6 R7
  LOADK R7 K7 [""]
  CALL R4 3 1
  MOVE R3 R4
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["formatName"]
  MOVE R5 R3
  CALL R4 1 1
  MOVE R3 R4
  LOADK R5 K8 ["%* (%*)"]
  MOVE R7 R3
  MOVE R8 R2
  NAMECALL R5 R5 K6 ["format"]
  CALL R5 3 1
  MOVE R4 R5
  RETURN R4 1
  JUMPIFNOT R0 [+6]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["formatName"]
  MOVE R3 R0
  CALL R2 1 -1
  RETURN R2 -1
  JUMPIFNOT R1 [+6]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["formatName"]
  MOVE R3 R1
  CALL R2 1 -1
  RETURN R2 -1
  LOADK R2 K9 ["Untitled"]
  RETURN R2 1

PROTO_2:
  JUMPIFNOTEQKS R0 K0 ["animationId"] [+3]
  LOADK R1 K1 ["Clip"]
  RETURN R1 1
  GETIMPORT R1 K4 [string.gsub]
  MOVE R2 R0
  LOADK R3 K5 ["([a-z])([A-Z])"]
  LOADK R4 K6 ["%1 %2"]
  CALL R1 3 1
  MOVE R0 R1
  GETIMPORT R1 K4 [string.gsub]
  MOVE R2 R0
  LOADK R3 K7 ["^%l"]
  GETIMPORT R4 K9 [string.upper]
  CALL R1 3 1
  MOVE R0 R1
  RETURN R0 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["CompositorDebugger"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["AnimationEditor"]
  CALL R1 1 1
  GETTABLEKS R2 R1 K8 ["Constants"]
  NEWTABLE R3 4 0
  DUPCLOSURE R4 K9 [PROTO_0]
  SETTABLEKS R4 R3 K10 ["formatName"]
  DUPCLOSURE R4 K11 [PROTO_1]
  CAPTURE VAL R2
  CAPTURE VAL R3
  SETTABLEKS R4 R3 K12 ["getDisplayName"]
  DUPCLOSURE R4 K13 [PROTO_2]
  SETTABLEKS R4 R3 K14 ["formatPropertyName"]
  RETURN R3 1

PROTO_0:
  NEWTABLE R2 0 0
  GETIMPORT R3 K1 [pairs]
  MOVE R4 R1
  CALL R3 1 3
  FORGPREP_NEXT R3
  GETUPVAL R10 0
  GETTABLEKS R9 R10 K2 ["SubjectNameKey"]
  GETTABLE R8 R7 R9
  NAMECALL R8 R8 K3 ["lower"]
  CALL R8 1 1
  NAMECALL R10 R0 K3 ["lower"]
  CALL R10 1 1
  LOADN R11 1
  LOADB R12 1
  NAMECALL R8 R8 K4 ["find"]
  CALL R8 4 1
  JUMPIFNOT R8 [+7]
  FASTCALL2 TABLE_INSERT R2 R7 [+5]
  MOVE R9 R2
  MOVE R10 R7
  GETIMPORT R8 K7 [table.insert]
  CALL R8 2 0
  FORGLOOP R3 2 [-24]
  RETURN R2 1

PROTO_1:
  GETTABLEKS R2 R0 K0 ["localUserFriends"]
  GETUPVAL R3 0
  JUMPIFEQ R2 R3 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_2:
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K0 ["SubjectNameKey"]
  GETTABLE R4 R0 R5
  NAMECALL R4 R4 K1 ["lower"]
  CALL R4 1 1
  MOVE R2 R4
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K0 ["SubjectNameKey"]
  GETTABLE R3 R1 R4
  NAMECALL R3 R3 K1 ["lower"]
  CALL R3 1 1
  JUMPIFNOTEQ R2 R3 [+3]
  LOADB R4 0
  RETURN R4 1
  GETUPVAL R4 1
  NAMECALL R4 R4 K1 ["lower"]
  CALL R4 1 1
  JUMPIFNOTEQ R2 R4 [+3]
  LOADB R4 1
  RETURN R4 1
  GETUPVAL R4 1
  NAMECALL R4 R4 K1 ["lower"]
  CALL R4 1 1
  JUMPIFNOTEQ R3 R4 [+3]
  LOADB R4 0
  RETURN R4 1
  JUMPIFLT R2 R3 [+2]
  LOADB R4 0 +1
  LOADB R4 1
  RETURN R4 1

PROTO_3:
  GETTABLEKS R3 R0 K0 ["cachedSearchResults"]
  GETTABLEKS R4 R0 K1 ["searchText"]
  NEWCLOSURE R5 P0
  CAPTURE UPVAL U0
  CAPTURE VAL R4
  DUPTABLE R6 K3 [{"Users"}]
  NEWTABLE R7 0 0
  SETTABLEKS R7 R6 K2 ["Users"]
  NEWTABLE R7 0 0
  NEWTABLE R8 0 0
  GETTABLEKS R11 R0 K4 ["localUserFriends"]
  FASTCALL1 TYPEOF R11 [+2]
  GETIMPORT R10 K6 [typeof]
  CALL R10 1 1
  JUMPIFNOTEQKS R10 K7 ["table"] [+7]
  GETUPVAL R9 1
  MOVE R10 R4
  GETTABLEKS R11 R0 K4 ["localUserFriends"]
  CALL R9 2 1
  JUMPIF R9 [+2]
  NEWTABLE R9 0 0
  GETIMPORT R10 K9 [table.sort]
  MOVE R11 R9
  MOVE R12 R5
  CALL R10 2 0
  LOADB R10 0
  LENGTH R11 R7
  LOADN R12 0
  JUMPIFNOTLT R12 R11 [+16]
  GETTABLEN R12 R7 1
  GETUPVAL R14 0
  GETTABLEKS R13 R14 K10 ["SubjectNameKey"]
  GETTABLE R11 R12 R13
  NAMECALL R11 R11 K11 ["lower"]
  CALL R11 1 1
  NAMECALL R12 R4 K11 ["lower"]
  CALL R12 1 1
  JUMPIFEQ R11 R12 [+2]
  LOADB R10 0 +1
  LOADB R10 1
  JUMPIFNOT R10 [+2]
  LOADN R12 1
  JUMP [+1]
  LOADN R12 2
  LENGTH R14 R8
  ADDK R13 R14 K12 [1]
  FASTCALL2 MATH_MIN R12 R13 [+3]
  GETIMPORT R11 K15 [math.min]
  CALL R11 2 1
  GETIMPORT R12 K17 [pairs]
  MOVE R13 R9
  CALL R12 1 3
  FORGPREP_NEXT R12
  GETUPVAL R19 0
  GETTABLEKS R18 R19 K18 ["SubjectIdKey"]
  GETTABLE R17 R16 R18
  JUMPIFNOT R1 [+20]
  GETUPVAL R21 0
  GETTABLEKS R20 R21 K19 ["UserSubjectKey"]
  GETTABLE R19 R1 R20
  GETTABLE R18 R19 R17
  JUMPIFNOT R18 [+14]
  GETUPVAL R22 0
  GETTABLEKS R21 R22 K19 ["UserSubjectKey"]
  GETTABLE R20 R1 R21
  GETTABLE R19 R20 R17
  GETUPVAL R21 0
  GETTABLEKS R20 R21 K20 ["ActionKey"]
  GETTABLE R18 R19 R20
  GETUPVAL R20 0
  GETTABLEKS R19 R20 K21 ["NoAccessKey"]
  JUMPIFNOTEQ R18 R19 [+12]
  GETTABLE R18 R7 R17
  JUMPIF R18 [+9]
  FASTCALL3 TABLE_INSERT R8 R11 R16
  MOVE R19 R8
  MOVE R20 R11
  MOVE R21 R16
  GETIMPORT R18 K23 [table.insert]
  CALL R18 3 0
  ADDK R11 R11 K12 [1]
  FORGLOOP R12 2 [-37]
  SETTABLEKS R8 R6 K2 ["Users"]
  RETURN R6 1

PROTO_4:
  LOADNIL R3
  JUMPIFNOTEQKS R0 K0 [""] [+4]
  NEWTABLE R4 0 0
  RETURN R4 1
  DUPTABLE R4 K2 [{"Users"}]
  DUPTABLE R5 K4 [{"LayoutOrder"}]
  LOADN R6 0
  SETTABLEKS R6 R5 K3 ["LayoutOrder"]
  SETTABLEKS R5 R4 K1 ["Users"]
  MOVE R3 R4
  GETIMPORT R4 K6 [pairs]
  GETTABLEKS R5 R1 K1 ["Users"]
  CALL R4 1 3
  FORGPREP_NEXT R4
  GETTABLEKS R11 R3 K1 ["Users"]
  LENGTH R10 R11
  ADDK R9 R10 K7 [1]
  GETUPVAL R11 0
  GETTABLEKS R10 R11 K8 ["MaxSearchResultsPerSubjectType"]
  JUMPIFLT R10 R9 [+77]
  GETTABLEKS R10 R3 K1 ["Users"]
  DUPTABLE R11 K12 [{"Icon", "Name", "Key"}]
  GETUPVAL R13 1
  GETTABLEKS R12 R13 K13 ["createElement"]
  GETUPVAL R13 2
  DUPTABLE R14 K17 [{"Image", "Size", "UseMask"}]
  GETUPVAL R16 3
  GETTABLEKS R15 R16 K18 ["constructRBXThumbUrl"]
  GETUPVAL R18 4
  GETTABLEKS R17 R18 K20 ["rbxThumbTypes"]
  GETTABLEKS R16 R17 K19 ["AvatarHeadShot"]
  GETUPVAL R19 0
  GETTABLEKS R18 R19 K21 ["SubjectIdKey"]
  GETTABLE R17 R8 R18
  GETUPVAL R20 4
  GETTABLEKS R19 R20 K22 ["rbxThumbSizes"]
  GETTABLEKS R18 R19 K23 ["AvatarHeadshotImageSize"]
  CALL R15 3 1
  SETTABLEKS R15 R14 K14 ["Image"]
  GETIMPORT R15 K26 [UDim2.new]
  LOADN R16 1
  LOADN R17 0
  LOADN R18 1
  LOADN R19 0
  CALL R15 4 1
  SETTABLEKS R15 R14 K15 ["Size"]
  LOADB R15 1
  SETTABLEKS R15 R14 K16 ["UseMask"]
  CALL R12 2 1
  SETTABLEKS R12 R11 K9 ["Icon"]
  GETUPVAL R14 0
  GETTABLEKS R13 R14 K27 ["SubjectNameKey"]
  GETTABLE R12 R8 R13
  SETTABLEKS R12 R11 K10 ["Name"]
  DUPTABLE R12 K30 [{"Type", "Id", "Name"}]
  GETUPVAL R14 0
  GETTABLEKS R13 R14 K31 ["UserSubjectKey"]
  SETTABLEKS R13 R12 K28 ["Type"]
  GETUPVAL R15 0
  GETTABLEKS R14 R15 K21 ["SubjectIdKey"]
  GETTABLE R13 R8 R14
  SETTABLEKS R13 R12 K29 ["Id"]
  GETUPVAL R15 0
  GETTABLEKS R14 R15 K27 ["SubjectNameKey"]
  GETTABLE R13 R8 R14
  SETTABLEKS R13 R12 K10 ["Name"]
  SETTABLEKS R12 R11 K11 ["Key"]
  FASTCALL2 TABLE_INSERT R10 R11 [+3]
  GETIMPORT R9 K34 [table.insert]
  CALL R9 2 0
  FORGLOOP R4 2 [-84]
  NEWTABLE R4 1 0
  LOADK R7 K35 ["AssetConfigPackagePermissions"]
  LOADK R8 K36 ["UsersCollaboratorType"]
  NAMECALL R5 R2 K37 ["getText"]
  CALL R5 3 1
  GETTABLEKS R8 R3 K1 ["Users"]
  LENGTH R7 R8
  LOADN R8 0
  JUMPIFNOTLT R8 R7 [+4]
  GETTABLEKS R6 R3 K1 ["Users"]
  JUMP [+1]
  LOADNIL R6
  SETTABLE R6 R4 R5
  MOVE R3 R4
  RETURN R3 1

PROTO_5:
  LOADNIL R4
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K0 ["UserSubjectKey"]
  JUMPIFNOTEQ R0 R5 [+42]
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K1 ["Dictionary"]
  GETTABLEKS R5 R6 K2 ["join"]
  GETUPVAL R7 2
  GETTABLEKS R6 R7 K3 ["Permissions"]
  NEWTABLE R7 1 0
  GETUPVAL R10 1
  GETTABLEKS R9 R10 K1 ["Dictionary"]
  GETTABLEKS R8 R9 K2 ["join"]
  GETUPVAL R11 2
  GETTABLEKS R10 R11 K3 ["Permissions"]
  GETTABLE R9 R10 R0
  NEWTABLE R10 1 0
  NEWTABLE R11 4 0
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K4 ["SubjectNameKey"]
  SETTABLE R2 R11 R12
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K5 ["SubjectIdKey"]
  SETTABLE R1 R11 R12
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K6 ["ActionKey"]
  SETTABLE R3 R11 R12
  SETTABLE R11 R10 R1
  CALL R8 2 1
  SETTABLE R8 R7 R0
  CALL R5 2 1
  MOVE R4 R5
  JUMP [+23]
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K7 ["GroupSubjectKey"]
  JUMPIFNOTEQ R0 R5 [+9]
  GETUPVAL R6 2
  GETTABLEKS R5 R6 K8 ["GroupCollaboratorAdded"]
  MOVE R6 R1
  MOVE R7 R2
  MOVE R8 R3
  CALL R5 3 0
  JUMP [+10]
  GETIMPORT R5 K10 [error]
  LOADK R7 K11 ["Unsupported type: "]
  FASTCALL1 TOSTRING R0 [+3]
  MOVE R9 R0
  GETIMPORT R8 K13 [tostring]
  CALL R8 1 1
  CONCAT R6 R7 R8
  CALL R5 1 0
  JUMPIFNOT R4 [+5]
  GETUPVAL R6 2
  GETTABLEKS R5 R6 K14 ["PermissionsChanged"]
  MOVE R6 R4
  CALL R5 1 0
  RETURN R0 0

PROTO_6:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["SearchRequested"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["props"]
  GETTABLEKS R3 R4 K2 ["Network"]
  GETTABLEKS R2 R3 K3 ["networkInterface"]
  MOVE R3 R0
  LOADB R4 1
  CALL R1 3 0
  RETURN R0 0

PROTO_7:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["SearchRequested"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["props"]
  GETTABLEKS R3 R4 K2 ["Network"]
  GETTABLEKS R2 R3 K3 ["networkInterface"]
  MOVE R3 R0
  LOADB R4 0
  CALL R1 3 0
  RETURN R0 0

PROTO_8:
  JUMPIFNOTEQKNIL R0 [+6]
  GETIMPORT R1 K1 [print]
  LOADK R2 K2 ["TODO: enable friends option"]
  CALL R1 1 0
  RETURN R0 0
  GETUPVAL R1 0
  GETTABLEKS R2 R0 K3 ["Type"]
  GETTABLEKS R3 R0 K4 ["Id"]
  GETTABLEKS R4 R0 K5 ["Name"]
  GETUPVAL R5 1
  CALL R1 4 0
  RETURN R0 0

PROTO_9:
  GETIMPORT R3 K1 [require]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["Src"]
  GETTABLEKS R5 R6 K3 ["Flags"]
  GETTABLEKS R4 R5 K4 ["getFFlagAACPermissionsPage"]
  CALL R3 1 1
  CALL R3 0 1
  NOT R2 R3
  FASTCALL2K ASSERT R2 K5 [+4]
  LOADK R3 K5 ["This component is deprecated by FFlagAACPermissionsPage"]
  GETIMPORT R1 K7 [assert]
  CALL R1 2 0
  GETTABLEKS R1 R0 K8 ["props"]
  GETTABLEKS R2 R1 K9 ["SearchData"]
  GETTABLEKS R3 R1 K10 ["Stylizer"]
  GETTABLEKS R4 R2 K11 ["SearchText"]
  GETUPVAL R5 1
  MOVE R6 R2
  GETTABLEKS R7 R1 K12 ["Permissions"]
  GETTABLEKS R8 R1 K13 ["GroupMetadata"]
  CALL R5 3 1
  GETTABLEKS R7 R2 K14 ["localUserFriends"]
  GETUPVAL R8 2
  JUMPIFEQ R7 R8 [+2]
  LOADB R6 0 +1
  LOADB R6 1
  LOADN R7 255
  GETTABLEKS R8 R1 K13 ["GroupMetadata"]
  JUMPIFNOT R8 [+9]
  GETIMPORT R8 K16 [pairs]
  GETTABLEKS R9 R1 K13 ["GroupMetadata"]
  CALL R8 1 3
  FORGPREP_NEXT R8
  ADDK R7 R7 K17 [1]
  FORGLOOP R8 2 [-2]
  GETTABLEKS R9 R1 K12 ["Permissions"]
  GETUPVAL R11 3
  GETTABLEKS R10 R11 K18 ["UserSubjectKey"]
  GETTABLE R8 R9 R10
  JUMPIFNOT R8 [+13]
  GETIMPORT R8 K16 [pairs]
  GETTABLEKS R11 R1 K12 ["Permissions"]
  GETUPVAL R13 3
  GETTABLEKS R12 R13 K18 ["UserSubjectKey"]
  GETTABLE R9 R11 R12
  CALL R8 1 3
  FORGPREP_NEXT R8
  ADDK R7 R7 K17 [1]
  FORGLOOP R8 2 [-2]
  LOADN R9 200
  JUMPIFLE R9 R7 [+2]
  LOADB R8 0 +1
  LOADB R8 1
  NEWCLOSURE R9 P0
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CAPTURE VAL R1
  GETUPVAL R10 5
  MOVE R11 R4
  MOVE R12 R5
  GETTABLEKS R13 R1 K19 ["Localization"]
  CALL R10 3 1
  GETTABLEKS R11 R1 K19 ["Localization"]
  LOADK R13 K20 ["AssetConfigPackagePermissions"]
  LOADK R14 K21 ["TooManyCollaboratorsText"]
  DUPTABLE R15 K23 [{"maxNumCollaborators"}]
  LOADN R16 200
  SETTABLEKS R16 R15 K22 ["maxNumCollaborators"]
  NAMECALL R11 R11 K24 ["getText"]
  CALL R11 4 1
  GETUPVAL R13 6
  GETTABLEKS R12 R13 K25 ["createElement"]
  LOADK R13 K26 ["Frame"]
  DUPTABLE R14 K30 [{"AutomaticSize", "BackgroundTransparency", "LayoutOrder"}]
  GETIMPORT R15 K33 [Enum.AutomaticSize.XY]
  SETTABLEKS R15 R14 K27 ["AutomaticSize"]
  LOADN R15 1
  SETTABLEKS R15 R14 K28 ["BackgroundTransparency"]
  GETTABLEKS R15 R1 K29 ["LayoutOrder"]
  SETTABLEKS R15 R14 K29 ["LayoutOrder"]
  DUPTABLE R15 K37 [{"UIListLayout", "Title", "CollaboratorSearchBar"}]
  GETUPVAL R17 6
  GETTABLEKS R16 R17 K25 ["createElement"]
  LOADK R17 K34 ["UIListLayout"]
  DUPTABLE R18 K40 [{"SortOrder", "Padding"}]
  GETIMPORT R19 K41 [Enum.SortOrder.LayoutOrder]
  SETTABLEKS R19 R18 K38 ["SortOrder"]
  GETIMPORT R19 K44 [UDim.new]
  LOADN R20 0
  LOADN R21 24
  CALL R19 2 1
  SETTABLEKS R19 R18 K39 ["Padding"]
  CALL R16 2 1
  SETTABLEKS R16 R15 K34 ["UIListLayout"]
  GETUPVAL R17 6
  GETTABLEKS R16 R17 K25 ["createElement"]
  LOADK R17 K45 ["TextLabel"]
  DUPTABLE R18 K51 [{"AutomaticSize", "LayoutOrder", "Text", "TextXAlignment", "BackgroundTransparency", "Font", "TextSize", "TextColor3"}]
  GETIMPORT R19 K33 [Enum.AutomaticSize.XY]
  SETTABLEKS R19 R18 K27 ["AutomaticSize"]
  LOADN R19 0
  SETTABLEKS R19 R18 K29 ["LayoutOrder"]
  GETTABLEKS R19 R1 K19 ["Localization"]
  LOADK R21 K20 ["AssetConfigPackagePermissions"]
  LOADK R22 K52 ["ShareWith"]
  NAMECALL R19 R19 K24 ["getText"]
  CALL R19 3 1
  SETTABLEKS R19 R18 K46 ["Text"]
  GETIMPORT R19 K54 [Enum.TextXAlignment.Left]
  SETTABLEKS R19 R18 K47 ["TextXAlignment"]
  LOADN R19 1
  SETTABLEKS R19 R18 K28 ["BackgroundTransparency"]
  GETUPVAL R20 7
  GETTABLEKS R19 R20 K55 ["FONT"]
  SETTABLEKS R19 R18 K48 ["Font"]
  GETUPVAL R20 7
  GETTABLEKS R19 R20 K56 ["FONT_SIZE_TITLE"]
  SETTABLEKS R19 R18 K49 ["TextSize"]
  GETTABLEKS R21 R3 K57 ["assetConfig"]
  GETTABLEKS R20 R21 K58 ["packagePermissions"]
  GETTABLEKS R19 R20 K59 ["subTextColor"]
  SETTABLEKS R19 R18 K50 ["TextColor3"]
  CALL R16 2 1
  SETTABLEKS R16 R15 K35 ["Title"]
  GETUPVAL R17 6
  GETTABLEKS R16 R17 K25 ["createElement"]
  GETUPVAL R17 8
  DUPTABLE R18 K70 [{"LayoutOrder", "Enabled", "HeaderHeight", "ErrorText", "DefaultText", "NoResultsText", "LoadingMore", "onSearchRequested", "onTextChanged", "OnItemClicked", "Results"}]
  LOADN R19 1
  SETTABLEKS R19 R18 K29 ["LayoutOrder"]
  GETTABLEKS R19 R1 K60 ["Enabled"]
  JUMPIFNOT R19 [+1]
  NOT R19 R8
  SETTABLEKS R19 R18 K60 ["Enabled"]
  LOADN R19 25
  SETTABLEKS R19 R18 K61 ["HeaderHeight"]
  JUMPIFNOT R8 [+2]
  MOVE R19 R11
  JUMPIF R19 [+1]
  LOADNIL R19
  SETTABLEKS R19 R18 K62 ["ErrorText"]
  GETTABLEKS R19 R1 K19 ["Localization"]
  LOADK R21 K20 ["AssetConfigPackagePermissions"]
  LOADK R22 K71 ["SearchbarDefault"]
  NAMECALL R19 R19 K24 ["getText"]
  CALL R19 3 1
  SETTABLEKS R19 R18 K63 ["DefaultText"]
  GETTABLEKS R19 R1 K19 ["Localization"]
  LOADK R21 K20 ["AssetConfigPackagePermissions"]
  LOADK R22 K72 ["NoResults"]
  NAMECALL R19 R19 K24 ["getText"]
  CALL R19 3 1
  SETTABLEKS R19 R18 K64 ["NoResultsText"]
  SETTABLEKS R6 R18 K65 ["LoadingMore"]
  NEWCLOSURE R19 P1
  CAPTURE VAL R1
  CAPTURE VAL R0
  SETTABLEKS R19 R18 K66 ["onSearchRequested"]
  NEWCLOSURE R19 P2
  CAPTURE VAL R1
  CAPTURE VAL R0
  SETTABLEKS R19 R18 K67 ["onTextChanged"]
  NEWCLOSURE R19 P3
  CAPTURE VAL R9
  CAPTURE UPVAL U9
  SETTABLEKS R19 R18 K68 ["OnItemClicked"]
  SETTABLEKS R10 R18 K69 ["Results"]
  CALL R16 2 1
  SETTABLEKS R16 R15 K36 ["CollaboratorSearchBar"]
  CALL R12 3 -1
  RETURN R12 -1

PROTO_10:
  DUPTABLE R2 K1 [{"SearchData"}]
  DUPTABLE R3 K5 [{"searchText", "cachedSearchResults", "localUserFriends"}]
  GETTABLEKS R4 R0 K2 ["searchText"]
  SETTABLEKS R4 R3 K2 ["searchText"]
  GETTABLEKS R4 R0 K3 ["cachedSearchResults"]
  SETTABLEKS R4 R3 K3 ["cachedSearchResults"]
  GETTABLEKS R4 R0 K4 ["localUserFriends"]
  SETTABLEKS R4 R3 K4 ["localUserFriends"]
  SETTABLEKS R3 R2 K0 ["SearchData"]
  RETURN R2 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R5 K1 [script]
  GETTABLEKS R4 R5 K2 ["Parent"]
  GETTABLEKS R3 R4 K2 ["Parent"]
  GETTABLEKS R2 R3 K2 ["Parent"]
  GETTABLEKS R1 R2 K2 ["Parent"]
  GETTABLEKS R0 R1 K2 ["Parent"]
  GETTABLEKS R1 R0 K3 ["Packages"]
  GETIMPORT R2 K5 [require]
  GETTABLEKS R3 R1 K6 ["Cryo"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R4 R1 K7 ["Roact"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R5 R1 K8 ["RoactRodux"]
  CALL R4 1 1
  GETTABLEKS R6 R0 K9 ["Src"]
  GETTABLEKS R5 R6 K10 ["Util"]
  GETIMPORT R7 K5 [require]
  GETTABLEKS R8 R5 K11 ["Keys"]
  CALL R7 1 1
  GETTABLEKS R6 R7 K12 ["LoadingInProgress"]
  GETIMPORT R7 K5 [require]
  GETTABLEKS R8 R5 K13 ["Urls"]
  CALL R7 1 1
  GETIMPORT R8 K5 [require]
  GETTABLEKS R9 R5 K14 ["Constants"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETTABLEKS R10 R5 K15 ["AssetConfigConstants"]
  CALL R9 1 1
  GETTABLEKS R13 R0 K9 ["Src"]
  GETTABLEKS R12 R13 K16 ["Components"]
  GETTABLEKS R11 R12 K17 ["AssetConfiguration"]
  GETTABLEKS R10 R11 K18 ["Permissions"]
  GETIMPORT R11 K5 [require]
  GETTABLEKS R12 R10 K19 ["CollaboratorSearchBar"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETTABLEKS R13 R10 K20 ["PermissionsConstants"]
  CALL R12 1 1
  GETIMPORT R13 K5 [require]
  GETTABLEKS R14 R10 K21 ["CollaboratorThumbnail"]
  CALL R13 1 1
  GETIMPORT R14 K5 [require]
  GETTABLEKS R15 R1 K22 ["Framework"]
  CALL R14 1 1
  GETTABLEKS R15 R14 K23 ["ContextServices"]
  GETTABLEKS R16 R15 K24 ["withContext"]
  GETIMPORT R17 K5 [require]
  GETTABLEKS R20 R0 K9 ["Src"]
  GETTABLEKS R19 R20 K23 ["ContextServices"]
  GETTABLEKS R18 R19 K25 ["NetworkContext"]
  CALL R17 1 1
  GETTABLEKS R18 R12 K26 ["UseViewKey"]
  DUPCLOSURE R19 K27 [PROTO_0]
  CAPTURE VAL R12
  DUPCLOSURE R20 K28 [PROTO_1]
  CAPTURE VAL R6
  DUPCLOSURE R21 K29 [PROTO_3]
  CAPTURE VAL R12
  CAPTURE VAL R19
  DUPCLOSURE R22 K30 [PROTO_4]
  CAPTURE VAL R12
  CAPTURE VAL R3
  CAPTURE VAL R13
  CAPTURE VAL R7
  CAPTURE VAL R9
  GETTABLEKS R23 R3 K31 ["PureComponent"]
  LOADK R25 K32 ["CollaboratorSearchWidget"]
  NAMECALL R23 R23 K33 ["extend"]
  CALL R23 2 1
  DUPCLOSURE R24 K34 [PROTO_9]
  CAPTURE VAL R0
  CAPTURE VAL R21
  CAPTURE VAL R6
  CAPTURE VAL R12
  CAPTURE VAL R2
  CAPTURE VAL R22
  CAPTURE VAL R3
  CAPTURE VAL R8
  CAPTURE VAL R11
  CAPTURE VAL R18
  SETTABLEKS R24 R23 K35 ["render"]
  DUPCLOSURE R24 K36 [PROTO_10]
  MOVE R25 R16
  DUPTABLE R26 K40 [{"Stylizer", "Localization", "Network"}]
  GETTABLEKS R27 R15 K37 ["Stylizer"]
  SETTABLEKS R27 R26 K37 ["Stylizer"]
  GETTABLEKS R27 R15 K38 ["Localization"]
  SETTABLEKS R27 R26 K38 ["Localization"]
  SETTABLEKS R17 R26 K39 ["Network"]
  CALL R25 1 1
  MOVE R26 R23
  CALL R25 1 1
  MOVE R23 R25
  GETTABLEKS R25 R4 K41 ["connect"]
  MOVE R26 R24
  CALL R25 1 1
  MOVE R26 R23
  CALL R25 1 -1
  RETURN R25 -1

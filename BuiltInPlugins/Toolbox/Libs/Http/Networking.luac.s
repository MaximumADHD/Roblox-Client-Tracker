PROTO_0:
        0 GETIMPORT                        R1 K1 [pairs]
        2 GETUPVAL                         R2 0
        3 CALL                             R1 1 3
        4 FORGPREP_NEXT                    R1
        5 LOADN                            R6 100
        6 JUMPIFNOTLE                      R6 R5 ; [+11]
        8 FASTCALL1                        TOSTRING R5 ; [+3]
        9 MOVE                             R9 R5
       10 GETIMPORT                        R8 K3 [tostring]
       12 CALL                             R8 1 1
       13 NAMECALL                         R6 R0 K4 ["find"]
       15 CALL                             R6 2 1
       16 JUMPIFNOT                        R6 ; [+1]
       17 RETURN                           R5 1
       18 FORGLOOP                         R1 2 ; [-14]
       20 LOADK                            R3 K5 ["2%d%d"]
       21 NAMECALL                         R1 R0 K4 ["find"]
       23 CALL                             R1 2 1
       24 JUMPIFNOT                        R1 ; [+4]
       25 GETUPVAL                         R1 0
       26 GETTABLEKS                       R1 R1 K6 ["OK"]
       28 RETURN                           R1 1
       29 LOADK                            R3 K7 ["curl_easy_perform"]
       30 NAMECALL                         R1 R0 K4 ["find"]
       32 CALL                             R1 2 1
       33 JUMPIFNOT                        R1 ; [+9]
       34 LOADK                            R3 K8 ["SSL"]
       35 NAMECALL                         R1 R0 K4 ["find"]
       37 CALL                             R1 2 1
       38 JUMPIFNOT                        R1 ; [+4]
       39 GETUPVAL                         R1 0
       40 GETTABLEKS                       R1 R1 K9 ["BAD_TLS"]
       42 RETURN                           R1 1
       43 GETUPVAL                         R1 0
       44 GETTABLEKS                       R1 R1 K10 ["UNKNOWN_ERROR"]
       46 RETURN                           R1 1

PROTO_1:
        0 GETIMPORT                        R3 K1 [tick]
        2 CALL                             R3 0 1
        3 GETIMPORT                        R4 K3 [pcall]
        5 MOVE                             R5 R2
        6 CALL                             R4 1 2
        7 GETIMPORT                        R6 K1 [tick]
        9 CALL                             R6 0 1
       10 SUB                              R8 R6 R3
       11 MULK                             R7 R8 K4 [1000]
       12 GETUPVAL                         R8 0
       13 GETTABLEKS                       R8 R8 K5 ["OK"]
       15 JUMPIF                           R4 ; [+4]
       16 GETUPVAL                         R9 1
       17 MOVE                             R10 R5
       18 CALL                             R9 1 1
       19 MOVE                             R8 R9
       20 GETUPVAL                         R9 2
       21 GETTABLEKS                       R9 R9 K6 ["new"]
       23 MOVE                             R10 R5
       24 MOVE                             R11 R7
       25 MOVE                             R12 R8
       26 MOVE                             R13 R0
       27 MOVE                             R14 R1
       28 CALL                             R9 5 -1
       29 RETURN                           R9 -1

PROTO_2:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["GetAsyncFullUrl"]
        4 CALL                             R0 2 -1
        5 RETURN                           R0 -1

PROTO_3:
        0 GETUPVAL                         R2 0
        1 LOADK                            R3 K0 ["GET"]
        2 MOVE                             R4 R1
        3 NEWCLOSURE                       R5 P0
        4 CAPTURE                          VAL R0
        5 CAPTURE                          VAL R1
        6 CALL                             R2 3 -1
        7 RETURN                           R2 -1

PROTO_4:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 GETUPVAL                         R3 2
        3 GETIMPORT                        R4 K3 [Enum.ThrottlingPriority.Default]
        5 GETUPVAL                         R5 3
        6 NAMECALL                         R0 R0 K4 ["PostAsyncFullUrl"]
        8 CALL                             R0 5 -1
        9 RETURN                           R0 -1

PROTO_5:
        0 JUMPIF                           R3 ; [+2]
        1 GETIMPORT                        R3 K3 [Enum.HttpContentType.ApplicationJson]
        3 GETUPVAL                         R4 0
        4 LOADK                            R5 K4 ["POST"]
        5 MOVE                             R6 R1
        6 NEWCLOSURE                       R7 P0
        7 CAPTURE                          VAL R0
        8 CAPTURE                          VAL R1
        9 CAPTURE                          VAL R2
       10 CAPTURE                          REF R3
       11 CALL                             R4 3 -1
       12 CLOSEUPVALS                      R3
       13 RETURN                           R4 -1

PROTO_6:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["RequestAsync"]
        4 CALL                             R0 2 -1
        5 RETURN                           R0 -1

PROTO_7:
        0 GETUPVAL                         R2 0
        1 LOADK                            R3 K0 ["DELETE"]
        2 MOVE                             R4 R1
        3 NEWCLOSURE                       R5 P0
        4 CAPTURE                          VAL R0
        5 CAPTURE                          VAL R1
        6 CALL                             R2 3 -1
        7 RETURN                           R2 -1

PROTO_8:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["RequestAsync"]
        4 CALL                             R0 2 -1
        5 RETURN                           R0 -1

PROTO_9:
        0 GETUPVAL                         R2 0
        1 LOADK                            R3 K0 ["PATCH"]
        2 MOVE                             R4 R1
        3 NEWCLOSURE                       R5 P0
        4 CAPTURE                          VAL R0
        5 CAPTURE                          VAL R1
        6 CALL                             R2 3 -1
        7 RETURN                           R2 -1

PROTO_10:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["RequestAsync"]
        4 CALL                             R0 2 -1
        5 RETURN                           R0 -1

PROTO_11:
        0 GETUPVAL                         R2 0
        1 LOADK                            R3 K0 ["PUT"]
        2 MOVE                             R4 R1
        3 NEWCLOSURE                       R5 P0
        4 CAPTURE                          VAL R0
        5 CAPTURE                          VAL R1
        6 CALL                             R2 3 -1
        7 RETURN                           R2 -1

PROTO_12:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 FASTCALL1                        TABLE_UNPACK R2 ; [+2]
        3 GETIMPORT                        R1 K1 [unpack]
        5 CALL                             R1 1 -1
        6 CALL                             R0 -1 1
        7 GETTABLEKS                       R1 R0 K2 ["responseCode"]
        9 GETUPVAL                         R2 2
       10 GETTABLEKS                       R2 R2 K3 ["OK"]
       12 JUMPIFNOTEQ                      R1 R2 ; [+5]
       14 GETUPVAL                         R1 3
       15 MOVE                             R2 R0
       16 CALL                             R1 1 0
       17 RETURN                           R0 0
       18 GETUPVAL                         R1 4
       19 MOVE                             R2 R0
       20 CALL                             R1 1 0
       21 RETURN                           R0 0

PROTO_13:
        0 GETIMPORT                        R2 K1 [spawn]
        2 NEWCLOSURE                       R3 P0
        3 CAPTURE                          UPVAL U0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          UPVAL U2
        6 CAPTURE                          VAL R0
        7 CAPTURE                          VAL R1
        8 CALL                             R2 1 0
        9 RETURN                           R0 0

PROTO_14:
        0 PREPVARARGS                      1
        1 NEWTABLE                         R1 0 0
        3 GETVARARGS                       R2 -1
        4 SETLIST                          R1 R2 -1 [1]
        6 GETUPVAL                         R2 0
        7 GETTABLEKS                       R2 R2 K0 ["new"]
        9 NEWCLOSURE                       R3 P0
       10 CAPTURE                          VAL R0
       11 CAPTURE                          VAL R1
       12 CAPTURE                          UPVAL U1
       13 CALL                             R2 1 1
       14 RETURN                           R2 1

PROTO_15:
        0 DUPTABLE                         R0 K1 [{"_httpImpl"}]
        1 GETUPVAL                         R1 0
        2 SETTABLEKS                       R1 R0 K0 ["_httpImpl"]
        4 GETUPVAL                         R3 1
        5 FASTCALL2                        SETMETATABLE R0 R3 ; [+4]
        7 MOVE                             R2 R0
        8 GETIMPORT                        R1 K3 [setmetatable]
       10 CALL                             R1 2 0
       11 RETURN                           R0 1

PROTO_16:
        0 GETUPVAL                         R2 0
        1 MOVE                             R4 R1
        2 NAMECALL                         R2 R2 K0 ["JSONEncode"]
        4 CALL                             R2 2 -1
        5 RETURN                           R2 -1

PROTO_17:
        0 GETUPVAL                         R2 0
        1 MOVE                             R4 R1
        2 NAMECALL                         R2 R2 K0 ["JSONDecode"]
        4 CALL                             R2 2 -1
        5 RETURN                           R2 -1

PROTO_18:
        0 GETUPVAL                         R1 0
        1 GETTABLEKS                       R3 R0 K0 ["responseBody"]
        3 NAMECALL                         R1 R1 K1 ["jsonDecode"]
        5 CALL                             R1 2 1
        6 SETTABLEKS                       R1 R0 K0 ["responseBody"]
        8 RETURN                           R0 1

PROTO_19:
        0 GETUPVAL                         R2 0
        1 GETUPVAL                         R3 1
        2 GETTABLEKS                       R4 R0 K0 ["_httpImpl"]
        4 MOVE                             R5 R1
        5 CALL                             R2 3 1
        6 NEWCLOSURE                       R4 P0
        7 CAPTURE                          VAL R0
        8 NAMECALL                         R2 R2 K1 ["andThen"]
       10 CALL                             R2 2 -1
       11 RETURN                           R2 -1

PROTO_20:
        0 GETUPVAL                         R1 0
        1 GETTABLEKS                       R3 R0 K0 ["responseBody"]
        3 NAMECALL                         R1 R1 K1 ["jsonDecode"]
        5 CALL                             R1 2 1
        6 SETTABLEKS                       R1 R0 K0 ["responseBody"]
        8 RETURN                           R0 1

PROTO_21:
        0 GETUPVAL                         R3 0
        1 GETUPVAL                         R4 1
        2 GETTABLEKS                       R5 R0 K0 ["_httpImpl"]
        4 MOVE                             R6 R1
        5 MOVE                             R7 R2
        6 CALL                             R3 4 1
        7 NEWCLOSURE                       R5 P0
        8 CAPTURE                          VAL R0
        9 NAMECALL                         R3 R3 K1 ["andThen"]
       11 CALL                             R3 2 -1
       12 RETURN                           R3 -1

PROTO_22:
        0 GETUPVAL                         R3 0
        1 GETUPVAL                         R4 1
        2 GETTABLEKS                       R5 R0 K0 ["_httpImpl"]
        4 MOVE                             R6 R1
        5 MOVE                             R7 R2
        6 CALL                             R3 4 -1
        7 RETURN                           R3 -1

PROTO_23:
        0 GETUPVAL                         R2 0
        1 GETUPVAL                         R3 1
        2 GETTABLEKS                       R4 R0 K0 ["_httpImpl"]
        4 MOVE                             R5 R1
        5 CALL                             R2 3 -1
        6 RETURN                           R2 -1

PROTO_24:
        0 DUPTABLE                         R2 K3 [{[1], ["Method"] = "DELETE"}]
        1 SETTABLEKS                       R1 R2 K0 ["Url"]
        3 GETUPVAL                         R3 0
        4 GETUPVAL                         R4 1
        5 GETTABLEKS                       R5 R0 K4 ["_httpImpl"]
        7 MOVE                             R6 R2
        8 CALL                             R3 3 -1
        9 RETURN                           R3 -1

PROTO_25:
        0 DUPTABLE                         R3 K5 [{[1], ["Method"] = "DELETE", ["Body"], ["Headers"]}]
        1 SETTABLEKS                       R1 R3 K0 ["Url"]
        3 SETTABLEKS                       R2 R3 K3 ["Body"]
        5 NEWTABLE                         R4 1 0
        7 LOADK                            R5 K6 ["application/json"]
        8 SETTABLEKS                       R5 R4 K7 ["Content-Type"]
       10 SETTABLEKS                       R4 R3 K4 ["Headers"]
       12 GETUPVAL                         R4 0
       13 GETUPVAL                         R5 1
       14 GETTABLEKS                       R6 R0 K8 ["_httpImpl"]
       16 MOVE                             R7 R3
       17 CALL                             R4 3 -1
       18 RETURN                           R4 -1

PROTO_26:
        0 DUPTABLE                         R3 K5 [{[1], ["Method"] = "PATCH", ["Body"], ["Headers"]}]
        1 SETTABLEKS                       R1 R3 K0 ["Url"]
        3 SETTABLEKS                       R2 R3 K3 ["Body"]
        5 NEWTABLE                         R4 1 0
        7 LOADK                            R5 K6 ["application/json"]
        8 SETTABLEKS                       R5 R4 K7 ["Content-Type"]
       10 SETTABLEKS                       R4 R3 K4 ["Headers"]
       12 GETUPVAL                         R4 0
       13 GETUPVAL                         R5 1
       14 GETTABLEKS                       R6 R0 K8 ["_httpImpl"]
       16 MOVE                             R7 R3
       17 CALL                             R4 3 -1
       18 RETURN                           R4 -1

PROTO_27:
        0 DUPTABLE                         R3 K5 [{[1], ["Method"] = "PUT", ["Body"], ["Headers"]}]
        1 SETTABLEKS                       R1 R3 K0 ["Url"]
        3 SETTABLEKS                       R2 R3 K3 ["Body"]
        5 NEWTABLE                         R4 1 0
        7 LOADK                            R5 K6 ["application/json"]
        8 SETTABLEKS                       R5 R4 K7 ["Content-Type"]
       10 SETTABLEKS                       R4 R3 K4 ["Headers"]
       12 GETUPVAL                         R4 0
       13 GETUPVAL                         R5 1
       14 GETTABLEKS                       R6 R0 K8 ["_httpImpl"]
       16 MOVE                             R7 R3
       17 CALL                             R4 3 -1
       18 RETURN                           R4 -1

PROTO_28:
        0 JUMPIFNOT                        R0 ; [+23]
        1 GETTABLEKS                       R2 R1 K0 ["StatusCode"]
        3 GETUPVAL                         R3 0
        4 GETTABLEKS                       R3 R3 K1 ["BAD_REQUEST"]
        6 JUMPIFNOTLE                      R3 R2 ; [+12]
        8 GETUPVAL                         R2 1
        9 LOADK                            R4 K2 ["HTTP error: "]
       10 GETTABLEKS                       R6 R1 K0 ["StatusCode"]
       12 FASTCALL1                        TOSTRING R6 ; [+2]
       13 GETIMPORT                        R5 K4 [tostring]
       15 CALL                             R5 1 1
       16 CONCAT                           R3 R4 R5
       17 CALL                             R2 1 0
       18 RETURN                           R0 0
       19 GETUPVAL                         R2 2
       20 GETTABLEKS                       R3 R1 K5 ["Body"]
       22 CALL                             R2 1 0
       23 RETURN                           R0 0
       24 GETUPVAL                         R2 1
       25 LOADK                            R4 K2 ["HTTP error: "]
       26 GETTABLEKS                       R6 R1 K6 ["HttpError"]
       28 FASTCALL1                        TOSTRING R6 ; [+2]
       29 GETIMPORT                        R5 K4 [tostring]
       31 CALL                             R5 1 1
       32 CONCAT                           R3 R4 R5
       33 CALL                             R2 1 0
       34 RETURN                           R0 0

PROTO_29:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["RequestInternal"]
        4 CALL                             R0 2 1
        5 NEWCLOSURE                       R2 P0
        6 CAPTURE                          UPVAL U2
        7 CAPTURE                          UPVAL U3
        8 CAPTURE                          UPVAL U4
        9 NAMECALL                         R0 R0 K1 ["Start"]
       11 CALL                             R0 2 0
       12 RETURN                           R0 0

PROTO_30:
        0 GETIMPORT                        R2 K1 [spawn]
        2 NEWCLOSURE                       R3 P0
        3 CAPTURE                          UPVAL U0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          UPVAL U2
        6 CAPTURE                          VAL R1
        7 CAPTURE                          VAL R0
        8 CALL                             R2 1 0
        9 RETURN                           R0 0

PROTO_31:
        0 GETUPVAL                         R2 0
        1 GETTABLEKS                       R2 R2 K0 ["new"]
        3 NEWCLOSURE                       R3 P0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          VAL R1
        6 CAPTURE                          UPVAL U2
        7 CALL                             R2 1 -1
        8 RETURN                           R2 -1

PROTO_32:
        0 JUMPIFNOT                        R0 ; [+11]
        1 GETTABLEKS                       R2 R1 K0 ["StatusCode"]
        3 GETUPVAL                         R3 0
        4 GETTABLEKS                       R3 R3 K1 ["BAD_REQUEST"]
        6 JUMPIFNOTLT                      R2 R3 ; [+5]
        8 GETUPVAL                         R2 1
        9 MOVE                             R3 R1
       10 CALL                             R2 1 0
       11 RETURN                           R0 0
       12 GETUPVAL                         R2 2
       13 MOVE                             R3 R1
       14 CALL                             R2 1 0
       15 RETURN                           R0 0

PROTO_33:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R2 1
        2 NAMECALL                         R0 R0 K0 ["RequestInternal"]
        4 CALL                             R0 2 1
        5 NEWCLOSURE                       R2 P0
        6 CAPTURE                          UPVAL U2
        7 CAPTURE                          UPVAL U3
        8 CAPTURE                          UPVAL U4
        9 NAMECALL                         R0 R0 K1 ["Start"]
       11 CALL                             R0 2 0
       12 RETURN                           R0 0

PROTO_34:
        0 GETIMPORT                        R2 K1 [spawn]
        2 NEWCLOSURE                       R3 P0
        3 CAPTURE                          UPVAL U0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          UPVAL U2
        6 CAPTURE                          VAL R0
        7 CAPTURE                          VAL R1
        8 CALL                             R2 1 0
        9 RETURN                           R0 0

PROTO_35:
        0 GETUPVAL                         R2 0
        1 GETTABLEKS                       R2 R2 K0 ["new"]
        3 NEWCLOSURE                       R3 P0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          VAL R1
        6 CAPTURE                          UPVAL U2
        7 CALL                             R2 1 -1
        8 RETURN                           R2 -1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [script]
        3 GETTABLEKS                       R0 R0 K2 ["Parent"]
        5 GETTABLEKS                       R0 R0 K2 ["Parent"]
        7 GETTABLEKS                       R0 R0 K2 ["Parent"]
        9 GETTABLEKS                       R1 R0 K3 ["Packages"]
       11 GETIMPORT                        R2 K5 [require]
       13 GETTABLEKS                       R3 R1 K6 ["Framework"]
       15 CALL                             R2 1 1
       16 GETTABLEKS                       R2 R2 K7 ["Util"]
       18 GETTABLEKS                       R2 R2 K8 ["Promise"]
       20 GETIMPORT                        R3 K5 [require]
       22 GETIMPORT                        R4 K1 [script]
       24 GETTABLEKS                       R4 R4 K2 ["Parent"]
       26 GETTABLEKS                       R4 R4 K9 ["HttpResponse"]
       28 CALL                             R3 1 1
       29 GETIMPORT                        R4 K5 [require]
       31 GETIMPORT                        R5 K1 [script]
       33 GETTABLEKS                       R5 R5 K2 ["Parent"]
       35 GETTABLEKS                       R5 R5 K10 ["StatusCodes"]
       37 CALL                             R4 1 1
       38 GETIMPORT                        R5 K12 [game]
       40 LOADK                            R7 K13 ["HttpService"]
       41 NAMECALL                         R5 R5 K14 ["GetService"]
       43 CALL                             R5 2 1
       44 GETIMPORT                        R6 K12 [game]
       46 LOADK                            R8 K15 ["HttpRbxApiService"]
       47 NAMECALL                         R6 R6 K14 ["GetService"]
       49 CALL                             R6 2 1
       50 DUPCLOSURE                       R7 K16 [PROTO_0]
       51 CAPTURE                          VAL R4
       52 DUPCLOSURE                       R8 K17 [PROTO_1]
       53 CAPTURE                          VAL R4
       54 CAPTURE                          VAL R7
       55 CAPTURE                          VAL R3
       56 DUPCLOSURE                       R9 K18 [PROTO_3]
       57 CAPTURE                          VAL R8
       58 DUPCLOSURE                       R10 K19 [PROTO_5]
       59 CAPTURE                          VAL R8
       60 DUPCLOSURE                       R11 K20 [PROTO_7]
       61 CAPTURE                          VAL R8
       62 DUPCLOSURE                       R12 K21 [PROTO_9]
       63 CAPTURE                          VAL R8
       64 DUPCLOSURE                       R13 K22 [PROTO_11]
       65 CAPTURE                          VAL R8
       66 DUPCLOSURE                       R14 K23 [PROTO_14]
       67 CAPTURE                          VAL R2
       68 CAPTURE                          VAL R4
       69 NEWTABLE                         R15 16 0
       71 SETTABLEKS                       R15 R15 K24 ["__index"]
       73 DUPCLOSURE                       R16 K25 [PROTO_15]
       74 CAPTURE                          VAL R6
       75 CAPTURE                          VAL R15
       76 SETTABLEKS                       R16 R15 K26 ["new"]
       78 DUPCLOSURE                       R16 K27 [PROTO_16]
       79 CAPTURE                          VAL R5
       80 SETTABLEKS                       R16 R15 K28 ["jsonEncode"]
       82 DUPCLOSURE                       R16 K29 [PROTO_17]
       83 CAPTURE                          VAL R5
       84 SETTABLEKS                       R16 R15 K30 ["jsonDecode"]
       86 DUPCLOSURE                       R16 K31 [PROTO_19]
       87 CAPTURE                          VAL R14
       88 CAPTURE                          VAL R9
       89 SETTABLEKS                       R16 R15 K32 ["httpGetJson"]
       91 DUPCLOSURE                       R16 K33 [PROTO_21]
       92 CAPTURE                          VAL R14
       93 CAPTURE                          VAL R10
       94 SETTABLEKS                       R16 R15 K34 ["httpPostJson"]
       96 DUPCLOSURE                       R16 K35 [PROTO_22]
       97 CAPTURE                          VAL R14
       98 CAPTURE                          VAL R10
       99 SETTABLEKS                       R16 R15 K36 ["httpPost"]
      101 DUPCLOSURE                       R16 K37 [PROTO_23]
      102 CAPTURE                          VAL R14
      103 CAPTURE                          VAL R9
      104 SETTABLEKS                       R16 R15 K38 ["httpGet"]
      106 DUPCLOSURE                       R16 K39 [PROTO_24]
      107 CAPTURE                          VAL R14
      108 CAPTURE                          VAL R11
      109 SETTABLEKS                       R16 R15 K40 ["httpDelete"]
      111 DUPCLOSURE                       R16 K41 [PROTO_25]
      112 CAPTURE                          VAL R14
      113 CAPTURE                          VAL R11
      114 SETTABLEKS                       R16 R15 K42 ["httpDeleteWithPayload"]
      116 DUPCLOSURE                       R16 K43 [PROTO_26]
      117 CAPTURE                          VAL R14
      118 CAPTURE                          VAL R12
      119 SETTABLEKS                       R16 R15 K44 ["httpPatch"]
      121 DUPCLOSURE                       R16 K45 [PROTO_27]
      122 CAPTURE                          VAL R14
      123 CAPTURE                          VAL R13
      124 SETTABLEKS                       R16 R15 K46 ["httpPut"]
      126 DUPCLOSURE                       R16 K47 [PROTO_31]
      127 CAPTURE                          VAL R2
      128 CAPTURE                          VAL R5
      129 CAPTURE                          VAL R4
      130 SETTABLEKS                       R16 R15 K48 ["requestInternal"]
      132 DUPCLOSURE                       R16 K49 [PROTO_35]
      133 CAPTURE                          VAL R2
      134 CAPTURE                          VAL R5
      135 CAPTURE                          VAL R4
      136 SETTABLEKS                       R16 R15 K50 ["requestInternalRaw"]
      138 RETURN                           R15 1

return function()
	...
	describe("getCreatorTypeValueFromName", function()
		it("returns the type", function()
			expect(CreatorInfoHelper.getCreatorTypeValueFromName(Enum.CreatorType.Group.Name)).to.equal(
				Enum.CreatorType.Group.Value
			)
			expect(CreatorInfoHelper.getCreatorTypeValueFromName(Enum.CreatorType.User.Name)).to.equal(
				Enum.CreatorType.User.Value
			)
		end)
	...
	describe("isCached", function()
		...
		describe("returns false if", function()
			...
			it("the cachedCreatorInfo Id does not match", function()
				expect(CreatorInfoHelper.isCached(
					getStoreStub({
						Id = 2,

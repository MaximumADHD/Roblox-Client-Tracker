PROTO_0:
  GETUPVAL R1 0
  CALL R1 0 1
  JUMPIFNOT R1 [+14]
  GETTABLEKS R1 R0 K0 ["responseBody"]
  JUMPIFNOT R1 [+9]
  GETTABLEKS R2 R0 K0 ["responseBody"]
  GETTABLEKS R1 R2 K1 ["totalCount"]
  LOADN R2 0
  JUMPIFNOTLT R2 R1 [+3]
  LOADB R1 1
  RETURN R1 1
  LOADB R1 0
  RETURN R1 1
  GETTABLEKS R1 R0 K0 ["responseBody"]
  JUMPIFNOTEQKS R1 K2 ["null"] [+3]
  LOADB R1 0
  RETURN R1 1
  LOADB R1 1
  RETURN R1 1

PROTO_1:
  GETUPVAL R1 0
  GETUPVAL R3 1
  GETUPVAL R4 2
  GETUPVAL R6 3
  CALL R6 0 1
  JUMPIFNOT R6 [+14]
  GETTABLEKS R6 R0 K0 ["responseBody"]
  JUMPIFNOT R6 [+9]
  GETTABLEKS R7 R0 K0 ["responseBody"]
  GETTABLEKS R6 R7 K1 ["totalCount"]
  LOADN R7 0
  JUMPIFNOTLT R7 R6 [+3]
  LOADB R5 1
  JUMP [+10]
  LOADB R5 0
  JUMP [+8]
  GETTABLEKS R6 R0 K0 ["responseBody"]
  JUMPIFNOTEQKS R6 K2 ["null"] [+3]
  LOADB R5 0
  JUMP [+2]
  LOADB R5 1
  JUMP [0]
  CALL R3 2 -1
  NAMECALL R1 R1 K3 ["dispatch"]
  CALL R1 -1 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R1 0
  GETUPVAL R3 1
  MOVE R4 R0
  CALL R3 1 -1
  NAMECALL R1 R1 K0 ["dispatch"]
  CALL R1 -1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R1 0
  GETUPVAL R3 1
  GETUPVAL R4 2
  GETUPVAL R6 3
  CALL R6 0 1
  JUMPIFNOT R6 [+14]
  GETTABLEKS R6 R0 K0 ["responseBody"]
  JUMPIFNOT R6 [+9]
  GETTABLEKS R7 R0 K0 ["responseBody"]
  GETTABLEKS R6 R7 K1 ["totalCount"]
  LOADN R7 0
  JUMPIFNOTLT R7 R6 [+3]
  LOADB R5 1
  JUMP [+10]
  LOADB R5 0
  JUMP [+8]
  GETTABLEKS R6 R0 K0 ["responseBody"]
  JUMPIFNOTEQKS R6 K2 ["null"] [+3]
  LOADB R5 0
  JUMP [+2]
  LOADB R5 1
  JUMP [0]
  CALL R3 2 -1
  NAMECALL R1 R1 K3 ["dispatch"]
  CALL R1 -1 0
  RETURN R0 0

PROTO_4:
  GETUPVAL R1 0
  GETUPVAL R3 1
  MOVE R4 R0
  CALL R3 1 -1
  NAMECALL R1 R1 K0 ["dispatch"]
  CALL R1 -1 0
  RETURN R0 0

PROTO_5:
  GETUPVAL R1 0
  CALL R1 0 1
  JUMPIFNOT R1 [+18]
  GETUPVAL R1 1
  GETUPVAL R3 2
  GETUPVAL R4 3
  NAMECALL R1 R1 K0 ["getSave"]
  CALL R1 3 1
  NEWCLOSURE R3 P0
  CAPTURE VAL R0
  CAPTURE UPVAL U4
  CAPTURE UPVAL U2
  CAPTURE UPVAL U0
  NEWCLOSURE R4 P1
  CAPTURE VAL R0
  CAPTURE UPVAL U5
  NAMECALL R1 R1 K1 ["andThen"]
  CALL R1 3 -1
  RETURN R1 -1
  GETUPVAL R1 1
  GETUPVAL R3 6
  GETUPVAL R4 2
  NAMECALL R1 R1 K2 ["getFavorited"]
  CALL R1 3 1
  NEWCLOSURE R3 P2
  CAPTURE VAL R0
  CAPTURE UPVAL U4
  CAPTURE UPVAL U2
  CAPTURE UPVAL U0
  NEWCLOSURE R4 P3
  CAPTURE VAL R0
  CAPTURE UPVAL U5
  NAMECALL R1 R1 K1 ["andThen"]
  CALL R1 3 -1
  RETURN R1 -1

PROTO_6:
  NEWCLOSURE R4 P0
  CAPTURE UPVAL U0
  CAPTURE VAL R0
  CAPTURE VAL R2
  CAPTURE VAL R3
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  CAPTURE VAL R1
  RETURN R4 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R4 K1 [script]
  GETTABLEKS R3 R4 K2 ["Parent"]
  GETTABLEKS R2 R3 K2 ["Parent"]
  GETTABLEKS R1 R2 K2 ["Parent"]
  GETTABLEKS R0 R1 K2 ["Parent"]
  GETIMPORT R1 K4 [require]
  GETTABLEKS R4 R0 K5 ["Core"]
  GETTABLEKS R3 R4 K6 ["Actions"]
  GETTABLEKS R2 R3 K7 ["GetFavorited"]
  CALL R1 1 1
  GETIMPORT R2 K4 [require]
  GETTABLEKS R5 R0 K5 ["Core"]
  GETTABLEKS R4 R5 K6 ["Actions"]
  GETTABLEKS R3 R4 K8 ["NetworkError"]
  CALL R2 1 1
  GETIMPORT R3 K4 [require]
  GETTABLEKS R7 R0 K5 ["Core"]
  GETTABLEKS R6 R7 K9 ["Util"]
  GETTABLEKS R5 R6 K10 ["SharedFlags"]
  GETTABLEKS R4 R5 K11 ["getFFlagToolboxSaves"]
  CALL R3 1 1
  DUPCLOSURE R4 K12 [PROTO_0]
  CAPTURE VAL R3
  DUPCLOSURE R5 K13 [PROTO_6]
  CAPTURE VAL R3
  CAPTURE VAL R1
  CAPTURE VAL R2
  RETURN R5 1

PROTO_0:
  GETTABLEKS R2 R0 K0 ["purchase"]
  JUMPIFNOT R2 [+5]
  GETTABLEKS R2 R0 K0 ["purchase"]
  GETTABLEKS R1 R2 K1 ["isPurchaseDialogVisible"]
  RETURN R1 1
  LOADNIL R1
  RETURN R1 1

PROTO_1:
  GETUPVAL R0 0
  GETUPVAL R1 1
  LOADB R2 0
  CALL R1 1 -1
  CALL R0 -1 0
  GETUPVAL R1 2
  GETTABLEKS R0 R1 K0 ["OnClose"]
  JUMPIFNOT R0 [+4]
  GETUPVAL R1 2
  GETTABLEKS R0 R1 K0 ["OnClose"]
  CALL R0 0 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["TryInsert"]
  MOVE R4 R0
  LOADB R5 0
  LOADK R6 K1 ["PreviewClickInsertButton"]
  MOVE R7 R1
  MOVE R8 R2
  CALL R3 5 -1
  RETURN R3 -1

PROTO_3:
  JUMPIFNOT R0 [+61]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["Asset"]
  GETTABLEKS R1 R2 K1 ["Id"]
  GETUPVAL R2 1
  JUMPIF R2 [+15]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["Product"]
  LOADK R5 K3 ["Attempted to purchase asset without a Product: "]
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R7 R1
  GETIMPORT R6 K5 [tostring]
  CALL R6 1 1
  CONCAT R4 R5 R6
  FASTCALL2 ASSERT R3 R4 [+3]
  GETIMPORT R2 K7 [assert]
  CALL R2 2 0
  LOADNIL R2
  LOADNIL R3
  GETUPVAL R4 2
  CALL R4 0 1
  JUMPIF R4 [+14]
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K2 ["Product"]
  JUMPIFNOT R4 [+3]
  GETTABLEKS R5 R4 K8 ["Price"]
  JUMPIF R5 [+1]
  LOADN R5 0
  MOVE R3 R5
  MOVE R5 R4
  JUMPIFNOT R5 [+2]
  GETTABLEKS R5 R4 K9 ["ProductId"]
  MOVE R2 R5
  GETUPVAL R4 3
  GETUPVAL R5 4
  GETUPVAL R6 5
  MOVE R7 R1
  MOVE R8 R2
  MOVE R9 R3
  LOADNIL R10
  LOADNIL R11
  GETUPVAL R13 6
  CALL R13 0 1
  JUMPIFNOT R13 [+2]
  GETUPVAL R12 1
  JUMP [+1]
  LOADNIL R12
  GETUPVAL R13 7
  CALL R5 8 -1
  CALL R4 -1 0
  GETUPVAL R4 8
  CALL R4 0 0
  RETURN R0 0
  GETUPVAL R1 8
  CALL R1 0 0
  RETURN R0 0

PROTO_4:
  GETUPVAL R1 0
  NAMECALL R1 R1 K0 ["use"]
  CALL R1 1 1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["Localization"]
  NAMECALL R2 R2 K0 ["use"]
  CALL R2 1 1
  GETUPVAL R5 2
  GETTABLEKS R4 R5 K0 ["use"]
  CALL R4 0 1
  GETTABLEKS R3 R4 K2 ["networkInterface"]
  GETUPVAL R4 3
  CALL R4 0 1
  GETTABLEKS R6 R0 K3 ["AssetData"]
  GETUPVAL R8 4
  GETTABLEKS R7 R8 K4 ["None"]
  JUMPIFEQ R6 R7 [+4]
  GETTABLEKS R5 R0 K3 ["AssetData"]
  JUMP [+1]
  LOADNIL R5
  JUMPIFNOT R5 [+3]
  GETTABLEKS R6 R5 K5 ["Asset"]
  JUMP [+1]
  LOADNIL R6
  JUMPIFNOT R5 [+3]
  GETTABLEKS R7 R5 K6 ["FiatProduct"]
  JUMP [+1]
  LOADNIL R7
  JUMPIFNOT R7 [+3]
  GETTABLEKS R8 R7 K7 ["Purchasable"]
  JUMP [+1]
  LOADNIL R8
  GETUPVAL R10 5
  CALL R10 0 1
  JUMPIFNOT R10 [+4]
  JUMPIFNOT R7 [+3]
  GETTABLEKS R9 R7 K8 ["Price"]
  JUMP [+1]
  LOADNIL R9
  GETUPVAL R11 6
  CALL R11 0 1
  JUMPIFNOT R11 [+2]
  LOADNIL R10
  JUMP [+10]
  JUMPIFNOT R5 [+8]
  GETTABLEKS R11 R5 K9 ["Product"]
  JUMPIFNOT R11 [+5]
  GETTABLEKS R11 R5 K9 ["Product"]
  GETTABLEKS R10 R11 K8 ["Price"]
  JUMP [+1]
  LOADN R10 0
  JUMPIFNOT R9 [+6]
  GETUPVAL R12 7
  GETTABLEKS R11 R12 K10 ["isFree"]
  MOVE R12 R9
  CALL R11 1 1
  JUMP [+4]
  JUMPIFEQKN R10 K11 [0] [+2]
  LOADB R11 0 +1
  LOADB R11 1
  GETUPVAL R12 8
  DUPCLOSURE R13 K12 [PROTO_0]
  CALL R12 1 1
  GETUPVAL R14 9
  GETTABLEKS R13 R14 K13 ["useCallback"]
  NEWCLOSURE R14 P1
  CAPTURE VAL R4
  CAPTURE UPVAL U10
  CAPTURE VAL R0
  NEWTABLE R15 0 2
  MOVE R16 R4
  GETTABLEKS R17 R0 K14 ["OnClose"]
  SETLIST R15 R16 2 [1]
  CALL R13 2 1
  GETUPVAL R15 9
  GETTABLEKS R14 R15 K13 ["useCallback"]
  NEWCLOSURE R15 P2
  CAPTURE VAL R0
  NEWTABLE R16 0 2
  MOVE R17 R4
  GETTABLEKS R18 R0 K15 ["TryInsert"]
  SETLIST R16 R17 2 [1]
  CALL R14 2 1
  GETUPVAL R16 9
  GETTABLEKS R15 R16 K13 ["useCallback"]
  NEWCLOSURE R16 P3
  CAPTURE VAL R5
  CAPTURE VAL R9
  CAPTURE UPVAL U6
  CAPTURE VAL R4
  CAPTURE UPVAL U11
  CAPTURE VAL R3
  CAPTURE UPVAL U12
  CAPTURE VAL R14
  CAPTURE VAL R13
  NEWTABLE R17 0 6
  MOVE R18 R5
  MOVE R19 R4
  MOVE R20 R9
  MOVE R21 R3
  MOVE R22 R13
  MOVE R23 R14
  SETLIST R17 R18 6 [1]
  CALL R15 2 1
  JUMPIFNOT R12 [+1]
  JUMPIF R5 [+1]
  RETURN R0 0
  JUMPIFNOT R11 [+4]
  MOVE R16 R15
  LOADB R17 1
  CALL R16 1 0
  RETURN R0 0
  JUMPIFNOT R9 [+48]
  GETUPVAL R16 5
  CALL R16 0 1
  JUMPIFNOT R16 [+58]
  JUMPIFNOT R9 [+57]
  JUMPIFNOT R8 [+56]
  GETUPVAL R17 13
  GETTABLEKS R16 R17 K16 ["constructAssetThumbnailUrl"]
  GETTABLEKS R17 R6 K17 ["Id"]
  GETUPVAL R19 14
  GETTABLEKS R18 R19 K18 ["X"]
  GETUPVAL R20 14
  GETTABLEKS R19 R20 K19 ["Y"]
  CALL R16 3 1
  GETUPVAL R18 9
  GETTABLEKS R17 R18 K20 ["createElement"]
  GETUPVAL R18 15
  DUPTABLE R19 K25 [{"Name", "CreatorName", "OnClose", "OnButtonClicked", "Price", "Thumbnail"}]
  GETTABLEKS R20 R6 K21 ["Name"]
  SETTABLEKS R20 R19 K21 ["Name"]
  GETTABLEKS R21 R5 K26 ["Creator"]
  GETTABLEKS R20 R21 K21 ["Name"]
  SETTABLEKS R20 R19 K22 ["CreatorName"]
  SETTABLEKS R13 R19 K14 ["OnClose"]
  SETTABLEKS R15 R19 K23 ["OnButtonClicked"]
  GETUPVAL R21 7
  GETTABLEKS R20 R21 K27 ["displayStringFromMoney"]
  MOVE R21 R9
  CALL R20 1 1
  SETTABLEKS R20 R19 K8 ["Price"]
  SETTABLEKS R16 R19 K24 ["Thumbnail"]
  CALL R17 2 -1
  RETURN R17 -1
  RETURN R0 0
  GETUPVAL R17 9
  GETTABLEKS R16 R17 K20 ["createElement"]
  GETUPVAL R17 16
  DUPTABLE R18 K30 [{"AssetData", "Cancel", "OnPurchaseClicked"}]
  SETTABLEKS R5 R18 K3 ["AssetData"]
  SETTABLEKS R13 R18 K28 ["Cancel"]
  SETTABLEKS R15 R18 K29 ["OnPurchaseClicked"]
  CALL R16 2 -1
  RETURN R16 -1
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["Toolbox"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETTABLEKS R1 R0 K4 ["Packages"]
  GETIMPORT R2 K6 [require]
  GETTABLEKS R6 R0 K7 ["Core"]
  GETTABLEKS R5 R6 K8 ["Util"]
  GETTABLEKS R4 R5 K9 ["SharedFlags"]
  GETTABLEKS R3 R4 K10 ["getFFlagToolboxRemoveRobuxProductEntirely"]
  CALL R2 1 1
  GETIMPORT R3 K6 [require]
  GETTABLEKS R7 R0 K7 ["Core"]
  GETTABLEKS R6 R7 K8 ["Util"]
  GETTABLEKS R5 R6 K9 ["SharedFlags"]
  GETTABLEKS R4 R5 K11 ["getFFlagToolboxAddFiatPricesIntoAssetTile"]
  CALL R3 1 1
  GETIMPORT R4 K6 [require]
  GETTABLEKS R8 R0 K7 ["Core"]
  GETTABLEKS R7 R8 K8 ["Util"]
  GETTABLEKS R6 R7 K9 ["SharedFlags"]
  GETTABLEKS R5 R6 K12 ["getFFlagToolboxEnableFiatPurchasing"]
  CALL R4 1 1
  GETIMPORT R5 K6 [require]
  GETTABLEKS R7 R0 K4 ["Packages"]
  GETTABLEKS R6 R7 K13 ["React"]
  CALL R5 1 1
  GETIMPORT R6 K6 [require]
  GETTABLEKS R8 R0 K4 ["Packages"]
  GETTABLEKS R7 R8 K14 ["Roact"]
  CALL R6 1 1
  GETIMPORT R7 K6 [require]
  GETTABLEKS R10 R0 K7 ["Core"]
  GETTABLEKS R9 R10 K15 ["Hooks"]
  GETTABLEKS R8 R9 K16 ["useDispatch"]
  CALL R7 1 1
  GETIMPORT R8 K6 [require]
  GETTABLEKS R11 R0 K7 ["Core"]
  GETTABLEKS R10 R11 K15 ["Hooks"]
  GETTABLEKS R9 R10 K17 ["useSelector"]
  CALL R8 1 1
  GETIMPORT R9 K6 [require]
  GETTABLEKS R13 R0 K7 ["Core"]
  GETTABLEKS R12 R13 K18 ["Components"]
  GETTABLEKS R11 R12 K19 ["PurchaseFlow"]
  GETTABLEKS R10 R11 K20 ["BuyAssetDialog"]
  CALL R9 1 1
  GETIMPORT R10 K6 [require]
  GETTABLEKS R14 R0 K7 ["Core"]
  GETTABLEKS R13 R14 K18 ["Components"]
  GETTABLEKS R12 R13 K19 ["PurchaseFlow"]
  GETTABLEKS R11 R12 K21 ["PluginPurchaseFlow"]
  CALL R10 1 1
  GETIMPORT R11 K6 [require]
  GETTABLEKS R14 R0 K7 ["Core"]
  GETTABLEKS R13 R14 K8 ["Util"]
  GETTABLEKS R12 R13 K22 ["FiatUtil"]
  CALL R11 1 1
  GETIMPORT R12 K6 [require]
  GETTABLEKS R15 R0 K7 ["Core"]
  GETTABLEKS R14 R15 K8 ["Util"]
  GETTABLEKS R13 R14 K23 ["Urls"]
  CALL R12 1 1
  GETIMPORT R13 K6 [require]
  GETTABLEKS R14 R1 K24 ["Framework"]
  CALL R13 1 1
  GETTABLEKS R14 R13 K25 ["ContextServices"]
  GETTABLEKS R15 R14 K26 ["Stylizer"]
  GETIMPORT R16 K6 [require]
  GETTABLEKS R19 R0 K7 ["Core"]
  GETTABLEKS R18 R19 K25 ["ContextServices"]
  GETTABLEKS R17 R18 K27 ["NetworkContext"]
  CALL R16 1 1
  GETIMPORT R17 K6 [require]
  GETTABLEKS R20 R0 K7 ["Core"]
  GETTABLEKS R19 R20 K28 ["Types"]
  GETTABLEKS R18 R19 K29 ["AssetLogicTypes"]
  CALL R17 1 1
  GETIMPORT R18 K6 [require]
  GETTABLEKS R22 R0 K7 ["Core"]
  GETTABLEKS R21 R22 K30 ["Networking"]
  GETTABLEKS R20 R21 K31 ["Requests"]
  GETTABLEKS R19 R20 K32 ["PurchaseAssetRequest"]
  CALL R18 1 1
  GETIMPORT R19 K6 [require]
  GETTABLEKS R22 R0 K7 ["Core"]
  GETTABLEKS R21 R22 K33 ["Actions"]
  GETTABLEKS R20 R21 K34 ["SetPurchaseDialogVisibility"]
  CALL R19 1 1
  GETIMPORT R20 K37 [Vector2.new]
  LOADN R21 150
  LOADN R22 150
  CALL R20 2 1
  DUPCLOSURE R21 K38 [PROTO_4]
  CAPTURE VAL R15
  CAPTURE VAL R14
  CAPTURE VAL R16
  CAPTURE VAL R7
  CAPTURE VAL R6
  CAPTURE VAL R3
  CAPTURE VAL R2
  CAPTURE VAL R11
  CAPTURE VAL R8
  CAPTURE VAL R5
  CAPTURE VAL R19
  CAPTURE VAL R18
  CAPTURE VAL R4
  CAPTURE VAL R12
  CAPTURE VAL R20
  CAPTURE VAL R9
  CAPTURE VAL R10
  RETURN R21 1

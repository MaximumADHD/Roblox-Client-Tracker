--[[
	...
	position = UDim2
	size = UDim2
	currentPreview = Model, the model is only for previewing. So, it contains the assetInstance with all he
...
function ModelPreview:init(props)
	self.orbitDrag = false
	self.panDrag = false
	self.doubleClickTimestamp = tick()

	-- Need reference to VireportFrame so I can set the preview model to it.
...
function ModelPreview:makeViewportModel()
	local currentPreview = self.props.currentPreview

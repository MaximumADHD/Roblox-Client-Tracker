...
local function getUserCollaboratorPermissions(props, localized)
	...
	local permissions = {
		{
			Key = PermissionsConstants.UseViewKey,
			Display = localized.PackagePermissions.ActionDropdown.UseViewLabel,
			Description = localized.PackagePermissions.ActionDropdown.UseViewDescription,
		},
		{
...
function CollaboratorsWidget:renderContent(theme, localization, localized)
	...
	if props.Permissions[PermissionsConstants.UserSubjectKey] then
		for userId, permission in pairs(props.Permissions[PermissionsConstants.UserSubjectKey]) do
			if
				props.OwnerType ~= Enum.CreatorType.User
				or props.OwnerId ~= userId
					and permission[PermissionsConstants.ActionKey] ~= PermissionsConstants.NoAccessKey
			then
				table.insert(users, {
					Name = permission[PermissionsConstants.SubjectNameKey],
					Id = permission[PermissionsConstants.SubjectIdKey],
					Action = permission[PermissionsConstants.ActionKey],
				})
			end
		end

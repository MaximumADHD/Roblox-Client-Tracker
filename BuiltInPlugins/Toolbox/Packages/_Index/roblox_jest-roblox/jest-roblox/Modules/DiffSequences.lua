-- upstream: https://github.com/facebook/jest/blob/v26.5.3/packages/diff-sequences/src/index.ts
-- /**
...
-- deviation: replacing uses of NOT_YET_SET with 0 since it's a js specific language workaround
...
local function extendPathsF(
	...
	aIndexesF[iF + 1] = aIndexesF[iF + 1] + countCommonItemsF(
		aFirst + 1,
		aEnd,
		bF + aFirst - kF + 1,
		bEnd,
...
local function extendPathsR(
	...
	aIndexesR[iR + 1] = aIndexesR[iR + 1] - countCommonItemsR(
		aStart,
		aFirst - 1,
		bStart,
		bR + aFirst - kR - 1,
...
local function extendOverlappablePathsR(
	...
	local aIndexPrev1 = 0 -- // prev value of [iR - 1] in next iteration
...
local function findSubsequences(nChange,
	aStart: number,
	aEnd: number,
	bStart: number,
	bEnd: number,

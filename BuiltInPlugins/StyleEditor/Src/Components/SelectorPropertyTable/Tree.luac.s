PROTO_0:
  GETTABLEKS R1 R0 K0 ["Children"]
  JUMPIF R1 [+2]
  NEWTABLE R1 0 0
  RETURN R1 1

PROTO_1:
  GETTABLEKS R2 R0 K0 ["Id"]
  OR R1 R2 R0
  RETURN R1 1

PROTO_2:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["_buildStyleRule"]
  MOVE R2 R0
  GETUPVAL R3 1
  CALL R1 2 -1
  RETURN R1 -1

PROTO_3:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["createItemId"]
  MOVE R3 R0
  CALL R2 1 1
  GETUPVAL R3 1
  GETUPVAL R5 2
  GETTABLEKS R4 R5 K1 ["getOrderedStyleRules"]
  MOVE R5 R0
  CALL R4 1 1
  NEWCLOSURE R5 P0
  CAPTURE UPVAL U3
  CAPTURE VAL R1
  CALL R3 2 1
  GETTABLEKS R4 R0 K2 ["SelectorError"]
  GETUPVAL R5 4
  JUMPIF R5 [+14]
  JUMPIFNOTEQKS R4 K3 [""] [+13]
  GETUPVAL R6 5
  GETTABLEKS R5 R6 K4 ["DEPRECATED_isNestedPseudoInstance"]
  MOVE R6 R0
  CALL R5 1 1
  JUMPIFNOT R5 [+6]
  LOADK R7 K5 ["Label"]
  LOADK R8 K6 ["NestedPseudoInstanceError"]
  NAMECALL R5 R1 K7 ["getText"]
  CALL R5 3 1
  MOVE R4 R5
  DUPTABLE R5 K12 [{"Id", "Name", "Instance", "Children"}]
  SETTABLEKS R2 R5 K8 ["Id"]
  DUPTABLE R6 K18 [{"ErrorMessage", "FullSpan", "Actions", "Schema", "Value"}]
  SETTABLEKS R4 R6 K13 ["ErrorMessage"]
  LOADB R7 1
  SETTABLEKS R7 R6 K14 ["FullSpan"]
  GETUPVAL R8 6
  GETTABLEKS R7 R8 K19 ["FullSpanMoreAction"]
  SETTABLEKS R7 R6 K15 ["Actions"]
  GETUPVAL R8 7
  GETTABLEKS R7 R8 K20 ["SelectorSchema"]
  SETTABLEKS R7 R6 K16 ["Schema"]
  GETTABLEKS R7 R0 K21 ["Selector"]
  SETTABLEKS R7 R6 K17 ["Value"]
  SETTABLEKS R6 R5 K9 ["Name"]
  SETTABLEKS R0 R5 K10 ["Instance"]
  GETUPVAL R6 8
  GETUPVAL R8 3
  GETTABLEKS R7 R8 K22 ["_buildProperties"]
  MOVE R8 R2
  MOVE R9 R0
  CALL R7 2 1
  MOVE R8 R3
  CALL R6 2 1
  SETTABLEKS R6 R5 K11 ["Children"]
  RETURN R5 1

PROTO_4:
  DUPTABLE R2 K4 [{"Id", "Name", "Value", "Children"}]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K5 ["createItemId"]
  MOVE R4 R0
  LOADK R5 K6 ["@Empty"]
  CALL R3 2 1
  SETTABLEKS R3 R2 K0 ["Id"]
  DUPTABLE R3 K8 [{"Schema", "Value"}]
  GETUPVAL R4 1
  GETUPVAL R6 2
  GETTABLEKS R5 R6 K9 ["PropertyNameSchema"]
  DUPTABLE R6 K11 [{"StyleRuleInfo"}]
  SETTABLEKS R1 R6 K10 ["StyleRuleInfo"]
  CALL R4 2 1
  SETTABLEKS R4 R3 K7 ["Schema"]
  LOADK R4 K12 [""]
  SETTABLEKS R4 R3 K2 ["Value"]
  SETTABLEKS R3 R2 K1 ["Name"]
  DUPTABLE R3 K14 [{"Actions", "Schema", "Value"}]
  GETUPVAL R5 3
  GETTABLEKS R4 R5 K15 ["MoreAction"]
  SETTABLEKS R4 R3 K13 ["Actions"]
  DUPTABLE R4 K17 [{"Type"}]
  LOADK R5 K18 ["Empty"]
  SETTABLEKS R5 R4 K16 ["Type"]
  SETTABLEKS R4 R3 K7 ["Schema"]
  LOADK R4 K12 [""]
  SETTABLEKS R4 R3 K2 ["Value"]
  SETTABLEKS R3 R2 K2 ["Value"]
  NEWTABLE R3 0 0
  SETTABLEKS R3 R2 K3 ["Children"]
  RETURN R2 1

PROTO_5:
  GETUPVAL R3 0
  GETTABLE R2 R3 R1
  LOADNIL R3
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K0 ["parseAttributeName"]
  MOVE R6 R2
  CALL R5 1 1
  JUMPIFNOTEQKNIL R5 [+2]
  LOADB R4 0 +1
  LOADB R4 1
  GETUPVAL R5 2
  JUMPIFNOT R5 [+11]
  GETUPVAL R5 2
  JUMPIFEQKS R5 K1 [""] [+9]
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K2 ["getPropertySchema"]
  MOVE R6 R1
  GETUPVAL R7 2
  CALL R5 2 1
  MOVE R3 R5
  JUMP [+20]
  JUMPIFEQKNIL R2 [+11]
  JUMPIF R4 [+9]
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K3 ["PropertySchemas"]
  FASTCALL1 TYPEOF R2 [+3]
  MOVE R7 R2
  GETIMPORT R6 K5 [typeof]
  CALL R6 1 1
  GETTABLE R3 R5 R6
  JUMPIF R3 [+7]
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K2 ["getPropertySchema"]
  MOVE R6 R1
  LOADK R7 K1 [""]
  CALL R5 2 1
  MOVE R3 R5
  JUMPIF R3 [+7]
  JUMPIFNOTEQKS R1 K1 [""] [+3]
  LOADNIL R5
  RETURN R5 1
  GETUPVAL R5 3
  GETTABLEKS R3 R5 K6 ["TextSchema"]
  JUMPIFNOT R4 [+17]
  GETUPVAL R6 3
  GETTABLEKS R5 R6 K7 ["getPossiblePropertyTypes"]
  MOVE R6 R1
  CALL R5 1 1
  GETUPVAL R6 4
  GETUPVAL R8 3
  GETTABLEKS R7 R8 K8 ["TokenReferenceSchema"]
  DUPTABLE R8 K11 [{"Instance", "ExpectedTypes"}]
  GETUPVAL R9 5
  SETTABLEKS R9 R8 K9 ["Instance"]
  SETTABLEKS R5 R8 K10 ["ExpectedTypes"]
  CALL R6 2 1
  MOVE R3 R6
  GETUPVAL R6 6
  GETTABLEKS R5 R6 K12 ["createItemId"]
  GETUPVAL R6 5
  MOVE R7 R1
  CALL R5 2 1
  DUPTABLE R6 K17 [{"Id", "Name", "Value", "Children"}]
  SETTABLEKS R5 R6 K13 ["Id"]
  DUPTABLE R7 K19 [{"Schema", "Value"}]
  GETUPVAL R8 4
  GETUPVAL R10 3
  GETTABLEKS R9 R10 K20 ["PropertyNameSchema"]
  DUPTABLE R10 K22 [{"StyleRuleInfo"}]
  GETUPVAL R11 7
  SETTABLEKS R11 R10 K21 ["StyleRuleInfo"]
  CALL R8 2 1
  SETTABLEKS R8 R7 K18 ["Schema"]
  SETTABLEKS R1 R7 K15 ["Value"]
  SETTABLEKS R7 R6 K14 ["Name"]
  DUPTABLE R7 K24 [{"Actions", "Schema", "Value"}]
  GETUPVAL R9 8
  GETTABLEKS R8 R9 K25 ["MoreAction"]
  SETTABLEKS R8 R7 K23 ["Actions"]
  SETTABLEKS R3 R7 K18 ["Schema"]
  GETTABLEKS R9 R3 K26 ["Validate"]
  MOVE R10 R2
  CALL R9 1 1
  JUMPIFNOT R9 [+2]
  MOVE R8 R2
  JUMP [+3]
  GETTABLEKS R8 R3 K27 ["GetDefaultValue"]
  CALL R8 0 1
  SETTABLEKS R8 R7 K15 ["Value"]
  SETTABLEKS R7 R6 K15 ["Value"]
  GETTABLEKS R8 R3 K26 ["Validate"]
  MOVE R9 R2
  CALL R8 1 1
  JUMPIFNOT R8 [+9]
  GETTABLEKS R8 R3 K28 ["GetChildren"]
  JUMPIFNOT R8 [+6]
  GETTABLEKS R7 R3 K28 ["GetChildren"]
  MOVE R8 R2
  MOVE R9 R5
  CALL R7 2 1
  JUMP [+2]
  NEWTABLE R7 0 0
  SETTABLEKS R7 R6 K16 ["Children"]
  RETURN R6 1

PROTO_6:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["findFirstClassSelector"]
  MOVE R3 R1
  CALL R2 1 1
  NAMECALL R3 R1 K1 ["GetProperties"]
  CALL R3 1 1
  GETUPVAL R4 1
  MOVE R5 R3
  CALL R4 1 1
  GETUPVAL R6 2
  GETTABLEKS R5 R6 K2 ["getStyleInfo"]
  MOVE R6 R1
  CALL R5 1 1
  GETIMPORT R6 K5 [table.sort]
  MOVE R7 R4
  CALL R6 1 0
  GETUPVAL R6 3
  MOVE R7 R4
  NEWCLOSURE R8 P0
  CAPTURE VAL R3
  CAPTURE UPVAL U4
  CAPTURE VAL R2
  CAPTURE UPVAL U5
  CAPTURE UPVAL U6
  CAPTURE VAL R1
  CAPTURE UPVAL U7
  CAPTURE VAL R5
  CAPTURE UPVAL U8
  CALL R6 2 1
  MOVE R8 R6
  GETUPVAL R10 9
  GETTABLEKS R9 R10 K6 ["_buildEmptyRow"]
  MOVE R10 R0
  MOVE R11 R5
  CALL R9 2 -1
  FASTCALL TABLE_INSERT [+2]
  GETIMPORT R7 K8 [table.insert]
  CALL R7 -1 0
  RETURN R6 1

PROTO_7:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["_buildStyleRule"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 1
  NEWTABLE R3 0 1
  MOVE R4 R2
  SETLIST R3 R4 1 [1]
  GETUPVAL R4 1
  MOVE R6 R3
  NAMECALL R4 R4 K1 ["updatePaths"]
  CALL R4 2 -1
  RETURN R4 -1

PROTO_8:
  JUMPIFNOT R0 [+3]
  LENGTH R2 R0
  JUMPIFNOTEQKN R2 K0 [0] [+2]
  RETURN R0 0
  GETIMPORT R2 K2 [ipairs]
  MOVE R3 R0
  CALL R2 1 3
  FORGPREP_INEXT R2
  GETTABLEKS R8 R6 K3 ["Id"]
  GETTABLE R7 R1 R8
  JUMPIFEQKB R7 FALSE [+10]
  GETTABLEKS R8 R6 K4 ["Name"]
  GETTABLEKS R7 R8 K5 ["FullSpan"]
  JUMPIFNOT R7 [+4]
  GETTABLEKS R7 R6 K3 ["Id"]
  LOADB R8 1
  SETTABLE R8 R1 R7
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K6 ["expandNewRows"]
  GETTABLEKS R8 R6 K7 ["Children"]
  MOVE R9 R1
  CALL R7 2 0
  FORGLOOP R2 2 [inext] [-22]
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R4 K1 [script]
  GETTABLEKS R3 R4 K2 ["Parent"]
  GETTABLEKS R2 R3 K2 ["Parent"]
  GETTABLEKS R1 R2 K2 ["Parent"]
  GETTABLEKS R0 R1 K2 ["Parent"]
  GETIMPORT R1 K4 [require]
  GETTABLEKS R3 R0 K5 ["Packages"]
  GETTABLEKS R2 R3 K6 ["Framework"]
  CALL R1 1 1
  GETTABLEKS R3 R1 K7 ["Util"]
  GETTABLEKS R2 R3 K8 ["EditTree"]
  GETIMPORT R3 K4 [require]
  GETTABLEKS R6 R0 K9 ["Src"]
  GETTABLEKS R5 R6 K10 ["Flags"]
  GETTABLEKS R4 R5 K11 ["getEngineFeatureNestedPseudoInstance"]
  CALL R3 1 1
  CALL R3 0 1
  GETIMPORT R4 K4 [require]
  GETTABLEKS R6 R0 K5 ["Packages"]
  GETTABLEKS R5 R6 K12 ["Dash"]
  CALL R4 1 1
  GETTABLEKS R5 R4 K13 ["append"]
  GETTABLEKS R6 R4 K14 ["collectArray"]
  GETTABLEKS R7 R4 K15 ["join"]
  GETTABLEKS R8 R4 K16 ["keys"]
  GETTABLEKS R9 R4 K17 ["map"]
  GETIMPORT R10 K4 [require]
  GETTABLEKS R13 R0 K9 ["Src"]
  GETTABLEKS R12 R13 K7 ["Util"]
  GETTABLEKS R11 R12 K18 ["MenuHelpers"]
  CALL R10 1 1
  GETIMPORT R11 K4 [require]
  GETTABLEKS R14 R0 K9 ["Src"]
  GETTABLEKS R13 R14 K7 ["Util"]
  GETTABLEKS R12 R13 K19 ["SelectorHelpers"]
  CALL R11 1 1
  GETIMPORT R12 K4 [require]
  GETTABLEKS R15 R0 K9 ["Src"]
  GETTABLEKS R14 R15 K7 ["Util"]
  GETTABLEKS R13 R14 K20 ["StyleRuleHelpers"]
  CALL R12 1 1
  GETIMPORT R13 K4 [require]
  GETTABLEKS R16 R0 K9 ["Src"]
  GETTABLEKS R15 R16 K7 ["Util"]
  GETTABLEKS R14 R15 K21 ["StyleSchema"]
  CALL R13 1 1
  GETIMPORT R14 K4 [require]
  GETTABLEKS R17 R0 K9 ["Src"]
  GETTABLEKS R16 R17 K7 ["Util"]
  GETTABLEKS R15 R16 K22 ["Styling"]
  CALL R14 1 1
  GETIMPORT R15 K4 [require]
  GETTABLEKS R18 R0 K9 ["Src"]
  GETTABLEKS R17 R18 K7 ["Util"]
  GETTABLEKS R16 R17 K23 ["TokenHelpers"]
  CALL R15 1 1
  GETIMPORT R16 K4 [require]
  GETTABLEKS R19 R0 K9 ["Src"]
  GETTABLEKS R18 R19 K7 ["Util"]
  GETTABLEKS R17 R18 K24 ["TreeTableHelpers"]
  CALL R16 1 1
  GETIMPORT R17 K4 [require]
  GETTABLEKS R19 R0 K9 ["Src"]
  GETTABLEKS R18 R19 K25 ["Types"]
  CALL R17 1 1
  NEWTABLE R18 16 0
  NEWTABLE R19 0 2
  DUPTABLE R20 K29 [{"Name", "Key", "Width"}]
  LOADK R21 K26 ["Name"]
  SETTABLEKS R21 R20 K26 ["Name"]
  LOADK R21 K26 ["Name"]
  SETTABLEKS R21 R20 K27 ["Key"]
  GETIMPORT R21 K32 [UDim.new]
  LOADK R22 K33 [0.5]
  LOADN R23 0
  CALL R21 2 1
  SETTABLEKS R21 R20 K28 ["Width"]
  DUPTABLE R21 K29 [{"Name", "Key", "Width"}]
  LOADK R22 K34 ["Value"]
  SETTABLEKS R22 R21 K26 ["Name"]
  LOADK R22 K34 ["Value"]
  SETTABLEKS R22 R21 K27 ["Key"]
  GETIMPORT R22 K32 [UDim.new]
  LOADK R23 K33 [0.5]
  LOADN R24 0
  CALL R22 2 1
  SETTABLEKS R22 R21 K28 ["Width"]
  SETLIST R19 R20 2 [1]
  SETTABLEKS R19 R18 K35 ["Columns"]
  LOADN R19 1
  SETTABLEKS R19 R18 K36 ["NAME_COLUMN_INDEX"]
  LOADN R19 2
  SETTABLEKS R19 R18 K37 ["VALUE_COLUMN_INDEX"]
  DUPCLOSURE R19 K38 [PROTO_0]
  SETTABLEKS R19 R18 K39 ["getItemChildren"]
  DUPCLOSURE R19 K40 [PROTO_1]
  SETTABLEKS R19 R18 K41 ["getItemId"]
  GETTABLEKS R19 R2 K31 ["new"]
  DUPTABLE R20 K44 [{"getChildren", "getId"}]
  GETTABLEKS R21 R18 K39 ["getItemChildren"]
  SETTABLEKS R21 R20 K42 ["getChildren"]
  GETTABLEKS R21 R18 K41 ["getItemId"]
  SETTABLEKS R21 R20 K43 ["getId"]
  CALL R19 1 1
  SETTABLEKS R19 R18 K8 ["EditTree"]
  DUPCLOSURE R20 K45 [PROTO_3]
  CAPTURE VAL R16
  CAPTURE VAL R9
  CAPTURE VAL R12
  CAPTURE VAL R18
  CAPTURE VAL R3
  CAPTURE VAL R11
  CAPTURE VAL R10
  CAPTURE VAL R13
  CAPTURE VAL R5
  SETTABLEKS R20 R18 K46 ["_buildStyleRule"]
  DUPCLOSURE R20 K47 [PROTO_4]
  CAPTURE VAL R16
  CAPTURE VAL R7
  CAPTURE VAL R13
  CAPTURE VAL R10
  SETTABLEKS R20 R18 K48 ["_buildEmptyRow"]
  DUPCLOSURE R20 K49 [PROTO_6]
  CAPTURE VAL R11
  CAPTURE VAL R8
  CAPTURE VAL R14
  CAPTURE VAL R6
  CAPTURE VAL R15
  CAPTURE VAL R13
  CAPTURE VAL R7
  CAPTURE VAL R16
  CAPTURE VAL R10
  CAPTURE VAL R18
  SETTABLEKS R20 R18 K50 ["_buildProperties"]
  DUPCLOSURE R20 K51 [PROTO_7]
  CAPTURE VAL R18
  CAPTURE VAL R19
  SETTABLEKS R20 R18 K52 ["buildItems"]
  DUPCLOSURE R20 K53 [PROTO_8]
  CAPTURE VAL R18
  SETTABLEKS R20 R18 K54 ["expandNewRows"]
  RETURN R18 1

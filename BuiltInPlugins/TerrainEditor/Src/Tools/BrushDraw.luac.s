PROTO_0:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["OnFormChanged"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K2 ["OnGizmoChanged"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  RETURN R0 0

PROTO_1:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["Set"]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K1 ["BrushSettings"]
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K2 ["Storage"]
  GETUPVAL R4 3
  GETTABLEKS R3 R4 K3 ["_pluginController"]
  NEWTABLE R4 1 0
  GETUPVAL R6 4
  GETTABLEKS R5 R6 K4 ["PlaneTransform"]
  GETIMPORT R6 K7 [CFrame.Angles]
  LOADK R7 K8 [1.5707963267949]
  LOADN R8 0
  LOADN R9 0
  CALL R6 3 1
  SETTABLE R6 R4 R5
  CALL R0 4 0
  GETIMPORT R0 K11 [task.spawn]
  NEWCLOSURE R1 P0
  CAPTURE UPVAL U3
  CALL R0 1 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["init"]
  MOVE R4 R0
  MOVE R5 R1
  MOVE R6 R2
  CALL R3 3 0
  NEWTABLE R3 1 0
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["BrushSettings"]
  NEWTABLE R5 1 0
  GETUPVAL R7 2
  GETTABLEKS R6 R7 K2 ["ManualPlaneLock"]
  DUPTABLE R7 K4 [{"Schema"}]
  DUPTABLE R8 K6 [{"OnReset"}]
  NEWCLOSURE R9 P0
  CAPTURE UPVAL U3
  CAPTURE UPVAL U1
  CAPTURE UPVAL U4
  CAPTURE VAL R0
  CAPTURE UPVAL U2
  SETTABLEKS R9 R8 K5 ["OnReset"]
  SETTABLEKS R8 R7 K3 ["Schema"]
  SETTABLE R7 R5 R6
  SETTABLE R5 R3 R4
  SETTABLEKS R3 R0 K7 ["_overrides"]
  RETURN R0 0

PROTO_3:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["saveForm"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 0
  GETTABLEKS R2 R0 K1 ["_operation"]
  JUMPIFNOT R2 [+8]
  GETTABLEKS R2 R0 K1 ["_operation"]
  NAMECALL R4 R0 K2 ["getPayload"]
  CALL R4 1 -1
  NAMECALL R2 R2 K3 ["updatePayload"]
  CALL R2 -1 0
  RETURN R0 0

PROTO_4:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["saveGizmos"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 0
  GETTABLEKS R2 R0 K1 ["_operation"]
  JUMPIFNOT R2 [+8]
  GETTABLEKS R2 R0 K1 ["_operation"]
  NAMECALL R4 R0 K2 ["getPayload"]
  CALL R4 1 -1
  NAMECALL R2 R2 K3 ["updatePayload"]
  CALL R2 -1 0
  RETURN R0 0

PROTO_5:
  GETUPVAL R0 0
  GETUPVAL R1 1
  LOADK R2 K0 ["Draw"]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K1 ["_services"]
  CALL R1 2 1
  SETTABLEKS R1 R0 K2 ["_operation"]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K2 ["_operation"]
  DUPTABLE R2 K4 [{"Payload"}]
  GETUPVAL R3 0
  NAMECALL R3 R3 K5 ["getPayload"]
  CALL R3 1 1
  SETTABLEKS R3 R2 K3 ["Payload"]
  NAMECALL R0 R0 K6 ["start"]
  CALL R0 2 0
  RETURN R0 0

PROTO_6:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_operation"]
  JUMPIFNOT R0 [+13]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_operation"]
  NAMECALL R0 R0 K1 ["isRunning"]
  CALL R0 1 1
  JUMPIFNOT R0 [+6]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_operation"]
  NAMECALL R0 R0 K2 ["destroy"]
  CALL R0 1 0
  RETURN R0 0

PROTO_7:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["activate"]
  MOVE R2 R0
  CALL R1 1 0
  GETTABLEKS R1 R0 K1 ["_shortcutController"]
  NAMECALL R1 R1 K2 ["getMouse"]
  CALL R1 1 1
  GETTABLEKS R3 R0 K1 ["_shortcutController"]
  GETTABLEKS R2 R3 K3 ["MouseDownAction"]
  NEWCLOSURE R4 P0
  CAPTURE VAL R0
  CAPTURE UPVAL U1
  NAMECALL R2 R2 K4 ["Connect"]
  CALL R2 2 1
  SETTABLEKS R2 R0 K5 ["_mouseDownConnection"]
  GETTABLEKS R2 R1 K6 ["Button1Up"]
  NEWCLOSURE R4 P1
  CAPTURE VAL R0
  NAMECALL R2 R2 K4 ["Connect"]
  CALL R2 2 1
  SETTABLEKS R2 R0 K7 ["_mouseUpConnection"]
  RETURN R0 0

PROTO_8:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["deactivate"]
  MOVE R2 R0
  CALL R1 1 0
  GETTABLEKS R1 R0 K1 ["_mouseDownConnection"]
  NAMECALL R1 R1 K2 ["Disconnect"]
  CALL R1 1 0
  GETTABLEKS R1 R0 K3 ["_mouseUpConnection"]
  NAMECALL R1 R1 K2 ["Disconnect"]
  CALL R1 1 0
  GETTABLEKS R1 R0 K4 ["_operation"]
  JUMPIFNOT R1 [+11]
  GETTABLEKS R1 R0 K4 ["_operation"]
  NAMECALL R1 R1 K5 ["isRunning"]
  CALL R1 1 1
  JUMPIFNOT R1 [+5]
  GETTABLEKS R1 R0 K4 ["_operation"]
  NAMECALL R1 R1 K6 ["destroy"]
  CALL R1 1 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["TerrainEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETIMPORT R4 K1 [script]
  GETTABLEKS R3 R4 K6 ["Parent"]
  GETTABLEKS R2 R3 K7 ["BaseTool"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R6 R0 K8 ["Src"]
  GETTABLEKS R5 R6 K9 ["Schemas"]
  GETTABLEKS R4 R5 K10 ["Settings"]
  GETTABLEKS R3 R4 K11 ["Brush"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R7 R0 K8 ["Src"]
  GETTABLEKS R6 R7 K9 ["Schemas"]
  GETTABLEKS R5 R6 K10 ["Settings"]
  GETTABLEKS R4 R5 K12 ["Material"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R8 R0 K8 ["Src"]
  GETTABLEKS R7 R8 K13 ["Util"]
  GETTABLEKS R6 R7 K14 ["Operations"]
  GETTABLEKS R5 R6 K15 ["DrawOperation"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R7 R0 K8 ["Src"]
  GETTABLEKS R6 R7 K16 ["Types"]
  CALL R5 1 1
  GETTABLEKS R6 R5 K17 ["BrushMode"]
  GETTABLEKS R7 R5 K18 ["BrushSettings"]
  GETTABLEKS R8 R5 K19 ["BrushShape"]
  GETTABLEKS R9 R5 K20 ["Categories"]
  GETTABLEKS R10 R5 K21 ["Gizmo"]
  GETTABLEKS R11 R5 K22 ["MaterialSettings"]
  GETTABLEKS R12 R5 K23 ["PivotPosition"]
  GETTABLEKS R13 R5 K24 ["Snapping"]
  GETTABLEKS R14 R5 K25 ["ToolCategories"]
  GETTABLEKS R15 R5 K26 ["Tools"]
  GETIMPORT R16 K5 [require]
  GETTABLEKS R19 R0 K8 ["Src"]
  GETTABLEKS R18 R19 K13 ["Util"]
  GETTABLEKS R17 R18 K27 ["SettingsHelper"]
  CALL R16 1 1
  NEWTABLE R17 0 2
  DUPTABLE R18 K31 [{"Defaults", "Id", "Schema"}]
  NEWTABLE R19 16 0
  GETTABLEKS R20 R7 K17 ["BrushMode"]
  GETTABLEKS R21 R6 K32 ["Add"]
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K19 ["BrushShape"]
  GETTABLEKS R21 R8 K33 ["Sphere"]
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K34 ["BaseSize"]
  LOADN R21 6
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K35 ["BaseHeight"]
  LOADN R21 6
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K23 ["PivotPosition"]
  GETTABLEKS R21 R12 K36 ["Center"]
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K24 ["Snapping"]
  GETTABLEKS R21 R13 K37 ["Off"]
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K38 ["State"]
  DUPTABLE R21 K40 [{"Position"}]
  LOADN R23 0
  LOADN R24 0
  LOADN R25 0
  FASTCALL VECTOR [+2]
  GETIMPORT R22 K43 [Vector3.new]
  CALL R22 3 1
  SETTABLEKS R22 R21 K39 ["Position"]
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K44 ["IgnoreWater"]
  LOADB R21 0
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K45 ["IgnoreParts"]
  LOADB R21 1
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K46 ["TemporarySmooth"]
  LOADB R21 0
  SETTABLE R21 R19 R20
  GETTABLEKS R20 R7 K47 ["WaterAutofill"]
  LOADB R21 0
  SETTABLE R21 R19 R20
  SETTABLEKS R19 R18 K28 ["Defaults"]
  GETTABLEKS R19 R9 K18 ["BrushSettings"]
  SETTABLEKS R19 R18 K29 ["Id"]
  SETTABLEKS R2 R18 K30 ["Schema"]
  DUPTABLE R19 K31 [{"Defaults", "Id", "Schema"}]
  NEWTABLE R20 2 0
  GETTABLEKS R21 R11 K48 ["AutoMaterial"]
  LOADB R22 0
  SETTABLE R22 R20 R21
  GETTABLEKS R21 R11 K49 ["SourceMaterial"]
  GETIMPORT R22 K52 [Enum.Material.Grass]
  SETTABLE R22 R20 R21
  SETTABLEKS R20 R19 K28 ["Defaults"]
  GETTABLEKS R20 R9 K22 ["MaterialSettings"]
  SETTABLEKS R20 R19 K29 ["Id"]
  SETTABLEKS R3 R19 K30 ["Schema"]
  SETLIST R17 R18 2 [1]
  NEWTABLE R18 0 2
  DUPTABLE R19 K53 [{"Id", "Schema"}]
  GETTABLEKS R20 R10 K54 ["Plane"]
  SETTABLEKS R20 R19 K29 ["Id"]
  DUPTABLE R20 K56 [{"Type"}]
  GETTABLEKS R21 R10 K54 ["Plane"]
  SETTABLEKS R21 R20 K55 ["Type"]
  SETTABLEKS R20 R19 K30 ["Schema"]
  DUPTABLE R20 K53 [{"Id", "Schema"}]
  GETTABLEKS R21 R10 K11 ["Brush"]
  SETTABLEKS R21 R20 K29 ["Id"]
  DUPTABLE R21 K56 [{"Type"}]
  GETTABLEKS R22 R10 K11 ["Brush"]
  SETTABLEKS R22 R21 K55 ["Type"]
  SETTABLEKS R21 R20 K30 ["Schema"]
  SETLIST R18 R19 2 [1]
  GETTABLEKS R21 R15 K57 ["Draw"]
  GETTABLEKS R22 R14 K58 ["Edit"]
  MOVE R23 R17
  MOVE R24 R18
  NAMECALL R19 R1 K42 ["new"]
  CALL R19 5 1
  DUPCLOSURE R20 K59 [PROTO_2]
  CAPTURE VAL R1
  CAPTURE VAL R9
  CAPTURE VAL R7
  CAPTURE VAL R16
  CAPTURE VAL R2
  SETTABLEKS R20 R19 K60 ["init"]
  DUPCLOSURE R20 K61 [PROTO_3]
  CAPTURE VAL R1
  SETTABLEKS R20 R19 K62 ["saveForm"]
  DUPCLOSURE R20 K63 [PROTO_4]
  CAPTURE VAL R1
  SETTABLEKS R20 R19 K64 ["saveGizmos"]
  DUPCLOSURE R20 K65 [PROTO_7]
  CAPTURE VAL R1
  CAPTURE VAL R4
  SETTABLEKS R20 R19 K66 ["activate"]
  DUPCLOSURE R20 K67 [PROTO_8]
  CAPTURE VAL R1
  SETTABLEKS R20 R19 K68 ["deactivate"]
  RETURN R19 1

PROTO_0:
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["init"]
  MOVE R4 R0
  MOVE R5 R1
  MOVE R6 R2
  CALL R3 3 0
  NEWTABLE R3 0 0
  SETTABLEKS R3 R0 K1 ["_tools"]
  GETUPVAL R3 1
  LOADNIL R4
  LOADNIL R5
  FORGPREP R3
  NAMECALL R8 R7 K2 ["new"]
  CALL R8 1 1
  MOVE R11 R1
  MOVE R12 R2
  NAMECALL R9 R8 K0 ["init"]
  CALL R9 3 0
  GETTABLEKS R9 R0 K1 ["_tools"]
  SETTABLE R8 R9 R6
  FORGLOOP R3 2 [-12]
  RETURN R0 0

PROTO_1:
  NAMECALL R1 R0 K0 ["getPayload"]
  CALL R1 1 1
  GETTABLEKS R3 R0 K1 ["_tools"]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["BuildSettings"]
  GETTABLE R5 R1 R6
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K3 ["TerrainType"]
  GETTABLE R4 R5 R6
  GETTABLE R2 R3 R4
  NAMECALL R2 R2 K4 ["form"]
  CALL R2 1 -1
  RETURN R2 -1

PROTO_2:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["OnFormChanged"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["OnGizmoChanged"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  RETURN R0 0

PROTO_4:
  NAMECALL R2 R0 K0 ["getPayload"]
  CALL R2 1 1
  GETTABLEKS R4 R0 K1 ["_tools"]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K2 ["BuildSettings"]
  GETTABLE R6 R2 R7
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K3 ["TerrainType"]
  GETTABLE R5 R6 R7
  GETTABLE R3 R4 R5
  MOVE R5 R1
  NAMECALL R3 R3 K4 ["saveForm"]
  CALL R3 2 0
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K2 ["BuildSettings"]
  GETTABLE R4 R1 R5
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K3 ["TerrainType"]
  GETTABLE R3 R4 R5
  GETTABLEKS R5 R0 K1 ["_tools"]
  GETUPVAL R9 0
  GETTABLEKS R8 R9 K2 ["BuildSettings"]
  GETTABLE R7 R2 R8
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K3 ["TerrainType"]
  GETTABLE R6 R7 R8
  GETTABLE R4 R5 R6
  NAMECALL R4 R4 K0 ["getPayload"]
  CALL R4 1 1
  JUMPIFNOT R3 [+46]
  GETTABLEKS R5 R0 K5 ["_onFormChangedForwarder"]
  JUMPIFNOT R5 [+5]
  GETTABLEKS R5 R0 K5 ["_onFormChangedForwarder"]
  NAMECALL R5 R5 K6 ["Disconnect"]
  CALL R5 1 0
  GETTABLEKS R5 R0 K7 ["_onGizmoChangedForwarder"]
  JUMPIFNOT R5 [+5]
  GETTABLEKS R5 R0 K7 ["_onGizmoChangedForwarder"]
  NAMECALL R5 R5 K6 ["Disconnect"]
  CALL R5 1 0
  GETTABLEKS R7 R0 K1 ["_tools"]
  GETTABLE R6 R7 R3
  GETTABLEKS R5 R6 K8 ["OnFormChanged"]
  NEWCLOSURE R7 P0
  CAPTURE VAL R0
  NAMECALL R5 R5 K9 ["Connect"]
  CALL R5 2 1
  SETTABLEKS R5 R0 K5 ["_onFormChangedForwarder"]
  GETTABLEKS R7 R0 K1 ["_tools"]
  GETTABLE R6 R7 R3
  GETTABLEKS R5 R6 K10 ["OnGizmoChanged"]
  NEWCLOSURE R7 P1
  CAPTURE VAL R0
  NAMECALL R5 R5 K9 ["Connect"]
  CALL R5 2 1
  SETTABLEKS R5 R0 K7 ["_onGizmoChangedForwarder"]
  MOVE R7 R3
  MOVE R8 R4
  NAMECALL R5 R0 K11 ["createStamp"]
  CALL R5 3 0
  RETURN R0 0
  GETTABLEKS R5 R0 K12 ["_stamp"]
  JUMPIFNOT R5 [+8]
  GETTABLEKS R5 R0 K12 ["_stamp"]
  MOVE R7 R4
  MOVE R8 R1
  NAMECALL R5 R5 K13 ["update"]
  CALL R5 3 0
  RETURN R0 0
  GETIMPORT R5 K15 [warn]
  LOADK R6 K16 ["stamp not created!"]
  CALL R5 1 0
  RETURN R0 0

PROTO_5:
  NAMECALL R1 R0 K0 ["getPayload"]
  CALL R1 1 1
  GETTABLEKS R3 R0 K1 ["_tools"]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["BuildSettings"]
  GETTABLE R5 R1 R6
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K3 ["TerrainType"]
  GETTABLE R4 R5 R6
  GETTABLE R2 R3 R4
  NAMECALL R2 R2 K4 ["gizmos"]
  CALL R2 1 -1
  RETURN R2 -1

PROTO_6:
  NAMECALL R2 R0 K0 ["getPayload"]
  CALL R2 1 1
  GETTABLEKS R4 R0 K1 ["_tools"]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K2 ["BuildSettings"]
  GETTABLE R6 R2 R7
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K3 ["TerrainType"]
  GETTABLE R5 R6 R7
  GETTABLE R3 R4 R5
  MOVE R5 R1
  NAMECALL R3 R3 K4 ["saveGizmos"]
  CALL R3 2 0
  GETTABLEKS R3 R0 K5 ["_stamp"]
  JUMPIFNOT R3 [+21]
  GETTABLEKS R4 R0 K1 ["_tools"]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K2 ["BuildSettings"]
  GETTABLE R6 R2 R7
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K3 ["TerrainType"]
  GETTABLE R5 R6 R7
  GETTABLE R3 R4 R5
  NAMECALL R3 R3 K0 ["getPayload"]
  CALL R3 1 1
  GETTABLEKS R4 R0 K5 ["_stamp"]
  MOVE R6 R3
  MOVE R7 R1
  NAMECALL R4 R4 K6 ["update"]
  CALL R4 3 0
  RETURN R0 0

PROTO_7:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["OnFormChanged"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  RETURN R0 0

PROTO_8:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["OnGizmoChanged"]
  NAMECALL R0 R0 K1 ["Fire"]
  CALL R0 1 0
  RETURN R0 0

PROTO_9:
  NAMECALL R1 R0 K0 ["getPayload"]
  CALL R1 1 1
  GETTABLEKS R3 R0 K1 ["_tools"]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["BuildSettings"]
  GETTABLE R5 R1 R6
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K3 ["TerrainType"]
  GETTABLE R4 R5 R6
  GETTABLE R2 R3 R4
  NAMECALL R2 R2 K4 ["activate"]
  CALL R2 1 0
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K2 ["BuildSettings"]
  GETTABLE R3 R1 R4
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K3 ["TerrainType"]
  GETTABLE R2 R3 R4
  JUMPIFNOT R2 [+44]
  GETTABLEKS R5 R0 K1 ["_tools"]
  GETTABLE R4 R5 R2
  GETTABLEKS R3 R4 K5 ["OnFormChanged"]
  NEWCLOSURE R5 P0
  CAPTURE VAL R0
  NAMECALL R3 R3 K6 ["Connect"]
  CALL R3 2 1
  SETTABLEKS R3 R0 K7 ["_onFormChangedForwarder"]
  GETTABLEKS R5 R0 K1 ["_tools"]
  GETTABLE R4 R5 R2
  GETTABLEKS R3 R4 K8 ["OnGizmoChanged"]
  NEWCLOSURE R5 P1
  CAPTURE VAL R0
  NAMECALL R3 R3 K6 ["Connect"]
  CALL R3 2 1
  SETTABLEKS R3 R0 K9 ["_onGizmoChangedForwarder"]
  GETTABLEKS R4 R0 K1 ["_tools"]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K2 ["BuildSettings"]
  GETTABLE R6 R1 R7
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K3 ["TerrainType"]
  GETTABLE R5 R6 R7
  GETTABLE R3 R4 R5
  NAMECALL R3 R3 K0 ["getPayload"]
  CALL R3 1 1
  MOVE R6 R2
  MOVE R7 R3
  NAMECALL R4 R0 K10 ["createStamp"]
  CALL R4 3 0
  RETURN R0 0
  GETTABLEKS R3 R0 K11 ["_stamp"]
  JUMPIFNOT R3 [+8]
  GETTABLEKS R3 R0 K11 ["_stamp"]
  NAMECALL R3 R3 K12 ["destroy"]
  CALL R3 1 0
  LOADNIL R3
  SETTABLEKS R3 R0 K11 ["_stamp"]
  RETURN R0 0

PROTO_10:
  NAMECALL R1 R0 K0 ["getPayload"]
  CALL R1 1 1
  GETTABLEKS R3 R0 K1 ["_tools"]
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["BuildSettings"]
  GETTABLE R5 R1 R6
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K3 ["TerrainType"]
  GETTABLE R4 R5 R6
  GETTABLE R2 R3 R4
  NAMECALL R2 R2 K4 ["deactivate"]
  CALL R2 1 0
  GETTABLEKS R2 R0 K5 ["_onFormChangedForwarder"]
  JUMPIFNOT R2 [+5]
  GETTABLEKS R2 R0 K5 ["_onFormChangedForwarder"]
  NAMECALL R2 R2 K6 ["Disconnect"]
  CALL R2 1 0
  GETTABLEKS R2 R0 K7 ["_onGizmoChangedForwarder"]
  JUMPIFNOT R2 [+5]
  GETTABLEKS R2 R0 K7 ["_onGizmoChangedForwarder"]
  NAMECALL R2 R2 K6 ["Disconnect"]
  CALL R2 1 0
  GETTABLEKS R2 R0 K8 ["_stamp"]
  JUMPIFNOT R2 [+8]
  GETTABLEKS R2 R0 K8 ["_stamp"]
  NAMECALL R2 R2 K9 ["destroy"]
  CALL R2 1 0
  LOADNIL R2
  SETTABLEKS R2 R0 K8 ["_stamp"]
  RETURN R0 0

PROTO_11:
  GETTABLEKS R3 R0 K0 ["_stamp"]
  JUMPIFNOT R3 [+5]
  GETTABLEKS R3 R0 K0 ["_stamp"]
  NAMECALL R3 R3 K1 ["destroy"]
  CALL R3 1 0
  GETUPVAL R3 0
  MOVE R5 R1
  NAMECALL R3 R3 K2 ["new"]
  CALL R3 2 1
  SETTABLEKS R3 R0 K0 ["_stamp"]
  GETTABLEKS R3 R0 K0 ["_stamp"]
  MOVE R5 R2
  GETTABLEKS R6 R0 K3 ["_services"]
  NAMECALL R3 R3 K4 ["init"]
  CALL R3 3 0
  GETTABLEKS R3 R0 K0 ["_stamp"]
  MOVE R5 R2
  NAMECALL R3 R3 K5 ["update"]
  CALL R3 2 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["TerrainEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R4 R0 K6 ["Src"]
  GETTABLEKS R3 R4 K7 ["Tools"]
  GETTABLEKS R2 R3 K8 ["BaseTool"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R5 R0 K6 ["Src"]
  GETTABLEKS R4 R5 K9 ["Flags"]
  GETTABLEKS R3 R4 K10 ["getFFlagTerrainEditorRemoveSchema"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R7 R0 K6 ["Src"]
  GETTABLEKS R6 R7 K11 ["Schemas"]
  GETTABLEKS R5 R6 K12 ["Settings"]
  GETTABLEKS R4 R5 K13 ["Build"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R7 R0 K6 ["Src"]
  GETTABLEKS R6 R7 K14 ["Stamps"]
  GETTABLEKS R5 R6 K15 ["BaseStamp"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R7 R0 K6 ["Src"]
  GETTABLEKS R6 R7 K16 ["Types"]
  CALL R5 1 1
  GETTABLEKS R6 R5 K17 ["BuildSettings"]
  GETTABLEKS R7 R5 K18 ["Category"]
  GETTABLEKS R8 R5 K19 ["Gizmo"]
  GETTABLEKS R9 R5 K20 ["Tab"]
  GETTABLEKS R10 R5 K21 ["TerrainType"]
  GETTABLEKS R11 R5 K22 ["Tool"]
  NEWTABLE R12 8 0
  GETTABLEKS R13 R10 K23 ["Arctic"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K23 ["Arctic"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K24 ["Crater"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K24 ["Crater"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K25 ["Desert"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K25 ["Desert"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K26 ["Lake"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K26 ["Lake"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K27 ["Mesa"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K27 ["Mesa"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K28 ["Mountain"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K28 ["Mountain"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K29 ["Plain"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K29 ["Plain"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  GETTABLEKS R13 R10 K30 ["Volcano"]
  GETIMPORT R14 K5 [require]
  GETIMPORT R16 K1 [script]
  GETTABLEKS R15 R16 K30 ["Volcano"]
  CALL R14 1 1
  SETTABLE R14 R12 R13
  NEWTABLE R13 0 1
  DUPTABLE R14 K34 [{"Defaults", "Id", "Schema"}]
  NEWTABLE R15 0 0
  SETTABLEKS R15 R14 K31 ["Defaults"]
  GETTABLEKS R15 R7 K17 ["BuildSettings"]
  SETTABLEKS R15 R14 K32 ["Id"]
  MOVE R16 R2
  CALL R16 0 1
  JUMPIFNOT R16 [+2]
  LOADNIL R15
  JUMP [+1]
  MOVE R15 R3
  SETTABLEKS R15 R14 K33 ["Schema"]
  SETLIST R13 R14 1 [1]
  NEWTABLE R14 0 1
  DUPTABLE R15 K35 [{"Id", "Schema"}]
  GETTABLEKS R16 R8 K36 ["Region"]
  SETTABLEKS R16 R15 K32 ["Id"]
  DUPTABLE R16 K40 [{"Type", "Wireframe", "Rotation"}]
  GETTABLEKS R17 R8 K36 ["Region"]
  SETTABLEKS R17 R16 K37 ["Type"]
  LOADB R17 0
  SETTABLEKS R17 R16 K38 ["Wireframe"]
  LOADB R17 0
  SETTABLEKS R17 R16 K39 ["Rotation"]
  SETTABLEKS R16 R15 K33 ["Schema"]
  SETLIST R14 R15 1 [1]
  GETTABLEKS R17 R11 K13 ["Build"]
  GETTABLEKS R18 R9 K41 ["Create"]
  MOVE R19 R13
  MOVE R20 R14
  NAMECALL R15 R1 K42 ["new"]
  CALL R15 5 1
  DUPCLOSURE R16 K43 [PROTO_0]
  CAPTURE VAL R1
  CAPTURE VAL R12
  SETTABLEKS R16 R15 K44 ["init"]
  DUPCLOSURE R16 K45 [PROTO_1]
  CAPTURE VAL R7
  CAPTURE VAL R6
  SETTABLEKS R16 R15 K46 ["form"]
  DUPCLOSURE R16 K47 [PROTO_4]
  CAPTURE VAL R7
  CAPTURE VAL R6
  SETTABLEKS R16 R15 K48 ["saveForm"]
  DUPCLOSURE R16 K49 [PROTO_5]
  CAPTURE VAL R7
  CAPTURE VAL R6
  SETTABLEKS R16 R15 K50 ["gizmos"]
  DUPCLOSURE R16 K51 [PROTO_6]
  CAPTURE VAL R7
  CAPTURE VAL R6
  SETTABLEKS R16 R15 K52 ["saveGizmos"]
  DUPCLOSURE R16 K53 [PROTO_9]
  CAPTURE VAL R7
  CAPTURE VAL R6
  SETTABLEKS R16 R15 K54 ["activate"]
  DUPCLOSURE R16 K55 [PROTO_10]
  CAPTURE VAL R7
  CAPTURE VAL R6
  SETTABLEKS R16 R15 K56 ["deactivate"]
  DUPCLOSURE R16 K57 [PROTO_11]
  CAPTURE VAL R4
  SETTABLEKS R16 R15 K58 ["createStamp"]
  RETURN R15 1

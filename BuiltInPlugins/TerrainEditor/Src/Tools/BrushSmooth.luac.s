PROTO_0:
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["init"]
  MOVE R4 R0
  MOVE R5 R1
  MOVE R6 R2
  CALL R3 3 0
  LOADN R3 0
  SETTABLEKS R3 R0 K1 ["_brushTime"]
  LOADN R3 0
  SETTABLEKS R3 R0 K2 ["_brushUsages"]
  RETURN R0 0

PROTO_1:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["saveForm"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 0
  GETTABLEKS R2 R0 K1 ["_operation"]
  JUMPIFNOT R2 [+8]
  GETTABLEKS R2 R0 K1 ["_operation"]
  NAMECALL R4 R0 K2 ["getPayload"]
  CALL R4 1 -1
  NAMECALL R2 R2 K3 ["updatePayload"]
  CALL R2 -1 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["saveGizmos"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 0
  GETTABLEKS R2 R0 K1 ["_operation"]
  JUMPIFNOT R2 [+8]
  GETTABLEKS R2 R0 K1 ["_operation"]
  NAMECALL R4 R0 K2 ["getPayload"]
  CALL R4 1 -1
  NAMECALL R2 R2 K3 ["updatePayload"]
  CALL R2 -1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R2 0
  LOADK R3 K0 ["Smooth"]
  GETTABLEKS R4 R0 K1 ["_services"]
  CALL R2 2 1
  SETTABLEKS R2 R0 K2 ["_operation"]
  GETTABLEKS R2 R0 K2 ["_operation"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K3 ["join"]
  DUPTABLE R5 K5 [{"Payload"}]
  NAMECALL R6 R0 K6 ["getPayload"]
  CALL R6 1 1
  SETTABLEKS R6 R5 K4 ["Payload"]
  MOVE R6 R1
  CALL R4 2 -1
  NAMECALL R2 R2 K7 ["start"]
  CALL R2 -1 0
  RETURN R0 0

PROTO_4:
  GETUPVAL R0 0
  NAMECALL R0 R0 K0 ["startOperation"]
  CALL R0 1 0
  RETURN R0 0

PROTO_5:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_operation"]
  JUMPIFNOT R0 [+35]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_operation"]
  NAMECALL R0 R0 K1 ["isRunning"]
  CALL R0 1 1
  JUMPIFNOT R0 [+28]
  GETUPVAL R0 0
  GETTABLEKS R1 R0 K2 ["_brushUsages"]
  ADDK R1 R1 K3 [1]
  SETTABLEKS R1 R0 K2 ["_brushUsages"]
  GETUPVAL R0 0
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["_operation"]
  NAMECALL R2 R2 K4 ["getCurrentTimeTaken"]
  CALL R2 1 1
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["_operation"]
  NAMECALL R3 R3 K5 ["getStepsTaken"]
  CALL R3 1 1
  DIV R1 R2 R3
  SETTABLEKS R1 R0 K6 ["_brushTime"]
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_operation"]
  NAMECALL R0 R0 K7 ["destroy"]
  CALL R0 1 0
  RETURN R0 0

PROTO_6:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["activate"]
  MOVE R2 R0
  CALL R1 1 0
  GETTABLEKS R1 R0 K1 ["_shortcutController"]
  NAMECALL R1 R1 K2 ["getMouse"]
  CALL R1 1 1
  GETTABLEKS R3 R0 K1 ["_shortcutController"]
  GETTABLEKS R2 R3 K3 ["MouseDownAction"]
  NEWCLOSURE R4 P0
  CAPTURE VAL R0
  NAMECALL R2 R2 K4 ["Connect"]
  CALL R2 2 1
  SETTABLEKS R2 R0 K5 ["_mouseDownConnection"]
  GETTABLEKS R2 R1 K6 ["Button1Up"]
  NEWCLOSURE R4 P1
  CAPTURE VAL R0
  NAMECALL R2 R2 K4 ["Connect"]
  CALL R2 2 1
  SETTABLEKS R2 R0 K7 ["_mouseUpConnection"]
  GETTABLEKS R2 R0 K8 ["_analytics"]
  LOADK R4 K9 ["Activated"]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K10 ["Smooth"]
  NAMECALL R2 R2 K11 ["report"]
  CALL R2 3 0
  RETURN R0 0

PROTO_7:
  GETTABLEKS R1 R0 K0 ["_mouseDownConnection"]
  NAMECALL R1 R1 K1 ["Disconnect"]
  CALL R1 1 0
  GETTABLEKS R1 R0 K2 ["_mouseUpConnection"]
  NAMECALL R1 R1 K1 ["Disconnect"]
  CALL R1 1 0
  GETTABLEKS R1 R0 K3 ["_operation"]
  JUMPIFNOT R1 [+29]
  GETTABLEKS R1 R0 K3 ["_operation"]
  NAMECALL R1 R1 K4 ["isRunning"]
  CALL R1 1 1
  JUMPIFNOT R1 [+23]
  GETTABLEKS R1 R0 K5 ["_brushUsages"]
  ADDK R1 R1 K6 [1]
  SETTABLEKS R1 R0 K5 ["_brushUsages"]
  GETTABLEKS R2 R0 K3 ["_operation"]
  NAMECALL R2 R2 K7 ["getCurrentTimeTaken"]
  CALL R2 1 1
  GETTABLEKS R3 R0 K3 ["_operation"]
  NAMECALL R3 R3 K8 ["getStepsTaken"]
  CALL R3 1 1
  DIV R1 R2 R3
  SETTABLEKS R1 R0 K9 ["_brushTime"]
  GETTABLEKS R1 R0 K3 ["_operation"]
  NAMECALL R1 R1 K10 ["destroy"]
  CALL R1 1 0
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K11 ["deactivate"]
  MOVE R2 R0
  CALL R1 1 0
  RETURN R0 0

PROTO_8:
  NAMECALL R1 R0 K0 ["getPayload"]
  CALL R1 1 1
  GETTABLEKS R2 R0 K1 ["_brushTime"]
  JUMPIFNOT R2 [+27]
  GETTABLEKS R2 R0 K2 ["_analytics"]
  LOADK R4 K3 ["BrushProperties"]
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K4 ["getBrushPayload"]
  MOVE R6 R1
  GETTABLEKS R7 R0 K1 ["_brushTime"]
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K5 ["Smooth"]
  CALL R5 3 -1
  NAMECALL R2 R2 K6 ["report"]
  CALL R2 -1 0
  GETTABLEKS R2 R0 K2 ["_analytics"]
  LOADK R4 K7 ["Usage"]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K5 ["Smooth"]
  GETTABLEKS R6 R0 K8 ["_brushUsages"]
  NAMECALL R2 R2 K6 ["report"]
  CALL R2 4 0
  LOADN R2 0
  SETTABLEKS R2 R0 K8 ["_brushUsages"]
  LOADN R2 0
  SETTABLEKS R2 R0 K1 ["_brushTime"]
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["TerrainEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Dash"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETIMPORT R5 K1 [script]
  GETTABLEKS R4 R5 K8 ["Parent"]
  GETTABLEKS R3 R4 K9 ["BaseTool"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R6 R0 K10 ["Src"]
  GETTABLEKS R5 R6 K11 ["Flags"]
  GETTABLEKS R4 R5 K12 ["getFFlagTerrainEditorRemoveSchema"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R8 R0 K10 ["Src"]
  GETTABLEKS R7 R8 K13 ["Schemas"]
  GETTABLEKS R6 R7 K14 ["Settings"]
  GETTABLEKS R5 R6 K15 ["Brush"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K10 ["Src"]
  GETTABLEKS R7 R8 K16 ["Util"]
  GETTABLEKS R6 R7 K17 ["AnalyticsHelper"]
  CALL R5 1 1
  GETIMPORT R6 K5 [require]
  GETTABLEKS R10 R0 K10 ["Src"]
  GETTABLEKS R9 R10 K16 ["Util"]
  GETTABLEKS R8 R9 K18 ["Operations"]
  GETTABLEKS R7 R8 K19 ["SmoothOperation"]
  CALL R6 1 1
  GETIMPORT R7 K5 [require]
  GETTABLEKS R9 R0 K10 ["Src"]
  GETTABLEKS R8 R9 K20 ["Types"]
  CALL R7 1 1
  GETTABLEKS R8 R7 K21 ["BrushSettings"]
  GETTABLEKS R9 R7 K22 ["BrushShape"]
  GETTABLEKS R10 R7 K23 ["Category"]
  GETTABLEKS R11 R7 K24 ["Gizmo"]
  GETTABLEKS R12 R7 K25 ["PivotPosition"]
  GETTABLEKS R13 R7 K26 ["Snapping"]
  GETTABLEKS R14 R7 K27 ["Tab"]
  GETTABLEKS R15 R7 K28 ["Tool"]
  NEWTABLE R16 0 1
  DUPTABLE R17 K32 [{"Defaults", "Id", "Schema"}]
  NEWTABLE R18 8 0
  GETTABLEKS R19 R8 K22 ["BrushShape"]
  GETTABLEKS R20 R9 K33 ["Sphere"]
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K34 ["BrushSize"]
  DUPTABLE R20 K38 [{"Height", "Locked", "Size"}]
  LOADN R21 6
  SETTABLEKS R21 R20 K35 ["Height"]
  LOADB R21 1
  SETTABLEKS R21 R20 K36 ["Locked"]
  LOADN R21 6
  SETTABLEKS R21 R20 K37 ["Size"]
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K39 ["Strength"]
  LOADN R20 1
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K25 ["PivotPosition"]
  GETTABLEKS R20 R12 K40 ["Center"]
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K26 ["Snapping"]
  GETTABLEKS R20 R13 K41 ["Off"]
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K42 ["State"]
  DUPTABLE R20 K44 [{"Position"}]
  LOADN R22 0
  LOADN R23 0
  LOADN R24 0
  FASTCALL VECTOR [+2]
  GETIMPORT R21 K47 [Vector3.new]
  CALL R21 3 1
  SETTABLEKS R21 R20 K43 ["Position"]
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K48 ["IgnoreWater"]
  LOADB R20 0
  SETTABLE R20 R18 R19
  GETTABLEKS R19 R8 K49 ["IgnoreParts"]
  LOADB R20 1
  SETTABLE R20 R18 R19
  SETTABLEKS R18 R17 K29 ["Defaults"]
  GETTABLEKS R18 R10 K21 ["BrushSettings"]
  SETTABLEKS R18 R17 K30 ["Id"]
  MOVE R19 R3
  CALL R19 0 1
  JUMPIFNOT R19 [+2]
  LOADNIL R18
  JUMP [+1]
  MOVE R18 R4
  SETTABLEKS R18 R17 K31 ["Schema"]
  SETLIST R16 R17 1 [1]
  NEWTABLE R17 0 2
  DUPTABLE R18 K50 [{"Id", "Schema"}]
  GETTABLEKS R19 R11 K15 ["Brush"]
  SETTABLEKS R19 R18 K30 ["Id"]
  DUPTABLE R19 K52 [{"Type"}]
  GETTABLEKS R20 R11 K15 ["Brush"]
  SETTABLEKS R20 R19 K51 ["Type"]
  SETTABLEKS R19 R18 K31 ["Schema"]
  DUPTABLE R19 K50 [{"Id", "Schema"}]
  GETTABLEKS R20 R11 K53 ["Plane"]
  SETTABLEKS R20 R19 K30 ["Id"]
  DUPTABLE R20 K52 [{"Type"}]
  GETTABLEKS R21 R11 K53 ["Plane"]
  SETTABLEKS R21 R20 K51 ["Type"]
  SETTABLEKS R20 R19 K31 ["Schema"]
  SETLIST R17 R18 2 [1]
  GETTABLEKS R20 R15 K54 ["Smooth"]
  GETTABLEKS R21 R14 K55 ["Edit"]
  MOVE R22 R16
  MOVE R23 R17
  NAMECALL R18 R2 K46 ["new"]
  CALL R18 5 1
  DUPCLOSURE R19 K56 [PROTO_0]
  CAPTURE VAL R2
  SETTABLEKS R19 R18 K57 ["init"]
  DUPCLOSURE R19 K58 [PROTO_1]
  CAPTURE VAL R2
  SETTABLEKS R19 R18 K59 ["saveForm"]
  DUPCLOSURE R19 K60 [PROTO_2]
  CAPTURE VAL R2
  SETTABLEKS R19 R18 K61 ["saveGizmos"]
  DUPCLOSURE R19 K62 [PROTO_3]
  CAPTURE VAL R6
  CAPTURE VAL R1
  SETTABLEKS R19 R18 K63 ["startOperation"]
  DUPCLOSURE R19 K64 [PROTO_6]
  CAPTURE VAL R2
  CAPTURE VAL R15
  SETTABLEKS R19 R18 K65 ["activate"]
  DUPCLOSURE R19 K66 [PROTO_7]
  CAPTURE VAL R2
  SETTABLEKS R19 R18 K67 ["deactivate"]
  DUPCLOSURE R19 K68 [PROTO_8]
  CAPTURE VAL R5
  CAPTURE VAL R15
  SETTABLEKS R19 R18 K69 ["reportAnalytics"]
  RETURN R18 1

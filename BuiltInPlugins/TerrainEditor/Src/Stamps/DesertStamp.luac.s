PROTO_0:
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K0 ["init"]
  MOVE R4 R0
  MOVE R5 R1
  MOVE R6 R2
  CALL R3 3 0
  RETURN R0 1

PROTO_1:
  GETTABLEKS R3 R0 K0 ["_payload"]
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K1 ["BuildSettings"]
  GETTABLE R2 R3 R4
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K2 ["NoiseScale"]
  GETTABLE R1 R2 R3
  GETTABLEKS R6 R0 K0 ["_payload"]
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K1 ["BuildSettings"]
  GETTABLE R5 R6 R7
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K4 ["AdvancedNoise"]
  GETTABLE R4 R5 R6
  GETTABLEKS R3 R4 K3 ["Children"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K5 ["Offset"]
  GETTABLE R2 R3 R4
  GETTABLEKS R7 R0 K0 ["_payload"]
  GETUPVAL R9 0
  GETTABLEKS R8 R9 K1 ["BuildSettings"]
  GETTABLE R6 R7 R8
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K4 ["AdvancedNoise"]
  GETTABLE R5 R6 R7
  GETTABLEKS R4 R5 K3 ["Children"]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K6 ["Seed"]
  GETTABLE R3 R4 R5
  GETIMPORT R4 K9 [table.create]
  GETTABLEKS R5 R0 K10 ["_heightMapSize"]
  LOADN R6 0
  CALL R4 2 1
  SETTABLEKS R4 R0 K11 ["_noiseMap"]
  LOADN R4 0
  JUMPIFNOTLT R4 R1 [+52]
  LOADN R5 50
  DIV R4 R5 R1
  LOADN R7 1
  GETTABLEKS R5 R0 K12 ["_sliceX"]
  LOADN R6 1
  FORNPREP R5
  LOADN R10 1
  GETTABLEKS R8 R0 K13 ["_sliceZ"]
  LOADN R9 1
  FORNPREP R8
  MOVE R13 R7
  MOVE R14 R10
  NAMECALL R11 R0 K14 ["getIndex"]
  CALL R11 3 1
  GETTABLEKS R12 R0 K15 ["_getRotatedPoint"]
  SUBK R13 R7 K16 [0.5]
  SUBK R14 R10 K16 [0.5]
  CALL R12 2 2
  GETUPVAL R16 2
  MUL R15 R12 R16
  GETTABLEKS R16 R2 K17 ["X"]
  ADD R14 R15 R16
  GETUPVAL R17 2
  MUL R16 R13 R17
  GETTABLEKS R17 R2 K18 ["Y"]
  ADD R15 R16 R17
  GETUPVAL R16 3
  MOVE R17 R14
  MOVE R18 R15
  MOVE R19 R3
  MOVE R20 R4
  CALL R16 4 1
  MULK R17 R16 K16 [0.5]
  ADDK R16 R17 K16 [0.5]
  GETUPVAL R18 4
  GETTABLEKS R17 R18 K20 ["Ridge"]
  MOVE R18 R16
  CALL R17 1 1
  DIVK R16 R17 K19 [2]
  GETTABLEKS R17 R0 K11 ["_noiseMap"]
  SETTABLE R16 R17 R11
  FORNLOOP R8
  FORNLOOP R5
  RETURN R0 0

PROTO_2:
  GETTABLEKS R3 R0 K0 ["_payload"]
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K1 ["BuildSettings"]
  GETTABLE R2 R3 R4
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K2 ["NoiseStrength"]
  GETTABLE R1 R2 R3
  GETTABLEKS R3 R0 K3 ["_sliceY"]
  LOADN R5 1
  SUB R4 R5 R1
  MUL R2 R3 R4
  GETIMPORT R3 K6 [table.create]
  GETTABLEKS R4 R0 K7 ["_heightMapSize"]
  LOADN R5 0
  CALL R3 2 1
  SETTABLEKS R3 R0 K8 ["_heightMap"]
  LOADN R5 1
  GETTABLEKS R3 R0 K7 ["_heightMapSize"]
  LOADN R4 1
  FORNPREP R3
  GETTABLEKS R7 R0 K9 ["_noiseMap"]
  GETTABLE R6 R7 R5
  GETTABLEKS R9 R0 K3 ["_sliceY"]
  MUL R8 R9 R1
  MUL R7 R8 R6
  ADD R8 R2 R7
  GETTABLEKS R9 R0 K8 ["_heightMap"]
  MOVE R11 R8
  LOADN R12 1
  GETTABLEKS R13 R0 K3 ["_sliceY"]
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R10 K12 [math.clamp]
  CALL R10 3 1
  SETTABLE R10 R9 R5
  FORNLOOP R3
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["TerrainEditor"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETIMPORT R4 K1 [script]
  GETTABLEKS R3 R4 K6 ["Parent"]
  GETTABLEKS R2 R3 K7 ["BaseStamp"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R5 R0 K8 ["Src"]
  GETTABLEKS R4 R5 K9 ["Resources"]
  GETTABLEKS R3 R4 K10 ["Constants"]
  CALL R2 1 1
  GETTABLEKS R3 R2 K11 ["VoxelResolution"]
  GETIMPORT R4 K5 [require]
  GETTABLEKS R6 R0 K8 ["Src"]
  GETTABLEKS R5 R6 K12 ["Types"]
  CALL R4 1 1
  GETTABLEKS R5 R4 K13 ["BuildSettings"]
  GETTABLEKS R6 R4 K14 ["Category"]
  GETTABLEKS R7 R4 K15 ["TerrainType"]
  GETTABLEKS R10 R0 K8 ["Src"]
  GETTABLEKS R9 R10 K16 ["Util"]
  GETTABLEKS R8 R9 K17 ["Generation"]
  GETIMPORT R9 K5 [require]
  GETTABLEKS R10 R8 K18 ["FractalNoise"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R11 R8 K19 ["Filter"]
  CALL R10 1 1
  MOVE R11 R9
  LOADN R12 1
  LOADN R13 1
  LOADN R14 1
  CALL R11 3 1
  GETTABLEKS R14 R7 K20 ["Desert"]
  NAMECALL R12 R1 K21 ["new"]
  CALL R12 2 1
  DUPCLOSURE R13 K22 [PROTO_0]
  CAPTURE VAL R1
  SETTABLEKS R13 R12 K23 ["init"]
  DUPCLOSURE R13 K24 [PROTO_1]
  CAPTURE VAL R6
  CAPTURE VAL R5
  CAPTURE VAL R3
  CAPTURE VAL R11
  CAPTURE VAL R10
  SETTABLEKS R13 R12 K25 ["generateNoiseMap"]
  DUPCLOSURE R13 K26 [PROTO_2]
  CAPTURE VAL R6
  CAPTURE VAL R5
  SETTABLEKS R13 R12 K27 ["generateHeightMap"]
  RETURN R12 1

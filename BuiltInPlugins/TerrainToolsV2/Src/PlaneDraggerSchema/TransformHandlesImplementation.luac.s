PROTO_0:
  DUPTABLE R2 K2 [{"_draggerContext", "_updatePlaneFn"}]
  SETTABLEKS R0 R2 K0 ["_draggerContext"]
  GETTABLEKS R3 R0 K3 ["updatePlaneFn"]
  SETTABLEKS R3 R2 K1 ["_updatePlaneFn"]
  GETUPVAL R3 0
  FASTCALL2 SETMETATABLE R2 R3 [+3]
  GETIMPORT R1 K5 [setmetatable]
  CALL R1 2 1
  RETURN R1 1

PROTO_1:
  LENGTH R5 R1
  JUMPIFEQKN R5 K0 [1] [+2]
  LOADB R4 0 +1
  LOADB R4 1
  FASTCALL2K ASSERT R4 K1 [+4]
  LOADK R5 K1 ["Expected selection to have length 1."]
  GETIMPORT R3 K3 [assert]
  CALL R3 2 0
  GETTABLEN R4 R1 1
  GETTABLEKS R3 R4 K4 ["planeCFrame"]
  SETTABLEKS R3 R0 K5 ["initCFrame"]
  RETURN R0 0

PROTO_2:
  GETTABLEKS R2 R0 K0 ["_updatePlaneFn"]
  GETTABLEKS R4 R0 K1 ["initCFrame"]
  MUL R3 R1 R4
  CALL R2 1 0
  RETURN R1 1

PROTO_3:
  RETURN R0 0

PROTO_4:
  LOADNIL R2
  RETURN R2 1

MAIN:
  PREPVARARGS 0
  NEWTABLE R0 8 0
  SETTABLEKS R0 R0 K0 ["__index"]
  DUPCLOSURE R1 K1 [PROTO_0]
  CAPTURE VAL R0
  SETTABLEKS R1 R0 K2 ["new"]
  DUPCLOSURE R1 K3 [PROTO_1]
  SETTABLEKS R1 R0 K4 ["beginDrag"]
  DUPCLOSURE R1 K5 [PROTO_2]
  SETTABLEKS R1 R0 K6 ["updateDrag"]
  DUPCLOSURE R1 K7 [PROTO_3]
  SETTABLEKS R1 R0 K8 ["endDrag"]
  DUPCLOSURE R1 K9 [PROTO_4]
  SETTABLEKS R1 R0 K10 ["render"]
  RETURN R0 1

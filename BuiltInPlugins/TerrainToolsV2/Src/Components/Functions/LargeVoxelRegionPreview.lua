...
local TILE_DIM = 512
local TILE_TO_STUD = Constants.VOXEL_RESOLUTION * TILE_DIM
local TILE_SIZE = Vector3.new(TILE_DIM,TILE_DIM,TILE_DIM) * Constants.VOXEL_RESOLUTION
...
function LargeVoxelRegionPreview.new(mouse)
	local self = {}
...
local function deepTableFunc(table, callback, depth)
	if depth then
		assert( depth > 10 , "this is a very nested table")
	end

	for i,v in pairs(table) do
		if type(v) == "table" then
			deepTableFunc(v, callback, depth and depth + 1 or 1)
		else
			callback(v)
		end
	end
...
function LargeVoxelRegionPreview:updateBlockMesh(isVisible)
	...
	self._rootPart.Parent = workspace.Terrain

PROTO_0:
  GETUPVAL R2 0
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 0
  GETIMPORT R2 K2 [task.delay]
  LOADK R3 K3 [0.05]
  GETUPVAL R4 1
  CALL R2 2 0
  RETURN R0 0

PROTO_1:
  GETUPVAL R1 0
  NAMECALL R1 R1 K0 ["use"]
  CALL R1 1 1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["useContext"]
  GETUPVAL R3 2
  CALL R2 1 1
  GETUPVAL R3 3
  GETUPVAL R5 4
  CALL R5 0 1
  JUMPIFNOT R5 [+5]
  DUPTABLE R4 K3 [{"layerType"}]
  LOADK R5 K4 ["layered"]
  SETTABLEKS R5 R4 K2 ["layerType"]
  JUMP [+1]
  LOADNIL R4
  CALL R3 1 1
  GETUPVAL R4 3
  DUPTABLE R5 K3 [{"layerType"}]
  LOADK R6 K5 ["makeup"]
  SETTABLEKS R6 R5 K2 ["layerType"]
  CALL R4 1 1
  LOADNIL R5
  GETUPVAL R6 5
  CALL R6 0 1
  JUMPIF R6 [+18]
  GETUPVAL R6 6
  GETTABLEKS R7 R0 K6 ["Dummy"]
  CALL R6 1 1
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K7 ["useCallback"]
  NEWCLOSURE R8 P0
  CAPTURE VAL R4
  CAPTURE VAL R6
  NEWTABLE R9 0 2
  MOVE R10 R4
  MOVE R11 R6
  SETLIST R9 R10 2 [1]
  CALL R7 2 1
  MOVE R5 R7
  GETTABLEKS R6 R0 K6 ["Dummy"]
  JUMPIFNOTEQKNIL R6 [+3]
  LOADNIL R6
  RETURN R6 1
  GETTABLEKS R8 R0 K6 ["Dummy"]
  JUMPIFNOTEQKNIL R8 [+2]
  LOADB R7 0 +1
  LOADB R7 1
  GETUPVAL R9 7
  GETTABLEKS R8 R9 K8 ["LUAU_ANALYZE_ERROR"]
  FASTCALL2 ASSERT R7 R8 [+3]
  GETIMPORT R6 K10 [assert]
  CALL R6 2 0
  GETTABLEKS R7 R0 K6 ["Dummy"]
  GETTABLEKS R6 R7 K11 ["Head"]
  LOADK R8 K12 ["FaceControls"]
  NAMECALL R6 R6 K13 ["FindFirstChildWhichIsA"]
  CALL R6 2 1
  JUMPIFNOTEQKNIL R6 [+22]
  GETTABLEKS R8 R0 K14 ["SelectedTab"]
  JUMPIFNOTEQKS R8 K15 ["animations_face"] [+18]
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K16 ["createElement"]
  GETUPVAL R8 8
  DUPTABLE R9 K19 [{"key", "Text"}]
  LOADK R10 K20 ["Notice"]
  SETTABLEKS R10 R9 K17 ["key"]
  LOADK R12 K21 ["CheckFace"]
  LOADK R13 K22 ["DynamicHeadNotice"]
  NAMECALL R10 R1 K23 ["getText"]
  CALL R10 3 1
  SETTABLEKS R10 R9 K18 ["Text"]
  CALL R7 2 1
  JUMP [+27]
  GETTABLEKS R9 R2 K24 ["avatarAssets"]
  GETTABLEKS R8 R9 K25 ["emotion"]
  JUMPIFEQKNIL R8 [+21]
  GETUPVAL R8 1
  GETTABLEKS R7 R8 K16 ["createElement"]
  GETUPVAL R8 9
  DUPTABLE R9 K27 [{"key", "Dummy", "AnimationId"}]
  LOADK R10 K28 ["AnimationController"]
  SETTABLEKS R10 R9 K17 ["key"]
  GETTABLEKS R10 R0 K6 ["Dummy"]
  SETTABLEKS R10 R9 K6 ["Dummy"]
  GETTABLEKS R11 R2 K24 ["avatarAssets"]
  GETTABLEKS R10 R11 K25 ["emotion"]
  SETTABLEKS R10 R9 K26 ["AnimationId"]
  CALL R7 2 1
  JUMP [+1]
  LOADNIL R7
  GETUPVAL R9 1
  GETTABLEKS R8 R9 K16 ["createElement"]
  GETUPVAL R10 1
  GETTABLEKS R9 R10 K29 ["Fragment"]
  NEWTABLE R10 0 0
  NEWTABLE R11 1 1
  GETUPVAL R14 1
  GETTABLEKS R13 R14 K16 ["createElement"]
  GETUPVAL R14 10
  DUPTABLE R15 K32 [{"OnClothingLayerOrderChanged", "OnMakeupLayerOrderChanged"}]
  SETTABLEKS R3 R15 K30 ["OnClothingLayerOrderChanged"]
  GETUPVAL R17 5
  CALL R17 0 1
  JUMPIFNOT R17 [+2]
  MOVE R16 R4
  JUMP [+6]
  GETUPVAL R17 4
  CALL R17 0 1
  JUMPIFNOT R17 [+2]
  MOVE R16 R5
  JUMP [+1]
  LOADNIL R16
  SETTABLEKS R16 R15 K31 ["OnMakeupLayerOrderChanged"]
  CALL R13 2 1
  SETTABLEKS R13 R11 K33 ["EquippedItemsSidebar"]
  MOVE R12 R7
  SETLIST R11 R12 1 [1]
  CALL R8 3 -1
  RETURN R8 -1

PROTO_2:
  GETTABLEKS R1 R0 K0 ["Head"]
  DUPTABLE R2 K4 [{"focus", "cframe", "fov"}]
  GETIMPORT R3 K7 [CFrame.new]
  GETTABLEKS R4 R1 K8 ["Position"]
  CALL R3 1 1
  SETTABLEKS R3 R2 K1 ["focus"]
  GETIMPORT R3 K10 [CFrame.lookAt]
  GETTABLEKS R6 R1 K8 ["Position"]
  GETTABLEKS R10 R1 K12 ["Size"]
  GETTABLEKS R9 R10 K13 ["Magnitude"]
  GETTABLEKS R11 R1 K5 ["CFrame"]
  GETTABLEKS R10 R11 K14 ["LookVector"]
  MUL R8 R9 R10
  MULK R7 R8 K11 [2]
  ADD R5 R6 R7
  GETTABLEKS R9 R1 K12 ["Size"]
  GETTABLEKS R8 R9 K13 ["Magnitude"]
  GETTABLEKS R10 R1 K5 ["CFrame"]
  GETTABLEKS R9 R10 K16 ["RightVector"]
  MUL R7 R8 R9
  MULK R6 R7 K15 [1]
  SUB R4 R5 R6
  GETTABLEKS R5 R1 K8 ["Position"]
  CALL R3 2 1
  SETTABLEKS R3 R2 K2 ["cframe"]
  LOADN R3 35
  SETTABLEKS R3 R2 K3 ["fov"]
  RETURN R2 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AvatarCompatibilityPreviewer"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Framework"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K6 ["Packages"]
  GETTABLEKS R3 R4 K8 ["React"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R6 R0 K9 ["Src"]
  GETTABLEKS R5 R6 K10 ["Components"]
  GETTABLEKS R4 R5 K11 ["AnimationController"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R7 R0 K9 ["Src"]
  GETTABLEKS R6 R7 K10 ["Components"]
  GETTABLEKS R5 R6 K12 ["EquippedItemsSidebar"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K9 ["Src"]
  GETTABLEKS R7 R8 K10 ["Components"]
  GETTABLEKS R6 R7 K13 ["Notice"]
  CALL R5 1 1
  GETIMPORT R6 K5 [require]
  GETTABLEKS R9 R0 K9 ["Src"]
  GETTABLEKS R8 R9 K14 ["Hooks"]
  GETTABLEKS R7 R8 K15 ["useEquipAvatarMakeup_DEPRECATED"]
  CALL R6 1 1
  GETIMPORT R7 K5 [require]
  GETTABLEKS R10 R0 K9 ["Src"]
  GETTABLEKS R9 R10 K14 ["Hooks"]
  GETTABLEKS R8 R9 K16 ["useOnClothingLayerOrderChanged"]
  CALL R7 1 1
  GETIMPORT R8 K5 [require]
  GETTABLEKS R11 R0 K9 ["Src"]
  GETTABLEKS R10 R11 K17 ["Flags"]
  GETTABLEKS R9 R10 K18 ["getFFlagAvatarPreviewerMakeup"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETTABLEKS R12 R0 K9 ["Src"]
  GETTABLEKS R11 R12 K17 ["Flags"]
  GETTABLEKS R10 R11 K19 ["getFFlagAvatarPreviewerMakeupDescription"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R13 R0 K9 ["Src"]
  GETTABLEKS R12 R13 K20 ["Util"]
  GETTABLEKS R11 R12 K21 ["Constants"]
  CALL R10 1 1
  GETIMPORT R11 K5 [require]
  GETTABLEKS R14 R0 K9 ["Src"]
  GETTABLEKS R13 R14 K20 ["Util"]
  GETTABLEKS R12 R13 K22 ["EquipmentStateContext"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETIMPORT R15 K1 [script]
  GETTABLEKS R14 R15 K23 ["Parent"]
  GETTABLEKS R13 R14 K24 ["StageType"]
  CALL R12 1 1
  GETTABLEKS R14 R1 K25 ["ContextServices"]
  GETTABLEKS R13 R14 K26 ["Localization"]
  DUPTABLE R14 K30 [{"tabs", "render", "getPreviewCameraModifications"}]
  MOVE R16 R8
  CALL R16 0 1
  JUMPIFNOT R16 [+8]
  NEWTABLE R15 0 3
  LOADK R16 K31 ["animations_face"]
  LOADK R17 K32 ["accessories_face"]
  LOADK R18 K33 ["makeup"]
  SETLIST R15 R16 3 [1]
  JUMP [+6]
  NEWTABLE R15 0 2
  LOADK R16 K31 ["animations_face"]
  LOADK R17 K32 ["accessories_face"]
  SETLIST R15 R16 2 [1]
  SETTABLEKS R15 R14 K27 ["tabs"]
  DUPCLOSURE R15 K34 [PROTO_1]
  CAPTURE VAL R13
  CAPTURE VAL R2
  CAPTURE VAL R11
  CAPTURE VAL R7
  CAPTURE VAL R8
  CAPTURE VAL R9
  CAPTURE VAL R6
  CAPTURE VAL R10
  CAPTURE VAL R5
  CAPTURE VAL R3
  CAPTURE VAL R4
  SETTABLEKS R15 R14 K28 ["render"]
  DUPCLOSURE R15 K35 [PROTO_2]
  SETTABLEKS R15 R14 K29 ["getPreviewCameraModifications"]
  RETURN R14 1

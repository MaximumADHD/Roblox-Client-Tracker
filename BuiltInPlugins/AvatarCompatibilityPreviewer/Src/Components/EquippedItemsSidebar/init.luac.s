PROTO_0:
  GETIMPORT R1 K2 [table.find]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K3 ["clothing"]
  GETTABLEKS R3 R0 K4 ["palette"]
  CALL R1 2 1
  JUMPIFEQKNIL R1 [+3]
  LOADB R1 1
  RETURN R1 1
  GETTABLEKS R2 R0 K5 ["item"]
  GETTABLEKS R1 R2 K6 ["source"]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K7 ["SOURCE_INSTANCE"]
  JUMPIFEQ R1 R2 [+3]
  LOADB R1 0
  RETURN R1 1
  GETTABLEKS R4 R0 K5 ["item"]
  GETTABLEKS R3 R4 K6 ["source"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K7 ["SOURCE_INSTANCE"]
  JUMPIFEQ R3 R4 [+2]
  LOADB R2 0 +1
  LOADB R2 1
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K8 ["LUAU_ANALYZE_ERROR"]
  FASTCALL2 ASSERT R2 R3 [+3]
  GETIMPORT R1 K10 [assert]
  CALL R1 2 0
  GETTABLEKS R3 R0 K5 ["item"]
  GETTABLEKS R2 R3 K11 ["instance"]
  LOADK R4 K12 ["WrapLayer"]
  LOADB R5 1
  NAMECALL R2 R2 K13 ["FindFirstChildWhichIsA"]
  CALL R2 3 1
  JUMPIFNOTEQKNIL R2 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_1:
  GETUPVAL R2 0
  LOADK R4 K0 ["changedLayers"]
  NAMECALL R2 R2 K1 ["report"]
  CALL R2 2 0
  GETUPVAL R2 1
  CALL R2 0 1
  JUMPIFNOT R2 [+13]
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K2 ["OnClothingLayerOrderChanged"]
  GETUPVAL R6 3
  LENGTH R5 R6
  SUB R4 R5 R0
  ADDK R3 R4 K3 [1]
  GETUPVAL R7 3
  LENGTH R6 R7
  SUB R5 R6 R1
  ADDK R4 R5 K3 [1]
  CALL R2 2 0
  RETURN R0 0
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K2 ["OnClothingLayerOrderChanged"]
  MOVE R4 R0
  LOADN R5 1
  GETUPVAL R7 3
  LENGTH R6 R7
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R3 K6 [math.clamp]
  CALL R3 3 1
  MOVE R5 R1
  LOADN R6 1
  GETUPVAL R8 3
  LENGTH R7 R8
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R4 K6 [math.clamp]
  CALL R4 3 1
  CALL R2 2 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R1 0
  GETTABLEKS R2 R0 K0 ["AbsoluteSize"]
  CALL R1 1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["createElement"]
  GETUPVAL R3 1
  DUPTABLE R4 K4 [{"ItemData", "Size", "IsDragging"}]
  SETTABLEKS R0 R4 K1 ["ItemData"]
  GETIMPORT R5 K7 [UDim2.fromScale]
  LOADN R6 1
  LOADN R7 1
  CALL R5 2 1
  SETTABLEKS R5 R4 K2 ["Size"]
  SETTABLEKS R1 R4 K3 ["IsDragging"]
  CALL R2 2 1
  GETUPVAL R4 2
  CALL R4 0 1
  JUMPIFNOT R4 [+16]
  GETTABLEKS R8 R0 K8 ["palette"]
  FASTCALL1 TOSTRING R8 [+2]
  GETIMPORT R7 K10 [tostring]
  CALL R7 1 1
  MOVE R4 R7
  LOADK R5 K11 ["/"]
  GETTABLEKS R7 R0 K12 ["item"]
  FASTCALL1 TOSTRING R7 [+2]
  GETIMPORT R6 K10 [tostring]
  CALL R6 1 1
  CONCAT R3 R4 R6
  RETURN R2 2
  FASTCALL1 TOSTRING R0 [+3]
  MOVE R4 R0
  GETIMPORT R3 K10 [tostring]
  CALL R3 1 1
  RETURN R2 2

PROTO_4:
  DUPTABLE R1 K1 [{"ItemData"}]
  SETTABLEKS R0 R1 K0 ["ItemData"]
  RETURN R1 1

PROTO_5:
  GETUPVAL R1 0
  NAMECALL R1 R1 K0 ["use"]
  CALL R1 1 1
  GETUPVAL R2 1
  LOADK R4 K1 ["EquippedItemsSidebar"]
  NAMECALL R2 R2 K0 ["use"]
  CALL R2 2 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K2 ["useContext"]
  GETUPVAL R4 3
  CALL R3 1 1
  NEWTABLE R4 0 0
  NEWTABLE R5 0 0
  GETUPVAL R7 4
  GETTABLEKS R6 R7 K3 ["new"]
  CALL R6 0 1
  GETTABLEKS R7 R3 K4 ["equippedItems"]
  LOADNIL R8
  LOADNIL R9
  FORGPREP R7
  GETUPVAL R13 5
  CALL R13 0 1
  JUMPIFNOT R13 [+8]
  GETUPVAL R13 6
  MOVE R14 R11
  CALL R13 1 1
  JUMPIFEQKS R13 K5 ["layered"] [+2]
  LOADB R12 0 +1
  LOADB R12 1
  JUMP [+10]
  GETUPVAL R13 7
  CALL R13 0 1
  JUMPIFNOT R13 [+4]
  GETUPVAL R12 8
  MOVE R13 R11
  CALL R12 1 1
  JUMP [+3]
  GETUPVAL R12 9
  MOVE R13 R11
  CALL R12 1 1
  JUMPIFNOT R12 [+8]
  MOVE R13 R4
  LOADN R14 1
  MOVE R15 R11
  FASTCALL TABLE_INSERT [+2]
  GETIMPORT R12 K8 [table.insert]
  CALL R12 3 0
  JUMP [+22]
  MOVE R13 R5
  GETUPVAL R15 2
  GETTABLEKS R14 R15 K9 ["createElement"]
  GETUPVAL R15 10
  DUPTABLE R16 K13 [{"ItemData", "LayoutOrder", "Size"}]
  SETTABLEKS R11 R16 K10 ["ItemData"]
  NAMECALL R17 R6 K14 ["getNextOrder"]
  CALL R17 1 1
  SETTABLEKS R17 R16 K11 ["LayoutOrder"]
  GETTABLEKS R17 R2 K15 ["CellSize"]
  SETTABLEKS R17 R16 K12 ["Size"]
  CALL R14 2 -1
  FASTCALL TABLE_INSERT [+2]
  GETIMPORT R12 K8 [table.insert]
  CALL R12 -1 0
  FORGLOOP R7 2 [-53]
  GETIMPORT R7 K18 [UDim2.fromOffset]
  GETTABLEKS R10 R2 K15 ["CellSize"]
  GETTABLEKS R9 R10 K19 ["X"]
  GETTABLEKS R8 R9 K20 ["Offset"]
  GETTABLEKS R12 R2 K15 ["CellSize"]
  GETTABLEKS R11 R12 K21 ["Y"]
  GETTABLEKS R10 R11 K20 ["Offset"]
  LENGTH R11 R4
  MUL R9 R10 R11
  CALL R7 2 1
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K22 ["useState"]
  GETIMPORT R9 K24 [Vector2.new]
  CALL R9 0 -1
  CALL R8 -1 2
  GETUPVAL R11 2
  GETTABLEKS R10 R11 K25 ["useCallback"]
  NEWCLOSURE R11 P0
  CAPTURE VAL R1
  CAPTURE UPVAL U7
  CAPTURE VAL R0
  CAPTURE VAL R4
  NEWTABLE R12 0 2
  GETTABLEKS R13 R0 K26 ["OnClothingLayerOrderChanged"]
  LENGTH R14 R4
  SETLIST R12 R13 2 [1]
  CALL R10 2 1
  GETUPVAL R12 2
  GETTABLEKS R11 R12 K9 ["createElement"]
  GETUPVAL R12 11
  DUPTABLE R13 K31 [{"AnchorPoint", "Position", "Size", "Layout", "Spacing"}]
  GETIMPORT R14 K24 [Vector2.new]
  LOADN R15 1
  LOADN R16 1
  CALL R14 2 1
  SETTABLEKS R14 R13 K27 ["AnchorPoint"]
  GETIMPORT R15 K33 [UDim2.fromScale]
  LOADN R16 1
  LOADN R17 1
  CALL R15 2 1
  GETTABLEKS R16 R2 K34 ["PositionOffset"]
  SUB R14 R15 R16
  SETTABLEKS R14 R13 K28 ["Position"]
  GETTABLEKS R14 R2 K12 ["Size"]
  SETTABLEKS R14 R13 K12 ["Size"]
  GETIMPORT R14 K38 [Enum.FillDirection.Vertical]
  SETTABLEKS R14 R13 K29 ["Layout"]
  GETTABLEKS R14 R2 K39 ["SidebarSpacing"]
  SETTABLEKS R14 R13 K30 ["Spacing"]
  DUPTABLE R14 K42 [{"Contents", "OtherEquipment"}]
  GETUPVAL R16 2
  GETTABLEKS R15 R16 K9 ["createElement"]
  GETUPVAL R16 11
  NEWTABLE R17 8 0
  GETIMPORT R18 K44 [Enum.AutomaticSize.Y]
  SETTABLEKS R18 R17 K43 ["AutomaticSize"]
  GETUPVAL R19 12
  CALL R19 0 1
  JUMPIFNOT R19 [+3]
  GETIMPORT R18 K47 [Enum.HorizontalAlignment.Right]
  JUMP [+1]
  LOADNIL R18
  SETTABLEKS R18 R17 K45 ["HorizontalAlignment"]
  GETIMPORT R18 K38 [Enum.FillDirection.Vertical]
  SETTABLEKS R18 R17 K29 ["Layout"]
  GETTABLEKS R18 R2 K39 ["SidebarSpacing"]
  SETTABLEKS R18 R17 K30 ["Spacing"]
  GETUPVAL R20 2
  GETTABLEKS R19 R20 K48 ["Change"]
  GETTABLEKS R18 R19 K49 ["AbsoluteSize"]
  NEWCLOSURE R19 P1
  CAPTURE VAL R9
  SETTABLE R19 R17 R18
  DUPTABLE R18 K52 [{"ResetCameraButton", "LayeredClothingEquipment"}]
  GETUPVAL R20 2
  GETTABLEKS R19 R20 K9 ["createElement"]
  GETUPVAL R20 13
  DUPTABLE R21 K53 [{"LayoutOrder"}]
  NAMECALL R22 R6 K14 ["getNextOrder"]
  CALL R22 1 1
  SETTABLEKS R22 R21 K11 ["LayoutOrder"]
  CALL R19 2 1
  SETTABLEKS R19 R18 K50 ["ResetCameraButton"]
  GETUPVAL R20 7
  CALL R20 0 1
  JUMPIFNOT R20 [+26]
  GETUPVAL R20 2
  GETTABLEKS R19 R20 K9 ["createElement"]
  GETUPVAL R20 14
  DUPTABLE R21 K59 [{"cellSize", "layoutOrder", "tiles", "render", "move"}]
  GETTABLEKS R22 R2 K15 ["CellSize"]
  SETTABLEKS R22 R21 K54 ["cellSize"]
  NAMECALL R22 R6 K14 ["getNextOrder"]
  CALL R22 1 1
  SETTABLEKS R22 R21 K55 ["layoutOrder"]
  SETTABLEKS R4 R21 K56 ["tiles"]
  DUPCLOSURE R22 K60 [PROTO_3]
  CAPTURE UPVAL U2
  CAPTURE UPVAL U10
  CAPTURE UPVAL U15
  SETTABLEKS R22 R21 K57 ["render"]
  SETTABLEKS R10 R21 K58 ["move"]
  CALL R19 2 1
  JUMP [+26]
  GETUPVAL R20 2
  GETTABLEKS R19 R20 K9 ["createElement"]
  GETUPVAL R20 16
  DUPTABLE R21 K66 [{"LayoutOrder", "TileComponent", "TileList", "Capacity", "Size", "GetTileProps", "OnOrderChanged"}]
  NAMECALL R22 R6 K14 ["getNextOrder"]
  CALL R22 1 1
  SETTABLEKS R22 R21 K11 ["LayoutOrder"]
  GETUPVAL R22 10
  SETTABLEKS R22 R21 K61 ["TileComponent"]
  SETTABLEKS R4 R21 K62 ["TileList"]
  LENGTH R22 R4
  SETTABLEKS R22 R21 K63 ["Capacity"]
  SETTABLEKS R7 R21 K12 ["Size"]
  DUPCLOSURE R22 K67 [PROTO_4]
  SETTABLEKS R22 R21 K64 ["GetTileProps"]
  SETTABLEKS R10 R21 K65 ["OnOrderChanged"]
  CALL R19 2 1
  SETTABLEKS R19 R18 K51 ["LayeredClothingEquipment"]
  CALL R15 3 1
  SETTABLEKS R15 R14 K40 ["Contents"]
  GETUPVAL R16 2
  GETTABLEKS R15 R16 K9 ["createElement"]
  GETUPVAL R16 17
  DUPTABLE R17 K71 [{"AutomaticSize", "AutoSizeCanvas", "AutomaticCanvasSize", "HorizontalAlignment", "LayoutOrder", "Layout", "Spacing", "Size", "ScrollingFrameSize"}]
  GETIMPORT R18 K72 [Enum.AutomaticSize.X]
  SETTABLEKS R18 R17 K43 ["AutomaticSize"]
  LOADB R18 0
  SETTABLEKS R18 R17 K68 ["AutoSizeCanvas"]
  GETIMPORT R18 K44 [Enum.AutomaticSize.Y]
  SETTABLEKS R18 R17 K69 ["AutomaticCanvasSize"]
  GETUPVAL R19 15
  CALL R19 0 1
  JUMPIFNOT R19 [+3]
  GETIMPORT R18 K47 [Enum.HorizontalAlignment.Right]
  JUMP [+1]
  LOADNIL R18
  SETTABLEKS R18 R17 K45 ["HorizontalAlignment"]
  NAMECALL R18 R6 K14 ["getNextOrder"]
  CALL R18 1 1
  SETTABLEKS R18 R17 K11 ["LayoutOrder"]
  GETIMPORT R18 K38 [Enum.FillDirection.Vertical]
  SETTABLEKS R18 R17 K29 ["Layout"]
  GETTABLEKS R18 R2 K73 ["ItemSpacing"]
  SETTABLEKS R18 R17 K30 ["Spacing"]
  GETUPVAL R19 15
  CALL R19 0 1
  JUMPIFNOT R19 [+18]
  GETIMPORT R18 K74 [UDim2.new]
  LOADN R19 0
  GETTABLEKS R23 R2 K15 ["CellSize"]
  GETTABLEKS R22 R23 K19 ["X"]
  GETTABLEKS R21 R22 K20 ["Offset"]
  GETTABLEKS R22 R2 K75 ["FrameSizeOffset"]
  ADD R20 R21 R22
  LOADN R21 1
  GETTABLEKS R23 R8 K21 ["Y"]
  MINUS R22 R23
  CALL R18 4 1
  JUMP [+14]
  GETIMPORT R18 K74 [UDim2.new]
  LOADN R19 0
  GETTABLEKS R22 R2 K15 ["CellSize"]
  GETTABLEKS R21 R22 K19 ["X"]
  GETTABLEKS R20 R21 K20 ["Offset"]
  LOADN R21 1
  GETTABLEKS R23 R8 K21 ["Y"]
  MINUS R22 R23
  CALL R18 4 1
  SETTABLEKS R18 R17 K12 ["Size"]
  GETUPVAL R19 15
  CALL R19 0 1
  JUMPIFNOT R19 [+9]
  GETIMPORT R18 K74 [UDim2.new]
  LOADN R19 0
  GETTABLEKS R20 R2 K76 ["ScrollingFrameSizeOffset"]
  LOADN R21 1
  LOADN R22 0
  CALL R18 4 1
  JUMP [+1]
  LOADNIL R18
  SETTABLEKS R18 R17 K70 ["ScrollingFrameSize"]
  MOVE R18 R5
  CALL R15 3 1
  SETTABLEKS R15 R14 K41 ["OtherEquipment"]
  CALL R11 3 -1
  RETURN R11 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AvatarCompatibilityPreviewer"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Framework"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K6 ["Packages"]
  GETTABLEKS R3 R4 K8 ["React"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETIMPORT R5 K1 [script]
  GETTABLEKS R4 R5 K9 ["EquippedItemPreview"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETIMPORT R7 K1 [script]
  GETTABLEKS R6 R7 K10 ["Parent"]
  GETTABLEKS R5 R6 K11 ["ResetCameraButton"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K12 ["Src"]
  GETTABLEKS R7 R8 K13 ["Resources"]
  GETTABLEKS R6 R7 K14 ["CatalogItems"]
  CALL R5 1 1
  GETIMPORT R6 K5 [require]
  GETTABLEKS R9 R0 K12 ["Src"]
  GETTABLEKS R8 R9 K15 ["Util"]
  GETTABLEKS R7 R8 K16 ["Constants"]
  CALL R6 1 1
  GETIMPORT R7 K5 [require]
  GETTABLEKS R10 R0 K12 ["Src"]
  GETTABLEKS R9 R10 K17 ["Components"]
  GETTABLEKS R8 R9 K18 ["DraggableTileList"]
  CALL R7 1 1
  GETIMPORT R8 K5 [require]
  GETTABLEKS R11 R0 K12 ["Src"]
  GETTABLEKS R10 R11 K15 ["Util"]
  GETTABLEKS R9 R10 K19 ["EquipmentStateContext"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETTABLEKS R12 R0 K12 ["Src"]
  GETTABLEKS R11 R12 K15 ["Util"]
  GETTABLEKS R10 R11 K20 ["getItemAccessoryType"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R13 R0 K12 ["Src"]
  GETTABLEKS R12 R13 K15 ["Util"]
  GETTABLEKS R11 R12 K21 ["DEPRECATED_isLayeredClothing"]
  CALL R10 1 1
  GETIMPORT R11 K5 [require]
  GETTABLEKS R14 R0 K12 ["Src"]
  GETTABLEKS R13 R14 K13 ["Resources"]
  GETTABLEKS R12 R13 K22 ["Theme"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETTABLEKS R14 R0 K12 ["Src"]
  GETTABLEKS R13 R14 K23 ["Types"]
  CALL R12 1 1
  GETIMPORT R13 K5 [require]
  GETTABLEKS R16 R0 K12 ["Src"]
  GETTABLEKS R15 R16 K24 ["Flags"]
  GETTABLEKS R14 R15 K25 ["getFFlagAvatarPreviewerAlignResetCameraButton"]
  CALL R13 1 1
  GETIMPORT R14 K5 [require]
  GETTABLEKS R17 R0 K12 ["Src"]
  GETTABLEKS R16 R17 K24 ["Flags"]
  GETTABLEKS R15 R16 K26 ["getFFlagAvatarPreviewerEquippedHoverMenu"]
  CALL R14 1 1
  GETIMPORT R15 K5 [require]
  GETTABLEKS R18 R0 K12 ["Src"]
  GETTABLEKS R17 R18 K24 ["Flags"]
  GETTABLEKS R16 R17 K27 ["getFFlagAvatarPreviewerNewDraggableTileList"]
  CALL R15 1 1
  GETIMPORT R16 K5 [require]
  GETTABLEKS R19 R0 K12 ["Src"]
  GETTABLEKS R18 R19 K24 ["Flags"]
  GETTABLEKS R17 R18 K28 ["getFFlagAvatarPreviewerClothingTypeIcons"]
  CALL R16 1 1
  GETTABLEKS R17 R1 K29 ["UI"]
  GETTABLEKS R18 R17 K18 ["DraggableTileList"]
  GETTABLEKS R19 R17 K30 ["Pane"]
  GETTABLEKS R20 R17 K31 ["ScrollingFrame"]
  GETTABLEKS R22 R1 K15 ["Util"]
  GETTABLEKS R21 R22 K32 ["LayoutOrderIterator"]
  GETTABLEKS R23 R1 K33 ["ContextServices"]
  GETTABLEKS R22 R23 K34 ["Analytics"]
  GETTABLEKS R24 R1 K33 ["ContextServices"]
  GETTABLEKS R23 R24 K35 ["Stylizer"]
  DUPCLOSURE R24 K36 [PROTO_0]
  CAPTURE VAL R5
  CAPTURE VAL R6
  DUPCLOSURE R25 K37 [PROTO_5]
  CAPTURE VAL R22
  CAPTURE VAL R23
  CAPTURE VAL R2
  CAPTURE VAL R8
  CAPTURE VAL R21
  CAPTURE VAL R16
  CAPTURE VAL R9
  CAPTURE VAL R15
  CAPTURE VAL R10
  CAPTURE VAL R24
  CAPTURE VAL R3
  CAPTURE VAL R19
  CAPTURE VAL R13
  CAPTURE VAL R4
  CAPTURE VAL R7
  CAPTURE VAL R14
  CAPTURE VAL R18
  CAPTURE VAL R20
  RETURN R25 1

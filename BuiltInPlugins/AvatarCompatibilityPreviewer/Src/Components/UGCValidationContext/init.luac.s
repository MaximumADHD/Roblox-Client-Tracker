PROTO_0:
  DUPTABLE R1 K2 [{"type", "metadata"}]
  LOADK R2 K3 ["success"]
  SETTABLEKS R2 R1 K0 ["type"]
  SETTABLEKS R0 R1 K1 ["metadata"]
  RETURN R1 1

PROTO_1:
  GETUPVAL R1 0
  LOADK R3 K0 ["bundlesMetadataError"]
  MOVE R4 R0
  NAMECALL R1 R1 K1 ["report"]
  CALL R1 3 0
  DUPTABLE R1 K3 [{"type"}]
  LOADK R2 K4 ["error"]
  SETTABLEKS R2 R1 K2 ["type"]
  RETURN R1 1

PROTO_2:
  GETIMPORT R1 K2 [table.clone]
  MOVE R2 R0
  CALL R1 1 1
  MOVE R0 R1
  GETUPVAL R1 0
  DUPTABLE R2 K5 [{"completed", "ugcValidationResponse"}]
  LOADB R3 0
  SETTABLEKS R3 R2 K3 ["completed"]
  GETUPVAL R3 1
  SETTABLEKS R3 R2 K4 ["ugcValidationResponse"]
  SETTABLE R2 R0 R1
  RETURN R0 1

PROTO_3:
  GETUPVAL R1 0
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U1
  CAPTURE VAL R0
  CALL R1 1 0
  RETURN R0 0

PROTO_4:
  GETIMPORT R1 K2 [table.clone]
  MOVE R2 R0
  CALL R1 1 1
  MOVE R0 R1
  GETUPVAL R1 0
  DUPTABLE R2 K6 [{"completed", "duration", "ugcValidationResponse"}]
  LOADB R3 1
  SETTABLEKS R3 R2 K3 ["completed"]
  GETIMPORT R4 K9 [os.clock]
  CALL R4 0 1
  GETUPVAL R5 1
  SUB R3 R4 R5
  SETTABLEKS R3 R2 K4 ["duration"]
  GETUPVAL R3 2
  SETTABLEKS R3 R2 K5 ["ugcValidationResponse"]
  SETTABLE R2 R0 R1
  RETURN R0 1

PROTO_5:
  GETUPVAL R1 0
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  CAPTURE VAL R0
  CALL R1 1 0
  RETURN R0 0

PROTO_6:
  GETIMPORT R1 K2 [table.clone]
  MOVE R2 R0
  CALL R1 1 1
  MOVE R0 R1
  GETUPVAL R1 0
  DUPTABLE R2 K5 [{"completed", "ugcValidationResponse"}]
  LOADB R3 0
  SETTABLEKS R3 R2 K3 ["completed"]
  GETUPVAL R3 1
  SETTABLEKS R3 R2 K4 ["ugcValidationResponse"]
  SETTABLE R2 R0 R1
  RETURN R0 1

PROTO_7:
  GETUPVAL R1 0
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U1
  CAPTURE VAL R0
  CALL R1 1 0
  RETURN R0 0

PROTO_8:
  GETIMPORT R1 K2 [table.clone]
  MOVE R2 R0
  CALL R1 1 1
  MOVE R0 R1
  GETUPVAL R1 0
  DUPTABLE R2 K6 [{"completed", "duration", "ugcValidationResponse"}]
  LOADB R3 1
  SETTABLEKS R3 R2 K3 ["completed"]
  GETIMPORT R4 K9 [os.clock]
  CALL R4 0 1
  GETUPVAL R5 1
  SUB R3 R4 R5
  SETTABLEKS R3 R2 K4 ["duration"]
  GETUPVAL R3 2
  SETTABLEKS R3 R2 K5 ["ugcValidationResponse"]
  SETTABLE R2 R0 R1
  RETURN R0 1

PROTO_9:
  GETUPVAL R1 0
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U1
  CAPTURE UPVAL U2
  CAPTURE VAL R0
  CALL R1 1 0
  RETURN R0 0

PROTO_10:
  GETTABLEKS R1 R0 K0 ["type"]
  JUMPIFEQKS R1 K1 ["success"] [+2]
  RETURN R0 0
  GETTABLEKS R3 R0 K0 ["type"]
  JUMPIFEQKS R3 K1 ["success"] [+2]
  LOADB R2 0 +1
  LOADB R2 1
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["LUAU_ANALYZE_ERROR"]
  FASTCALL2 ASSERT R2 R3 [+3]
  GETIMPORT R1 K4 [assert]
  CALL R1 2 0
  GETIMPORT R1 K7 [os.clock]
  CALL R1 0 1
  GETUPVAL R2 1
  JUMPIFNOT R2 [+29]
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K8 ["validateBundleReadyForUpload"]
  GETUPVAL R3 3
  GETTABLEKS R5 R0 K9 ["metadata"]
  GETTABLEKS R4 R5 K10 ["allowedBundleTypeSettings"]
  LOADK R5 K11 ["Body"]
  NEWCLOSURE R6 P0
  CAPTURE UPVAL U4
  CAPTURE UPVAL U3
  LOADNIL R7
  DUPTABLE R8 K13 [{"skipValidateHSR"}]
  LOADB R9 1
  SETTABLEKS R9 R8 K12 ["skipValidateHSR"]
  CALL R2 6 1
  NEWCLOSURE R4 P1
  CAPTURE UPVAL U4
  CAPTURE UPVAL U3
  CAPTURE VAL R1
  NAMECALL R2 R2 K14 ["andThen"]
  CALL R2 2 1
  NAMECALL R2 R2 K15 ["finally"]
  CALL R2 1 0
  RETURN R0 0
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K8 ["validateBundleReadyForUpload"]
  GETUPVAL R3 3
  GETTABLEKS R5 R0 K9 ["metadata"]
  GETTABLEKS R4 R5 K10 ["allowedBundleTypeSettings"]
  LOADK R5 K11 ["Body"]
  NEWCLOSURE R6 P2
  CAPTURE UPVAL U4
  CAPTURE UPVAL U3
  CALL R2 4 1
  NEWCLOSURE R4 P3
  CAPTURE UPVAL U4
  CAPTURE UPVAL U3
  CAPTURE VAL R1
  NAMECALL R2 R2 K14 ["andThen"]
  CALL R2 2 1
  NAMECALL R2 R2 K15 ["finally"]
  CALL R2 1 0
  RETURN R0 0

PROTO_11:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["current"]
  GETTABLE R1 R2 R0
  JUMPIFNOT R1 [+1]
  RETURN R0 0
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["current"]
  LOADB R2 1
  SETTABLE R2 R1 R0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K0 ["current"]
  JUMPIFNOTEQKNIL R1 [+21]
  GETUPVAL R1 1
  GETUPVAL R5 2
  GETTABLEKS R4 R5 K1 ["util"]
  GETTABLEKS R3 R4 K2 ["BundlesMetadata"]
  GETTABLEKS R2 R3 K3 ["fetch"]
  CALL R2 0 1
  DUPCLOSURE R4 K4 [PROTO_0]
  NAMECALL R2 R2 K5 ["andThen"]
  CALL R2 2 1
  NEWCLOSURE R4 P1
  CAPTURE UPVAL U3
  NAMECALL R2 R2 K6 ["catch"]
  CALL R2 2 1
  SETTABLEKS R2 R1 K0 ["current"]
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K0 ["current"]
  JUMPIFNOTEQKNIL R3 [+2]
  LOADB R2 0 +1
  LOADB R2 1
  GETUPVAL R4 4
  GETTABLEKS R3 R4 K7 ["LUAU_ANALYZE_ERROR"]
  FASTCALL2 ASSERT R2 R3 [+3]
  GETIMPORT R1 K9 [assert]
  CALL R1 2 0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K0 ["current"]
  NEWCLOSURE R3 P2
  CAPTURE UPVAL U4
  CAPTURE UPVAL U5
  CAPTURE UPVAL U2
  CAPTURE VAL R0
  CAPTURE UPVAL U6
  NAMECALL R1 R1 K5 ["andThen"]
  CALL R1 2 0
  RETURN R0 0

PROTO_12:
  GETUPVAL R1 0
  NAMECALL R1 R1 K0 ["use"]
  CALL R1 1 1
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K1 ["useState"]
  NEWTABLE R3 0 0
  CALL R2 1 2
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K2 ["useRef"]
  NEWTABLE R5 0 0
  CALL R4 1 1
  GETTABLEKS R7 R4 K3 ["current"]
  JUMPIFNOTEQKNIL R7 [+2]
  LOADB R6 0 +1
  LOADB R6 1
  GETUPVAL R8 2
  GETTABLEKS R7 R8 K4 ["LUAU_ANALYZE_ERROR"]
  FASTCALL2 ASSERT R6 R7 [+3]
  GETIMPORT R5 K6 [assert]
  CALL R5 2 0
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K2 ["useRef"]
  LOADNIL R6
  CALL R5 1 1
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K7 ["useCallback"]
  NEWCLOSURE R7 P0
  CAPTURE VAL R4
  CAPTURE VAL R5
  CAPTURE UPVAL U3
  CAPTURE VAL R1
  CAPTURE UPVAL U2
  CAPTURE UPVAL U4
  CAPTURE VAL R3
  NEWTABLE R8 0 0
  CALL R6 2 1
  GETUPVAL R7 5
  GETUPVAL R10 3
  GETTABLEKS R9 R10 K8 ["util"]
  GETTABLEKS R8 R9 K9 ["canUploadBundlesAsync"]
  CALL R7 1 2
  DUPTABLE R9 K14 [{"cachedResponses", "validateBundle", "canUploadBundles", "requestCanUploadBundles"}]
  SETTABLEKS R2 R9 K10 ["cachedResponses"]
  SETTABLEKS R6 R9 K11 ["validateBundle"]
  SETTABLEKS R7 R9 K12 ["canUploadBundles"]
  SETTABLEKS R8 R9 K13 ["requestCanUploadBundles"]
  GETUPVAL R11 1
  GETTABLEKS R10 R11 K15 ["createElement"]
  GETUPVAL R12 6
  GETTABLEKS R11 R12 K16 ["Provider"]
  DUPTABLE R12 K18 [{"value"}]
  SETTABLEKS R9 R12 K17 ["value"]
  GETTABLEKS R13 R0 K19 ["children"]
  CALL R10 3 -1
  RETURN R10 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AvatarCompatibilityPreviewer"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Framework"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K6 ["Packages"]
  GETTABLEKS R3 R4 K8 ["React"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R5 R0 K6 ["Packages"]
  GETTABLEKS R4 R5 K9 ["UGCValidation"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R7 R0 K10 ["Src"]
  GETTABLEKS R6 R7 K11 ["Util"]
  GETTABLEKS R5 R6 K12 ["Constants"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K10 ["Src"]
  GETTABLEKS R7 R8 K11 ["Util"]
  GETTABLEKS R6 R7 K13 ["createUnimplemented"]
  CALL R5 1 1
  GETIMPORT R6 K5 [require]
  GETIMPORT R8 K1 [script]
  GETTABLEKS R7 R8 K14 ["useCanUploadBundles"]
  CALL R6 1 1
  GETTABLEKS R8 R1 K15 ["ContextServices"]
  GETTABLEKS R7 R8 K16 ["Analytics"]
  GETIMPORT R8 K18 [game]
  LOADK R10 K19 ["AvatarPreviewerBypassHSRValidation2"]
  LOADB R11 0
  NAMECALL R8 R8 K20 ["DefineFastFlag"]
  CALL R8 3 1
  DUPTABLE R9 K25 [{"cachedResponses", "validateBundle", "canUploadBundles", "requestCanUploadBundles"}]
  NEWTABLE R10 0 0
  SETTABLEKS R10 R9 K21 ["cachedResponses"]
  MOVE R10 R5
  LOADK R11 K22 ["validateBundle"]
  CALL R10 1 1
  SETTABLEKS R10 R9 K22 ["validateBundle"]
  LOADNIL R10
  SETTABLEKS R10 R9 K23 ["canUploadBundles"]
  MOVE R10 R5
  LOADK R11 K24 ["requestCanUploadBundles"]
  CALL R10 1 1
  SETTABLEKS R10 R9 K24 ["requestCanUploadBundles"]
  GETTABLEKS R10 R2 K26 ["createContext"]
  MOVE R11 R9
  CALL R10 1 1
  DUPCLOSURE R11 K27 [PROTO_12]
  CAPTURE VAL R7
  CAPTURE VAL R2
  CAPTURE VAL R4
  CAPTURE VAL R3
  CAPTURE VAL R8
  CAPTURE VAL R6
  CAPTURE VAL R10
  DUPTABLE R12 K30 [{"Context", "Provider"}]
  SETTABLEKS R10 R12 K28 ["Context"]
  SETTABLEKS R11 R12 K29 ["Provider"]
  RETURN R12 1

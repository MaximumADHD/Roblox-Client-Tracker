...
function PolicyInfoController:postContactEmail(universeId, contactEmail)
	assert(type(universeId) == "number", string.format("universeId must be a number, received %s", type(universeId)))
	assert(
	...
	local responseCode
	studioModerationContactPOST(networking, universeId, contactEmail)

...
local shouldDisablePrivateServersAndPaidAccess =
	require(Plugin.Src.Util.GameSettingsUtilities).shouldDisablePrivateServersAndPaidAccess
...
function VIPServers:render()
	...
	if subsCount < 0 then
		subscriptionsText = localization:getText(
			"Monetization",
			"OverPrivateServerSubscriptions",
			{ numOfSubscriptions = subsCount * -1 }
		)
	else
		subscriptionsText =
			localization:getText("Monetization", "PrivateServerSubscriptions", { numOfSubscriptions = subsCount })
	end
	totalVIPServersText =
		localization:getText("Monetization", "PrivateServersActive", { totalVipServers = serversCount })
	...
	local toggleSubTextSize = GetTextSize(
		toggleSubText,
	...
	local maxPriceConfigHeight = subText and theme.robuxFeeBase.height.withSubText
		or theme.robuxFeeBase.height.withoutSubText

	local showToggleSubText = not selected or (not selected and hasPriceChanged)
	local toggleSubTextTheme = (not selected and (hasPriceChanged or willShutdown)) and theme.fontStyle.SmallError
		or theme.fontStyle.Subtext

...
local function loadSettings(store, contextItems)
	...
	local settingsLoadJobs = {
		...
		function(loadedSettings)
			local vipServersActiveSubscriptionsCount =
				monetizationController:getVIPServersActiveSubscriptionsCount(gameId)
...
local function displayMonetizationPage(props)
	...
	return {
		BadgesLoadingIndicatorContainer = showBadges
			and badgeLoadState == LoadState.Loading
			and Roact.createElement(Container, {
...
local function displayEditDevProductsPage(props)
	...
	if not initialName then
		initialName = currentDevProduct.name
	end
	...
	if dpPriceError then
		dpPriceErrorSize =
			GetTextSize(dpPriceError, theme.fontStyle.SmallError.TextSize, theme.fontStyle.SmallError.Font)
	else
		dpPriceErrorSize = {}
	end
...
Monetization = RoactRodux.connect(function(state, props)
	if not state then
		return
	end

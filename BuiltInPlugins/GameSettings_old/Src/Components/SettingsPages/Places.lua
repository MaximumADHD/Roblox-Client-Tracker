...
local FFlagStudioPlaceVersionHistory = game:GetFastFlag("StudioPlaceVersionHistory")
...
local function displayPlaceListPage(props, localization)
	...
	if FFlagStudioPlaceVersionHistory then
		table.insert(menuItems, { Key = "VerisonHistoryKey", Text = localization:getText("Places", "VersionHistory") })
	end
	...
	return
	{
		...
		PlacesTable = Roact.createElement(TableWithMenu, {
			...
			OnItemClicked = function(key, id)
				...
				elseif FFlagStudioPlaceVersionHistory and key == "VerisonHistoryKey" then
...
local function displayEditPlacePage(props, localization)
	...
	return {
		...
		VersionHistory = FFlagStudioPlaceVersionHistory and Roact.createElement(TitledFrame, {

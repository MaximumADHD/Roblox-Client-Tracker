PROTO_0:
  GETUPVAL R0 0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K0 ["Name"]
  SETTABLEKS R1 R0 K0 ["Name"]
  GETUPVAL R1 2
  GETTABLEKS R0 R1 K1 ["createElement"]
  GETUPVAL R1 3
  DUPTABLE R2 K4 [{"Plugin", "pluginLoaderContext"}]
  GETUPVAL R3 0
  SETTABLEKS R3 R2 K2 ["Plugin"]
  GETUPVAL R3 4
  SETTABLEKS R3 R2 K3 ["pluginLoaderContext"]
  CALL R0 2 1
  GETUPVAL R2 2
  GETTABLEKS R1 R2 K5 ["mount"]
  MOVE R2 R0
  CALL R1 1 1
  SETUPVAL R1 5
  GETUPVAL R1 6
  JUMPIFNOT R1 [+6]
  GETUPVAL R1 6
  LOADK R3 K6 ["Roact tree"]
  GETUPVAL R4 5
  NAMECALL R1 R1 K7 ["addRoactTree"]
  CALL R1 3 0
  RETURN R0 0

PROTO_1:
  GETUPVAL R0 0
  JUMPIFNOT R0 [+4]
  GETUPVAL R0 0
  NAMECALL R0 R0 K0 ["destroy"]
  CALL R0 1 0
  GETUPVAL R0 1
  JUMPIFNOT R0 [+7]
  GETUPVAL R1 2
  GETTABLEKS R0 R1 K1 ["unmount"]
  GETUPVAL R1 1
  CALL R0 1 0
  LOADNIL R0
  SETUPVAL R0 1
  RETURN R0 0

PROTO_2:
  JUMPIF R0 [+1]
  RETURN R0 0
  GETIMPORT R2 K1 [require]
  GETIMPORT R5 K3 [script]
  GETTABLEKS R4 R5 K4 ["Parent"]
  GETTABLEKS R3 R4 K5 ["commonInit"]
  CALL R2 1 1
  MOVE R3 R2
  CALL R3 0 0
  GETIMPORT R5 K3 [script]
  GETTABLEKS R4 R5 K4 ["Parent"]
  GETTABLEKS R3 R4 K4 ["Parent"]
  GETIMPORT R4 K1 [require]
  GETTABLEKS R6 R3 K6 ["Packages"]
  GETTABLEKS R5 R6 K7 ["Roact"]
  CALL R4 1 1
  GETIMPORT R5 K1 [require]
  GETTABLEKS R7 R3 K8 ["Src"]
  GETTABLEKS R6 R7 K9 ["MainPlugin"]
  CALL R5 1 1
  GETIMPORT R6 K1 [require]
  GETTABLEKS R9 R3 K8 ["Src"]
  GETTABLEKS R8 R9 K10 ["Util"]
  GETTABLEKS R7 R8 K11 ["hasInternalPermission"]
  CALL R6 1 1
  CALL R6 0 1
  LOADNIL R7
  LOADNIL R8
  JUMPIFNOT R6 [+14]
  GETIMPORT R10 K1 [require]
  GETTABLEKS R12 R3 K6 ["Packages"]
  GETTABLEKS R11 R12 K12 ["DeveloperTools"]
  CALL R10 1 1
  GETTABLEKS R9 R10 K13 ["forPlugin"]
  GETTABLEKS R10 R3 K14 ["Name"]
  MOVE R11 R0
  CALL R9 2 1
  MOVE R8 R9
  NEWCLOSURE R9 P0
  CAPTURE VAL R0
  CAPTURE VAL R3
  CAPTURE VAL R4
  CAPTURE VAL R5
  CAPTURE VAL R1
  CAPTURE REF R7
  CAPTURE REF R8
  GETTABLEKS R10 R0 K15 ["Unloading"]
  NEWCLOSURE R12 P1
  CAPTURE REF R8
  CAPTURE REF R7
  CAPTURE VAL R4
  NAMECALL R10 R10 K16 ["Connect"]
  CALL R10 2 0
  MOVE R10 R9
  CALL R10 0 0
  CLOSEUPVALS R7
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  DUPCLOSURE R0 K0 [PROTO_2]
  RETURN R0 1

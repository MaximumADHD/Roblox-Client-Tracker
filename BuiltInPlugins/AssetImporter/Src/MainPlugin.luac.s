PROTO_0:
  GETUPVAL R2 0
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  GETUPVAL R7 1
  MOVE R8 R1
  LOADK R9 K0 ["Callouts"]
  GETTABLEKS R10 R6 K1 ["Title"]
  CALL R7 3 1
  GETUPVAL R8 1
  MOVE R9 R1
  LOADK R10 K0 ["Callouts"]
  GETTABLEKS R11 R6 K2 ["Description"]
  CALL R8 3 1
  JUMPIFNOT R7 [+9]
  JUMPIFNOT R8 [+8]
  MOVE R11 R5
  MOVE R12 R7
  MOVE R13 R8
  GETTABLEKS R14 R6 K3 ["Link"]
  NAMECALL R9 R0 K4 ["defineCallout"]
  CALL R9 5 0
  FORGLOOP R2 2 [-23]
  RETURN R0 0

PROTO_1:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["props"]
  GETTABLEKS R0 R1 K1 ["FileController"]
  NAMECALL R0 R0 K2 ["onQueueButtonClicked"]
  CALL R0 1 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["props"]
  GETTABLEKS R0 R1 K1 ["FileController"]
  NAMECALL R0 R0 K2 ["onPluginButtonClicked"]
  CALL R0 1 0
  RETURN R0 0

PROTO_3:
  GETUPVAL R2 0
  CALL R2 0 1
  JUMPIFNOT R2 [+18]
  GETUPVAL R3 1
  GETTABLEKS R2 R3 K0 ["Parent"]
  LOADK R4 K1 ["Actions"]
  NAMECALL R2 R2 K2 ["GetPluginComponent"]
  CALL R2 2 1
  GETUPVAL R5 2
  NAMECALL R3 R2 K3 ["BindToActivatedAsync"]
  CALL R3 2 1
  NEWCLOSURE R5 P0
  CAPTURE VAL R0
  NAMECALL R3 R3 K4 ["Connect"]
  CALL R3 2 1
  SETTABLEKS R3 R0 K5 ["toggleQueueAction"]
  GETTABLEKS R3 R1 K6 ["PluginLoaderContextItem"]
  GETTABLEKS R2 R3 K7 ["mainButtonClickedSignal"]
  NEWCLOSURE R4 P1
  CAPTURE VAL R0
  NAMECALL R2 R2 K4 ["Connect"]
  CALL R2 2 0
  RETURN R0 0

PROTO_4:
  GETTABLEKS R1 R0 K0 ["props"]
  GETTABLEKS R3 R1 K1 ["Store"]
  NAMECALL R3 R3 K2 ["getState"]
  CALL R3 1 1
  GETTABLEKS R2 R3 K3 ["showPreview"]
  GETTABLEKS R4 R1 K4 ["PluginLoaderContextItem"]
  GETTABLEKS R3 R4 K5 ["mainButton"]
  MOVE R5 R2
  NAMECALL R3 R3 K6 ["SetActive"]
  CALL R3 2 0
  RETURN R0 0

PROTO_5:
  GETTABLEKS R1 R0 K0 ["props"]
  GETTABLEKS R2 R1 K1 ["Localization"]
  NEWTABLE R3 0 12
  GETTABLEKS R4 R1 K2 ["Plugin"]
  GETTABLEKS R5 R1 K3 ["Design"]
  GETTABLEKS R6 R1 K4 ["Mouse"]
  GETTABLEKS R7 R1 K5 ["Store"]
  GETTABLEKS R8 R1 K6 ["Theme"]
  MOVE R9 R2
  GETTABLEKS R10 R1 K7 ["Analytics"]
  GETTABLEKS R11 R1 K8 ["CalloutController"]
  GETTABLEKS R12 R1 K9 ["PresetController"]
  GETTABLEKS R13 R1 K10 ["FileController"]
  GETTABLEKS R14 R1 K11 ["Networking"]
  GETTABLEKS R15 R1 K12 ["QueueController"]
  SETLIST R3 R4 12 [1]
  GETUPVAL R4 0
  CALL R4 0 1
  JUMPIFNOT R4 [+8]
  GETTABLEKS R6 R1 K13 ["UploadController"]
  FASTCALL2 TABLE_INSERT R3 R6 [+4]
  MOVE R5 R3
  GETIMPORT R4 K16 [table.insert]
  CALL R4 2 0
  GETUPVAL R4 1
  GETTABLEKS R5 R1 K8 ["CalloutController"]
  MOVE R6 R2
  CALL R4 2 0
  GETUPVAL R5 2
  GETTABLEKS R4 R5 K17 ["provide"]
  MOVE R5 R3
  DUPTABLE R6 K21 [{"ImportQueue", "Preview", "UploadWidget"}]
  GETUPVAL R8 3
  GETTABLEKS R7 R8 K22 ["createElement"]
  GETUPVAL R8 4
  DUPTABLE R9 K24 [{"Title"}]
  LOADK R12 K18 ["ImportQueue"]
  LOADK R13 K25 ["WindowTitle"]
  NAMECALL R10 R2 K26 ["getText"]
  CALL R10 3 1
  SETTABLEKS R10 R9 K23 ["Title"]
  CALL R7 2 1
  SETTABLEKS R7 R6 K18 ["ImportQueue"]
  GETUPVAL R8 3
  GETTABLEKS R7 R8 K22 ["createElement"]
  GETUPVAL R8 5
  DUPTABLE R9 K24 [{"Title"}]
  LOADK R12 K2 ["Plugin"]
  LOADK R13 K25 ["WindowTitle"]
  NAMECALL R10 R2 K26 ["getText"]
  CALL R10 3 1
  SETTABLEKS R10 R9 K23 ["Title"]
  CALL R7 2 1
  SETTABLEKS R7 R6 K19 ["Preview"]
  GETUPVAL R8 3
  GETTABLEKS R7 R8 K22 ["createElement"]
  GETUPVAL R8 6
  DUPTABLE R9 K24 [{"Title"}]
  LOADK R12 K27 ["Upload"]
  LOADK R13 K25 ["WindowTitle"]
  NAMECALL R10 R2 K26 ["getText"]
  CALL R10 3 1
  SETTABLEKS R10 R9 K23 ["Title"]
  CALL R7 2 1
  SETTABLEKS R7 R6 K20 ["UploadWidget"]
  CALL R4 2 -1
  RETURN R4 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R2 K1 [script]
  GETTABLEKS R1 R2 K2 ["Parent"]
  GETTABLEKS R0 R1 K2 ["Parent"]
  GETIMPORT R1 K4 [require]
  GETTABLEKS R3 R0 K5 ["Packages"]
  GETTABLEKS R2 R3 K6 ["RoactCompat"]
  CALL R1 1 1
  GETIMPORT R2 K4 [require]
  GETTABLEKS R4 R0 K5 ["Packages"]
  GETTABLEKS R3 R4 K7 ["Framework"]
  CALL R2 1 1
  GETTABLEKS R3 R2 K8 ["ContextServices"]
  GETTABLEKS R5 R0 K9 ["Src"]
  GETTABLEKS R4 R5 K10 ["Components"]
  GETIMPORT R5 K4 [require]
  GETTABLEKS R7 R4 K11 ["Dialogs"]
  GETTABLEKS R6 R7 K12 ["MeshImportDialog"]
  CALL R5 1 1
  GETIMPORT R6 K4 [require]
  GETTABLEKS R8 R4 K11 ["Dialogs"]
  GETTABLEKS R7 R8 K13 ["UploadWidget"]
  CALL R6 1 1
  GETIMPORT R7 K4 [require]
  GETTABLEKS R9 R4 K11 ["Dialogs"]
  GETTABLEKS R8 R9 K14 ["ImportQueue"]
  CALL R7 1 1
  GETIMPORT R8 K4 [require]
  GETTABLEKS R11 R0 K9 ["Src"]
  GETTABLEKS R10 R11 K15 ["Resources"]
  GETTABLEKS R9 R10 K16 ["CalloutList"]
  CALL R8 1 1
  GETIMPORT R9 K4 [require]
  GETTABLEKS R12 R0 K9 ["Src"]
  GETTABLEKS R11 R12 K17 ["Utility"]
  GETTABLEKS R10 R11 K18 ["GetLocalizedString"]
  CALL R9 1 1
  GETIMPORT R10 K4 [require]
  GETTABLEKS R13 R0 K9 ["Src"]
  GETTABLEKS R12 R13 K19 ["Flags"]
  GETTABLEKS R11 R12 K20 ["getFFlagImportQueueRibbonDropdown"]
  CALL R10 1 1
  GETIMPORT R11 K4 [require]
  GETTABLEKS R14 R0 K9 ["Src"]
  GETTABLEKS R13 R14 K19 ["Flags"]
  GETTABLEKS R12 R13 K21 ["getFFlagCinUploadController"]
  CALL R11 1 1
  DUPTABLE R12 K26 [{"DataModel", "PluginId", "Category", "ItemId"}]
  LOADK R13 K27 ["Standalone"]
  SETTABLEKS R13 R12 K22 ["DataModel"]
  LOADK R13 K28 ["MeshImporter"]
  SETTABLEKS R13 R12 K23 ["PluginId"]
  LOADK R13 K29 ["Actions"]
  SETTABLEKS R13 R12 K24 ["Category"]
  LOADK R13 K30 ["ToggleQueue"]
  SETTABLEKS R13 R12 K25 ["ItemId"]
  GETTABLEKS R13 R1 K31 ["PureComponent"]
  LOADK R15 K32 ["MainPlugin"]
  NAMECALL R13 R13 K33 ["extend"]
  CALL R13 2 1
  DUPCLOSURE R14 K34 [PROTO_0]
  CAPTURE VAL R8
  CAPTURE VAL R9
  DUPCLOSURE R15 K35 [PROTO_3]
  CAPTURE VAL R10
  CAPTURE VAL R0
  CAPTURE VAL R12
  SETTABLEKS R15 R13 K36 ["init"]
  DUPCLOSURE R15 K37 [PROTO_4]
  SETTABLEKS R15 R13 K38 ["didUpdate"]
  DUPCLOSURE R15 K39 [PROTO_5]
  CAPTURE VAL R11
  CAPTURE VAL R14
  CAPTURE VAL R3
  CAPTURE VAL R1
  CAPTURE VAL R7
  CAPTURE VAL R5
  CAPTURE VAL R6
  SETTABLEKS R15 R13 K40 ["render"]
  RETURN R13 1

PROTO_0:
  GETUPVAL R1 0
  CALL R1 0 1
  JUMPIFNOT R1 [+80]
  GETUPVAL R1 1
  NAMECALL R1 R1 K0 ["PickMultipleFilesWithPrompt"]
  CALL R1 1 1
  LENGTH R2 R1
  JUMPIFNOTEQKN R2 K1 [0] [+5]
  GETUPVAL R2 2
  MOVE R3 R1
  CALL R2 1 0
  RETURN R0 0
  NAMECALL R5 R0 K2 ["getState"]
  CALL R5 1 1
  GETTABLEKS R4 R5 K3 ["Sessions"]
  GETTABLEKS R3 R4 K4 ["sessionCount"]
  LENGTH R4 R1
  ADD R2 R3 R4
  GETUPVAL R5 3
  MOVE R6 R2
  CALL R5 1 -1
  NAMECALL R3 R0 K5 ["dispatch"]
  CALL R3 -1 0
  GETUPVAL R5 4
  LOADN R6 0
  CALL R5 1 -1
  NAMECALL R3 R0 K5 ["dispatch"]
  CALL R3 -1 0
  GETUPVAL R5 5
  MOVE R6 R2
  CALL R5 1 -1
  NAMECALL R3 R0 K5 ["dispatch"]
  CALL R3 -1 0
  GETUPVAL R5 6
  LOADB R6 1
  CALL R5 1 -1
  NAMECALL R3 R0 K5 ["dispatch"]
  CALL R3 -1 0
  MOVE R3 R1
  LOADNIL R4
  LOADNIL R5
  FORGPREP R3
  GETUPVAL R8 1
  MOVE R10 R7
  NAMECALL R8 R8 K6 ["StartSessionWithPath"]
  CALL R8 2 1
  GETUPVAL R11 7
  MOVE R12 R7
  MOVE R13 R8
  CALL R11 2 -1
  NAMECALL R9 R0 K5 ["dispatch"]
  CALL R9 -1 0
  GETUPVAL R11 4
  MOVE R12 R6
  CALL R11 1 -1
  NAMECALL R9 R0 K5 ["dispatch"]
  CALL R9 -1 0
  FORGLOOP R3 2 [-19]
  GETUPVAL R5 6
  LOADB R6 0
  CALL R5 1 -1
  NAMECALL R3 R0 K5 ["dispatch"]
  CALL R3 -1 0
  GETUPVAL R3 2
  JUMPIFNOT R3 [+105]
  GETUPVAL R3 2
  MOVE R4 R1
  CALL R3 1 0
  RETURN R0 0
  LOADNIL R1
  LOADNIL R2
  LOADNIL R3
  GETUPVAL R4 1
  NAMECALL R4 R4 K7 ["StartSessionWithPrompt"]
  CALL R4 1 1
  MOVE R1 R4
  JUMPIFNOT R1 [+8]
  NAMECALL R4 R1 K8 ["GetImportTree"]
  CALL R4 1 1
  MOVE R2 R4
  NAMECALL R4 R1 K9 ["GetFilename"]
  CALL R4 1 1
  MOVE R3 R4
  JUMPIFNOT R2 [+60]
  JUMPIFNOT R3 [+59]
  NAMECALL R4 R2 K10 ["GetPreview"]
  CALL R4 1 1
  NEWTABLE R5 0 0
  NAMECALL R6 R2 K11 ["GetDescendants"]
  CALL R6 1 3
  FORGPREP R6
  GETTABLEKS R11 R10 K12 ["ShouldImport"]
  SETTABLE R11 R5 R10
  FORGLOOP R6 2 [-4]
  LOADB R6 1
  SETTABLE R6 R5 R2
  GETUPVAL R8 8
  CALL R8 0 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R8 9
  MOVE R9 R4
  CALL R8 1 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R8 10
  MOVE R9 R2
  CALL R8 1 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R8 11
  MOVE R9 R3
  CALL R8 1 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R8 12
  MOVE R9 R2
  CALL R8 1 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R8 13
  MOVE R9 R5
  CALL R8 1 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R6 14
  CALL R6 0 1
  JUMPIFNOT R6 [+5]
  GETUPVAL R8 15
  CALL R8 0 -1
  NAMECALL R6 R0 K5 ["dispatch"]
  CALL R6 -1 0
  GETUPVAL R6 16
  MOVE R7 R1
  CALL R6 1 -1
  NAMECALL R4 R0 K5 ["dispatch"]
  CALL R4 -1 0
  GETUPVAL R4 2
  JUMPIFNOT R4 [+15]
  LOADB R4 0
  JUMPIFEQKNIL R2 [+5]
  JUMPIFNOTEQKNIL R3 [+2]
  LOADB R4 0 +1
  LOADB R4 1
  JUMPIFEQKNIL R1 [+2]
  LOADB R5 0 +1
  LOADB R5 1
  GETUPVAL R6 2
  MOVE R7 R4
  MOVE R8 R5
  CALL R6 2 0
  RETURN R0 0

PROTO_1:
  NEWCLOSURE R1 P0
  CAPTURE UPVAL U0
  CAPTURE UPVAL U1
  CAPTURE VAL R0
  CAPTURE UPVAL U2
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CAPTURE UPVAL U5
  CAPTURE UPVAL U6
  CAPTURE UPVAL U7
  CAPTURE UPVAL U8
  CAPTURE UPVAL U9
  CAPTURE UPVAL U10
  CAPTURE UPVAL U11
  CAPTURE UPVAL U12
  CAPTURE UPVAL U13
  CAPTURE UPVAL U14
  CAPTURE UPVAL U15
  RETURN R1 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [game]
  LOADK R2 K2 ["AssetImportService"]
  NAMECALL R0 R0 K3 ["GetService"]
  CALL R0 2 1
  GETIMPORT R1 K5 [script]
  LOADK R3 K6 ["AssetImporter"]
  NAMECALL R1 R1 K7 ["FindFirstAncestor"]
  CALL R1 2 1
  GETTABLEKS R3 R1 K8 ["Src"]
  GETTABLEKS R2 R3 K9 ["Actions"]
  GETIMPORT R3 K11 [require]
  GETTABLEKS R4 R2 K12 ["ResetState"]
  CALL R3 1 1
  GETIMPORT R4 K11 [require]
  GETTABLEKS R5 R2 K13 ["SetAssetImportSession"]
  CALL R4 1 1
  GETIMPORT R5 K11 [require]
  GETTABLEKS R6 R2 K14 ["SetFilename"]
  CALL R5 1 1
  GETIMPORT R6 K11 [require]
  GETTABLEKS R7 R2 K15 ["SetImportTree"]
  CALL R6 1 1
  GETIMPORT R7 K11 [require]
  GETTABLEKS R8 R2 K16 ["SetSelectedImportItem"]
  CALL R7 1 1
  GETIMPORT R8 K11 [require]
  GETTABLEKS R9 R2 K17 ["SetSessionCount"]
  CALL R8 1 1
  GETIMPORT R9 K11 [require]
  GETTABLEKS R10 R2 K18 ["SetProgress"]
  CALL R9 1 1
  GETIMPORT R10 K11 [require]
  GETTABLEKS R11 R2 K19 ["SetProgressGoal"]
  CALL R10 1 1
  GETIMPORT R11 K11 [require]
  GETTABLEKS R12 R2 K20 ["SetShowProgress"]
  CALL R11 1 1
  GETTABLEKS R13 R1 K8 ["Src"]
  GETTABLEKS R12 R13 K21 ["Thunks"]
  GETIMPORT R13 K11 [require]
  GETTABLEKS R14 R12 K22 ["CreateQueuedSession"]
  CALL R13 1 1
  GETIMPORT R14 K11 [require]
  GETTABLEKS R15 R12 K23 ["LoadManagedGroups"]
  CALL R14 1 1
  GETIMPORT R15 K11 [require]
  GETTABLEKS R16 R12 K24 ["UpdateChecked"]
  CALL R15 1 1
  GETIMPORT R16 K11 [require]
  GETTABLEKS R17 R12 K25 ["UpdatePreviewInstance"]
  CALL R16 1 1
  GETIMPORT R17 K11 [require]
  GETTABLEKS R20 R1 K8 ["Src"]
  GETTABLEKS R19 R20 K26 ["Flags"]
  GETTABLEKS R18 R19 K27 ["getFFlagAssetImportEnableMultipleFiles"]
  CALL R17 1 1
  GETIMPORT R18 K11 [require]
  GETTABLEKS R21 R1 K8 ["Src"]
  GETTABLEKS R20 R21 K26 ["Flags"]
  GETTABLEKS R19 R20 K28 ["getFFlagAssetImportUseUploadId"]
  CALL R18 1 1
  DUPCLOSURE R19 K29 [PROTO_1]
  CAPTURE VAL R17
  CAPTURE VAL R0
  CAPTURE VAL R8
  CAPTURE VAL R9
  CAPTURE VAL R10
  CAPTURE VAL R11
  CAPTURE VAL R13
  CAPTURE VAL R3
  CAPTURE VAL R16
  CAPTURE VAL R6
  CAPTURE VAL R5
  CAPTURE VAL R7
  CAPTURE VAL R15
  CAPTURE VAL R18
  CAPTURE VAL R14
  CAPTURE VAL R4
  RETURN R19 1

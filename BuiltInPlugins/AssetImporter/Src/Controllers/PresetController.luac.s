PROTO_0:
  DUPTABLE R5 K8 [{"lastImportName", "studioDefaultName", "defaultName", "_assetImportServiceWrapper", "_plugin", "_store", "_localization", "_mock"}]
  LOADK R8 K9 ["Templates"]
  LOADK R9 K10 ["LastImported"]
  NAMECALL R6 R2 K11 ["getText"]
  CALL R6 3 1
  SETTABLEKS R6 R5 K0 ["lastImportName"]
  LOADK R8 K9 ["Templates"]
  LOADK R9 K12 ["StudioDefault"]
  NAMECALL R6 R2 K11 ["getText"]
  CALL R6 3 1
  SETTABLEKS R6 R5 K1 ["studioDefaultName"]
  LOADK R6 K13 [""]
  SETTABLEKS R6 R5 K2 ["defaultName"]
  GETGLOBAL R6 K14 ["wrapAssetImportService"]
  MOVE R7 R3
  CALL R6 1 1
  SETTABLEKS R6 R5 K3 ["_assetImportServiceWrapper"]
  SETTABLEKS R0 R5 K4 ["_plugin"]
  SETTABLEKS R1 R5 K5 ["_store"]
  SETTABLEKS R2 R5 K6 ["_localization"]
  SETTABLEKS R3 R5 K7 ["_mock"]
  GETUPVAL R6 0
  FASTCALL2 SETMETATABLE R5 R6 [+3]
  GETIMPORT R4 K16 [setmetatable]
  CALL R4 2 1
  JUMPIF R3 [+14]
  GETTABLEKS R6 R4 K4 ["_plugin"]
  LOADK R8 K17 ["AssetImporter_DefaultPreset"]
  NAMECALL R6 R6 K18 ["GetSetting"]
  CALL R6 2 1
  JUMPIFNOT R6 [+7]
  GETTABLEKS R5 R4 K4 ["_plugin"]
  LOADK R7 K17 ["AssetImporter_DefaultPreset"]
  NAMECALL R5 R5 K18 ["GetSetting"]
  CALL R5 2 1
  JUMP [+2]
  GETTABLEKS R5 R4 K1 ["studioDefaultName"]
  SETTABLEKS R5 R4 K2 ["defaultName"]
  RETURN R4 1

PROTO_1:
  GETUPVAL R3 0
  GETTABLE R2 R3 R1
  JUMPIF R2 [+2]
  NEWTABLE R2 0 0
  RETURN R2 1

PROTO_2:
  GETUPVAL R1 0
  RETURN R1 1

PROTO_3:
  GETUPVAL R2 0
  NEWTABLE R3 0 0
  SETTABLE R3 R2 R1
  LOADB R2 1
  RETURN R2 1

PROTO_4:
  GETUPVAL R2 0
  LOADNIL R3
  SETTABLE R3 R2 R1
  RETURN R0 0

PROTO_5:
  NEWTABLE R0 0 0
  SETUPVAL R0 0
  RETURN R0 0

PROTO_6:
  JUMPIFNOT R0 [+26]
  NEWTABLE R1 0 0
  DUPTABLE R2 K5 [{"GetPreset", "GetAllPresets", "SavePreset", "RemovePreset", "_clearPresets"}]
  NEWCLOSURE R3 P0
  CAPTURE REF R1
  SETTABLEKS R3 R2 K0 ["GetPreset"]
  NEWCLOSURE R3 P1
  CAPTURE REF R1
  SETTABLEKS R3 R2 K1 ["GetAllPresets"]
  NEWCLOSURE R3 P2
  CAPTURE REF R1
  SETTABLEKS R3 R2 K2 ["SavePreset"]
  NEWCLOSURE R3 P3
  CAPTURE REF R1
  SETTABLEKS R3 R2 K3 ["RemovePreset"]
  NEWCLOSURE R3 P4
  CAPTURE REF R1
  SETTABLEKS R3 R2 K4 ["_clearPresets"]
  CLOSEUPVALS R1
  RETURN R2 1
  CLOSEUPVALS R1
  GETIMPORT R1 K7 [game]
  LOADK R3 K8 ["AssetImportService"]
  NAMECALL R1 R1 K9 ["GetService"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_7:
  RETURN R2 1

PROTO_8:
  RETURN R0 0

PROTO_9:
  DUPTABLE R1 K1 [{"getText"}]
  DUPCLOSURE R2 K2 [PROTO_7]
  SETTABLEKS R2 R1 K0 ["getText"]
  DUPTABLE R2 K4 [{"SetSetting"}]
  DUPCLOSURE R3 K5 [PROTO_8]
  SETTABLEKS R3 R2 K3 ["SetSetting"]
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K6 ["new"]
  MOVE R4 R2
  MOVE R5 R0
  MOVE R6 R1
  LOADB R7 1
  CALL R3 4 -1
  RETURN R3 -1

PROTO_10:
  GETTABLEKS R1 R0 K0 ["_mock"]
  JUMPIFNOT R1 [+5]
  GETTABLEKS R1 R0 K1 ["_assetImportServiceWrapper"]
  NAMECALL R1 R1 K2 ["_clearPresets"]
  CALL R1 1 0
  LOADK R1 K3 [""]
  SETTABLEKS R1 R0 K4 ["defaultName"]
  RETURN R0 0

PROTO_11:
  LOADNIL R2
  SETTABLEKS R2 R1 K0 ["Material"]
  MOVE R2 R1
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  LOADNIL R7
  SETTABLEKS R7 R6 K1 ["ImportName"]
  LOADNIL R7
  SETTABLEKS R7 R6 K2 ["ShouldImport"]
  FORGLOOP R2 2 [-7]
  RETURN R0 0

PROTO_12:
  MOVE R4 R1
  NAMECALL R2 R0 K0 ["isStudioDefault"]
  CALL R2 2 1
  JUMPIF R2 [+5]
  MOVE R4 R1
  NAMECALL R2 R0 K1 ["isLastImport"]
  CALL R2 2 1
  JUMPIFNOT R2 [+2]
  LOADB R2 0
  RETURN R2 1
  GETTABLEKS R4 R0 K2 ["_store"]
  NAMECALL R4 R4 K3 ["getState"]
  CALL R4 1 1
  GETTABLEKS R3 R4 K4 ["Preview"]
  GETTABLEKS R2 R3 K5 ["assetImportSession"]
  GETTABLEKS R5 R0 K2 ["_store"]
  NAMECALL R5 R5 K3 ["getState"]
  CALL R5 1 1
  GETTABLEKS R4 R5 K4 ["Preview"]
  GETTABLEKS R3 R4 K6 ["selectedImportItem"]
  MOVE R6 R3
  NAMECALL R4 R2 K7 ["CreatePresetFromData"]
  CALL R4 2 1
  MOVE R7 R4
  NAMECALL R5 R0 K8 ["_filter"]
  CALL R5 2 0
  GETTABLEKS R5 R0 K9 ["_assetImportServiceWrapper"]
  MOVE R7 R1
  NAMECALL R5 R5 K10 ["GetPreset"]
  CALL R5 2 1
  GETUPVAL R6 0
  MOVE R7 R5
  MOVE R8 R4
  CALL R6 2 1
  GETTABLEKS R9 R0 K2 ["_store"]
  NAMECALL R9 R9 K3 ["getState"]
  CALL R9 1 1
  GETTABLEKS R8 R9 K4 ["Preview"]
  GETTABLEKS R7 R8 K11 ["activeQueueItem"]
  JUMPIFNOT R7 [+45]
  GETUPVAL R8 1
  CALL R8 0 1
  JUMPIFNOT R8 [+15]
  GETTABLEKS R8 R0 K2 ["_store"]
  GETUPVAL R10 2
  MOVE R11 R7
  DUPTABLE R12 K14 [{"currentPreset", "settingsChanged"}]
  SETTABLEKS R1 R12 K12 ["currentPreset"]
  LOADB R13 0
  SETTABLEKS R13 R12 K13 ["settingsChanged"]
  CALL R10 2 -1
  NAMECALL R8 R8 K15 ["dispatch"]
  CALL R8 -1 0
  JUMP [+27]
  GETUPVAL R9 3
  GETTABLEKS R8 R9 K16 ["update"]
  MOVE R9 R7
  DUPTABLE R10 K14 [{"currentPreset", "settingsChanged"}]
  SETTABLEKS R1 R10 K12 ["currentPreset"]
  LOADB R11 0
  SETTABLEKS R11 R10 K13 ["settingsChanged"]
  CALL R8 2 1
  GETTABLEKS R9 R0 K2 ["_store"]
  GETUPVAL R11 4
  MOVE R12 R8
  CALL R11 1 -1
  NAMECALL R9 R9 K15 ["dispatch"]
  CALL R9 -1 0
  GETTABLEKS R9 R0 K2 ["_store"]
  GETUPVAL R11 5
  MOVE R12 R8
  CALL R11 1 -1
  NAMECALL R9 R9 K15 ["dispatch"]
  CALL R9 -1 0
  GETTABLEKS R8 R0 K9 ["_assetImportServiceWrapper"]
  MOVE R10 R1
  MOVE R11 R6
  NAMECALL R8 R8 K17 ["SavePreset"]
  CALL R8 3 -1
  RETURN R8 -1

PROTO_13:
  GETTABLEKS R4 R0 K0 ["_store"]
  NAMECALL R4 R4 K1 ["getState"]
  CALL R4 1 1
  GETTABLEKS R3 R4 K2 ["Preview"]
  GETTABLEKS R2 R3 K3 ["assetImportSession"]
  MOVE R5 R1
  NAMECALL R3 R2 K4 ["CreatePresetFromData"]
  CALL R3 2 1
  MOVE R6 R3
  NAMECALL R4 R0 K5 ["_filter"]
  CALL R4 2 0
  GETTABLEKS R4 R0 K6 ["_assetImportServiceWrapper"]
  LOADK R6 K7 ["Last Imported"]
  MOVE R7 R3
  NAMECALL R4 R4 K8 ["SavePreset"]
  CALL R4 3 0
  RETURN R0 0

PROTO_14:
  GETTABLEKS R3 R0 K0 ["_store"]
  NAMECALL R3 R3 K1 ["getState"]
  CALL R3 1 1
  NEWTABLE R4 0 0
  GETTABLEKS R5 R0 K0 ["_store"]
  NAMECALL R6 R5 K1 ["getState"]
  CALL R6 1 3
  FORGPREP R6
  NAMECALL R14 R5 K1 ["getState"]
  CALL R14 1 1
  GETTABLEKS R13 R14 K2 ["Sessions"]
  GETTABLEKS R12 R13 K3 ["sessionQueue"]
  GETTABLE R11 R12 R9
  GETTABLEKS R12 R11 K4 ["currentPreset"]
  JUMPIFNOTEQ R12 R2 [+14]
  GETUPVAL R14 0
  GETUPVAL R16 1
  GETTABLEKS R15 R16 K5 ["update"]
  MOVE R16 R11
  DUPTABLE R17 K6 [{"currentPreset"}]
  SETTABLEKS R1 R17 K4 ["currentPreset"]
  CALL R15 2 -1
  CALL R14 -1 -1
  NAMECALL R12 R5 K7 ["dispatch"]
  CALL R12 -1 0
  FORGLOOP R6 2 [-26]
  GETTABLEKS R7 R3 K8 ["Dialogs"]
  GETTABLEKS R6 R7 K9 ["showPreview"]
  JUMPIFNOT R6 [+14]
  GETTABLEKS R7 R3 K10 ["Preview"]
  GETTABLEKS R6 R7 K11 ["activeQueueItem"]
  GETTABLEKS R7 R0 K0 ["_store"]
  GETUPVAL R9 2
  GETTABLEKS R11 R6 K12 ["filepath"]
  GETTABLE R10 R4 R11
  CALL R9 1 -1
  NAMECALL R7 R7 K7 ["dispatch"]
  CALL R7 -1 0
  RETURN R0 0

PROTO_15:
  GETTABLEKS R3 R0 K0 ["_store"]
  NAMECALL R3 R3 K1 ["getState"]
  CALL R3 1 1
  NEWTABLE R4 0 0
  GETTABLEKS R5 R0 K0 ["_store"]
  NAMECALL R10 R5 K1 ["getState"]
  CALL R10 1 1
  GETTABLEKS R9 R10 K2 ["Sessions"]
  GETTABLEKS R6 R9 K3 ["sessionQueue"]
  LOADNIL R7
  LOADNIL R8
  FORGPREP R6
  NAMECALL R14 R5 K1 ["getState"]
  CALL R14 1 1
  GETTABLEKS R13 R14 K2 ["Sessions"]
  GETTABLEKS R12 R13 K3 ["sessionQueue"]
  GETTABLE R11 R12 R9
  GETTABLEKS R12 R11 K4 ["currentPreset"]
  JUMPIFNOTEQ R12 R1 [+23]
  GETUPVAL R12 0
  CALL R12 0 1
  JUMPIFNOT R12 [+8]
  GETUPVAL R14 1
  MOVE R15 R11
  MOVE R16 R2
  CALL R14 2 -1
  NAMECALL R12 R5 K5 ["dispatch"]
  CALL R12 -1 0
  JUMP [+11]
  GETUPVAL R14 2
  GETUPVAL R16 3
  GETTABLEKS R15 R16 K6 ["update"]
  MOVE R16 R11
  MOVE R17 R2
  CALL R15 2 -1
  CALL R14 -1 -1
  NAMECALL R12 R5 K5 ["dispatch"]
  CALL R12 -1 0
  FORGLOOP R6 2 [-35]
  GETTABLEKS R7 R3 K7 ["Dialogs"]
  GETTABLEKS R6 R7 K8 ["showPreview"]
  JUMPIFNOT R6 [+14]
  GETTABLEKS R7 R3 K9 ["Preview"]
  GETTABLEKS R6 R7 K10 ["activeQueueItem"]
  GETTABLEKS R7 R0 K0 ["_store"]
  GETUPVAL R9 4
  GETTABLEKS R11 R6 K11 ["filepath"]
  GETTABLE R10 R4 R11
  CALL R9 1 -1
  NAMECALL R7 R7 K5 ["dispatch"]
  CALL R7 -1 0
  RETURN R0 0

PROTO_16:
  GETTABLEKS R3 R0 K0 ["_assetImportServiceWrapper"]
  MOVE R6 R2
  NAMECALL R4 R3 K1 ["GetPreset"]
  CALL R4 2 1
  MOVE R7 R2
  NAMECALL R5 R3 K2 ["RemovePreset"]
  CALL R5 2 0
  MOVE R7 R1
  MOVE R8 R4
  NAMECALL R5 R3 K3 ["SavePreset"]
  CALL R5 3 0
  GETTABLEKS R5 R0 K4 ["defaultName"]
  JUMPIFNOTEQ R5 R2 [+10]
  GETTABLEKS R5 R0 K5 ["_plugin"]
  LOADK R7 K6 ["AssetImporter_DefaultPreset"]
  MOVE R8 R1
  NAMECALL R5 R5 K7 ["SetSetting"]
  CALL R5 3 0
  SETTABLEKS R1 R0 K4 ["defaultName"]
  MOVE R7 R2
  DUPTABLE R8 K9 [{"currentPreset"}]
  SETTABLEKS R1 R8 K8 ["currentPreset"]
  NAMECALL R5 R0 K10 ["_updateQueueItems"]
  CALL R5 3 0
  RETURN R0 0

PROTO_17:
  GETTABLEKS R4 R0 K0 ["_store"]
  NAMECALL R4 R4 K1 ["getState"]
  CALL R4 1 1
  GETTABLEKS R3 R4 K2 ["Preview"]
  GETTABLEKS R2 R3 K3 ["currentPreset"]
  GETTABLEKS R3 R0 K4 ["_assetImportServiceWrapper"]
  MOVE R6 R2
  NAMECALL R4 R3 K5 ["GetPreset"]
  CALL R4 2 1
  MOVE R7 R2
  NAMECALL R5 R3 K6 ["RemovePreset"]
  CALL R5 2 0
  MOVE R7 R1
  MOVE R8 R4
  NAMECALL R5 R3 K7 ["SavePreset"]
  CALL R5 3 0
  GETTABLEKS R5 R0 K8 ["defaultName"]
  JUMPIFNOTEQ R5 R2 [+10]
  GETTABLEKS R5 R0 K9 ["_plugin"]
  LOADK R7 K10 ["AssetImporter_DefaultPreset"]
  MOVE R8 R1
  NAMECALL R5 R5 K11 ["SetSetting"]
  CALL R5 3 0
  SETTABLEKS R1 R0 K8 ["defaultName"]
  GETTABLEKS R5 R0 K0 ["_store"]
  GETUPVAL R7 0
  MOVE R8 R1
  CALL R7 1 -1
  NAMECALL R5 R5 K12 ["dispatch"]
  CALL R5 -1 0
  RETURN R0 0

PROTO_18:
  GETTABLEKS R2 R0 K0 ["_assetImportServiceWrapper"]
  MOVE R4 R1
  NAMECALL R2 R2 K1 ["RemovePreset"]
  CALL R2 2 0
  GETTABLEKS R2 R0 K2 ["defaultName"]
  JUMPIFNOTEQ R1 R2 [+5]
  GETTABLEKS R2 R0 K3 ["studioDefaultName"]
  SETTABLEKS R2 R0 K2 ["defaultName"]
  MOVE R4 R1
  DUPTABLE R5 K6 [{"currentPreset", "settingsChanged"}]
  GETTABLEKS R6 R0 K3 ["studioDefaultName"]
  SETTABLEKS R6 R5 K4 ["currentPreset"]
  LOADB R6 1
  SETTABLEKS R6 R5 K5 ["settingsChanged"]
  NAMECALL R2 R0 K7 ["_updateQueueItems"]
  CALL R2 3 0
  RETURN R0 0

PROTO_19:
  GETTABLEKS R2 R0 K0 ["_plugin"]
  LOADK R4 K1 ["AssetImporter_DefaultPreset"]
  MOVE R5 R1
  NAMECALL R2 R2 K2 ["SetSetting"]
  CALL R2 3 0
  SETTABLEKS R1 R0 K3 ["defaultName"]
  RETURN R0 0

PROTO_20:
  LOADB R1 0
  GETTABLEKS R2 R0 K0 ["defaultName"]
  JUMPIFEQKS R2 K1 [""] [+9]
  GETTABLEKS R2 R0 K0 ["defaultName"]
  GETTABLEKS R3 R0 K2 ["studioDefaultName"]
  JUMPIFNOTEQ R2 R3 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_21:
  GETTABLEKS R6 R0 K0 ["_store"]
  NAMECALL R6 R6 K1 ["getState"]
  CALL R6 1 1
  GETTABLEKS R5 R6 K2 ["Sessions"]
  GETTABLEKS R4 R5 K3 ["sessionQueue"]
  MOVE R5 R4
  LOADNIL R6
  LOADNIL R7
  FORGPREP R5
  GETUPVAL R10 0
  CALL R10 0 1
  JUMPIFNOT R10 [+32]
  GETTABLEKS R10 R9 K4 ["state"]
  GETUPVAL R13 1
  GETTABLEKS R12 R13 K5 ["SessionState"]
  GETTABLEKS R11 R12 K6 ["Parsed"]
  JUMPIFNOTEQ R10 R11 [+35]
  MOVE R10 R2
  GETTABLEKS R11 R9 K7 ["filepath"]
  MOVE R12 R3
  CALL R10 2 0
  GETTABLEKS R12 R0 K0 ["_store"]
  NAMECALL R12 R12 K1 ["getState"]
  CALL R12 1 1
  GETTABLEKS R11 R12 K2 ["Sessions"]
  GETTABLEKS R10 R11 K3 ["sessionQueue"]
  GETTABLEKS R11 R9 K7 ["filepath"]
  GETTABLE R9 R10 R11
  MOVE R12 R1
  MOVE R13 R9
  NAMECALL R10 R0 K8 ["resolvePreset"]
  CALL R10 3 0
  JUMP [+11]
  GETTABLEKS R10 R9 K9 ["validSession"]
  JUMPIFNOT R10 [+8]
  GETTABLEKS R10 R9 K10 ["uploaded"]
  JUMPIF R10 [+5]
  MOVE R12 R1
  MOVE R13 R9
  NAMECALL R10 R0 K8 ["resolvePreset"]
  CALL R10 3 0
  FORGLOOP R5 2 [-47]
  RETURN R0 0

PROTO_22:
  GETTABLEKS R4 R0 K0 ["_store"]
  NAMECALL R4 R4 K1 ["getState"]
  CALL R4 1 1
  GETTABLEKS R3 R4 K2 ["Dialogs"]
  GETTABLEKS R2 R3 K3 ["showPreview"]
  JUMPIF R2 [+1]
  RETURN R0 0
  NAMECALL R2 R1 K4 ["CreatePresetFromData"]
  CALL R2 1 1
  MOVE R3 R2
  LOADNIL R4
  LOADNIL R5
  FORGPREP R3
  LOADNIL R8
  SETTABLEKS R8 R7 K5 ["ImportName"]
  FORGLOOP R3 2 [-4]
  GETTABLEKS R5 R0 K0 ["_store"]
  NAMECALL R5 R5 K1 ["getState"]
  CALL R5 1 1
  GETTABLEKS R4 R5 K6 ["Preview"]
  GETTABLEKS R3 R4 K7 ["activeQueueItem"]
  GETTABLEKS R4 R3 K8 ["session"]
  MOVE R6 R2
  NAMECALL R4 R4 K9 ["ApplyPreset"]
  CALL R4 2 0
  GETUPVAL R4 0
  CALL R4 0 1
  JUMPIFNOT R4 [+11]
  GETTABLEKS R4 R0 K0 ["_store"]
  GETUPVAL R6 1
  MOVE R7 R3
  NEWTABLE R8 0 0
  CALL R6 2 -1
  NAMECALL R4 R4 K10 ["dispatch"]
  CALL R4 -1 0
  RETURN R0 0
  GETTABLEKS R4 R0 K0 ["_store"]
  GETUPVAL R6 2
  MOVE R7 R3
  CALL R6 1 -1
  NAMECALL R4 R4 K10 ["dispatch"]
  CALL R4 -1 0
  RETURN R0 0

PROTO_23:
  GETTABLEKS R3 R0 K0 ["defaultName"]
  JUMPIFEQ R3 R1 [+2]
  LOADB R2 0 +1
  LOADB R2 1
  RETURN R2 1

PROTO_24:
  LOADB R2 1
  GETTABLEKS R3 R0 K0 ["lastImportName"]
  JUMPIFEQ R3 R1 [+5]
  JUMPIFEQKS R1 K1 ["Last Imported"] [+2]
  LOADB R2 0 +1
  LOADB R2 1
  RETURN R2 1

PROTO_25:
  LOADB R2 1
  GETTABLEKS R3 R0 K0 ["studioDefaultName"]
  JUMPIFEQ R3 R1 [+5]
  JUMPIFEQKS R1 K1 ["Studio Default"] [+2]
  LOADB R2 0 +1
  LOADB R2 1
  RETURN R2 1

PROTO_26:
  NAMECALL R2 R1 K0 ["GetImportTree"]
  CALL R2 1 1
  NEWTABLE R3 1 0
  GETTABLEKS R4 R2 K1 ["ImportName"]
  SETTABLE R4 R3 R2
  NAMECALL R4 R2 K2 ["GetDescendants"]
  CALL R4 1 3
  FORGPREP R4
  GETTABLEKS R9 R8 K1 ["ImportName"]
  SETTABLE R9 R3 R8
  FORGLOOP R4 2 [-4]
  NAMECALL R4 R1 K3 ["Reset"]
  CALL R4 1 0
  GETTABLE R4 R3 R2
  SETTABLEKS R4 R2 K1 ["ImportName"]
  NAMECALL R4 R1 K0 ["GetImportTree"]
  CALL R4 1 1
  NAMECALL R4 R4 K2 ["GetDescendants"]
  CALL R4 1 3
  FORGPREP R4
  GETTABLE R9 R3 R8
  SETTABLEKS R9 R8 K1 ["ImportName"]
  FORGLOOP R4 2 [-4]
  RETURN R0 0

PROTO_27:
  LOADNIL R3
  LOADNIL R4
  GETUPVAL R5 0
  CALL R5 0 1
  JUMPIFNOT R5 [+8]
  DUPTABLE R5 K2 [{"currentPreset", "settingsChanged"}]
  SETTABLEKS R1 R5 K0 ["currentPreset"]
  LOADB R6 0
  SETTABLEKS R6 R5 K1 ["settingsChanged"]
  MOVE R4 R5
  JUMP [+12]
  GETUPVAL R6 1
  GETTABLEKS R5 R6 K3 ["update"]
  MOVE R6 R2
  DUPTABLE R7 K2 [{"currentPreset", "settingsChanged"}]
  SETTABLEKS R1 R7 K0 ["currentPreset"]
  LOADB R8 0
  SETTABLEKS R8 R7 K1 ["settingsChanged"]
  CALL R5 2 1
  MOVE R3 R5
  GETTABLEKS R5 R2 K4 ["session"]
  JUMPIFNOT R5 [+101]
  NAMECALL R6 R5 K5 ["GetImportTree"]
  CALL R6 1 1
  JUMPIFNOT R6 [+97]
  MOVE R8 R5
  NAMECALL R6 R0 K6 ["reset"]
  CALL R6 2 0
  MOVE R9 R1
  NAMECALL R7 R0 K7 ["isLastImport"]
  CALL R7 2 1
  JUMPIFNOT R7 [+8]
  GETTABLEKS R6 R0 K8 ["_assetImportServiceWrapper"]
  GETTABLEKS R8 R0 K9 ["lastImportName"]
  NAMECALL R6 R6 K10 ["GetPreset"]
  CALL R6 2 1
  JUMP [+6]
  GETTABLEKS R6 R0 K8 ["_assetImportServiceWrapper"]
  MOVE R8 R1
  NAMECALL R6 R6 K10 ["GetPreset"]
  CALL R6 2 1
  MOVE R9 R6
  NAMECALL R7 R5 K11 ["ApplyPreset"]
  CALL R7 2 0
  GETTABLEKS R9 R0 K12 ["_store"]
  NAMECALL R9 R9 K13 ["getState"]
  CALL R9 1 1
  GETTABLEKS R8 R9 K14 ["Dialogs"]
  GETTABLEKS R7 R8 K15 ["showPreview"]
  JUMPIFNOT R7 [+60]
  NEWTABLE R7 0 0
  GETTABLEKS R10 R0 K12 ["_store"]
  NAMECALL R10 R10 K13 ["getState"]
  CALL R10 1 1
  GETTABLEKS R9 R10 K16 ["Preview"]
  GETTABLEKS R8 R9 K17 ["importDataChecked"]
  MOVE R9 R8
  LOADNIL R10
  LOADNIL R11
  FORGPREP R9
  GETTABLEKS R14 R12 K18 ["ShouldImport"]
  SETTABLE R14 R7 R12
  FORGLOOP R9 2 [-4]
  GETTABLEKS R9 R0 K12 ["_store"]
  GETUPVAL R11 2
  MOVE R12 R7
  CALL R11 1 -1
  NAMECALL R9 R9 K19 ["dispatch"]
  CALL R9 -1 0
  GETTABLEKS R11 R0 K12 ["_store"]
  NAMECALL R11 R11 K13 ["getState"]
  CALL R11 1 1
  GETTABLEKS R10 R11 K16 ["Preview"]
  GETTABLEKS R9 R10 K20 ["selectedImportItem"]
  JUMPIFNOT R9 [+11]
  NAMECALL R10 R9 K21 ["GetPreview"]
  CALL R10 1 1
  GETTABLEKS R11 R0 K12 ["_store"]
  GETUPVAL R13 3
  MOVE R14 R10
  CALL R13 1 -1
  NAMECALL R11 R11 K19 ["dispatch"]
  CALL R11 -1 0
  GETUPVAL R10 0
  CALL R10 0 1
  JUMPIF R10 [+8]
  GETTABLEKS R10 R0 K12 ["_store"]
  GETUPVAL R12 4
  MOVE R13 R3
  CALL R12 1 -1
  NAMECALL R10 R10 K19 ["dispatch"]
  CALL R10 -1 0
  GETUPVAL R6 0
  CALL R6 0 1
  JUMPIFNOT R6 [+10]
  GETTABLEKS R6 R0 K12 ["_store"]
  GETUPVAL R8 5
  MOVE R9 R2
  MOVE R10 R4
  CALL R8 2 -1
  NAMECALL R6 R6 K19 ["dispatch"]
  CALL R6 -1 0
  RETURN R0 0
  GETTABLEKS R6 R0 K12 ["_store"]
  GETUPVAL R8 6
  MOVE R9 R3
  CALL R8 1 -1
  NAMECALL R6 R6 K19 ["dispatch"]
  CALL R6 -1 0
  RETURN R0 0

PROTO_28:
  GETUPVAL R2 0
  JUMPIFEQ R0 R2 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_29:
  GETUPVAL R3 0
  NAMECALL R4 R0 K0 ["getAllPresets"]
  CALL R4 1 -1
  CALL R3 -1 1
  JUMPIFNOT R2 [+3]
  GETTABLEKS R4 R2 K1 ["settingsChanged"]
  JUMP [+1]
  LOADB R4 0
  GETUPVAL R5 1
  MOVE R6 R3
  NEWCLOSURE R7 P0
  CAPTURE VAL R1
  CALL R5 2 1
  JUMPIFNOT R5 [+5]
  JUMPIFNOT R4 [+4]
  MOVE R7 R1
  LOADK R8 K2 ["*"]
  CONCAT R6 R7 R8
  SETTABLE R6 R3 R5
  RETURN R3 1

PROTO_30:
  GETTABLEKS R1 R0 K0 ["_assetImportServiceWrapper"]
  NAMECALL R1 R1 K1 ["GetAllPresets"]
  CALL R1 1 1
  GETTABLEKS R2 R1 K2 ["Last Imported"]
  JUMPIFNOT R2 [+8]
  GETTABLEKS R2 R1 K2 ["Last Imported"]
  LOADNIL R3
  SETTABLEKS R3 R1 K2 ["Last Imported"]
  GETTABLEKS R3 R0 K3 ["lastImportName"]
  SETTABLE R2 R1 R3
  GETTABLEKS R2 R0 K4 ["studioDefaultName"]
  NEWTABLE R3 0 0
  SETTABLE R3 R1 R2
  RETURN R1 1

PROTO_31:
  MOVE R2 R1
  LOADNIL R3
  LOADNIL R4
  FORGPREP R2
  GETTABLEKS R10 R0 K0 ["_store"]
  NAMECALL R10 R10 K1 ["getState"]
  CALL R10 1 1
  GETTABLEKS R9 R10 K2 ["Sessions"]
  GETTABLEKS R8 R9 K3 ["sessionQueue"]
  GETTABLE R7 R8 R6
  GETUPVAL R8 0
  CALL R8 0 1
  JUMPIFNOT R8 [+16]
  GETTABLEKS R8 R7 K4 ["state"]
  GETUPVAL R11 1
  GETTABLEKS R10 R11 K5 ["SessionState"]
  GETTABLEKS R9 R10 K6 ["Invalid"]
  JUMPIFEQ R8 R9 [+17]
  GETTABLEKS R10 R0 K7 ["defaultName"]
  MOVE R11 R7
  NAMECALL R8 R0 K8 ["resolvePreset"]
  CALL R8 3 0
  JUMP [+9]
  GETTABLEKS R8 R7 K9 ["validSession"]
  JUMPIFNOT R8 [+6]
  GETTABLEKS R10 R0 K7 ["defaultName"]
  MOVE R11 R7
  NAMECALL R8 R0 K8 ["resolvePreset"]
  CALL R8 3 0
  FORGLOOP R2 2 [-39]
  RETURN R0 0

PROTO_32:
  NAMECALL R1 R0 K0 ["getAllPresets"]
  CALL R1 1 1
  GETTABLEKS R4 R0 K1 ["lastImportName"]
  GETTABLE R3 R1 R4
  JUMPIFNOT R3 [+2]
  LOADN R2 1
  JUMP [+1]
  LOADN R2 0
  GETUPVAL R5 0
  CALL R5 0 1
  GETUPVAL R7 1
  MOVE R8 R1
  CALL R7 1 1
  LENGTH R6 R7
  SUB R4 R5 R6
  ADD R3 R4 R2
  RETURN R3 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AssetImporter"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Src"]
  GETTABLEKS R2 R3 K7 ["Types"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K8 ["Packages"]
  GETTABLEKS R3 R4 K9 ["Framework"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R5 R0 K8 ["Packages"]
  GETTABLEKS R4 R5 K10 ["Dash"]
  CALL R3 1 1
  GETTABLEKS R5 R2 K11 ["ContextServices"]
  GETTABLEKS R4 R5 K12 ["ContextItem"]
  GETTABLEKS R5 R3 K13 ["join"]
  GETTABLEKS R6 R3 K14 ["keys"]
  GETTABLEKS R7 R3 K15 ["findIndex"]
  GETIMPORT R8 K5 [require]
  GETTABLEKS R11 R0 K6 ["Src"]
  GETTABLEKS R10 R11 K16 ["Actions"]
  GETTABLEKS R9 R10 K17 ["SetActiveQueueItem"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETTABLEKS R12 R0 K6 ["Src"]
  GETTABLEKS R11 R12 K16 ["Actions"]
  GETTABLEKS R10 R11 K18 ["SetCurrentPreset"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R13 R0 K6 ["Src"]
  GETTABLEKS R12 R13 K19 ["Thunks"]
  GETTABLEKS R11 R12 K20 ["DEPRECATED_UpdateSessionInfo"]
  CALL R10 1 1
  GETIMPORT R11 K5 [require]
  GETTABLEKS R14 R0 K6 ["Src"]
  GETTABLEKS R13 R14 K19 ["Thunks"]
  GETTABLEKS R12 R13 K21 ["UpdateQueueItem"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETTABLEKS R15 R0 K6 ["Src"]
  GETTABLEKS R14 R15 K19 ["Thunks"]
  GETTABLEKS R13 R14 K22 ["UpdateChecked"]
  CALL R12 1 1
  GETIMPORT R13 K5 [require]
  GETTABLEKS R16 R0 K6 ["Src"]
  GETTABLEKS R15 R16 K19 ["Thunks"]
  GETTABLEKS R14 R15 K23 ["UpdatePreviewInstance"]
  CALL R13 1 1
  GETIMPORT R14 K5 [require]
  GETTABLEKS R16 R0 K6 ["Src"]
  GETTABLEKS R15 R16 K7 ["Types"]
  CALL R14 1 1
  GETIMPORT R15 K5 [require]
  GETTABLEKS R18 R0 K6 ["Src"]
  GETTABLEKS R17 R18 K7 ["Types"]
  GETTABLEKS R16 R17 K24 ["QueuedSession"]
  CALL R15 1 1
  GETIMPORT R16 K5 [require]
  GETTABLEKS R19 R0 K6 ["Src"]
  GETTABLEKS R18 R19 K25 ["Flags"]
  GETTABLEKS R17 R18 K26 ["getFIntAssetImportMaxPresetCount"]
  CALL R16 1 1
  GETIMPORT R17 K5 [require]
  GETTABLEKS R20 R0 K6 ["Src"]
  GETTABLEKS R19 R20 K25 ["Flags"]
  GETTABLEKS R18 R19 K27 ["getFFlagImportQueueSessionThunksRefactor"]
  CALL R17 1 1
  GETIMPORT R18 K5 [require]
  GETTABLEKS R21 R0 K6 ["Src"]
  GETTABLEKS R20 R21 K25 ["Flags"]
  GETTABLEKS R19 R20 K28 ["getFFlagImportQueuePerSessionState"]
  CALL R18 1 1
  LOADK R21 K29 ["PresetController"]
  NAMECALL R19 R4 K30 ["extend"]
  CALL R19 2 1
  DUPCLOSURE R20 K31 [PROTO_0]
  CAPTURE VAL R19
  SETTABLEKS R20 R19 K32 ["new"]
  DUPCLOSURE R20 K33 [PROTO_6]
  SETGLOBAL R20 K34 ["wrapAssetImportService"]
  DUPCLOSURE R20 K35 [PROTO_9]
  CAPTURE VAL R19
  SETTABLEKS R20 R19 K36 ["mock"]
  DUPCLOSURE R20 K37 [PROTO_10]
  SETTABLEKS R20 R19 K38 ["destroy"]
  DUPCLOSURE R20 K39 [PROTO_11]
  SETTABLEKS R20 R19 K40 ["_filter"]
  DUPCLOSURE R20 K41 [PROTO_12]
  CAPTURE VAL R5
  CAPTURE VAL R17
  CAPTURE VAL R11
  CAPTURE VAL R15
  CAPTURE VAL R10
  CAPTURE VAL R8
  SETTABLEKS R20 R19 K42 ["save"]
  DUPCLOSURE R20 K43 [PROTO_13]
  SETTABLEKS R20 R19 K44 ["createPresetFromLastImport"]
  MOVE R20 R17
  CALL R20 0 1
  JUMPIF R20 [+6]
  DUPCLOSURE R20 K45 [PROTO_14]
  CAPTURE VAL R10
  CAPTURE VAL R15
  CAPTURE VAL R8
  SETTABLEKS R20 R19 K46 ["DEPRECATED_updateQueueItems"]
  DUPCLOSURE R20 K47 [PROTO_15]
  CAPTURE VAL R17
  CAPTURE VAL R11
  CAPTURE VAL R10
  CAPTURE VAL R15
  CAPTURE VAL R8
  SETTABLEKS R20 R19 K48 ["_updateQueueItems"]
  DUPCLOSURE R20 K49 [PROTO_16]
  SETTABLEKS R20 R19 K50 ["rename"]
  DUPCLOSURE R20 K51 [PROTO_17]
  CAPTURE VAL R9
  SETTABLEKS R20 R19 K52 ["DEPRECATED_rename"]
  DUPCLOSURE R20 K53 [PROTO_18]
  SETTABLEKS R20 R19 K54 ["delete"]
  DUPCLOSURE R20 K55 [PROTO_19]
  SETTABLEKS R20 R19 K56 ["setDefault"]
  DUPCLOSURE R20 K57 [PROTO_20]
  SETTABLEKS R20 R19 K58 ["hasCustomDefault"]
  DUPCLOSURE R20 K59 [PROTO_21]
  CAPTURE VAL R18
  CAPTURE VAL R14
  SETTABLEKS R20 R19 K60 ["applyPresetToAllSessions"]
  DUPCLOSURE R20 K61 [PROTO_22]
  CAPTURE VAL R17
  CAPTURE VAL R11
  CAPTURE VAL R10
  SETTABLEKS R20 R19 K62 ["applyImportDataToSession"]
  DUPCLOSURE R20 K63 [PROTO_23]
  SETTABLEKS R20 R19 K64 ["isDefault"]
  DUPCLOSURE R20 K65 [PROTO_24]
  SETTABLEKS R20 R19 K66 ["isLastImport"]
  DUPCLOSURE R20 K67 [PROTO_25]
  SETTABLEKS R20 R19 K68 ["isStudioDefault"]
  DUPCLOSURE R20 K69 [PROTO_26]
  SETTABLEKS R20 R19 K70 ["reset"]
  DUPCLOSURE R20 K71 [PROTO_27]
  CAPTURE VAL R17
  CAPTURE VAL R15
  CAPTURE VAL R12
  CAPTURE VAL R13
  CAPTURE VAL R8
  CAPTURE VAL R11
  CAPTURE VAL R10
  SETTABLEKS R20 R19 K72 ["resolvePreset"]
  DUPCLOSURE R20 K73 [PROTO_29]
  CAPTURE VAL R6
  CAPTURE VAL R7
  SETTABLEKS R20 R19 K74 ["getNameList"]
  DUPCLOSURE R20 K75 [PROTO_30]
  SETTABLEKS R20 R19 K76 ["getAllPresets"]
  DUPCLOSURE R20 K77 [PROTO_31]
  CAPTURE VAL R18
  CAPTURE VAL R14
  SETTABLEKS R20 R19 K78 ["initializePresets"]
  DUPCLOSURE R20 K79 [PROTO_32]
  CAPTURE VAL R16
  CAPTURE VAL R6
  SETTABLEKS R20 R19 K80 ["getNumPresetsRemaining"]
  RETURN R19 1

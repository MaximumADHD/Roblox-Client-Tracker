PROTO_0:
  GETUPVAL R0 0
  LOADB R2 0
  NAMECALL R0 R0 K0 ["setVisible"]
  CALL R0 2 0
  LOADNIL R0
  SETUPVAL R0 1
  RETURN R0 0

PROTO_1:
  GETUPVAL R1 0
  NAMECALL R1 R1 K0 ["getFilteredSelection"]
  CALL R1 1 1
  LENGTH R2 R1
  JUMPIFEQKN R2 K1 [1] [+9]
  GETUPVAL R2 1
  LOADB R4 0
  NAMECALL R2 R2 K2 ["setVisible"]
  CALL R2 2 0
  LOADNIL R2
  SETUPVAL R2 2
  RETURN R0 0
  GETTABLEN R2 R1 1
  GETTABLEKS R3 R2 K3 ["Parent"]
  LOADK R6 K4 ["GuiBase2d"]
  NAMECALL R4 R3 K5 ["IsA"]
  CALL R4 2 1
  JUMPIF R4 [+8]
  GETUPVAL R4 1
  LOADB R6 0
  NAMECALL R4 R4 K2 ["setVisible"]
  CALL R4 2 0
  LOADNIL R4
  SETUPVAL R4 2
  RETURN R0 0
  SETUPVAL R2 2
  GETUPVAL R4 1
  NAMECALL R4 R4 K6 ["update"]
  CALL R4 1 0
  GETUPVAL R4 1
  LOADB R6 1
  NAMECALL R4 R4 K2 ["setVisible"]
  CALL R4 2 0
  RETURN R0 0

PROTO_2:
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K0 ["new"]
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K1 ["X"]
  CALL R1 1 1
  SETUPVAL R1 0
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K0 ["new"]
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K2 ["Y"]
  CALL R1 1 1
  SETUPVAL R1 3
  RETURN R0 0

PROTO_3:
  GETUPVAL R1 0
  NAMECALL R1 R1 K0 ["destroy"]
  CALL R1 1 0
  GETUPVAL R1 1
  NAMECALL R1 R1 K0 ["destroy"]
  CALL R1 1 0
  LOADNIL R1
  SETUPVAL R1 0
  LOADNIL R1
  SETUPVAL R1 1
  RETURN R0 0

PROTO_4:
  GETUPVAL R1 0
  JUMPIFNOTEQKNIL R1 [+2]
  RETURN R0 0
  GETUPVAL R1 1
  GETUPVAL R3 0
  NAMECALL R1 R1 K0 ["update"]
  CALL R1 2 0
  GETUPVAL R1 2
  GETUPVAL R3 0
  NAMECALL R1 R1 K0 ["update"]
  CALL R1 2 0
  RETURN R0 0

PROTO_5:
  GETUPVAL R2 0
  JUMPIF R2 [+1]
  RETURN R0 0
  GETUPVAL R2 1
  JUMPIFNOT R2 [+5]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["Parent"]
  JUMPIF R2 [+1]
  RETURN R0 0
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K1 ["AbsoluteRotation"]
  FASTCALL1 MATH_ABS R4 [+2]
  GETIMPORT R3 K4 [math.abs]
  CALL R3 1 1
  LOADK R4 K5 [0.001]
  JUMPIFLE R4 R3 [+2]
  LOADB R2 0 +1
  LOADB R2 1
  JUMPIFNOT R1 [+12]
  JUMPIFNOT R2 [+11]
  GETUPVAL R3 2
  LOADB R5 0
  NAMECALL R3 R3 K6 ["setVisible"]
  CALL R3 2 0
  GETUPVAL R3 3
  LOADB R5 0
  NAMECALL R3 R3 K6 ["setVisible"]
  CALL R3 2 0
  RETURN R0 0
  GETUPVAL R3 2
  MOVE R5 R1
  NAMECALL R3 R3 K6 ["setVisible"]
  CALL R3 2 0
  GETUPVAL R3 3
  MOVE R5 R1
  NAMECALL R3 R3 K6 ["setVisible"]
  CALL R3 2 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  NEWTABLE R0 8 0
  GETIMPORT R1 K1 [require]
  GETIMPORT R5 K3 [script]
  GETTABLEKS R4 R5 K4 ["Parent"]
  GETTABLEKS R3 R4 K5 ["Enum"]
  GETTABLEKS R2 R3 K6 ["Axis"]
  CALL R1 1 1
  GETIMPORT R2 K1 [require]
  GETIMPORT R5 K3 [script]
  GETTABLEKS R4 R5 K4 ["Parent"]
  GETTABLEKS R3 R4 K7 ["DistanceLine"]
  CALL R2 1 1
  GETIMPORT R3 K1 [require]
  GETIMPORT R6 K3 [script]
  GETTABLEKS R5 R6 K4 ["Parent"]
  GETTABLEKS R4 R5 K8 ["Log"]
  CALL R3 1 1
  GETIMPORT R4 K1 [require]
  GETIMPORT R7 K3 [script]
  GETTABLEKS R6 R7 K4 ["Parent"]
  GETTABLEKS R5 R6 K9 ["SelectionManager"]
  CALL R4 1 1
  LOADNIL R5
  LOADNIL R6
  LOADNIL R7
  GETIMPORT R8 K11 [game]
  LOADK R10 K12 ["UIEditorDistanceLinesDontUpdateForUnparentedObject"]
  LOADB R11 0
  NAMECALL R8 R8 K13 ["DefineFastFlag"]
  CALL R8 3 1
  NEWCLOSURE R9 P0
  CAPTURE VAL R0
  CAPTURE REF R5
  NEWCLOSURE R10 P1
  CAPTURE VAL R4
  CAPTURE VAL R0
  CAPTURE REF R5
  SETTABLEKS R10 R0 K14 ["onSelectionChanged"]
  NEWCLOSURE R10 P2
  CAPTURE REF R6
  CAPTURE VAL R2
  CAPTURE VAL R1
  CAPTURE REF R7
  SETTABLEKS R10 R0 K15 ["On"]
  NEWCLOSURE R10 P3
  CAPTURE REF R6
  CAPTURE REF R7
  SETTABLEKS R10 R0 K16 ["Off"]
  NEWCLOSURE R10 P4
  CAPTURE REF R5
  CAPTURE REF R6
  CAPTURE REF R7
  SETTABLEKS R10 R0 K17 ["update"]
  NEWCLOSURE R10 P5
  CAPTURE REF R5
  CAPTURE VAL R8
  CAPTURE REF R6
  CAPTURE REF R7
  SETTABLEKS R10 R0 K18 ["setVisible"]
  CLOSEUPVALS R5
  RETURN R0 1

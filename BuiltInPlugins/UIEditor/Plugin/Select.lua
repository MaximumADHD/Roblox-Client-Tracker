...
local function calculateVisibleBounds(instance)
	...
	while clipper do
		if clipper.ClipsDescendants then
			...
			upperLeft =
				Vector2.new(math.max(upperLeft.X, clipperUpperLeft.X), math.max(upperLeft.Y, clipperUpperLeft.Y))
			lowerRight =
				Vector2.new(math.min(lowerRight.X, clipperLowerRight.X), math.min(lowerRight.Y, clipperLowerRight.Y))
		end
...
local function doesPointExistInInstance(point, instance)
	...
	return point.x >= upperLeft.x
		and point.x <= upperLeft.x + sizeBuffer.x
		and point.y >= upperLeft.y
		and point.y <= upperLeft.y + sizeBuffer.y
end
...
function Select:selectAllObjectsInBounds(point1, point2)
	...
	for i = 1, #paintOrder do
		if
			paintOrder[i].AbsolutePosition.X >= min.X
			and paintOrder[i].AbsolutePosition.Y >= min.Y
			and paintOrder[i].AbsolutePosition.X + paintOrder[i].AbsoluteSize.X <= max.X
			and paintOrder[i].AbsolutePosition.Y + paintOrder[i].AbsoluteSize.Y <= max.Y
		then

...
local function retainAspectRatio(extents, mouseLocation, hasSnappedX, hasSnappedY)
	...
	if m_draggingHandle == HANDLE_E or m_draggingHandle == HANDLE_W then
		local height = extents.Width / m_originalAspectRatio
		extents:expandFromCenter(Vector2.new(0, height - extents.Height))
	elseif m_draggingHandle == HANDLE_N or m_draggingHandle == HANDLE_S then
	...
	elseif m_draggingHandle == HANDLE_NE then
		...
		if isCloserX then
			extents.TopRight = extents.BottomLeft
				+ (Vector2.new(extents.Width, extents.Width / m_originalAspectRatio) * multiplier)
		else
			extents.TopRight = extents.BottomLeft
				+ (Vector2.new(m_originalAspectRatio * extents.Height, extents.Height) * multiplier)
		end
	elseif m_draggingHandle == HANDLE_SW then
		...
		if isCloserX then
			extents.BottomLeft = extents.TopRight
				+ (Vector2.new(extents.Width, extents.Width / m_originalAspectRatio) * multiplier)
		else
			extents.BottomLeft = extents.TopRight
				+ (Vector2.new(m_originalAspectRatio * extents.Height, extents.Height) * multiplier)
	...
	elseif m_draggingHandle == HANDLE_NW then
		...
		if (isCloserX or snapXOnly) and not snapYOnly then
			extents.TopLeft = extents.BottomRight
				+ (Vector2.new(extents.Width, extents.Width / m_originalAspectRatio) * multiplier)
		else
			extents.TopLeft = extents.BottomRight
				+ (Vector2.new(m_originalAspectRatio * extents.Height, extents.Height) * multiplier)
...
function Resize:updateDrag(location)
	if not m_currentlyDragging then
		return
	end
	...
	if m_draggingHandle == HANDLE_NW or m_draggingHandle == HANDLE_W or m_draggingHandle == HANDLE_SW then
		newExtents:translate(Vector2.new(delta.X, 0))
		newExtents:resize(newExtents.Size - Vector2.new(delta.X, 0))
	elseif m_draggingHandle == HANDLE_NE or m_draggingHandle == HANDLE_E or m_draggingHandle == HANDLE_SE then
		newExtents:resize(newExtents.Size + Vector2.new(delta.X, 0))
	end

	if m_draggingHandle == HANDLE_NW or m_draggingHandle == HANDLE_N or m_draggingHandle == HANDLE_NE then
		newExtents:translate(Vector2.new(0, delta.Y))
		newExtents:resize(newExtents.Size - Vector2.new(0, delta.Y))
	elseif m_draggingHandle == HANDLE_SW or m_draggingHandle == HANDLE_S or m_draggingHandle == HANDLE_SE then
		newExtents:resize(newExtents.Size + Vector2.new(0, delta.Y))
	end
	...
	newExtents, snappingLines, hasSnappedX, hasSnappedY =
		SnappingPointManager:snapExtents(newExtents, getSnappingTypeFromHandle(m_draggingHandle))
	...
	for i = 1, #m_originalSelectionData do
		...
		local shouldUseScalePosition = Utility:isOnlyScaleUDim2(data[DATA_POSITION])
			or (not Utility:isOnlyOffsetUDim2(data[DATA_POSITION]) and GlobalValues:isScale())

		local shouldUseScaleSize = Utility:isOnlyScaleUDim2(data[DATA_SIZE])
			or (not Utility:isOnlyOffsetUDim2(data[DATA_SIZE]) and GlobalValues:isScale())

		local guiAncestor = guiObject:FindFirstAncestorWhichIsA("GuiBase2d")
		intendedPosition[guiObject], intendedSize[guiObject] = Convert:convertAbsoluteToScaleOrOffset(
			shouldUseScalePosition,
			shouldUseScaleSize,
			newAbsolutePosition,
			newAbsoluteSize,
			data[DATA_POSITION],

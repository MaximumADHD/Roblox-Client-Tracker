...
local modulesParent = robloxGui.Modules
...
-- this will always be true now, but leaving here to not mess with things too much
...
for _,child in pairs(screenGui:GetChildren()) do
	child.Parent = guiRoot
end
guiRoot.Parent = screenGui


local edgeShadingGui = starterGui:WaitForChild('EdgeShading'):Clone()

-- get all 3D background stuff
local replicatedStorage = game:GetService('ReplicatedStorage')
local AvatarEditorScene = replicatedStorage.AvatarEditorScene:Clone()
local background3D = AvatarEditorScene.Model

local particleScreenPart = AvatarEditorScene.ParticleScreenPart
local particleScreen = particleScreenPart.ParticleEmitter
local particleScreen2 = particleScreenPart.ParticleEmitter2

local cameraPositionOffset = AvatarEditorScene.CameraPositionOffset
local cameraRotationOffset = AvatarEditorScene.CameraRotationOffset
local centerPixel = AvatarEditorScene.CenterPixel
...
local topFrame = guiRoot:WaitForChild('TopFrame')
local mainFrame = guiRoot:WaitForChild('Frame')
local userInputService = game:GetService('UserInputService')
...
local shadeLayer = guiRoot.ShadeLayer
local menuFrame = guiRoot.MenuFrame
local menuTitleLabel = menuFrame:WaitForChild('TitleLabel')
local menuCancelButton = menuFrame:WaitForChild('CancelButton')
local avatarTypeSwitchFrame = guiRoot:WaitForChild('AvatarTypeSwitch')
...
local accessoriesColumn = guiRoot:WaitForChild('AccessoriesColumn')
local selectionFrameTemplate = guiRoot:WaitForChild('SelectionFrameTemplate')
local whiteFrameTemplate = guiRoot:WaitForChild('WhiteFrameTemplate')
local fakeScrollBar = mainFrame:WaitForChild('FakeScrollBar')
local detailsMenuFrame = guiRoot:WaitForChild('DetailsFrame')
...
local categoryButtonTemplate = guiRoot:WaitForChild('CategoryButtonTemplate')
local darkCover = guiRoot:WaitForChild('DarkCover')
...
if flagManager.AvatarEditorCameraZoomingEnabled then
	...
	workspace.CurrentCamera.CFrame = CFrame.new(cameraCFrames.default.p, background3D.Unions.BaseCylinder.CFrame.p)
end
...
if spriteManager.enabled then
	spriteManager.equipDescendants(guiRoot)
end
...
if tabletMode then
	...
	background3D.Unions.BaseCylinder.BillboardGui.TextLabel.Size = UDim2.new(0.3, 0, 1, 0)
	background3D.Unions.BaseCylinder.BillboardGui.TextLabel.Position = UDim2.new(0.5, 0, 0, 0)
...
require(coreGui.RobloxGui.Modules.AvatarEditorV2.Header).new("Avatar").rbx.Parent = guiRoot
...
local function setErrorMessage(text)
	--[[guiRoot.ErrorLabel.Visible = true
	guiRoot.ErrorLabel.Text = 'Error: '..text]]
end
...
function adjustCameraProperties(instant)
	...
	if cameraPositionOffset and not isCoreSetup then
		targetCFrame = targetCFrame * CFrame.new(cameraPositionOffset.Value)
	end
	if cameraPositionOffset and not isCoreSetup then
		targetCFrame = targetCFrame * CFrame.Angles(
			math.rad(cameraPositionOffset.Value.x),
			math.rad(cameraPositionOffset.Value.y),
			math.rad(cameraPositionOffset.Value.z)
		)
	end
	...
	if flagManager.AvatarEditorCameraZoomingEnabled then
		...
		local baseCF = background3D.Unions.BaseCylinder.CFrame
		...
		do -- pixel perfect camera rotation
			...
			if viewMode then
				centerPixel = camera.ViewportSize * Vector2.new(0.5, 0.5) + Vector2.new(0, -headerHeight)
			elseif tabletMode then
				centerPixel = camera.ViewportSize * Vector2.new(0.3, 0.5) + Vector2.new(-45, 0)
			elseif phoneMode then
				centerPixel = camera.ViewportSize * Vector2.new(0.5, 0.75) + Vector2.new(0, -headerHeight+18)
			end
...
if cameraPositionOffset and not isCoreSetup then
	cameraPositionOffset.Changed:connect(function()
...
-- needed to know when switching from other app into AE
-- can only call equip animation if the character is in the workspace
...
function createR15Rig()
	if not buildingCharacterLock then
		...
		if flagManager.AvatarEditorUsesNewR15Loading then
            if background3D.Parent == workspace then
			...
			if assetId then
				if accessoryButton.Image ~= assetImageUrl..tostring(assetId) then
					...
					delay(numberOfHatsScale*.25,function()
						...
						if accessoryButton:IsDescendantOf(game:GetService("CoreGui")) then
							...
							accessoryButton.Position = basePosition
						end
...
do
	...
	local function renderAvatarSwitch()
        ...
        if not success then
            avatarTypeSwitch.Position = desiredPosition
        end
		runParticleEmitter()
...
function renderSlider(name, changedFunction, currentPercent, intervals, scrollingFrame)
	local slider = guiRoot:WaitForChild('SliderFrameTemplate'):clone()
	...
	function loadMoreListContent()
		...
		elseif page.specialPageType == 'Skin Tone' then
			...
			if tabletMode then
				...
				local frame = guiRoot:WaitForChild('SimpleBackgroundTemplate'):Clone()
				frame.Visible = true
		...
		elseif page.specialPageType == 'Scale' then
			...
			local background = guiRoot:WaitForChild('SimpleBackgroundTemplate'):Clone()
		...
		if thisLoadingContentCall == currentLoadingContentCall then
			...
			if page.typeName then
				if #assetList <= 0 then
					if reachedBottom and not renderedNoAssetsMessage then
						...
						local shopInCatalogButtonTemplate = guiRoot:FindFirstChild('ShopInCatalogButtonTemplate')
			...
			if reachedBottom and not renderedRecommended and page.recommendedSort and flagManager.AvatarEditorUsesRecommended and page.typeName then
				...
				local shopNowButtonTemplate = guiRoot:FindFirstChild('ShopNowButtonTemplate')
				...
				if recommendedAssetListRequest and recommendedAssetListRequest.data and recommendedAssetListRequest.data.Items and thisRecommendedDebounceCounter == recommendedDebounceCounter then
...
function openTopMenu()
	...
	for index, category in pairs(categories) do
		...
		elseif tabletMode then
			categoryButton = guiRoot.TabletCategoryButtonTemplate:clone()
			categoryButton.Name = 'CategoryButton'..category.name
...
local rotGui = background3D.Unions.BaseCylinder.BillboardGui.Spins --workspace:WaitForChild('Model'):WaitForChild('Unions'):WaitForChild('BaseCylinder'):WaitForChild('BillboardGui'):WaitForChild('Spins')
...
local function adjustOnTopBarResize()
	headerHeight = userInputService.NavBarSize.Y+userInputService.StatusBarSize.Y
	guiRoot.Position = UDim2.new(0,0,0,headerHeight)
	guiRoot.Size = UDim2.new(1,0,1,-headerHeight)
	adjustCameraProperties()
end
...
function appMain.Start()
    if background3D.Parent ~= workspace then
        background3D.Parent = workspace
        particleScreenPart.Parent = workspace
        cameraPositionOffset.Parent = workspace
        cameraRotationOffset.Parent = workspace
        centerPixel.Parent = workspace
    end

    screenGui.Parent = coreGui
    screenGui.DisplayOrder = edgeShadingGui.DisplayOrder + 1
    edgeShadingGui.Parent = coreGui
    if character then

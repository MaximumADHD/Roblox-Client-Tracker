PROTO_0:
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["getFFlagPath2DEnabled"]
  CALL R0 0 1
  JUMPIF R0 [+1]
  RETURN R0 0
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K2 ["getFIntPath2DMaxControlPointLimit"]
  CALL R1 0 1
  SUBK R0 R1 K1 [1]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K3 ["pathContainerRef"]
  NAMECALL R1 R1 K4 ["getValue"]
  CALL R1 1 1
  JUMPIF R1 [+1]
  RETURN R0 0
  LOADN R2 1
  NAMECALL R3 R1 K5 ["GetChildren"]
  CALL R3 1 3
  FORGPREP R3
  NEWTABLE R8 0 0
  GETUPVAL R12 1
  GETTABLEKS R11 R12 K6 ["props"]
  GETTABLEKS R10 R11 K7 ["ControlPoints"]
  LENGTH R9 R10
  MOVE R12 R2
  ADD R13 R2 R0
  SUBK R10 R13 K1 [1]
  LOADN R11 1
  FORNPREP R10
  JUMPIFLT R9 R12 [+14]
  GETUPVAL R18 1
  GETTABLEKS R17 R18 K6 ["props"]
  GETTABLEKS R16 R17 K7 ["ControlPoints"]
  GETTABLE R15 R16 R12
  FASTCALL2 TABLE_INSERT R8 R15 [+4]
  MOVE R14 R8
  GETIMPORT R13 K10 [table.insert]
  CALL R13 2 0
  FORNLOOP R10
  MOVE R12 R8
  NAMECALL R10 R7 K11 ["SetControlPoints"]
  CALL R10 2 0
  SUBK R10 R0 K1 [1]
  ADD R2 R2 R10
  FORGLOOP R3 2 [-35]
  RETURN R0 0

PROTO_1:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["createRef"]
  CALL R1 0 1
  SETTABLEKS R1 R0 K1 ["pathContainerRef"]
  NEWCLOSURE R1 P0
  CAPTURE UPVAL U1
  CAPTURE VAL R0
  SETTABLEKS R1 R0 K2 ["setControlPoints"]
  RETURN R0 0

PROTO_2:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K1 ["getFIntPath2DMaxControlPointLimit"]
  CALL R2 0 1
  SUBK R1 R2 K0 [1]
  LOADK R2 K2 [""]
  GETTABLEKS R4 R0 K3 ["props"]
  GETTABLEKS R3 R4 K4 ["Tag"]
  JUMPIFNOT R3 [+4]
  GETTABLEKS R3 R0 K3 ["props"]
  GETTABLEKS R2 R3 K4 ["Tag"]
  NEWTABLE R3 0 0
  LOADN R6 2
  GETTABLEKS R8 R0 K3 ["props"]
  GETTABLEKS R7 R8 K5 ["ControlPoints"]
  LENGTH R4 R7
  SUBK R5 R1 K0 [1]
  FORNPREP R4
  MOVE R8 R3
  GETUPVAL R10 1
  GETTABLEKS R9 R10 K6 ["createElement"]
  LOADK R10 K7 ["Path2D"]
  NEWTABLE R11 1 0
  GETUPVAL R13 1
  GETTABLEKS R12 R13 K4 ["Tag"]
  SETTABLE R2 R11 R12
  CALL R9 2 -1
  FASTCALL TABLE_INSERT [+2]
  GETIMPORT R7 K10 [table.insert]
  CALL R7 -1 0
  FORNLOOP R4
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K11 ["getFFlagPath2DEnabled"]
  CALL R4 0 1
  JUMPIFNOT R4 [+18]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K6 ["createElement"]
  GETUPVAL R5 2
  DUPTABLE R6 K14 [{"ref", "Size"}]
  GETTABLEKS R7 R0 K15 ["pathContainerRef"]
  SETTABLEKS R7 R6 K12 ["ref"]
  GETIMPORT R7 K18 [UDim2.fromScale]
  LOADN R8 1
  LOADN R9 1
  CALL R7 2 1
  SETTABLEKS R7 R6 K13 ["Size"]
  MOVE R7 R3
  CALL R4 3 1
  RETURN R4 1

PROTO_3:
  GETTABLEKS R1 R0 K0 ["setControlPoints"]
  CALL R1 0 0
  RETURN R0 0

PROTO_4:
  GETTABLEKS R1 R0 K0 ["setControlPoints"]
  CALL R1 0 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R1 K1 [script]
  GETTABLEKS R0 R1 K2 ["Parent"]
  GETIMPORT R1 K1 [script]
  LOADK R3 K3 ["AudioEqualizerEditor"]
  NAMECALL R1 R1 K4 ["FindFirstAncestor"]
  CALL R1 2 1
  GETIMPORT R2 K6 [require]
  GETTABLEKS R4 R1 K7 ["Packages"]
  GETTABLEKS R3 R4 K8 ["React"]
  CALL R2 1 1
  GETIMPORT R3 K6 [require]
  GETTABLEKS R5 R1 K7 ["Packages"]
  GETTABLEKS R4 R5 K9 ["Framework"]
  CALL R3 1 1
  GETTABLEKS R5 R3 K10 ["UI"]
  GETTABLEKS R4 R5 K11 ["Pane"]
  GETTABLEKS R6 R3 K12 ["Styling"]
  GETTABLEKS R5 R6 K13 ["joinTags"]
  GETIMPORT R6 K6 [require]
  GETTABLEKS R9 R1 K14 ["Bin"]
  GETTABLEKS R8 R9 K15 ["Common"]
  GETTABLEKS R7 R8 K16 ["defineLuaFlags"]
  CALL R6 1 1
  GETIMPORT R7 K6 [require]
  GETTABLEKS R10 R1 K17 ["Src"]
  GETTABLEKS R9 R10 K18 ["Util"]
  GETTABLEKS R8 R9 K19 ["Path2DUtil"]
  CALL R7 1 1
  GETTABLEKS R8 R2 K20 ["Component"]
  LOADK R10 K21 ["Path2DWrapper"]
  NAMECALL R8 R8 K22 ["extend"]
  CALL R8 2 1
  DUPCLOSURE R9 K23 [PROTO_1]
  CAPTURE VAL R2
  CAPTURE VAL R6
  SETTABLEKS R9 R8 K24 ["init"]
  DUPCLOSURE R9 K25 [PROTO_2]
  CAPTURE VAL R6
  CAPTURE VAL R2
  CAPTURE VAL R4
  SETTABLEKS R9 R8 K26 ["render"]
  DUPCLOSURE R9 K27 [PROTO_3]
  SETTABLEKS R9 R8 K28 ["didMount"]
  DUPCLOSURE R9 K29 [PROTO_4]
  SETTABLEKS R9 R8 K30 ["didUpdate"]
  RETURN R8 1

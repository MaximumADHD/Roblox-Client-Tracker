...
local function nilCheckFillIn(table, stepStateBundle)
	...
	if
		table[stepStateBundle.debuggerStateToken][stepStateBundle.threadId][stepStateBundle.frameNumber].Watches == nil
	then
...
return Rodux.createReducer(productionStartStore, {
	[SimPaused.name] = function(state: WatchStore, action: SimPaused.Props)
		assert(
			(
					...
					or state.stateTokenToRoots[action.debuggerStateToken][action.threadId] == nil
				)
				and (
					state.stateTokenToFlattenedTree[action.debuggerStateToken] == nil
					or state.stateTokenToFlattenedTree[action.debuggerStateToken][action.threadId] == nil
				)
		)

		return Cryo.Dictionary.join(state, {
			stateTokenToRoots = Cryo.Dictionary.join(state.stateTokenToRoots, {
				[action.debuggerStateToken] = Cryo.Dictionary.join(
					state.stateTokenToRoots[action.debuggerStateToken] or {},
					{
						[action.threadId] = {},
					}
				),
			...
			stateTokenToFlattenedTree = Cryo.Dictionary.join(state.stateTokenToFlattenedTree, {
				[action.debuggerStateToken] = Cryo.Dictionary.join(
					state.stateTokenToFlattenedTree[action.debuggerStateToken] or {},
					{
						[action.threadId] = {},
					}
				),
	...
	[ClearConnectionData.name] = function(state: WatchStore, action: Resumed.Props)
		...
		local hasTheStateToken = state.stateTokenToRoots[action.debuggerStateToken] ~= nil
			and (state.stateTokenToFlattenedTree[action.debuggerStateToken] ~= nil)
		...
		return Cryo.Dictionary.join(state, {
			stateTokenToRoots = Cryo.Dictionary.join(
				state.stateTokenToRoots,
				{ [action.debuggerStateToken] = Cryo.None }
			),
			stateTokenToFlattenedTree = Cryo.Dictionary.join(
				state.stateTokenToFlattenedTree,
				{ [action.debuggerStateToken] = Cryo.None }
			),
		})
	end,
	...
	[AddRootVariables.name] = function(state: WatchStore, action: AddRootVariables.Props)
		...
		local target =
			stateTokenToRootsCopy[stepStateBundle.debuggerStateToken][stepStateBundle.threadId][stepStateBundle.frameNumber].Variables
		...
		local target2 =
			stateTokenToFlattenedTreeCopy[stepStateBundle.debuggerStateToken][stepStateBundle.threadId][stepStateBundle.frameNumber].Variables
	...
	[SetVariableExpanded.name] = function(state: WatchStore, action: SetVariableExpanded.Props)
		return Cryo.Dictionary.join(state, {
			pathToExpansionState = Cryo.Dictionary.join(
				state.pathToExpansionState,
	...
	[SetExpressionExpanded.name] = function(state: WatchStore, action: SetExpressionExpanded.Props)
		return Cryo.Dictionary.join(state, {
			expressionToExpansionState = Cryo.Dictionary.join(
				state.expressionToExpansionState,
				{ [action.path] = action.expanded }
			),
		})
	end,
	...
	[SetVariablesTextFilteredOut.name] = function(state: WatchStore, action: SetVariablesTextFilteredOut.Props)
		...
		local flattenedTree = if action.isVariablesTab
			then
				stateTokenToFlattenedTreeCopy[stepStateBundle.debuggerStateToken][stepStateBundle.threadId][stepStateBundle.frameNumber].Variables
			else
	...
	[AddChildExpression.name] = function(state: WatchStore, action: AddChildExpression.Props)
		...
		local target =
			stateTokenToFlattenedTreeCopy[action.stepStateBundle.debuggerStateToken][action.stepStateBundle.threadId][action.stepStateBundle.frameNumber].Watches
		for varPath, variable in pairs(action.newVarsMapping) do
			target[varPath] = variable
		end
	...
	[ExpressionEvaluated.name] = function(state: WatchStore, action: ExpressionEvaluated.Props)
		...
		local target =
			stateTokenToRootsCopy[action.stepStateBundle.debuggerStateToken][action.stepStateBundle.threadId][action.stepStateBundle.frameNumber].Watches
	...
	[SetWatchSortState.name] = function(state: WatchStore, action: SetWatchSortState.Props)
		local toReturn = Cryo.Dictionary.join(
			state,
			{ sortDirection = action.sortDirection, columnIndex = action.columnIndex }
		)
		return toReturn
	end,
})

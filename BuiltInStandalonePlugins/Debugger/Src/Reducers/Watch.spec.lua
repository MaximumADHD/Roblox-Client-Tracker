...
return function()
	...
	describe(SimPaused.name, function()
		it("should add empty data to state map", function()
			...
			expect(state.stateTokenToRoots[defaultDebuggerToken][defaultThreadId]).to.be.ok()
			expect(#state.listOfEnabledScopes).to.be.ok()
			expect(#state.listOfEnabledScopes).to.equal(3)
			expect(#state.listOfExpressions).to.be.ok()
			expect(#state.listOfExpressions).to.equal(0)
	...
	describe(Resumed.name, function()
		...
		it("should preserve immutability", function()
			local immutabilityPreserved = testImmutability(WatchReducer, Resumed)
			expect(immutabilityPreserved).to.equal(true)
		end)
	end)
	...
	describe(ClearConnectionData.name, function()
		it("should clear data on connection ended", function()
			...
			expect(state.stateTokenToRoots[defaultDebuggerToken]).to.be.ok()
			expect(state.stateTokenToRoots[defaultDebuggerToken][defaultThreadId]).to.be.ok()
			expect(state.stateTokenToRoots[secondDebuggerToken]).to.equal(nil)
			expect(#state.listOfEnabledScopes).to.be.ok()
			expect(#state.listOfEnabledScopes).to.equal(3)
			expect(#state.listOfExpressions).to.be.ok()
			expect(#state.listOfExpressions).to.equal(0)

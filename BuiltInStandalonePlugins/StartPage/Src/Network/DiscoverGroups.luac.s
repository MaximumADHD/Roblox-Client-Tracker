PROTO_0:
  DUPTABLE R2 K3 [{"DropdownLabel", "Key", "Query"}]
  GETTABLEKS R3 R1 K4 ["name"]
  SETTABLEKS R3 R2 K0 ["DropdownLabel"]
  GETTABLEKS R3 R1 K5 ["id"]
  SETTABLEKS R3 R2 K1 ["Key"]
  DUPTABLE R3 K7 [{"creatorTargetId"}]
  GETTABLEKS R4 R1 K5 ["id"]
  SETTABLEKS R4 R3 K6 ["creatorTargetId"]
  SETTABLEKS R3 R2 K2 ["Query"]
  RETURN R2 1

PROTO_1:
  GETTABLEKS R3 R0 K0 ["DropdownLabel"]
  GETTABLEKS R4 R1 K0 ["DropdownLabel"]
  JUMPIFLT R3 R4 [+2]
  LOADB R2 0 +1
  LOADB R2 1
  RETURN R2 1

PROTO_2:
  GETUPVAL R1 0
  MOVE R2 R0
  CALL R1 1 1
  GETTABLEKS R2 R1 K0 ["groups"]
  JUMPIF R2 [+1]
  RETURN R0 0
  GETUPVAL R2 1
  GETTABLEKS R3 R1 K0 ["groups"]
  DUPCLOSURE R4 K1 [PROTO_0]
  CALL R2 2 1
  GETIMPORT R3 K4 [table.sort]
  MOVE R4 R2
  DUPCLOSURE R5 K5 [PROTO_1]
  CALL R3 2 0
  GETUPVAL R3 2
  MOVE R4 R2
  CALL R3 1 0
  RETURN R0 0

PROTO_3:
  GETIMPORT R1 K1 [warn]
  LOADK R3 K2 ["DiscoverGroups experienced an error: %*"]
  MOVE R5 R0
  NAMECALL R3 R3 K3 ["format"]
  CALL R3 2 1
  MOVE R2 R3
  CALL R1 1 0
  RETURN R0 0

PROTO_4:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["composeUrl"]
  GETUPVAL R4 1
  GETTABLEKS R3 R4 K1 ["CREATOR_GROUPS_SEARCH_URL"]
  GETTABLEKS R2 R3 K2 ["Prefix"]
  GETUPVAL R5 1
  GETTABLEKS R4 R5 K1 ["CREATOR_GROUPS_SEARCH_URL"]
  GETTABLEKS R3 R4 K3 ["Url"]
  LOADK R4 K4 ["StudioStartPage"]
  CALL R3 1 -1
  CALL R1 -1 1
  GETUPVAL R2 2
  GETUPVAL R4 2
  MOVE R6 R1
  NAMECALL R4 R4 K5 ["get"]
  CALL R4 2 -1
  NAMECALL R2 R2 K6 ["parseJson"]
  CALL R2 -1 1
  NEWCLOSURE R5 P0
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CAPTURE VAL R0
  NAMECALL R3 R2 K7 ["andThen"]
  CALL R3 2 1
  DUPCLOSURE R5 K8 [PROTO_3]
  NAMECALL R3 R3 K9 ["catch"]
  CALL R3 2 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["StartPage"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R4 R0 K6 ["Src"]
  GETTABLEKS R3 R4 K7 ["SharedFlags"]
  GETTABLEKS R2 R3 K8 ["getFIntDebugLuaStartPageLogging"]
  CALL R1 1 1
  CALL R1 0 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R4 R0 K9 ["Packages"]
  GETTABLEKS R3 R4 K10 ["Framework"]
  CALL R2 1 1
  GETIMPORT R3 K5 [require]
  GETTABLEKS R5 R0 K6 ["Src"]
  GETTABLEKS R4 R5 K11 ["Types"]
  CALL R3 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R7 R0 K6 ["Src"]
  GETTABLEKS R6 R7 K12 ["Models"]
  GETTABLEKS R5 R6 K13 ["ApiArrayResponseGroupModel"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K6 ["Src"]
  GETTABLEKS R7 R8 K14 ["Network"]
  GETTABLEKS R6 R7 K15 ["Urls"]
  CALL R5 1 1
  GETIMPORT R7 K5 [require]
  GETTABLEKS R10 R0 K6 ["Src"]
  GETTABLEKS R9 R10 K16 ["Util"]
  GETTABLEKS R8 R9 K17 ["Services"]
  CALL R7 1 1
  GETTABLEKS R6 R7 K18 ["Networking"]
  GETIMPORT R7 K5 [require]
  GETTABLEKS R10 R0 K6 ["Src"]
  GETTABLEKS R9 R10 K16 ["Util"]
  GETTABLEKS R8 R9 K19 ["createResponseValidator"]
  CALL R7 1 1
  GETTABLEKS R9 R2 K20 ["RobloxAPI"]
  GETTABLEKS R8 R9 K21 ["Url"]
  GETTABLEKS R9 R6 K22 ["new"]
  DUPTABLE R10 K25 [{"isInternal", "loggingLevel"}]
  LOADB R11 1
  SETTABLEKS R11 R10 K23 ["isInternal"]
  SETTABLEKS R1 R10 K24 ["loggingLevel"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R12 R0 K9 ["Packages"]
  GETTABLEKS R11 R12 K26 ["Dash"]
  CALL R10 1 1
  GETTABLEKS R11 R10 K27 ["collectArray"]
  MOVE R12 R7
  GETTABLEKS R13 R4 K28 ["validate"]
  CALL R12 1 1
  GETTABLEKS R13 R8 K22 ["new"]
  CALL R13 0 1
  DUPCLOSURE R14 K29 [PROTO_4]
  CAPTURE VAL R13
  CAPTURE VAL R5
  CAPTURE VAL R9
  CAPTURE VAL R12
  CAPTURE VAL R11
  RETURN R14 1

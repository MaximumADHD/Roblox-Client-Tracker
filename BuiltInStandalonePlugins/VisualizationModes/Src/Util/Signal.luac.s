PROTO_0:
  PREPVARARGS 1
  GETUPVAL R1 0
  LOADNIL R2
  SETUPVAL R2 0
  MOVE R2 R0
  GETVARARGS R3 -1
  CALL R2 -1 0
  SETUPVAL R1 0
  RETURN R0 0

PROTO_1:
  GETUPVAL R0 0
  GETIMPORT R1 K2 [coroutine.yield]
  CALL R1 0 -1
  CALL R0 -1 0
  JUMPBACK [-6]
  RETURN R0 0

PROTO_2:
  DUPTABLE R3 K4 [{"_connected", "_signal", "_fn", "_next"}]
  LOADB R4 1
  SETTABLEKS R4 R3 K0 ["_connected"]
  SETTABLEKS R0 R3 K1 ["_signal"]
  SETTABLEKS R1 R3 K2 ["_fn"]
  LOADB R4 0
  SETTABLEKS R4 R3 K3 ["_next"]
  GETUPVAL R4 0
  FASTCALL2 SETMETATABLE R3 R4 [+3]
  GETIMPORT R2 K6 [setmetatable]
  CALL R2 2 1
  RETURN R2 1

PROTO_3:
  LOADB R1 0
  SETTABLEKS R1 R0 K0 ["_connected"]
  GETTABLEKS R2 R0 K1 ["_signal"]
  GETTABLEKS R1 R2 K2 ["_handlerListHead"]
  JUMPIFNOTEQ R1 R0 [+8]
  GETTABLEKS R1 R0 K1 ["_signal"]
  GETTABLEKS R2 R0 K3 ["_next"]
  SETTABLEKS R2 R1 K2 ["_handlerListHead"]
  RETURN R0 0
  GETTABLEKS R2 R0 K1 ["_signal"]
  GETTABLEKS R1 R2 K2 ["_handlerListHead"]
  JUMPIFNOT R1 [+7]
  GETTABLEKS R2 R1 K3 ["_next"]
  JUMPIFEQ R2 R0 [+4]
  GETTABLEKS R1 R1 K3 ["_next"]
  JUMPBACK [-8]
  JUMPIFNOT R1 [+4]
  GETTABLEKS R2 R0 K3 ["_next"]
  SETTABLEKS R2 R1 K3 ["_next"]
  RETURN R0 0

PROTO_4:
  GETIMPORT R2 K1 [error]
  LOADK R3 K2 ["Attempt to get Connection::%s (not a valid member)"]
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R6 R1
  GETIMPORT R5 K4 [tostring]
  CALL R5 1 1
  NAMECALL R3 R3 K5 ["format"]
  CALL R3 2 1
  LOADN R4 2
  CALL R2 2 0
  RETURN R0 0

PROTO_5:
  GETIMPORT R3 K1 [error]
  LOADK R4 K2 ["Attempt to set Connection::%s (not a valid member)"]
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R7 R1
  GETIMPORT R6 K4 [tostring]
  CALL R6 1 1
  NAMECALL R4 R4 K5 ["format"]
  CALL R4 2 1
  LOADN R5 2
  CALL R3 2 0
  RETURN R0 0

PROTO_6:
  DUPTABLE R1 K1 [{"_handlerListHead"}]
  LOADB R2 0
  SETTABLEKS R2 R1 K0 ["_handlerListHead"]
  GETUPVAL R2 0
  FASTCALL2 SETMETATABLE R1 R2 [+3]
  GETIMPORT R0 K3 [setmetatable]
  CALL R0 2 1
  RETURN R0 1

PROTO_7:
  LOADB R1 0
  FASTCALL1 TYPE R0 [+3]
  MOVE R3 R0
  GETIMPORT R2 K1 [type]
  CALL R2 1 1
  JUMPIFNOTEQKS R2 K2 ["table"] [+11]
  FASTCALL1 GETMETATABLE R0 [+3]
  MOVE R3 R0
  GETIMPORT R2 K4 [getmetatable]
  CALL R2 1 1
  GETUPVAL R3 0
  JUMPIFEQ R2 R3 [+2]
  LOADB R1 0 +1
  LOADB R1 1
  RETURN R1 1

PROTO_8:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["new"]
  MOVE R3 R0
  MOVE R4 R1
  CALL R2 2 1
  GETTABLEKS R3 R0 K1 ["_handlerListHead"]
  JUMPIFNOT R3 [+7]
  GETTABLEKS R3 R0 K1 ["_handlerListHead"]
  SETTABLEKS R3 R2 K2 ["_next"]
  SETTABLEKS R2 R0 K1 ["_handlerListHead"]
  RETURN R2 1
  SETTABLEKS R2 R0 K1 ["_handlerListHead"]
  RETURN R2 1

PROTO_9:
  LOADB R1 0
  SETTABLEKS R1 R0 K0 ["_handlerListHead"]
  RETURN R0 0

PROTO_10:
  PREPVARARGS 1
  GETTABLEKS R1 R0 K0 ["_handlerListHead"]
  JUMPIFNOT R1 [+24]
  GETTABLEKS R2 R1 K1 ["_connected"]
  JUMPIFNOT R2 [+18]
  GETUPVAL R2 0
  JUMPIF R2 [+9]
  GETIMPORT R2 K4 [coroutine.create]
  GETUPVAL R3 1
  CALL R2 1 1
  SETUPVAL R2 0
  GETIMPORT R2 K6 [coroutine.resume]
  GETUPVAL R3 0
  CALL R2 1 0
  GETIMPORT R2 K9 [task.spawn]
  GETUPVAL R3 0
  GETTABLEKS R4 R1 K10 ["_fn"]
  GETVARARGS R5 -1
  CALL R2 -1 0
  GETTABLEKS R1 R1 K11 ["_next"]
  JUMPBACK [-25]
  RETURN R0 0

PROTO_11:
  PREPVARARGS 0
  GETUPVAL R0 0
  NAMECALL R0 R0 K0 ["Disconnect"]
  CALL R0 1 0
  GETIMPORT R0 K3 [task.spawn]
  GETUPVAL R1 1
  GETVARARGS R2 -1
  CALL R0 -1 0
  RETURN R0 0

PROTO_12:
  GETIMPORT R1 K2 [coroutine.running]
  CALL R1 0 1
  LOADNIL R2
  NEWCLOSURE R5 P0
  CAPTURE REF R2
  CAPTURE VAL R1
  NAMECALL R3 R0 K3 ["Connect"]
  CALL R3 2 1
  MOVE R2 R3
  GETIMPORT R3 K5 [coroutine.yield]
  CALL R3 0 -1
  CLOSEUPVALS R2
  RETURN R3 -1

PROTO_13:
  PREPVARARGS 0
  GETUPVAL R1 0
  GETTABLEKS R0 R1 K0 ["_connected"]
  JUMPIFNOT R0 [+4]
  GETUPVAL R0 0
  NAMECALL R0 R0 K1 ["Disconnect"]
  CALL R0 1 0
  GETUPVAL R0 1
  GETVARARGS R1 -1
  CALL R0 -1 0
  RETURN R0 0

PROTO_14:
  LOADNIL R2
  NEWCLOSURE R5 P0
  CAPTURE REF R2
  CAPTURE VAL R1
  NAMECALL R3 R0 K0 ["Connect"]
  CALL R3 2 1
  MOVE R2 R3
  CLOSEUPVALS R2
  RETURN R2 1

PROTO_15:
  GETIMPORT R2 K1 [error]
  LOADK R3 K2 ["Attempt to get Signal::%s (not a valid member)"]
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R6 R1
  GETIMPORT R5 K4 [tostring]
  CALL R5 1 1
  NAMECALL R3 R3 K5 ["format"]
  CALL R3 2 1
  LOADN R4 2
  CALL R2 2 0
  RETURN R0 0

PROTO_16:
  GETIMPORT R3 K1 [error]
  LOADK R4 K2 ["Attempt to set Signal::%s (not a valid member)"]
  FASTCALL1 TOSTRING R1 [+3]
  MOVE R7 R1
  GETIMPORT R6 K4 [tostring]
  CALL R6 1 1
  NAMECALL R4 R4 K5 ["format"]
  CALL R4 2 1
  LOADN R5 2
  CALL R3 2 0
  RETURN R0 0

MAIN:
  PREPVARARGS 0
  LOADNIL R0
  NEWCLOSURE R1 P0
  CAPTURE REF R0
  DUPCLOSURE R2 K0 [PROTO_1]
  CAPTURE VAL R1
  NEWTABLE R3 8 0
  SETTABLEKS R3 R3 K1 ["__index"]
  DUPCLOSURE R4 K2 [PROTO_2]
  CAPTURE VAL R3
  SETTABLEKS R4 R3 K3 ["new"]
  DUPCLOSURE R4 K4 [PROTO_3]
  SETTABLEKS R4 R3 K5 ["Disconnect"]
  GETTABLEKS R4 R3 K5 ["Disconnect"]
  SETTABLEKS R4 R3 K6 ["Destroy"]
  GETTABLEKS R4 R3 K5 ["Disconnect"]
  SETTABLEKS R4 R3 K7 ["destroy"]
  DUPTABLE R6 K9 [{"__index", "__newindex"}]
  DUPCLOSURE R7 K10 [PROTO_4]
  SETTABLEKS R7 R6 K1 ["__index"]
  DUPCLOSURE R7 K11 [PROTO_5]
  SETTABLEKS R7 R6 K8 ["__newindex"]
  FASTCALL2 SETMETATABLE R3 R6 [+4]
  MOVE R5 R3
  GETIMPORT R4 K13 [setmetatable]
  CALL R4 2 0
  NEWTABLE R4 16 0
  SETTABLEKS R4 R4 K1 ["__index"]
  DUPCLOSURE R5 K14 [PROTO_6]
  CAPTURE VAL R4
  SETTABLEKS R5 R4 K3 ["new"]
  DUPCLOSURE R5 K15 [PROTO_7]
  CAPTURE VAL R4
  SETTABLEKS R5 R4 K16 ["isSignal"]
  DUPCLOSURE R5 K17 [PROTO_8]
  CAPTURE VAL R3
  SETTABLEKS R5 R4 K18 ["Connect"]
  DUPCLOSURE R5 K19 [PROTO_9]
  SETTABLEKS R5 R4 K20 ["DisconnectAll"]
  NEWCLOSURE R5 P10
  CAPTURE REF R0
  CAPTURE VAL R2
  SETTABLEKS R5 R4 K21 ["Fire"]
  DUPCLOSURE R5 K22 [PROTO_12]
  SETTABLEKS R5 R4 K23 ["Wait"]
  DUPCLOSURE R5 K24 [PROTO_14]
  SETTABLEKS R5 R4 K25 ["Once"]
  GETTABLEKS R5 R4 K20 ["DisconnectAll"]
  SETTABLEKS R5 R4 K6 ["Destroy"]
  GETTABLEKS R5 R4 K20 ["DisconnectAll"]
  SETTABLEKS R5 R4 K7 ["destroy"]
  DUPTABLE R7 K9 [{"__index", "__newindex"}]
  DUPCLOSURE R8 K26 [PROTO_15]
  SETTABLEKS R8 R7 K1 ["__index"]
  DUPCLOSURE R8 K27 [PROTO_16]
  SETTABLEKS R8 R7 K8 ["__newindex"]
  FASTCALL2 SETMETATABLE R4 R7 [+4]
  MOVE R6 R4
  GETIMPORT R5 K13 [setmetatable]
  CALL R5 2 0
  CLOSEUPVALS R0
  RETURN R4 1

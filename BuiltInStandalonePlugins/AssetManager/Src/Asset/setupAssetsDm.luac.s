PROTO_0:
  MOVE R1 R0
  LOADNIL R2
  LOADNIL R3
  FORGPREP R1
  FASTCALL1 TYPE R5 [+3]
  MOVE R7 R5
  GETIMPORT R6 K1 [type]
  CALL R6 1 1
  JUMPIFNOTEQKS R6 K2 ["table"] [+7]
  GETIMPORT R6 K4 [table.concat]
  MOVE R7 R5
  LOADK R8 K5 [","]
  CALL R6 2 1
  SETTABLE R6 R0 R4
  FORGLOOP R1 2 [-14]
  RETURN R0 0

PROTO_1:
  GETUPVAL R0 0
  LOADK R2 K0 ["GameId"]
  GETIMPORT R4 K2 [game]
  GETTABLEKS R3 R4 K0 ["GameId"]
  NAMECALL R0 R0 K3 ["SetItem"]
  CALL R0 3 0
  RETURN R0 0

PROTO_2:
  GETIMPORT R2 K2 [task.spawn]
  GETUPVAL R3 0
  GETUPVAL R4 1
  MOVE R5 R0
  MOVE R6 R1
  CALL R2 4 0
  RETURN R0 0

PROTO_3:
  GETIMPORT R1 K1 [pcall]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K2 ["OpenPlace"]
  GETUPVAL R3 0
  MOVE R4 R0
  CALL R1 3 2
  JUMPIF R1 [+4]
  GETIMPORT R3 K4 [warn]
  LOADK R4 K5 ["Failed to open place"]
  CALL R3 1 0
  RETURN R0 0

PROTO_4:
  GETIMPORT R0 K1 [pcall]
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K2 ["AddNewPlace"]
  GETUPVAL R2 0
  GETIMPORT R4 K4 [game]
  GETTABLEKS R3 R4 K5 ["GameId"]
  CALL R0 3 2
  JUMPIFNOT R0 [+6]
  GETUPVAL R2 1
  LOADK R4 K6 ["OnAddPlaceSucceeded"]
  NAMECALL R2 R2 K7 ["Invoke"]
  CALL R2 2 0
  RETURN R0 0
  GETIMPORT R2 K9 [warn]
  LOADK R3 K10 ["Failed to add new place"]
  CALL R2 1 0
  RETURN R0 0

PROTO_5:
  GETIMPORT R1 K1 [pcall]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K2 ["RemovePlace"]
  GETUPVAL R3 0
  MOVE R4 R0
  CALL R1 3 1
  JUMPIFNOT R1 [+7]
  GETUPVAL R2 1
  LOADK R4 K3 ["OnRemovePlaceFromGameSucceeded"]
  MOVE R5 R0
  NAMECALL R2 R2 K4 ["Invoke"]
  CALL R2 3 0
  RETURN R0 0
  GETIMPORT R2 K6 [warn]
  LOADK R3 K7 ["Failed to remove place"]
  CALL R2 1 0
  RETURN R0 0

PROTO_6:
  GETIMPORT R1 K1 [pcall]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K2 ["RenamePlace"]
  GETUPVAL R3 0
  GETTABLEKS R4 R0 K3 ["Id"]
  GETTABLEKS R5 R0 K4 ["Name"]
  CALL R1 4 2
  JUMPIFNOT R1 [+7]
  GETUPVAL R3 1
  LOADK R5 K5 ["OnRenamePlaceSucceeded"]
  MOVE R6 R0
  NAMECALL R3 R3 K6 ["Invoke"]
  CALL R3 3 0
  RETURN R0 0
  GETIMPORT R3 K8 [warn]
  LOADK R4 K9 ["Failed to rename place"]
  CALL R3 1 0
  RETURN R0 0

PROTO_7:
  DUPTABLE R1 K5 [{"eventType", "placeId", "universeId", "studioSid", "clientId"}]
  GETTABLEKS R2 R0 K6 ["Event"]
  SETTABLEKS R2 R1 K0 ["eventType"]
  GETIMPORT R3 K8 [game]
  GETTABLEKS R2 R3 K9 ["PlaceId"]
  SETTABLEKS R2 R1 K1 ["placeId"]
  GETIMPORT R3 K8 [game]
  GETTABLEKS R2 R3 K10 ["GameId"]
  SETTABLEKS R2 R1 K2 ["universeId"]
  GETUPVAL R2 0
  NAMECALL R2 R2 K11 ["GetSessionId"]
  CALL R2 1 1
  SETTABLEKS R2 R1 K3 ["studioSid"]
  GETUPVAL R2 0
  NAMECALL R2 R2 K12 ["GetClientId"]
  CALL R2 1 1
  SETTABLEKS R2 R1 K4 ["clientId"]
  GETUPVAL R2 1
  GETTABLEKS R3 R0 K13 ["Args"]
  CALL R2 1 0
  GETUPVAL R3 2
  GETTABLEKS R2 R3 K14 ["join"]
  MOVE R3 R1
  GETTABLEKS R4 R0 K13 ["Args"]
  CALL R2 2 1
  GETUPVAL R3 0
  LOADK R5 K15 ["studio"]
  LOADK R6 K16 ["AssetManager"]
  LOADK R7 K6 ["Event"]
  MOVE R8 R2
  NAMECALL R3 R3 K17 ["SendEventDeferred"]
  CALL R3 5 0
  RETURN R0 0

PROTO_8:
  GETUPVAL R3 0
  LOADK R5 K0 ["BulkImportAssetImported"]
  MOVE R6 R2
  NAMECALL R3 R3 K1 ["Invoke"]
  CALL R3 3 0
  RETURN R0 0

PROTO_9:
  NEWTABLE R1 16 0
  LOADK R4 K0 ["GameId"]
  GETIMPORT R6 K2 [game]
  GETTABLEKS R5 R6 K0 ["GameId"]
  NAMECALL R2 R0 K3 ["SetItem"]
  CALL R2 3 0
  GETIMPORT R2 K2 [game]
  LOADK R4 K0 ["GameId"]
  NAMECALL R2 R2 K4 ["GetPropertyChangedSignal"]
  CALL R2 2 1
  NEWCLOSURE R4 P0
  CAPTURE VAL R0
  NAMECALL R2 R2 K5 ["Connect"]
  CALL R2 2 1
  SETTABLEKS R2 R1 K6 ["OnGameIdChangedConnection"]
  GETUPVAL R2 0
  CALL R2 0 1
  JUMPIFNOT R2 [+9]
  LOADK R4 K7 ["OnInsertAndGrantItems"]
  NEWCLOSURE R5 P1
  CAPTURE UPVAL U1
  CAPTURE VAL R0
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K9 ["OnInsertAndGrantItemsConnection"]
  LOADK R4 K10 ["OnInsertItems"]
  GETUPVAL R5 2
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K11 ["OnInsertItemsConnection"]
  GETUPVAL R2 3
  CALL R2 0 1
  JUMPIFNOT R2 [+7]
  LOADK R4 K12 ["OnSelectItems"]
  GETUPVAL R5 4
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K13 ["OnSelectItemsConnection"]
  LOADK R4 K14 ["OnOpenBulkImport"]
  GETUPVAL R5 5
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K15 ["OnOpenBulkImportConnection"]
  LOADK R4 K16 ["OnOpenPlace"]
  DUPCLOSURE R5 K17 [PROTO_3]
  CAPTURE UPVAL U6
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K18 ["OnOpenPlaceConnection"]
  LOADK R4 K19 ["OnAddPlace"]
  NEWCLOSURE R5 P3
  CAPTURE UPVAL U6
  CAPTURE VAL R0
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K20 ["OnAddPlaceConnection"]
  LOADK R4 K21 ["OnRemovePlace"]
  NEWCLOSURE R5 P4
  CAPTURE UPVAL U6
  CAPTURE VAL R0
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K22 ["OnRemovePlaceConnection"]
  LOADK R4 K23 ["OnRenamePlace"]
  NEWCLOSURE R5 P5
  CAPTURE UPVAL U6
  CAPTURE VAL R0
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K24 ["OnRenamePlaceConnection"]
  LOADK R4 K25 ["SendAnalyticsEvent"]
  DUPCLOSURE R5 K26 [PROTO_7]
  CAPTURE UPVAL U7
  CAPTURE UPVAL U8
  CAPTURE UPVAL U9
  NAMECALL R2 R0 K8 ["OnInvoke"]
  CALL R2 3 1
  SETTABLEKS R2 R1 K27 ["SendAnalyticsEventConnection"]
  GETUPVAL R2 10
  CALL R2 0 1
  JUMPIF R2 [+10]
  GETUPVAL R3 11
  GETTABLEKS R2 R3 K28 ["AssetImported"]
  NEWCLOSURE R4 P7
  CAPTURE VAL R0
  NAMECALL R2 R2 K5 ["Connect"]
  CALL R2 2 1
  SETTABLEKS R2 R1 K29 ["BulkImportImportedConnection"]
  RETURN R1 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["AssetManager"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R3 R0 K6 ["Packages"]
  GETTABLEKS R2 R3 K7 ["Dash"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R5 R0 K8 ["Src"]
  GETTABLEKS R4 R5 K9 ["Util"]
  GETTABLEKS R3 R4 K10 ["Services"]
  CALL R2 1 1
  GETTABLEKS R3 R2 K11 ["GetService"]
  LOADK R4 K12 ["AssetManagerService"]
  CALL R3 1 1
  GETTABLEKS R4 R2 K11 ["GetService"]
  LOADK R5 K13 ["BulkImportService"]
  CALL R4 1 1
  GETTABLEKS R5 R2 K11 ["GetService"]
  LOADK R6 K14 ["RbxAnalyticsService"]
  CALL R5 1 1
  GETIMPORT R6 K5 [require]
  GETTABLEKS R8 R0 K8 ["Src"]
  GETTABLEKS R7 R8 K15 ["Analytics"]
  CALL R6 1 1
  GETIMPORT R7 K5 [require]
  GETTABLEKS R11 R0 K8 ["Src"]
  GETTABLEKS R10 R11 K16 ["Asset"]
  GETTABLEKS R9 R10 K9 ["Util"]
  GETTABLEKS R8 R9 K17 ["handleInvokeOnInsertItems"]
  CALL R7 1 1
  GETIMPORT R8 K5 [require]
  GETTABLEKS R12 R0 K8 ["Src"]
  GETTABLEKS R11 R12 K16 ["Asset"]
  GETTABLEKS R10 R11 K9 ["Util"]
  GETTABLEKS R9 R10 K18 ["handleInvokeOnOpenBulkImport"]
  CALL R8 1 1
  GETIMPORT R9 K5 [require]
  GETTABLEKS R13 R0 K8 ["Src"]
  GETTABLEKS R12 R13 K16 ["Asset"]
  GETTABLEKS R11 R12 K9 ["Util"]
  GETTABLEKS R10 R11 K19 ["handleInvokeOnGrantAndInsertItems"]
  CALL R9 1 1
  GETIMPORT R10 K5 [require]
  GETTABLEKS R14 R0 K8 ["Src"]
  GETTABLEKS R13 R14 K16 ["Asset"]
  GETTABLEKS R12 R13 K9 ["Util"]
  GETTABLEKS R11 R12 K20 ["handleInvokeOnSelectItems"]
  CALL R10 1 1
  GETIMPORT R11 K5 [require]
  GETTABLEKS R14 R0 K8 ["Src"]
  GETTABLEKS R13 R14 K21 ["Flags"]
  GETTABLEKS R12 R13 K22 ["getFFlagAmrUseAssetAccessController"]
  CALL R11 1 1
  GETIMPORT R12 K5 [require]
  GETTABLEKS R15 R0 K8 ["Src"]
  GETTABLEKS R14 R15 K21 ["Flags"]
  GETTABLEKS R13 R14 K23 ["getEFCinUnifiedImportQueue"]
  CALL R12 1 1
  GETIMPORT R13 K5 [require]
  GETTABLEKS R16 R0 K8 ["Src"]
  GETTABLEKS R15 R16 K21 ["Flags"]
  GETTABLEKS R14 R15 K24 ["getFFlagAmrSelectAssetsInExplorer"]
  CALL R13 1 1
  DUPCLOSURE R14 K25 [PROTO_0]
  DUPCLOSURE R15 K26 [PROTO_9]
  CAPTURE VAL R11
  CAPTURE VAL R9
  CAPTURE VAL R7
  CAPTURE VAL R13
  CAPTURE VAL R10
  CAPTURE VAL R8
  CAPTURE VAL R3
  CAPTURE VAL R5
  CAPTURE VAL R14
  CAPTURE VAL R1
  CAPTURE VAL R12
  CAPTURE VAL R4
  RETURN R15 1

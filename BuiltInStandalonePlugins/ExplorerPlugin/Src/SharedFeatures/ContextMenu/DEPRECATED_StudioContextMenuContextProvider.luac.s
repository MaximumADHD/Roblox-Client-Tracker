PROTO_0:
        0 GETUPVAL                         R0 0
        1 ADDK                             R0 R0 K0 [1]
        2 SETUPVAL                         R0 0
        3 GETUPVAL                         R0 0
        4 RETURN                           R0 1

PROTO_1:
        0 MOVE                             R2 R0
        1 LOADNIL                          R3
        2 LOADNIL                          R4
        3 FORGPREP                         R2
        4 GETUPVAL                         R8 0
        5 ADDK                             R8 R8 K0 [1]
        6 SETUPVAL                         R8 0
        7 GETUPVAL                         R7 0
        8 MOVE                             R8 R6
        9 LOADNIL                          R9
       10 LOADNIL                          R10
       11 FORGPREP                         R8
       12 GETUPVAL                         R13 1
       13 GETTABLEKS                       R14 R12 K1 ["key"]
       15 DUPTABLE                         R15 K6 [{"item", "categoryIndex", "submenusInside", "index"}]
       16 SETTABLEKS                       R12 R15 K2 ["item"]
       18 SETTABLEKS                       R7 R15 K3 ["categoryIndex"]
       20 SETTABLEKS                       R1 R15 K4 ["submenusInside"]
       22 GETUPVAL                         R17 0
       23 ADDK                             R17 R17 K0 [1]
       24 SETUPVAL                         R17 0
       25 GETUPVAL                         R16 0
       26 SETTABLEKS                       R16 R15 K5 ["index"]
       28 SETTABLE                         R15 R13 R14
       29 GETTABLEKS                       R13 R12 K7 ["source"]
       31 GETTABLEKS                       R13 R13 K8 ["type"]
       33 JUMPIFNOTEQKS                    R13 K9 ["submenu"] ; [+20]
       35 GETIMPORT                        R13 K12 [table.clone]
       37 MOVE                             R14 R1
       38 CALL                             R13 1 1
       39 GETTABLEKS                       R16 R12 K1 ["key"]
       41 FASTCALL2                        TABLE_INSERT R13 R16 ; [+4]
       43 MOVE                             R15 R13
       44 GETIMPORT                        R14 K14 [table.insert]
       46 CALL                             R14 2 0
       47 GETUPVAL                         R14 2
       48 GETTABLEKS                       R15 R12 K7 ["source"]
       50 GETTABLEKS                       R15 R15 K15 ["submenuCategories"]
       52 MOVE                             R16 R13
       53 CALL                             R14 2 0
       54 FORGLOOP                         R8 2 ; [-43]
       56 FORGLOOP                         R2 2 ; [-53]
       58 RETURN                           R0 0

PROTO_2:
        0 GETUPVAL                         R2 0
        1 MOVE                             R3 R0
        2 LOADNIL                          R4
        3 LOADNIL                          R5
        4 FORGPREP                         R3
        5 GETUPVAL                         R9 1
        6 GETTABLE                         R8 R9 R7
        7 GETTABLEKS                       R10 R8 K0 ["categoryIndex"]
        9 GETTABLE                         R9 R2 R10
       10 JUMPIFNOTEQKNIL                  R9 ; [+6]
       12 NEWTABLE                         R9 0 0
       14 GETTABLEKS                       R10 R8 K0 ["categoryIndex"]
       16 SETTABLE                         R9 R2 R10
       17 JUMPIFNOTEQKNIL                  R9 ; [+2]
       19 LOADB                            R11 0 +1
       20 LOADB                            R11 1
       21 FASTCALL2K                       ASSERT R11 K1 ; [+4]
       23 LOADK                            R12 K1 ["Luau"]
       24 GETIMPORT                        R10 K3 [assert]
       26 CALL                             R10 2 0
       27 GETTABLEKS                       R11 R8 K4 ["index"]
       29 GETTABLE                         R10 R9 R11
       30 JUMPIFNOTEQKNIL                  R10 ; [+12]
       32 DUPTABLE                         R11 K9 [{["key"], ["type"] = "submenu", ["items"]}]
       33 SETTABLEKS                       R7 R11 K5 ["key"]
       35 NEWTABLE                         R12 0 0
       37 SETTABLEKS                       R12 R11 K8 ["items"]
       39 MOVE                             R10 R11
       40 GETTABLEKS                       R11 R8 K4 ["index"]
       42 SETTABLE                         R10 R9 R11
       43 JUMPIFNOTEQKNIL                  R10 ; [+2]
       45 LOADB                            R12 0 +1
       46 LOADB                            R12 1
       47 FASTCALL2K                       ASSERT R12 K1 ; [+4]
       49 LOADK                            R13 K1 ["Luau"]
       50 GETIMPORT                        R11 K3 [assert]
       52 CALL                             R11 2 0
       53 GETTABLEKS                       R13 R10 K6 ["type"]
       55 JUMPIFEQKS                       R13 K7 ["submenu"] ; [+2]
       57 LOADB                            R12 0 +1
       58 LOADB                            R12 1
       59 FASTCALL2K                       ASSERT R12 K10 ; [+4]
       61 LOADK                            R13 K10 ["Found submenu is not a submenu"]
       62 GETIMPORT                        R11 K3 [assert]
       64 CALL                             R11 2 0
       65 GETTABLEKS                       R2 R10 K8 ["items"]
       67 FORGLOOP                         R3 2 ; [-63]
       69 GETTABLE                         R3 R2 R1
       70 JUMPIFNOTEQKNIL                  R3 ; [+5]
       72 NEWTABLE                         R4 0 0
       74 SETTABLE                         R4 R2 R1
       75 RETURN                           R4 1
       76 RETURN                           R3 1

PROTO_3:
        0 NEWTABLE                         R2 0 0
        2 NEWCLOSURE                       R3 P0
        3 CAPTURE                          VAL R2
        4 CAPTURE                          UPVAL U0
        5 MOVE                             R4 R0
        6 LOADNIL                          R5
        7 LOADNIL                          R6
        8 FORGPREP                         R4
        9 GETUPVAL                         R10 0
       10 GETTABLE                         R9 R10 R7
       11 JUMPIFNOTEQKNIL                  R9 ; [+10]
       13 GETIMPORT                        R10 K1 [error]
       15 LOADK                            R12 K2 ["Couldn't find action that Studio gave: %*"]
       16 MOVE                             R14 R7
       17 NAMECALL                         R12 R12 K3 ["format"]
       19 CALL                             R12 2 1
       20 MOVE                             R11 R12
       21 CALL                             R10 1 0
       22 MOVE                             R10 R3
       23 GETTABLEKS                       R11 R9 K4 ["submenusInside"]
       25 GETTABLEKS                       R12 R9 K5 ["categoryIndex"]
       27 CALL                             R10 2 1
       28 GETTABLEKS                       R11 R9 K6 ["item"]
       30 GETTABLEKS                       R11 R11 K7 ["source"]
       32 GETTABLEKS                       R11 R11 K8 ["type"]
       34 JUMPIFNOTEQKS                    R11 K9 ["studioAction"] ; [+22]
       36 GETTABLEKS                       R11 R1 K10 ["getStudioActionState"]
       38 GETTABLEKS                       R12 R9 K6 ["item"]
       40 GETTABLEKS                       R12 R12 K7 ["source"]
       42 GETTABLEKS                       R12 R12 K11 ["uri"]
       44 CALL                             R11 1 1
       45 GETTABLEKS                       R12 R9 K12 ["index"]
       47 DUPTABLE                         R13 K14 [{["type"] = "action", ["action"]}]
       48 DUPTABLE                         R14 K17 [{"key", "state"}]
       49 SETTABLEKS                       R7 R14 K15 ["key"]
       51 SETTABLEKS                       R11 R14 K16 ["state"]
       53 SETTABLEKS                       R14 R13 K13 ["action"]
       55 SETTABLE                         R13 R10 R12
       56 JUMP                             ; [+26]
       57 GETTABLEKS                       R11 R9 K6 ["item"]
       59 GETTABLEKS                       R11 R11 K7 ["source"]
       61 GETTABLEKS                       R11 R11 K8 ["type"]
       63 JUMPIFNOTEQKS                    R11 K18 ["submenu"] ; [+11]
       65 GETIMPORT                        R11 K1 [error]
       67 LOADK                            R13 K19 ["Submenu is being sent as something that is enabled, but this should just be real actions: %*"]
       68 MOVE                             R15 R7
       69 NAMECALL                         R13 R13 K3 ["format"]
       71 CALL                             R13 2 1
       72 MOVE                             R12 R13
       73 CALL                             R11 1 0
       74 JUMP                             ; [+8]
       75 GETUPVAL                         R11 1
       76 GETTABLEKS                       R12 R9 K6 ["item"]
       78 GETTABLEKS                       R12 R12 K7 ["source"]
       80 GETTABLEKS                       R12 R12 K8 ["type"]
       82 CALL                             R11 1 0
       83 FORGLOOP                         R4 1 ; [-75]
       85 RETURN                           R2 1

PROTO_4:
        0 GETUPVAL                         R0 0
        1 GETUPVAL                         R1 1
        2 CALL                             R0 1 -1
        3 RETURN                           R0 -1

PROTO_5:
        0 GETUPVAL                         R0 0
        1 GETTABLEKS                       R0 R0 K0 ["destroy"]
        3 CALL                             R0 0 0
        4 RETURN                           R0 0

PROTO_6:
        0 NEWCLOSURE                       R0 P0
        1 CAPTURE                          UPVAL U0
        2 RETURN                           R0 1

PROTO_7:
        0 GETUPVAL                         R1 0
        1 LOADK                            R4 K0 ["Explorer_"]
        2 GETUPVAL                         R5 1
        3 NAMECALL                         R5 R5 K1 ["GenerateGUID"]
        5 CALL                             R5 1 1
        6 CONCAT                           R3 R4 R5
        7 NAMECALL                         R1 R1 K2 ["CreatePluginMenu"]
        9 CALL                             R1 2 1
       10 GETUPVAL                         R2 2
       11 MOVE                             R3 R0
       12 CALL                             R2 1 1
       13 LOADB                            R3 0
       14 MOVE                             R4 R2
       15 LOADNIL                          R5
       16 LOADNIL                          R6
       17 FORGPREP                         R4
       18 GETUPVAL                         R12 2
       19 MOVE                             R13 R8
       20 CALL                             R12 1 1
       21 MOVE                             R9 R12
       22 LOADNIL                          R10
       23 LOADNIL                          R11
       24 FORGPREP                         R9
       25 GETTABLEKS                       R14 R13 K3 ["type"]
       27 JUMPIFNOTEQKS                    R14 K4 ["submenu"] ; [+22]
       29 GETUPVAL                         R14 3
       30 GETTABLEKS                       R15 R13 K5 ["items"]
       32 CALL                             R14 1 1
       33 JUMPIFEQKNIL                     R14 ; [+84]
       35 GETUPVAL                         R15 4
       36 LOADK                            R17 K6 ["ContextMenu"]
       37 GETTABLEKS                       R18 R13 K7 ["key"]
       39 NAMECALL                         R15 R15 K8 ["getText"]
       41 CALL                             R15 3 1
       42 SETTABLEKS                       R15 R14 K9 ["Title"]
       44 MOVE                             R17 R14
       45 NAMECALL                         R15 R1 K10 ["AddMenu"]
       47 CALL                             R15 2 0
       48 LOADB                            R3 1
       49 JUMP                             ; [+68]
       50 GETTABLEKS                       R14 R13 K3 ["type"]
       52 JUMPIFNOTEQKS                    R14 K11 ["action"] ; [+65]
       54 GETTABLEKS                       R14 R13 K11 ["action"]
       56 GETTABLEKS                       R14 R14 K12 ["state"]
       58 GETTABLEKS                       R14 R14 K13 ["Enabled"]
       60 JUMPIFNOT                        R14 ; [+57]
       61 GETTABLEKS                       R14 R13 K11 ["action"]
       63 GETTABLEKS                       R14 R14 K12 ["state"]
       65 GETTABLEKS                       R14 R14 K14 ["Visible"]
       67 JUMPIFNOT                        R14 ; [+50]
       68 LOADB                            R3 1
       69 GETUPVAL                         R15 5
       70 GETTABLEKS                       R16 R13 K11 ["action"]
       72 GETTABLEKS                       R16 R16 K7 ["key"]
       74 GETTABLE                         R14 R15 R16
       75 JUMPIFNOTEQKNIL                  R14 ; [+2]
       77 LOADB                            R16 0 +1
       78 LOADB                            R16 1
       79 FASTCALL2K                       ASSERT R16 K15 ; [+4]
       81 LOADK                            R17 K15 ["Couldn't find plugin action"]
       82 GETIMPORT                        R15 K17 [assert]
       84 CALL                             R15 2 0
       85 GETTABLEKS                       R15 R13 K11 ["action"]
       87 GETTABLEKS                       R15 R15 K12 ["state"]
       89 GETTABLEKS                       R15 R15 K18 ["Text"]
       91 SETTABLEKS                       R15 R14 K18 ["Text"]
       93 GETTABLEKS                       R16 R14 K18 ["Text"]
       95 LOADK                            R17 K19 ["     "]
       96 CONCAT                           R15 R16 R17
       97 SETTABLEKS                       R15 R14 K18 ["Text"]
       99 GETTABLEKS                       R16 R13 K11 ["action"]
      101 GETTABLEKS                       R16 R16 K12 ["state"]
      103 GETTABLEKS                       R16 R16 K20 ["Shortcuts"]
      105 GETTABLEN                        R15 R16 1
      106 JUMPIFNOTEQKNIL                  R15 ; [+5]
      108 LOADK                            R16 K21 [""]
      109 SETTABLEKS                       R16 R14 K22 ["DefaultShortcut"]
      111 JUMP                             ; [+2]
      112 SETTABLEKS                       R15 R14 K22 ["DefaultShortcut"]
      114 MOVE                             R18 R14
      115 NAMECALL                         R16 R1 K23 ["AddAction"]
      117 CALL                             R16 2 0
      118 FORGLOOP                         R9 2 ; [-94]
      120 LENGTH                           R9 R2
      121 JUMPIFEQ                         R7 R9 ; [+5]
      123 JUMPIFNOT                        R3 ; [+3]
      124 NAMECALL                         R9 R1 K24 ["AddSeparator"]
      126 CALL                             R9 1 0
      127 FORGLOOP                         R4 2 ; [-110]
      129 JUMPIFNOT                        R3 ; [+8]
      130 GETUPVAL                         R5 6
      131 FASTCALL2                        TABLE_INSERT R5 R1 ; [+4]
      133 MOVE                             R6 R1
      134 GETIMPORT                        R4 K27 [table.insert]
      136 CALL                             R4 2 0
      137 RETURN                           R1 1
      138 LOADNIL                          R4
      139 RETURN                           R4 1

PROTO_8:
        0 NEWTABLE                         R0 0 0
        2 NEWCLOSURE                       R1 P0
        3 CAPTURE                          UPVAL U0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          UPVAL U2
        6 CAPTURE                          VAL R1
        7 CAPTURE                          UPVAL U3
        8 CAPTURE                          UPVAL U4
        9 CAPTURE                          VAL R0
       10 MOVE                             R2 R1
       11 GETUPVAL                         R3 5
       12 GETUPVAL                         R4 6
       13 GETUPVAL                         R5 7
       14 CALL                             R3 2 1
       15 CALL                             R2 1 1
       16 JUMPIFEQKNIL                     R2 ; [+4]
       18 NAMECALL                         R3 R2 K0 ["ShowAsync"]
       20 CALL                             R3 1 0
       21 GETUPVAL                         R3 4
       22 LOADNIL                          R4
       23 LOADNIL                          R5
       24 FORGPREP                         R3
       25 JUMPIFNOTEQKNIL                  R7 ; [+2]
       27 LOADB                            R9 0 +1
       28 LOADB                            R9 1
       29 FASTCALL2K                       ASSERT R9 K1 ; [+4]
       31 LOADK                            R10 K1 ["Luau"]
       32 GETIMPORT                        R8 K3 [assert]
       34 CALL                             R8 2 0
       35 LOADK                            R8 K4 [""]
       36 SETTABLEKS                       R8 R7 K5 ["DefaultShortcut"]
       38 FORGLOOP                         R3 2 ; [-14]
       40 MOVE                             R3 R0
       41 LOADNIL                          R4
       42 LOADNIL                          R5
       43 FORGPREP                         R3
       44 NAMECALL                         R8 R7 K6 ["Destroy"]
       46 CALL                             R8 1 0
       47 FORGLOOP                         R3 2 ; [-4]
       49 RETURN                           R0 0

PROTO_9:
        0 GETIMPORT                        R1 K2 [task.spawn]
        2 NEWCLOSURE                       R2 P0
        3 CAPTURE                          UPVAL U0
        4 CAPTURE                          UPVAL U1
        5 CAPTURE                          UPVAL U2
        6 CAPTURE                          UPVAL U3
        7 CAPTURE                          UPVAL U4
        8 CAPTURE                          UPVAL U5
        9 CAPTURE                          VAL R0
       10 CAPTURE                          UPVAL U6
       11 CALL                             R1 1 0
       12 RETURN                           R0 0

PROTO_10:
        0 GETUPVAL                         R1 0
        1 GETTABLEKS                       R1 R1 K0 ["ContextServices"]
        3 GETTABLEKS                       R1 R1 K1 ["Plugin"]
        5 NAMECALL                         R1 R1 K2 ["use"]
        7 CALL                             R1 1 1
        8 NAMECALL                         R1 R1 K3 ["get"]
       10 CALL                             R1 1 1
       11 GETUPVAL                         R2 1
       12 NAMECALL                         R2 R2 K2 ["use"]
       14 CALL                             R2 1 1
       15 GETUPVAL                         R3 2
       16 GETTABLEKS                       R3 R3 K4 ["useState"]
       18 NEWCLOSURE                       R4 P0
       19 CAPTURE                          UPVAL U3
       20 CAPTURE                          VAL R1
       21 CALL                             R3 1 1
       22 GETUPVAL                         R4 2
       23 GETTABLEKS                       R4 R4 K5 ["useEffect"]
       25 NEWCLOSURE                       R5 P1
       26 CAPTURE                          VAL R3
       27 NEWTABLE                         R6 0 1
       29 MOVE                             R7 R3
       30 SETLIST                          R6 R7 1 [1]
       32 CALL                             R4 2 0
       33 GETUPVAL                         R4 2
       34 GETTABLEKS                       R4 R4 K4 ["useState"]
       36 GETUPVAL                         R5 4
       37 GETTABLEKS                       R5 R5 K6 ["Signal"]
       39 GETTABLEKS                       R5 R5 K7 ["new"]
       41 CALL                             R4 1 1
       42 GETUPVAL                         R5 5
       43 MOVE                             R6 R4
       44 CALL                             R5 1 1
       45 GETUPVAL                         R6 2
       46 GETTABLEKS                       R6 R6 K8 ["useCallback"]
       48 NEWCLOSURE                       R7 P2
       49 CAPTURE                          VAL R1
       50 CAPTURE                          UPVAL U6
       51 CAPTURE                          UPVAL U7
       52 CAPTURE                          VAL R2
       53 CAPTURE                          VAL R5
       54 CAPTURE                          UPVAL U8
       55 CAPTURE                          VAL R3
       56 NEWTABLE                         R8 0 2
       58 MOVE                             R9 R2
       59 MOVE                             R10 R5
       60 SETLIST                          R8 R9 2 [1]
       62 CALL                             R6 2 1
       63 GETUPVAL                         R8 4
       64 GETTABLEKS                       R8 R8 K9 ["Flags"]
       66 GETTABLEKS                       R8 R8 K10 ["getFFlagExplorerStreaming"]
       68 CALL                             R8 0 1
       69 JUMPIFNOT                        R8 ; [+3]
       70 GETUPVAL                         R7 9
       71 CALL                             R7 0 1
       72 JUMP                             ; [+1]
       73 LOADNIL                          R7
       74 GETUPVAL                         R8 10
       75 GETUPVAL                         R9 4
       76 GETTABLEKS                       R9 R9 K11 ["Components"]
       78 GETTABLEKS                       R9 R9 K12 ["Contexts"]
       80 GETTABLEKS                       R9 R9 K13 ["ContextMenuContextProvider"]
       82 DUPTABLE                         R10 K18 [{"showContextMenu", "actionTriggeredSignal", "showStreamingContextMenu", "streamingActionTriggeredSignal"}]
       83 SETTABLEKS                       R6 R10 K14 ["showContextMenu"]
       85 SETTABLEKS                       R4 R10 K15 ["actionTriggeredSignal"]
       87 MOVE                             R11 R7
       88 JUMPIFNOT                        R11 ; [+2]
       89 GETTABLEKS                       R11 R7 K16 ["showStreamingContextMenu"]
       91 SETTABLEKS                       R11 R10 K16 ["showStreamingContextMenu"]
       93 MOVE                             R11 R7
       94 JUMPIFNOT                        R11 ; [+2]
       95 GETTABLEKS                       R11 R7 K17 ["streamingActionTriggeredSignal"]
       97 SETTABLEKS                       R11 R10 K17 ["streamingActionTriggeredSignal"]
       99 GETTABLEKS                       R11 R0 K19 ["children"]
      101 CALL                             R8 3 -1
      102 RETURN                           R8 -1

MAIN:
        0 PREPVARARGS                      0
        1 GETIMPORT                        R0 K1 [game]
        3 LOADK                            R2 K2 ["HttpService"]
        4 NAMECALL                         R0 R0 K3 ["GetService"]
        6 CALL                             R0 2 1
        7 GETIMPORT                        R1 K5 [script]
        9 LOADK                            R3 K6 ["ExplorerPlugin"]
       10 NAMECALL                         R1 R1 K7 ["FindFirstAncestor"]
       12 CALL                             R1 2 1
       13 GETIMPORT                        R2 K9 [require]
       15 GETTABLEKS                       R3 R1 K10 ["Src"]
       17 GETTABLEKS                       R3 R3 K11 ["SharedFeatures"]
       19 GETTABLEKS                       R3 R3 K12 ["ContextMenu"]
       21 GETTABLEKS                       R3 R3 K13 ["ContextMenuActionList"]
       23 CALL                             R2 1 1
       24 GETIMPORT                        R3 K9 [require]
       26 GETTABLEKS                       R4 R1 K10 ["Src"]
       28 GETTABLEKS                       R4 R4 K11 ["SharedFeatures"]
       30 GETTABLEKS                       R4 R4 K12 ["ContextMenu"]
       32 GETTABLEKS                       R4 R4 K14 ["ContextMenuTypes"]
       34 CALL                             R3 1 1
       35 GETIMPORT                        R4 K9 [require]
       37 GETTABLEKS                       R5 R1 K10 ["Src"]
       39 GETTABLEKS                       R5 R5 K11 ["SharedFeatures"]
       41 GETTABLEKS                       R5 R5 K12 ["ContextMenu"]
       43 GETTABLEKS                       R5 R5 K15 ["DEPRECATED_createContextMenuActionWatcher"]
       45 CALL                             R4 1 1
       46 GETIMPORT                        R5 K9 [require]
       48 GETTABLEKS                       R6 R1 K10 ["Src"]
       50 GETTABLEKS                       R6 R6 K11 ["SharedFeatures"]
       52 GETTABLEKS                       R6 R6 K12 ["ContextMenu"]
       54 GETTABLEKS                       R6 R6 K16 ["DEPRECATED_usePluginActions"]
       56 CALL                             R5 1 1
       57 GETIMPORT                        R6 K9 [require]
       59 GETTABLEKS                       R7 R1 K17 ["Packages"]
       61 GETTABLEKS                       R7 R7 K18 ["Explorer"]
       63 CALL                             R6 1 1
       64 GETIMPORT                        R7 K9 [require]
       66 GETTABLEKS                       R8 R1 K17 ["Packages"]
       68 GETTABLEKS                       R8 R8 K19 ["Framework"]
       70 CALL                             R7 1 1
       71 GETIMPORT                        R8 K9 [require]
       73 GETTABLEKS                       R9 R1 K17 ["Packages"]
       75 GETTABLEKS                       R9 R9 K20 ["React"]
       77 CALL                             R8 1 1
       78 GETIMPORT                        R9 K9 [require]
       80 GETTABLEKS                       R10 R1 K10 ["Src"]
       82 GETTABLEKS                       R10 R10 K21 ["Util"]
       84 GETTABLEKS                       R10 R10 K22 ["cleanHoles"]
       86 CALL                             R9 1 1
       87 GETIMPORT                        R10 K9 [require]
       89 GETTABLEKS                       R11 R1 K10 ["Src"]
       91 GETTABLEKS                       R11 R11 K21 ["Util"]
       93 GETTABLEKS                       R11 R11 K23 ["exhaustiveMatch"]
       95 CALL                             R10 1 1
       96 GETIMPORT                        R11 K9 [require]
       98 GETTABLEKS                       R12 R1 K10 ["Src"]
      100 GETTABLEKS                       R12 R12 K11 ["SharedFeatures"]
      102 GETTABLEKS                       R12 R12 K12 ["ContextMenu"]
      104 GETTABLEKS                       R12 R12 K24 ["useStreamingContextMenu"]
      106 CALL                             R11 1 1
      107 GETTABLEKS                       R12 R7 K25 ["ContextServices"]
      109 GETTABLEKS                       R12 R12 K26 ["Localization"]
      111 GETTABLEKS                       R13 R8 K27 ["createElement"]
      113 NEWTABLE                         R14 0 0
      115 LOADN                            R15 0
      116 NEWCLOSURE                       R16 P0
      117 CAPTURE                          REF R15
      118 NEWCLOSURE                       R17 P1
      119 CAPTURE                          REF R15
      120 CAPTURE                          VAL R14
      121 CAPTURE                          VAL R17
      122 MOVE                             R18 R17
      123 MOVE                             R19 R2
      124 NEWTABLE                         R20 0 0
      126 CALL                             R18 2 0
      127 DUPCLOSURE                       R18 K28 [PROTO_3]
      128 CAPTURE                          VAL R14
      129 CAPTURE                          VAL R10
      130 DUPCLOSURE                       R19 K29 [PROTO_10]
      131 CAPTURE                          VAL R7
      132 CAPTURE                          VAL R12
      133 CAPTURE                          VAL R8
      134 CAPTURE                          VAL R4
      135 CAPTURE                          VAL R6
      136 CAPTURE                          VAL R5
      137 CAPTURE                          VAL R0
      138 CAPTURE                          VAL R9
      139 CAPTURE                          VAL R18
      140 CAPTURE                          VAL R11
      141 CAPTURE                          VAL R13
      142 CLOSEUPVALS                      R15
      143 RETURN                           R19 1

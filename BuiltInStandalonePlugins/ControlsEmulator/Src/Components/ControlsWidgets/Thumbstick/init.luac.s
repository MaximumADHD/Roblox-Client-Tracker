PROTO_0:
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K0 ["inputState"]
  JUMPIFNOTEQKNIL R1 [+9]
  DUPTABLE R0 K3 [{"xOffset", "yOffset"}]
  LOADN R1 0
  SETTABLEKS R1 R0 K1 ["xOffset"]
  LOADN R1 0
  SETTABLEKS R1 R0 K2 ["yOffset"]
  RETURN R0 1
  DUPTABLE R0 K3 [{"xOffset", "yOffset"}]
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K0 ["inputState"]
  GETTABLEKS R3 R4 K4 ["X"]
  GETUPVAL R4 1
  MUL R2 R3 R4
  GETUPVAL R4 1
  MINUS R3 R4
  GETUPVAL R4 1
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R1 K7 [math.clamp]
  CALL R1 3 1
  SETTABLEKS R1 R0 K1 ["xOffset"]
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K0 ["inputState"]
  GETTABLEKS R4 R5 K8 ["Y"]
  MINUS R3 R4
  GETUPVAL R4 1
  MUL R2 R3 R4
  GETUPVAL R4 1
  MINUS R3 R4
  GETUPVAL R4 1
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R1 K7 [math.clamp]
  CALL R1 3 1
  SETTABLEKS R1 R0 K2 ["yOffset"]
  RETURN R0 1

PROTO_1:
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K0 ["xOffset"]
  GETTABLEKS R6 R0 K1 ["X"]
  GETTABLEKS R5 R6 K2 ["Offset"]
  ADD R3 R4 R5
  LOADN R5 0
  GETUPVAL R6 1
  SUB R4 R5 R6
  GETUPVAL R5 1
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R2 K5 [math.clamp]
  CALL R2 3 1
  GETUPVAL R3 1
  ADD R1 R2 R3
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K6 ["yOffset"]
  GETTABLEKS R7 R0 K7 ["Y"]
  GETTABLEKS R6 R7 K2 ["Offset"]
  ADD R4 R5 R6
  LOADN R6 0
  GETUPVAL R7 1
  SUB R5 R6 R7
  GETUPVAL R6 1
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R3 K5 [math.clamp]
  CALL R3 3 1
  GETUPVAL R4 1
  ADD R2 R3 R4
  GETIMPORT R3 K10 [UDim2.new]
  LOADN R4 0
  MOVE R5 R1
  LOADN R6 0
  MOVE R7 R2
  CALL R3 4 -1
  RETURN R3 -1

PROTO_2:
  GETTABLEKS R6 R0 K1 ["X"]
  GETTABLEKS R5 R6 K2 ["Offset"]
  GETUPVAL R6 0
  DIV R4 R5 R6
  MULK R3 R4 K0 [2]
  LOADN R4 255
  LOADN R5 1
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R2 K5 [math.clamp]
  CALL R2 3 1
  GETTABLEKS R8 R0 K6 ["Y"]
  GETTABLEKS R7 R8 K2 ["Offset"]
  MINUS R6 R7
  GETUPVAL R7 0
  DIV R5 R6 R7
  MULK R4 R5 K0 [2]
  LOADN R5 255
  LOADN R6 1
  FASTCALL MATH_CLAMP [+2]
  GETIMPORT R3 K5 [math.clamp]
  CALL R3 3 1
  LOADN R4 0
  FASTCALL VECTOR [+2]
  GETIMPORT R1 K9 [Vector3.new]
  CALL R1 3 1
  RETURN R1 1

PROTO_3:
  JUMPIFNOT R0 [+3]
  GETUPVAL R1 0
  SETTABLEKS R0 R1 K0 ["current"]
  RETURN R0 0

PROTO_4:
  GETUPVAL R0 0
  GETUPVAL R2 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K0 ["keyCode"]
  GETUPVAL R4 3
  GETIMPORT R5 K3 [Vector3.zero]
  NAMECALL R0 R0 K4 ["SendGamepadEvent"]
  CALL R0 5 0
  RETURN R0 0

PROTO_5:
  GETTABLEKS R1 R0 K0 ["UserInputType"]
  GETIMPORT R2 K3 [Enum.UserInputType.MouseMovement]
  JUMPIFNOTEQ R1 R2 [+52]
  GETUPVAL R1 0
  GETIMPORT R2 K6 [UDim2.new]
  LOADN R3 0
  GETTABLEKS R6 R0 K7 ["Position"]
  GETTABLEKS R5 R6 K8 ["X"]
  GETUPVAL R7 1
  GETTABLEKS R6 R7 K9 ["current"]
  SUB R4 R5 R6
  LOADN R5 0
  GETTABLEKS R8 R0 K7 ["Position"]
  GETTABLEKS R7 R8 K10 ["Y"]
  GETUPVAL R9 2
  GETTABLEKS R8 R9 K9 ["current"]
  SUB R6 R7 R8
  CALL R2 4 1
  SETTABLEKS R2 R1 K9 ["current"]
  GETUPVAL R2 3
  GETTABLEKS R1 R2 K9 ["current"]
  JUMPIFNOT R1 [+10]
  GETUPVAL R2 3
  GETTABLEKS R1 R2 K9 ["current"]
  GETUPVAL R2 4
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K9 ["current"]
  CALL R2 1 1
  SETTABLEKS R2 R1 K7 ["Position"]
  GETUPVAL R1 5
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K9 ["current"]
  CALL R1 1 1
  GETIMPORT R2 K12 [pcall]
  NEWCLOSURE R3 P0
  CAPTURE UPVAL U6
  CAPTURE UPVAL U7
  CAPTURE UPVAL U8
  CAPTURE VAL R1
  CALL R2 1 0
  RETURN R0 0

PROTO_6:
  GETUPVAL R0 0
  GETUPVAL R2 1
  GETUPVAL R4 2
  GETTABLEKS R3 R4 K0 ["keyCode"]
  GETIMPORT R4 K3 [Vector3.zero]
  GETIMPORT R5 K3 [Vector3.zero]
  NAMECALL R0 R0 K4 ["SendGamepadEvent"]
  CALL R0 5 0
  RETURN R0 0

PROTO_7:
  GETTABLEKS R1 R0 K0 ["UserInputType"]
  GETIMPORT R2 K3 [Enum.UserInputType.MouseButton1]
  JUMPIFNOTEQ R1 R2 [+56]
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K4 ["current"]
  JUMPIFEQKNIL R1 [+51]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K4 ["current"]
  JUMPIFEQKNIL R1 [+46]
  GETUPVAL R2 0
  GETTABLEKS R1 R2 K4 ["current"]
  NAMECALL R1 R1 K5 ["Disconnect"]
  CALL R1 1 0
  GETUPVAL R1 0
  LOADNIL R2
  SETTABLEKS R2 R1 K4 ["current"]
  GETUPVAL R2 1
  GETTABLEKS R1 R2 K4 ["current"]
  NAMECALL R1 R1 K5 ["Disconnect"]
  CALL R1 1 0
  GETUPVAL R1 1
  LOADNIL R2
  SETTABLEKS R2 R1 K4 ["current"]
  GETIMPORT R1 K7 [pcall]
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U2
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CALL R1 1 0
  GETUPVAL R1 5
  GETUPVAL R2 6
  SETTABLEKS R2 R1 K4 ["current"]
  GETUPVAL R2 7
  GETTABLEKS R1 R2 K4 ["current"]
  JUMPIFNOT R1 [+10]
  GETUPVAL R2 7
  GETTABLEKS R1 R2 K4 ["current"]
  GETUPVAL R2 8
  GETUPVAL R4 5
  GETTABLEKS R3 R4 K4 ["current"]
  CALL R2 1 1
  SETTABLEKS R2 R1 K8 ["Position"]
  RETURN R0 0

PROTO_8:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["current"]
  JUMPIFNOTEQKNIL R2 [+71]
  GETUPVAL R2 1
  JUMPIFEQKNIL R2 [+68]
  GETUPVAL R2 2
  JUMPIFEQKNIL R2 [+65]
  GETTABLEKS R2 R1 K1 ["UserInputType"]
  GETIMPORT R3 K4 [Enum.UserInputType.MouseButton1]
  JUMPIFNOTEQ R2 R3 [+59]
  GETUPVAL R3 3
  GETTABLEKS R2 R3 K0 ["current"]
  JUMPIFEQKNIL R2 [+54]
  GETUPVAL R3 4
  GETTABLEKS R2 R3 K0 ["current"]
  JUMPIFEQKNIL R2 [+49]
  GETUPVAL R2 3
  GETTABLEKS R4 R1 K5 ["Position"]
  GETTABLEKS R3 R4 K6 ["X"]
  SETTABLEKS R3 R2 K0 ["current"]
  GETUPVAL R2 4
  GETTABLEKS R4 R1 K5 ["Position"]
  GETTABLEKS R3 R4 K7 ["Y"]
  SETTABLEKS R3 R2 K0 ["current"]
  GETUPVAL R2 0
  GETUPVAL R3 1
  NEWCLOSURE R5 P0
  CAPTURE UPVAL U5
  CAPTURE UPVAL U3
  CAPTURE UPVAL U4
  CAPTURE UPVAL U6
  CAPTURE UPVAL U7
  CAPTURE UPVAL U8
  CAPTURE UPVAL U9
  CAPTURE UPVAL U10
  CAPTURE UPVAL U11
  NAMECALL R3 R3 K8 ["Connect"]
  CALL R3 2 1
  SETTABLEKS R3 R2 K0 ["current"]
  GETUPVAL R2 12
  GETUPVAL R3 2
  NEWCLOSURE R5 P1
  CAPTURE UPVAL U0
  CAPTURE UPVAL U12
  CAPTURE UPVAL U9
  CAPTURE UPVAL U10
  CAPTURE UPVAL U11
  CAPTURE UPVAL U5
  CAPTURE UPVAL U13
  CAPTURE UPVAL U6
  CAPTURE UPVAL U7
  NAMECALL R3 R3 K8 ["Connect"]
  CALL R3 2 1
  SETTABLEKS R3 R2 K0 ["current"]
  RETURN R0 0

PROTO_9:
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K0 ["useContext"]
  GETUPVAL R3 1
  CALL R2 1 1
  GETTABLEKS R1 R2 K1 ["gamepadId"]
  GETUPVAL R3 0
  GETTABLEKS R2 R3 K2 ["useRef"]
  LOADNIL R3
  CALL R2 1 1
  GETUPVAL R4 0
  GETTABLEKS R3 R4 K2 ["useRef"]
  LOADNIL R4
  CALL R3 1 1
  GETUPVAL R5 0
  GETTABLEKS R4 R5 K2 ["useRef"]
  LOADN R5 0
  CALL R4 1 1
  GETUPVAL R6 0
  GETTABLEKS R5 R6 K2 ["useRef"]
  LOADN R6 0
  CALL R5 1 1
  GETUPVAL R7 0
  GETTABLEKS R6 R7 K2 ["useRef"]
  GETUPVAL R7 2
  CALL R6 1 1
  GETUPVAL R8 0
  GETTABLEKS R7 R8 K2 ["useRef"]
  LOADNIL R8
  CALL R7 1 1
  GETUPVAL R9 0
  GETTABLEKS R8 R9 K0 ["useContext"]
  GETUPVAL R9 3
  CALL R8 1 1
  GETTABLEKS R9 R8 K3 ["InputChanged"]
  GETTABLEKS R10 R8 K4 ["InputEnded"]
  GETUPVAL R12 0
  GETTABLEKS R11 R12 K5 ["useMemo"]
  NEWCLOSURE R12 P0
  CAPTURE VAL R0
  CAPTURE UPVAL U4
  NEWTABLE R13 0 1
  GETTABLEKS R14 R0 K6 ["inputState"]
  SETLIST R13 R14 1 [1]
  CALL R11 2 1
  GETUPVAL R13 0
  GETTABLEKS R12 R13 K7 ["useCallback"]
  NEWCLOSURE R13 P1
  CAPTURE VAL R11
  CAPTURE UPVAL U4
  NEWTABLE R14 0 1
  MOVE R15 R11
  SETLIST R14 R15 1 [1]
  CALL R12 2 1
  GETUPVAL R14 0
  GETTABLEKS R13 R14 K7 ["useCallback"]
  DUPCLOSURE R14 K8 [PROTO_2]
  CAPTURE UPVAL U5
  CALL R13 1 1
  GETUPVAL R15 0
  GETTABLEKS R14 R15 K9 ["createElement"]
  LOADK R15 K10 ["Frame"]
  NEWTABLE R16 4 0
  GETUPVAL R18 0
  GETTABLEKS R17 R18 K11 ["Tag"]
  LOADK R18 K12 ["Widget-Thumbstick X-Circle data-testid=Widget-Thumbstick"]
  SETTABLE R18 R16 R17
  GETTABLEKS R17 R0 K13 ["Position"]
  SETTABLEKS R17 R16 K13 ["Position"]
  GETTABLEKS R17 R0 K14 ["Size"]
  SETTABLEKS R17 R16 K14 ["Size"]
  DUPTABLE R17 K16 [{"Stick"}]
  GETUPVAL R19 0
  GETTABLEKS R18 R19 K9 ["createElement"]
  LOADK R19 K10 ["Frame"]
  NEWTABLE R20 8 0
  MOVE R21 R12
  GETTABLEKS R22 R6 K17 ["current"]
  CALL R21 1 1
  SETTABLEKS R21 R20 K13 ["Position"]
  NEWCLOSURE R21 P3
  CAPTURE VAL R7
  SETTABLEKS R21 R20 K18 ["ref"]
  GETTABLEKS R22 R0 K6 ["inputState"]
  JUMPIFEQKNIL R22 [+8]
  GETIMPORT R21 K21 [Color3.new]
  LOADN R22 1
  LOADN R23 0
  LOADN R24 0
  CALL R21 3 1
  JUMP [+1]
  LOADNIL R21
  SETTABLEKS R21 R20 K22 ["BackgroundColor3"]
  GETUPVAL R22 0
  GETTABLEKS R21 R22 K11 ["Tag"]
  LOADK R22 K23 ["Widget-Thumbstick-Stick X-Circle data-testid=Widget-Thumbstick-Stick"]
  SETTABLE R22 R20 R21
  GETUPVAL R23 0
  GETTABLEKS R22 R23 K24 ["Event"]
  GETTABLEKS R21 R22 K25 ["InputBegan"]
  NEWCLOSURE R22 P4
  CAPTURE VAL R2
  CAPTURE VAL R9
  CAPTURE VAL R10
  CAPTURE VAL R4
  CAPTURE VAL R5
  CAPTURE VAL R6
  CAPTURE VAL R7
  CAPTURE VAL R12
  CAPTURE VAL R13
  CAPTURE UPVAL U6
  CAPTURE VAL R1
  CAPTURE VAL R0
  CAPTURE VAL R3
  CAPTURE UPVAL U2
  SETTABLE R22 R20 R21
  CALL R18 2 1
  SETTABLEKS R18 R17 K15 ["Stick"]
  CALL R14 3 -1
  RETURN R14 -1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["ControlsEmulator"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [require]
  GETTABLEKS R4 R0 K6 ["Src"]
  GETTABLEKS R3 R4 K7 ["Util"]
  GETTABLEKS R2 R3 K8 ["RootContainerInputContext"]
  CALL R1 1 1
  GETIMPORT R2 K5 [require]
  GETTABLEKS R5 R0 K6 ["Src"]
  GETTABLEKS R4 R5 K7 ["Util"]
  GETTABLEKS R3 R4 K9 ["Constants"]
  CALL R2 1 1
  GETIMPORT R4 K5 [require]
  GETTABLEKS R7 R0 K6 ["Src"]
  GETTABLEKS R6 R7 K7 ["Util"]
  GETTABLEKS R5 R6 K10 ["MockableProxyPluginComponents"]
  CALL R4 1 1
  GETTABLEKS R3 R4 K11 ["StudioDeviceEmulator"]
  GETIMPORT R4 K5 [require]
  GETTABLEKS R6 R0 K12 ["Packages"]
  GETTABLEKS R5 R6 K13 ["React"]
  CALL R4 1 1
  GETIMPORT R5 K5 [require]
  GETTABLEKS R8 R0 K6 ["Src"]
  GETTABLEKS R7 R8 K7 ["Util"]
  GETTABLEKS R6 R7 K14 ["MappingsContext"]
  CALL R5 1 1
  GETTABLEKS R6 R2 K15 ["THUMBSTICK_SIZE"]
  GETTABLEKS R7 R2 K16 ["THUMBSTICK_SOCKET_SIZE"]
  DIVK R8 R7 K17 [2]
  GETIMPORT R9 K20 [UDim2.new]
  LOADN R10 0
  SUB R11 R8 R6
  LOADN R12 0
  SUB R13 R8 R6
  CALL R9 4 1
  DUPCLOSURE R10 K21 [PROTO_9]
  CAPTURE VAL R4
  CAPTURE VAL R5
  CAPTURE VAL R9
  CAPTURE VAL R1
  CAPTURE VAL R8
  CAPTURE VAL R7
  CAPTURE VAL R3
  GETTABLEKS R11 R4 K22 ["memo"]
  MOVE R12 R10
  CALL R11 1 -1
  RETURN R11 -1

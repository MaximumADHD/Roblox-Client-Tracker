PROTO_0:
  GETUPVAL R0 0
  LOADK R2 K0 ["AssistantApplication"]
  NAMECALL R0 R0 K1 ["GetPluginComponent"]
  CALL R0 2 -1
  RETURN R0 -1

PROTO_1:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["ConvertImageDataToTempIdAsync"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_2:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["ReleaseTempIdAsync"]
  CALL R1 2 0
  RETURN R0 0

PROTO_3:
  GETIMPORT R2 K1 [pcall]
  NEWCLOSURE R3 P0
  CAPTURE VAL R1
  CALL R2 1 2
  JUMPIF R2 [+1]
  RETURN R0 0
  NEWCLOSURE R4 P1
  CAPTURE VAL R3
  SETTABLEKS R4 R0 K2 ["convertImageDataToTempIdAsync"]
  NEWCLOSURE R4 P2
  CAPTURE VAL R3
  SETTABLEKS R4 R0 K3 ["releaseTempIdAsync"]
  RETURN R0 0

PROTO_4:
  LOADB R0 1
  RETURN R0 1

PROTO_5:
  GETUPVAL R0 0
  LOADK R2 K0 ["AssistantBridge"]
  NAMECALL R0 R0 K1 ["GetPluginComponent"]
  CALL R0 2 -1
  RETURN R0 -1

PROTO_6:
  GETUPVAL R0 0
  LOADK R2 K0 ["AssistantExperimentation"]
  NAMECALL R0 R0 K1 ["GetPluginComponent"]
  CALL R0 2 -1
  RETURN R0 -1

PROTO_7:
  GETUPVAL R0 0
  GETUPVAL R2 1
  NAMECALL R0 R0 K0 ["RunBoundCodeAsync"]
  CALL R0 2 -1
  RETURN R0 -1

PROTO_8:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["BindCodeAsync"]
  CALL R1 2 1
  NEWCLOSURE R2 P0
  CAPTURE UPVAL U0
  CAPTURE VAL R1
  RETURN R1 2

PROTO_9:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["StopCodeAsync"]
  CALL R1 2 0
  RETURN R0 0

PROTO_10:
  GETUPVAL R5 0
  CALL R5 0 1
  JUMPIFNOT R5 [+16]
  GETUPVAL R5 1
  DUPTABLE R7 K5 [{"requestId", "instanceId", "script", "source", "isNewScript"}]
  SETTABLEKS R0 R7 K0 ["requestId"]
  SETTABLEKS R1 R7 K1 ["instanceId"]
  SETTABLEKS R2 R7 K2 ["script"]
  SETTABLEKS R3 R7 K3 ["source"]
  SETTABLEKS R4 R7 K4 ["isNewScript"]
  NAMECALL R5 R5 K6 ["UpdateScriptSourceAsync"]
  CALL R5 2 0
  RETURN R0 0
  GETUPVAL R5 2
  LOADK R7 K7 ["UpdateScriptSource"]
  DUPTABLE R8 K5 [{"requestId", "instanceId", "script", "source", "isNewScript"}]
  SETTABLEKS R0 R8 K0 ["requestId"]
  SETTABLEKS R1 R8 K1 ["instanceId"]
  SETTABLEKS R2 R8 K2 ["script"]
  SETTABLEKS R3 R8 K3 ["source"]
  SETTABLEKS R4 R8 K4 ["isNewScript"]
  NAMECALL R5 R5 K8 ["DisplayContent"]
  CALL R5 3 0
  RETURN R0 0

PROTO_11:
  GETIMPORT R0 K1 [error]
  LOADK R1 K2 ["Code execution not available for this environment"]
  CALL R0 1 0
  RETURN R0 0

PROTO_12:
  GETIMPORT R0 K1 [error]
  LOADK R1 K2 ["updateScriptSource is not available for this environment"]
  CALL R0 1 0
  RETURN R0 0

PROTO_13:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["CopyToClipboard"]
  CALL R1 2 0
  RETURN R0 0

PROTO_14:
  GETUPVAL R1 0
  MOVE R3 R0
  NAMECALL R1 R1 K0 ["GetClassIcon"]
  CALL R1 2 -1
  RETURN R1 -1

PROTO_15:
  GETUPVAL R0 0
  RETURN R0 1

PROTO_16:
  NEWTABLE R2 0 0
  DUPCLOSURE R3 K0 [PROTO_4]
  SETTABLEKS R3 R2 K1 ["isDevFrameworkAvailable"]
  GETUPVAL R3 0
  CALL R3 0 1
  JUMPIFNOT R3 [+4]
  GETUPVAL R3 1
  MOVE R4 R2
  MOVE R5 R0
  CALL R3 2 0
  GETIMPORT R3 K3 [pcall]
  NEWCLOSURE R4 P1
  CAPTURE VAL R0
  CALL R3 1 2
  GETIMPORT R5 K3 [pcall]
  NEWCLOSURE R6 P2
  CAPTURE VAL R0
  CALL R5 1 2
  JUMPIFNOT R3 [+15]
  NEWCLOSURE R7 P3
  CAPTURE VAL R4
  SETTABLEKS R7 R2 K4 ["loadCode"]
  NEWCLOSURE R7 P4
  CAPTURE VAL R4
  SETTABLEKS R7 R2 K5 ["stopCode"]
  NEWCLOSURE R7 P5
  CAPTURE UPVAL U2
  CAPTURE VAL R4
  CAPTURE UPVAL U3
  SETTABLEKS R7 R2 K6 ["updateScriptSource"]
  JUMP [+8]
  DUPCLOSURE R7 K7 [PROTO_11]
  SETTABLEKS R7 R2 K4 ["loadCode"]
  SETTABLEKS R7 R2 K5 ["stopCode"]
  DUPCLOSURE R8 K8 [PROTO_12]
  SETTABLEKS R8 R2 K6 ["updateScriptSource"]
  DUPCLOSURE R7 K9 [PROTO_13]
  CAPTURE UPVAL U4
  SETTABLEKS R7 R2 K10 ["copyToClipboard"]
  DUPCLOSURE R7 K11 [PROTO_14]
  CAPTURE UPVAL U4
  SETTABLEKS R7 R2 K12 ["getClassIcon"]
  GETUPVAL R10 5
  GETTABLEKS R9 R10 K13 ["Guest"]
  GETTABLEKS R8 R9 K14 ["LoadAssetHandlers"]
  GETTABLEKS R7 R8 K15 ["getRobloxScriptHandler"]
  GETIMPORT R8 K17 [game]
  CALL R7 1 1
  SETTABLEKS R7 R2 K18 ["loadAssetAsync"]
  GETUPVAL R10 5
  GETTABLEKS R9 R10 K13 ["Guest"]
  GETTABLEKS R8 R9 K19 ["RecordingHandlers"]
  GETTABLEKS R7 R8 K20 ["getPluginHandlers"]
  GETUPVAL R8 6
  CALL R7 1 1
  GETTABLEKS R8 R7 K21 ["startRecording"]
  SETTABLEKS R8 R2 K21 ["startRecording"]
  GETTABLEKS R8 R7 K22 ["endRecording"]
  SETTABLEKS R8 R2 K22 ["endRecording"]
  NEWCLOSURE R8 P10
  CAPTURE VAL R1
  SETTABLEKS R8 R2 K23 ["getNetworking"]
  GETUPVAL R8 7
  JUMPIFNOT R5 [+2]
  MOVE R9 R6
  JUMPIF R9 [+1]
  LOADNIL R9
  CALL R8 1 1
  SETTABLEKS R8 R2 K24 ["EventLogger"]
  RETURN R2 1

MAIN:
  PREPVARARGS 0
  GETIMPORT R0 K1 [script]
  LOADK R2 K2 ["Assistant"]
  NAMECALL R0 R0 K3 ["FindFirstAncestor"]
  CALL R0 2 1
  GETIMPORT R1 K5 [game]
  LOADK R3 K6 ["ChangeHistoryService"]
  NAMECALL R1 R1 K7 ["GetService"]
  CALL R1 2 1
  GETIMPORT R2 K5 [game]
  LOADK R4 K8 ["StudioService"]
  NAMECALL R2 R2 K7 ["GetService"]
  CALL R2 2 1
  GETIMPORT R3 K10 [pcall]
  GETIMPORT R5 K5 [game]
  GETTABLEKS R4 R5 K7 ["GetService"]
  GETIMPORT R5 K5 [game]
  LOADK R6 K11 ["ChatbotUIService"]
  CALL R3 3 2
  GETIMPORT R5 K13 [require]
  GETTABLEKS R7 R0 K14 ["Packages"]
  GETTABLEKS R6 R7 K15 ["AssistantUI"]
  CALL R5 1 1
  GETIMPORT R6 K13 [require]
  GETTABLEKS R9 R0 K16 ["Src"]
  GETTABLEKS R8 R9 K17 ["Flags"]
  GETTABLEKS R7 R8 K18 ["FFlagStudioAssistantApplication"]
  CALL R6 1 1
  GETIMPORT R7 K13 [require]
  GETTABLEKS R10 R0 K16 ["Src"]
  GETTABLEKS R9 R10 K17 ["Flags"]
  GETTABLEKS R8 R9 K19 ["FFlagStudioAssistantBridgeScriptDiff"]
  CALL R7 1 1
  GETIMPORT R8 K13 [require]
  GETTABLEKS R11 R0 K16 ["Src"]
  GETTABLEKS R10 R11 K20 ["Util"]
  GETTABLEKS R9 R10 K21 ["StudioNetworking"]
  CALL R8 1 1
  GETIMPORT R9 K13 [require]
  GETTABLEKS R12 R0 K16 ["Src"]
  GETTABLEKS R11 R12 K22 ["Host"]
  GETTABLEKS R10 R11 K23 ["StudioEventLogger"]
  CALL R9 1 1
  GETTABLEKS R10 R6 K24 ["Get"]
  GETTABLEKS R11 R7 K24 ["Get"]
  DUPCLOSURE R12 K25 [PROTO_3]
  DUPCLOSURE R13 K26 [PROTO_16]
  CAPTURE VAL R10
  CAPTURE VAL R12
  CAPTURE VAL R11
  CAPTURE VAL R4
  CAPTURE VAL R2
  CAPTURE VAL R5
  CAPTURE VAL R1
  CAPTURE VAL R9
  RETURN R13 1

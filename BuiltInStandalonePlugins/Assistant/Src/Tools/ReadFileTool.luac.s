PROTO_0:
  ...
  CALL R6 0 1
  GETTABLEKS R5 R6 K12 ["getScriptSource"]
  MOVE R6 R4
  CALL R5 1 1
  JUMPIFNOT R3 [+33]
  JUMPIFNOTEQKS R5 K13 [""] [+3]
  LOADK R6 K13 [""]
  RETURN R6 1
  LOADK R8 K14 ["
"]
  NAMECALL R6 R5 K15 ["split"]
  CALL R6 2 1
  NEWTABLE R7 0 0
  MOVE R8 R6
  LOADNIL R9
...
"]
  ...
  LOADK R7 K24 ["`start_line_one_indexed` and `end_line_one_indexed_inclusive` are required when `should_read_entire_file` is false"]
  CALL R6 1 0
  LOADN R6 1
  JUMPIFNOTLT R1 R6 [+5]
  GETIMPORT R6 K2 [error]
  LOADK R7 K25 ["`start_line_one_indexed` must be at least 1"]
  CALL R6 1 0
  JUMPIFNOTLT R2 R1 [+5]
  GETIMPORT R6 K2 [error]
  LOADK R7 K26 ["`end_line_one_indexed_inclusive` must be >= `start_line_one_indexed`"]
  CALL R6 1 0
  LOADK R8 K14 ["
"]
  NAMECALL R6 R5 K15 ["split"]
  ...
  MOVE R9 R2
  MOVE R10 R7
  GETIMPORT R8 K29 [math.min]
  CALL R8 2 1
  JUMPIFNOTLT R7 R1 [+10]
  GETIMPORT R9 K2 [error]
  LOADK R11 K30 ["`start_line_one_indexed` ("]
  MOVE R12 R1
  LOADK R13 K31 [") exceeds file length ("]
  MOVE R14 R7
  LOADK R15 K32 [")"]
  CONCAT R10 R11 R15
  CALL R9 1 0
  ...
  LOADK R13 K33 ["Note: Requested to read until line %*, but file only has %* lines"]
  MOVE R15 R2
  MOVE R16 R7
  ...
  CALL R10 2 0
  GETIMPORT R10 K23 [table.concat]
  MOVE R11 R9
  LOADK R12 K14 ["
"]
  CALL R10 2 -1
...
MAIN:
  ...
  GETTABLEKS R8 R1 K20 ["Utils"]
  GETTABLEKS R7 R8 K10 ["Tools"]
  GETTABLEKS R9 R2 K21 ["Util"]
  GETTABLEKS R8 R9 K22 ["ToolBuilder"]
  GETTABLEKS R10 R2 K21 ["Util"]
  GETTABLEKS R9 R10 K23 ["ToolResult"]
  GETTABLEKS R10 R3 K24 ["ToolNames"]

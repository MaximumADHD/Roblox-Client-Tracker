...
Item {
    ...
    function openDropdown() {
        ...
        var showX = container.x + container.width - gameGridView.contextMenuDropdown.width;
        var showY = container.y + container.height - dropdownOffset;

        if (gameGridView.contextMenuDropdown.attachedParent === container.attachedParent) {
            // The dropdown is already attached to this widget because we right clicked
            // So just move it
            gameGridView.contextMenuDropdown.moveTo(showX, showY);

        } else {
            // Currently on a different widget or not being shown
            // So move, become visible and give it model data
            gameGridView.contextMenuDropdown.show(showX, showY, container.attachedParent, container.model, container.modelIndex);
        }
